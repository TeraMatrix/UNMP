...
class CcuCommonSetValidation(object):
    ...
    # return dic_result
        try:
                        ...
                        for i in result["result"]:
                            if result["result"][i] != 0:
                                ...
                                oid_list_table_field_value = sqlalche_obj.session.query(
                                    CcuOids.table_name, CcuOids.coloumn_name).filter(and_(CcuOids.oid_name == i, CcuOids.device_type_id == device_type_id)).all()
            ...
            if len(dependent_oid) > 0:
                for i in range(0, len(dependent_oid)):
                    query_admin_result = sqlalche_obj.session.query(CcuOids.oid, CcuOids.oid_type, CcuOids.indexes).filter(
                        CcuOids.oid_name == "ru.ra.raConfTable.raAdminState").one()
                    if 'ru.ruConfTable.adminstate-1' in dependent_oid[i]:
                        admin_state = "ru.ruConfTable.adminstate"
                        query_admin_result = sqlalche_obj.session.query(
                            CcuOids.oid, CcuOids.oid_type, CcuOids.indexes).filter(CcuOids.oid_name == "ru.ruConfTable.adminstate").one()
                        ...
                        query_admin_result = sqlalche_obj.session.query(
                            CcuOids.oid, CcuOids.oid_type, CcuOids.indexes).filter(CcuOids.oid_name == "ru.ipConfigTable.adminState").one()
                        ...
                        query_admin_result = sqlalche_obj.session.query(
                            CcuOids.oid, CcuOids.oid_type, CcuOids.indexes).filter(CcuOids.oid_name == "ru.ra.raConfTable.raAdminState").one()
                ...
                for keys in dic_result.iterkeys():
                    if keys == "success":
                        ...
                        oid_list_min_max_value = sqlalche_obj.session.query(
                            CcuOids.min_value, CcuOids.max_value).filter(and_(CcuOids.oid_name == keys, CcuOids.device_type_id == device_type_id)).all()
    ...
    def default_reconciliation_controller(self, host_id, device_type_id, table_prefix, current_time, created_by, is_reconciliation):
        ...
        if len(panel_data) > 0:
            for i in range(0, len(panel_data)):
                ccu_panel_data_add = CcuBatteryPanelConfigTable(new_profile_id, panel_data[i].ccuBPCIndex, panel_data[i].ccuBPCSiteBatteryCapacity, panel_data[i]
        ...
        if len(ccu_threshold_data) > 0:
            for i in range(0, len(ccu_threshold_data)):
                ccu_threshold_data_add = CcuAlarmAndThresholdTable(new_profile_id, ccu_threshold_data[i].ccuATIndex, ccu_threshold_data[i].ccuATHighTemperatureAlarm, ccu_threshold_data[i].ccuATPSMRequest, ccu_threshold_data[i]
                                                                   .ccuATSMPSMaxCurrentLimit, ccu_threshold_data[i].ccuATPeakLoadCurrent, ccu_threshold_data[i].ccuATLowVoltageDisconnectLevel)
                                ...
                                if len(table_result) > 0:
                                    ...
                                    if len(rec_table_dic) > 0:
                                        ...
                                        for j in rec_table_dic:
                                            ...
                                            # time.sleep(5)
                                            result = ccu_bulk(str(rec_table_dic[j]) + str(".1"), host_data[0].ip_address, int(
                                                host_data[0].snmp_port), host_data[0].snmp_read_community, int(table_var_bind.get(j)))
                                            if result['success'] == 1:
...
# "%(tablename,host_data[0].host_id))
                                                            ...
                                                            for row in result['result']:
                                                                if len(sqlalche_table_result) >= row:
                                                                    for val in range(0, len(result['result'][row])):
                                                                        setattr(
                                                                            sqlalche_table_result[row - 1], column_list[val], str(result['result'][row][val]))
                                                                        if timestamp == 1:
                                        ...
                                        sqlalche_obj.db.execute(
                                            "Update ccu_oid_table set status = 0 where table_name in ('%s')" % ("\',\'".join(rec_not_done)))
                                        a = str(current_time)
                                        sqlalche_obj.db.execute("Update ccu_oid_table set status = 1,timestamp='%s' where table_name in ('%s')" %
