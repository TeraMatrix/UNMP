...
class CCUDeviceList(object):
    ...
    def ccu_device_list(self,ip_address,mac_address,selected_device,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,userid=None,html_req={}): 
        """
                    ...
                    if result["success"] == 0 or result["success"] == '0':                    
                        ...
                        for i in result["result"]:
                            if result["result"][i] != 0:
                                ...
                                oid_list_table_field_value = sqlalche_obj.session.query(CcuOids.table_name,CcuOids.coloumn_name).filter(and_(CcuOids.oid_name == i,CcuOids.device_type_id == device_type_id)).all()
                                if len(oid_list_table_field_value) == 0:
                                    continue
                                else:

                                    if i in dic_result:
                                        table_name = "ccu_" + oid_list_table_field_value[0][0]
                        ...
                        for i in result["result"]:
                            ...
                            success_result["result"] = errorStatus.get(i,'unknown SNMP Error')

            if len(dependent_oid)>0:  
                for i in range(0,len(dependent_oid)):               
                    query_admin_result = sqlalche_obj.session.query(CcuOids.oid,CcuOids.oid_type,CcuOids.indexes).filter(CcuOids.oid_name == "ru.ra.raConfTable.raAdminState").one()                            
                    if dependent_oid[i].has_key('ru.ruConfTable.adminstate-1'):                    
                        admin_state = "ru.ruConfTable.adminstate"
                        query_admin_result = sqlalche_obj.session.query(CcuOids.oid,CcuOids.oid_type,CcuOids.indexes).filter(CcuOids.oid_name == "ru.ruConfTable.adminstate").one()
                        ...
                        if len(query_admin_result) > 0:                       
                            ...
                            if result["success"] == 0 or result["success"] == '0':
                                ...
                                for i in result["result"]:
                                    ...
                                    success_result["result"] = errorStatus.get(result["result"][i],'Unknown SNMP Error')

                    elif dependent_oid[i].has_key('ru.ipConfigTable.adminState-1'):                  
                        query_admin_result = sqlalche_obj.session.query(CcuOids.oid,CcuOids.oid_type,CcuOids.indexes).filter(CcuOids.oid_name == "ru.ipConfigTable.adminState").one()
                                    ...
                                    if result["result"][i] != 0:
                                        ...
                                        if len(oid_list_table_field_value) == 0:
                                            ...
                                            if i in dic_result:
                                                ...
                                                table_name = "_" + oid_list_table_field_value[0][0]
                                                table_name = rename_tablename(table_name)
                                                exec "table_result = sqlalche_obj.session.query(%s).filter(%s.config_profile_id == \"%s\").all()"%(table_name,table_name,device_param_list[3])
                                                exec "table_result[0].%s = '%s'"%(oid_list_table_field_value[0][1],dic_result[i])                                       
            ...
            if(j == -1): 
                if len(depend_oid_value)>0:  
                    for i in range(0,len(depend_oid_value)):                    
                        ...
                        result = pysnmp_seter(depend_oid_value[i],device_param_list[0],device_param_list[1],device_param_list[2])      
                        print result             
                        if result["success"] == 0 or result["success"] == '0':
                            for i in result["result"]:
                                if result["result"][i] != 0:
                                    ...
                                    if len(oid_list_table_field_value) == 0:
                                        ...
                                        if i in dic_result:
                                            sql_table_name = "_" + oid_list_table_field_value[0][0]

                                            tableName= "ccu_"+sql_table_name+"_id" 
            ...
            if dic_result["success"] == '0' or dic_result["success"] == 0:
                for keys in dic_result.iterkeys():
                    if keys == "success":
                        ...
                        oid_list_min_max_value = sqlalche_obj.session.query(CcuOids.min_value,CcuOids.max_value).filter(and_(CcuOids.oid_name == keys,CcuOids.device_type_id == device_type_id)).all()
...
class CCUReconcilation(object):
    def default_reconciliation_controller(self,host_id,device_type_id,table_prefix,current_time,created_by,is_reconciliation):
        global sqlalche_obj
        sqlalche_obj.sql_alchemy_db_connection_open()
        new_profile = Odu16ConfigProfiles(device_type_id,"CCU","Master",None,datetime.now(),None,datetime.now(),None,0) # pass the parameter.
        ...
        print default_id
        panel_data = sqlalche_obj.session.query(CcuBatteryPanelConfigTable).filter(CcuBatteryPanelConfigTable.config_profile_id ==default_id).all()
        if len(panel_data)>0:
            for i in range(0,len(panel_data)):
                ccu_panel_data_add = CcuBatteryPanelConfigTable(new_profile_id,panel_data[i].ccuBPCIndex,panel_data[i].ccuBPCSiteBatteryCapacity,panel_data[i].ccuBPCSiteSolarPanelwP,panel_data[i].ccuBPCSiteSolarPanelCount,datetime.now())
                sqlalche_obj.session.add(ccu_panel_data_add)

        ccu_threshold_data = sqlalche_obj.session.query(CcuAlarmAndThresholdTable).filter(CcuAlarmAndThresholdTable.config_profile_id == default_id).all()
        if len(ccu_threshold_data)>0:
            for i in range(0,len(ccu_threshold_data)):
                ccu_threshold_data_add = CcuAlarmAndThresholdTable(new_profile_id,ccu_threshold_data[i].ccuATIndex,ccu_threshold_data[i].ccuATHighTemperatureAlarm,ccu_threshold_data[i].ccuATPSMRequest,ccu_threshold_data[i].ccuATSMPSMaxCurrentLimit,ccu_threshold_data[i].ccuATPeakLoadCurrent,ccu_threshold_data[i].ccuATLowVoltageDisconnectLevel)
                sqlalche_obj.session.add(ccu_threshold_data_add)

        ccu_aux_data = sqlalche_obj.session.query(CcuAuxIOTable).filter(CcuAuxIOTable.config_profile_id == default_id).all()
        if len(ccu_aux_data)>0:
            for i in range(0,len(ccu_aux_data)):
                ccu_aux_data_add = CcuAuxIOTable(new_profile_id,ccu_aux_data[i].ccuAIIndex,ccu_aux_data[i].ccuAIExternalOutput1,\
                                                 ccu_aux_data[i].ccuAIExternalOutput2,ccu_aux_data[i].ccuAIExternalOutput3,ccu_aux_data[i].ccuAIExternalInput1,\
                                                 ccu_aux_data[i].ccuAIExternalInput2,ccu_aux_data[i].ccuAIExternalInput3,ccu_aux_data[i].ccuAIExternalInput1AlarmType,\
        ...
        ccu_peer_data = sqlalche_obj.session.query(CcuPeerInformationTable).filter(CcuPeerInformationTable.config_profile_id == default_id).all()

        if len(ccu_peer_data)>0:
            for i in range(0,len(ccu_peer_data)):
                ccu_peer_data_add = CcuPeerInformationTable(new_profile_id,ccu_peer_data[i].ccuPIIndex,ccu_peer_data[i].ccuPIPeer1MACID,\
                                                            ccu_peer_data[i].ccuPIPeer2MACID,ccu_peer_data[i].ccuPIPeer3MACID,\
                                                            ccu_peer_data[i].ccuPIPeer4MACID)
                sqlalche_obj.session.add(ccu_peer_data_add)
        ccu_site_data = sqlalche_obj.session.query(CcuSiteInformationTable).filter(CcuSiteInformationTable.config_profile_id == default_id).all()

        if len(ccu_site_data)>0:
            for i in range(0,len(ccu_site_data)):
                ccu_site_data_add = CcuSiteInformationTable(new_profile_id,ccu_site_data[i].ccuSITIndex,ccu_site_data[i].ccuSITSiteName)
                sqlalche_obj.session.add(ccu_site_data_add)

        ccu_control_data = sqlalche_obj.session.query(CcuControlTable).filter(CcuControlTable.config_profile_id == default_id).all()
        if len(ccu_control_data)>0:
            for i in range(0,len(ccu_site_data)):
                ccu_control_data_add = CcuControlTable(new_profile_id,ccu_control_data[i].ccuCTIndex,\
                                                       ccu_control_data[i].ccuCTLoadTurnOff,ccu_control_data[i].ccuCTSMPSCharging,ccu_control_data[i].ccuCTRestoreDefault,\
        ...
        ccu_network_data_add = CcuNetworkConfigurationTable(host_id,0,"00:00:00:00:00:00","192.168.1.2","255.255.255.0","0.0.0.0","","255.255.255.0","")
        sqlalche_obj.session.add(ccu_network_data_add)

        ccu_software_data_add = CcuSoftwareInformationTable(host_id,0,"000.000.000","000.000.000","000.000.000","000.000.000")
    ...
    #######################  Device Reconciliation with isReconciliation reconciliation for new device #########################
                    ...
                    if int(host_op_status)==0:
                        ...
                        if len(host_data)>0:
                            ...
                            if int(ping_chk)==0:
                                ...
                                sqlalche_obj.session.commit()
                                if device_type=="ccu":
                                    table_result = sqlalche_obj.session.query(CcuOidTable).filter(CcuOidTable.is_recon==1).all()
                                if len(table_result)>0:
                                    ...
                                    if len(rec_table_dic)>0:
                                        ...
                                        for j in rec_table_dic:
                                            ...
                                            #print tablename,"\n\n\n"
                                            sqlalche_tablename = rename_tablename("_"+j) 
                                            #print sqlalche_tablename,"\n\n\n\n"
                                            ...
                                            #time.sleep(5)
                                            result = ccu_bulk(str(rec_table_dic[j])+str(".1"),host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community,int(table_var_bind.get(j)))                                            
                                            if result['success']==1:
...
##                                                        sqlalche_obj.db.execute("delete from %s where host_id = '%s' "%(tablename,host_data[0].host_id))
                                        ...
                                        sqlalche_obj.db.execute("Update ccu_oid_table set status = 0 where table_name in ('%s')"%("\',\'".join(rec_not_done)))
                                        a = str(current_time)
                                        sqlalche_obj.db.execute("Update ccu_oid_table set status = 1,timestamp='%s' where table_name in ('%s')"%(a[:a.find('.')-1],"\',\'".join(rec_done)))
...
#print ccu_obj.common_validation('7','ccu',{'ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteSolarPanelwP': '123', 'ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteSolarPanelCount': '187', 'ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteBatteryCapacity': '123', 'success': 0})
