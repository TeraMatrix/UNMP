    ...
    try:
        ...
        if ip_address == None and port == None and community == None:
            ...
            if ip_address != None and port != None and community != None:
                response_dict = {}
                temp_dict = {}
                prev_str = "errorIndication, errorStatus, errorIndex, varBinds = cmdgen.CommandGenerator().setCmd(cmdgen.CommunityData('unmp_set_agent', '%s'),cmdgen.UdpTransportTarget(('%s', %s))"%(community,ip_address,port)
                state_present = 0
                set_str = prev_str
