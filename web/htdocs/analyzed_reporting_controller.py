...
#    device_type_user_selected_name="UBR"
    ...
    js_list = ["js/calendrical.js","js/jquery-ui-1.8.6.custom.min.js","js/pages/jquery.multiselect.min.js","js/pages/jquery.multiselect.filter.js","js/jquery.dataTables.min.js","js/pages/analyzed_reporting.js"]#,
    if(device_type_user_selected_name==None):
        html.new_header("Analyzed Report","analyzed_report.py","",css_list,js_list)
    else:
        str_header="analyzed_report.py?device_type_user_selected_id=%s&device_type_user_selected_name=%s"%(device_type_user_selected_id,device_type_user_selected_name)
        html.new_header(device_type_user_selected_name +" Report",str_header,"",css_list,js_list)    
    r=AnalyzedReportBll()
...
def analyzed_reporting_get_host_data(h):
    ...
    column_result=r.analyzed_reporting_get_column_template(device_type_id,report_type)
    selected_columns=[]
...
def analyzed_reporting_get_column_range(h):
    ...
    r=AnalyzedReportBll()
    column_result=r.analyzed_reporting_get_column_range(device_type_id,report_type,range_type)
    selected_columns=[]
    non_selected_columns=[]
    if (column_result["success"]==0 or column_result["success"]=="0"):
	selected_columns=column_result["result"][0].split(",")
	...
	res={'html':Report.get_columns(selected_columns,non_selected_columns)}
	html.write(str(res))
    else:
...
#    no_of_devices=str(html.var("no_of_devices"))
    ...
    #	range_type="Range"	
    result=r.analyzed_reporting_get_excel(date_start,date_end,time_start,time_end,all_host,report_type,column_value,device_type,hostgroup,view_type,range_type,range_duration,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,html_var,username)
