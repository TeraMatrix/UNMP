...
def idu4_dashboard_profiling(h):
    ...
    javascript_list = ["js/highcharts.js","js/idu4Dashboard.js","calendrical/calendrical.js"]
    html.new_header("IDU4 Dashboard","","",css_list,javascript_list)
    html.write('<div class=\"form-div\">')
    host_id = ""
    host_id = html.var("host_id")
    extr_button={'tag':'button','id':'adSrhIDU4','value':'Advance Graph','name':'adSrhIDU4'}
    ...
    if device_list_param == [] or device_list_param == None:
        ...
        if  int(output)==1:
            html.write(page_header_search("","","IDU4Port",None,"enabled","device_type",extr_button))
            #html.write(page_header_search("","","UBR,UBRe",device_type,"enabled","device_type",extr_button))
        else:
            html.write(page_header_search(host_id,mac_address,"IDU4Port",device_type,"enabled","device_type",extr_button))
    else:
        html.write(page_header_search(device_list_param[0][0],device_list_param[0][1],"IDU4Port",device_list_param[0][2],device_list_state,"device_type",extr_button))
    if host_id == "" or host_id == "None":
        val = ""
        flag=1
        html.write("<div id=\"idu4_show_msg\"></div><div id=\"tab_yo\">There is no profile selected</div>")
    else:
        flag=0
        html.write("<div id=\"idu4_show_msg\"></div><div id=\"tab_yo\" style=\"display:block;overflow:hidden\">")
        idu4_dashboard(h)
        html.write("</div>")
...
def idu4_dashboard(h):
    ...
    if int(result['success'])==0:
        html.write(idu4View.idu4_table(result['output'],odu_start_date,odu_start_time,odu_end_date,odu_end_time,str(idu_refresh_time),str(total_count)))
    else:
...
def idu4_network_interface_graph(h):
    ...
    idu_bll_obj=IDUDashboard()
    output_result=idu_bll_obj.idu4_interface(odu_start_date,odu_start_time,odu_end_date,odu_end_time,interface_value,ip_address,limitFlag)
...
def idu4_tdmoip_network_interface_graph(h):
    ...
    limitFlag=html.var("limitFlag")
    idu_bll_obj=IDUDashboard()
    output_result=idu_bll_obj.idu4_tdmoip(odu_start_date,odu_start_time,odu_end_date,odu_end_time,interface_value,ip_address,limitFlag)
    html.write(str(output_result))
...
def idu4_port_statistics_graph(h):
    ...
    limitFlag=html.var("limitFlag")
    idu_bll_obj=IDUDashboard()
    output_result=idu_bll_obj.idu4_port_statistics(odu_start_date,odu_start_time,odu_end_date,odu_end_time,port,ip_address,limitFlag)
    html.write(str(output_result))
...
def idu4_device_details(h):
    ...
    if len(output['result'])>0:
        view_output=idu4View.device_information_view(output['result'],ip_address)
    else:
...
def idu4_alarm_event_table(h):
    ...
    if output_result['success']==1:
        ...
        html.write(str(idu4View.idu4_event_view(output_result['output'],table_option,ip_address)))
...
def idu4_pdf_generating(h):
    ...
    limitFlag=html.var("limitFlag")
    idu_bll_obj=IDUDashboard()
    output=idu_bll_obj.idu4_pdf_report(ip_address,odu_start_date,odu_start_time,odu_end_date,odu_end_time,select_option,limitFlag)
...
def idu4_excel_generating(h):
    ...
    limitFlag=html.var("limitFlag")
    idu_bll_obj=IDUDashboard()
    output=idu_bll_obj.idu4_excel_report(ip_address,odu_start_date,odu_start_time,odu_end_date,odu_end_time,select_option,limitFlag)
    html.write(str(output))
...
def idu4_link_status_table(h):
    ...
    limitFlag=html.var("limitFlag")
    idu_bll_obj=IDUDashboard()
    link_status_result=idu_bll_obj.idu4_link_status(odu_start_date,odu_start_time,odu_end_date,odu_end_time,interface_value,ip_address,limitFlag)
    link_statistics_result=idu_bll_obj.idu4_linkstatistcis(odu_start_date,odu_start_time,odu_end_date,odu_end_time,interface_value,ip_address,limitFlag)
    if int(link_statistics_result['success'])==1 or int(link_status_result['success'])==1:
        output_result={'success':1,'link_statistics_result':link_statistics_result,'link_status_result':link_status_result}
    else:
        link_status_view_output=idu4View.idu4_link_status_view(link_status_result['output'],1)
        output_result={'success':0,'graph_result':link_statistics_result['result'],'table_result':link_status_view_output['output']}
    html.write(str(output_result))
...
def idu4_e1_port_status_table(h):
    ...
    limitFlag=html.var("limitFlag")
    idu_bll_obj=IDUDashboard()
    output=idu_bll_obj.idu4_e1_port_status(odu_start_date,odu_start_time,odu_end_date,odu_end_time,port_num,ip_address,limitFlag)
    if int(output['success'])==1:
        link_statistics_output=idu4View.idu4_e1_port_statistics_view(output['output'],1)
        output_result={'success':1,'output':output['output']}
        html.write(str(output_result))
    else:
        link_statistics_output=idu4View.idu4_e1_port_statistics_view(output['output'],0)
        output_result={'success':0,'e1_port_graph':output['bpv'],'e1_port_table':link_statistics_output['link_table'],'time_stamp':output['timestamp']}
        html.write(str(output_result))
