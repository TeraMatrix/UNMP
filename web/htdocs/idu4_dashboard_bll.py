...
#                    ip_address=''
    ...
    def get_dashboard_data(self):
        ...
        get_data=DashboardConfig.get_config_attributes('idu4_dashboard',devcie_type_attr)
    ...
    def idu4_interface(self,odu_start_date,odu_start_time,odu_end_date,odu_end_time,interface_value,ip_address,limitFlag):
        ...
        try:
            ...
            output_dict={'success':0,'interface_rx':rx0,'interface_tx':tx0,'time_stamp0':time_stamp0}
            return output_dict
        # Exception Handling
        except MySQLdb as e:
            output_dict={'success':3,'output':str(e[-1])}
    ...
    def idu4_tdmoip(self,odu_start_date,odu_start_time,odu_end_date,odu_end_time,interface_value,ip_address,limitFlag):
        ...
        try:
            ...
            output_dict={'success':0,'transmit':transmit,'receive':receive,'time_stamp':time_stamp}
            return output_dict
        # Exception Handling
        except MySQLdb as e:
            output_dict={'success':3,'output':str(e[-1])}
    ...
    def idu4_linkstatistcis(self,odu_start_date,odu_start_time,odu_end_date,odu_end_time,link_num,ip_address,limitFlag):
        ...
        try:
            ...
            linkstatistcis={'success':0,'result':[good_frm_eth0,good_frm_rx,lost_pct_rx,discard_pct,record_pct,event,time_stamp]}
    ...
    def idu4_get_link_name(self):
        ...
        try:
            ...
            for link in link_result:
                link_value.append(0 if link[0]=='' or link[0]==None else int(link[0]))
                link_name.append('Link '+str(link[0]))
            output_dic={'success':0,'link_name':link_name,'link_value':link_value}
            return output_dic
        # Exception Handling
        except MySQLdb as e:
    ...
    def idu4_e1_port_status(self,odu_start_date,odu_start_time,odu_end_date,odu_end_time,port_num,ip_address,limitFlag):
        ...
        try:
            ...
            for i in range(len(result)-1):
                ...
                time_stamp.append(result[i][0].strftime('%H:%M'))
            output_dict={'success':0,'output':result,'bpv':bpv,'timestamp':time_stamp}
            return output_dict
        # Exception Handling
        except MySQLdb as e:
            output_dict={'success':3,'output':str(e[-1])}
    ...
    def idu4_device_information(self,ip_address):
        ...
        try:
            ...
            output_dict={'success':0,'result':result,'last_reboot_time':last_reboot_time}
            return output_dict 
        # Exception Handling
        except MySQLdb as e:
            output_dict={'success':3,'output':str(e[-1])}
    ...
    def idu4_event(self,ip_address):
        ...
        try:
            ...
            major.reverse()
            critical.reverse()
            output_dict={'success':0,'output':{'graph':[normal,inforamtional,minor,major,critical],'timestamp':time_stamp}}
            return output_dict 
        # Exception Handling
        except MySQLdb as e:
            output_dict={'success':3,'output':str(e[-1])}
    ...
    def idu4_excel_report(self,ip_address,odu_start_date,odu_start_time,odu_end_date,odu_end_time,select_option,limitFlag):
        ...
        try:
            ...
            for value in device_detail:
                table_output.append(['H/W Serial Number',str('--' if device_detail[0][0]==None or device_detail[0][0]==""  else device_detail[0][0])])
                table_output.append(['System MAC',str('--' if device_detail[0][1]==None or device_detail[0][1]==""  else device_detail[0][1])])
                table_output.append(['TDMOIP Interface MAC',str('--' if device_detail[0][2]==None or device_detail[0][2]==""  else device_detail[0][2])])
                table_output.append(['Active Version',str('--' if device_detail[0][3]==None or device_detail[0][3]==""  else device_detail[0][3])])
                table_output.append(['Passive Version',str('--' if device_detail[0][4]==None or device_detail[0][4]==""  else device_detail[0][4])])
                table_output.append(['BootLoader Version ',str('--' if device_detail[0][5]==None or device_detail[0][5]==""  else device_detail[0][5])])
                table_output.append(['Temperature(C)',str('--' if device_detail[0][6]==None or device_detail[0][6]==""  else device_detail[0][6])])
                table_output.append(['UpTime',str(str(hour)+"Hr "+str(minute)+"Min "+str(second)+"Sec")])
            sql="SELECT trap_receive_date from trap_alarms where trap_event_type = 'NODE_UP' and agent_id='%s' order by timestamp desc limit 1"%ip_address
            cursor.execute(sql)
    ...
    #        heading=[table_output[0][1],table_output[0][2],table_output[0][3],table_output[0][4],table_output[0][5],table_output[0][6],table_output[0][7]]
            ...
            for interface_value in range(len(interface)):
                ...
                for i in range(0,len(nw_result)-1):
                    temp_list=[]
                    temp_list=[nw_result[i][2].strftime('%d-%m-%Y %H:%M'),0 if (int(nw_result[i][1])-int(nw_result[i+1][0]))<0 else (int(nw_result[i][0])-int(nw_result[i+1][0])),0 if (int(nw_result[i][1])-int(nw_result[i+1][1]))<0 else (int(nw_result[i][1])-int(nw_result[i+1][1]))]
                    network_list.append(temp_list)
                network_list=sorted(network_list, key=itemgetter(0))
                xls_sheet=xls_book.add_sheet('network_bandwidth(%s)'%interface[interface_value],cell_overwrite_ok=True)
                xls_sheet.row(0).height = 521
                xls_sheet.row(1).height = 421
                xls_sheet.write_merge(0,0,0,2,"Network Bandwidth Information (%s)"%interface[interface_value],style)
            ...
            for interface_value in range(len(interface)):
                if int(select_option)==0:
                    if int(limitFlag)==0:
                        ...
                        limit_data=' limit 16'
                    sel_query="select IFNULL((idu.bytesTransmitted),0),IFNULL((idu.bytesReceived),0),idu.timestamp from  idu_tdmoipNetworkInterfaceStatisticsTable as idu INNER JOIN hosts as h  on  idu.host_id = h.host_id  where h.ip_address='%s' AND idu.timestamp <='%s' AND idu.timestamp >= '%s' AND indexid=%s order by idu.timestamp"%(ip_address,end_time,start_time,interface_value)
                    sel_query+=limit_data
                else:
                ...
                for i in range(0,len(nw_result)-1):
                    ...
                    network_list.append(temp_list)
                network_list=sorted(network_list, key=itemgetter(0))
                xls_sheet=xls_book.add_sheet('TDMOIP_network_bandwidth(%s)'%interface[interface_value],cell_overwrite_ok=True)
                xls_sheet.row(0).height = 521
                xls_sheet.row(1).height = 421
                xls_sheet.write_merge(0,0,0,2,"TDMOIP Network Bandwidth Information (%s)"%interface[interface_value],style)
                xls_sheet.write(1,0,device_type,style)
                xls_sheet.write(1,1,device_name,style)
                xls_sheet.write(1,2,ip_address,style)
                xls_sheet.write_merge(2,2,0,2,"")
        ...
        # TDMOIP Network bandwith Excel ending here.
            ...
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
            xls_sheet.write_merge(0,0,0,5," %s Days Event Information"%total_days,style)
    ...
    #            sql="SELECT ta.serevity,ta.trap_event_id,ta.trap_event_type,ta.trap_receive_date FROM trap_alarm_current as ta WHERE ta.agent_id='%s' AND date(ta.timestamp)=current_date()  order by ta.timestamp "%(ip_address)
            ...
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
            xls_sheet.write_merge(0,0,0,3,"%s Days Alarm Information"%(int(trap_days)+1),style)
            ...
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
            xls_sheet.write_merge(0,0,0,3,"%s Days Event Information"%(int(trap_days)+1),style)
            xls_sheet.write_merge(1,1,0,3,str(device_type)+'       '+str(device_name)+ '         '+str(ip_address) ,style)
            xls_sheet.write_merge(2,2,0,3,"")
            i=4
            heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
            ...
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
            xls_sheet.write_merge(0,0,0,2,"%s Days Outage Information"%total_days,style)
            xls_sheet.write_merge(1,1,0,2,str(device_type)+'       '+str(device_name)+ '         '+str(ip_address) ,style)
            xls_sheet.write_merge(2,2,0,2,"")
            i=4
            heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
            ...
            xls_book.save('/omd/sites/%s/share/check_mk/web/htdocs/download/IDU4_excel.xls' % nms_instance)
...
#            pdfdoc="/omd/sites/%s/share/check_mk/web/htdocs/download/IDU4_PDF_Report.pdf"%(nms_instance,start_time,end_time)
            pdfdoc="/omd/sites/%s/share/check_mk/web/htdocs/download/IDU4_PDF_Report.pdf"%(nms_instance)
            ...
            data=[]
            data.append(['IDU4 '+str(ip_address),str(start_time)+'--'+str(end_time)])
            ...
            t = Table(data1, [.021*inch,1.24 * inch, 3.14 * inch, 2.57*inch])
            t.setStyle(TableStyle([('BACKGROUND',(1,0),(1,0),(0.35,0.35,0.35)),('FONT', (0, 0), (1, 0), 'Helvetica', 11),('TEXTCOLOR',(1,0),(2,0),colors.white)]))
            idu4_report.append(t)
            ...
            table_output=[]
            k=0
            table_output.append(['DateTime','Informational','Normal','Minor','Major','Critical'])
            for time in time_stamp:
                table_output.append([str(time_stamp[k]),int(inforamtional[k]),int(normal[k]),int(minor[k]),int(major[k]),int(critical[k])])
                k+=1
            ...
            if total_days ==0:
                ...
                t = Table(data1, [.021*inch,2.41 * inch, 2.44 * inch, 2.1*inch])
            else:
                data1.append(['','Last '+str(total_days+1)+' Days Events Information','',''])
                t = Table(data1, [.021*inch,2.21 * inch, 2.64 * inch, 2.1*inch])
            t.setStyle(TableStyle([('BACKGROUND',(1,0),(1,0),(0.35,0.35,0.35)),('FONT', (0, 0), (1, 0), 'Helvetica', 11),('TEXTCOLOR',(1,0),(2,0),colors.white)]))
            idu4_report.append(t)
            data=table_output
            ...
            for date_time1 in date_days:
                table_output.append([date_time1,round(up_state[i],2),round(down_state[i],2)])
                i+=1
            data=table_output
            ...
            for interface in nw_interface:
                ...
                #result1=cursor.fetchall()
                if int(select_option)==0:
                    sel_query="select (idu.rxBytes),(idu.txBytes),idu.timestamp from idu_iduNetworkStatisticsTable as idu INNER JOIN hosts as h  on  idu.host_id = h.host_id  where  h.ip_address='%s' AND  idu.timestamp >= '%s' AND idu.timestamp <='%s' and idu.interfaceName = %s order by idu.timestamp desc"%(ip_address,start_time,end_time,interface_index)
                    if int(limitFlag)==0:
                        limit_data=''
                    else:
                        limit_data=' limit 16'
                    ...
                    sel_query="select (idu.rxBytes),(idu.txBytes),idu.timestamp from idu_iduNetworkStatisticsTable as idu INNER JOIN hosts as h  on  idu.host_id = h.host_id  where  h.ip_address='%s' AND date(idu.timestamp) <=current_date() AND date(idu.timestamp) >= current_date()-%s AND idu.interfaceName = %s order by idu.timestamp desc"%(ip_address,total_days,interface_index)
                cursor.execute(sel_query)
                result1=cursor.fetchall()
                table_output=bll_obj.nw_table_list_creation(result1,interface,'Receving(Kbps)','Transmitting(Kbps)','Time(HH:MM:SS)',)
                # close the database and cursor connection.
                cursor.close()
                data1=[]
                data1.append(['',interface,'',''])
                t = Table(data1, [.021*inch,2.62 * inch, 2.15 * inch, 2.22*inch])
                t.setStyle(TableStyle([('BACKGROUND',(1,0),(1,0),(0.35,0.35,0.35)),('FONT', (0, 0), (1, 0), 'Helvetica', 11),('TEXTCOLOR',(1,0),(2,0),colors.white)]))
                idu4_report.append(t)
            ...
            for interface in nw_interface:
                ...
                #result1=cursor.fetchall()
                if int(select_option)==0:
                    sel_query="select IFNULL((idu.bytesTransmitted),0),IFNULL((idu.bytesReceived),0),idu.timestamp from  idu_tdmoipNetworkInterfaceStatisticsTable as idu INNER JOIN hosts as h  on  idu.host_id = h.host_id  where h.ip_address='%s' AND idu.timestamp <='%s' AND idu.timestamp >= '%s' AND indexid=%s order by idu.timestamp"%(ip_address,end_time,start_time,interface_index)
                    if int(limitFlag)==0:
                        limit_data=''
                    else:
                        limit_data=' limit 16'
                    ...
                    sel_query="select IFNULL((idu.bytesTransmitted),0),IFNULL((idu.bytesReceived),0),idu.timestamp from  idu_tdmoipNetworkInterfaceStatisticsTable as idu INNER JOIN hosts as h  on  idu.host_id = h.host_id  where h.ip_address='%s' AND date(idu.timestamp) <=current_date() AND date(idu.timestamp) >= current_date()-%s AND indexid=%s order by idu.timestamp"%(ip_address,total_days,interface_index)
                cursor.execute(sel_query)
                result1=cursor.fetchall()
                table_output=bll_obj.nw_table_list_creation(result1,interface,'Transmitting(Kbps)','Receving(Kbps)','Time(HH:MM:SS)')
                # close the database and cursor connection.
                cursor.close()
                data1=[]
                data1.append(['',interface,'',''])
