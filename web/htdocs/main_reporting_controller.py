...
#    device_type_user_selected_name="UBR"
    css_list = ["css/demo_table_jui.css","css/calendrical.css","css/fcbkcomplete.css","css/style12.css","css/jquery.multiselect.css","css/jquery.multiselect.filter.css","css/jquery-ui-1.8.4.custom.css"]#,"css/jquery.multiselect.filter.css"]
    js_list = ["js/calendrical.js","js/jquery-ui-1.8.6.custom.min.js","js/pages/jquery.multiselect.min.js","js/pages/jquery.multiselect.filter.js","js/jquery.dataTables.min.js","js/pages/main_reporting.js"]#,
    ...
    if(device_type_user_selected_name==None):
        html.new_header("General Report","main_report.py","",css_list,js_list)#,snapin_list)
    else:
        str_header="main_report_%s.py?device_type_user_selected_id=%s&device_type_user_selected_name=%s"%(device_type_user_selected_id,device_type_user_selected_id,device_type_user_selected_name)
        html.new_header(device_type_user_selected_name +" Report",str_header,"",css_list,js_list)#,snapin_list)    
    r=MainReportBll()
    user_id=html.req.session["user_id"]
    es = Essential()
    hostgroup_id_list = es.get_hostgroup_ids(user_id)
    if(hostgroup_id_list==[]):
        html.write(Report.list_form([],[],device_type_user_selected_id,device_type_user_selected_name))
    else:
        result=r.get_hostgroup_device(user_id,hostgroup_id_list)
        if (result["success"]==0):
	    html.write(Report.list_form(result["result"],result["host_data"],device_type_user_selected_id,device_type_user_selected_name))
        else:
    	    pass
    html.new_footer()
...
def main_reporting_get_host_data(h):
    ...
    column_result=r.main_reporting_get_column_template(device_type_id,report_type)
    selected_columns=[]
	...
	res={'html':Report.get_columns(selected_columns,non_selected_columns),'result':str(result['result'])}
	html.write(str(res))
    else:
	pass
...
#    no_of_devices=str(html.var("no_of_devices"))
    ...
    ####
    result=r.main_reporting_get_excel(date_start,date_end,time_start,time_end,all_host,report_type,column_value,device_type,hostgroup,view_type,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,new_user_report_dict,username)
...
def trap_main_report(h):
    ...
    html = h
    user_report_dict={}
    css_list = ["css/demo_table_jui.css","css/calendrical.css","css/fcbkcomplete.css","css/style12.css","css/jquery.multiselect.css","css/jquery.multiselect.filter.css","css/jquery-ui-1.8.4.custom.css"]
    js_list = ["js/calendrical.js","js/jquery-ui-1.8.6.custom.min.js","js/pages/jquery.multiselect.min.js","js/pages/jquery.multiselect.filter.js","js/jquery.dataTables.min.js","js/pages/main_reporting_trap.js"]
    #snapin_list = ["reports","views","Alarm","Inventory","Settings","NetworkMaps","user_management","schedule","Listing"]
    html.new_header("Events Report","trapreport.py","",css_list,js_list)#,snapin_list)
    r=MainReportBll()
    user_id=html.req.session["user_id"]
    es = Essential()
    hostgroup_id_list = es.get_hostgroup_ids(user_id)
    if(hostgroup_id_list==[]):
        ...
        result=r.get_hostgroup_device(user_id,hostgroup_id_list)
        if (result["success"]==0):
	    html.write(Report.trap_list_form(result["result"],result["host_data"]))
        else:
    	    pass
    html.new_footer()
...
def trap_main_reporting_get_excel(h):
    ...
    result=r.trap_main_reporting_get_excel(date_start,date_end,time_start,time_end,all_host,report_type,device_type,hostgroup,view_type,i_display_start,
