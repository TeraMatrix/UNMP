...
engine = create_engine('%s://%s:%s@%s/%s' % SystemConfig.get_sqlalchemy_credentials(), echo=False,pool_size=12,max_overflow=2,pool_timeout=30)
...
class Oids(Base):
    ...
    def __repr__(self):
        return "<Oids('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')>" %(self.oid_id,self.device_type_id,self.oid,self.oid_name,self.oid_type,self.access,self.default_value,self.min_value,self.max_value,self.indexes,self.dependent_id,self.multivalue,self.table_name,self.coloumn_name,self.indexes_name)
...
class Odu100RaConfTable(Base):
    ...
    link_distance = Column(INTEGER)

    def __init__(self,config_profile_id,raIndex,raAdminState,aclMode,ssID,guaranteedBroadcastBW,dba,acm,acs,dfs,numSlaves,antennaPort,link_distance=0):
    ...
    def __repr__(self):
        return "<Odu100RaCOnfTable('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')>" %(self.odu100_raConfTable_id,self.config_profile_id,self.raIndex,self.raAdminState,self.aclMode,self.ssID,self.guaranteedBroadcastBW,self.dba,self.acm,self.acs,self.dfs,self.numSlaves,self.antennaPort,self.link_distance)
...
class IduIduInfoTable(Base):
	...
	hwType = Column(SMALLINT(6))
	hwConfigE1 = Column(SMALLINT(6))
	hwConfigEth = Column(SMALLINT(6))
	hwConfigAlarm = Column(SMALLINT(6))
...
class Ap25BasicACLsetup(Base):
    __tablename__= "ap25_basicACLsetup"
    ap25_basicACLsetup_id = Column(INTEGER,primary_key=True)
    config_profile_id = Column(INTEGER,ForeignKey('config_profiles.config_profile_id'))
    vapselection_id = Column(INTEGER,ForeignKey('ap25_vapSelection.ap25_vapSelection_id'))
    aclState = Column(SMALLINT)
    aclMode = Column(SMALLINT)

    def __init__(self,config_profile_id,vapselection_id,aclState,aclMode):
        self.ap25_basicACLsetup_id = None
        self.config_profile_id = config_profile_id
        self.vapselection_id = vapselection_id
    ...
    def __repr__(self):
        return "<Ap25BasicACLsetup('%s','%s','%s','%s','%s')>" %(self.ap25_basicACLsetup_id,self.config_profile_id,self.vapselection_id,self.aclState,self.aclMode)
...
class Ap25BasicVAPsetup(Base):
    __tablename__= "ap25_basicVAPsetup"
    ap25_basicVAPsetup_id = Column(INTEGER,primary_key=True)
    config_profile_id = Column(INTEGER,ForeignKey('config_profiles.config_profile_id'))
    vapselection_id = Column(INTEGER,ForeignKey('ap25_vapSelection.ap25_vapSelection_id'))
    vapESSID = Column(VARCHAR(22))
    ...
    def __init__(self,config_profile_id,vapselection_id,vapESSID,vapHiddenESSIDstate,vapRTSthresholdValue,vapFragmentationThresholdValue,vapBeaconInterval,vlanid=0,vlanpriority=0,vapmode=0):
        self.ap25_basicVAPsetup_id = None
        self.config_profile_id = config_profile_id
        self.vapselection_id = vapselection_id
    ...
    def __repr__(self):
        return "<Ap25BasicVAPsetup('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')>"\
     %(self.ap25_basicVAPsetup_id,self.config_profile_id,self.vapselection_id,self.vapESSID,self.vapHiddenESSIDstate,\
    self.vapRTSthresholdValue,self.vapFragmentationThresholdValue,self.vapBeaconInterval,self.vlanid,self.vlanpriority,self.vapmode)

class Ap25DhcpServer(Base):
    __tablename__= "ap25_dhcpServer"
    ap25_dhcpServer_id = Column(INTEGER,primary_key=True)
    config_profile_id = Column(INTEGER,ForeignKey('config_profiles.config_profile_id'))
    dhcpServerStatus = Column(TINYINT(4))
...
class Ap25RadioSetup(Base):
    ...
    def __init__(self,config_profile_id,radioState,radioAPmode,radioManagementVLANstate,countrycode,numberofVAPs,radioChannel,wifiMode,radioTxPower,radioGatingIndex,radioAggregation,radioAggFrames,radioAggSize,radioAggMinSize,radioChannelWidth,radioTXChainMask,radioRXChainMask):
...
class Ap25VapWEPsecuritySetup(Base):
    __tablename__= "ap25_vapWEPsecuritySetup"
    ap25_vapWEPsecuritySetup_id = Column(INTEGER,primary_key=True)
    config_profile_id = Column(INTEGER,ForeignKey('config_profiles.config_profile_id'))    
    ...
    def __init__(self,config_profile_id,vapWEPmode,vapWEPprimaryKey,vapWEPkey1,vapWEPkey2,vapWEPkey3,vapWEPkey4):
        self.ap25_vapWEPsecuritySetup_id = None
        self.config_profile_id = config_profile_id        
    ...
    def __repr__(self):
        ...
        %(self.ap25_vapWEPsecuritySetup_id,self.config_profile_id,self.vapWEPmode,self.vapWEPprimaryKey,self.vapWEPkey1,self.vapWEPkey2,self.vapWEPkey3,self.vapWEPkey4)

class Ap25VapWPAsecuritySetup(Base):
    __tablename__= "ap25_vapWPAsecuritySetup"
    ap25_vapWPAsecuritySetup_id = Column(INTEGER,primary_key=True)
    config_profile_id = Column(INTEGER,ForeignKey('config_profiles.config_profile_id'))
    vapselection_id = Column(INTEGER,ForeignKey('ap25_vapSelection.ap25_vapSelection_id'))
    ...
    def __init__(self,config_profile_id,vapselection_id,vapWPAmode,vapWPAcypher,vapWPArekeyInterval,vapWPAmasterReKey,vapWEPrekeyInt,vapWPAkeyMode,vapWPAconfigPSKPassPhrase,vapWPArsnPreAuth,vapWPArsnPreAuthInterface,vapWPAeapReAuthPeriod,vapWPAserverIP,vapWPAserverPort,vapWPAsharedSecret):
        self.ap25_vapWPAsecuritySetup_id = None
        self.config_profile_id = config_profile_id
        self.vapselection_id = vapselection_id
    ...
    def __repr__(self):
        return "<Ap25VapWPAsecuritySetup('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')>" %(self.ap25_vapWPAsecuritySetup_id,self.config_profile_id,self.vapselection_id,self.vapWPAmode,self.vapWPAcypher,self.vapWPArekeyInterval,self.vapWPAmasterReKey,self.vapWEPrekeyInt,self.vapWPAkeyMode,self.vapWPAconfigPSKPassPhrase,self.vapWPArsnPreAuth,self.vapWPArsnPreAuthInterface,self.vapWPAeapReAuthPeriod,self.vapWPAserverIP,self.vapWPAserverPort,self.vapWPAsharedSecret)
...
class Ap25OidTable(Base):
    __tablename__= "ap25_oid_table"
    table_name = Column(VARCHAR(64),primary_key=True)
    table_oid = Column(VARCHAR(64))
    isNode = Column(TINYINT)
    isVap = Column(TINYINT)

    def __init__(self,table_name,table_oid,isNode):
        self.table_name = None
        self.table_oid = table_oid
        self.isNode = isNode
        self.isVap = isVap
    def __repr__(self):
        return "<Ap25Oid_table('%s','%s','%s','%s')>" %(self.table_name,self.table_oid,self.isNode,self.isVap)
...
class Odu100Oid_table(Base):
	__tablename__= "odu100_oid_table"
	table_name = Column(VARCHAR(64),primary_key=True)
	table_oid = Column(VARCHAR(64))
	varbinds = Column(TINYINT)
	...
	def __repr__(self):
		return "<Odu100Oid_table('%s','%s','%s','%s','%s','%s')>" %(self.table_name,self.table_oid,self.varbinds,self.is_recon,self.status,self.timestamp)
...
mapper(Odu100Oid_table,Odu100Oid_table.__table__)
...
mapper(Ap25VapWEPsecuritySetup,Ap25VapWEPsecuritySetup.__table__)
mapper(Ap25VapWPAsecuritySetup,Ap25VapWPAsecuritySetup.__table__)
mapper(Ap25Versions,Ap25Versions.__table__)
mapper(Ap25OidTable,Ap25OidTable.__table__)
...
'ap25_basicVAPsetup':relationship(Ap25BasicVAPsetup,backref="ap25_vapSelection",cascade="all,delete,delete-orphan"),\
'ap25_vapWPAsecuritySetup':relationship(Ap25VapWPAsecuritySetup,backref="ap25_vapSelection",cascade="all,delete,delete-orphan")})
...
'ap25_basicACLsetup':relationship(Ap25BasicACLsetup,backref="config_profiles",cascade="all,delete,delete-orphan"),\
'ap25_basicVAPsecurity':relationship(Ap25BasicVAPsecurity,backref="config_profiles",cascade="all,delete,delete-orphan"),\
'ap25_basicVAPsetup':relationship(Ap25BasicVAPsetup,backref="config_profiles",cascade="all,delete,delete-orphan"),\
'ap25_vapWPAsecuritySetup':relationship(Ap25VapWPAsecuritySetup,backref="config_profiles",cascade="all,delete,delete-orphan"),\
'ap25_vapWEPsecuritySetup':relationship(Ap25VapWEPsecuritySetup,backref="config_profiles",cascade="all,delete,delete-orphan"),\
...
"oids":relationship(Oids,backref="device_type",cascade="all,delete,delete-orphan"),\
