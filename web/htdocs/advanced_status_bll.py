...
class AdvancedStatusBll(object):
    ...
    def total_graph_name_display(self,device_type_id,user_id):
        try:
            ...
            if db ==1:
                raise SelfException(cursor)
            sel_query="SELECT graph_display_name,graph_display_id FROM graph_templet_table WHERE device_type_id='%s' and user_id='%s' and dashboard_type=1"%(device_type_id,user_id)
            cursor.execute(sel_query)
...
#		    graph_json.append(graph_dict)
	...
	result=()
	table_name_dict={'odu100NWstatus':'odu100_nwInterfaceStatusTable','odu100syncstatus':'odu100_synchStatusTable',
	...
	default_value='Device Unreachable'
	    ...
	    column_name=[col[0].strip() for col in coloum_result]
            ...
            clm_dict=dict(zip([coloum_result[i][0].strip() for i in range(len(coloum_result))],[coloum_result[i][1] for i in range(len(coloum_result))]))
            table_field_dict1={graph_id:clm_dict}
            graph_field_dict=dict((field[0],field[2]) for field in coloum_result)

            sel_query="select graph_title,graph_subtitle from graph_templet_table where graph_display_id='%s' and user_id='%s' and device_type_id='%s' and dashboard_type=1"%(graph_id,user_id,device_type_id)
            cursor.execute(sel_query)
		...
		sel_query="select idu.portNum,idu.bundleNum,\
		 ...
		 from   idu_linkStatusTable as idu INNER JOIN hosts ON hosts.host_id=idu.host_id where hosts.ip_address='%s' and idu.timestamp >='%s' and idu.timestamp <='%s' order by idu.timestamp desc,idu.bundleNum asc "%(ip_address,start_date,end_date)
		cursor.execute(sel_query)
		result=cursor.fetchall()
		given=['bundleNum','portNum','operationalStatus', 'minJBLevel', 'maxJBLevel', 'underrunOccured', 'overrunOccured', 'timestamp']
		req.append('timestamp')
		req.append('portNum')
		...
		sel_query="select odu.currentTimeSlot,\
			...
			from   odu100_raStatusTable as odu INNER JOIN hosts ON hosts.host_id=odu.host_id where hosts.ip_address='%s' and odu.timestamp >='%s' and odu.timestamp <='%s' order by odu.timestamp desc "%(ip_address,start_date,end_date)
		cursor.execute(sel_query)
		result=cursor.fetchall()
		given=['currentTimeSlot', 'raMacAddress', 'raoperationalState', 'unusedTxTimeUL', 'unusedTxTimeDL', 'timestamp']
		req.append('timestamp')
		req.extend(column_name)
		td=manage_list(req,given,result)
		th.append('Timestamp')
		...
		sel_query="select (if(odu.syncoperationalState=0,'Disable',if(odu.syncoperationalState=1111111,odu.syncoperationalState,'Enable'))) as syncoperationalState,\
			...
			from   odu100_synchStatusTable as odu INNER JOIN hosts ON hosts.host_id=odu.host_id where hosts.ip_address='%s' and odu.timestamp >='%s' and odu.timestamp <='%s' order by odu.timestamp desc"%(ip_address,start_date,end_date)
		cursor.execute(sel_query)
		result=cursor.fetchall()
		given=['syncoperationalState', 'syncrasterTime', 'timerAdjust', 'syncpercentageDownlinkTransmitTime', 'timestamp']
		req.append('timestamp')
		req.extend(column_name)
		td=manage_list(req,given,result)
		th.append('Timestamp')
		...
		sel_query="select odu.nwStatusIndex,\
			...
			from    odu100_nwInterfaceStatusTable as odu INNER JOIN hosts ON hosts.host_id=odu.host_id where hosts.ip_address='%s' and odu.timestamp >='%s' and odu.timestamp <='%s' order by odu.timestamp desc,odu.nwInterfaceName asc "%(ip_address,start_date,end_date)
		cursor.execute(sel_query)
		result=cursor.fetchall()
		given=['nwStatusIndex', 'nwInterfaceName', 'operationalState', 'macAddress', 'timestamp']
		req.append('timestamp')
		req.extend(column_name)
		td=manage_list(req,given,result)
		th.append('Timestamp')
    ...
    def advaeced_excel_report(self,report_type,device_type_id,user_id,ip_address,start_date,end_date,graph_id,select_option):
        try:
            ...
            sel_query="select graph_display_id,graph_display_name from graph_templet_table where graph_display_id='%s' and user_id='%s' and device_type_id='%s' and dashboard_type=1"%(graph_id,user_id,device_type_id)
	    cursor.execute(sel_query)
	    graph_name_result=cursor.fetchall()
#            graph_name_dict=dict((field[0],field[2]) for field in graph_name_result)
	    graph_name=[field[1] for field in graph_name_result]
            headings=[]
            merge_result=[]
            output_result=self.advanced_graph_data(user_id,ip_address,start_date,end_date,graph_id,device_type_id)
...
'idu_e1PortStatusTable':{'1':'(port1)','2':'(port2)','3':'(port3)','4':'(port4)'},'idu_portstatisticsTable':idu4_port,'idu_swPrimaryPortStatisticsTable':idu4_port,'idu_portSecondaryStatisticsTable':idu4_port}
            ...
            if report_type=='excelReport':
                ...
                xls_sheet=xls_book.add_sheet('Device Status Sheet',cell_overwrite_ok=True)
                xls_sheet.row(0).height = 521
                xls_sheet.row(1).height = 421
                xls_sheet.write_merge(0,0,0,len(headings)-1,str(graph_name[0]),style)
                xls_sheet.write_merge(1,1,0,len(headings)-1,str(device_type)+'       '+str(ip_address)+str(device_postfix)+'     ('+str(start_date)+'-'+str(end_date)+')',style)
                ...
                second_row=[str(device_type),str(ip_address)+str(device_postfix)+'      ('+str(start_date)+'-'+str(end_date)+')']
    ...
    def ap_data_table(self,user_id,ip_address,start_date,end_date,graph_id,device_type):
	    ...
	    merge_result=[]
            ...
            if conn ==1:
                raise SelfException(cursor)
            output_result=self.advanced_graph_data(user_id,ip_address,start_date,end_date,graph_id,device_type)
	    if int(output_result['success'])==0:
