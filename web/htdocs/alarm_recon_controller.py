...
def recon_display(h):
    ...
    try:
            ...
            if es.is_host_allow(user_id,host_id) == 0:
                ...
                if bll_dict['success'] == 0:
                    ...
                    if host_uptime['success'] == 0:
                        bll_dict['result']['output'] = host_uptime['result'].get('output', '')
                        bll_dict['result']['state'] = host_uptime['result'].get('state', '')
                    else:
        ...
        html.write("<div class=\"error\" > %s </div>"%( " Encounterd an Error "))
        #import traceback
        #html.new_header("Alarm Reconciliation","recon_display.py","",[],["js/alarm_recon.js"])
        ...
        if host_ip:
            ...
            for site, state, description, output, all_output in services:
                ...
                if description != 'SNMP UPTIME':
                    continue                
                if int(state) == 0:
                    s = output+str('' if all_device_detail.strip()=='()' else all_device_detail)
                    find_str = "Host Uptime -"
                    if s.find(find_str) != -1:
                        result['output'] = s[s.find(find_str)+len(find_str):s.find(")")-1]
                if description == 'SNMP UPTIME':
                    break
                
            result['state'] = "<span class=\"span-status-container icon-%s\">%s</span>" % (state, temp.get(int(state), 3))
            success = 0
        else:
...
def start_alarm(h):
    ...
    if host_id:
        ...
        if es.is_host_allow(user_id,host_id) == 0:
            ...
            result_dict = {'success': 1, 'result': 'No such host exists : Access Restricted',}
    else:
    ...
    if host_id:
        ...
        if es.is_host_allow(user_id,host_id) == 0:
            ...
            if result_dict['success'] == 0:
                ...
                if host_uptime['success'] == 0:
                    result_dict['result']['output'] = host_uptime['result'].get('output', '')
                    result_dict['result']['state'] = host_uptime['result'].get('state', '')
                else:
