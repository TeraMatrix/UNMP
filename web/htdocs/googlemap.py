...
def save_updates(h):
	...
	try:
		...
		if len(host_asset_info)>0:
			...
			info_str = "Device %s location has changed from old longitude:%s,old latitude:%s to new longitude:%s,new latitude:%s"%(host_asset_info[0][1],host_asset_info[0][2],host_asset_info[0][3],host_log,host_lat)
...
def show_details(h):
	...
	try:
		...
		if db ==1:
			raise SelfException(cursor)
		device_name_dict={'odu100':'UBRe','odu16':'UBR','ap25':'Access Point','idu4':'IDU','idu8':'IDU','unknown':'Generic'}
...
def new_discover_deviec(h):
	...
	try:
		...
		for  i in range(len(new_disable_result)):
			disable_host_json+="{ \"type\":\"host\",\"name\":\"%s\",\"latitude\":%s,\"longitude\":%s}%s"%(("-" if new_disable_result[i][2]=="" or new_disable_result[i][2]==None else new_disable_result[i][2]),(0 if new_disable_result[i][0]=="" or new_disable_result[i][0]==None else new_disable_result[i][0]),(0 if new_disable_result[i][1]=="" or new_disable_result[i][1]==None else new_disable_result[i][1]),(',' if i < len(new_disable_result) - 1 else ''))
		disable_host_json+="]"


		output_dictt="{\"success\":0,\"output\":%s,\"disable_hosts\":%s}" %(new_host_json,disable_host_json)
		html.write(str(output_dictt))
	# Exception Handling
	except MySQLdb as e:
		output_dictt={"success":1,"output":str(e[-1])}
...
def host_status_update_information(h):
	...
	try:
		...
		for index_val,address,state,name in host_result:
			...
			device_type='Unknown' if len(device_type_result)==0 else device_type_result[0][0]
			host_state_id='6' if len(device_type_result)==0 else device_type_result[0][1]

			if host_state_id=='6' or host_state_id=='d' or host_state_id==6:
				state_status=6
			else:
				# This is get severity for change the device image according to severity.
				sel_query="SELECT serevity FROM trap_alarm_current WHERE agent_id='%s' order by timestamp desc limit 1"%(address)
				cursor.execute(sel_query)
				state_status_result=cursor.fetchall()
				state_status=0 if len(state_status_result)==0 else int(state_status_result[0][0])
			...
			if device_type=='ap25':
				...
				if int(master_slave_status)!=0:
					...
					odu_result='DU' if str(signal_result[0][0])=='1111111' else  str(signal_result[0][0])+' (dBm)'
				else:
					if device_type=='odu100':
					...
					cursor.execute(sel_query)
					signal_result=cursor.fetchall()
					odu_result='DU' if str(signal_result[0][0])=='1111111' else  str(signal_result[0][0])+' (TS)'
			else:
				client_result="--"
				odu_result="--"
			host_state_json.append({"id":str(address),"state":str(state_status),"name":str(name),"client":client_result,"signal":odu_result})
		cursor.close()
		db.close()
...
def enabled_device_state(h):
	...
	try:
		...
		db.commit()
		output_dict={"success":0,"output":'Host State successfully updated.'}
