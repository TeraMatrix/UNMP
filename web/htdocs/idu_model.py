...
import uuid
from nms_config import open_database_sqlalchemy_connection,close_database_sqlalchemy_connection
...
class Idu4ConfigProfiles(Base):
    ...
    config_profile_type=relationship(ConfigProfileType,backref=backref('odu16_config_profiles',order_by=config_profile_type_id))
    device_type=relationship(DeviceType,backref=backref('odu16_config_profiles',order_by=device_type_id))
...
class Hosts(Base):
    ...
    #relationship	
    host=relationship(DeviceType,backref=backref('hosts'),order_by=device_type_id)
...
class IduAclPortTable:
    ...
    def __repr__(self):
        return "<IduAclPortTable('%s','%s',%d,%d,'%s',%d)>"\
                %(self.idu_acl_port_table_id,self.config_profile_id,self.acl_port_num,self.acl_index,self.acl_mac_address,self.port_row_status)
...
class IduAlarmPortConfiguration:
    ...
    def __repr__(self):
        return "<IduAlarmPortConfiguration('%s','%s',%d,%d,'%s',%d)>"\
                %(self.idu_alarm_port_configuration_table_id ,self.config_profile_id,self.alarm_pin,self.alarm_admin_status,self.alarm_string,\
                self.alarm_level)
...
class IduAtuConfig:
    ...
    def __init__(self,idu_atu_config_table_id ,config_profile_id,atu_id,atu_state,entry_type,priority,mac_address,atu_memeber_ports):
        self.config_profile_id = config_profile_id 	
        self.atu_id = atu_id
    ...
    def __repr__(self):
        return "<IduAtuConfig('%s','%s',%d,%d,%d,%d,'%s',%d)>"\
                %(self.idu_atu_config_table_id,self.config_profile_id,self.atu_id,self.atu_state,self.entry_type,self.priority,self.mac_address\
                ,self.atu_memeber_ports)
...
class IduE1PortConfiguration:
    ...
    def __repr__(self):
        return "<IduE1PortConfiguration('%s','%s',%d,%d,%d,%d,%d,%d)>"\
                %(self.idu_e1_port_configuration_table_id ,self.config_profile_id,self.port_number,self.admin_state,self.clock_source,\
...
class IduE1PortStatus:
    ...
    def __init__(self,host_id,port_num,op_status,los,ais,rai,rx_frame_slip,tx_frame_slip,adpt_clk_state,bpv,adpt_clk_state,hold_over_status):
    ...
    def __repr__(self):
        return "<IduE1PortStatus('%s','%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d)>" \
                %(self.idu_e1_port_status_table_id,self.host_id,self.port_num,self.op_status,self.los,self.lof,self.ais,self.rai,self.rx_frame_slip,\
...
class IduIduAdminState:
    ...
    def __repr__(self):
        return"<IduIduAdminState('%s','%s',%d,%d)>" %(self.idu_idu_admin_state_table_id,self.config_profile_id,self.state_id,self.admin_state)
...
class IduIduInfoTable:
    ...
    def __init__(self,host_id,info_index,hw_serial_number,hw_type,hw_config_e1,hw_config_eth,hw_config_alarm,system_interface_mac,tdmoip_interface_mac\
    ,current_temperature,sys_uptime):
    ...
    def __repr__(self):
        return "<IduIduInfoTable('%s','%s',%d,'%s',%d,%d,%d,%d,'%s','%s',%d,%d)>" %(self.idu_idu_info_table_id,\
                self.host_id,self.info_index,self.hw_serial_number,self.hw_type,self.hw_config_e1,self.hw_config_eth ,self.hw_config_alarm,\
                self.system_interface_mac,self.tdmoip_interface_mac\
    ,self.current_temperature,self.sys_uptime)
...
class IduIduNetworkStatistics:
    ...
    def __init__(self,host_id,interface_name,rx_packets,tx_packets,rx_bytes,tx_bytes,rx_error,tx_error,rx_dropped,tx_dropped,multicasts,collisions):
    ...
    def __repr__(self):
        return "<IduIduNetworkStatistics('%s','%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d)>"\
                %(self.idu_idu_network_statistics_table_id,self.host_id,interface_name,self.rx_packets,self.tx_packets,self.rx_bytes,self.tx_bytes,
...
class IduIduOmOperations:
    ...
    def __init__(self,config_profile_id ,om_index,om_operation_req,user_name,password,ftp_server_address,path_name,om_operation_result,om_specific_cause):
    ...
    def __repr__(self):
        return "<IduIduOmOperations('%s','%s',%d,%d,'%s','%s','%s','%s',%d,%d)>" \
                % (self.idu_idu_om_operations_table_id,self.config_profile_id,self.om_index,self.om_operation_req,self.user_name,\
                self.password,self.ftp_server_address\
                ,self.path_name,self.om_operation_result,self.om_specific_cause)
...
class IduLinkConfiguration:
    ...
    def __init__(self,config_profile_id,bundle_number,port_number,admin_status,src_bundle_id,dst_bundle_id,dst_ip_addr,tsa_assign,clock_recovery,bundle_size,\
                buffer_size):
    ...
    def __repr__(self):
        return "<IduLinkConfiguration('%s','%s',%d,%d,%d,%d,%d,'%s','%s',%d,%d,%d)>" %\
                (self.idu_link_configuration_table_id,self.config_profile_id,self.bundle_number,self.port_number,self.admin_status,self.src_bundle_id\
                ,self.dst_bundle_id,self.dst_ip_addr,self.tsa_assign,self.clock_recovery,self.bundle_size,self.buffer_size)
...
class IduLinkStatistics:
    ...
    def __init__(self,host_id,bundle_number,port_number,good_frames_to_eth,good_frames_rx,lost_packets_at_rx,lost_packets_at_rx,discarded_packets,\
    ...
    def __return__(self):
        return "<IduLinkStatistics('%s','%s',%d,%d,%d,%d,%d,%d,%d,%d)>" \
                %(self.idu_link_statistics_table_id,self.host_id,self.bundle_number,self.port_number,self.good_frames_to_eth,\
                self.good_frames_rx,self.lost_packets_at_rx,self.discarded_packets,self.reordered_packets,self.underrun_events)
...
class IduLinkStatus:
    ...
    def __init__(self,host_id,bundle_num,operational_status,min_jb_level,max_jb_level,underrun_occured,overrun_occured):
...
class IduPortSecondaryStatistics:
    ...
    def __init__(self,host_id ,switch_port_num,in_unicast,out_unicast,in_broadcast,out_broadcast,in_multicast,out_multicast,\
    in_undersize_rx ,in_fragments_rx ,in_oversize_rx ,in_jabber_rx ,in_mac_rcv_error_rx,in_fcs_error_rx ,out_fcs_error_tx ,\
...
class IduPortStatistics:
    ...
    def __init__(self,host_id,software_stat_port_num,frame_rx,frame_tx,in_discards,in_good_octets,in_bad_octet,out_octets):
...
class IduPortStatBadFrame:
    ...
    def __init__(self,host_id,switch_bad_frame_port,in_undersize_rx,in_fragmnets_rx,in_oversize_rx ,in_mac_rcv_error_rx\
    ...
    def __repr__(self):
        return "<IduPortStatBadFrame('%s','%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,)>" \
                ...
                self.in_fragmnets_rx,self.in_oversize_rx ,self.in_mac_rcv_error_rx\
                ,self.in_jabber_rx ,self.in_fcs_error_rx ,self.out_fcserr_tx ,self.deffered_tx ,self.collision_tx,\
...
class IduPortStatGoodFrame:
    ...
    def __init__(self,host_id,software_good_frame_port_num,in_unicast,out_unicast,in_broadcast,\
