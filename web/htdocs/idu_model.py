...
class IduAtuConfig:
    ...
    def __repr__(self):
        return "<IduAtuConfig('%s','%s',%d,%d,%d,%d,'%s',%d)>"\
               % (self.idu_atu_config_table_id, self.config_profile_id, self.atu_id, self.atu_state, self.entry_type, self.priority, self.mac_address, self.atu_memeber_ports)
...
class IduIduInfoTable:
    ...
    def __init__(self, host_id, info_index, hw_serial_number, hw_type, hw_config_e1, hw_config_eth, hw_config_alarm, system_interface_mac, tdmoip_interface_mac, current_temperature, sys_uptime):
    ...
    def __repr__(self):
        return "<IduIduInfoTable('%s','%s',%d,'%s',%d,%d,%d,%d,'%s','%s',%d,%d)>" % (self.idu_idu_info_table_id,
                                                                                     ...
                                                                                     self.system_interface_mac, self.tdmoip_interface_mac, self.current_temperature, self.sys_uptime)
...
class IduLinkConfiguration:
    ...
    def __init__(
        self, config_profile_id, bundle_number, port_number, admin_status, src_bundle_id, dst_bundle_id, dst_ip_addr, tsa_assign, clock_recovery, bundle_size,
            buffer_size):
    ...
    def __repr__(self):
        return "<IduLinkConfiguration('%s','%s',%d,%d,%d,%d,%d,'%s','%s',%d,%d,%d)>" %\
               (self.idu_link_configuration_table_id, self.config_profile_id, self.bundle_number, self.port_number, self.admin_status, self.src_bundle_id, self.dst_bundle_id,
...
class IduLinkStatistics:
    ...
    def __init__(
        self, host_id, bundle_number, port_number, good_frames_to_eth, good_frames_rx, lost_packets_at_rx, lost_packets_at_rx, discarded_packets,
...
class IduPortSecondaryStatistics:
    ...
    def __init__(
        self, host_id, switch_port_num, in_unicast, out_unicast, in_broadcast, out_broadcast, in_multicast, out_multicast,
...
class IduPortStatBadFrame:
    ...
    def __init__(
        self, host_id, switch_bad_frame_port, in_undersize_rx, in_fragmnets_rx, in_oversize_rx, in_mac_rcv_error_rx, in_jabber_rx, in_fcs_error_rx, out_fcserr_tx, deffered_tx, collision_tx, late_tx, excessive_tx, single_tx,
            multiple_tx):
    ...
    def __repr__(self):
        return "<IduPortStatBadFrame('%s','%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,)>" \
               % (
                   ...
                   self.in_fragmnets_rx, self.in_oversize_rx, self.in_mac_rcv_error_rx, self.in_jabber_rx, self.in_fcs_error_rx, self.out_fcserr_tx, self.deffered_tx, self.collision_tx,
