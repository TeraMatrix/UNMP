                    ...
                    if errorStatus:
                        ...
                        for varBindTableRow in varBindTable:
                            for name, val in varBindTableRow: 
                                ...
                                oid_values_list = []
                                for i in range(1,4):
                                    oid_values_list.append(name.prettyPrint().split('.')[-i])
...
#print pysnmp_peer_reconcile('1.3.6.1.4.1.26149.2.2.13.9.1', '172.22.0.102', 161, 'public')

#print pysnmp_peer_reconcile('1.3.6.1.4.1.26149.2.2.13.5','172.22.0.102',161,'public')
...
#print pysnmp_get_table('1.3.6.1.4.1.26149.2.1.6.3','172.22.0.104',8001,'public')
...
def pysnmp_acl_reconcile(oid,ip_address,port,community):
    ...
    try:
                ...
                if errorIndication:
                    ...
                    if errorStatus:
                        ...
                        for varBindTableRow in varBindTable:
                            ...
                            for name, val in varBindTableRow:
                                if flag == 0:
                                    mac_oid = '.'.join(name.prettyPrint().split('.')[:-1])
                                    flag = 1
                                if name.prettyPrint().find(mac_oid) == -1:
                                    row_dict[int(name.prettyPrint().split('.')[-1])] = val.prettyPrint()
                                else:
                                    mac_dict[int(name.prettyPrint().split('.')[-1])] = str(val)
                                print '%s = %s' % (name.prettyPrint(), val.prettyPrint())
        ...
        return result_dict 
#print pysnmp_acl_reconcile('1.3.6.1.4.1.26149.2.2.13.5','172.22.0.102',161,'public')        
...
def pysnmp_reconcile(received_dict = {},ip_address=None,port=None,community=None):
    ...
    try:
        ...
        if ip_address != None and port != None and community != None:
            ...
            state = 0
            temp_dict = {}
            prev_str = "errorIndication, errorStatus, errorIndex, varBinds = cmdgen.CommandGenerator().getCmd(cmdgen.CommunityData('unmp-agent', '%s'),cmdgen.UdpTransportTarget(('%s', %s))"%(community,ip_address,port)
            state_present = 0
            get_str = prev_str
                    ...
                    if errorStatus > 0 and errorIndex != None:                                                                                   
                        ...
                        for name, val in varBinds:
                            ...
                            if val != '' and val != None and val != ' ' and str(val).find('populated') < 1:   # if needed put that and val != 'No Such Instance currently exists at this OID' 
                                if isinstance(val,v2c.IpAddress):
                                   response_dict[temp_dict[name]] = str(val.prettyPrint())
                                else:
                                    response_dict[temp_dict[name]] = str(val) #val.prettyPrint() #str(val)
                        return
        ...
        # outer elif
            ...
            response_dict = {}
            success = 1
            response_dict[96] = 'arguments are missing' #' internal error prev_Str not passed'
...
def snmp_ping(ip_address_str,community,port):
    try:
        ...
        return 3        
#print snmp_ping('172.22.0.101','public',161)
                    ...
                    if errorStatus > 0 and errorIndex != None:
                        ...
                        for name, val in varBinds:
                            ...
                            if val != '' and val != None:   # if needed put that and val != 'No Such Instance currently exists at this OID' 
                                ...
                                response_dict[102] = 'No Such Instance or no Value present'
                                success = 1
                                return 
    ...
    try:
        ...
        if ip_address != None and port != None and community != None:
            ...
            if len(acl_dict) == 1 and len(admin_dict) == 1:
                ...
                set_str = set_str+",(%s,v2c.%s(%s))"%(aoid_tuple,'Integer32','0')
                ...
                oid_str = admin_dict.values()[0][0]
                aoid_tuple = make_tuple(oid_str)
                set_str = set_str+",(%s,v2c.%s(%s))"%(aoid_tuple,'Integer32','1')
            ...
            if len(rowStatus_dict) == 1:
                ...
                row_present = 1
                set_str = set_str+",(%s,v2c.%s('%s'))"%(row_tuple,datatype,row_value)
            ...
            if len(macAddr_dict) == 1:
                ...
                set_str = set_str+",(%s,v2c.%s('%s'))"%(mac_tuple,datatype,value)
                ...
                if errorIndication:
                    ...
                    if errorStatus > 0 and errorIndex != None:
                        ...
                        if state_present == 1 and int(errorIndex) == 1:
                            ...
                            response_dict[temp_dict[varBinds[int(errorIndex)-1][0]]] = int(errorStatus)
                            error_dict[varBinds[int(errorIndex)-1][0]] = 1
                            ...
                            #return pysnmp_setAcl(None, None, None,acl_dict,admin_dict,macAddr_dict,rowStatus_dict,prev_str, response_dict, temp_dict, error_dict,state)
                        elif temp_dict[varBinds[int(errorIndex)-1][0]] == admin_name:
                            print "    ok ",response_dict
                            #response_dict[admin_name+'1'] = 55
                            response_dict[temp_dict[varBinds[int(errorIndex)-1][0]]+'1'] = int(errorStatus)
                            error_dict[varBinds[int(errorIndex)-1][0]] = 1
                            state = 1
                            #return pysnmp_set(None, None, None,acl_dict,admin_dict,macAddr_dict,rowStatus_dict, prev_str, response_dict, temp_dict, error_dict,state)
                            ...
                            if temp_dict[varBinds[int(errorIndex)-1][0]] == row_name:
                                if str(row_value) == '4' :
                                    ...
                                    #return pysnmp_setAcl(None, None, None,acl_dict,admin_dict,macAddr_dict,rowStatus_dict, prev_str, response_dict, temp_dict, error_dict,state,row)
                                else:
                                    row = 2
                        ...
                        if len(error_dict) >=  len(acl_dict)+len(admin_dict)+len(macAddr_dict)+len(rowStatus_dict):
                            ...
                            response_dict = pysnmp_setAcl( None, None, None, acl_dict,admin_dict,macAddr_dict,rowStatus_dict,prev_str, response_dict, temp_dict, error_dict,state,row)
                            return
        ...
        #outer        
            ...
            if prev_str == None and ip_address == None and port == None and community == None:
                ...
                if len(rowStatus_dict) == 1 and row == 0:
                    ...
                    row_present = 1
                    set_str = set_str+",(%s,v2c.%s('%s'))"%(oid_tuple,datatype,row_value)
                    
                if len(macAddr_dict) == 1 and row != 1:
                    mac_name = macAddr_dict.keys()[0]
                    oid_str,datatype,value = macAddr_dict[mac_name]
                    mac_tuple = make_tuple(oid_str)      
                    if error_dict.has_key(mac_tuple):
                        pass
                    else:                
                        temp_dict[mac_tuple] = mac_name
                        set_str = set_str+",(%s,v2c.%s('%s'))"%(mac_tuple,datatype,value)
        ...
        if response_dict.has_key('success'):
                    ...
                    if errorIndication:
                        ...
                        if errorStatus > 0 and errorIndex != None:
                            ...
                            if state_present == 1 and int(errorIndex) == 1:
                                ...
                                state = 0
                                response_dict[temp_dict[varBinds[int(errorIndex)-1][0]]] = int(errorIndex)
                                error_dict[varBinds[int(errorIndex)-1][0]] = 1
                    ...
                    if errorIndication:
                        ...
                        if errorStatus > 0 and errorIndex != None:
                            ...
                            if state_present == 1 and int(errorIndex) == 1:
                                ...
                                response_dict[admin_name+'1'] = 55
                                state = 1
                                response_dict = pysnmp_set(received_dict, None, None, None, admin_state,prev_str, response_dict, temp_dict, error_dict,state)                                                                                    
                                ...
                                response_dict = pysnmp_set(received_dict, None, None, None, admin_state,prev_str, response_dict, temp_dict, error_dict)                                
                ...
                if errorIndication:
                    ...
                    if errorStatus > 0 and errorIndex != None:
                        ...
                        if state_present == 1 and int(errorIndex) == 1:
                            ...
                            error_dict[varBinds[int(errorIndex)-1][0]] = 1
                            for name, val in varBinds:
                                if response_dict.has_key(temp_dict[name]):
                                    pass
                                else:
