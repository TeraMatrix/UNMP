...
def get_advanced_status_value(h):
    ...
    javascript_list = ["js/highcharts.js","js/advanced_status.js","calendrical/calendrical.js","js/jquery.dataTables.min.js"]
    html.new_header('%s %s Historical Status'%(device_type_dict[device_type_id],ip_address.replace("'","")),"","",css_list,javascript_list)    
    html_content=AdvancedStatusView.ap_set_variable(ip_address,device_type_id,user_id)
...
def ap_total_status_name(h):
    ...
    controller_dict=AdvancedStatusView.graph_name_listing(result_dict,ip_address)
...
def status_data_table_creation(h):
    ...
    end_date=datetime.strptime(end_date+' '+end_time,"%d/%m/%Y %H:%M")
    user_id=html.req.session["user_id"]    
    controller_dict=bll_obj.ap_data_table(user_id,ip_address,start_date,end_date,graph_id,device_type)
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(controller_dict)))
...
def advanced_status_excel_creating(h):
    ...
    user_id=html.req.session["user_id"]
    controller_dict=bll_obj.advaeced_excel_report(report_type,device_type,user_id,ip_address,start_date,end_date,graph_id,select_option)
#    html.req.content_type = 'application/json'
#    html.req.write(str(JSONEncoder().encode(controller_dict)))
