...
class ManageLoginBll(object):
	...
	'''
    	try:
    	    ...
    	    cursor = conn.cursor ()
    	    #query="SELECT lg.user_name , CONCAT(CONCAT(users.first_name,' '),users.last_name) , lg.login_time ,lg.is_logged_in FROM `login_info` as lg JOIN (select user_id ,user_name from user_login) as ul on lg.user_name=ul.user_name join (select first_name,last_name,user_id from users ) as users on users.user_id=ul.user_id order by lg.login_time"
		...
		if temp_list[4]=="1":
		    ...
		    if(res2[0][0]=="SuperAdmin"):
		        if (temp_list[2]=="SuperAdmin"):
		            ...
		            #temp_list.pop()
		            temp_list.append('<button class=\"destroy-button\" id="del_user_%s" onclick="delUser(\'%s\');">Destroy</button>' %(temp_list[0],temp_list[0]))
		    else:
		        if(temp_list[2]=="Admin" or temp_list[2]=="SuperAdmin"):
		            ...
		            ###temp_list.pop()
		            #temp_list.append('<img original-title="Delete User" style="width: 18px; height: 18px; margin: 6px 20px 6px 10px;"  id="del_user_%s" src="images/new_icons/logoutuser.png" class="n-tip-image" onclick="delUser(\'%s\');">' %(temp_list[0],temp_list[0]))
		                 temp_list.append('<button class=\"destroy-button\" id="del_user_%s" onclick="delUser(\'%s\');">Destroy</button>' %(temp_list[0],temp_list[0]))
	...
	'''
    	try:
    	    conn = MySQLdb.connect (*SystemConfig.get_mysql_credentials())
    	    cursor = conn.cursor ()
    	    query="update login_info set next_time_delete=1,is_logged_in=0 where user_name='%s'" %(user_id)
            cursor.execute(query)
            conn.commit()
            return 0
    	except Exception,e:
	...
	'''
    	try:
    	    conn = MySQLdb.connect (*SystemConfig.get_mysql_credentials())
    	    cursor = conn.cursor ()
    	    query="update login_info set is_logged_in='0' where user_name='%s'" %(user_id)
            cursor.execute(query)
            conn.commit()
            return []
    	except Exception,e:
