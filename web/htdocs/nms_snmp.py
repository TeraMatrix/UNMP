...
def snmp_get(version,community,ip,port,oid):
    arg=["snmpget","-Le","-Ln","-v",version,"-c",community,(ip+":"+port),oid]
    SS = subprocess.Popen(arg, stdout=subprocess.PIPE).communicate()[0]
    return SS

def snmp_set(version,community,ip,port,oid,type,val):
    arg=["snmpset","-OsQ","-Le","-Ln","-v",version,"-c",community,(ip+":"+port),oid,type,val]
    SS = subprocess.Popen(arg, stdout=subprocess.PIPE).communicate()[0]
    return SS
def snmp_setmultiple(version,community,ip,port,oid,type,val,oid1,type1,val1):
    arg=["snmpset","-Le","-Ln","-v",version,"-c",community,(ip+":"+port),oid,type,val,oid1,type1,val1]
    SS = subprocess.Popen(arg, stdout=subprocess.PIPE).communicate()[0]
    return SS
def snmp_walk(version,community,ip,port,oid,option,option1):
    ...
    SS=subprocess.Popen(arg, stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate()
    row=re.split("[\n=]",SS[0])
    dictarr=[]
    dict={}
