...
host_status_dic = {0:'No operation', 1:'Firmware download', 2:'Firmware upgrade', 3:'Restore default config', 4:'Flash commit', 5:'Reboot', 6:'Site survey', 7:'Calculate BW', 8:'Uptime service', 9:'Statistics gathering', 10:'Reconciliation', 11:'Table reconciliation', 12:'Set operation', 13:'Live monitoring', 14:'Status capturing'}
...
def odu_listing(h):
    ...
    html.new_header("UBR/UBRe Listing","","",css_list,javascript_list)
    ...
    html.write(str(page_header_search(ip_address,mac_address,"UBRe,UBR,IDU",selected_device_type,"enabled","device_type")))
...
def page_tip_odu_listing(h):
    ...
    "<h1>UBR Listing</h1>"\
    "<div><strong>UBRe/UBR Listing</strong> has shown all UBRe/UBR Type Devices.On This Page You Can see Various Options</div>"\
    "<br/>"\
    ...
    "<div class=\"action-tip\"><div class=\"img-div img-div2\"><img style=\"width:16px;height:16px;\" src=\"images/new/r-green.png\"/></div><div class=\"txt-div\">Reconciliation done b/w 91% and 100%</div></div>"\
...
def page_tip_odu_profiling(h):
    ...
    html_view = ""\
    "<div id=\"help_container\">"\
    "<h1>UBR/UBRe Profiling</h1>"\
    "<div><strong>UBRe/UBR</strong> Profiling of (Device). You can edit the profiling of individual device.</div>"\
    "<br/>"\
    ...
    "<br/>"\
    "<div><strong><u>Radio Access</u></strong> On this tab you can manage RF Frequency,RF Coding,MAC tx Power,Max Crc Errors,Leaky Bucket timer</div>"\
    "<br/>"\
...
def odu_profiling(h):
    ...
    if device_list_param == [] or device_list_param == None:
        html.write(page_header_search("","","UBRe,UBR,IDU",None,"enabled","device_type"))#call the function of common_controller , it is used for listing the Devices based on IPaddress,Macaddress,DeviceTy
    else:
        snapin_list = ["reports","views","Alarm","Inventory","Settings","NetworkMaps","user_management","schedule","Listing"]
        html.new_header("%s %s Configuration"%("UBR" if device_type=="odu16" else "UBRe",device_list_param[0][0]),"odu_listing.py","",css_list,jss_list)
        
        html.write(page_header_search(device_list_param[0][0],device_list_param[0][1],"UBRe,UBR,IDU",device_type,device_list_state,"device_type"))
        ...
        if device_type == "odu16":
            ...
            <div id=\"adminDiv\" style=\"float: left;margin-left:15px\">\
                <ul class=\"button_group\" style=\" margin:10px 0 0 10px !important;\">\
                    <li>\
                        <a class=\"%s\" id=\"ru.ruConfTable.adminstate\" name=\"ru.ruConfTable.adminstate\" state=\"%s\" onclick=\"adminStateCheck(event,this,'%s','ru.ruConfTable.adminstate')\">Radio Unit %s</a>\
                    </li>\
                    <li>\
                        <a class=\"%s\" id=\"ru.ra.raConfTable.raAdminState\" name=\"ru.ra.raConfTable.raAdminState\" state=\"%s\" onclick=\"adminStateCheck(event,this,'%s','ru.ra.raConfTable.raAdminState')\">Radio Access %s</a>\
                    </li>\
                    <li>\
                        <a class=\"%s\" id=\"ru.syncClock.syncConfigTable.adminStatus\" name=\"ru.syncClock.syncConfigTable.adminStatus\" state=\"%s\" onclick=\"adminStateCheck(event,this,'%s','ru.syncClock.syncConfigTable.adminStatus')\">SYN %s</a>\
                    </li>\
                </ul>\
            </div>\
            ...
            if op_status==None:
                op_img = "images/host_status0.jpg"
            ...
            html.write("<div class=\"form-div-footer\">\
                <div id=\"adminDiv\" style=\"float: left;margin-left:15px\">\
                    <ul class=\"button_group\" style=\"margin:10px 0 0 10px !important;\">\
                        <li>\
                            <a class=\"%s\" id=\"ru.ruConfTable.adminstate\" name=\"ru.ruConfTable.adminstate\" title=\"%s\" state=\"%s\" onclick=\"adminStateCheck(event,this,'%s','ru.ruConfTable.adminstate')\">Radio Unit %s</a>\
                        </li>\
                        <li>\
                            <a class=\"%s\" id=\"ru.ra.raConfTable.raAdminState\" name=\"ru.ra.raConfTable.raAdminState\" title=\"%s\" state=\"%s\" onclick=\"adminStateCheck(event,this,'%s','ru.ra.raConfTable.raAdminState')\">Radio Access %s</a>\
                        </li>\
                        <li>\
                            <a class=\"%s\" id=\"ru.syncClock.syncConfigTable.adminStatus\" name=\"ru.syncClock.syncConfigTable.adminStatus\" title=\"%s\" state=\"%s\" onclick=\"adminStateCheck(event,this,'%s','ru.syncClock.syncConfigTable.adminStatus')\">SYN %s</a>\
                        </li>\
                    </ul>\
                </div>\
            ...
            "red" if ra_state==0 else "green",image_ra_title,ra_state,device_list_param[0][2],"Locked" if ra_state==0 else "Unlocked",\
            "red" if sync_state==0 else "green",image_sync_title,sync_state,device_list_param[0][2],"Locked" if sync_state==0 else "Unlocked",op_img,op_title,host_id))
...
# This makes the filter buttons and a commit to flash button and writes on page  
        ...
        tab_str += "<div class=\"yo-tabs\" id=\"config_tabs\" style=\"display:block\">\
                    ...
                    </div>"%(RU_Configuration(h,host_id),Omc_Config(h,host_id),SysOmc_Registration_Configuration(h,host_id),\
...
def odu100_profiling_form(host_id,selected_device):
    ...
    if device_list_param[0][3] == None or device_list_param[0][3] == "":
        ...
        tab_str += "<div class=\"yo-tabs\" id=\"config_tabs\" style=\"display:block\">\
                        ...
                        <div id=\"content_9\" class=\"tab-content form-div\" style=\"margin-bottom: 0px; margin-top: 26px; display: block;\">\
                            %s\
                        </div>\
                        <input type=\"hidden\" name=\"ip_address\" value=\"%s\"/>\
                        <input type=\"hidden\" name=\"mac_address\" value=\"%s\"/>\
                    </div>"%(odu_configuration_object.odu100_ru_configuration(host_id,selected_device),\
                                odu_configuration_object.odu100_omc_configuration(host_id,selected_device),\
                                odu_configuration_object.odu100_sys_omc_registration(host_id,selected_device),\
...
class OduConfiguration(object):
    ...
    @copyright : 2011 Anuj Samariya from Codescape Consultants Pvt. Ltd.
    """
    odu100_select_list_object = MakeOdu100SelectListWithDic()
    ...
    def odu100_ru_configuration(self,host_id,selected_device):
        ...
        if len(ru_configuration) > 0:
            odu100_ru_config_form = "\
                                ...
                                self.odu100_select_list_object.sync_source_select_list(("0" if ru_configuration[0][5] == 0 or ru_configuration[0][5] == 2 else str(ru_configuration[0][1])),("disabled" if ru_configuration[0][5] == 0 or ru_configuration[0][5] == 2 else "enabled"),"ru.ruConfTable.synchSource","false","Sync Source"),\
                                self.odu100_select_list_object.country_code_select_list(str("" if ru_configuration[0][2]== None else str(ru_configuration[0][2])),"enabled","ru.ruConfTable.countryCode","false","Country Code"),\
                                self.odu100_select_list_object.poe_state_select_list(str("" if ru_configuration[0][3]== None else ru_configuration[0][3]),"enabled","ru.ruConfTable.poeState","false","Select Poe State"),\
                ...
                odu100_ru_config_form = "\
                                            <form id = \"odu100_ru_configuration_form\" name=\"odu100_ru_configuration_form\" action=\"odu100_ru_configuration.py\" method=\"get\">\
                                                <div id=\"odu100_ru_configuration_form_parmeters_container\" name=\"odu100_ru_configuration_form_parmeters_container\">\
                                                    ...
                                                    <div class=\"row-elem\">\
                                                        <label class=\"lbl\">Alignment Control</label>\
                                                        <input type = \"text\" id =\"RU.RUConfTable.alignmentControl\" name = \"RU.RUConfTable.alignmentControl\" value = \"\" disabled=\"disabled\"/>\
                                                    </div>\
                                                    <div class=\"row-elem\">\
                                                        <input type=\"submit\" name =\"odu100_submit\" value=\"Save\" id=\"id_omc_submit_save\" onClick=\"return odu100RuConfigFormSubmitCheck('odu100_ru_configuration_form',this)\" class=\"yo-small yo-button\" \>\
                                                        <input type =\"submit\" name =\"odu100_submit\"  id=\"id_omc_button_retry_all\" value=\"Retry\" style=\"Display:None\" onClick=\"return odu100RuConfigFormSubmitCheck('odu100_ru_configuration_form',this)\" class=\"yo-small yo-button\" />\
                                    ...
                                    self.odu100_select_list_object.poe_state_select_list("","enabled","ru.ruConfTable.poeState","false","Select Poe State"),\
    ...
    def  odu100_ra_configuration(self,host_id,selected_device):
        ...
        if len(ra_configuration) >0 and len(tddmac_configuration) > 0:
            odu100_ra_config_form = "\
                                    <form id=\"odu100_ra_configuration_form\" name=\"odu100_ra_configuration_form\" action=\"odu100_ra_configuration.py\" method=\"get\">\
                                        <div id=\"odu100_ra_configuration_form_parmeters_container\" name=\"odu100_ra_configuration_form_parmeters_container\">\
                                            ...
                                            <div class=\"row-elem\">\
                                                ...
                                                <input type = \"text\" id =\"ru.ra.raConfTable.ssID\" name = \"ru.ra.raConfTable.ssID\" value = \"%s\" maxsize = \"26\"/>\
                                            </div>\
                                            <div class=\"row-elem\">\
                                            ...
                                            <div class=\"row-elem\">\
                                                ...
                                                <input type = \"text\" id =\"ru.ra.tddMac.raTddMacConfigTable.passPhrase\" name = \"ru.ra.tddMac.raTddMacConfigTable.passPhrase\" value=\"%s\"/>\
                                            </div>\
                                            <div class=\"row-elem\">\
                                            ...
                                            <div class=\"row-elem\">\
                                                ...
                                                <input type = \"text\" id =\"ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors\" name = \"ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors\" value = \"%s\" maxsize = \"15\">\
                                                <span style=\"font-size:9px\">&nbsp;&nbsp;0 to 50000</span>\
                                            </div>\
                                            <div class=\"row-elem\">\
                                                <label class=\"lbl\">Leaky Bucket Timer</label>\
                                                <input type = \"text\" id =\"ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue\" name = \"ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue\"\" value = \"%s\" maxsize = \"15\">\
                                            ...
                                            <div class=\"row-elem\">\
                                                ...
                                                <input type = \"text\" id=\"ru.ra.raConfTable.guaranteedBroadcastBW\" name=\"ru.ra.raConfTable.guaranteedBroadcastBW\" value=\"%s\" maxsize=\"15\" \>\
                                            </div>\
                                            <div class=\"row-elem\">\
                                ...
                                " %(self.odu100_select_list_object.timeslot_select_list(int(0 if ra_configuration[0][0]==None else ra_configuration[0][0]),"enabled" if ru_configuration[0].defaultNodeType==0 or ru_configuration[0].defaultNodeType==2 else "disabled","ru.ra.raConfTable.numSlaves","false","TimeSlot"),\
                                        ...
                                        self.odu100_select_list_object.acm_select_list(str("" if ra_configuration[0][2]==None else ra_configuration[0][2]),"enabled" if ru_configuration[0].defaultNodeType==0 or ru_configuration[0].defaultNodeType==2 else "disabled","ru.ra.raConfTable.acm","false","ACM"),\
                                        self.odu100_select_list_object.dba_select_list(str("" if ra_configuration[0][3]==None else ra_configuration[0][3]),"enabled" if ru_configuration[0].defaultNodeType==0 or ru_configuration[0].defaultNodeType==2 else "disabled","ru.ra.raConfTable.dba","false","DBA"),\
                                        "" if ra_configuration[0][4]==None else ra_configuration[0][4],\
                                        self.odu100_select_list_object.acs_select_list(str("" if ra_configuration[0][5]==None else ra_configuration[0][5]),"enabled" if ru_configuration[0].defaultNodeType==0 or ru_configuration[0].defaultNodeType==2 else "disabled","ru.ra.raConfTable.acs","false","ACS"),\
                                        self.odu100_select_list_object.dfs_select_list(str("" if ra_configuration[0][6]== None else ra_configuration[0][6]),"enabled" if ru_configuration[0].defaultNodeType==0 or ru_configuration[0].defaultNodeType==2 else "disabled","ru.ra.raConfTable.dfs","false","DFS"),\
                ...
                odu100_ra_config_form = "\
                                        <form id=\"odu100_ra_configuration_form\" name=\"odu100_ra_configuration_form\" action=\"odu100_ra_configuration.py\" method=\"get\">\
                                            <div id=\"odu100_ra_configuration_form_parmeters_container\" name=\"odu100_ra_configuration_form_parmeters_container\">\
                                                ...
                                                <div class=\"row-elem\">\
                                                    <label class=\"lbl\">SSID</label>\
                                                    <input type = \"text\" id =\"ru.ra.raConfTable.ssID\" name = \"ru.ra.raConfTable.ssID\" value = \"\" maxsize = \"26\"/>\
                                                </div>\
                                                <div class=\"row-elem\">\
                                                    <label class=\"lbl\">Encryption Type</label>\
                                                    %s\
                                                ...
                                                <div class=\"row-elem\">\
                                                    <label class=\"lbl\">Max Crc Errors</label>\
                                                    <input type = \"text\" id =\"ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors\" name = \"ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors\" value = \"\" maxsize = \"15\">\
                                                </div>\
                                                <div class=\"row-elem\">\
                                                    <label class=\"lbl\">Leaky Bucket Timer</label>\
                                                    <input type = \"text\" id =\"ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue\" name = \"ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue\" value = \"\" maxsize = \"15\">\
                                                </div>\
                                                <div class=\"row-elem\">\
                                                    <label class=\"lbl\">ACM</label>\
                                                    %s\
                                                ...
                                                <div class=\"row-elem\">\
                                                    <label class=\"lbl\">Guaranteed Broadcast </label>\
                                                    <input type = \"text\" id =\"ru.ra.raConfTable.guaranteedBroadcastBW\" name = \"ru.ra.raConfTable.guaranteedBroadcastBW\" value = \"\" maxsize = \"15\" disabled=\"disabled\">\
                                                </div>\
                                                <div class=\"row-elem\">\
                                                    <label class=\"lbl\">ACS</label>\
                                                    %s\
            ...
            for i in range(0,16):
                if len(peer_configuration) > i:
                    odu100_peer_config_form += "<div class=\"row-elem\" style=\"width:1230px;display:none\">\
                                                    ...
                                                    <input type = \"text\" id =\"ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s\" name = \"ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s\" value = \"%s\" style=\"width:61px;\"/>\
                                                    <input type = \"text\" id =\"ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s\" name = \"ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s\" value = \"%s\"  style=\"width:61px;\"/>\
                                                    <input type = \"text\" id =\"ru.ra.peerNode.peerConfigTable.maxDownlinkBW.%s\" name=\"ru.ra.peerNode.peerConfigTable.maxDownlinkBW.%s\" value = \"\"  readonly=\"readonly\"style=\"width:61px;\" />\
                                                    <input type = \"text\" id =\"ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s\" name = \"ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s\" value = \"\"  readonly=\"readonly\" style=\"width:61px;\"/>\
                                                    %s\
                                                </div>"%(i+1,i+1,\
                                                "" if peer_configuration[i].peerMacAddress==None else peer_configuration[i].peerMacAddress,\
                                                        ...
                                                        "enabled",'ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.%s'%(i+1),False,"MCS Index") if int(antenna_port)==1 or int(antenna_port)==2 else self.odu100_select_list_object.mcs_index_select_list_mimo("-1" if peer_configuration[i].basicrateMCSIndex==None or peer_configuration[i].basicrateMCSIndex=="" else peer_configuration[i].basicrateMCSIndex,\
                                                        "enabled",'ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.%s'%(i+1),False,"MCS Index"))
                    ...
                    odu100_peer_config_form += "<div class=\"row-elem\" style=\"width:1230px;display:none\">\
                                                    ...
                                                    <input type = \"text\" id =\"ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s\" name = \"ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s\" value = \"%s\" style=\"width:61px;\"/>\
                                                    <input type = \"text\" id =\"ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s\" name = \"ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s\" value = \"%s\"  style=\"width:61px;\"/>\
                                                    <input type = \"text\" id =\"ru.ra.peerNode.peerConfigTable.maxDownlinkBW.%s\" name=\"ru.ra.peerNode.peerConfigTable.maxDownlinkBW.%s\" value = \"100000\"  readonly=\"readonly\" style=\"width:61px;\"/>\
                                                    <input type = \"text\" id =\"ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s\" name = \"ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s\" value = \"100000\"  readonly=\"readonly\" style=\"width:61px;\"/>\
                                                    %s\
                                                </div>"%(i+1,i+1,"",\
                                                        i+1,i+1,"512",i+1,i+1,"512",i+1,i+1,i+1,i+1,\
                                                        self.odu100_select_list_object.mcs_index_select_list(-1,\
                                                        "enabled",'ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.%s'%(i+1),False,"MCS Index")if int(antenna_port)==1 or int(antenna_port)==2 else self.odu100_select_list_object.mcs_index_select_list_mimo(-1,\
                                                        "enabled",'ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.%s'%(i+1),False,"MCS Index"))
                ...
                for i in range(0,16):
                    if len(peer_configuration) > i:
                        odu100_peer_config_form += "<div class=\"row-elem\" style=\"width:1230px;display:none\">\
                                                        ...
                                                        <input type = \"text\" id =\"ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s\" name = \"ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s\" value = \"%s\" />\
                                                        <input type = \"text\" id =\"ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s\" name = \"ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s\" value = \"%s\"  />\
                                                        <input type = \"text\" id =\"ru.ra.peerNode.peerConfigTable.maxDownlinkBW.%s\" name=\"ru.ra.peerNode.peerConfigTable.maxDownlinkBW.%s\" value = \"\"  readonly=\"readonly\"/>\
                                                        <input type = \"text\" id =\"ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s\" name = \"ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s\" value = \"\"  readonly=\"readonly\"/>\
                                                        %s\
                                                    </div>"%(i+1,i+1,\
                                                    "" if peer_configuration[i].peerMacAddress==None else peer_configuration[i].peerMacAddress,\
                                                            i+1,i+1,\
...
##        
    ...
    def odu100_sync_configuration(self,host_id,selected_device):
        ...
        if len(sync_configuration) > 0:
            odu100_sync_config_form = "\
                                            <form id = \"odu100_sync_config_form\" name = \"odu100_sync_config_form\" action = \"odu100_sync_config.py\" method=\"get\">\
                                                <div id=\"odu100_sync_form_parmeters_container\" name=\"odu100_sync_form_parmeters_container\">\
                                                    ...
                                                    <div class=\"row-elem\">\
                                                        ...
                                                        <input type = \"text\" id =\"ru.syncClock.syncConfigTable.syncLossThreshold\" name = \"ru.syncClock.syncConfigTable.syncLossThreshold\" value = \"%s\" maxsize = \"15\"/>\
                                                        <span style=\"font-size:9px\">&nbsp;&nbsp;1 to 100</span>\
                                                    </div>\
                                                    <div class=\"row-elem\">\
                                                        <label class=\"lbl\">Leaky Bucket Timer</label>\
                                                        <input type = \"text\" id =\"ru.syncClock.syncConfigTable.leakyBucketTimer\" name = \"ru.syncClock.syncConfigTable.leakyBucketTimer\" value = \"%s\" maxsize = \"15\"/>\
                                                        <span style=\"font-size:9px\">&nbsp;&nbsp;1 to 60</span>\
                                                    </div>\
                                                    <div class=\"row-elem\">\
                                                        <label class=\"lbl\">Sync Loss Timeout</label>\
                                                        <input type = \"text\" id =\"ru.syncClock.syncConfigTable.syncLostTimeout\" name = \"ru.syncClock.syncConfigTable.syncLostTimeout\" value = \"%s\" maxsize = \"15\"/>\
                                                        <span style=\"font-size:9px\">&nbsp;&nbsp;30 to 600</span>\
                                                    </div>\
                                                    <div class=\"row-elem\">\
                                                        <label class=\"lbl\">Sync Timer Adjust</label>\
                                                        <input type = \"text\" id =\"ru.syncClock.syncConfigTable.syncConfigTimerAdjust\" name = \"ru.syncClock.syncConfigTable.syncConfigTimerAdjust\" value = \"%s\" maxsize = \"15\" \"%s\"/>\
                                                        <span style=\"font-size:9px\">&nbsp;&nbsp;-127 to 127</span>\
                                                    </div>\
                                                    <div class=\"row-elem\">\
                                                        <label class=\"lbl\">Percentage Downlink Transmit Time</label>\
                                                        <input type = \"text\" id =\"ru.syncClock.syncConfigTable.percentageDownlinkTransmitTime\" name = \"ru.syncClock.syncConfigTable.percentageDownlinkTransmitTime\" value = \"%s\" maxsize = \"15\" \"%s\"/>\
                                        ...
                                        " %(self.odu100_select_list_object.raster_time_select_list(str("" if sync_configuration[0][0]==None else sync_configuration[0][0]),"enabled" if ru_configuration[0].defaultNodeType==0 or ru_configuration[0].defaultNodeType==2 else "disabled","ru.syncClock.syncConfigTable.rasterTime","false","Raster Time")\
                                                ...
                                                "" if sync_configuration[0][4]==None else sync_configuration[0][4],"" if int(ru_configuration[0].defaultNodeType)==0 or ru_configuration[0].defaultNodeType==2 else "disabled=disabled",\
                                                sync_configuration[0][5],"" if int(ru_configuration[0].defaultNodeType)==0 or ru_configuration[0].defaultNodeType==2 else "disabled=disabled",host_id,selected_device,ru_configuration[0].defaultNodeType)
        else:
                ...
                odu100_sync_config_form = "\
                                                <form id = \"odu100_sync_config_form\" name = \"odu100_sync_config_form\" action = \"odu100_sync_config.py\" method=\"get\">\
                                                    <div id=\"odu100_sync_form_parmeters_container\" name=\"odu100_sync_form_parmeters_container\">\
                                                        ...
                                                        <div class=\"row-elem\">\
                                                            <label class=\"lbl\">Sync Loss Threshold</label>\
                                                            <input type = \"text\" id =\"ru.syncClock.syncConfigTable.syncLossThreshold\" name = \"ru.syncClock.syncConfigTable.syncLossThreshold\" value = \"\" maxsize = \"15\"/>\
                                                        </div>\
                                                        <div class=\"row-elem\">\
                                                            <label class=\"lbl\">Leaky Bucket Timer</label>\
                                                            <input type = \"text\" id =\"ru.syncClock.syncConfigTable.leakyBucketTimer\" name = \"ru.syncClock.syncConfigTable.leakyBucketTimer\" value = \"\" maxsize = \"15\"/>\
                                                        </div>\
                                                        <div class=\"row-elem\">\
                                                            <label class=\"lbl\">Sync Loss Timeout</label>\
                                                            <input type = \"text\" id =\"ru.syncClock.syncConfigTable.syncLostTimeout\" name = \"ru.syncClock.syncConfigTable.syncLostTimeout\" value = \"\" maxsize = \"15\"/>\
                                                        </div>\
                                                        <div class=\"row-elem\">\
                                                            <label class=\"lbl\">Sync Timer Adjust</label>\
                                                            <input type = \"text\" id =\"ru.syncClock.syncConfigTable.syncConfigTimerAdjust\" name = \"ru.syncClock.syncConfigTable.syncConfigTimerAdjust\" value = \"\" maxsize = \"15\"/>\
                                                        </div>\
                                                        <div class=\"row-elem\">\
                                                            <label class=\"lbl\">Percentage Downlink Transmit Time</label>\
                                                            <input type = \"text\" id =\"ru.syncClock.syncConfigTable.percentageDownlinkTransmitTime\" name = \"ru.syncClock.syncConfigTable.percentageDownlinkTransmitTime\" value = \"\" maxsize = \"15\"/>\
                                                        </div>\
                                                        <div class=\"row-elem\">\
                                                            <input type=\"submit\" name =\"odu100_submit\" value=\"Save\" id=\"id_omc_submit_save\" onClick=\"return odu100CommonFormSubmit('odu100_sync_config_form',this)\" class=\"yo-small yo-button\" />\
                                                            <input type =\"submit\" name =\"odu100_submit\"  id=\"id_omc_button_retry_all\" value=\"Retry\" style=\"Display:None\" onClick=\"return odu100CommonFormSubmit('odu100_sync_config_form',this)\" class=\"yo-small yo-button\" />\
                                            ...
                                            <form id = \"odu100_ip_config_form\" name = \"odu100_ip_config_form\" action=\"odu100_ip_config.py\" method =\"get\">\
                                                <div id=\"ip_config_form_parmeters_container\" name=\"ip_config_form_parmeters_container\">\
                                                    <div class=\"row-elem\">\
                                                        ...
                                                        <input type = \"text\" id =\"ru.ipConfigTable.ipAddress\" name = \"ru.ipConfigTable.ipAddress\" value = \"%s\" maxsize = \"15\" disabled=\"disabled\"/>\
                                                    </div>\
                                                    <div class=\"row-elem\">\
                                                        <label class=\"lbl\">IP Netmask</label>\
                                                        <input type = \"text\" id =\"ru.ipConfigTable.ipNetworkMask\" name = \"ru.ipConfigTable.ipNetworkMask\" value = \"%s\" maxsize = \"15\"/>\
                                                    </div>\
                                                    <div class=\"row-elem\">\
                                                        <label class=\"lbl\">IP Default Gateway</label>\
                                                        <input type = \"text\" id =\"ru.ipConfigTable.ipDefaultGateway\" name = \"ru.ipConfigTable.ipDefaultGateway\" value = \"%s\" maxsize = \"15\"/>\
                                                    </div>\
                ...
                odu100_ip_config_form = "\
                                                <form id = \"odu100_ip_config_form\" name = \"odu100_ip_config_form\" action=\"odu100_ip_config.py\" method =\"get\">\
                                                    <div id=\"ip_config_form_parmeters_container\" name=\"ip_config_form_parmeters_container\">\
                                                        <div class=\"row-elem\">\
                                                            <label class=\"lbl\">IP Address</label>\
                                                            <input type = \"text\" id =\"ru.ipConfigTable.ipAddress\" name = \"ru.ipConfigTable.ipAddress\" value = \"\" maxsize = \"15\"/>\
                                                        </div>\
                                                        <div class=\"row-elem\">\
                                                            <label class=\"lbl\">IP Netmask</label>\
                                                            <input type = \"text\" id =\"ru.ipConfigTable.ipNetworkMask\" name = \"ru.ipConfigTable.ipNetworkMask\" value = \"\" maxsize = \"15\"/>\
                                                        </div>\
                                                        <div class=\"row-elem\">\
                                                            <label class=\"lbl\">IP Default Gateway</label>\
                                                            <input type = \"text\" id =\"ru.ipConfigTable.ipDefaultGateway\" name = \"ru.ipConfigTable.ipDefaultGateway\" value = \"\" maxsize = \"15\"/>\
                                                        </div>\
                                                        <div class=\"row-elem\">\
                                                            <label class=\"lbl\">DHCP state</label>\
                                                            %s\
                                                ...
                                                <form id=\"odu100_omc_config_form\" name=\"odu100_omc_config_form\" action=\"odu100_omc_config.py\" method=\"get\">\
                                                    <div class=\"omc_config_form_parmeters_container\" name=\"omc_config_form_parmeters_container\">\
                                                        <div class=\"row-elem\">\
                                                            ...
                                                            <input type=\"text\" id=\"ru.omcConfTable.omcIpAddress\" name=\"ru.omcConfTable.omcIpAddress\" value=\"%s\" maxsize=\"15\"/>\
                                                        </div>\
                                                        <div class=\"row-elem\" style=\"display:none\">\
                                                            <label class=\"lbl\">Periodic Statistics Timer</label>\
                                                            <input type = \"text\" id =\"ru.omcConfTable.periodicStatsTimer\" name = \"ru.omcConfTable.periodicStatsTimer\" value = \"%s\" maxsize = \"15\"/>\
                                                        </div>\
                    ...
                    odu100_omc_config_form = "\
                                                    <form id=\"odu100_omc_config_form\" name=\"odu100_omc_config_form\" action=\"odu100_omc_config.py\" method=\"get\">\
                                                        <div class=\"omc_config_form_parmeters_container\" name=\"omc_config_form_parmeters_container\">\
                                                            <div class=\"row-elem\">\
                                                                <label class=\"lbl\">UNMP IP</label>\
                                                                <input type=\"text\" id=\"ru.omcConfTable.omcIpAddress\" name=\"ru.omcConfTable.omcIpAddress\" value=\"\" maxsize=\"15\"/>\
                                                            </div>\
                                                            <div class=\"row-elem\" style=\"display:none\">\
                                                                <label class=\"lbl\">Periodic Statistics Timer</label>\
                                                                <input type = \"text\" id =\"ru.omcConfTable.periodicStatsTimer\" name = \"ru.omcConfTable.periodicStatsTimer\" value=\"\" maxsize = \"15\"/>\
                                                            </div>\
                                                            <div class=\"row-elem\">\
                                                                <input type=\"submit\" name =\"odu100_submit\" value=\"Save\" id=\"id_omc_submit_save\" onClick=\"return odu100CommonFormSubmit('odu100_omc_config_form',this)\" class=\"yo-small yo-button\" />\
                                                                <input type =\"submit\" name =\"odu100_submit\"  id=\"id_omc_button_retry_all\" value=\"Retry\" style=\"Display:None\" onClick=\"return odu100CommonFormSubmit('odu100_omc_config_form',this)\" class=\"yo-small yo-button\" />\
    ...
    def odu100_acl_configuration(self,host_id,selected_device):
        ...
        if len(acl_configuration) > 0:
            ...
            odu100_acl_config_form += "<div class=\"row-elem\">\
                                        ...
                                        </div>"%(self.odu100_select_list_object.acl_index_select_list("" if acl_configuration[0][0]==None else acl_configuration[0][0],"enabled","aclindex","false","Index"),\
            ...
            if ra_config_table_data["success"] == 1 or acl_config_table_data["success"] == 1:
                ...
                odu100_acl_config_form += "<div class=\"row-elem\">\
                                            ...
                                            <div class=\"row-elem\">\
                                                ...
                                                <input type = \"hidden\" name = \"device_type\" value=\"%s\" />\
                                                <input type = \"hidden\" name = \"total_rows\" value=\"%s\" id=\"total_rows\"/>\
                                            </div>"%(self.odu100_select_list_object.acl_index_select_list("","enabled","aclindex","false","Index"),\
                                                   "",\
    ...
    def odu100_channel_configuration(self,host_id,selected_device,i=0):
        ...
        if int(i)==1:
            result = refresh_channel_freq_list(host_id,selected_device)
            #return str(result)
            if result['success']==1:
                if len(ru_configuration)>0:
                ...
                for i in range(0,10):
                    str_form+="<div class=\"row-elem\" style=\"align:center\">\
                                        \
                                        %s\
                                    </div>"%(self.odu100_select_list_object.preffered_channel_select_list_snmp((0 if channel_configuration[i].rafrequency==None or channel_configuration[i].rafrequency=="" or int(channel_configuration[i].rafrequency)==0 else str(channel_configuration[i].rafrequency)),"enabled",'RU.RA.PrefRfChanFreq.frequency.%s'%(i+1),"false","Channel list"))             
                str_form+="<div class=\"row-elem\">\
                                    <input type =\"submit\" name =\"odu100_submit\" id=\"id_omc_submit_save\" value=\"Save\" onClick=\"return odu100CommonFormSubmit('channel_configuration_form',this)\" class=\"yo-small yo-button\"/>\
                                    <input type =\"submit\" name =\"refresh_channel\" id=\"refresh_channel\" value=\"Channel List Reconciliation\"  onClick=\"return refreshchannelList()\" class=\"yo-small yo-button\" />\
                                    <input type =\"submit\" name =\"odu100_submit\" id=\"id_omc_button_retry_all\" value=\"Retry\" style=\"Display:None\" onClick=\"return odu100CommonFormSubmit('channel_configuration_form',this)\" class=\"yo-small yo-button\" />\
            ...
            for i in range(0,10):
                str_form+="<div class=\"row-elem\" style=\"align:center\">\
                                    \
                                    %s\
                                </div>"%(self.odu100_select_list_object.preffered_channel_select_list((0 if channel_configuration[i].rafrequency==None or channel_configuration[i].rafrequency=="" else channel_configuration[i].rafrequency),"enabled",'RU.RA.PrefRfChanFreq.frequency.%s'%(i+1),"false","Channel list",host_id,selected_device))             
            str_form+="<div class=\"row-elem\">\
                                <input type =\"submit\" name =\"odu100_submit\" id=\"id_omc_submit_save\" value=\"Save\" onClick=\"return odu100CommonFormSubmit('channel_configuration_form',this)\" class=\"yo-small yo-button\"/>\
                                <input type =\"submit\" name =\"refresh_channel\" id=\"refresh_channel\" value=\"Channel List Reconciliation\"  onClick=\"return refreshchannelList()\" class=\"yo-small yo-button\" />\
                                <input type =\"submit\" name =\"odu100_submit\" id=\"id_omc_button_retry_all\" value=\"Retry\" style=\"Display:None\" onClick=\"return odu100CommonFormSubmit('channel_configuration_form',this)\" class=\"yo-small yo-button\" />\
    ...
    def site_survey_form(self,site_survey_record):
        ...
        form_str += "<div id=\"site_survey_result\">\
                        ...
                        <tr>\
                            ...
                            <th>Number Of CRC Errors</th>\
                            <th>Maximun CRC Errors</th>\
                            <th>Number of PHY Errors</th>\
                            <th>Maximum PHY Errors</th>\
                            <th>Maximum Valid Frames</th>\
        ...
        str_form+="\
                    <form action=\"bw_action.py\" method=\"get\" id=\"bw_action_form\" name=\"bw_action_form\" style=\"overflow:hidden;\">\
                        <div class=\"row-elem\">\
                            ...
                            <input type=\"text\" name=\"tx_rate\" id=\"tx_rate\" />\
                        </div>\
                        <div class=\"row-elem\">\
                            <label class=\"lbl lbl-big\">TX Time(usec)</label>\
                            <input type=\"text\" name=\"tx_time\" id=\"tx_time\" />\
                        </div>\
                        <div class=\"row-elem\">\
                            <label class=\"lbl lbl-big\">TX BW(kbps)</label>\
                            <input type=\"text\" name=\"tx_bw\" id=\"tx_bw\" readonly=\"readonly\" />\
                        </div>\
                        <div class=\"row-elem\">\
                            <input type =\"submit\" name =\"odu100_submit\"  id=\"id_omc_submit_save\" value=\"BW Calculator\" onClick=\"return odu100BWCalc('bw_action_form','%s','%s')\" class=\"yo-small yo-button\"/>\
                        </div>\
                ...
                if html.var("host_id") == "" or html.var("host_id") == None:
                    ...
                    if html.var("ru.ra.tddMac.raTddMacConfigTable.encryptionType")!=None:
                        if Validation.is_required(html.var("ru.ra.tddMac.raTddMacConfigTable.encryptionType")):
                            ...
                            flag = 1
                            dic_result["Encryption"] = "Please Select Encryption"
                    ...
                    if html.var("ru.ra.tddMac.raTddMacConfigTable.txPower")!=None:
                        if Validation.is_required(html.var("ru.ra.tddMac.raTddMacConfigTable.txPower")):
                            ...
                            if Validation.is_number(html.var("ru.ra.tddMac.raTddMacConfigTable.txPower")):
                                ...
                                flag = 1 
                                dic_result["Tx Power"] = "Tx Power Must be Integer"
                        else:
                            flag = 1
                    ...
                    if html.var("ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors") != None:    
                        if Validation.is_required(html.var("ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors")):
                            if Validation.is_number(html.var("ru.ra.tddMac.raTddMacConfigTable.maxCrcErrors")):
                                ...
                                flag = 1 
                                dic_result["MAc Crc Error"] = "Max Crc Error Must be Integer"
                        else:
                            flag = 1
                            dic_result["Max Crc Errors "] = "Max Crc Error is required"
                    ...
                    if html.var("ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue")!=None: 
                        if Validation.is_required(html.var("ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue")):
                            if Validation.is_number(html.var("ru.ra.tddMac.raTddMacConfigTable.leakyBucketTimerValue")):
                                ...
                                flag = 1 
                                dic_result["Leaky Bucket Timer"] = "Leaky Bucket Timer Must be Integer"
                        else:
                            flag = 1
                            dic_result["Leaky Bucket Timer"] = "Leaky Bucket Timer is required"
                    if int(html.var("node_type"))==0 or int(html.var("node_type"))==2:
                        if html.var("ru.ra.raConfTable.acm")!=None:    
                            if Validation.is_required(html.var("ru.ra.raConfTable.acm")):
                                ...
                                flag = 1
                                dic_result["ACM"] = "Please Select ACM "
                    if int(html.var("node_type"))==0 or int(html.var("node_type"))==2:
                        if html.var("ru.ra.raConfTable.dba")!=None:
                    ...
                    if html.var("ru.ra.raConfTable.guaranteedBroadcastBW")!=None:
                        if Validation.is_required(html.var("ru.ra.raConfTable.guaranteedBroadcastBW")):
                            if Validation.is_number(html.var("ru.ra.raConfTable.guaranteedBroadcastBW")):
                                    ...
                                    if Validation.is_required(html.var("ru.ra.raConfTable.numSlaves")):
                                        if int(html.var("ru.ra.raConfTable.numSlaves"))==1:
                                            ...
                                            if int(html.var("ru.ra.raConfTable.numSlaves"))>1 and int(html.var("ru.ra.raConfTable.guaranteedBroadcastBW"))>0:
                                                ...
                                                flag = 1
                                                dic_result["result"] = "If number of slaves more than one, guaranteed broadcast BW cant be zero"
                                    else:
                                        flag = 1
                                else:
                                    dic_result["ru.ra.raConfTable.guaranteedBroadcastBW"] = html.var("ru.ra.raConfTable.guaranteedBroadcastBW")
                                ...
                                flag = 1 
                                dic_result["ru.ra.raConfTable.guaranteedBroadcastBW"] = "GuaranteedBroadcastBW Must be Integer"                       
                        else:
                            flag = 1
                            dic_result["ru.ra.raConfTable.guaranteedBroadcastBW"] = "GuaranteedBroadcast is required"
                    if int(html.var("node_type"))==0 or int(html.var("node_type"))==2:
                        if html.var("ru.ra.raConfTable.acs")!=None:
                            if Validation.is_required(html.var("ru.ra.raConfTable.acs")):
                                ...
                                flag = 1
                                dic_result["ACS"] = "Please Select ACS"
                    if int(html.var("node_type"))==0 or int(html.var("node_type"))==2:
                        if html.var("ru.ra.raConfTable.dfs")!=None:
                            if Validation.is_required(html.var("ru.ra.raConfTable.dfs")):
                                ...
                                flag = 1
                                dic_result["DFS"] = "Please Select DFS"   
        ...
        if host_op_status==None or host_op_status==0:
            if html.var("odu100_submit") == "Save":
                ...
                if Validation.is_required(html.var("ru.ruConfTable.channelBandwidth")):
                    ...
                    flag = 1
                    dic_result["Channel bandwidth"] = "Please Select Channel Bandwidth "
                ...
                if html.var("ru.ruConfTable.synchSource")!=None:
                    if Validation.is_required(html.var("ru.ruConfTable.synchSource")):
                        ...
                        flag = 1
                        dic_result["SynchSource"] = "Please Select SynchSource"
                ...
                if Validation.is_required(html.var("ru.ruConfTable.countryCode")):
                    ...
                    flag = 1
                    dic_result["Country Code"] = "Please Select Country Code"
                ...
                if Validation.is_required(html.var("ru.ruConfTable.poeState")):
                    ...
                    flag = 1
                    dic_result["PoeState"] = "Please Select PoeState"
                ...
                if Validation.is_required(html.var("RU.RUConfTable.alignmentControl")):
                    if Validation.is_number(html.var("RU.RUConfTable.alignmentControl")):
                        ...
                        dic_result["Alignment Control"] = "Number is required"
                else:
                        dic_result["Alignment Control"] = "Alignment Control is required"                
                    ...
                    if html.var("timeslot_val")!= None:
                        ...
                        if flag == 1:
                            ...
                            if chk_mac_duplicacy(mac_duplicate_list)==0:
                                ...
                                if mac_accept ==0 and mac_deny==0:
                                    dic_result["success"] = 0
    ...
    try:
        if host_op_status==None or host_op_status==0:
            ...
            if html.var("odu100_submit") == "Add" or html.var("odu100_submit") == "Edit":
                if html.var("aclindex") == "" or html.var("aclindex") == None:
                    flag = 1
                    dic_result["aclindex"] = "Index is not selected"
                else:
        ...
        ##                dic_result["ru.ra.raConfTable.aclMode"] = html.var("ru.ra.raConfTable.aclMode")
                if html.var("ru.ra.raAclConfigTable.macaddress") == "" or html.var("ru.ra.raAclConfigTable.macaddress")==None: 
                    flag = 1
                    dic_result["ru.ra.raAclConfigTable.macaddress"] = "MAC Address is Empty"
                else :
                    if check_mac(host_id,html.var("ru.ra.raAclConfigTable.macaddress"),html.var("ru.ra.raConfTable.aclMode"))==1:
                        mac_flag = 1
...
def site_survey_result(h):
    ...
    if len(site_survey_record)>0 or  site_survey_record != [] :
        ...
        html.write("<input type=\"button\" name=\"refresh_scan_list\" id=\"refresh_scan_list\" value=\"Recalculate\" class=\"yo-small yo-button\"/><br/><br/>")
        html.write(form_str)
    else:
        html.write("true")
...
def site_survey_snmp(h):
    try:
        ...
        if host_op_status==None or host_op_status==0:
            ...
            obj = SiteSurvey()
            result = obj.site_survey(host_id,node_type)
        else:
            result=host_status_dic[host_op_status]
        html.req.content_type = 'application/json'
        html.req.write(str(JSONEncoder().encode(result)))
    except Exception as e:
