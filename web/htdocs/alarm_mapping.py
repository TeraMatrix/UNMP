...
#import logging
#logging.basicConfig(filename='/omd/daemon/debug.log',format='%(levelname)s: %(asctime)s >> %(message)s', level=logging.DEBUG)
        ...
        if len(tuple2d[0]) == 7:
            ...
            for tp in tuple2d:
                if tp[6] != 'odu16':
                    ...
                    odu16_clear_alarm_dict[tp[1]] = tp[5] # This is for clear event type dict.
                    if str(tp[2]) != '9':
            ...
            f.write('\n')
            f.write('odu16_mask_severity_dict='+str(odu16_mask_severity_dict))
            f.write('\n')
    ...
    try:
        ...
        if option == "ADD":
            ...
            for key in device_tp:
                if device_type_dict.has_key(key):
...
def get_trap_lists(h):
    try:
        ...
        if db ==1:
            raise SelfException(cursor)
        html_form=""
        sql="SELECT trap_event_type,trap_clear_mapping_type From trap_id_mapping where device_type = '%s'"%(device_type)
        cursor.execute(sql)
        tp=cursor.fetchall()
        tp1 = sum(tp,())
...
user can define it or if needs to be treated as it is, mark it as default.\
        ...
        "<div class=\"action-tip\"><div class=\"img-div\"><img style=\"width:16px;height:16px;\" src=\"images/new_icons/round_plus.png\"/>\
