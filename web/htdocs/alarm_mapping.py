...
global ap25, ubr, idu4, di, device_type_dict, severity_dict

ap25, ubr, idu4, di, device_type_dict, severity_dict = {}, {}, {}, {}, {}, {}
...
#
#device_type_dict={'ubr':'UBR/UBRe','idu4':'IDU 4 port'}
#
...
def write_mask_file(tuple2d):
    if len(tuple2d) > 0:                                #mask_dict = dict(zip(map(lambda x: x[0],tuple2d),map(lambda x: x[1],tuple2d))) other way
        if len(tuple2d[0]) == 6:
            ...
            for tp in tuple2d:
                mask_dict[tp[0]] = tp[1]
                clear_alarm_dict[tp[1]] = tp[5] # This is for clear event type dict.
                if str(tp[2]) != '9':
...
def start_page(h):
    global html
    html=h
    css_list = ["css/jquery.multiselect.css","css/jquery.multiselect.filter.css","css/demo_table_jui.css","css/jquery-ui-1.8.4.custom.css"]
    js_list = ["js/jquery-ui-1.8.6.custom.min.js","js/pages/jquery.multiselect.min.js","js/pages/jquery.multiselect.filter.js","js/jquery.dataTables.min.js","js/alarmMapping.js"]
    header_btn = "<div class=\"header-icon\"><img class=\"n-tip-image\" src=\"images/new_icons/round_plus.png\" id=\"add_alarm\" style=\"width: 16px; height: 16px; margin: 6px 20px 6px 10px;\" original-title=\"Add Alarm\"></div>"
    html.new_header("Alarm Masking","alarm_mapping.py",header_btn,css_list,js_list)
...
def alarm_datail_function(h):
    ...
    html=h
    try:
        db,cursor=mysql_connection('mahimws') ##hard coded. need to be changed
        if db ==1: SelfException(cursor)
        sql="SELECT trap_id_mapping_id, trap_event_type, trap_severity, trap_clear_mapping_type, clear_severity, device_type, is_alarm, is_deleted From trap_id_mapping "
        cursor.execute(sql)
        ...
        if len(result) < 1 :
            ...
            for row in result_list:
                ...
                debug = 11
                actions = ""
                actions += "<img src=\"images/edit16.png\" alt=\"edit\" style=\"opacity: %s\" title=\"Edit Alarm\" class=\"imgbutton\" onclick=\"editAlarm(\'%s\',\'%s\')\" />"%(str(opac),row[0],flag)
                actions += "&nbsp; &nbsp; &nbsp;<img src=\"images/delete16.png\" alt=\"delete\" style=\"opacity: %s\" title=\"Delete Alarm\" class=\"imgbutton\" onclick=\"checkDeleteAlarm(\'%s\',\'%s\')\" />"%(str(opac),row[0],flag)
                debug = 1
                row.append(actions)
                debug = 2
...
def add_edit_form_show(h):
    ...
    option=html.var("option") #-- its define the add or edit form diplay --#
    try:
        ...
        sql = "SELECT trap_event_type,trap_clear_mapping_type From trap_id_mapping "
        cursor.execute(sql)
        tp = cursor.fetchall()
        tp1 = sum(tp,())
        ...
        if option == "ADD":
            ...
            html_form="<div id=\"Columns\">\
                  ...
                  <div class=\"form-div\">\
                    ...
                    <div class=\"row-elem\">\
                        ...
                        <select name=\"device_type\" id=\"device_type\" class=\"multiselect\" multiple=\"multiple\" title=\"Select Device Type\"> "
                                                
            for key in device_type_dict:
                html_form += "<option value=\"%s\">%s</option>"%(key,device_type_dict[key])
                
            html_form += "</select></div>\
                        <div class=\"row-elem\">\
                            <label  class = \"lbl lbl-big\" for=\"trap_event_type\" align=\"right\">Alarm Type</label >\
                            <select  class=\"multiselect\" multiple=\"multiple\" id=\"trap_event_type\" name=\"trap_event_type\" title=\"Select Trap Event Type\"> "

            html_form+="</select></div>\
                    <div class=\"row-elem\">\
                        <label  class = \"lbl lbl-big\" for=\"trap_severity\" align=\"right\">Severity</label >\
                        <select  class=\"multiselect\" multiple=\"multiple\" id=\"trap_severity\" name=\"trap_severity\" title=\"Select Severity\"> "
                                                
            for i in severity_dict:
                html_form += "<option value=\"%s\">%s</option>"%(i,severity_dict[i])
            ...
            html_form+="</select></div>\
                    <div class=\"row-elem\">\
                        ...
                        <select  class=\"multiselect\" multiple=\"multiple\" id=\"clear_severity\" name=\"clear_severity\" title=\"Select Clear type Severity\"> "
...
#-- its select the information from trap_mapping_id for show on the edit form ----#        
            ...
            html_form="<div id=\"Columns\">\
                   <form id=\"alarm_form_detail\" action=\"add_form_entry.py\" method=\"get\">\
                        <div class=\"form-div\">\
                            ...
                            <div class=\"row-elem\">\
                                <label  class = \"lbl lbl-big\" for=\"device_type\" align=\"right\">Device Type</label >\
                                <select  class=\"multiselect\" multiple=\"multiple\" id=\"device_type\" name=\"device_type\" title=\"Select Device Type\">\
        ...
        cursor.close()
        db.close()
        html.write("{output:{table:'"+str(html_form).replace("\"","\\\"")+"'},success:0}")
    # Exception Handling  
...
def add_form_entry(h):
    ...
    try:
        if len(temp_di) < 0 and event_type == None and device_type == None:
            html.write("{output:{Exception:'Arguments are not proper, Please refresh your webpage and retry again'},success:'1'}")
            return
    except Exception,e:
        html.write("{output:{Exception:'Arguments missing, Please refresh your webpage and retry again'},success:'1'}")
        return    
    ...
    try:
        ...
        if db ==1:
            raise SelfException(cursor)
        sql="SELECT trap_event_type,trap_event_id from trap_id_mapping where trap_event_type='%s' "%(event_type)
        cursor.execute(sql)
        result=cursor.fetchall()

        if len(result) < 1:
            sql="INSERT INTO trap_id_mapping(trap_event_type, trap_event_id, trap_clear_mapping_type, trap_clear_mapping_id, trap_severity, clear_severity, is_alarm, device_type, created_by, creation_time) values \
            ('%s', '%s',  '%s', '%s','%s', '%s', '%s', '%s', '%s', now())\
            "%(event_type, event_id, event_clear_type, clear_event_id, trap_severity,clear_severity, (0 if is_alarm == None or is_alarm == "" else 1),device_type, cr_by)
        ...
        cursor.execute("select trap_event_id, trap_clear_mapping_id, trap_severity, clear_severity, is_deleted,trap_clear_mapping_type from trap_id_mapping where is_alarm = 1")
        tp2d = cursor.fetchall()
        if len(tp2d) > 0:
            write_mask_file(tp2d) # MASK FILE WRITE CODE alarm_mask.rg
        cursor.close()
        db.close()
        html.write("{output:{table:'"+"success full add"+"'},success:'"+str(error_no)+"'}")
...
#=-- database connection creation  and cursor creation --- #
        ...
        if db ==1:
            raise SelfException(cursor)
        sql="UPDATE trap_id_mapping SET trap_event_type = '%s' ,trap_event_id = '%s' , trap_severity = '%s', trap_clear_mapping_id = '%s', trap_clear_mapping_type='%s', clear_severity = '%s', updated_by='%s', is_alarm=%s where trap_id_mapping_id = '%s' " %(event_type, event_id, trap_severity, clear_event_id, event_clear_type, clear_severity, up_by, (0 if is_alarm == None or is_alarm == "" else 1),uniqe_id)      
        cursor.execute(sql)
        db.commit()
        #cursor.execute("select trap_event_type,trap_clear_mapping_type, trap_severity, clear_severity, is_deleted from trap_id_mapping where is_alarm = 1")
        cursor.execute("select trap_event_id, trap_clear_mapping_id, trap_severity, clear_severity, is_deleted,trap_clear_mapping_type from trap_id_mapping where is_alarm = 1")
        tp2d = cursor.fetchall()
        if len(tp2d) > 0:
...
def delete_alarm_id(h):
    ...
    try:
        ...
        if db ==1:
            raise SelfException(cursor)
        sql="DELETE from trap_id_mapping where trap_id_mapping_id = '%s' "% (alarm_delete_id)
        cursor.execute(sql)
        db.commit()
        #cursor.execute("select trap_event_type, trap_clear_mapping_type, trap_severity, clear_severity, is_deleted from trap_id_mapping where is_alarm = 1")
        cursor.execute("select trap_event_id, trap_clear_mapping_id, trap_severity, clear_severity, is_deleted,trap_clear_mapping_type from trap_id_mapping where is_alarm = 1")
...
def page_tip_alarm_mapping(h):
        ...
        "<div>Alarm Masking page show all the alarms masked with current and clear events of device.<br/>\
            ...
            On this page User can add an event as alarm for a device, after that these events works like an actual alarm for UNMP System and will be appear in current and clear event details respectively.<br/>\
Those alarms you can not edit or delete are real alarms of that device.So UNMP System automatically watch them.<br/>\
When adding an event as an alarm, user could modify its severity according need. for an example if NODE_DOWN comes with severity 'Normal' and needs to be treated as 'Major', user can define it or if needs to be treated as it is, mark it as default.\
</div>"\
        "<br/>"\
