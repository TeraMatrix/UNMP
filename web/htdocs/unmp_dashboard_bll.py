#!/usr/bin/python2.6
import config, time, cgi,MySQLdb,sys,csv ,time
    ...
    def common_graph_json(self,device_type,graph_id):
		...
		inner join hosts on ta.agent_id = hosts.ip_address \
		 ...
		 group by serevity,ta.timestamp order by  ta.timestamp desc "
                        ...
                        for row in trap_result:
                            if row[1]>=datetime.now()+timedelta(minutes=-(i+1)*30) and row[1]<=datetime.now()+timedelta(minutes=-i*30):
                                if int(row[2])==0 :
                                    ...
                                    critical1+=int(row[0])
                        time_list.append(time.mktime((datetime.now()+timedelta(minutes=-(i+1)*30)).timetuple())*1000)
                ...
                #critical.reverse()
                #time_list.reverse()
                json_data.append({'normal':normal,'inforamtional':inforamtional,'minor':minor,'major':major,'critical':critical})
		    ...
		    <th >Timestamp</th>\
                ...
                if len(unreachable_device) > 0:
                    for row in unreachable_device:
                        ...
                        second = 0
                        duration = "New"
                        minute,second = (0,0) if (datetime.now()-row[3]).seconds < 60 else ((datetime.now()-row[3]).seconds/60,(datetime.now()-row[3]).seconds%60)
                        hour,minute = (0,minute) if minute < 60 else (minute/60,minute%60)

                        if int(hour) != 0 or int(minute) != 0:
                            duration = str(hour)+" Hr "+str(minute)+" Min"
                        img = "<img src=\"%s\" alt=\"%s\" title=\"%s\" class=\"imgbutton\" style=\"width:12px\" onclick=\"alarmDetail(\'%s\',1)\"/>"%(image_dic[int(row[0])],image_title_name[int(row[0])],image_title_name[int(row[0])],int(row[4]))
                        reachable_table +="\
				<tr>\
				    <td >"+img+"</td>\
				    <td >"+row[1]+"</td>\
		...
		<tr>\
		    ...
		    <th >Timestamp</th>\
                ...
                if len(unreachable_device) > 0:
                    for row in unreachable_device:
                        img = "<img src=\"%s\" alt=\"%s\" title=\"%s\" class=\"imgbutton\" style=\"width:12px\" onclick=\"alarmDetail(\'%s\',1)\"/>"%(image_dic[int(row[0])],image_title_name[int(row[0])],image_title_name[int(row[0])],int(row[6]))
                        reachable_table +="\
				<tr>\
				    <td >"+img+"</td>\
				    <td >"+row[1]+"</td>\
			...
			<tr>\
                ...
                pass                
            output_dic={'success':0,'timestamp':time_list,'data':json_data,'graph_title':graph_title,'graph_sub_title':graph_sub_title,'table_html':table_html}
            return  output_dic
        except MySQLdb as e:
