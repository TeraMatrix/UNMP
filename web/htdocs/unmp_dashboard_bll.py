...
class DashboardBll(object):
	...
	image_title_name = {0:"Normal",1:"Informationl",2:"Normal",3:"Minor",4:"Major",5:"Critical"}
		...
		if len(unreachable_device) > 0:
			for row in unreachable_device:
				img = "<img src=\"%s\" alt=\"%s\" title=\"%s\" class=\"imgbutton\" style=\"width:12px\" onclick=\"alarmDetail(\'%s\',3)\"/>"%(image_dic[int(row[0])],image_title_name[int(row[0])],image_title_name[int(row[0])],int(row[6]))
				reachable_table +="\
				<tr>\
				    <td >"+img+"</td>\
				    <td >"+row[1]+"</td>\
