...
def sp_dashboard_profiling(h):
    ...
    device_name_dict={'odu16':'RM18','odu100':'RM','idu4':'IDU','ap25':'Access Point','ccu':'CCU'}
    ...
    html.new_header(str(device_name_dict[device_type])+" %s Dashboard"%(ip_address),selected_listing,SPDashboardView.header_buttons(),css_list,javascript_list)
    ...
    if device_list_param == [] or device_list_param == None:
        ...
        if  int(output)==1:
            html.write(page_header_search("","","RM18,RM,Access Point,IDU,CCU",None,"enabled","device_type",extr_button))
        else:
                html.write(page_header_search(host_id,mac_address,"RM18,RM,Access Point,IDU,CCU",device_type,"enabled","device_type",extr_button))
    else:
        html.write(page_header_search(device_list_param[0][0],device_list_param[0][1],"RM18,RM,Access Point,IDU,CCU",
    ...
    try:
        ...
        if int(result['success'])==0:
            ...
            if int(select_option)>0:
                ...
                start_date=datetime.strptime(start_date+' '+start_time,"%d/%m/%Y %H:%M")
                end_date=datetime.strptime(end_date+' '+end_time,"%d/%m/%Y %H:%M")
            result_dict=sp_bll_obj.sp_csv_report(device_type_id,user_id,ip_address,cal_list,tab_list,field_list,table_name_list,graph_name_list,
    ...
    try:
        ...
        if int(result['success'])==0:
            ...
            if int(select_option)>0:
                ...
                start_date=datetime.strptime(start_date+' '+start_time,"%d/%m/%Y %H:%M")
                end_date=datetime.strptime(end_date+' '+end_time,"%d/%m/%Y %H:%M")
            result_dict=sp_bll_obj.sp_pdf_report(device_type_id,user_id,ip_address,cal_list,graph_id_list,tab_list,field_list,table_name_list,graph_name_list,
            start_date,end_date,select_option,limitFlag,graph_list,start_list,limit_list)
            h.req.write(str(JSONEncoder().encode(result_dict)))
        else:
            h.req.write(str(JSONEncoder().encode(result)))
