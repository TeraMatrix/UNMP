...
def sp_dashboard_profiling(h):
    ...
    device_name_dict={'odu16':'UBR','odu100':'UBRe','idu4':'IDU','ap25':'Access Point'}
    css_list = ["css/style.css","css/custom.css","calendrical/calendrical.css",'css/ccpl_jquery_combobox.css']
    javascript_list = ["js/highcharts.js",'js/ccpl_jquery_autocomplete.js',"js/specific_dashboard.js","calendrical/calendrical.js"]
#    header_btn = SPDashboardView.header_buttons()
    ...
    html.new_header(str(device_name_dict[device_type])+" %s Dashboard"%(ip_address),"odu_listing.py","",css_list,javascript_list)
...
#    extr_button=[{'tag':'button','id':'adSrhAP','value':'Advance Search','name':'adSrhAP'},{'tag':'button','id':'ap_ad_graph','value':'Advance Graph','name':'ap_ad_graph'}]
    extr_button=[{'tag':'button','id':'sp_ad_graph','value':'Historical Graphs','name':'sp_ad_graph'},{'tag':'button','id':'sp_show_graph_list','value':'Dashboard Configuration','name':'sp_show_graph_list'}]
    ...
    if device_list_param == [] or device_list_param == None:
        ...
        if  int(output)==1:
            html.write(page_header_search("","","UBR,UBRe,Access Point,IDU",None,"enabled","device_type",extr_button))
            #html.write(page_header_search("","","UBR,UBRe",device_type,"enabled","device_type",extr_button))
        else:
                html.write(page_header_search(host_id,mac_address,"UBR,UBRe,Access Point,IDU",device_type,"enabled","device_type",extr_button))
    else:
        html.write(page_header_search(device_list_param[0][0],device_list_param[0][1],"UBR,UBRe,Access Point,IDU",device_list_param[0][2],device_list_state,"device_type",extr_button))
...
def sp_add_date_time_on_slide(h):
    ...
    try:
        ...
        if int(graph_result['success'])==1:
            ...
            output_dict={'success':0,'end_date':sp_end_date,'end_time':sp_end_time,'start_date':sp_start_date,'start_time':sp_start_time,'show_graph_table':SPDashboardView.sp_get_graph(graph_result['selected'],graph_result['non_selected'])}
...
#    sp_bll_obj=SPDashboardBll()
    ...
    if int(result['success'])==0:
        html.write(SPDashboardView.sp_table(result['output'],sp_start_date,sp_start_time,sp_end_date,sp_end_time,str(sp_refresh_time),str(total_count)))
    else:
...
def sp_device_details(h):
    ...
    if int(output['success'])==0:
        view_output=SPDashboardView.device_information_view(output,ip_address)
...
def sp_event_alarm_information(h):
    ...
    if output['success']==0 or output['success']==0:
        event_alarm_output=SPDashboardView.sp_event_alarm_table_view(output,ip_address)
...
def sp_common_graph_creation(h):
    ...
    if int(graph_type)==2:
        ap_advanced_graph_controller.advanced_graph_creation(h) # This is call a advanced graph function
    else:
        ...
        result_dict=sp_bll_obj.common_graph_json(display_type,user_id,table_name[0],table_name[1],table_name[-2],table_name[-1],flag,start_date,end_date,start,limit,ip_address,graph_type,update_field_name,interface_value,cal_type,column_name)
        h.req.content_type = 'application/json'
        h.req.write(str(JSONEncoder().encode(result_dict)))
