...
class DashboardConfig(object):
    ...
    def get_config_attributes(tag_name,attribute_name,get_text = False):
        ...
        '''
        dom = DashboardConfig.load_config_file()                        # create xml dom of config file
        ...
        for elem in dom_element:                                    # iterate selected elements  
            ...
            if get_text == True:
                text_value.append(DashboardConfig.get_tag_text(elem.childNodes))    # append text value in text_value if get_text is True
        if get_text == True:
