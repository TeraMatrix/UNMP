...
import config, htmllib, pprint, sidebar, views, time, defaults, os, cgi, subprocess, commands, MySQLdb, datetime,os.path
...
def update(h):	
    ...
    html.write("<table class=\"addform\" style=\"border:0px none;\"><colgroup><col width=\"140px\"/><col width=\"auto\"/></colgroup>")
    html.write("<tr><td>Select Devices:</td><td><select name=\"select_devices\" id=\"select_devices\">")
    if(os.path.isfile(path)):
        f=parse(path)
        try:
            for node in f.getElementsByTagName('device'):
                if(i==0):
                    html.write("<option selected=\"true\" value=\"NoDevice\">-- Select Device Type --</option>")
                i += 1
                html.write("<option value=\"%s\">%s</option>" % (node.getAttribute('id'),node.getAttribute('name')))
            if(i==0):
                html.write("<option selected=\"true\" value=\"\">-- No Device Found --</option>")
        except Exception, e:
            html.write("<option selected=\"true\" value=\"\">-- No Device Found --</option>")
    else:
        html.write("<option selected=\"true\" value=\"\">-- No Device Found --</option>")
    html.write("</select></td><td rowspan=\"2\"><div id=\"image_load\"></div></td></tr>")
    html.write("<tr><td>")
...
def device_table(h):
    ...
    if device_type is not None:
        ...
        sql = "SELECT hostname,ipaddress,devicetype FROM nms_devices where devicetype='%s';" %(device_type)
