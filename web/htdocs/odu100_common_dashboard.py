...
def odu100_common_dashboard(h):
    ...
    javascript_list = ["js/highcharts.js","js/odu100CommonDashboard.js"]
...
def odu100_common_dashboard_report_generating(h):
	...
	# function calling
        ...
        cursor.close()
	data1=[]
	data1.append(['','UBRe Signal Strength'+str(signal_interface[int(ss_interface)]),'',''])
        t = Table(data1, [.021*inch,2.45 * inch, 2.3 * inch, 2.2*inch])
	t.setStyle(TableStyle([('BACKGROUND',(1,0),(1,0),(0.35,0.35,0.35)),('FONT', (0, 0), (1, 0), 'Helvetica', 11),('TEXTCOLOR',(1,0),(2,0),colors.white)]))
	odu100_common_report.append(t)
	...
	# calling the procedure 
        ...
        current_datetime=datetime.strptime(str(current_date)+" 00:00:00",'%Y-%m-%d %H:%M:%S') # convert the string in  datetime. 
        last_datetime=datetime.strptime(str(current_date)+" 23:59:59",'%Y-%m-%d %H:%M:%S')
	sel_query="SELECT ip_address FROM hosts WHERE device_type_id='odu100' order by ip_address limit %s,%s"%(int(start),int(limit))
	cursor.execute(sel_query)
        ip_result=cursor.fetchall()
	for ip in ip_result:
	    ip_address_list.append(ip[0])
...
def ubr_outage_calculation(start_date,end_date,ip_address_list):
	ip_address=[] # this list store the days information with date.
		...
		ip_address.append(row[1])
            ...
            output_dict={'success':0,'up_state':up_state,'down_state':down_state,'odu_name':ip_address}
            return output_dict 
    ...
    try:
		...
		if temp_ip == main_ip:
			...
			if day_diff > 1:
			    for i in range(1,day_diff+1):
			        ...
			        if prev_value == '50002':
			            main_list.append([leftout_date,main_ip,timedelta(0, 86399),None])
			        elif prev_value == '50001':
			            main_list.append([leftout_date,main_ip,None,timedelta(0, 86399)])
		...
		if is_date:
			...
			if prev_value == '50002':
			    ...
			    downtime = delta
		    main_list.append([main_date,main_ip,uptime,downtime])
		    main_date = temp_date
		    uptime = None
		    downtime = None
		    is_date = 0
