...
# UPNP class for getting, sending and parsing SSDP/SOAP XML data (among
# other things...)
...
# self.csock.setsockopt(SOL_SOCKET,IN.SO_BINDTODEVICE,struct.pack("%ds" %
# (len(self.IFACE)+1,), self.IFACE))
    ...
    def parseURL(self, url):
        ...
        try:
            ...
            # If '://' is not in the url, then it's not a full URL, so assume
            # that it's just a relative path
            page = url
    ...
    def parseSSDPInfo(self, data, showUniq, verbose):
        ...
        if messageType != False:
            ...
            # 2. We've already seen this host, but the uniq hosts setting is
            # disabled
            ...
            if (hostFound and not self.UNIQ) or not hostFound:
                # Get the new host's index number and create an entry in
                # ENUM_HOSTS
            ...
            # print " Host ip : ",host.split(':')[0]
            if upnpType:
                upnp_host_dict[host.split(':')[0]] = [upnpType.split(' ')[0]]
            else:
                upnp_host_dict[host.split(':')[0]] = ['']
            ...
            # if upnpType:
                ...
                # print " Device is running : ",upnpType.split(' ')[0]
...
#            print ''
    ...
    def sendSOAP(self, hostName, serviceType, controlURL, actionName, actionArguments):
        ...
        # Check if a port number was specified in the host name; default is
        # port 80
        if ':' in hostName:
    ...
    def getHostInfo(self, xmlData, xmlHeaders, index):
        ...
        if index >= 0 and index < len(self.ENUM_HOSTS):
            try:
                ...
                self.ENUM_HOSTS[index][
                    'serverType'] = xmlHeaders.getheader('Server')
    ...
    def parseDeviceInfo(self, xmlRoot, index):
        ...
        deviceTags = [
            "friendlyName", "modelDescription", "modelName", "modelNumber",
        ...
        for device in xmlRoot.getElementsByTagName(devTag):
            ...
            # Create a new device entry for this host in the ENUM_HOSTS
            # structure
            deviceEntryPointer = self.ENUM_HOSTS[index][
        ...
        try:
            ...
            for action in actions:
                ...
                try:
                    ...
                    # Some actions may take no arguments, so continue without
                ...
                # Loop through the action's arguments, appending them to the
                ...
                for argument in arguments:
                    ...
                    # Get each required argument tag value and add them to
                    # ENUM_HOSTS
                    for tag in argTags:
                        try:
    ...
    def parseServiceStateVars(self, xmlRoot, servicePointer):
        ...
        try:
            stateVars = xmlRoot.getElementsByTagName(
                serviceStateTable)[0].getElementsByTagName(stateVariable)
        except:
            # Don't necessarily want to throw an error here, as there may be no
        ...
        for var in stateVars:
            for tag in varVals:
                ...
                try:
                    servicePointer['serviceStateVariables'][varName][
                        'dataType'] = str(var.getElementsByTagName(dataType)[0].childNodes[0].data)
                except:
                    servicePointer[
                        'serviceStateVariables'][varName]['dataType'] = na
                try:
                    servicePointer['serviceStateVariables'][varName]['sendEvents'] = str(
                        var.getElementsByTagName(sendEvents)[0].childNodes[0].data)
                except:
                    servicePointer[
                        'serviceStateVariables'][varName]['sendEvents'] = na

                servicePointer['serviceStateVariables'][
                    varName][allowedValueList] = []
                ...
                try:
                    ...
                    # Add the list of allowed values to the ENUM_HOSTS
                    # dictionary
                    for val in vals:
                        servicePointer['serviceStateVariables'][
                            varName][allowedValueList].append(str(val.childNodes[0].data))
                ...
                try:
                    ...
                    servicePointer[
                        'serviceStateVariables'][varName][allowedValueRange] = []
                    try:
...
def msearch(hp, time_out=10):
    ...
    # Have to create a new socket since replies will be sent directly to our
    # IP, not the multicast IP
        ...
        if action == 'list':
                ...
                if index < 0 or index >= len(hp.ENUM_HOSTS):
                    ...
                    try:
                        ...
                        if hostInfo != False:
                            ...
                            if hostInfo['dataComplete'] == False:
                                (xmlHeaders,
                                 xmlData) = hp.getXML(hostInfo['xmlFile'])
                                if xmlData == False:
            ...
            if argc != 6:
                ...
                try:
                    ...
                    controlURL2 = hostInfo['deviceList'][
                        deviceName]['services'][serviceName]['controlURL']
                ...
                try:
                    actionArgs = hostInfo['deviceList'][deviceName][
                        'services'][serviceName]['actions'][actionName]['arguments']
                    fullServiceName = hostInfo['deviceList'][
                        deviceName]['services'][serviceName]['fullName']
                except Exception, e:
                    print 'Caught exception:', e
                ...
                for argName, argVals in actionArgs.iteritems():
                    ...
                    stateVar = hostInfo['deviceList'][deviceName][
                        'services'][serviceName]['serviceStateVariables'][actionStateVar]
    ...
    try:
        ...
        for (opt, arg) in opts:
            if opt == '-s':
                ...
                # Get a list of network interfaces. This only works on unix
                # boxes.
                try:
...
def upnp_discover(time_out):
    ...
    try:
        ...
        return upnp_host_dict

# print upnp_discover(3)
