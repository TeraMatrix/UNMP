...
##print idu_device_list_bll_obj.idu_device_list_profiling('172.22.0.105','11:22:11:22:44:11','idu4')
...
class IduReconcilation(object):
    ...
    def default_reconciliation_controller(self,host_id,device_type_id,table_prefix,insert_update):
        global sqlalche_obj
        sqlalche_obj.sql_alchemy_db_connection_open()
        new_profile = Odu16ConfigProfiles(device_type_id,"IduProfile","Master",None,datetime.now(),None,datetime.now(),None,0)
        ...
        idu_poe_data = sqlalche_obj.session.query(IduPoeConfigurationTable).filter(IduPoeConfigurationTable.config_profile_id == default_id).all()
        if len(idu_poe_data)>0:
            for i in range(0,len(idu_poe_data)):
                idu_poe_data_add = IduPoeConfigurationTable(new_profile_id,idu_poe_data[i].indexId,idu_poe_data[i].poeAdminStatus)
                sqlalche_obj.session.add(idu_poe_data_add)

        idu_e1_port_data = sqlalche_obj.session.query(IduE1PortConfigurationTable).filter(IduE1PortConfigurationTable.config_profile_id == default_id).all()
        if len(idu_e1_port_data)>0:
            for i in range(0,len(idu_e1_port_data)):
                idu_e1_port_data_add = IduE1PortConfigurationTable(new_profile_id,idu_e1_port_data[i].portNumber,idu_e1_port_data[i].adminState,idu_e1_port_data[i].clockSource,idu_e1_port_data[i].lineType,idu_e1_port_data[i].lineCode)
                sqlalche_obj.session.add(idu_e1_port_data_add)

        idu_switch_port_data = sqlalche_obj.session.query(IduSwitchPortconfigTable).filter(IduSwitchPortconfigTable.config_profile_id == default_id).all()
        if len(idu_switch_port_data)>0:
            for i in range(0,len(idu_switch_port_data)):
                idu_swtch_port_data_add = IduSwitchPortconfigTable(new_profile_id,idu_switch_port_data[i].switchportNum,idu_switch_port_data[i].swadminState,\
                                                                   idu_switch_port_data[i].swlinkMode,idu_switch_port_data[i].portvid,idu_switch_port_data[i].macauthState,\
        ...
        idu_temperature_data = sqlalche_obj.session.query(IduTemperatureSensorConfigurationTable).filter(IduTemperatureSensorConfigurationTable.config_profile_id == default_id).all()
        ...
        idu_mirroring_data = sqlalche_obj.session.query(IduMirroringportTable).filter(IduMirroringportTable.config_profile_id == default_id).all()
        if len(idu_mirroring_data)>0:
            for i in range(0,len(idu_mirroring_data)):
                idu_mirroring_data_add = IduMirroringportTable(new_profile_id,idu_mirroring_data[i].mirroringindexid,idu_mirroring_data[i].mirroringport)
                sqlalche_obj.session.add(idu_mirroring_data_add)

        idu_qinq_data = sqlalche_obj.session.query(IduPortqinqTable).filter(IduPortqinqTable.config_profile_id == default_id).all()
        if len(idu_qinq_data)>0:
            for i in range(0,len(idu_qinq_data)):
                idu_qinq_data_add = IduPortqinqTable(new_profile_id,idu_qinq_data[i].switchportnumber,idu_qinq_data[i].portqinqstate,idu_qinq_data[i].providertag)
                sqlalche_obj.session.add(idu_qinq_data_add)

        idu_bandwidth_data = sqlalche_obj.session.query(IduPortBwTable).filter(IduPortBwTable.config_profile_id == default_id).all()
        if len(idu_bandwidth_data)>0:
            for i in range(0,len(idu_bandwidth_data)):
                idu_bandwidth_data_add = IduPortBwTable(new_profile_id,idu_bandwidth_data[i].switchportnum,idu_bandwidth_data[i].egressbwvalue,idu_bandwidth_data[i].ingressbwvalue)
                sqlalche_obj.session.add(idu_bandwidth_data_add)

        idu_rtc_data = sqlalche_obj.session.query(IduRtcConfigurationTable).filter(IduRtcConfigurationTable.config_profile_id == default_id).all()
        if len(idu_rtc_data)>0:
            for i in range(0,len(idu_rtc_data)):
                idu_rtc_data_add = IduRtcConfigurationTable(new_profile_id,idu_rtc_data[i].rtcIndex,idu_rtc_data[i].year,idu_rtc_data[i].month,\
                                                            idu_rtc_data[i].day,idu_rtc_data[i].hour,\
                                                            idu_rtc_data[i].min,idu_rtc_data[i].sec)
                sqlalche_obj.session.add(idu_rtc_data_add)

        idu_omc_data = sqlalche_obj.session.query(IduOmcConfigurationTable).filter(IduOmcConfigurationTable.config_profile_id == default_id).all()
        if len(idu_omc_data)>0:
            for i in range(0,len(idu_omc_data)):
                idu_omc_data_add = IduOmcConfigurationTable(new_profile_id,idu_omc_data[i].omcIndex,idu_omc_data[i].omcIpAddress,idu_omc_data[i].periodicStatsTimer)
                sqlalche_obj.session.add(idu_omc_data_add)

        idu_admin_data = sqlalche_obj.session.query(IduIduAdminStateTable).filter(IduIduAdminStateTable.config_profile_id == default_id).all()
        if len(idu_admin_data)>0:
            for i in range(0,len(idu_admin_data)):
                idu_admin_data_add = IduIduAdminStateTable(new_profile_id,idu_admin_data[i].stateId,idu_admin_data[i].adminstate)
        ...
        idu_info_data_add = IduIduInfoTable(host_id,1,None,None,None,None,None,None,None,None,None,datetime.now())
            ...
            if host_id =="" or host_id == None:
                ...
                if len(device_param_list)==0:
                    if device_param_list[0].config_profile_id == None or device_param_list[0].config_profile_id == "":
                        result = {"success":1,"result":"Configuration Not Exist For This Host"}
                        return result
                    else:
                        result = {"success":1,"result":"Host Not Exist"}
                        return result
                    ...
                    query_result = sqlalche_obj.session.query(IduOidTable.table_name,IduOidTable.table_oid).distinct().filter(not_(IduOidTable.table_name.in_((fill_not_table)))).all()
                    var_bind = sqlalche_obj.session.query(IduOidTable.table_name,IduOidTable.varbinds).distinct().filter(not_(IduOidTable.table_name.in_((fill_not_table)))).all()
...
##                                else:
                                time.sleep(1)
                                result = bulktable(str(oid_table_dict[i])+".1",device_param_list[0].ip_address,int(device_param_list[0].snmp_port),device_param_list[0].snmp_read_community,var_bind_dic[i])                                
                                ...
                                if result["success"] == 1:
                                    ...
                                    rec = rec + 1
                                    exec "sqlalche_obj.session.query(%s).filter(%s.config_profile_id == %s).delete()"%(sql_table_name,sql_table_name,device_param_list[0].config_profile_id)
                                else:
                                    ...
                                    if "config_profile_id" in column_list:
                                        exec "sqlalche_table_result = sqlalche_obj.session.query(%s).filter(%s.config_profile_id==%s).all()"%(sql_table_name,sql_table_name,device_param_list[0].config_profile_id)
                                    else:
                                        exec "sqlalche_table_result = sqlalche_obj.session.query(%s).filter(%s.host_id==%s).all()"%(sql_table_name,sql_table_name,host_id)
                                    ...
                                    if len(sqlalche_table_result)>0:
                                        ...
                                        for row in result['result']:
                                            if config_type ==1:
                                                sqlalche_obj.db.execute("Insert into %s values(NULL,%s,%s)"%(tablename,device_param_list[0].config_profile_id,str(result["result"][row])[1:-1]))
                                            else:
                                                if timestamp == 0:
...
##                                                    else:
##                                                        sqlalche_obj.db.execute("Insert into %s values(NULL,%s,%s,'%s')"%(tablename,host_id,str(result["result"][i])[1:-1],datetime.now()))                                        


                            reconcile_per = (float(rec)/float(total_per))
                            reconcile_per = int(reconcile_per * 100)

                            device_param_list[0].reconcile_health = reconcile_per
                            ...
                            sqlalche_obj.session.commit()   
                            result = {"success":0,"result":{reconcile_per:[device_param_list[0].host_name,device_param_list[0].ip_address]}}
                            return result
                        else:
                            result = {"success":1,"result":"UNMP has shut down.Please contact UNMP vendor"}
                            device_param_list[0].reconcile_status = 0
                            sqlalche_obj.session.commit()
                            return result
                    else:
                        result = {"success":1,"result":"%s(%s) Device may not be connected or may be Rebooting"%(device_param_list[0].host_name,device_param_list[0].ip_address)}
...
##                                    else:
##                                        result = pysnmp_get_table(oid_table_dict[i],device_param_list[0].ip_address,int(device_param_list[0].snmp_port),device_param_list[0].snmp_read_community)
                                    time.sleep(2)
                                    result = bulktable(str(oid_table_dict[i])+".1",device_param_list[0].ip_address,int(device_param_list[0].snmp_port),device_param_list[0].snmp_read_community,var_bind_dic[i])
                                    if result["success"] == 1:
                                        if int(rec)>0:
...
##                                                    else:
##                                                        sqlalche_obj.db.execute("insert into %s values(NULL,%s,%s,%s)"%(tablename,host_id,j,str(result["result"][j])[1:-1]))
                        ...
                        device_param_list[0].reconcile_status = 0
                        sqlalche_obj.session.commit()
                        config_profile_id,reconcile_per = self.default_reconciliation_controller(host_id,device_type_id,table_prefix,reconcile_chk)
                        return str(config_profile_id),reconcile_per
                else:
                    config_profile_id,reconcile_per = self.default_reconciliation_controller(host_id,device_type_id,table_prefix,reconcile_chk)
                    return str(config_profile_id),reconcile_per
    ...
    def reconciliation_status(self):
        ...
        rec_dir = {}
        sqlalche_obj.sql_alchemy_db_connection_open()
        rec_list = sqlalche_obj.session.query(Hosts.host_id,Hosts.reconcile_status,Hosts.reconcile_health).filter(and_(Hosts.device_type_id.like("idu4%"))).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
        if len(rec_list)>0:
    ...
    def commit_to_flash(self,host_id,device_type_id):
        ...
        host_param = sqlalche_obj.session.query(Hosts.ip_address,Hosts.snmp_port,Hosts.snmp_write_community).filter(Hosts.host_id==host_id).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
        if len(host_param)>0:
    ...
    def reboot(self,host_id,device_type_id):
        ...
        sqlalche_obj.sql_alchemy_db_connection_close()
        if len(host_param)>0:
            snmp_get_result = pysnmp_get('1.3.6.1.4.1.26149.2.1.5.1.1.7',host_param[0].ip_address,int(host_param[0].snmp_port),host_param[0].snmp_read_community)            
            if snmp_get_result['success']==0:
                if int(snmp_get_result['result'].values()[0]) == 1:
                    snmp_get_om_operation = pysnmp_get('1.3.6.1.4.1.26149.2.1.5.1.1.2',host_param[0].ip_address,int(host_param[0].snmp_port),host_param[0].snmp_read_community)
...
class IduGetData(object):
    def common_get_data(self,class_name,host_id, config=True):
        try:
            ...
            if config:
                ...
                if len(config_id) <= 0:
                    return []
                exec_string = "get_data = sqlalche_obj.session.query(%s).filter(%s.config_profile_id==%s).all()"%(class_name,class_name,config_id[0].config_profile_id)
            else:            
                exec_string = "get_data = sqlalche_obj.session.query(%s).filter(%s.host_id==%s).all()"%(class_name,class_name,host_id)
    ...
    def common_get_multivalue(self,device_type, oid_name, field_value):
        try:
            ...
            val_dict = {'device_type':device_type, 'oid_name':oid_name, 'field_value':field_value }
    ...
    def get_e1_op_status(self,host_id):
        try:
            ...
            if host_id == "" or host_id == None:
                return []
            get_data = sqlalche_obj.session.query(IduE1PortStatusTable).filter(IduE1PortStatusTable.host_id==host_id).order_by(desc(IduE1PortStatusTable.timestamp)).order_by(asc(IduE1PortStatusTable.portNum)).limit(4).all()
            if len(get_data)>0:
                return get_data
            else:
                return []
...
class IduCommonSetValidation(object):
    ...
    ##return dic_result
        try:  
                        ...
                        if query_result[0][0].dependent_id == "" or query_result[0][0].dependent_id == None:
                            ...
                            if len(dependent_oid) > 0:
                                for i in range(0,len(dependent_oid)):
                                    ...
                                    if dependent_oid[i].has_key(admin_state):
                                        ...
                                        if len(depend_oid_value) > 0:
                                            depend_oid_value[pos][keys] = [query_result[0][0].oid + "."+str(id),query_result[0][0].oid_type,dic_result[keys]]
                                            break
                                        else:

                                            depend_oid_value.append({keys:[query_result[0][0].oid + + "."+str(id),query_result[0][0].oid_type,dic_result[keys]]})
                                            break
                                    else:
                                ...
                                if query_result[0][1] == 'ru.ruConfTable.adminstate':
                                    ...
                                    dependent_oid.append({query_result[0][1]:[query_result[0][2]+"."+str(id)]})
                                    depend_oid_value.append({keys:[query_result[0][0].oid+"."+str(id),query_result[0][0].oid_type,dic_result[keys]]})                                
                    else:
                    ...
                    if result["success"] == 0 or result["success"] == '0':                    
                        ...
                        for i in result["result"]:
                            if result["result"][i] != 0:
                                ...
                                oid_list_table_field_value = sqlalche_obj.session.query(IduOids.table_name,IduOids.coloumn_name).filter(and_(IduOids.oid_name == i,IduOids.device_type_id == device_type_id)).all()
            ...
            if len(dependent_oid)>0:  
                for i in range(0,len(dependent_oid)):               
                    query_admin_result = sqlalche_obj.session.query(IduOids.oid,IduOids.oid_type,IduOids.indexes).filter(IduOids.oid_name == "ru.ra.raConfTable.raAdminState").one()                            
                    if dependent_oid[i].has_key('ru.ruConfTable.adminstate-1'):                    
                        admin_state = "ru.ruConfTable.adminstate"
                        query_admin_result = sqlalche_obj.session.query(IduOids.oid,IduOids.oid_type,IduOids.indexes).filter(IduOids.oid_name == "ru.ruConfTable.adminstate").one()
                        ...
                        query_admin_result = sqlalche_obj.session.query(IduOids.oid,IduOids.oid_type,IduOids.indexes).filter(IduOids.oid_name == "ru.ipConfigTable.adminState").one()
                            ...
                            for i in result["result"]:
                                if result["result"][i] != 0:
                                    ...
                                    if len(oid_list_table_field_value) == 0:
                                        ...
                                        if i in dic_result:
                                            sql_table_name = "idu_" + oid_list_table_field_value[0][0]
...
##                                    success_result["result"] = errorStatus[result["result"][i]]
            ...
            if dic_result["success"] == '0' or dic_result["success"] == 0:
                for keys in dic_result.iterkeys():
                    if keys == "success":
                        ...
                        oid_list_min_max_value = sqlalche_obj.session.query(IduOids.min_value,IduOids.max_value).filter(and_(IduOids.oid_name == keys,IduOids.device_type_id == device_type_id)).all()
            ...
            if dic_result["success"] == 0:
                for keys in dic_result:
                    if keys == "success":
                        ...
                        oid_list_table_field_value = sqlalche_obj.session.query(IduOids.table_name,IduOids.coloumn_name).filter(and_(IduOids.oid_name == keys,IduOids.device_type_id == device_type_id)).all()
                        ...
                        if id==None:
                            ...
                            str_table_obj = "table_result = sqlalche_obj.session.query(%s.%s).filter(and_(%s.config_profile_id == \"%s\",%s.%s == \"%s\")).all()"%(table_name,oid_list_table_field_value[0][1],table_name,profile_id[0],table_name,primary_key_id,id)
                        exec str_table_obj
                        print table_result
                        if len(table_result)>0:
    ...
    def vlan_set(self,host_id,device_type_id,dic_result,addEdit,id,index=None):
        try:
            ...
            if int(addEdit) == 0:
                ...
                row_status_val = 1
            dic_result.update({'switch.vlanconfigTable.vlantype':1,'switch.vlanconfigTable.vlanrowstatus':row_status_val})
            global sqlalche_obj
            sqlalche_obj.sql_alchemy_db_connection_open()
            result = {}
            o1 = aliased(IduOids)
                ...
                if keys == "success":
                    ...
                    if len(query_result)>0:
                        try:
                            ...
                            key = dic_result[keys]
                        independent_oid.append({keys:[query_result[0][0].oid+"."+str(id),query_result[0][0].oid_type,key]})

            for i in independent_oid:
                for keys in i.iterkeys():
                    vlan_data.update(i)
            result = pysnmp_set(vlan_data,host_data[0].ip_address,host_data[0].snmp_port,host_data[0].snmp_write_community)
            if result["success"] == 0 or result["success"] == '0':
                if int(addEdit) == 0:
                    vlan_add_data = IduVlanconfigTable(host_data[0].config_profile_id,id,dic_result['switch.vlanconfigTable.vlanname'],\
                                                       dic_result['switch.vlanconfigTable.vlantype'],\
                                                       dic_result['switch.vlanconfigTable.vlantag'],\
                    ...
                    for i in result["result"]:
                        if result["result"][i] != 0:
                            ...
                            if i in dic_result:
                                ...
                                if len(oid_list_table_field_value)==0:
                                    continue
                                tablename = "idu_" + oid_list_table_field_value[0].table_name
                                sql_table_name = rename_tablename(tablename)                            
                                exec "table_result=sqlalche_obj.session.query(%s).filter(and_(%s.config_profile_id=='%s',%s.vlanid=='%s')).all()"%(sql_table_name,sql_table_name,host_data[0].config_profile_id,sql_table_name,id)
                                if len(table_result)>0:
    ...
    def delete_vlan_port(self,host_id,id):
        try:
            if host_id==None:
                result = {"success":1,"result":"No Host Exist.Please Reconcile or add the host properly"}
            else:
                global errorStatus 
                global sqlalche_obj
                sqlalche_obj.sql_alchemy_db_connection_open()
                host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id==host_id).all()
                result = pysnmp_set({'row_stats':['1.3.6.1.4.1.26149.2.1.6.5.1.6.%s'%(id),'Integer32',6]},host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_write_community)
                if result["success"] == 0 or result["success"] == '0':
                    vlan_delete = sqlalche_obj.session.query(IduVlanconfigTable).filter(and_(IduVlanconfigTable.config_profile_id==host_data[0].config_profile_id,IduVlanconfigTable.vlanid==id)).delete() 
                    sqlalche_obj.session.commit()
                else:
                    if 53 in result["result"]:
                        result = {"success":1,"result":"No Response From Device.Please Try Again"}
    ...
    def e1_port_set(self,host_id,device_type_id,dic_result,id,index):
        try:
            ...
            result = pysnmp_set(e1_port_data,host_data[0].ip_address,host_data[0].snmp_port,host_data[0].snmp_write_community,{'adminState':['1.3.6.1.4.1.26149.2.1.2.3.1.2.%s'%(id),'Integer32',0]})
            if result["success"] == 0 or result["success"] == '0':                
                for i in result["result"]:
                    if result["result"][i] != 0:
                        result["result"][i] = errorStatus[result["result"][i]]
                        ...
                        if i=="adminState1":
                            ...
                            tablename = "idu_" + oid_list_table_field_value[0].table_name
                            sql_table_name = rename_tablename(tablename)
                            exec "table_result=sqlalche_obj.session.query(%s).filter(and_(%s.config_profile_id=='%s',%s.portNumber=='%s')).all()"%(sql_table_name,sql_table_name,host_data[0].config_profile_id,sql_table_name,id)
                            if len(table_result)>0:
                                exec "table_result[0].%s='%s'"%(oid_list_table_field_value[0].coloumn_name,1)
    ...
    def check_vlan_tag(self,vlan_tag,host_id):
        ...
        vlan_data = sqlalche_obj.session.query(IduVlanconfigTable.vlantag).filter(IduVlanconfigTable.config_profile_id==host_data[0].config_profile_id).all()
        flag = 0
        sqlalche_obj.sql_alchemy_db_connection_close()
        if len(vlan_data)>0:
    ...
    def check_vlan_name(self,vlan_name,host_id):
        ...
        sqlalche_obj.sql_alchemy_db_connection_open()
        host_data = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id==host_id).all()
        vlan_data = sqlalche_obj.session.query(IduVlanconfigTable.vlanname).filter(IduVlanconfigTable.config_profile_id==host_data[0].config_profile_id).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
        if len(vlan_data)>0:
            for i in range(0,len(vlan_data)):
    ...
    def chk_link_e1_port(self,host_id,port_id):
        ...
        sqlalche_obj.sql_alchemy_db_connection_open()
        host_data = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id==host_id).all()
        e1_port_data = sqlalche_obj.session.query(IduLinkConfigurationTable).filter(and_(IduLinkConfigurationTable.config_profile_id==host_data[0].config_profile_id,IduLinkConfigurationTable.portNumber==port_id)).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
                    ...
                    if len(query_result)>0:
                        if keys == "iduConfiguration_linkConfigurationTable_tsaAssign":
                            ...
                            try:
                                ...
                                key = dic_result[keys]
                        independent_oid.append({keys:[query_result[0][0].oid+"."+str(index)+"."+str(id),query_result[0][0].oid_type,key]})

            for i in independent_oid:
                for keys in i.iterkeys():
                    link_port_data.update(i)
            if int(addEdit)==1:
                result = pysnmp_set(link_port_data,host_data[0].ip_address,host_data[0].snmp_port,host_data[0].snmp_write_community,{'adminState':['1.3.6.1.4.1.26149.2.1.2.2.1.3.%s.%s'%(index,id),'Integer32',0]})
            else:
                result = pysnmp_seter(link_port_data,host_data[0].ip_address,host_data[0].snmp_port,host_data[0].snmp_write_community,{'iduConfiguration.linkConfigurationTable.rowStatus':['1.3.6.1.4.1.26149.2.1.2.2.1.11.%s.%s'%(index,id),'Integer32',4]})
            if result["success"] == 0 or result["success"] == '0':      
                if int(addEdit)==1:          
                    for i in result["result"]:
                        if result["result"][i] != 0:
                    ...
                    add_link_row = IduLinkConfigurationTable(host_data[0].config_profile_id,index,id,0,\
                                                             dic_result['iduConfiguration.linkConfigurationTable.srcBundleID'],\
                                                             dic_result['iduConfiguration.linkConfigurationTable.dstBundleID'],\
                                                             dic_result['iduConfiguration.linkConfigurationTable.dstIPAddr'],\
                                                             dic_result['iduConfiguration_linkConfigurationTable_tsaAssign'],\
...
class LinkConfiguration(object):
    ...
    def delete_link_port(self,host_id,id,port_number,link_number):
        try:
            if host_id==None:
                ...
                sqlalche_obj.sql_alchemy_db_connection_open()
                host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id==host_id).all()
                result = pysnmp_set({'row_stats':['1.3.6.1.4.1.26149.2.1.2.2.1.11.%s.%s'%(port_number,link_number),'Integer32',6]},host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_write_community)
                if result["success"] == 0 or result["success"] == '0':
                    link_delete = sqlalche_obj.session.query(IduLinkConfigurationTable).filter(IduLinkConfigurationTable.idu_linkConfigurationTable_id==id).delete() 
                    sqlalche_obj.session.commit()
                else:
                    if 53 in result["result"]:
                        result = {"success":1,"result":"No Response From Device.Please Try Again"}
    ...
    def selected_timeslot(self,host_id,port_num):
        ...
        sqlalche_obj.sql_alchemy_db_connection_open()
        host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id==host_id).all()
        timeslot = sqlalche_obj.session.query(IduLinkConfigurationTable).filter(and_(IduLinkConfigurationTable.config_profile_id==host_data[0].config_profile_id,IduLinkConfigurationTable.portNumber==port_num)).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
        if len(timeslot)>0:
    ...
    def link_chk(self,host_id,link_num):
        ...
        link_data = []
        host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id==host_id).all()
        link_data = sqlalche_obj.session.query(IduLinkConfigurationTable).filter(and_(IduLinkConfigurationTable.config_profile_id==host_data[0].config_profile_id,IduLinkConfigurationTable.bundleNumber==link_num)).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
        if link_data==[]:
            return 0
        else:
    ...
    def src_bundle_chk(self,host_id,src_bundle_id,link,port):
        ...
        flag = 0
        host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id==host_id).all()
        src_bundle_data = sqlalche_obj.session.query(IduLinkConfigurationTable.srcBundleID).filter(IduLinkConfigurationTable.config_profile_id==host_data[0].config_profile_id).all()
        src_bundle_set = sqlalche_obj.session.query(IduLinkConfigurationTable.srcBundleID).filter(and_(IduLinkConfigurationTable.config_profile_id==host_data[0].config_profile_id,\
    ...
    def destination_bundle_chk(self,host_id,dst_bundle_id,link,port):
        ...
        sqlalche_obj.sql_alchemy_db_connection_open()
        host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id==host_id).all()
        dst_bundle_data = sqlalche_obj.session.query(IduLinkConfigurationTable.dstBundleID).filter(IduLinkConfigurationTable.config_profile_id==host_data[0].config_profile_id).all()
        dst_bundle_set = sqlalche_obj.session.query(IduLinkConfigurationTable.dstBundleID).filter(and_(IduLinkConfigurationTable.config_profile_id==host_data[0].config_profile_id,\
                                                                                                       IduLinkConfigurationTable.bundleNumber==link,IduLinkConfigurationTable.portNumber==port)).all()
    ...
    def link_count(self,host_id):
        try:
            ...
            all_link_count = sqlalche_obj.session.query(IduLinkConfigurationTable).\
                filter(IduLinkConfigurationTable.config_profile_id==host_data[0].config_profile_id).count()
            if all_link_count>0:
                ...
                total_link_unlocked = float(link_data_count)/float(all_link_count)
        ...
        try:
            ...
            if host_ids == "" or host_ids == None or host_ids == 'undefined':
                ...
                for i in range(0,len(host_ids_list)):
                    host_data_list = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id==host_ids_list[i]).all()
                    if len(host_data_list)>0:
                        e1_admin_data_list = sqlalche_obj.session.query(IduE1PortConfigurationTable.adminState).filter(IduE1PortConfigurationTable.config_profile_id==host_data_list[0][0]).all()
                        main_admin_data_list = sqlalche_obj.session.query(IduIduAdminStateTable.adminstate).filter(IduIduAdminStateTable.config_profile_id==host_data_list[0][0]).all()
                        ...
                        if len(e1_admin_data_list)>0:
                            ...
                            for j in range(0,len(e1_admin_data_list)):
                                temp_list.append(int(e1_admin_data_list[j].adminState))     
                            admin_data_dic.update({host_ids_list[i]:[temp_list,main_admin_data_list[0].adminstate if len(main_admin_data_list)>0 else 0,link_title]})

                    else:
                        admin_data_dic.update({host_ids_list[i]:"No Host Exist"})
                sqlalche_obj.sql_alchemy_db_connection_close()
        ...
        try:
            ...
            if host_ids == "" or host_ids == None or host_ids == 'undefined':
                ...
                for i in range(0,len(host_ids_list)):
                    host_data_list = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id==host_ids_list[i]).all()
                    if len(host_data_list)>0:
                        e1_status_data = sqlalche_obj.session.query(IduE1PortStatusTable).filter(IduE1PortStatusTable.host_id==host_ids_list[i]).order_by(desc(IduE1PortStatusTable.timestamp)).order_by(asc(IduE1PortStatusTable.portNum)).limit(4).all()
                        e1_admin_data_list = sqlalche_obj.session.query(IduE1PortConfigurationTable.adminState).filter(IduE1PortConfigurationTable.config_profile_id==host_data_list[0][0]).all()
                        main_admin_data_list = sqlalche_obj.session.query(IduIduAdminStateTable.adminstate).filter(IduIduAdminStateTable.config_profile_id==host_data_list[0][0]).all()
                        total_link_data = obj_get_data.common_get_data("IduLinkConfigurationTable",host_ids_list[i])
                        if len(total_link_data)>0:
                            for k in range(0,len(total_link_data)):
                                link_data_list.append(int(total_link_data[k].adminStatus))
                        if len(e1_admin_data_list)>0:
                            temp_list = []
                            for j in range(0,len(e1_admin_data_list)):
                                temp_list.append(int(e1_admin_data_list[j].adminState))     
                        if len(e1_status_data)>0:
                            for k in range(0,len(e1_status_data)):
                                e1_op_list.append(int(e1_status_data[k].opStatus))
                        admin_data_dic.update({host_ids_list[i]:[temp_list,main_admin_data_list[0].adminstate if len(main_admin_data_list)>0 else 0,e1_op_list,link_data_list]})
...
class IduAdminStateChange(object):
    ...
    def get_admin_op_state(self,host_id):
        ...
        try:
            #sqlalche_obj.sql_alchemy_db_connection_open()
            get_dic_operation = {'e1':'1.3.6.1.4.1.26149.2.1.2.3.1.2.1','e2':'1.3.6.1.4.1.26149.2.1.2.3.1.2.2','e3':'1.3.6.1.4.1.26149.2.1.2.3.1.2.3','e4':'1.3.6.1.4.1.26149.2.1.2.3.1.2.4',\
            ...
            sqlalche_obj.sql_alchemy_db_connection_open()
            host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id==host_id).all()
            e1_data = sqlalche_obj.session.query(IduE1PortConfigurationTable).filter(IduE1PortConfigurationTable.config_profile_id==host_data[0].config_profile_id).all()
            admin_data = sqlalche_obj.session.query(IduIduAdminStateTable).filter(IduIduAdminStateTable.config_profile_id==host_data[0].config_profile_id).all()
            e1_status_data = sqlalche_obj.session.query(IduE1PortStatusTable).filter(IduE1PortStatusTable.host_id==host_id).order_by(desc(IduE1PortStatusTable.timestamp)).order_by(asc(IduE1PortStatusTable.portNum)).limit(4).all()

            k=0
            snmp_get = pysnmp_geter(get_dic_operation,host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community)
                ...
                for i in snmp_get['result']:
                    if int(snmp_get['result'][i])!=-1:         
                        if i=='e1':
                            ...
                            if len(admin_data)>0:
                                admin_data[0].adminstate=snmp_get['result'][i]
                        elif i=='e1op':
                            if len(e1_status_data)>0:
                                e1_status_data[0].opStatus=snmp_get['result'][i]
                        elif i=='e2op':
                            if len(e1_status_data)>0:
                                e1_status_data[1].opStatus=snmp_get['result'][i]
                        elif i=='e3op':
                            if len(e1_status_data)>0:
                                e1_status_data[2].opStatus=snmp_get['result'][i]
                        elif i=='e4op':
                            if len(e1_status_data)>0:
                                e1_status_data[3].opStatus=snmp_get['result'][i]
                sqlalche_obj.session.commit()
            else:
                for i in snmp_get["result"]:
                    if i==553:
                        snmp_get["result"] = str(host_data[0].host_alias)+" ("+str(host_data[0].ip_address)+") "+str(errorStatus.get(i,"Device is not resposive"))
                    elif i==551:
                        snmp_get["result"] = str(host_data[0].host_alias)+" ("+str(host_data[0].ip_address)+") "+str(errorStatus.get(i,"Device is not resposive")) 
                    elif snmp_result["result"][i] != 0:
                        snmp_get["result"] = str(host_data[0].host_alias)+" ("+str(host_data[0].ip_address)+") "+str(errorStatus.get(snmp_get["result"][i],"Device is not resposive"))
        except Exception as e:
            snmp_get['success']=1
    ...
    def e1_port_admin_change(self,host_id,primary_id,port_number,admin_state_name,state):
        ...
        try:
            ...
            sqlalche_obj.sql_alchemy_db_connection_open()
            host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id==host_id).all()
            oid_data = sqlalche_obj.session.query(IduOids.oid,IduOids.oid_type).filter(IduOids.oid_name==admin_state_name).all()
            oid_dic = {admin_state_name:[str(oid_data[0].oid)+"."+str(port_number),oid_data[0].oid_type,state]}
                ...
                if 53 in snmp_result["result"]:
                    ...
                    for i in snmp_result["result"]:
                        if int(snmp_result["result"].values()[0]) != 0:
                            snmp_result["result"] = errorStatus.get(int(snmp_result["result"].values()[0]),"SNMP agent not respond")
    ...
    def link_port_admin_change(self,host_id,primary_id,port_number,bundle_number,admin_state_name,state):
        ...
        try:
            ...
            host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id==host_id).all()
            oid_data = sqlalche_obj.session.query(IduOids.oid,IduOids.oid_type).filter(IduOids.oid_name==admin_state_name).all()
            oid_dic = {admin_state_name:[str(oid_data[0].oid)+"."+str(port_number)+"."+str(bundle_number),oid_data[0].oid_type,state]}
            snmp_result = pysnmp_seter(oid_dic,host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_write_community)
            if snmp_result['success']==0:
                for i in snmp_result['result']:
                    if snmp_result['result'][i]==0:
                        link_port_data = sqlalche_obj.session.query(IduLinkConfigurationTable).filter(IduLinkConfigurationTable.idu_linkConfigurationTable_id==primary_id).all()
                        ...
                        if snmp_result["result"][i] != 0:
                            if snmp_result["result"].get(i) in errorStatus:
                                if snmp_result["result"].get(i)==12:
                                    snmp_result["result"]="The link is not on device.So please reconcile the device"
                                else:
                                    snmp_result["result"] = errorStatus[snmp_result["result"][i]]
    ...
    def main_admin_change(self,host_id,admin_state_name,state):
        ...
        try:
            ...
            sqlalche_obj.sql_alchemy_db_connection_open()
            host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id==host_id).all()
            oid_data = sqlalche_obj.session.query(IduOids.oid,IduOids.indexes,IduOids.oid_type).filter(IduOids.oid_name==admin_state_name).all()
...
##                    sqlalche_obj.session.commit()
        ...
        try:
            ...
            if len(port_num_list)>0:
                for i in range(0,len(port_num_list)):
                    temp_dic = {'admin_state_%s'%(port_num_list[i]):[str(oid_data[0].oid)+"."+str(port_num_list[i]),oid_data[0].oid_type,state]}
                    oid_dic.update(temp_dic)
                    primary_id_dic.update({'admin_state_%s'%(port_num_list[i]):primary_id_list[i]})
            snmp_result = pysnmp_seter(oid_dic,host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_write_community)
            if snmp_result['success']==0:
                for i in snmp_result['result']:
                    if snmp_result['result'][i]==0:
                        e1_port_data = sqlalche_obj.session.query(IduE1PortConfigurationTable).filter(IduE1PortConfigurationTable.idu_e1PortConfigurationTable_id==primary_id_dic[i]).all()
                        e1_port_data[0].adminState = state
                    else:
                        if snmp_result["result"].get(i) in errorStatus: 
                            snmp_result["result"][i] = errorStatus[snmp_result["result"][i]]
                sqlalche_obj.session.commit()
            else:
                if 53 in snmp_result["result"]:
                    snmp_result["result"] = errorStatus[53]
    ...
    def link_locked_unlocked_all(self,host_id,port_num,primary_ids,bundle_num,admin_state_name,state):
        ...
        try:
            ...
            if len(port_num_list)>0:
                for i in range(0,len(port_num_list)):
                    temp_dic = {'admin_state_%s'%(port_num_list[i]):[str(oid_data[0].oid)+"."+str(port_num_list[i])+"."+str(bundle_num_list[i]),oid_data[0].oid_type,state]}
                    oid_dic.update(temp_dic)
                    primary_id_dic.update({'admin_state_%s'%(port_num_list[i]):primary_id_list[i]})
            snmp_result = pysnmp_seter(oid_dic,host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_write_community)
            if snmp_result['success']==0:
                for i in snmp_result['result']:
                    if snmp_result['result'][i]==0:
                        link_port_data = sqlalche_obj.session.query(IduLinkConfigurationTable).filter(IduLinkConfigurationTable.idu_linkConfigurationTable_id==primary_id_dic[i]).all()
                        link_port_data[0].adminStatus = state
                    else:
                        if snmp_result["result"].get(i) in errorStatus: 
                            snmp_result["result"][i] = errorStatus[snmp_result["result"][i]]
...
#print obj.e1_port_admin_change(80,49,1,'iduConfiguration.e1PortConfigurationTable.adminState',1)
