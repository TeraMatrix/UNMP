...
from pysnmp_ap import pysnmp_seter,bulktable
...
class IduReconcilation(object):
    ...
    def default_reconciliation_controller(self,host_id,device_type_id,table_prefix,insert_update):
        ...
        idu_swstatus_data_add = IduSwStatusTable(host_id,"","",\
                                                "","",datetime.now())
        sqlalche_obj.session.add(idu_swstatus_data_add)
        
        idu_info_data_add = IduIduInfoTable(host_id,1,"",1,1,1,1,"","",1,1,datetime.now())
...
##                                            for i in range(0,len(result["result"])):
##                                                if config_type == 1:
##                                                    print "Insert into %s values (NULL,%s,%s)"%(tablename,config_profile_id,str(result["result"][i+1])[1:-1])
##                                                    sqlalche_obj.db.execute("Insert into %s values (NULL,%s,%s)"%(tablename,device_param_list[0].config_profile_id,str(result["result"][i+1])[1:-1]))
##                                                else:
##                                                    sqlalche_obj.db.execute("Insert into %s values (NULL,%s,%s)"%(tablename,host_id,str(result["result"][i+1])[1:-1]))
##                                        else:
...
#print obj.update_device_reconcilation_controller(42,'idu4','idu_',True,"")
                ...
                for i in range(0,len(host_ids_list)):
                    ...
                    if len(host_data_list)>0:
                        ...
                        if len(e1_admin_data_list)>0:
                            ...
                            for j in range(0,len(e1_admin_data_list)):
                                temp_list.append(int(e1_admin_data_list[j].adminState))     
                            admin_data_dic.update({host_ids_list[i]:[temp_list,main_admin_data_list[0].adminstate if len(main_admin_data_list)>0 else 0,link_data_list]})
...
#print obj.global_admin_request("82")
                ...
                if 53 in snmp_result["result"]:
                    ...
                    for i in snmp_result["result"]:
                        if int(snmp_result["result"].values()[0]) != 0:
                            snmp_result["result"] = errorStatus.get(int(snmp_result["result"].values()[0]),"SNMP Unknown error occured %s"%(snmp_result["result"].values()[0]))
