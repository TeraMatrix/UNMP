...
class IduGetData(object):
    def common_get_data(self,class_name,host_id):
        try:
            global sqlalche_obj
            get_data = []
            sqlalche_obj.sql_alchemy_db_connection_open()
            ...
            config_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).all()
            if len(config_id)>0:
                exec "get_data = sqlalche_obj.session.query(%s).filter(%s.config_profile_id==%s).all()"%(class_name,class_name,config_id[0].config_profile_id)
                if len(get_data)>0:
                    return get_data
                else:
                    return []
