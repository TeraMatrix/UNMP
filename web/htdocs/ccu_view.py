...
class CCUForms(object):    
    ...
    def status_data_form(host_id,selected_device):
        obj_bll = IduGetData()
        status_data_data = obj_bll.common_get_data("CcuStatusDataTable",host_id, False)
        #return (status_data_data,host_id)
        device_type = selected_device
        form_str = ""
        form_str += """<form id = \"status_data_form\" name = \"status_data_form\" action=\"\" method =\"get\">\
                        <div class=\"row-elem\">\
                    ...
                    """%("" if len(status_data_data)==0 else "" if status_data_data[0].ccuSDLastRebootReason==None else obj_bll.common_get_multivalue(device_type, "ccuOAM.ccuStatusDataTable.ccuSDLastRebootReason", status_data_data[0].ccuSDLastRebootReason), \
                       "" if len(status_data_data)==0 else "" if status_data_data[0].ccuSDUpTimeSecs==None else obj_bll.convert_time(status_data_data[0].ccuSDUpTimeSecs), "" if len(status_data_data)==0 else "" if status_data_data[0].ccuSDKwHReading==None else status_data_data[0].ccuSDKwHReading,\
    ...
    def unmp_ip_form(host_id,selected_device):
        ...
        form_str = ""
        form_str = ""
        form_str += "<form id = \"unmp_ip_form\" name = \"unmp_ip_form\" action=\"ccu_ip_action.py\" method =\"get\">\
                        <div class=\"row-elem\">\
