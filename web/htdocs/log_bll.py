...
# Required data for given user_id
    def get_log_data_bll(self,sEcho,iColumns,iDisplayLength,iDisplayStart,sColumns,sSearch,iSortCol_0,sSortDir_0,month,log_type,selected_user,group="",date_start="",date_end="",time_start="",time_end=""):
	    ...
	    if iSortCol_0 != None and iSortCol_0!=-1:
	                s_order += "%s %s, " % (" 0 + " + a_columns[int(iSortCol_0)] if a_columns[int(iSortCol_0)] == "time_taken" else a_columns[int(iSortCol_0)],sSortDir_0)
	                s_order = s_order[:-2]
	    ...
	    if month!="" and month!=None and month!="all":
	        ...
	        if s_where=="":
	            s_where = " where timestamp between '%s' and '%s' "%(date_temp_1,date_temp_2)
	        else:
	            s_where+= " and timestamp between '%s' and '%s' "%(date_temp_1,date_temp_2)
    ...
    def clear_old_logs(self):
    	try:
    	    ...
    	    conn = MySQLdb.connect (*SystemConfig.get_mysql_credentials())
    	    cursor = conn.cursor ()
    	    query="delete from event_log where timestamp <= '%s' "%(str(datetime.now()-timedelta(days=30))[:19])
            cursor.execute(query)
            conn.commit()
    ...
    def get_log_data_report(self,month,log_type,report_type,username,selected_user,date_start,date_end,time_start,time_end):
	   ...
	   if selected_user!="" and selected_user!=None:
    	   ...
    	   query="Select timestamp, time_taken, username , description from event_log %s order by timestamp desc"%(s_where)
           cursor.execute(query)
           log_tuple=cursor.fetchall()
           make_list = lambda x: [" - " if i == None or i == '' else str(i) for i in x]
           log_list=[]
           ...
           if report_type=="csv":     
	        sheet_dict={
				...
				"second_title":"" ,
				"headings":["Time","Time taken","Username","Description"],
		...
		el.log_event(desc, username, time1,datetime.now() )
	        return status		       
	   else:
	        sheet_dict={
				"sheet_name":"Log report" ,
