...
def performance_history(h):	
    ...
    html.write("<colgroup><col width=\"250px\"/><col width=\"auto\"/></colgroup>")
    html.write("<tr>")
    html.write("<td> Date Range")
    html.write("</td>")
    html.write("<td> <input type=\"text\" id=\"start_date\" value=\"%s\"/>&nbsp;To&nbsp; <input type=\"text\" id=\"end_date\" value=\"%s\"/>&nbsp; <input type=\"button\" id =\"submit_date\" class=\"dated\" value=\"submit\"/>"%(start_date,end_date))
    html.write("</td>")
    html.write("</tr>")
    html.write("</table>")
...
def service_status(h):
    ...
    if (html.var("start_date") and  html.var("end_date")) != None :
        ...
        start_date_val =     start_date_list[2]+"-"+start_date_list[1]+"-"+start_date_list[0]+" 00:00:00"

        end_date_list =html.var("end_date").split("/")
        end_date_val =end_date_list[2]+"-"+end_date_list[1]+"-"+end_date_list[0]+" 23:59:59"
    else:
    ...
    for service,host,time,status,output in result:
        if service!=temp_service and i!=0:
            all_i += 1
            dic_service={temp_service:{"ok_time":ok_time,"crit_time":crt_time}} 
            ...
            if crt_time_per < 15:
                if crt_time_per == 0:
                    ok_time_per = 100
                html.write("<div class=\"bar-first\" style=\"width:%.2f%%;\">" % (ok_time_per))
                html.write("<div class=\"bar-second\">%.2f%% OK</div></div><div class=\"bar-second\"></div></div>" % (ok_time_per))
            elif crt_time_per >= 15 and crt_time_per <= 30:
                html.write("<div class=\"bar-first\" style=\"width:%.2f%%;\">" % (ok_time_per))
                html.write("<div class=\"bar-second\">%.2f%% OK</div></div><div class=\"bar-second\">%.2f%%</div></div>" % (ok_time_per,crt_time_per))
            else:
                if ok_time_per < 15:
                    if ok_time_per == 0:
                        crt_time_per = 100
                    ...
                    html.write("<div class=\"bar-first\" style=\"width:%.2f%%;\">" % (ok_time_per))
                    html.write("<div class=\"bar-second\">%.2f%%</div></div><div class=\"bar-second\">%.2f%% Critical</div></div>" % (ok_time_per,crt_time_per))
                else:
                    html.write("<div class=\"bar-first\" style=\"width:%.2f%%;\">" % (ok_time_per))
                    html.write("<div class=\"bar-second\">%.2f%% OK</div></div><div class=\"bar-second\">%.2f%% Critical</div></div>" % (ok_time_per,crt_time_per))
...
##    cursor=db.cursor()
    ...
    for host,output,time,status in result:
        if host!=temp_host and i!=0:
            ...
            ok_time_per=(float(ok_time)/float(total_time))*100.0
            crt_time_per=(float(crt_time)/float(total_time))*100.0
            html.write("<td class=\"hostname\" style=\"cursor:pointer;\">%s" % temp_host)
            html.write("</td>")
            html.write("<td><div class=\"bar-main\">")
            if crt_time_per == ok_time_per:
                crt_time_per = ok_time_per = 50
            if crt_time_per < 15:
                if crt_time_per == 0:
                    ok_time_per = 100
                html.write("<div class=\"bar-first\" style=\"width:%.2f%%;\">" % (ok_time_per))
                html.write("<div class=\"bar-second\">%.2f%% Up</div></div><div class=\"bar-second\" title=\"%s%% Down\"></div></div>" % (ok_time_per,crt_time_per))
            elif crt_time_per >= 15 and crt_time_per <= 30:
                html.write("<div class=\"bar-first\" style=\"width:%.2f%%;\">" % (ok_time_per))
                html.write("<div class=\"bar-second\">%.2f%% Up</div></div><div class=\"bar-second\">%.2f%%</div></div>" % (ok_time_per,crt_time_per))
            else:
                if ok_time_per < 15:
                    if ok_time_per == 0:
                        crt_time_per = 100
                    ...
                    html.write("<div class=\"bar-first\" style=\"width:%.2f%%;\">" % (ok_time_per))
                    html.write("<div class=\"bar-second\">%.2f%%</div></div><div class=\"bar-second\">%.2f%% Down</div></div>" % (ok_time_per,crt_time_per))
                else:
                    html.write("<div class=\"bar-first\" style=\"width:%.2f%%;\">" % (ok_time_per))
                    html.write("<div class=\"bar-second\">%.2f%% Up</div></div><div class=\"bar-second\">%.2f%% Down</div></div>" % (ok_time_per,crt_time_per))
    ...
    if i > 0:
        ...
        if crt_time_per < 15:
            if crt_time_per == 0:
                ok_time_per = 100
            html.write("<div class=\"bar-first\" style=\"width:%.2f%%;\">" % (ok_time_per))
            html.write("<div class=\"bar-second\">%.2f%% Up</div></div><div class=\"bar-second\"></div></div>" % (ok_time_per))
        elif crt_time_per >= 15 and crt_time_per <= 30:
            html.write("<div class=\"bar-first\" style=\"width:%.2f%%;\">" % (ok_time_per))
            html.write("<div class=\"bar-second\">%.2f%% Up</div></div><div class=\"bar-second\">%.2f%%</div></div>" % (ok_time_per,crt_time_per))
        else:
