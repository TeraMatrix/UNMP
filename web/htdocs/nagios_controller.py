...
def nagios_load(h):
    global html
    html = h
    css_list = ["css/demo_table_jui.css","css/jquery.multiselect.css","css/jquery.multiselect.filter.css","css/jquery-ui-1.8.4.custom.css"]
    js_list =  ["js/jquery-ui-1.8.6.custom.min.js","js/pages/jquery.multiselect.min.js","js/pages/jquery.multiselect.filter.js","js/jquery.dataTables.min.js","js/pages/nagios.js"]
    ...
    html.new_header("Nagios","nagios_load.py",header_btn,css_list,js_list,snapin_list)    
...
def edit_nagios_host_from_inventory(h):
    ...
    js_list =  ["js/jquery-ui-1.8.6.custom.min.js","js/pages/jquery.multiselect.min.js","js/pages/jquery.multiselect.filter.js","js/jquery.dataTables.min.js","js/pages/nagios.js"]
    snapin_list = ["Nagios","reports","views","Alarm","Inventory","Settings","NetworkMaps","user_management","schedule","Listing"]
    html.new_header("Edit host","manage_host.py","",css_list,js_list,snapin_list)    
    #user_id=html.req.session["user_id"]

    result=n_bll.get_nagios_host_by_name(host_name)
    selected_columns=result["data"].get("use","").split(",")
    non_selected_columns =[ i[0] for i in result["options"].get("use",[[]]) if i[0] not in selected_columns and i[0]!=""  ]
    result["html"]=Nagios.get_columns(selected_columns,non_selected_columns," Select Host templates ")
    #html.write(result)
    #html.write(JSONEncoder().encode(result))
    html.write('<input type=\"text\" id=\"host_name_nagios\" value=\"%s\" style=\"display:none\"/>'%(host_name)+Nagios.main_menu_host_inventory())
    html.new_footer()
...
def nagios_load_host_template(h):
    ...
    html.new_header("Nagios","nagios_host_template.py",header_btn,css_list,js_list,snapin_list)    
    #user_id=html.req.session["user_id"]
...
def nagios_service(h):
    ...
    html.new_header("Nagios","nagios_service.py",header_btn,css_list,js_list,snapin_list)    
    #user_id=html.req.session["user_id"]
...
def edit_nagios_service(h):
    ...
    selected_columns=result["data"].get("use","").split(",")
    non_selected_columns =[ i[0] for i in result["options"].get("use",[[]]) if i[0] not in selected_columns and i[0]!=""  ]
    result["html"]=Nagios.get_columns(selected_columns,non_selected_columns," Select Service templates ")
    #html.write(result)
    html.write(JSONEncoder().encode(result))
...
def nagios_service_template(h):
    ...
    html.new_header("Nagios","nagios_service_template.py",header_btn,css_list,js_list,snapin_list)    
    #user_id=html.req.session["user_id"]
...
def nagios_hostgroup(h):
    ...
    html.new_header("Nagios","nagios_hostgroup_load.py",header_btn,css_list,js_list,snapin_list)    
    #user_id=html.req.session["user_id"]
...
def edit_nagios_hostgroup_from_inventory(h):
    ...
    js_list =  ["js/jquery-ui-1.8.6.custom.min.js","js/pages/jquery.multiselect.min.js","js/pages/jquery.multiselect.filter.js","js/jquery.dataTables.min.js","js/pages/nagios.js"]
    snapin_list = ["Nagios","reports","views","Alarm","Inventory","Settings","NetworkMaps","user_management","schedule","Listing"]
    html.new_header("Edit hostgroup","manage_hostgroup.py","",css_list,js_list,snapin_list)    
    #user_id=html.req.session["user_id"]
    html.write('<input type=\"text\" id=\"hostgroup_name_nagios\" value=\"%s\" style=\"display:none\"/>'%(hostgroup_name)+Nagios.main_menu_hostgroup_inventory())
    html.new_footer()
#
...
def nagios_servicegroup(h):
    ...
    <img onclick=\"verifyConfiguration();\" class=\"n-tip-image\" src=\"images/{0}/checkbox_checked.png\" id=\"verify_configuration_btn\" style=\"width: 16px; height: 16px; margin: 6px 20px 6px 10px;\" original-title=\"Verify Configuration\">\
    <img onclick=\"backupRestore();\" class=\"n-tip-image\" src=\"images/{0}/mail.png\" id=\"restore_configuration_btn\" style=\"width: 16px; height: 16px; margin: 6px 20px 6px 10px;\" original-title=\"Restore Configuration\"></div>".format(theme)
    html.new_header("Nagios","nagios_servicegroup_load.py",header_btn,css_list,js_list,snapin_list)    
    #user_id=html.req.session["user_id"]
...
def nagios_command(h):
    ...
    html.new_header("Nagios","nagios_command_load.py",header_btn,css_list,js_list,snapin_list)    
    #user_id=html.req.session["user_id"]
...
def nagios_hostdependency(h):
    ...
    html.new_header("Nagios","nagios_hostdependency.py",header_btn,css_list,js_list,snapin_list)    
    #user_id=html.req.session["user_id"]
...
def nagios_servicedependency(h):
    global html
    html = h
    css_list = ["css/demo_table_jui.css","css/jquery.multiselect.css","css/jquery.multiselect.filter.css","css/jquery-ui-1.8.4.custom.css","css/demo_page.css"]
    js_list =  ["js/jquery-ui-1.8.6.custom.min.js","js/pages/jquery.multiselect.min.js","js/pages/jquery.multiselect.filter.js","js/jquery.dataTables.min.js","js/ccpl_utility.js","js/pages/nagios.js"]
    snapin_list = ["Nagios","reports","views","Alarm","Inventory","Settings","NetworkMaps","user_management","schedule","Listing"]
    header_btn= "<div class=\"header-icon\"><img onclick=\"nagiosSettings();\" class=\"n-tip-image\" src=\"images/{0}/wrench.png\" id=\"settings_btn\" style=\"width: 16px; height: 16px; margin: 6px 20px 6px 10px;\" original-title=\"Settings\">\
    <img onclick=\"verifyConfiguration();\" class=\"n-tip-image\" src=\"images/{0}/checkbox_checked.png\" id=\"verify_configuration_btn\" style=\"width: 16px; height: 16px; margin: 6px 20px 6px 10px;\" original-title=\"Verify Configuration\">\
    <img onclick=\"backupRestore();\" class=\"n-tip-image\" src=\"images/{0}/mail.png\" id=\"restore_configuration_btn\" style=\"width: 16px; height: 16px; margin: 6px 20px 6px 10px;\" original-title=\"Restore Configuration\"></div>\
    ...
    html.new_header("Nagios","nagios_servicedependency.py",header_btn,css_list,js_list,snapin_list)    
    #user_id=html.req.session["user_id"]
