...
class DeviceStatus():
    ...
    def db_connect(self):
        ...
        try:
            ...
            #self.db = MySQLdb.connect("localhost","root","root","nmsp")        
        except MySQLdb.Error as e:
            print str(e)
        except Exception as e:
    ...
    def insert_status(self,host_id,ip_address,port_no,community,device_type,single_table_name=None):
        try:
            ...
            #print table_list
            host_state = self.host_status(host_id,14)                                         
            if host_state == 0:
                for table_name in table_dict:
                    ...
                    if is_table:
                        ...
                        #print table_name
                    snmp_result_dict = self.bulktable(str(table_dict[table_name].strip('.')),str(ip_address),int(port_no),community)
                    ...
                    if is_db_connect:
                           ...
                           if self.db == 1:
                               ...
                               #    return
                               #else:
                               #    self.host_id=cursor.fetchone()[0]
                               cursor.close()    
                    if snmp_result_dict['success'] == 0:    
                        ...
                        v = self.host_status(host_id,0,None,14)                        
                        snmp_flag = 1
                        return_value = 0
                        if len(table_list) > 0:
                        ...
                        if return_value < 2:
                            for table_name in table_list:
                                exit_status = self.defult_data_insert(table_name,ip_address)
                        else:
...
#How to use
#dd = DeviceStatus()
#print dd.insert_status('34','172.22.0.102', 161, 'public','odu100','odu100_synchStatusTable')
# query for getting 
# sql="SELECT host_id,snmp_port,snmp_read_community,device_type_id from hosts where ip_address = '%s'"%ip_address
