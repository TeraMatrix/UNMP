...
def ccu_listing(h):
    try:
        ...
        javascript_list = ["js/jquery.dataTables.min.js",'js/ccpl_jquery_autocomplete.js',"js/ccu_listing.js"]
        ...
        html.new_header("CCU Listing","","",css_list,javascript_list)
        ...
        html.write(str(page_header_search(ip_address,mac_address,"RM18,RM,Access Point,IDU,CCU",selected_device_type,"enabled","device_type")))
...
def ccu_profiling(h):
    ...
    if isinstance(device_list_parameter,list):
        if device_list_parameter == [] or device_list_parameter == None:
            html.write(page_header_search("","","RM18,RM,Access Point,IDU,CCU",None,"enabled","device_type"))#call the function of common_controller , it is used for listing the Devices based on IPaddress,Macaddress,DeviceTy
        else:
            html.new_header("%s %s Configuration"%("CCU",device_list_parameter[0].ip_address),"ccu_listing.py","",css_list,jss_list,snapin_list)
            html.write(page_header_search(device_list_parameter[0][0],device_list_parameter[0][1],"RM18,RM,Access Point,IDU,CCU",device_type,device_list_state,"device_type"))
            html.write(CCUProfiling.ccu_div(device_list_parameter[0][0],device_list_parameter[0][1],host_id))
...
def ccu_device_listing_table(h):
    ...
    if html.req.session["role"] == "admin" or html.req.session["role"] == "user":
        device_dict = ccu_device_list_bll_obj.ccu_device_list(ip_address,mac_address,selected_device,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,userid,html_req)
        #html.write(str(device_dict)+"bbbbbbbbbbbbbbbbbbbbbbbbbbbbb")
    ...
    if isinstance(device_list,list):
        ...
        for i in range(0,len(device_list)):
            #html.write(str(i))
            monitoring_status = "<a target=\"main\" <!--href=\"%s?host_id=%s&device_type=%s&device_list_state=%s\"--><img src=\"images/new/info1.png\" style=\"width:16px;height:16px;\" title=\"Current Device Status\" class=\"imgbutton n-reconcile w-reconcile\"/></a>"%('sp_status_profiling.py',\
            device_list[i][0],device_list[i][5],device_list_state) 
            ccu_real_data = obj_get_data.common_get_data_by_host('CcuRealTimeStatusTable',device_list[i][0])
            ...
            if html.req.session["role"] == "admin" or html.req.session["role"] == "user":
                result_device_list.append([str(device_list[i][1]),str(device_list[i][2]),str(device_list[i][3]),str(device_list[i][4]),\
                str(ccu_real_data[0].ccuRTSSystemVoltage if len(ccu_real_data)>0 else "-"),\
                str(ccu_real_data[0].ccuRTSSolarCurrent if len(ccu_real_data)>0 else  "-"),\
                str(ccu_real_data[0].ccuRTSInternalTemperature if len(ccu_real_data)>0 else "-"),\
                ...
                %s"
                %(device_list[i][0],device_list[i][5],device_list_state,device_list[i][0],\
                'sp_dashboard_profiling.py' if device_list[i][5] == "ccu" else 'sp_dashboard_profiling.py',\
                device_list[i][0],device_list[i][5],device_list_state,device_list[i][3],monitoring_status,images,device_list[i][6],device_list[i][0],device_list[i][5],\
                ...
                result_device_list.append([str(device_list[i][1]),str(device_list[i][2]),str(device_list[i][3]),str(device_list[i][4]),
                "<a target=\"main\" href=\"ccu_profiling.py?host_id=%s&device_type=%s&device_list_state=%s\">\
                <img id=\"%s\" src=\"images/new/edit.png\" title=\"Edit Profile\" style=\"width:16px;height:16px;\" class=\"imgbutton n-reconcile\"/></a>&nbsp;&nbsp;\<a target=\"main\" href=\"%s?host_id=%s&device_type=%s&device_list_state=%s\">\<a target=\"main\" href=\"%s?host_id=%s&device_type=%s&device_list_state=%s\">\
                ...
                %(device_list[i][0],device_list[i][5],device_list_state,device_list[i][0],\
                'sp_dashboard_profiling.py' if device_list[i][5] == "ccu" else 'sp_dashboard_profiling.py',\
                device_list[i][0],device_list[i][5],device_list_state,device_list[i][3],images,device_list[i][6],device_list[i][0],device_list[i][5],monitoring_status,images,device_list[i][6],device_list[i][0],device_list[i][5],\
                "<input type=\"hidden\" value=\"%s\" name=\"host_id\" id=\"host_id\" />"%(device_status_host_id) if i==len(device_list)-1 else "")])
...
def get_device_list_ccu_profiling(h): 
    try:
        ...
        device_list = ccu_device_list_bll_obj.ccu_device_list_profiling(ip_address,mac_address,selected_device)
        ...
        if result==0 or result==1 or result==2:
            ...
            device_list_parameter = ccu_device_list_bll_obj.get_device_parameter(result)# call the idu_profiling_controller function get_device_param returns the list.List Contains ipaddress,macaddress,devicetypeid,config_profile_id
            #html.write(str(device_list_parameter))
            html.write(CCUProfiling.ccu_profile_call(result,selected_device,device_list_parameter))
    except Exception as e:
        html.write(str(e[-1]))
...
def battery_panel_action(h):
    ...
    if html.var("host_id") == "" or html.var("host_id") == None or html.var("host_id") =='undefined':
        ...
        if html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteBatteryCapacity")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteBatteryCapacity")):
                if Validation.is_number(html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteBatteryCapacity")):
                    dic_result["ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteBatteryCapacity"]=html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteBatteryCapacity")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Battery capacity field is must be interger."
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
        ...
        if html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteSolarPanelwP")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteSolarPanelwP")):
                if Validation.is_number(html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteSolarPanelwP")): 
                    dic_result["ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteSolarPanelwP"]=html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteSolarPanelwP")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Solar Panel field must ne integer."
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
        ...
        if html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteSolarPanelCount")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteSolarPanelCount")):
                if Validation.is_number(html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteSolarPanelCount")):
                    dic_result["ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteSolarPanelCount"]=html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteSolarPanelCount")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Solar panel count is must be integer."
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
        ...
        if html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCNewBatteryInstallationDate")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCNewBatteryInstallationDate")):
                if Validation.is_string(html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCNewBatteryInstallationDate")):
                    dic_result["ccuOAM.ccuBatteryPanelConfigTable.ccuBPCNewBatteryInstallationDate"]=html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCNewBatteryInstallationDate")
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if flag == 1:
            ...
            result = ccu_bll_obj.common_validation(host_id,device_type_id,dic_result)
            html.req.content_type = 'application/json'
        ...
        if html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATHighTemperatureAlarm")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATHighTemperatureAlarm")):
                if Validation.is_number(html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATHighTemperatureAlarm")):
                    dic_result["ccuOAM.ccuAlarmAndThresholdTable.ccuATHighTemperatureAlarm"]=html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATHighTemperatureAlarm")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Temperature alarm field is must be interger."
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
                dic_result["result"] = "Temperature alarm field is must be required."
        ...
        if html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATPSMRequest")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATPSMRequest")):
                if Validation.is_number(html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATPSMRequest")): 
                    dic_result["ccuOAM.ccuAlarmAndThresholdTable.ccuATPSMRequest"]=html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATPSMRequest")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "PSM request field must ne integer."
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
        ...
        if html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATSMPSMaxCurrentLimit")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATSMPSMaxCurrentLimit")):
                if Validation.is_number(html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATSMPSMaxCurrentLimit")):
                    dic_result["ccuOAM.ccuAlarmAndThresholdTable.ccuATSMPSMaxCurrentLimit"]=html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATSMPSMaxCurrentLimit")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "SMPS Max current field must be integer."
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
        ...
        if html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATPeakLoadCurrent")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATPeakLoadCurrent")):
                if Validation.is_string(html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATPeakLoadCurrent")):
                    dic_result["ccuOAM.ccuAlarmAndThresholdTable.ccuATPeakLoadCurrent"]=html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATPeakLoadCurrent")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Peak load current field must be integer."
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
        ...
        if html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer1MACID")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer1MACID")):
                if Validation.is_valid_mac(html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer1MACID")):
                    dic_result["ccuOAM.ccuPeerInformationTable.ccuPIPeer1MACID"]=html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer1MACID")
                else:
                    flag = 1
                    dic_result["success"] = 1
                ...
                count+=1
                dic_result["success"] = 1
                dic_result["result"] = "Peer 1 MAC field is must be required."
        ...
        if html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer2MACID")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer2MACID")):
                if Validation.is_valid_mac(html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer2MACID")): 
                    dic_result["ccuOAM.ccuPeerInformationTable.ccuPIPeer2MACID"]=html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer2MACID")
                else:
                    flag = 1
                    dic_result["success"] = 1
                ...
                count+=1
                dic_result["success"] = 1
                dic_result["result"] = "Peer 2 MAC field is must be required."
        ...
        if html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer3MACID")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer3MACID")):
                if Validation.is_valid_mac(html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer3MACID")):
                    dic_result["ccuOAM.ccuPeerInformationTable.ccuPIPeer3MACID"]=html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer3MACID")
                else:
                    flag = 1
                    dic_result["success"] = 1
                ...
                count+=1
                dic_result["success"] = 1
                dic_result["result"] = "Peer 3 MAC field is must be required."
        ...
        if html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer4MACID")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer4MACID")):
                if Validation.is_valid_mac(html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer4MACID")):
                    dic_result["ccuOAM.ccuPeerInformationTable.ccuPIPeer4MACID"]=html.var("ccuOAM.ccuPeerInformationTable.ccuPIPeer4MACID")
                else:
                    flag = 1
                    dic_result["success"] = 1
                ...
                count+=1
                dic_result["success"] = 1
                dic_result["result"] = "Peer 4 MAC field is must be required."
        if flag == 1:
            if count > 1:
                dic_result["result"] = "All fields are required."
            html.req.content_type = 'application/json'
        ...
        if html.var("ccuOAM.ccuSiteInformationTable.ccuSITSiteName")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuSiteInformationTable.ccuSITSiteName")):
                if Validation.is_string(html.var("ccuOAM.ccuSiteInformationTable.ccuSITSiteName")):
                    dic_result["ccuOAM.ccuSiteInformationTable.ccuSITSiteName"]=html.var("ccuOAM.ccuSiteInformationTable.ccuSITSiteName")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Site name field is must be String."
            else:
                flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Site name field is must be required."
        if flag == 1:
            html.req.content_type = 'application/json'
            html.req.write(str(JSONEncoder().encode(dic_result)))
        else:
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalOutput1")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalOutput1")):
                dic_result["ccuOAM.ccuAuxIOTable.ccuAIExternalOutput1"]=html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalOutput1")
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
                dic_result["result"] = "External output 1 field is must be required."
        ...
        if html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalOutput2")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalOutput2")):
                dic_result["ccuOAM.ccuAuxIOTable.ccuAIExternalOutput2"]=html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalOutput2")
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
                dic_result["result"] = "External output 2 field is must be required."
        ...
        if html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalOutput3")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalOutput3")):
                dic_result["ccuOAM.ccuAuxIOTable.ccuAIExternalOutput3"]=html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalOutput3")
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
                dic_result["result"] = "External output 3 field is must be required."
                
        if html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput1")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput1")):
                dic_result["ccuOAM.ccuAuxIOTable.ccuAIExternalInput1"]=html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput1")
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
                dic_result["result"] = "External input 1 field is must be required."
        if html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput2")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput2")):
                dic_result["ccuOAM.ccuAuxIOTable.ccuAIExternalInput2"]=html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput2")
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
                dic_result["result"] = "External input 2 field is must be required."
        if html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput3")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput3")):
                dic_result["ccuOAM.ccuAuxIOTable.ccuAIExternalInput3"]=html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput3")
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
                dic_result["result"] = "External input 3 field is must be required."
        ...
        if html.var("ccuOAM.ccuControlTable.ccuCTLoadTurnOff")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuControlTable.ccuCTLoadTurnOff")):
                if Validation.is_number(html.var("ccuOAM.ccuControlTable.ccuCTLoadTurnOff")):
                    dic_result["ccuOAM.ccuControlTable.ccuCTLoadTurnOff"]=html.var("ccuOAM.ccuControlTable.ccuCTLoadTurnOff")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Load turn Off field is must be number."
            else:
                flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Load Trun Off field is must be required."
            if Validation.is_required(html.var("ccuOAM.ccuControlTable.ccuCTSMPSCharging")):
                dic_result["ccuOAM.ccuControlTable.ccuCTSMPSCharging"]=html.var("ccuOAM.ccuControlTable.ccuCTSMPSCharging")
            else:
                flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "SMPS Charging field is must be required."
        if flag == 1:
            html.req.content_type = 'application/json'
            html.req.write(str(JSONEncoder().encode(dic_result)))
        else:
...
def ccu_reconciliation(h):
    ...
    result = obj.update_reconcilation_controller(host_id,device_type,"ccu_",datetime.now(),user_name)
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(result)))