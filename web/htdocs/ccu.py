...
def ccu_listing(h):
    try:
        ...
        html.new_header("CCU Listing","ccu_listing.py","",css_list,javascript_list)
...
def ccu_device_listing_table(h):
    ...
    if isinstance(device_list,list):
        ...
        for i in range(0,len(device_list)):
            #html.write(str(i))
#            monitoring_status = "<a target=\"main\" <!--href=\"%s?host_id=%s&device_type=%s&device_list_state=%s\"--><img src=\"images/new/info1.png\" style=\"width:16px;height:16px;\" title=\"Current Device Status\" class=\"imgbutton n-reconcile w-reconcile\"/></a>"%('sp_status_profiling.py',\device_list[i][0],device_list[i][5],device_list_state) 
            ...
            if html.req.session["role"] == "admin" or html.req.session["role"] == "user":
                result_device_list.append([str(device_list[i][1]),str(device_list[i][2]),str(device_list[i][3]),str(device_list[i][4]),\
                                           str(ccu_real_data[0].ccuRTSSystemVoltage if len(ccu_real_data)>0 and str(ccu_real_data[0].ccuRTSSystemVoltage) != '1111111' else "Device Unreachable"),\
                                           str(ccu_real_data[0].ccuRTSSolarCurrent if len(ccu_real_data)>0 and str(ccu_real_data[0].ccuRTSSolarCurrent) != '1111111' else  "Device Unreachable"),\
                                           str(ccu_real_data[0].ccuRTSInternalTemperature if len(ccu_real_data)>0 and str(ccu_real_data[0].ccuRTSInternalTemperature) != '1111111' else "Device Unreachable"),\
...
#                monitoring_status = "<a target=\"main\" <!--href=\"%s?host_id=%s&device_type=%s&device_list_state=%s\"--><img src=\"images/new/info1.png\" style=\"width:16px;height:16px;\" title=\"Current Device Status\" class=\"imgbutton n-reconcile w-reconcile\"/></a>"%('sp_status_profiling.py',device_list[i][0],device_list[i][5],device_list_state) 
                ...
                state_rec=\"0\"/>%s"%(device_list[i][0],\
                                             'sp_dashboard_profiling.py' if device_list[i][5] == "ccu" else 'sp_dashboard_profiling.py',\
                                             device_list[i][0],device_list[i][5],device_list_state,device_list[i][3],monitoring_status,images,device_list[i][6],\
                                             "<input type=\"hidden\" value=\"%s\" name=\"host_id\" id=\"host_id\" />"%(device_status_host_id) if i==len(device_list)-1 else "")])
        ...
        if html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteBatteryCapacity")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteBatteryCapacity")):
                if Validation.is_number(html.var("ccuOAM.ccuBatteryPanelConfigTable.ccuBPCSiteBatteryCapacity")):
                    ...
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Battery capacity field must be interger."
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
        ...
        if html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATHighTemperatureAlarm")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATHighTemperatureAlarm")):
                if Validation.is_number(html.var("ccuOAM.ccuAlarmAndThresholdTable.ccuATHighTemperatureAlarm")):
                    ...
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Temperature alarm field must be interger."
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
        ...
        if html.var("ccuOAM.ccuSiteInformationTable.ccuSITSiteName")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuSiteInformationTable.ccuSITSiteName")):
                if Validation.is_string(html.var("ccuOAM.ccuSiteInformationTable.ccuSITSiteName")):
                    ...
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Site name field must be String Type."
            else:
                flag = 1
                dic_result["success"] = 1
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput1AlarmType")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput1AlarmType")):
                dic_result["ccuOAM.ccuAuxIOTable.ccuAIExternalInput1AlarmType"]=html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput1AlarmType")
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
        ...
        if html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput2AlarmType")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput2AlarmType")):
                dic_result["ccuOAM.ccuAuxIOTable.ccuAIExternalInput2AlarmType"]=html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput2AlarmType")
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
        ...
        if html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput3AlarmType")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput3AlarmType")):
                dic_result["ccuOAM.ccuAuxIOTable.ccuAIExternalInput3AlarmType"]=html.var("ccuOAM.ccuAuxIOTable.ccuAIExternalInput3AlarmType")
            else:
                flag = 1
                count+=1
                dic_result["success"] = 1
        ...
        if html.var("ccuOAM.ccuControlTable.ccuCTLoadTurnOff")!=None:
            if Validation.is_required(html.var("ccuOAM.ccuControlTable.ccuCTLoadTurnOff")):
                if Validation.is_number(html.var("ccuOAM.ccuControlTable.ccuCTLoadTurnOff")):
                    ...
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Load turn Off field must be number."
            else:
                flag = 1
                dic_result["success"] = 1
