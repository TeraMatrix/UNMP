...
def live_monitoring_page(h):
    ...
    js_list = ["js/highcharts.js","js/jquery.dataTables.min.js","js/pages/live_monitoring.js"]
    ...
    html.new_header("Live Monitoring",selected_listing,header_btn,css_list,js_list)
...
def get_graph_data(h):
    ...
    cf=html.var("cf")
    resolution=html.var("resolution")!=None and int(html.var("resolution")) or 1
    graph_data = lm_bll.graph_data(device_type,ip_address,host_id,graph_name,total,cf,resolution)
    html.req.content_type = 'application/json'
...
def settings_live_monitoring(h):
    ...
    protocol_select_list = Common.make_select_list(value, name, selected, list_id, list_name,title)
    ...
    dataset_select_list = Common.make_select_list(value, name, selected, list_id, list_name,title)
    ...
    ds_show_select_list = Common.make_select_list(value, name, selected, list_id, list_name,title)


    # get refresh_rate_select_list list[5/10/20/30/.....]
    value = ["5","10","15","20","30","60","120","180","240","300"]
    name = ["5 sec","10 sec","15 sec","20 sec","30 sec","1 min","2 min","3 min","4 min","5 min"]
    selected = "30"
    list_id = "rrd_step"
    list_name = "rrd_step"
    title = "Select Refresh Rate"
    refresh_rate_select_list = Common.make_select_list(value, name, selected, list_id, list_name,title)

    html.write(LiveMonitoring.settings_live_monitoring(device_type_select_list,protocol_select_list,dataset_select_list,ds_show_select_list,refresh_rate_select_list))
...
def save_live_monitoring_config(h):
    ...
    new_graph_data[graph_key]["is_snmp"] = html.var("is_snmp") == "1" and True or False
    new_graph_data[graph_key]["oid_table"] = html.var("oid_table")
    new_graph_data[graph_key]["row_index"] = map(int,str(html.var("row_index")).split(","))
    new_graph_data[graph_key]["column_index"] = map(int,str(html.var("column_index")).split(","))
    new_graph_data[graph_key]["ds_name"] = map(str,str(html.var("ds_name")).split(","))
    ...
    new_graph_data[graph_key]["get_dyn_name"] = None #html.var("get_dyn_name") == "" and None or html.var("get_dyn_name")
    new_graph_data[graph_key]["unreachable_value"] = html.var("unreachable_value")
    ...
    result = LiveMonitoringBll().write_live_monitoring_config_file(device_type,new_graph_data,new_rrd_param)
    html.req.content_type = 'application/json'
