import time,urllib2,base64, socket,MySQLdb
import pycurl
import urllib
import re
...
def odu100_ruConfTable(data):
    try:
        s=data
        main_str_list=['** Update RU admin state drop-down listbox.','** Update Tier drop-down listbox.','** Update channel bandwidth drop-down listbox.', \
        ...
        find_string_list=['var synchSource = ', 'var nodeType = ', 'var channelBandwidth = ',\
...
def odu100_ruStatusTable(data_ru_op_state,data): # view-source:http://172.22.0.120/eweb_index.shtml
    try:
        ...
        find_string_list=['<div id="rebootreason">','<div id="yesnostate">', '<div id="status_value">', \
...
def odu100_swStatusTable(data):
    ...
    try:
        s=data
        main_str_list=['Active Version Number','Passive Version Number','Boot Loader Version']
        find_string_list=['<td align="left">','<td align="left">', '<td align="left">']
...
def odu100_omcConfTable(data):
    ...
    try:
        ...
        find_string_list=['onBlur="checkField(this)" value="','onBlur="checkField(this)" value="']
        end_string_list=['" ','" ']
        list_data=[1]
        for i in range(len(main_str_list)):
            main_str=main_str_list[i]
...
def odu100_ipConfigTable(data):
    ...
    try:
        s=data
        main_str_list=['** Update IP admin state drop-down listbox.','<td align="left">IP Address<br/>','<td align="left">IP Netmask</td>',
        ...
        find_string_list=['var ipAdminState = ','id="ru.np.ip.ipAddress" value="','id="ru.np.ip.ipNetworkMask" value="',
                          'id="ru.np.ip.ipDefaultGateway" value="','var dhcpState = ','=','value="']
        end_string_list=[';','" size="20"></td>','" size="20"></td>','" size="20"></td>',';',';','"']
        list_data=[1]
        for i in range(len(main_str_list)):
            main_str=main_str_list[i]
            find_string=find_string_list[i]
...
def odu100_syncConfigTable(data):
    ...
    try:
        ...
        end_string_list=[';','" size="15"','" size="15">','" size="15">','" size="15"></td>','" size="3">','" size="15">']
        list_data=[1]
        for i in range(len(main_str_list)):
            main_str=main_str_list[i]
            find_string=find_string_list[i]
...
def odu100_raLlcConfTable(data):
    ...
    try:
        ...
        end_string_list=['" size="15"></td>','" size="15"></td>','" size="15"></td>','" size="15"></td>','" size="15"></td>']
        list_data=[]
        for i in range(len(main_str_list)):
            main_str=main_str_list[i]
            find_string=find_string_list[i]
...
def odu100_raTddMacConfigTable(data):
    ...
    try:
        ...
        end_string_list=['" size="15"></td>','";','";','" size="15"></td>','" size="15"></td>',';']
        list_data=[]
        for i in range(len(main_str_list)):
            main_str=main_str_list[i]
            find_string=find_string_list[i]
...
def odu100_raPreferredRFChannelTable(data):
    ...
    try:
        ...
        find_string_list=['var rfChannel = ','rfChannel = ','rfChannel = ','rfChannel = ',\
...
def odu100_peerConfigTable1(data):
    ...
    try:
        ...
        find_string_list=['mac= "','guaranteedDownlinkBW= "','guaranteedUplinkBW= "' ,'maxDownlinkBW= "',\
...
def odu100_peerConfigTable(data):
    ...
    try:
        s=data 
        main_str_list=['<td align ="left"><b>1</b></td>','<td align ="left"><b>2</b></td>',\
        ...
        find_string_list=['PEER1MAC" value="','guaranteedDownlinkBW" value="','guaranteedUplinkBW" value="' ,'maxDownlinkBW" value="',\
                          'maxUplinkBW" value="','basicRateMCSIndex" value="']
        end_string_list=['" size="','" size="','" size="','" size="','" size="','" size="']
        list_data=[]
        dict_data={}
        count=1
        for i in range(len(main_str_list)):
...
function_dict['odu100_raTddMacStatisticsTable']=odu100_raTddMacStatisticsTable
function_dict['odu100_nwInterfaceStatisticsTable']=odu100_nwInterfaceStatisticsTable
...
function_dict['odu100_raPreferredRFChannelTable']=odu100_raPreferredRFChannelTable
...
def cgi_opener(ip,username,password,table_list):
    try:
        ...
        if isinstance(url, str) and isinstance(username, str) and isinstance(password, str):
            ...
            success = 1
            result_dict[96]="arguments are not proper : (url , username, password) all arguments should be as String"
...
table_oid_dict = {'1.3.6.1.4.1.26149.2.2.11.2.1': 'odu100_synchStatisticsTable',
...
def cgi_recon(ip,table_oid,username='root',password='public'):
    try:
        success = 1
        result_dict = {}
        table_list = [table_oid_dict[table_oid]] if table_oid_dict.has_key(table_oid) else []
        url="http://"+ip+"/"
        if isinstance(url, str) and isinstance(username, str) and isinstance(password, str):
            loop = len(table_list)
            temp_list = deepcopy(table_list)
