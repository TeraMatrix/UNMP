...
host_status_dict = {0:'No operation', 1:'Firmware download', 
                    2:'Firmware upgrade', 3:'Restore default config', 
                    4:'Flash commit', 5:'Reboot', 6:'Site survey', 
                    7:'Calculate BW', 8:'Uptime service', 9:'Statistics gathering', 
                    10:'Reconciliation', 11:'Table reconciliation', 12:'Set operation', 
                    13:'Live monitoring', 14:'Status capturing', 15: 'Alarm Reconciliation'}
...
class EventLog(object):
    ...
    def log_event(self ,description ,user_name ,time1=datetime.datetime.now() ,time2=datetime.datetime.now() ,level=0):
    	"""
        ...
        try:
            ...
            if second<60:
                time_taken=str(second)+"."+str(microseconds/1000)+" sec"
            elif second<3600:
                if second > 120:
                    level=1 # warning
                elif second > 300:
                ...
                #print last_check
            self.db_connect()
            insert_query = "INSERT INTO `event_log` (`event_log_id`, `username`, `event_type_id`, `description`, `timestamp`,`level`,`time_taken`) VALUES (NULL,\"%s\",NULL,\"%s\",\"%s\",\"%s\",\"%s\")"%(user_name ,description ,datetime.datetime.now() ,level ,time_taken)
    ...
    for edge in node.edges:
        if edge not in resolved:
            if edge in unresolved:
                raise Exception('Circular reference detected between %s and %s' % (node.name, edge.name))
...
class Essential(object):
    ...
    def circular_check(self,child,parent):
        ...
        try:
            self.db_connect()
            sel_query = "SELECT  `host_id` , `parent_name`, `ip_address` FROM  `hosts` WHERE is_deleted = 0"
            #rel = (('b', 'a'), ('c', 'a'), ('d', 'b'), ('e', 'b'), ('f', 'd'), ('g', 'f'), ('h', 'e'))
            cursor = self.global_db.cursor()
            ...
            if len(rel) > 0:
                ...
                try:
                    if len(obj_dict)>0 and obj_dict.has_key(self.child):
...
def agent_start(ip_address,username='root',pwd='public'):
    ...
    try:
        ...
        #myssh.put('myssh.py')
        myssh.run("a=$( ps | grep snmpagent | grep -v grep | awk '{print $1}');c=1;for i in $a;do if [ $c -gt 1 ];then kill -9 $i;fi;c=`expr ${c} + 1`;done;")
