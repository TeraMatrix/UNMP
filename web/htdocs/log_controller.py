...
def main_log(h):
    global html
    html = h
    css_list = ["css/demo_table_jui.css","css/jquery.multiselect.css","css/jquery.multiselect.filter.css","css/jquery-ui-1.8.4.custom.css","css/calendrical.css"]
    js_list = ["js/jquery-ui-1.8.6.custom.min.js","js/jquery.dataTables.min.js","js/pages/jquery.multiselect.min.js","js/pages/jquery.multiselect.filter.js","js/calendrical.js","js/pages/log.js"]
    ...
    html.new_header("LOG DETAILS","log_user.py",header_btn,css_list,js_list)
...
def get_data(h):
	...
	result= l.get_log_data_bll(sEcho,iColumns,iDisplayLength,iDisplayStart,sColumns,sSearch,iSortCol_0,sSortDir_0,month,log_type,selected_user,group,date_start,date_end,time_start,time_end)
...
def get_log_data_excel(h):
    try:
        ...
        l=Log_bll()
        result= l.get_log_data_report(month,log_type,report_type,username,selected_user,date_start,date_end,time_start,time_end) 
        html.write(JSONEncoder().encode(result))   
    except Exception,e:
        html.write(str(e))
...
def edit_log_settings(h):
	...
	html_view="<div style=\"width:90%%\"><h2>User trail settings:</h2>\
		<div class=\"row-elem\"></div>\
		<div class=\"row-elem\">\
		<label class=\"lbl lbl-big\" style=\"width:100px;\" >User trail:</label>\
		...
		<button class=\"yo-small yo-button\" id=\"clear_old_logs\" type=\"button\">Clear old logs</button></div>"%(html_selected)
        html.write(html_view)
    except Exception,e:
        #import traceback      
        #html.write(str(traceback.format_exc()))      
        html.write(str(e))      
