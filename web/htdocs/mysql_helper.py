...
class MySQLConnection(object):
    ...
    def __init__(self,mysql_credentials):
        ...
        self.set_connection(mysql_credentials)          # connect with database using set_connction method
    ...
    def set_connection(self,mysql_credentials):         # set connection object [setter method]
        ...
        '''
        self._connection = MySQLdb.connect(*mysql_credentials)      # calling of MySQLdb.connect method to connect with database 

    connection = property(get_connection,set_connection)            # define connection property
    ...
    def close_connection(self):                         # call this method to close database connection
        ...
        '''
        self.connection.close()                         # calling of MySQLdb's close() method
    ...
    def reopen_connection(self):                        # call this method to reopen the connection 
        ...
        ''' 
        self.set_connection(self.mysql_credentials)     # calling of set_connection to recreate object
    ...
    def db_commit(self):                                # call this method to commit the database changes
        ...
        '''
        self.connection.commit()                        # calling of MySQLdb's commit method
    ...
    def db_rollback(self):                              # call this method to rollback the database changes
        ...
        '''
        self.connection.rollback()                      # calling of MySQLdb's rollback method
...
class MySQLHelper(object):
    ...
    def execute_non_query(self,mysql_credentials,command_type,command_text,sql_parameter=None):
        ...
        '''
        mysql_connection = MySQLConnection(mysql_credentials)           # create mysql database connection
        cursor,self.effected_row = self.execute_command(mysql_connection,command_type,command_text,sql_parameter,True)  # execute mysql command 
    ...
    def execute_all_query(self,mysql_credentials,command_type,command_text,sql_parameter=None):
        ...
        '''
        mysql_connection = MySQLConnection(mysql_credentials)           # create mysql database connection
        cursor,self.effected_row = self.execute_command(mysql_connection,command_type,command_text,sql_parameter)   # execute mysql command
    ...
    def execute_command(self,mysql_connection,command_type,command_text,sql_parameter,do_commit = False):
        ...
        if mysql_connection.connection_state() != MySQLConnectionState.open:    # check the connection is open or closed 
            mysql_connection.reopen_connection()                                # if connection is closed the reopen it 

        cursor = mysql_connection.connection_cursor()                           # create cursor object
        ...
        if command_type == CommandType.text:                                    # check the command type is text or not 
            ...
            effected_row = cursor.execute(command_text)                         # execute command then get the number of rows effected
        elif command_text == CommandType.stored_procedure:                      # check the command type is stored procedure or not
            effected_row = cursor.callproc(command_text,sql_parameter)          # execute stored procedure the get number of row  effected
        if do_commit == True:                                                   # check do_commit is True or False 
            mysql_connection.db_commit()                                        # if do_commit is True the Commit the database

        mysql_connection.close_connection()                                     # close the mysql connection
...
#sql_parameter = (now.strftime('%Y-%m-%d %H:%M:%S'),now2.strftime('%Y-%m-%d %H:%M:%S'), refresh_time,"eth0","172.22.0.101")
