...
class LiveMonitoringBll(object):
            ...
            if isfile(graph_file):
                ...
                if isfile(tmp_file):
                    ...
                    if table_dict.has_key(graph_name):
                        is_live = True
                try:
                    fetch_data = rrdtool.fetch(graph_file, cf,'-r',str(resolution), '-s','-%ssec' % total_sec)
                ...
                for i in range(int(total)):
                    for lbl_i in range(len(label)): 
                        data_series[lbl_i]["data"].append({"x":self.convert_utc_to_ist((timestamp+(sec*(i)))*1000),"y":None})
        ...
        try:
            if isfile(rrd_conf_file):
                ...
                for key in rrd_graph[device_type]:
                    ...
                    rra_dataset = map(int,rrd_graph[device_type][key]["rra_dataset"])
                    for i in range(0,len(rra_cf)):
                        rrd_graph[device_type][key]["rra_samples"][i] = ((rrd_size/(rrd_step*rra_dataset[i])))
                ...
                rrd_graph[device_type] = self.update_dict(rrd_graph[device_type],new_graph_data)
    ...
    def update_dict(self,a,b):
        if isinstance(a,dict) and isinstance(b,dict):
            for key in b:
                if a.has_key(key) and isinstance(a[key],dict) and isinstance(b[key],dict):
