...
def get_hostgroup_hosts(user_id,hostgroup_id_list):
        try:
            ...
            cursor = conn.cursor()
	    query="select hg.hostgroup_id,hg.hostgroup_name,h.host_id,h.host_alias,h.ip_address,h.priority_id,ifnull(h_s.normal_check_interval,'-') from hostgroups as hg \
			left join (select hostgroup_id,host_id from hosts_hostgroups)  as hhg on hhg.hostgroup_id=hg.hostgroup_id \
			left join (select host_id,device_type_id,ip_address,host_alias,priority_id,is_deleted,host_state_id from hosts) as h on h.host_id=hhg.host_id \
			left join (select normal_check_interval,host_id from host_services where service_description like 'Statictics Service%%') as h_s on h_s.host_id=h.host_id \
			left join (select device_type_id,device_name,is_deleted from device_type) as dt on dt.device_type_id=h.device_type_id \
			where hhg.hostgroup_id IN (%s) and dt.is_deleted<>1 and h.is_deleted=0 and h.host_state_id='e' order by hg.hostgroup_name,h.host_alias"%','.join(hostgroup_id_list)
	    ...
	    for row in result:
	        if di.has_key(row[0]):
		    li = di[row[0]]
		    l=[row[2],row[3],row[4],row[5],row[6]]
		    li.append(l)
		    di[row[0]] = li
		else:
		    l=make_list(row)
...
def make_service_box(host_id,hostgroup_id,host_alias,host_ip,priority,normal_check_interval):
    try:
        ...
        <div id=\"service_box_%s\" class=\"service-boxwhite\" style=\"height:12px;float:right;padding:2px 5px 5px 5px;\" >%s</div> \
        </div>'%(host_id,"hostgroup"+hostgroup_id,host_status,host_id,host_alias,priority_dict[priority],host_id,normal_check_interval)
        return html_str
    except Exception,e:
        return str(e)
    ...
    try:
		...
		'%(data_dict[data][0],data_dict[data][1],data_dict[data][0],data_dict[data][1].capitalize())
		    ...
		    if len(data_dict[data])>2:
			    ...
			    html_str+=make_service_box(hosts[0],data_dict[data][0],hosts[1],hosts[2],hosts[3],hosts[4])
		    html_str+='</div>'
	    else:
...
def edit_hostgroup_service_details(h): 
	...
	result = cursor.fetchall()
	conn.close()
        html_view='<div style="display:block;overflow:hidden"><h2>%s</h2>'%hostgroup_alias.capitalize()
	...
	html_view+='<div class=\"row-elem\">\
	   	...
	   	<select name="hosts_list" id="hosts_list" class="multiselect" multiple="multiple">'
	for row in result:
	...
	<button class=\"yo-small yo-button\" id=\"apply_changes_host_services\" type=\"button\">Apply Changes</button>'
        ...
        html.write(str(query))

def apply_nagios_hostgroup_changes(h):
    global html
    html = h
    hostgroup_id=html.var("hostgroup_id")
    hosts_list=html.var("hosts_list")
    time_service=html.var("time_service")
    try:
	conn = MySQLdb.connect(*SystemConfig.get_mysql_credentials())
	cursor = conn.cursor()
	query="update host_services set normal_check_interval='%s' where service_description like 'Statictics Service%%' and host_id in ('%s') "%(time_service, "','".join(hosts_list.split(',')))
	cursor.execute(query)
	conn.commit()
	conn.close()
	...
	nagios_config_obj.write_nagios_config(nms_instance)
        html.write(str({"success":0, "result":query}))
    except Exception,e:
