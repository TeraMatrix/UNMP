...
"""
# import the packeges
import htmllib,config,lib,MySQLdb
    ...
    try:
        ...
        if host_id!='' or host_id!=None or host_id.strip()!='undefined':
            ...
            if len(ip_address)>0:
                ...
                cursor.callproc("idu_nw_interfcace_statistics",(start_time,end_time,get_refresh_time(),0,host_id))
            ...
            db.close()
        output_dict={'success':0,'rxBytes':rxBytes,'txBytes':txBytes,'timestamp':timestamp,'idu_ip_address':idu_ip_address}
        html.write(str(output_dict))
    ...
    try:
        ...
        if len(result)>0:
            if result[7] !="" or result!=None:
                ...
                second=result[7]%60
            device_detail += "<td style=\"margin-left:2px; align:center;\">"+ str('--' if result[0]==None or result[0]==""  else result[0])+ "</td>"
            device_detail += "<td style=\"margin-left:2px; align:center;\">"+ str('--' if result[1]==None or result[1]==""  else result[1])+ "</td>"
            device_detail += "<td style=\"margin-left:2px; align:center;\">"+ str('--' if result[2]==None or result[2]==""  else result[2])+ "</td>"
            device_detail += "<td style=\"margin-left:2px; align:center;\">"+ str('--' if result[3]==None or result[3]==""  else result[3])+ "</td>"
            device_detail += "<td style=\"margin-left:2px; align:center;\">"+ str('--' if result[4]==None or result[4]==""  else result[4])+ "</td>"
            device_detail += "<td style=\"margin-left:2px; align:center;\">"+ str('--' if result[5]==None or result[5]==""  else result[5])+ "</td>"
            device_detail += "<td style=\"margin-left:2px; align:center;\">"+ str('--' if result[6]==None or result[6]==""  else result[6])+ "</td>"
            device_detail += "<td style=\"margin-left:2px; align:center;\">"+ str(str(hour)+"Hr "+str(minute)+"Min "+str(second)+"Sec")+ "</td>"
        else:
...
def idu_tdmoip_network_interface_graph(h):
    ...
    try:
        ...
        if db ==1 or db =='1' :
            raise SelfException(cursor)
        cursor.callproc("idu_tdmoip_nw_interfcace_statistics",(start_time,end_time,get_refresh_time(),0,host_id))
        idu_nw_result=cursor.fetchall()
        ...
        output_dict={'success':0,'tx_transmiting':tx_transmiting,'rx_receving':rx_receving,'timestamp':timestamp}
        html.write(str(output_dict))
    ...
    try:
        ...
        if len(idu_nw_result)>0:
            for row in idu_nw_result:
                ...
                link_speed.append(100 if str(row[1]).strip()=='enable' else 0)
                port_name.append('na' if row[0]==1 or row[0].strip()=='1' else row[0])

        output_dict={'success':0,'operation_state':operation_state,'link_speed':link_speed,'port_name':port_name}
        html.write(str(output_dict))
    ...
    try:
        ...
        for i in range(length):
            if i%2==0:
                ...
                tr_css="even"
            history_trap+="<tr class=\"%s\"><td><img src=\"%s\" alt=\"%s\" title=\"%s\" class=\"imgbutton\" style=\"width:13px\" /></td>"% (tr_css,image_dic[all_traps[i][0]],image_title_name[all_traps[i][0]],image_title_name[all_traps[i][0]])
        ...
        history_trap+="<tr class=\"%s\"><td></td><td></td><td></td><td style=\"text-align:right\">%s</td></tr></table>"%(tr_css,("<a href=\"status_snmptt.py?trap_status=history&ip_address="+ip_address+"\">more>></a>" if len(all_traps) >5 else "" ))
        ...
        for i in range(length):
            if i%2==0:
                ...
                tr_css="even"
            current_alarm_html+="<tr class=\"%s\"><td><img src=\"%s\" alt=\"%s\" title=\"%s\" class=\"imgbutton\" style=\"width:13px\" /></td>"% (tr_css,image_dic[current_alarm[i][0]],image_title_name[current_alarm[i][0]],image_title_name[current_alarm[i][0]])
        ...
        current_alarm_html+="<tr class=\"%s\"><td></td><td></td><td></td><td style=\"text-align:right\">%s</td></tr></table>"%(tr_css,"<a href=\"status_snmptt.py?trap_status=current&ip_address"+ip_address+"\" id=\"status_id\">more>></a>" if len(current_alarm) >5 else "" )
        # close database connection and cursor.
        cursor.close()
        db.close()

        history_trap_detail={'success':0,'trap_output':history_trap,'alarm_output':current_alarm_html}
...
def idu_outage_graph(h):
    ...
    current_datetime=datetime.strptime(str(current_date+timedelta(days=-10))+" 00:00:00",'%Y-%m-%d %H:%M:%S') # convert the string in  datetime. 
                ...
                if (temp_date+timedelta(days=-(i+1)))<=row[1]<= (temp_date+timedelta(days=-(i))):
                    ...
                    if row[2]==1:
                        if temp_down_time=='':
                            ...
                            total_down_time+=(temp_down_time-row[1]).days*1440+(temp_down_time-row[1]).seconds/60
                    else:
                        if temp_up_time=='': 
                            temp_up_time=row[1]
                        else:
                            total_up_time+=(temp_up_time-row[1]).days*1440+(temp_up_time-row[1]).seconds/60
