...
def client_dashboard_profiling(h):
    ...
    javascript_list = ["js/highcharts.js",'js/ccpl_jquery_autocomplete.js',"js/client_dashboard.js","calendrical/calendrical.js"]
    mac_address = ""
    ...
    html.new_header(str(device_name_dict[device_type])+" %s Dashboard"%('Client'),"ap_listing.py","",css_list,javascript_list)
...
def client_add_date_time_on_slide(h):
    ...
    try:
        ...
        graph_result=client_bll_obj.client_dashboard_get_graph_name(user_id,device_type)
        now=datetime.now()
        sp_end_date=now.strftime("%d/%m/%Y")
        sp_end_time=now.strftime("%H:%M")
        now=now+timedelta(minutes=-int(300))
        ...
        if int(graph_result['success'])==1:
            ...
            output_dict={'success':0,'end_date':sp_end_date,'end_time':sp_end_time,'start_date':sp_start_date,'start_time':sp_start_time,'show_graph_table':ClientDashboardView.sp_get_graph(graph_result['selected'],graph_result['non_selected'])}
...
#    sp_start_date=now.strftime("%d/%m/%Y")
#   sp_start_time=now.strftime("%H:%M")
    html.write(ClientDashboardView.sp_table(mac_address,str(sp_refresh_time),str(total_count),path,host_id))
...
def client_generic_json(h):
    ...
    user_id=html.req.session["user_id"]
    result_dict=client_bll_obj.client_all_graph_json(device_type,user_id,mac_address)
    h.req.content_type = 'application/json'
...
def client_graph_creation(h):
    ...
    if int(graph_type)==2:
        ...
        result_dict=client_bll_obj.client_graph_json(display_type,user_id,table_name[0],table_name[1],table_name[-2],table_name[-1],flag,start_date,end_date,start,limit,mac_address,graph_type,update_field_name,interface_value,cal_type,column_name)
        h.req.content_type = 'application/json'
        h.req.write(str(JSONEncoder().encode(result_dict)))
...
def update_client_graph(h):
    ...
    result=client_bll_obj.update_show_graph_table(device_type,user_id,show_graph)
    html.write(str(result))
...
def client_excel_report_genrating(h):
    ...
    try:
        ...
        user_id=html.req.session["user_id"]    
        result=client_bll_obj.client_total_display_graph(device_type_id,user_id)
        ...
        #    start_date=datetime.strptime(start_date+' '+start_time,"%d/%m/%Y %H:%M")
        #    end_date=datetime.strptime(end_date+' '+end_time,"%d/%m/%Y %H:%M")
            result_dict=client_bll_obj.client_excel_report(device_type_id,user_id,mac_address,cal_list,tab_list,field_list,table_name_list,graph_name_list,start_date,end_date,select_option,limitFlag,graph_list,start_list,limit_list)
            h.req.write(str(JSONEncoder().encode(result_dict)))
        else:
    ...
    try:
        ...
        if int(result['success'])==0:
            ...
            if int(select_option)>0:
                ...
                start_date=datetime.strptime(start_date+' '+start_time,"%d/%m/%Y %H:%M")
                end_date=datetime.strptime(end_date+' '+end_time,"%d/%m/%Y %H:%M")
            result_dict=client_bll_obj.client_csv_report(device_type_id,user_id,mac_address,cal_list,tab_list,field_list,table_name_list,graph_name_list,start_date,end_date,select_option,limitFlag,graph_list,start_list,limit_list)
            h.req.write(str(JSONEncoder().encode(result_dict)))
        else:
            h.req.write(str(JSONEncoder().encode(result)))
