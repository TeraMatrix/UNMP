...
def firmware_update_device_show(h):
    ...
    css_list = ['css/ie7.css','css/custom.css','css/demo_table_jui.css','css/jquery-ui-1.8.4.custom.css']
    jss_list = ['js/jquery.dataTables.min.js','js/firmware_updates.js','js/jquery-ui-personalized-1.6rc2.min.js']
    ...
    host_param = device_parameter_bll_obj.get_device_parameter(host_id)# call the idu_profiling_controller function get_device_param returns the list.List Contains ipaddress,macaddress,devicetypeid,config_profile_id
    if isinstance(host_param,list):
        if host_param == [] or host_param == None:
            html.write(page_header_search("","","UBR,UBRe",None,"enabled","device_type"))#call the function of common_controller , it is used for listing the Devices based on IPaddress,Macaddress,DeviceTy
        else:
            html.write(page_header_search(host_param[0][0],host_param[0][1],"UBR,UBRe",device_type,device_list_state,"device_type"))
...
def firmware_master_slave_list(h):
    ...
    device_list = firmware_device_list_bll_obj.device_list_profiling(ip_address,mac_address,selected_device)
    ...
    if host_id == None or host_id == "":
        ...
        if selected_device==None or selected_device == "":
            ...
            node_type = obj_bll_get_node_type.get_node_type(host_id,selected_device)   
            if len(node_type)>0 and len(node_type[0])>0:
                if node_type[0][0]!=0:
                    master_host_id = obj_bll_get_node_type.get_master_host_id(host_id) 
                    if len(master_host_id)>0 and len(master_host_id[0])>0:
                        master_slave_list,final_result = obj_bll_get_node_type.get_master_slave_snmp(str(master_host_id[0][0]),selected_device)
                else:
                    master_slave_list,final_result = obj_bll_get_node_type.get_master_slave_snmp(host_id,selected_device)
    ...
    if filename==None or filename == "":
        ...
        if extension[-1]=="img":
            if device_type == UNMPDeviceType.odu16:
                ...
                folder_name = "odu100"
            file_path = "/omd/sites/%s/share/check_mk/web/htdocs/download/firmware_downloads/%s/%s"%(nms_instance,folder_name,filename)
            try:
                ...
                fobj.write(filedata)
                fobj.close()
                upload_file = objbll.ftp_upload(file_path,device_type,filename)
            
        else:
            flag = 2
    if flag == 1:
        html.write("<p style=\"font-size:10px;\">Please select a file for Upload<br/><a href=\"javascript:history.go(-1);;\">back</a><p/>")
    elif flag == 2:
        ...
        html.write("<p style=\"font-size:10px;\">Path not exist for uploading the file.Please contact your administrator<br/></p>")
    else:
        html.write("<p style=\"font-size:10px;\">Firmware Uploading....<br/>Please Wait....<p/>")
        html.write("<p style=\"font-size:10px;\">"+str(upload_file["result"])+"<br/><p/>")    
