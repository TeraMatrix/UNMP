...
class APAdvancedGraph(object):
    ...
    def get_host_id(self,ip_address,device_type):
        try:
            ...
            sel_query="SELECT host_id FROM hosts WHERE device_type_id='%s' and ip_address=%s and is_deleted=0"%(device_type,str(ip_address))
...
#		graph_json.append(graph_dict)
	...
	sp_bll_obj=SPDashboardBll() # specific bll dashboard file object created for reusablity
		...
		if (end_date-start_date).days < 5:
			start_date=datetime.strptime(datetime.strftime(datetime.now()+timedelta(days=-4),"%Y-%m-%d %H:%M:%S"),"%Y-%m-%d %H:%M:%S")
		...
		if display_type=='table' or display_type=='excel':
			...
			if display_type=='table' or display_type=='excel':
                            ...
                            time_list.append(time.mktime(datetime.strptime(str(row[len(row)-1])[:19],'%Y-%m-%d %H:%M:%S').timetuple())*1000)
			...
			if display_type=='table' or display_type=='excel':
                            ...
                            time_list.append(time.mktime(datetime.strptime(str(result[k][len(result[k])-1])[:19],'%Y-%m-%d %H:%M:%S').timetuple())*1000)
...
#            datatable_column_list=[]
            ...
            device_name_list={'ap25':'AP25','odu16':'RM18','odu100':'RM','idu4':'IDU','ccu':'CCU'}
            login_user_name=''
		...
		return output_dict
            ...
            sel_query="SELECT device_type.device_name,hosts.host_alias FROM hosts INNER JOIN device_type ON hosts.device_type_id=device_type.device_type_id WHERE hosts.ip_address='%s' and hosts.is_deleted=0"%(ip_address)
            cursor.execute(sel_query)
            host_result=cursor.fetchall()
            device_type=''
            device_name=''
...
'idu_e1PortStatusTable':{'1':'(port1)','2':'(port2)','3':'(port3)','4':'(port4)'},'idu_portstatisticsTable':idu4_port,'idu_swPrimaryPortStatisticsTable':idu4_port,'idu_portSecondaryStatisticsTable':idu4_port}
		...
		interface_list['odu100_peerNodeStatusTable']={'1':'(Link1)','2':'(Link2)','3':'(Link3)','4':'(Link4)','5':'(Link5)','6':'(Link6)','7':'(Link7)','8':'(Link8)','9':'(Link9)',
            ...
            if report_type=='excelReport':
                ...
                sheet_no=1
                xls_sheet=xls_book.add_sheet('%s%s'%(graph_name1,sheet_no),cell_overwrite_ok=True)
			...
			sheet_no+=1
                ...
                ofile.close()
            output_dict={'success':0,'output':'Report %s Generated Successfully.'%(graph_name1),'file_name':str(save_file_name),'path':path}
            return output_dict 
        except MySQLdb as e:
            output_dict={'success':1,'error_msg':'Error No : 102 '+str(err_obj.get_error_msg(102)),'main_msg':str(e[-1])}
            return output_dict
