...
class APAdvancedGraph(object):
    ...
    def get_host_id(self,ip_address,device_type):
        try:
            ...
            sel_query="SELECT host_id FROM hosts WHERE device_type_id='%s' and ip_address=%s "%(device_type,str(ip_address))
            cursor.execute(sel_query)
            host_result=cursor.fetchall()
            if len(host_result)>0:
                host_id=host_result[0][0] if host_result[0][0]!=None or host_result[0][0]!="" else ""   
    ...
    def total_graph_name_display(self,device_type_id,user_id):
        try:
            ...
            if db ==1:
                raise SelfException(cursor)
            sel_query="SELECT graph_display_name,graph_display_id FROM graph_templet_table WHERE device_type_id='%s' and user_id='%s' and is_disabled=0 and dashboard_type=0"%(device_type_id,user_id)
            cursor.execute(sel_query)
...
#                        graph_json.append(graph_dict)
    ...
    def advanced_graph_data(self,display_type,user_id,table_name,x_axis_value,index_name,graph_id,start,limit,flag,start_date,end_date,ip_address,graph,update_field_name,interface=1,value_cal=1,*column_name):
        ...
        output_dic={}
        json_data=[]
	sp_bll_obj=SPDashboardBll() # specific bll dashboard file object created for reusablity
        ...
        try:
            ...
            if str(graph_id).strip()=='apVAPUserConnect':
                ...
                time_list=mac_list
                if var_count==2:
                            json_data.append({'name':['RSSI',' dbm'],'data':rssi_list})
                else:
		...
		if (end_date-start_date).days < 5:
			start_date=datetime.now()+timedelta(days=-4)
                outage_result=sp_bll_obj.sp_advanced_outage_graph(display_type,ip_address,start_date,end_date)
                if int(outage_result['success'])!=0:
                    return outage_result
                for name in column_name[0]:
                    json_data.append({"data":outage_result[name],"name":[name,'Time(%)']})
                ...
                return output_dic
            elif (table_name.strip()=='odu100_peerNodeStatusTable' or table_name.strip()=='get_odu16_peer_node_status_table') and (graph_id.strip()=='odu100rssi' or graph_id.strip()=='odu100link' or  graph_id.strip()=='odu100rssi2' or graph_id.strip()=='odu16rssi'):
...
#                odu100_rssi_result=self.odu100_signal_strength_graph(ip_address,start_date,end_date,table_name,column_name[0])
...
#				odu100_rssi_result['display_signal_strength'][k]['data'][col]='Device Unreachable'
#			k+=1
		...
		if display_type=='table' or display_type=='excel':
			...
			if display_type=='table' or display_type=='excel':
                            ...
                            time_list.append(time.mktime(datetime.strptime(str(row[len(row)-1])[:19],'%Y-%m-%d %H:%M:%S').timetuple())*1000+(5.5*3600000))
                            #time_list.append(datetime.strptime(str(row[len(row)-1])[:19],'%Y-%m-%d %H:%M:%S'))
                            #time_list.append(str((row[len(row)-1]).strftime("%H:%M")))
			    ...
			    for d3 in range(len(result[k])-1):
			        data_list[d3].append(default_value)
                            ...
                            for i  in range(len(result[k])-1):
                                #val=int(result[k][i])-int(result[k+1][i])
			...
			if display_type=='table' or display_type=='excel':
                            time_list.append(str((result[k][len(result[k])-1])))
			else:
                            #time_list.append(time.mktime(result[k][len(result[k])-1].timetuple())*1000+(5.5*3600000))
                            time_list.append(time.mktime(datetime.strptime(str(result[k][len(result[k])-1])[:19],'%Y-%m-%d %H:%M:%S').timetuple())*1000+(5.5*3600000))
#                            time_list.append(str((result[k][len(result[k])-1]).strftime("%H:%M")))
...
#            datatable_column_list.extend(field_column)
            
            conn,cursor=mysql_connection()
            if conn ==1:
                raise SelfException(cursor)

            device_name_list={'ap25':'AP25','odu16':'UBR','odu100':'UBRe','idu4':'IDU'}
#             we are geting the login user name here. 
            ...
            #sel_query='SELECT graph_field_value,graph_field_display_name FROM graph_field_table WHERE '
            merge_result=[]
            d1_list=[]
            output_result=self.advanced_graph_data('excel',user_id,table_name,x_axis,index_name,graph_id,start1,limit1,limitFlag,start_date,end_date,ip_address,graph1,update_field_name,tab1,cal1,field_column)
	    ...
	    # check the data exist or not or field select or not in graph.
	    if len(headings)<2:
		output_dict={'success':1,'error_msg':''+str(err_obj.get_error_msg(106))}
		return output_dict
...
'idu_e1PortStatusTable':{'1':'(port1)','2':'(port2)','3':'(port3)','4':'(port4)'},'idu_portstatisticsTable':idu4_port,'idu_swPrimaryPortStatisticsTable':idu4_port,'idu_portSecondaryStatisticsTable':idu4_port}
		...
		interface_list['odu100_peerNodeStatusTable']={'1':'(Link1)','2':'(Link2)','3':'(Link3)','4':'(Link4)','5':'(Link5)','6':'(Link6)','7':'(Link7)','8':'(Link8)','9':'(Link9)',
            ...
            interface_name='' if  len(table_dic_key)==0 else interface_list[table_dic_key[0].strip()][tab1] 
            if report_type=='excelReport':
                save_file_name=str(device_name_list[device_type_id])+'_'+str(login_user_name)+'_'+str(start_date)+'.xls'
                xls_sheet=xls_book.add_sheet('Device Statistics',cell_overwrite_ok=True)
                xls_sheet.row(0).height = 521
                xls_sheet.row(1).height = 421
                xls_sheet.write_merge(0,0,0,len(headings)-1,str(graph_name1)+' '+str(interface_name)+'  ('+str(start_date)+'-'+str(end_date)+')',style)
                xls_sheet.write_merge(1,1,0,len(headings)-1,str(device_type)+'       '+str(ip_address)+str(device_postfix)+'  Calculation Type -: '+str(cal_type) ,style)
    ...
    #            headings=datatable_column_list # create the column name for execl creating
        ...
        #        heading=[table_output[0][1],table_output[0][2],table_output[0][3],table_output[0][4],table_output[0][5],table_output[0][6],table_output[0][7]]
                ...
                save_file_name=str(device_name_list[device_type_id])+'_'+str(login_user_name)+'_'+str(start_date)+'.csv'
                path='/omd/sites/%s/share/check_mk/web/htdocs/download/%s'%(nms_instance,save_file_name)
                ofile = open(path, "wb")
                writer = csv.writer(ofile, delimiter=',', quotechar='"')
                blank_row=["","",""]
                main_row=[str(graph_name1)+str(interface_name),str(start_date),str(end_date)]
                second_row=[str(device_type),str(ip_address)+str(device_postfix),'Calculation Type -: '+str(cal_type)]
    ...
    def ap_data_table(self,user_id,ip_address,cal1,tab1,field1,table_name1,graph_name1,start_date,end_date,limitFlag,graph1,start1,limit1):
        try:
            ...
            graph_field_dict=dict((field[0],field[1]) for field in field_result)
            graph_tip_dict=dict((field[0],field[2]) for field in field_result)
            
	    master_slave_status=get_master_slave_value(ip_address)
	    table_name_list=['odu100_peerNodeStatusTable','get_odu16_peer_node_status_table']

	    if master_slave_status['success']==0:
		if int(master_slave_status['status'])==1 and table_name in table_name_list and graph_id!="odu100peernode":
            	    for val in field_column:
                	if str(val).strip()!="":
                    	    datatable_column_list.append('MASTER-Link'+' '+str(graph_tip_dict[val]))
		else:
            	    for val in field_column:
                	if str(val).strip()!="":
                    	    datatable_column_list.append(str(graph_field_dict[val])+' '+str(graph_tip_dict[val]))
	    else:
		raise SelfException('MySQL Problem.')

            merge_result=[]
            output_result=self.advanced_graph_data('table',user_id,table_name,x_axis,index_name,graph_id,start1,limit1,limitFlag,start_date,end_date,ip_address,graph1,update_field_name,tab1,cal1,field_column)
            if int(output_result['success'])==0:
                d1_list.append(output_result['timestamp'])
                for data_list in output_result['data']:
                    d1_list.append(data_list['data'])
            ...
            output_dict={'success':0,'table':merge_result,'column':datatable_column_list}
            return output_dict 
        except MySQLdb as e:
            output_dict={'success':1,'error_msg':'Error No : 102 '+str(err_obj.get_error_msg(102)),'main_msg':str(e[-1])}
            return output_dict
