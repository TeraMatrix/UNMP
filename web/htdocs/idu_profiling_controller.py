...
def idu_listing(h):
    try:
        ...
        javascript_list = ["js/jquery.dataTables.min.js",'js/ccpl_jquery_autocomplete.js',"js/idu_listing.js"]
        ...
        html.new_header("IDU Listing","idu_listing.py","",css_list,javascript_list,snapin_list)
...
def idu_profiling(h):
    ...
    jss_list = ["js/jquery-ui-1.8.6.custom.min.js","js/pages/jquery.multiselect.min.js",\
    ...
    if isinstance(device_list_parameter,list):
        if device_list_parameter == [] or device_list_parameter == None:
            html.write(page_header_search("","","RM18,RM,IDU,CCU",None,"enabled","device_type"))#call the function of common_controller , it is used for listing the Devices based on IPaddress,Macaddress,DeviceTy
        else:
            html.new_header("%s %s Configuration"%("IDU",device_list_parameter[0].ip_address),"idu_listing.py","",css_list,jss_list,snapin_list)
            html.write(page_header_search(device_list_parameter[0][0],device_list_parameter[0][1],"RM18,RM,IDU,CCU",device_type,device_list_state,"device_type"))
            html.write(IduProfiling.idu_div(device_list_parameter[0][0],device_list_parameter[0][1],host_id))
...
def idu_device_listing_table(h):
    ...
    device_dict = idu_device_list_bll_obj.idu_device_list(ip_address,mac_address,selected_device,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,userid,html_req)
...
#This is a empty list variable used for storing the device list
    if isinstance(device_list,list):
        for i in range(0,len(device_list)):
            ...
            e1_port_list = obj_get_data.common_get_data('IduE1PortConfigurationTable',device_list[i][0])
            main_admin_state_list = obj_get_data.common_get_data('IduIduAdminStateTable',device_list[i][0])
            ...
            if i==len(device_list)-1:
                ...
                device_status_host_id+=str(device_list[i][0])+","
            temp_list = []
            monitoring_status = "<a target=\"main\" href=\"%s?host_id=%s&device_type=%s&device_list_state=%s\"><img src=\"images/%s/info.png\" style=\"width:16px;height:16px;\" title=\"Current Device Status\" class=\"imgbutton n-reconcile\"/></a>"%('sp_status_profiling.py',\
                                                                                                                                                                                                                                                          device_list[i][0],device_list[i][7],device_list_state,theme) if device_list[i][7]=="idu4" else "<img src=\"images/%s/info.png\" style=\"width:16px;height:16px;\" title=\"Current Device Status\" class=\"imgbutton n-reconcile\"/>"%(theme)
            if html.req.session["role"] == "admin" or html.req.session["role"] == "user":
                ...
                temp_list.append("<ul class=\"button_group\" style=\"width: 113px;\">")
                for j in range(0,len(e1_port_list)):
                    ...
                    onClick=\"idu_admin_state_change(event,this,'%s','%s','%s','iduConfiguration.e1PortConfigurationTable.adminState',0);\">E1</a></li>"
                                     %("green" if int(e1_port_list[j].adminState)==1 else "red",str(e1_port_list[j].portNumber),"Unlocked" if int(e1_port_list[j].adminState)==1 else "Locked",\
                ...
                temp_list.append('<li><a class=\"%s n-reconcile imgEditodu16\" title=\"%s\" onclick=\"main_admin_state_change(event,this,\'%s\',\'iduinfo.iduAdminStateTable.adminstate\'); \"/>A</a></li>'%(main_admin_image_class,main_admin_title,device_list[i][0]))
                temp_list.append('<li><a class=\"%s n-reconcile imgEditodu16\" title=\"%s\" onclick=\"show_link_admin_state(event,this,\'%s\',\'%s\');\"/>L</a></li>'%(link_image_class,link_title,device_list[i][0],device_list[i][7]))
                temp_list.append("</ul>")
                ...
                &nbsp;&nbsp;%s%s"
                                 %(device_list[i][0],device_list[i][7],device_list_state,device_list[i][0],\
                                   'sp_dashboard_profiling.py' if device_list[i][7] == "idu4" else 'sp_dashboard_profiling.py',\
                                   device_list[i][0],device_list[i][7],device_list_state,device_list[i][3],device_list[i][0],device_list[i][7],\
                                   device_list_state,images,device_list[i][8],device_list[i][0],device_list[i][7],monitoring_status,\
                ...
                temp_list.append("<ul class=\"button_group\" style=\"width: 170px;\">")
                for j in range(0,len(e1_port_list)):
                    temp_list.append("<li><a class=\"%s imgEditodu16 n-reconcile\" title=\"E1 Port%s %s\" id=\"e1_admin_state_%s\" name=\"admin_state_%s\" state=\"%s\" \
                    onClick=\"idu_admin_state_change(event,this,'%s','%s','%s','iduConfiguration.e1PortConfigurationTable.adminState',0);\">E1</a></li>"
                                     %("green" if int(e1_port_list[j].adminState)==1 else "red",str(e1_port_list[j].portNumber),"Unlocked" if int(e1_port_list[j].adminState)==1 else "Locked",\
                ...
                &nbsp;&nbsp;%s%s"
                                 %(device_list[i][0],\
                                   'sp_dashboard_profiling.py' if device_list[i][7] == "idu4" else 'sp_dashboard_profiling.py',\
                                   device_list[i][0],device_list[i][7],device_list_state,device_list[i][3],images,device_list[i][8],monitoring_status,\
                                   "<input type=\"hidden\" value=\"%s\" name=\"host_id\" id=\"host_id\" />"%(device_status_host_id) if i==len(device_list)-1 else ""))
                result_device_list[i].append(" ".join(temp_list))
...
def get_device_list_idu_profiling(h): 
    try:
        ...
        device_list = idu_device_list_bll_obj.idu_device_list_profiling(ip_address,mac_address,selected_device)
        ...
        if result==0 or result==1 or result==2:
            ...
            device_parameter_bll_obj = DeviceParameters()
            device_list_parameter = device_parameter_bll_obj.get_device_parameter(result)# call the idu_profiling_controller function get_device_param returns the list.List Contains ipaddress,macaddress,devicetypeid,config_profile_id
            #html.write(str(device_list_parameter))
            html.write(IduProfiling.idu_profile_call(result,selected_device,device_list_parameter))
    except Exception as e:
        html.write(str(e[-1]))
...
def port_configuration_form(h):
    ...
    port_configuration_list = idu_get_database_obj.common_get_data_by_id(port_configuration_id,host_id,id_name,class_name)
    if isinstance(port_configuration_list,list):
        html.write(IduForms.port_configuration_form(port_configuration_list,host_id,selected_device,port_configuration_id))
...
def port_bandwidth_form(h):
    ...
    port_bandwidth_list = idu_get_database_obj.common_get_data_by_id(port_bandwidth_id,host_id,id_name,class_name)
    if isinstance(port_bandwidth_list,list):
        html.write(str(IduForms.port_bandwidth_form(port_bandwidth_list,host_id,selected_device,port_bandwidth_id)))
...
def port_QinQ_form(h):
    ...
    port_bandwidth_list = idu_get_database_obj.common_get_data_by_id(port_bandwidth_id,host_id,id_name,class_name)
    if isinstance(port_bandwidth_list,list):
        html.write(str(IduForms.port_QinQ_form(port_bandwidth_list,host_id,selected_device,port_bandwidth_id)))
    elif isinstance(port_bandwidth_list,Exception):
        html.write(str(e))
...
def e1_port(h):
    ...
    selected_device = html.var("selected_device")
    #index = html.var("index")
    port_e1_list = idu_get_database_obj.common_get_data_by_id(port_e1_id,host_id,id_name,class_name)
    if isinstance(port_e1_list,list):
        html.write(str(IduForms.form_e1_port(port_e1_list,host_id,selected_device,port_e1_id)))
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("e1_port_id")!=None:
            if obj_bll_cancel_set_valid.chk_link_e1_port(host_id,int(html.var("e1_port_id")))==0:
                ...
                flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Delete all link in this port before applying this change"
        ...
        if html.var("iduConfiguration.e1PortConfigurationTable.clockSource")!=None:
            if Validation.is_required(html.var("iduConfiguration.e1PortConfigurationTable.clockSource")):
                dic_result["iduConfiguration.e1PortConfigurationTable.clockSource"]=html.var("iduConfiguration.e1PortConfigurationTable.clockSource")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("iduConfiguration.e1PortConfigurationTable.lineType")!=None:
            if Validation.is_required(html.var("iduConfiguration.e1PortConfigurationTable.lineType")):            
                dic_result["iduConfiguration.e1PortConfigurationTable.lineType"]=html.var("iduConfiguration.e1PortConfigurationTable.lineType")
            else:
                flag = 1
                dic_result["success"] = 1
...
##                dic_result["adminState"]=html.var("adminState")
        ...
        if flag == 1:
            ...
            result = obj_bll_cancel_set_valid.e1_port_set(host_id,device_type_id,dic_result,id,index)
            #time.sleep(7)
            html.req.content_type = 'application/json'
        ...
        if html.var("iduConfiguration.e1PortConfigurationTable.lineCode")!=None:
            dic_result["iduConfiguration.e1PortConfigurationTable.lineCode"] = html.var("iduConfiguration.e1PortConfigurationTable.lineCode")
        result = obj_bll_cancel_set_valid.idu_cancel_form(host_id,device_type_id,dic_result)
        html.req.content_type = 'application/json'
        html.req.write(str(JSONEncoder().encode(result)))
...
def port_vlan_add_form(h):
    ...
    #index = html.var("index")
    if int(addEdit)==1:
        port_vlan_list = idu_get_database_obj.common_get_data_by_id(port_vlan_id,host_id,id_name,class_name)

    if isinstance(port_vlan_list,list):
        html.write(str(IduForms.port_vlan_form(port_vlan_list,host_id,selected_device,port_vlan_id,addEdit,vlanId)))
...
def vlan_form_action(h):
    try:
        ...
        if html.var("host_id") == "" or html.var("host_id") == None:
            ...
            if html.var("switch.vlanconfigTable.vlanname")!=None:
                if Validation.is_required(html.var("switch.vlanconfigTable.vlanname")):
                    if obj_bll_cancel_set_valid.check_vlan_name(html.var("switch.vlanconfigTable.vlanname"),host_id)==0:
                        dic_result["switch.vlanconfigTable.vlanname"]=html.var("switch.vlanconfigTable.vlanname")
                    else:
                        flag = 1
                        dic_result["success"] = 1
                        dic_result["result"] = "Vlan Name already exist.Please enter different vlan name"    
                else:
                    flag = 1
                    dic_result["success"] = 1
                    ...
                    if Validation.is_number(html.var("switch.vlanconfigTable.vlantag")):
                        if int(html.var("switch.vlanconfigTable.vlantag"))>0 and int(html.var("switch.vlanconfigTable.vlantag"))<4096:
                            if obj_bll_cancel_set_valid.check_vlan_tag(html.var("switch.vlanconfigTable.vlantag"),host_id)==0:
                                dic_result["switch.vlanconfigTable.vlantag"]=html.var("switch.vlanconfigTable.vlantag")
                            else:
                                flag = 1
                                dic_result["success"] = 1
                            ...
                            flag = 1
                            dic_result["success"] = 1
                            dic_result["result"] = "Vlan Tag is between 1 and 4095"        
                    else:
                        flag = 1
                        dic_result["success"] = 1
            ...
            dic_result["switch.vlanconfigTable.memberports"]=html.var("number")
            ...
            if flag == 1:
                ...
                #html.write(str(str(host_id)+str(device_type_id)+str(dic_result)+str(id)+str(index)))
                result = obj_bll_cancel_set_valid.vlan_set(host_id,device_type_id,dic_result,addEdit,id,index)
                html.req.content_type = 'application/json'
                html.req.write(str(JSONEncoder().encode(result)))
...
def device_update_reconciliation(h):
    ...
    if insert_update == None:
        insert_update = True
    result = bll_rec_obj.update_device_reconcilation_controller(host_id,device_type_id,table_prefix,insert_update,user_name)
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(result)))
...
def temperature_form_action(h):
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("iduConfiguration.temperatureSensorConfigurationTable.tempMin")!=None:
            if Validation.is_required(html.var("iduConfiguration.temperatureSensorConfigurationTable.tempMin")):
                if Validation.is_number(html.var("iduConfiguration.temperatureSensorConfigurationTable.tempMin")):
                    dic_result["iduConfiguration.temperatureSensorConfigurationTable.tempMin"]=html.var("iduConfiguration.temperatureSensorConfigurationTable.tempMin")
                    temp_min = int(html.var("iduConfiguration.temperatureSensorConfigurationTable.tempMin"))
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if html.var("iduConfiguration.temperatureSensorConfigurationTable.tempMax")!=None:
            if Validation.is_required(html.var("iduConfiguration.temperatureSensorConfigurationTable.tempMax")):
                if Validation.is_number(html.var("iduConfiguration.temperatureSensorConfigurationTable.tempMax")):
                    temp_max = int(html.var("iduConfiguration.temperatureSensorConfigurationTable.tempMax"))
                    dic_result["iduConfiguration.temperatureSensorConfigurationTable.tempMax"]=html.var("iduConfiguration.temperatureSensorConfigurationTable.tempMax")
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if flag == 1:
            ...
            result = obj_bll_cancel_set_valid.common_validation(host_id,device_type_id,dic_result)
            html.req.content_type = 'application/json'
            html.req.write(str(JSONEncoder().encode(result)))
        ...
        if html.var("iduConfiguration.rtcConfigurationTable.year")!=None:
            if Validation.is_required(html.var("iduConfiguration.rtcConfigurationTable.year")):
                if Validation.is_number(html.var("iduConfiguration.rtcConfigurationTable.year")):
                    dic_result["iduConfiguration.rtcConfigurationTable.year"]=html.var("iduConfiguration.rtcConfigurationTable.year")
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if html.var("iduConfiguration.rtcConfigurationTable.month")!=None:
            if Validation.is_required(html.var("iduConfiguration.rtcConfigurationTable.month")):
                if Validation.is_number(html.var("iduConfiguration.rtcConfigurationTable.month")):
                    dic_result["iduConfiguration.rtcConfigurationTable.month"]=html.var("iduConfiguration.rtcConfigurationTable.month")
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if html.var("iduConfiguration.rtcConfigurationTable.day")!=None:
            if Validation.is_required(html.var("iduConfiguration.rtcConfigurationTable.day")):
                if Validation.is_number(html.var("iduConfiguration.rtcConfigurationTable.day")):
                    dic_result["iduConfiguration.rtcConfigurationTable.day"]=html.var("iduConfiguration.rtcConfigurationTable.day")
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if html.var("iduConfiguration.rtcConfigurationTable.hour")!=None:
            if Validation.is_required(html.var("iduConfiguration.rtcConfigurationTable.hour")):
                if Validation.is_number(html.var("iduConfiguration.rtcConfigurationTable.hour")):
                    dic_result["iduConfiguration.rtcConfigurationTable.hour"]=html.var("iduConfiguration.rtcConfigurationTable.hour")
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if html.var("iduConfiguration.rtcConfigurationTable.min")!=None:
            if Validation.is_required(html.var("iduConfiguration.rtcConfigurationTable.min")):
                if Validation.is_number(html.var("iduConfiguration.rtcConfigurationTable.min")):
                    dic_result["iduConfiguration.rtcConfigurationTable.min"]=html.var("iduConfiguration.rtcConfigurationTable.min")
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if html.var("iduConfiguration.rtcConfigurationTable.sec")!=None:
            if Validation.is_required(html.var("iduConfiguration.rtcConfigurationTable.sec")):
                if Validation.is_number(html.var("iduConfiguration.rtcConfigurationTable.sec")):
                    dic_result["iduConfiguration.rtcConfigurationTable.sec"]=html.var("iduConfiguration.rtcConfigurationTable.sec")
                else:
                    flag = 1
                    dic_result["success"] = 1
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if Validation.is_required(html.var("iduConfiguration.poeConfigurationTable.poeAdminStatus")):
            if html.var("iduConfiguration.poeConfigurationTable.poeAdminStatus")!=None:
                dic_result["iduConfiguration.poeConfigurationTable.poeAdminStatus"]=html.var("iduConfiguration.poeConfigurationTable.poeAdminStatus")
            else:
                flag = 1
                dic_result["success"] = 1
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("switch.switchPortconfigTable.swadminState")!=None:
            if Validation.is_required(html.var("switch.switchPortconfigTable.swadminState")):
                dic_result["switch.switchPortconfigTable.swadminState"]=html.var("switch.switchPortconfigTable.swadminState")
            else:
                flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Admin State is required"        
        ...
        if html.var("switch.switchPortconfigTable.swlinkMode")!=None:
            if Validation.is_required(html.var("switch.switchPortconfigTable.swlinkMode")):            
                dic_result["switch.switchPortconfigTable.swlinkMode"]=html.var("switch.switchPortconfigTable.swlinkMode")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("switch.switchPortconfigTable.portvid")!=None:
            if Validation.is_required(html.var("switch.switchPortconfigTable.portvid")):
                if Validation.is_number(html.var("switch.switchPortconfigTable.portvid")):
                    dic_result["switch.switchPortconfigTable.portvid"]=html.var("switch.switchPortconfigTable.portvid")
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if html.var("switch.switchPortconfigTable.macauthState")!=None:
            if Validation.is_required(html.var("switch.switchPortconfigTable.macauthState")):            
                dic_result["switch.switchPortconfigTable.macauthState"]=html.var("switch.switchPortconfigTable.macauthState")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("switch.switchPortconfigTable.mirroringdirection")!=None:
            if Validation.is_required(html.var("switch.switchPortconfigTable.mirroringdirection")):            
                dic_result["switch.switchPortconfigTable.mirroringdirection"]=html.var("switch.switchPortconfigTable.mirroringdirection")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("switch.switchPortconfigTable.macflowcontrol")!=None:
            if Validation.is_required(html.var("switch.switchPortconfigTable.macflowcontrol")):            
                dic_result["switch.switchPortconfigTable.macflowcontrol"]=html.var("switch.switchPortconfigTable.macflowcontrol")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("switch.switchPortconfigTable.portdotqmode")!=None:
            if Validation.is_required(html.var("switch.switchPortconfigTable.portdotqmode")):            
                dic_result["switch.switchPortconfigTable.portdotqmode"]=html.var("switch.switchPortconfigTable.portdotqmode")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if flag == 1:
            ...
            #html.write(str(str(host_id)+str(device_type_id)+str(dic_result)+str(id)+str(index)))
            result = obj_bll_cancel_set_valid.common_validation(host_id,device_type_id,dic_result,id,index)
            html.req.content_type = 'application/json'
            html.req.write(str(JSONEncoder().encode(result)))
            #html.req.write(str(JSONEncoder().encode(dic_result)))
        ...
        if html.var("switch.portBwTable.ingressbwvalue")!=None:
            dic_result["switch.portBwTable.ingressbwvalue"] = html.var("switch.portBwTable.ingressbwvalue")
        if html.var("switch.portBwTable.ingressbwvalue")!=None:
            dic_result["switch.portBwTable.ingressbwvalue"] = html.var("switch.portBwTable.ingressbwvalue")
...
def update_swt_port_form(h):
    try:
        ...
        host_id = html.var("host_id")
        selected_device = html.var("selected_device")
        html.write(str(IduForms.table_port_configuration(host_id,selected_device)))
    except Exception as e:
        html.write(str(e))
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("switch.portBwTable.egressbwvalue")!=None:
            if Validation.is_required(html.var("switch.portBwTable.egressbwvalue")):            
                dic_result["switch.portBwTable.egressbwvalue"]=html.var("switch.portBwTable.egressbwvalue")
            else:
                flag = 1
                dic_result["success"] = 1
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("switch.mirroringportTable.mirroringport")!=None:
            if Validation.is_required(html.var("switch.mirroringportTable.mirroringport")):
                dic_result["switch.mirroringportTable.mirroringport"]=html.var("switch.mirroringportTable.mirroringport")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if flag == 1:
            ...
            result = obj_bll_cancel_set_valid.common_validation(host_id,device_type_id,dic_result,None,0,special_case)
            html.req.content_type = 'application/json'
            html.req.write(str(JSONEncoder().encode(result)))
        ...
        if html.var("iduConfiguration.omcConfigurationTable.omcIpAddress")!=None:
            if Validation.is_required(html.var("iduConfiguration.omcConfigurationTable.omcIpAddress")):
                if Validation.is_valid_ip(html.var("iduConfiguration.omcConfigurationTable.omcIpAddress")):
                    dic_result["iduConfiguration.omcConfigurationTable.omcIpAddress"]=html.var("iduConfiguration.omcConfigurationTable.omcIpAddress")
                else:
                    flag = 1
                    dic_result["success"] = 1
...
def update_bw_form(h):
    try:
        ...
        host_id = html.var("host_id")
        selected_device = html.var("selected_device")
        html.write(str(IduForms.port_bandwidth_show_table(host_id,selected_device)))
    except Exception as e:
        html.write(str(e))
...
def update_qinq_form(h):
    try:
        ...
        host_id = html.var("host_id")
        selected_device = html.var("selected_device")
        html.write(str(IduForms.port_QinQ_show_table(host_id,selected_device)))
    except Exception as e:
        html.write(str(e))
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("switch.portqinqTable.portqinqstate")!=None:
            if Validation.is_required(html.var("switch.portqinqTable.portqinqstate")):
                dic_result["switch.portqinqTable.portqinqstate"]=html.var("switch.portqinqTable.portqinqstate")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("switch.portqinqTable.providertag")!=None:
            if Validation.is_required(html.var("switch.portqinqTable.providertag")):            
                if Validation.is_number(html.var("switch.portqinqTable.providertag")):
                    ...
                    dic_result["switch.portqinqTable.providertag"] = actual_number    
        ...
        if html.var("switch.portqinqTable.portqinqstate")!=None:
            dic_result["switch.portqinqTable.portqinqstate"] = html.var("switch.portqinqTable.portqinqstate")
        if html.var("switch.portqinqTable.providertag")!=None:
            dic_result["switch.portqinqTable.providertag"] = html.var("switch.portqinqTable.providertag")
...
def port_link_form(h):
    ...
    #index = html.var("index")
    if int(addEdit) == 1:
        port_link_list = idu_get_database_obj.common_get_data_by_id(port_link_id,host_id,id_name,class_name)
    if isinstance(port_link_list,list):
        html.write(str(IduForms.port_link_form(port_link_list,host_id,selected_device,port_link_id,port_num,link_num,addEdit)))
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
            ...
            if Validation.is_required(html.var("bundle_id")):
                if Validation.is_number(html.var("bundle_id")):
                    if int(html.var("bundle_id"))>=0 and int(html.var("bundle_id"))<=63:
                        ...
                        flag = 1
                        dic_result["success"] = 1
                        dic_result["result"] = "Link Number must be in between 0 and 63"
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if html.var("iduConfiguration.linkConfigurationTable.dstIPAddr")!=None:
            if Validation.is_required(html.var("iduConfiguration.linkConfigurationTable.dstIPAddr")):
                if Validation.is_valid_ip(html.var("iduConfiguration.linkConfigurationTable.dstIPAddr")):
                    dic_result["iduConfiguration.linkConfigurationTable.dstIPAddr"] = html.var("iduConfiguration.linkConfigurationTable.dstIPAddr")
                else:
                    flag = 1
                    dic_result["success"] = 1
                ...
                if Validation.is_number(html.var("iduConfiguration.linkConfigurationTable.srcBundleID")):
                    if int(html.var("iduConfiguration.linkConfigurationTable.srcBundleID"))>=0 and int(html.var("iduConfiguration.linkConfigurationTable.srcBundleID"))<=63:
                        if obj_link.src_bundle_chk(host_id,int(html.var("iduConfiguration.linkConfigurationTable.srcBundleID")),html.var("bundle_id") if int(addEdit)==0 else id,port_num if int(addEdit)==0 else index)==0:
                            dic_result["iduConfiguration.linkConfigurationTable.srcBundleID"] = html.var("iduConfiguration.linkConfigurationTable.srcBundleID")
                        else:
                            flag = 1
                            dic_result["success"] = 1
                        ...
                        flag = 1
                        dic_result["success"] = 1
                        dic_result["result"] = "Source Link Id must be in between 0 and 63"
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Please enter number in Source Link Id"
            else:
                flag = 1
                dic_result["success"] = 1
                ...
                if Validation.is_number(html.var("iduConfiguration.linkConfigurationTable.dstBundleID")):
                    if int(html.var("iduConfiguration.linkConfigurationTable.dstBundleID"))>=0 and int(html.var("iduConfiguration.linkConfigurationTable.dstBundleID"))<=63:
                        if obj_link.destination_bundle_chk(host_id,int(html.var("iduConfiguration.linkConfigurationTable.dstBundleID")),html.var("bundle_id") if int(addEdit)==0 else id,port_num if int(addEdit)==0 else index)==0:
                            dic_result["iduConfiguration.linkConfigurationTable.dstBundleID"] = html.var("iduConfiguration.linkConfigurationTable.dstBundleID")
                        else:
                            flag = 1
                            dic_result["success"] = 1
                        ...
                        flag = 1
                        dic_result["success"] = 1
                        dic_result["result"] = "Destination Link Id must be in between 0 and 63"
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Please enter number in Destination Link Id"
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("iduConfiguration_linkConfigurationTable_tsaAssign")!=None:
            if Validation.is_required(html.var("iduConfiguration_linkConfigurationTable_tsaAssign")):
                timeslot = ""
                timeslot_str = html.var("iduConfiguration_linkConfigurationTable_tsaAssign") 
                ...
                for i in range(0,32):
                    if str(i) in timeslot_list:
                        ...
                        timeslot+="0"           
                dic_result["iduConfiguration_linkConfigurationTable_tsaAssign"]=timeslot
            else:
                flag = 1
                dic_result["success"] = 1
            ...
            if Validation.is_required(html.var("iduConfiguration.linkConfigurationTable.bundleSize")):
                if Validation.is_number(html.var("iduConfiguration.linkConfigurationTable.bundleSize")):
                    if int(html.var("iduConfiguration.linkConfigurationTable.bundleSize"))>=1 and int(html.var("iduConfiguration.linkConfigurationTable.bundleSize"))<=99:
                        dic_result["iduConfiguration.linkConfigurationTable.bundleSize"] = html.var("iduConfiguration.linkConfigurationTable.bundleSize")
                    else:
                        flag = 1
                        dic_result["success"] = 1
                        dic_result["result"] = "Payload must be in between 1 and 99"
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Please enter number in Payload size"
            else:
                flag = 1
                dic_result["success"] = 1
            ...
            if Validation.is_required(html.var("iduConfiguration.linkConfigurationTable.bufferSize")):
                if Validation.is_number(html.var("iduConfiguration.linkConfigurationTable.bufferSize")):
                    if int(html.var("iduConfiguration.linkConfigurationTable.bufferSize"))>=1000 and int(html.var("iduConfiguration.linkConfigurationTable.bufferSize"))<=25000:
                        dic_result["iduConfiguration.linkConfigurationTable.bufferSize"] = html.var("iduConfiguration.linkConfigurationTable.bufferSize")
                    else:
                        flag = 1
                        dic_result["success"] = 1
                        dic_result["result"] = "Jitter buffer must be in between 1000 and 25000"
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Please enter number in Jitter buffer"
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("iduConfiguration_linkConfigurationTable_clockRecovery")!=None:
            if Validation.is_required(html.var("iduConfiguration_linkConfigurationTable_clockRecovery")):
                dic_result["iduConfiguration_linkConfigurationTable_clockRecovery"] = html.var("iduConfiguration_linkConfigurationTable_clockRecovery")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if flag == 1:
            ...
            result = obj_bll_cancel_set_valid.set_link_port(host_id,device_type_id,dic_result,bundle_id if int(addEdit)==0 else id,port_num if int(addEdit)==0 else index,int(addEdit))
            #html.write(str(result))
            #time.sleep(30)
            html.req.content_type = 'application/json'
            html.req.write(str(JSONEncoder().encode(result)))
...
def get_selected_timeslot(h):
    ...
    if len(result)>0:
        ...
        for row in range(0,32):
            if row==0:
                select_list_html+="<option value=\"%s\" disabled=\"disabled\"> %s </option>"%(row,row)
            else:
                if len(pos)>0:
                    if row in pos:
                        select_list_html+="<option value=\"%s\" disabled=\"disabled\"> %s </option>"%(row,row)                
                    else:
                        select_list_html+="<option value=\"%s\"> %s </option>"%(row,row)                                    
                else:
                    select_list_html+="<option value=\"%s\"> %s </option>"%(row,row)                                                        
    else:
        for row in range(0,32):
...
class IduStatus(object):
    ...
    def idu_hw_sw_temperature_status(self,host_id,device_type):
        ...
        try:
            ...
            sw_status_list = obj_idu_get_data.common_get_data_by_host('IduSwStatusTable',host_id)
            hw_temperature_list = obj_idu_get_data.common_get_data_by_host('IduIduInfoTable',host_id)
            temp_dic = {}

            temp_dic.update({'active_version': sw_status_list[0].activeVersion if len(sw_status_list)>0 else "-",\
...
def idu_listing_status(h):
    ...
    if int(result_dic['success'])==0:
        ...
        html_str+="<td>%s</td><td>%s</td><td>%s</td>"%("-" if result_dic['result']['active_version']==None else result_dic['result']['active_version'],\
                                                       "-" if result_dic['result']['hw_serial_number']==None else result_dic['result']['hw_serial_number'],\
...
def e1_admin_state_show(h):
    ...
    device_type = html.var("device_type_id")
    obj_idu_get_data = IduGetData()
    e1_port_list = obj_idu_get_data.common_get_data('IduE1PortConfigurationTable',host_id)
    html_str = "<table class=\"yo-table\" id=\"table_status\" style=\"border:2px;border-style:solid;border-color:#6b6969;z-index:1px\">"
    ...
    if len(e1_port_list)>0:
        for i in range(0,len(e1_port_list)):
            ...
            html_str+="<td><img src=\"%s\" class=\"n-reconcile\" title=\"%s Port Admin State\" id=\"admin_state_%s\" name=\"admin_state_%s\" style=\"width:10px;height:10px;\" state=\"%s\" onClick=\"idu_admin_state_change(event,this,'%s','%s','%s','iduConfiguration.e1PortConfigurationTable.adminState',0);\"/></td></tr>"\
                %("images/temp/green_dot.png" if int(e1_port_list[i].adminState)==1 else "images/temp/red_dot.png","E"+str(e1_port_list[i].portNumber),\
...
def link_admin_state_show(h):
    ...
    device_type = html.var("device_type_id")
    obj_idu_get_data = IduGetData()
    link_port_list = obj_idu_get_data.common_get_data('IduLinkConfigurationTable',host_id)
    html_str = "<table class=\"yo-table\" id=\"table_status\" style=\"border:2px;border-style:solid;border-color:#6b6969;z-index:1px\">"
    html_str+="<!--<th></th>-->"
    html_str+="<th colspan=2>Link Admin State</th>"
    if len(link_port_list)>0:
        for i in range(0,len(link_port_list)):
            html_str+="<tr><!--<td>%s</td>-->"%(link_port_list[i].portNumber)
            html_str+="<td colspan=2><ul class=\"button_group\" style=\"width:40px;\"><li><a class=\"%s n-reconcile\" title=\"Link %s\" id=\"admin_state_%s\" name=\"admin_state_%s\" state=\"%s\" bundle_num=\"%s\" onClick=\"idu_admin_state_change(event,this,'%s','%s','%s','iduConfiguration.linkConfigurationTable.adminStatus',1);\">L %s</a></li></td></tr>"\
                %("green" if int(link_port_list[i].adminStatus)==1 else "red",\
                  str(link_port_list[i].portNumber)+" Unlocked" if int(link_port_list[i].adminStatus)==1 else str(link_port_list[i].portNumber)+" Locked",
...
def main_admin_state_show(h):
    ...
    device_type = html.var("device_type_id")
    obj_idu_get_data = IduGetData()
    main_admin_list = obj_idu_get_data.common_get_data('IduIduAdminStateTable',host_id)
    html_str = "<table class=\"yo-table\" id=\"table_status\" style=\"border:2px;border-style:solid;border-color:#6b6969;z-index:1px\">"    
    ...
    if len(main_admin_list)>0:
        for i in range(0,len(main_admin_list)):
            html_str+="<td><img src=\"%s\" class=\"n-reconcile\" title=\"System Admin State\" id=\"main_admin\" name=\"main_admin\" style=\"width:10px;height:10px;\" state=\"%s\" onClick=\"main_admin_state_change(event,this,'%s','iduinfo.iduAdminStateTable.adminstate');\"/></td></tr>"\
                %("images/temp/green_dot.png" if int(main_admin_list[i].adminstate)==1 else "images/temp/red_dot.png",\
...
def e1_admin_parameters(h):
    ...
    result_dic = obj.e1_port_admin_change(host_id,int(primary_id),int(port_num),admin_state_name,int(state))
    html.req.content_type = 'application/json'
...
def link_admin_parameters(h):
    ...
    state = html.var("state")
    obj = IduAdminStateChange()
    result_dic = obj.link_port_admin_change(host_id,int(primary_id),int(port_num),int(bundle_num),admin_state_name,int(state))
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(result_dic)))
    ...
    if len(e1_port_list)>0:
        for i in range(0,len(e1_port_list)):
            if i==len(e1_port_list)-1:
                ...
                primary_ids += str(e1_port_list[i].idu_e1PortConfigurationTable_id)
            else:
                port_str+=str(e1_port_list[i].portNumber)+","
                primary_ids += str(e1_port_list[i].idu_e1PortConfigurationTable_id)+","
    obj = IduAdminStateChange()
    result_dic = obj.locked_unlocked_all(host_id,port_str,primary_ids,admin_state_name,state)
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(result_dic)))
    ...
    if len(link_port_list)>0:
        for i in range(0,len(link_port_list)):
            if i==len(link_port_list)-1:
                ...
                primary_ids += str(link_port_list[i].idu_linkConfigurationTable_id)
                ...
                bundle_num += str(link_port_list[i].bundleNumber)+","
    obj = IduAdminStateChange()
    result_dic = obj.link_locked_unlocked_all(host_id,port_str,primary_ids,bundle_num,admin_state_name,state)    
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(result_dic)))
