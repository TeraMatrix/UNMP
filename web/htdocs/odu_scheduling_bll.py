...
# AVERAGE DATA FOR GIVEN DATE PERIOD 
    def create_scheduler(self,odu_list,event, startDate, endDate, startTime, endTime, repeat, repeatType, daysun, daymon, daytue, daywed, daythu, dayfri, daysat, monthjan, monthfeb, monthmar, monthapr, monthmay, monthjun, monthjul, monthaug, monthsep, monthoct, monthnov, monthdec, dates,firmware_file_name=""):
        try:
            ...
            for oduId in odu_list:
                if oduId.strip() != "":
                    if event=="Firmware":
                        sql = "INSERT INTO odu_host_schedule(schedule_id,host_id,is_success,firmware_file_name) VALUES(%s,%s,1,'%s')" % (newId,oduId,firmware_file_name)
    ...
    def view_odu_list(self,scheduleId):
        try:
            conn = MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor = conn.cursor()
            sql = "select device_type.device_name,hosts.ip_address,hosts.host_name FROM hosts \
    ...
    def update_odu_scheduler_delete(self,scheduleId):
        try:
            conn = MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor = conn.cursor()
            sql = "DELETE FROM odu_host_schedule WHERE schedule_id = %s" % (scheduleId)
            cursor.execute(sql)
            conn.commit()
            return 0
        except Exception,e:
    ...
    def update_odu_scheduler_insert(self,scheduleId,oduId,event="Up",firmware_file_name=""):
        try:
            conn = MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor = conn.cursor()
            #sql = "INSERT INTO odu_host_schedule(schedule_id,host_id,is_success) VALUES(%s,%s,1)" % (scheduleId,oduId) 
            if event=="Firmware":
                sql = "INSERT INTO odu_host_schedule(schedule_id,host_id,is_success,firmware_file_name) VALUES(%s,%s,1,'%s')" % (scheduleId,oduId,firmware_file_name)
                cursor.execute(sql)#### continue from here
            else:
                sql = "INSERT INTO odu_host_schedule(schedule_id,host_id,is_success) VALUES(%s,%s,1)" % (scheduleId,oduId)
                cursor.execute(sql)
