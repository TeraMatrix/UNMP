...
class HostgroupBll(object):
    def get_data_table_sqlalchemy(self,a_columns,table_columns,table_classes,table_join,other_conditions,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars,join_conditions=[]):
        i_total=0
        i_filtered_total=0
		...
		for i in range(0,int(req_vars.get("iSortingCols",0))):		# creating orderby in query
	                ...
	                b_sortable_ = req_vars.get("bSortable_%s" % i_sort_col_i,None)
	                if b_sortable_ == "true":
...
class HostBll(object):
    ...
    def grid_view_active_host(self,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars):
        ...
        try:
            #session = Session()                 # creating new session object
            a_columns = ["host_id", "is_localhost", "host_name", "host_alias","ip_address","device_name","mac_address","host_state_id"]
            table_columns = [["host_id", "is_localhost", "host_name", "host_alias","ip_address"],
                    	     ["device_name"],
                    	     ["mac_address","host_state_id"],
                    	     [],
                    	     [],
                    	     []
                    	    ]
            table_classes = [Hosts,DeviceType,Hosts,UsersGroups,HostgroupsGroups,HostsHostgroups]
	    ...
	    req_vars["iSortingCols"]=int(req_vars["iSortingCols"])+2
    ...
    def grid_view_deleted_host(self,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars):
	    ...
	    a_columns = ["host_id", "is_localhost", "host_name", "host_alias","ip_address","device_name","mac_address","updated_by","timestamp","host_state_id"]
            table_columns = [["host_id", "is_localhost", "host_name", "host_alias","ip_address","mac_address","host_state_id","timestamp","updated_by"],
                    	     ["device_name"],
                    	     ["mac_address","updated_by","timestamp","host_state_id"],
                    	     [],
                    	     [],
                    	     [],
                    	     []
    ...
    def get_host_by_id(self,host_id):
        ...
        try:
            ...
            hosts = session.query(Hosts.host_id,Hosts.host_name,Hosts.host_alias,Hosts.ip_address,Hosts.mac_address,Hosts.device_type_id,Hosts.config_profile_id,\
                                  ...
                                  Hosts.http_username,Hosts.http_password,Hosts.http_port,Hosts.snmp_read_community,Hosts.snmp_write_community,\
...
#		            return host.host_id
    ...
    def validate(self,host_name,host_alias,ip_address,mac_address,device_type_id,netmask,gateway,primary_dns,secondary_dns,host_state_id,priority_id,host_vendor_id,host_os_id,http_username,http_password,http_port,snmp_read_community,snmp_write_community,snmp_port,snmp_trap_port,comment,longitude,latitude,serial_number,hardware_version):
        ...
        if Validation.is_alpha_numeric(host_name) and Validation.no_space(host_name) and Validation.is_required(host_alias) and Validation.is_alpha_numeric(host_alias) and \
         ...
         Validation.is_required(priority_id) and Validation.is_alpha_numeric(http_username) and Validation.is_required(host_vendor_id) and Validation.is_required(host_os_id) and \
         Validation.is_number(http_port) and Validation.is_alpha_numeric(snmp_read_community) and Validation.is_alpha_numeric(snmp_write_community) and \
         Validation.is_number(snmp_port) and Validation.is_number(snmp_trap_port) and Validation.is_alpha_numeric(comment) and \
         Validation.is_number(longitude) and Validation.is_number(latitude) and Validation.is_alpha_numeric(serial_number) and Validation.is_alpha_numeric(hardware_version):
            return True
        else:
            return False
        ...
        if len(device_dict)>0:
			...
			change_admin_state = pysnmp_set(admin_dic,host_details[0].ip_address,host_details[0].snmp_port,host_details[0].snmp_write_community) 
                    ...
                    if device_dict["device_type_id"]==UNMPDeviceType.ap25:
                        ...
                        change_net_config = ap_seter(dic_ap,host_details[0].ip_address,host_details[0].snmp_port,host_details[0].snmp_write_community)
                    for i in range(0,3):
...
class ServiceBll(object):
    ...
    def grid_view(self,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars):
        ...
        try:
            #session = Session()                 # creating new session object
            a_columns = ["host_id","is_localhost","host_name","host_alias","ip_address","mac_address","host_state_id","device_name","hostgroup_name"]
    ...
    def set_service_time(self,hosts_list,service_name,service_time):
        Session = sessionmaker(bind=engine)     # making session of our current database
        total=[]
        try:
            session = Session() 
...
class NagioConfigurationBll(object):
    def write_nagios_config(self,nms_instance):
        ...
        try:
            ...
            hosts = session.query(Hosts.host_name, Hosts.host_alias,Hosts.ip_address,Hosts.priority_id,(Parents.host_name).label("parent")).outerjoin(Parents,Hosts.parent_name == Parents.host_id).filter(Hosts.is_deleted == 0).all()          # execute query and fetch data
            ...
            for host in hosts:
                h_config += ng_cng.host_config(use_template,host.host_name,host.host_alias,host.ip_address,None,check_command,host.parent)
            ng_cng.write_host_config_file(nms_instance,h_config)
            ## Mahipal code
            priority_dict={}
            priority_dict["high"]="5"
            priority_dict["normal"]="10"
            priority_dict["low"]="15"
