...
class HostgroupBll(object):
    def get_data_table_sqlalchemy(self,a_columns,table_columns,table_classes,table_join,other_conditions,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars,join_conditions=[],group_by=False,diff_count=3):
        ...
        try:
            ...
            for class_i in range(0,len(table_classes)):
                if class_i == 0:
                    query = session.query(*tuple(query_column))		# creating query to pass the table fields
                elif class_i > 0:
            ...
            if len(and_condition) == 0:
                ...
                if len(or_condition) == 0:
                    ...
                    query = query.filter(and_(or_(or_condition[0]),*tuple(and_condition)))
                else:
                    query = query.filter(and_(or_(*tuple(or_condition)),*tuple(and_condition)))
            ...
            for i in range(0,int(req_vars.get("iSortingCols",0))):		# creating orderby in query
                ...
                b_sortable_ = req_vars.get("bSortable_%s" % i_sort_col_i,"true")
                ...
                if b_sortable_ == "true":
                    ...
                    for class_i in range(0,len(table_classes)):
                        ...
                        if len(table_columns) > class_i:
                            for column_i in range(0,len(table_columns[class_i])):
                                temp_li.append([a_columns[i_sort_col_i],col_number,i_sort_col_i,table_columns[class_i][column_i]])
...
#	                        query = query.order_by(desc(order_dict["table_name"]))		                                    
    ...
    def grid_view(self,hostgroup_id_list,flag=1):
        ...
        try:
            ...
            if flag:
                hostgroup = session.query(Hostgroups.hostgroup_id, Hostgroups.hostgroup_name, Hostgroups.hostgroup_alias, Hostgroups.is_default, Groups.group_name).outerjoin(HostgroupsGroups,Hostgroups.hostgroup_id == HostgroupsGroups.hostgroup_id).outerjoin(Groups,HostgroupsGroups.group_id == Groups.group_id).filter(and_(Hostgroups.is_deleted == 0,Hostgroups.hostgroup_id.in_(hostgroup_id_list))).order_by(Hostgroups.hostgroup_name).all()          # execute query and fetch data
            else:
                hostgroup = session.query(Hostgroups.hostgroup_id, Hostgroups.hostgroup_name, Hostgroups.hostgroup_alias, Hostgroups.is_default, Groups.group_name).outerjoin(HostgroupsGroups,Hostgroups.hostgroup_id == HostgroupsGroups.hostgroup_id).outerjoin(Groups,HostgroupsGroups.group_id == Groups.group_id).filter(Hostgroups.is_deleted == 0).order_by(Hostgroups.hostgroup_name).all()          # execute query and fetch data
            return hostgroup
        except Exception,e:
            return e
        finally:
    ...
    def grid_view_number_of_hosts(self,hostgroup_id_list,flag=1):
        ...
        try:
            session = Session()                 # creating new session object
            if flag:
                hostgroup = session.query(Hostgroups.hostgroup_id, Hostgroups.hostgroup_name, Hostgroups.hostgroup_alias, func.count(Hosts.device_type_id), DeviceType.device_name).outerjoin(HostsHostgroups,Hostgroups.hostgroup_id == HostsHostgroups.hostgroup_id).outerjoin(Hosts,HostsHostgroups.host_id == Hosts.host_id).outerjoin(DeviceType,Hosts.device_type_id == DeviceType.device_type_id).filter(and_(Hostgroups.is_deleted == 0,Hosts.is_deleted == 0,Hosts.host_state_id =='e',Hostgroups.hostgroup_id.in_(hostgroup_id_list))).group_by(Hosts.device_type_id, Hostgroups.hostgroup_id).order_by(Hostgroups.hostgroup_name).all()          # execute query and fetch data
            else:
    ...
    def get_hostgroup_by_id(self,hostgroup_id):
        ...
        try:
            session = Session()                 # creating new session object
            hostgroups = session.query(Hostgroups.hostgroup_id, Hostgroups.hostgroup_name, Hostgroups.hostgroup_alias, Hostgroups.is_default, Hostgroups.timestamp, Hostgroups.created_by, Hostgroups.creation_time, Hostgroups.updated_by).filter(Hostgroups.hostgroup_id == hostgroup_id).all()          # execute query and fetch data
            if len(hostgroups) == 1:
                return hostgroups[0]
            else:
                err = ErrorMessages()
            ...
            if lic.check_license_for_hostgroup() == True:
                if self.validate(hostgroup_name,hostgroup_alias):
                    if self.is_duplicate_hostgroup_for_add(hostgroup_name,hostgroup_alias):
                        hostgroup = Hostgroups(hostgroup_name,hostgroup_alias,timestamp,created_by,creation_time,is_deleted,updated_by,is_default)
                        ...
                        if len(result_tuple) > 0:
                            if len(result_tuple) == 2:
                                insert_query = "insert into hostgroups_groups (`hostgroup_id`, `group_id`) values ('%s','%s'),('%s','%s')"%(str(hg_id),result_tuple[0][0],str(hg_id),result_tuple[1][0])
                            else:
                                insert_query = "insert into hostgroups_groups (`hostgroup_id`, `group_id`) values ('%s','%s')"%(str(hg_id),result_tuple[0][0])
                        ...
                        if(flag_nagios_call==0):
                            ...
                            n_bll=NagiosBll()
                            wnc=n_bll.add_new_hostgroup(hostgroup_name,hostgroup_alias)
        ...
        try:
            ...
            if self.validate(hostgroup_name,hostgroup_alias):
                if self.is_duplicate_hostgroup_for_edit(hostgroup_id,hostgroup_name,hostgroup_alias):
                    hostgroups = session.query(Hostgroups).filter(Hostgroups.hostgroup_id == hostgroup_id).all()
                    if len(hostgroups) == 1:
                        ...
                        if(flag_nagios_call==0):
                            ...
                            ncbll = NagiosBll()
                            wnc=n_bll.edit_old_hostgroup(old_hostgroup_name,hostgroup_name,hostgroup_alias)
                            condition_nagios = wnc["success"]==0
                        #ncbll = NagioConfigurationBll()
                        #wnc = ncbll.write_nagios_config(nms_instance)
                        if condition_nagios:
                            if flag_nagios_call:
                                SystemSetting.reload_nagios_config()
                            el = EventLog()
                            el.log_event("Hostgroup %s updated successfully " % (hostgroup_name),updated_by)
                            return hostgroup.hostgroup_id
                        else:
                            err = ErrorMessages()
                            err.error_msg = err.nagios_config_error
    ...
    def delete(self,hostgroup_ids,nms_instance,updated_by):
        ...
        try:
            ...
            default_hostgroup_ids = session.query(Hostgroups.hostgroup_id).filter(Hostgroups.is_default == 1).all()          # execute query and fetch data
            for hg_id in default_hostgroup_ids:
                default_hostgroup_id = hg_id[0]
            hosts = session.query(HostsHostgroups.host_id).filter(HostsHostgroups.hostgroup_id.in_(hostgroup_ids)).all()

            hostgroups = session.query(Hostgroups).filter(Hostgroups.hostgroup_id.in_(hostgroup_ids)).all()          # execute query and fetch data
            hostgroup_count = len(hostgroups)
            if hostgroup_count > 0:
                el = EventLog()
                wnc={}
                for hg in hostgroups:
                    el.log_event("Hostgroup %s Deleted." % (hg.hostgroup_alias),updated_by)
                ...
                if default_hostgroup_id != None:
                    for host in hosts:
                        hostgroup_obj = HostsHostgroups(host[0],default_hostgroup_id)
...
#                wnc = ncbll.write_nagios_config(nms_instance)
    ...
    def is_duplicate_hostgroup_for_add(self,hostgroup_name,hostgroup_alias):
        ...
        try:
            session = Session()                 # creating new session object
            hostgroup_count = session.query(Hostgroups).filter(or_(Hostgroups.hostgroup_name == hostgroup_name, Hostgroups.hostgroup_alias == hostgroup_alias)).count()
            if hostgroup_count == 0:
                return True
            else:
                return False
    ...
    def is_duplicate_hostgroup_for_edit(self,hostgroup_id,hostgroup_name,hostgroup_alias):
        ...
        try:
            session = Session()                 # creating new session object
            hostgroup_count = session.query(Hostgroups).filter(and_(Hostgroups.hostgroup_id != hostgroup_id,or_(Hostgroups.hostgroup_name == hostgroup_name, Hostgroups.hostgroup_alias == hostgroup_alias))).count()
            if hostgroup_count == 0:
                return True
            else:
                return False
...
class DiscoveryBll(object):
    def delete_discovered_host(self,discovery_type,host_id,updated_by):
        ...
        if len(discovery_type) == len(host_id):
            ...
            try:
                session = Session()                 # creating new session object
                tcp_host = session.query(TcpDiscovery).filter(TcpDiscovery.ne_id.in_(tcp_ne_id)).all()          # execute query and fetch data
                discovered_host = session.query(DiscoveredHosts).filter(DiscoveredHosts.discovered_host_id.in_(discovered_host_id)).all()          # execute query and fetch data
                tcp_host_count = len(tcp_host) + len (discovered_host)
                if tcp_host_count > 0:
                    el = EventLog()
                    for th in tcp_host:
                        el.log_event("TCP Discovered Host '%s' Deleted." % (th.ip_address),updated_by)
                        session.delete(th)
                    for dh in discovered_host:
                        el.log_event("Discovered Host '%s' Deleted." % (dh.ip_address),updated_by)
                        session.delete(dh)
                    session.commit()
                else:
                    err = ErrorMessages()
    ...
    def ping_discovery(self,base_ip,base_ip_start,base_ip_end,timeout,service_mngt,timestamp,created_by,creation_time,updated_by,is_deleted):
        ...
        try:
            ...
            if self.ping_validate(base_ip,base_ip_start,base_ip_end,timeout,service_mngt):
                discovery = Discovery(DiscoveryName.ping,(base_ip + "." + base_ip_start),(base_ip + "." + base_ip_end),timeout,None,None,None,None,None,service_mngt,0,timestamp,created_by,creation_time,updated_by,is_deleted)
    ...
    def snmp_discovery(self,base_ip,base_ip_start,base_ip_end,timeout,snmp_community,snmp_port,snmp_version,service_mngt,timestamp,created_by,creation_time,updated_by,is_deleted):
        ...
        try:
            ...
            if self.snmp_validate(base_ip,base_ip_start,base_ip_end,timeout,snmp_community,snmp_port,snmp_version,service_mngt):
                ...
                discovery = Discovery(DiscoveryName.snmp,(base_ip + "." + base_ip_start),(base_ip + "." + base_ip_end),timeout,snmp_community,snmp_port,snmp_version,None,None,service_mngt,0,timestamp,created_by,creation_time,updated_by,is_deleted)
                session.add(discovery)
                session.commit()
                el = EventLog()
    ...
    def upnp_discovery(self,timeout,service_mngt,timestamp,created_by,creation_time,updated_by,is_deleted):
        ...
        try:
            ...
            if self.upnp_validate(timeout,service_mngt):
                #discovery_type_id,ip_start_range,ip_end_range,timeout,snmp_community,snmp_port,snmp_version,sdm_device_list,scheduling_id,service_management,done_percent,timestamp,created_by,creation_time,updated_by,is_deleted
                discovery = Discovery(DiscoveryName.upnp,None,None,timeout,None,None,None,None,None,service_mngt,0,timestamp,created_by,creation_time,updated_by,is_deleted)
                session.add(discovery)
                session.commit()
                el = EventLog()
    ...
    def add_discovered_host(self,discovery_id,host_alias,ip_address,device_type_id,mac_address):
        ...
        try:
            session = Session()                 # creating new session object
            discovered_host = DiscoveredHosts(discovery_id,host_alias,ip_address,device_type_id,mac_address)
    ...
    def run_ping_discovery(self,discovery_id):
        ...
        try:
            session = Session()                 # creating new session object
            discovery = session.query(Discovery.ip_start_range, Discovery.ip_end_range, Discovery.timeout).filter(Discovery.discovery_id == discovery_id).all()          # execute query and fetch data
            if len(discovery) == 1:
                dis = discovery[0]
                ...
                timeout = dis.timeout
                pd = PingDiscovery()
                pd.ping_function(discovery_id,start_range,end_range,timeout,DiscoveryBll)
                discovery = session.query(Discovery).filter(Discovery.discovery_id == discovery_id).all()          # execute query and fetch data
                if len(discovery) > 0:
    ...
    def run_snmp_discovery(self,discovery_id):
        ...
        try:
            session = Session()                 # creating new session object
            discovery = session.query(Discovery.ip_start_range, Discovery.ip_end_range, Discovery.timeout, Discovery.snmp_community, Discovery.snmp_port, Discovery.snmp_version).filter(Discovery.discovery_id == discovery_id).all()          # execute query and fetch data
            if len(discovery) == 1:
                dis = discovery[0]
                start_range = dis.ip_start_range
                end_range = dis.ip_end_range
                ...
                pd = SnmpDiscovery()
                pd.snmp_function(discovery_id,start_range,end_range,timeout,community,port,version,DiscoveryBll)
                discovery = session.query(Discovery).filter(Discovery.discovery_id == discovery_id).all()          # execute query and fetch data
                if len(discovery) > 0:
                    discovery[0].is_deleted = 1
                    discovery[0].done_percent = 100
    ...
    def run_upnp_discovery(self,discovery_id):
        ...
        try:
            session = Session()                 # creating new session object
            discovery = session.query(Discovery.timeout).filter(Discovery.discovery_id == discovery_id).all()          # execute query and fetch data
            if len(discovery) == 1:
                dis = discovery[0]
                timeout = dis.timeout
                ud = UpnpDiscovery()
...
class HostBll(object):
    ...
    def grid_view_active_host(self,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars):
        ...
        try:
            ...
            a_columns = ["host_id", "is_localhost", "host_name", "host_alias","ip_address","device_name","hostgroup_name","mac_address","host_alias","priority_name","host_state_id"]
            table_columns = [["host_id", "is_localhost", "host_name", "host_alias","ip_address","mac_address","host_state_id"],
                             ["device_name"],
                             ["hostgroup_name"],
                             [],
                             [],
            ...
            table_classes = [Host_child,DeviceType,Hostgroups,UsersGroups,HostgroupsGroups,HostsHostgroups,Host_parent,Priority]
            table_join = ["user_defined_outerjoin","","user_defined_outerjoin","user_defined_outerjoin","user_defined_outerjoin","","user_defined_outerjoin"]
            ...
            diff_count=3
            hosts=hgbll_obj.get_data_table_sqlalchemy(a_columns,table_columns,table_classes,table_join,other_conditions,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars,join_conditions,group_by,diff_count)
    ...
    def grid_view_disable_host(self,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars):
        ...
        try:
            ...
            req_vars["iSortCol_0"]=int(req_vars["iSortCol_0"])+1		
            hosts=hgbll_obj.get_data_table_sqlalchemy(a_columns,table_columns,table_classes,table_join,other_conditions,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars,join_conditions,group_by,diff_count)
            #hosts = session.query(Hosts.host_id, Hosts.is_localhost, Hosts.host_name, Hosts.host_alias,Hosts.ip_address,Hosts.mac_address,Hosts.host_state_id,DeviceType.device_name).filter(and_(Hosts.is_deleted == 0,Hosts.host_state_id == HostState.disable,Hosts.device_type_id == DeviceType.device_type_id,UsersGroups.user_id=='%s'%(self.userid),UsersGroups.group_id==HostgroupsGroups.group_id,HostsHostgroups.hostgroup_id==HostgroupsGroups.hostgroup_id,Hosts.host_id==HostsHostgroups.host_id)).order_by(Hosts.host_name).all()          # execute query and fetch data
            return hosts
        except Exception,e:
            return str(e)
        #finally:
    ...
    def grid_view_deleted_host(self,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars):
        ...
        try:
            #session = Session()                 # creating new session object
            #hosts = session.query(Hosts.host_id, Hosts.is_localhost, Hosts.host_name, Hosts.host_alias,Hosts.ip_address,Hosts.mac_address,Hosts.host_state_id,DeviceType.device_name,Hosts.timestamp,Users.first_name, Users.last_name).filter(and_(Hosts.is_deleted == 1,Hosts.device_type_id == DeviceType.device_type_id,Hosts.updated_by == Users.user_id)).order_by(Hosts.host_name).all()          # execute query and fetch data
            Host_child = aliased(Hosts)
            Host_parent = aliased(Hosts)
            a_columns = ["host_id","host_alias", "is_localhost", "host_name", "ip_address","device_name","hostgroup_name",\
                         "mac_address","host_alias","priority_name","updated_by","timestamp","host_state_id"]
            table_columns = [["host_id", "host_alias","is_localhost","host_name", "ip_address","mac_address","host_state_id","updated_by","timestamp"],
                             ["device_name"],
                             [],
                             [],
                             [],
            ...
            table_classes = [Host_child,DeviceType,UsersGroups,HostgroupsGroups,HostsHostgroups,Hostgroups,Host_parent,Priority]
            table_join = ["user_defined_outerjoin","user_defined_outerjoin","user_defined_outerjoin","user_defined_outerjoin","user_defined_outerjoin","","user_defined_outerjoin"]
            hgbll_obj=HostgroupBll()
            type_grid="active_host"
            join_conditions = [
                [
            ...
            req_vars["iSortCol_0"]=int(req_vars["iSortCol_0"])+1 if int(req_vars["iSortCol_0"])>2 else int(req_vars["iSortCol_0"])-1 	
            hgbll_obj=HostgroupBll()
            diff_count=5
            ...
            #hosts = session.query(Hosts.host_id, Hosts.is_localhost, Hosts.host_name, Hosts.host_alias,Hosts.ip_address,Hosts.mac_address,Hosts.host_state_id,DeviceType.device_name,Hosts.timestamp,Hosts.updated_by).filter(and_(Hosts.is_deleted == 1,Hosts.device_type_id == DeviceType.device_type_id,UsersGroups.user_id=='%s'%(self.userid),UsersGroups.group_id==HostgroupsGroups.group_id,HostsHostgroups.hostgroup_id==HostgroupsGroups.hostgroup_id,Hosts.host_id==HostsHostgroups.host_id)).order_by(Hosts.host_name).all()          # execute query and fetch data
            return hosts
        except Exception,e:
            return e
        #finally:
    ...
    def grid_view_discovered_host(self,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars):
        ...
        try:
            ...
            for i in ip_list:
                ip_list2.append(i[0])
            a_columns = ["discovered_host_id", "ip_address", "mac_address","timestamp","discovery_type_id","device_name"]
            table_columns = [ ["timestamp","discovery_type_id","discovery_id"],
                              ["discovered_host_id", "ip_address","mac_address","discovery_id"],
            ...
            hosts=hgbll_obj.get_data_table_sqlalchemy(a_columns,table_columns,table_classes,table_join,other_conditions,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars,join_conditions)
            hosts["ip_address"]=ip_list2
            #hosts = session.query(DiscoveredHosts.discovered_host_id,DiscoveredHosts.ip_address,DiscoveredHosts.host_alias,DiscoveredHosts.mac_address, Discovery.timestamp,Discovery.discovery_type_id,DeviceType.device_name).filter(and_(~DiscoveredHosts.ip_address.in_(session.query(Hosts.ip_address)),Discovery.discovery_id == DiscoveredHosts.discovery_id,DiscoveredHosts.device_type_id == DeviceType.device_type_id))          # execute query and fetch data
            session.close()                     # close the session object
            return hosts
        except Exception,e:
            return str(e)
...
#            session.close()                     # close the session object
    ...
    def grid_view_tcp_discovered_host(self,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars):
        ...
        try:
            session = Session()                 # creating new session object
            a_columns = ["ne_id", "site_mac", "ip_address", "product_id","timestamp"]
            table_columns = [["ne_id", "site_mac", "ip_address", "product_id","timestamp"]]
            ...
            req_vars["iSortingCols"]=0
            hosts=hgbll_obj.get_data_table_sqlalchemy(a_columns,table_columns,table_classes,table_join,other_conditions,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars)
            #hosts = session.query(TcpDiscovery.ne_id,TcpDiscovery.site_mac, TcpDiscovery.ip_address, TcpDiscovery.timestamp).filter(~TcpDiscovery.ip_address.in_(session.query(Hosts.ip_address))).all()          # execute query and fetch data
            return hosts
        except Exception,e:
            return str(e)
        finally:
    ...
    def grid_view_active_host_report(self):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(Hosts.host_id, Hosts.is_localhost, Hosts.host_name, Hosts.host_alias,Hosts.ip_address,Hosts.mac_address,Hosts.created_by,Hosts.creation_time,Hosts.host_state_id,DeviceType.device_name).filter(and_(Hosts.is_deleted == 0,Hosts.host_state_id == HostState.enable,Hosts.device_type_id == DeviceType.device_type_id,\
                                                                                                                                                                                                                                       UsersGroups.user_id=='%s'%(self.userid),UsersGroups.group_id==HostgroupsGroups.group_id,HostsHostgroups.hostgroup_id==HostgroupsGroups.hostgroup_id,Hosts.host_id==HostsHostgroups.host_id)).order_by(Hosts.host_name).all()          # execute query and fetch data
            return hosts
        except Exception,e:
            return e
    ...
    def grid_view_disable_host_report(self):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(Hosts.host_id, Hosts.is_localhost, Hosts.host_name, Hosts.host_alias,Hosts.ip_address,Hosts.mac_address,Hosts.updated_by,Hosts.timestamp,Hosts.host_state_id,DeviceType.device_name).filter(and_(Hosts.is_deleted == 0,Hosts.host_state_id == HostState.disable,Hosts.device_type_id == DeviceType.device_type_id,UsersGroups.user_id=='%s'%(self.userid),UsersGroups.group_id==HostgroupsGroups.group_id,HostsHostgroups.hostgroup_id==HostgroupsGroups.hostgroup_id,Hosts.host_id==HostsHostgroups.host_id)).order_by(Hosts.host_name).all()          # execute query and fetch data
            return hosts
        except Exception,e:
            return e
        finally:
    ...
    def grid_view_tcp_discovered_host_report(self):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(TcpDiscovery.ne_id,TcpDiscovery.site_mac, TcpDiscovery.ip_address, TcpDiscovery.timestamp,TcpDiscovery.product_id).filter(~TcpDiscovery.ip_address.in_(session.query(Hosts.ip_address))).all()          # execute query and fetch data
            return hosts
        except Exception,e:
            return e
        finally:
    ...
    def get_master_id_by_macaddress(self,mac_address):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(Hosts.host_id).filter(Hosts.mac_address == mac_address).all()
            if len(hosts) > 0:
    ...
    def get_master_id_by_ra_mac(self,ra_mac):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(Hosts.host_id).join(HostAssets).filter(and_(Hosts.is_deleted == 0,HostAssets.ra_mac == ra_mac)).all()
            if len(hosts) > 0:
                host_id = hosts[0].host_id
            return host_id
        except Exception,e:
    ...
    def get_host_by_id(self,host_id):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(Hosts.host_id,Hosts.host_name,Hosts.host_alias,Hosts.ip_address,Hosts.mac_address,Hosts.device_type_id,Hosts.config_profile_id,\
...
Hosts.snmp_version_id,Hosts.snmp_port,Hosts.snmp_trap_port,Hosts.lock_status,HostAssets.longitude,HostAssets.latitude,HostAssets.serial_number,HostAssets.hardware_version,HostVendor.vendor_name,HostOs.os_name,DeviceType.device_name,Hosts.firmware_mapping_id).filter(and_(Hosts.host_id == host_id,Hosts.host_asset_id == HostAssets.host_asset_id,Hosts.host_vendor_id == HostVendor.host_vendor_id,Hosts.host_os_id == HostOs.host_os_id,Hosts.device_type_id == DeviceType.device_type_id)).all()          # execute query and fetch data
            hostgroup_obj = session.query(HostsHostgroups).filter(HostsHostgroups.host_id == host_id).all()
            ...
            if len(hosts) == 1:
                ...
                if UNMPDeviceType.odu16 == host.device_type_id:
                    ra_mac_table_obj = session.query(GetOdu16RaStatusTable.ra_mac_address).filter(GetOdu16RaStatusTable.host_id == host_id).all()
                    if len(ra_mac_table_obj) > 0:
                        ra_mac = ra_mac_table_obj[0].ra_mac_address

                    node_type_table_obj = session.query(GetOdu16_ru_conf_table.default_node_type).filter(GetOdu16_ru_conf_table.host_id == host_id).all()
                    if len(node_type_table_obj) > 0:
                        node_type = str(node_type_table_obj[0].default_node_type)

                    master_mac_table_obj = session.query(MasterSlaveLinking.master).filter(and_(MasterSlaveLinking.slave == host_id)).all()
                    if len(master_mac_table_obj) > 0:
                        master_mac = master_mac_table_obj[0].master

                elif UNMPDeviceType.odu100 == host.device_type_id:
                    ra_mac_table_obj = session.query(Odu100RaStatusTable.raMacAddress).filter(Odu100RaStatusTable.host_id == host_id).all()
                    if len(ra_mac_table_obj) > 0:
                        ra_mac = ra_mac_table_obj[0].raMacAddress

                    node_type_table_obj = session.query(Odu100RuConfTable.defaultNodeType).filter(Odu100RuConfTable.config_profile_id == host.config_profile_id).all()
                    if len(node_type_table_obj) > 0:
                        node_type = str(node_type_table_obj[0].defaultNodeType)

                    master_mac_table_obj = session.query(MasterSlaveLinking.master).filter(MasterSlaveLinking.slave == host_id).all()
                    if len(master_mac_table_obj) > 0:
                        master_mac = master_mac_table_obj[0].master

                    vlan_table_obj = session.query(Odu100IpConfigTable.managementMode,Odu100IpConfigTable.managementVlanTag).filter(Odu100IpConfigTable.config_profile_id == host.config_profile_id).all()
                    if len(vlan_table_obj) > 0:
                        odu100_management_mode = vlan_table_obj[0].managementMode
                        odu100_vlan_tag = vlan_table_obj[0].managementVlanTag

                elif UNMPDeviceType.ccu == host.device_type_id:
                    ccu_network_table_obj = session.query(CcuNetworkConfigurationTable.ccuNCDHCPNetMask).filter(CcuNetworkConfigurationTable.host_id == host_id).all()
                    if len(ccu_network_table_obj) > 0:
                        ccu_dhcp_netmask = ccu_network_table_obj[0].ccuNCDHCPNetMask
    ...
    def get_nms_instance_id(self,nms_name):
        ...
        try:
            session = Session()                 # creating new session object
            nms = session.query(NmsInstance.nms_id).filter(NmsInstance.nms_name == nms_name).all()          # execute query and fetch data
            if len(nms) == 1:
                return nms[0]
            else:
                err = ErrorMessages()
                ...
                if self.validate(host_name,host_alias,ip_address,mac_address,device_type_id,netmask,gateway,primary_dns,secondary_dns,odu100_management_mode,odu100_vlan_tag,idu4_management_mode,idu4_vlan_tag,idu4_tdm_ip,ccu_dhcp_netmask,host_state_id,priority_id,host_vendor_id,host_os_id,http_username,http_password,http_port,ssh_username,ssh_password,ssh_port,snmp_read_community,snmp_write_community,snmp_port,snmp_trap_port,comment,longitude,latitude,serial_number,hardware_version):
                    if self.is_duplicate_host_for_add(host_name,host_alias,ip_address,mac_address):
                        ...
                        if isinstance(nms, ErrorMessages):
                            ...
                            host_asset = HostAssets(longitude,latitude,serial_number,hardware_version,"0000-00-00 00:00:00",0,None,None,None,None,ra_mac)
                        ...
                        reconcile_health=0
                        reconcile_status=0
                        host = Hosts(host_name,host_alias,ip_address,mac_address,device_type_id,netmask,gateway,primary_dns,secondary_dns,dns_state,config_profile_id,timestamp,created_by,creation_time,is_deleted,updated_by,ne_id,site_id,host_state_id,priority_id,host_vendor_id,host_os_id,host_asset_id,http_username,http_password,http_port,snmp_read_community,snmp_write_community,snmp_port,snmp_trap_port,snmp_version_id,comment,nms_id,parent_name,lock_status,is_localhost,reconcile_health,reconcile_status,ssh_username,ssh_password,ssh_port,firmware_mapping_id)
                        session.add(host)
                        session.flush()
                        session.refresh(host)
                        services_list = self.get_services_from_templates(host.device_type_id)
                        host_data = session.query(Hosts).filter(and_(Hosts.host_id == host.host_id,Hosts.is_deleted == 0)).all()
                        n_bll=NagiosBll()
                        for srv in services_list:
                            # [service_description,check_command,max_check_attempts,normal_check_interval,retry_check_interval]
                            # new code
                            service = HostServices(host.host_id,srv.service_description,srv.check_command,srv.max_check_attempts,srv.normal_check_interval,srv.retry_check_interval,0)
                            session.add(service)
                            if(flag_nagios_call==1):
                                wnc=n_bll.add_new_service(str(host.host_name)+str(host.host_id),host.host_alias, srv.service_description, srv.max_check_attempts, srv.normal_check_interval, srv.retry_check_interval, srv.check_command)
                                condition_nagios = wnc["success"]==0
                        # add host status default entry
                        host_status = HostStatus(host.ip_address,host.host_id,'0',0)
                        ...
                        if device_type_id == 'odu16':
                            config_profile_id = obj_reconcilation.odu16_add_current_config_profile(host.host_id,'odu16','odu16_',is_reconciliation)
                            ...
                            #config_profile_id = obj_reconcilation.reconcilation_controller##(host.host_id,'odu100','odu100_',False)
                        elif device_type_id == 'odu100':
                            config_profile_id = obj_reconcilation.odu100_add_default_config_profile(host.host_id,'odu100','odu100_',datetime.now(),is_reconciliation,created_by)
                        elif device_type_id == 'idu4':
                            config_profile_id = idu_obj_reconciliation.default_reconciliation_controller(host.host_id,'idu4','idu_',True)
                        ...
                        #################### reconciliation for ap #################################################
                            ...
                            config_profile_id = ap_obj_reconciliation.default_configuration_added(host.host_id,'ap25','ap25_',datetime.now(),created_by,is_reconciliation)
                        #################### reconciliation for ap #################################################
                        #################### reconciliation for ccu #################################################
                        elif device_type_id == 'ccu':
                            config_profile_id = ccu_obj_reconciliation.default_reconciliation_controller(host.host_id,'ccu','ccu_',datetime.now(),created_by,is_reconciliation)
                        #################### reconciliation for ccu #################################################


                        host_data = session.query(Hosts).filter(and_(Hosts.host_id == host.host_id,Hosts.is_deleted == 0)).all()
                        if device_type_id=="odu100" and type(config_profile_id)==dict:
                            host_data[0].config_profile_id = config_profile_id['result'][0]
                            host_data[0].reconcile_health = config_profile_id['result'][1]
                            ...
                            if len(host_data)>0 and config_profile_id!=None and type(config_profile_id) == tuple and len(config_profile_id)==2:
                                host_data[0].config_profile_id = config_profile_id[0]
                                host_data[0].reconcile_health = config_profile_id[1]
                                temp_conf_id = config_profile_id[0]
                            elif len(host_data)>0 and config_profile_id!=None:
                                host_data[0].config_profile_id = config_profile_id
                                temp_conf_id = config_profile_id
                        if len(host_data) > 0:
                            host_name = "host" + str(host.host_id)
                            host_data[0].host_name = host_name

                        hostgroup_obj = HostsHostgroups(host.host_id,hostgroup)
                        session.add(hostgroup_obj)
                        session.commit()
#                        ###############Anuj samariya ###########################
...
#    	                session.commit()
                        ...
                        if UNMPDeviceType.odu16 == device_type_id:
                            ra_mac_table_obj = session.query(GetOdu16RaStatusTable).filter(GetOdu16RaStatusTable.host_id == host.host_id).all()
                            if len(ra_mac_table_obj) > 0:
                                ra_mac_table_obj[0].ra_mac_address = ra_mac
                            else:
                                ra_status_table = GetOdu16RaStatusTable(host.host_id,0,ra_mac)
                                session.add(ra_status_table)

                            node_type_table_obj = session.query(GetOdu16_ru_conf_table).filter(GetOdu16_ru_conf_table.host_id == host.host_id).all()
                            if len(node_type_table_obj) > 0:
                                node_type_table_obj[0].default_node_type = node_type

                            else:
                                ru_conf_table = GetOdu16_ru_conf_table(host.host_id,0,None,node_type,None)
                                session.add(ru_conf_table)

                            if master_id != "" and master_id != None:
                                master_mac_table_obj = MasterSlaveLinking(master_id,host.host_id)
                                session.add(master_mac_table_obj)
                            session.commit()
                            #return host.host_id
                        elif UNMPDeviceType.odu100 == device_type_id:
                            if is_reconciliation==True:
                                obj_reconcilation.update_reconcilation_controller(host.host_id,device_type_id,"odu100_",datetime.now(),created_by)
                            ra_mac_table_obj = session.query(Odu100RaStatusTable).filter(Odu100RaStatusTable.host_id == host.host_id).all()
                            if len(ra_mac_table_obj) > 0:
                                ra_mac_table_obj[0].raMacAddress = ra_mac
                            else:
                                ra_status_table = Odu100RaStatusTable(host.host_id, 1, None, ra_mac, None, None, None, 0, 0, datetime.now())
                                session.add(ra_status_table)

                            node_type_table_obj = session.query(Odu100RuConfTable).filter(Odu100RuConfTable.config_profile_id == temp_conf_id).all()
                            if len(node_type_table_obj) > 0:
                                node_type_table_obj[0].defaultNodeType = node_type
                            else:
                                if temp_conf_id != None:
                                    ru_conf_table = Odu100RuConfTable(temp_conf_id,1,None,node_type,None,None,None,None,0,0,0,0,0)
                                    session.add(ru_conf_table)
                                    #pass
                            ...
                            # manage vlan
                            vlan_table_obj = session.query(Odu100IpConfigTable).filter(Odu100IpConfigTable.config_profile_id == temp_conf_id).all()
                            if len(vlan_table_obj) > 0:
                                vlan_table_obj[0].managementMode = odu100_management_mode
                                vlan_table_obj[0].managementVlanTag = odu100_vlan_tag
                            else:
                                if odu100_management_mode != None and odu100_vlan_tag != None:
                                    vlan_table = Odu100IpConfigTable(temp_conf_id,1,None,ip_address,netmask,gateway,dns_state,odu100_management_mode,odu100_vlan_tag)
                                    session.add(vlan_table)
                            # end - manage vlan
                            session.commit()
                        elif UNMPDeviceType.idu4 == device_type_id:
                            if is_reconciliation==True:
                                idu_obj_reconciliation.update_device_reconcilation_controller(host.host_id,'idu4','idu_',True,created_by)
                        elif UNMPDeviceType.ap25 == device_type_id:
                            if is_reconciliation==True:
                                update_profile = ap_obj_reconciliation.update_configuration(host.host_id,'ap25','ap25_',datetime.now(),created_by)
                                if update_profile["success"]==0:
                                    for i in update_profile["result"]:
                                        host_data[0].reconcile_health = i
                        elif UNMPDeviceType.ccu == device_type_id:
                            if is_reconciliation==True:
                                update_profile = ccu_obj_reconciliation.update_reconcilation_controller(host.host_id,'ccu','ccu_',datetime.now(),created_by)
                                if update_profile["success"]==0:
                                    for i in update_profile["result"]:
                                        host_data[0].reconcile_health = i
                            # manage vlan
                            vlan_table_obj = session.query(CcuNetworkConfigurationTable).filter(CcuNetworkConfigurationTable.host_id == host.host_id).all()
                            if len(vlan_table_obj) > 0:
                                vlan_table_obj[0].ccuNCDHCPNetMask = ccu_dhcp_netmask
                            else:
                                if ccu_dhcp_netmask != None:
                                    vlan_table = CcuNetworkConfigurationTable(host.host_id,1,mac_address,ip_address,netmask,"","",ccu_dhcp_netmask,gateway)
                                    session.add(vlan_table)
                            # end - manage vlan
                        ...
                        if(flag_nagios_call==0):
                            ...
                            n_bll=NagiosBll()
                            wnc=n_bll.add_new_host(host_name,host_alias,ip_address,parent_name,hostgroup)
                            condition_nagios = wnc["success"]==0
                        if condition_nagios:
                            if flag_nagios_call:
                                SystemSetting.reload_nagios_config()
                            ...
                            el = EventLog()
                            el.log_event("Add New Host '%s'." % (ip_address) ,updated_by)
                            return host.host_id
                        else:
                            err = ErrorMessages()
                            err.error_msg = err.nagios_config_error
    ...
    def get_services_from_templates(self,device_type):
        ...
        try:
            session = Session()                 # creating new session object
            services = session.query(ServiceTemplates).filter(ServiceTemplates.device_type_id == device_type).all()          # execute query and fetch data
            return services
        except Exception,e:
            err = ErrorMessages()
            err.error_msg = err.db_error
        ...
        try:
            ...
            if self.validate(host_name,host_alias,ip_address,mac_address,device_type_id,netmask,gateway,primary_dns,secondary_dns,odu100_management_mode,odu100_vlan_tag,idu4_management_mode,idu4_vlan_tag,idu4_tdm_ip,ccu_dhcp_netmask,host_state_id,priority_id,host_vendor_id,host_os_id,http_username,http_password,http_port,ssh_username,ssh_password,ssh_port,snmp_read_community,snmp_write_community,snmp_port,snmp_trap_port,comment,longitude,latitude,serial_number,hardware_version):
                if self.is_duplicate_host_for_edit(host_id, host_name, host_alias,ip_address,mac_address):
                    hosts = session.query(Hosts).filter(Hosts.host_id == host_id).all()
                    if len(hosts) == 1:
                        host = hosts[0]
                        ...
                        if host.device_type_id != device_type_id:
                            if host.config_profile_id != None:
                                cursor.execute("delete from config_profiles where config_profile_id = %s" % host.config_profile_id)
                            if host.host_id != None: 
                                # delete services
                                cursor.execute("delete from host_services where host_id = %s" % host.host_id)
                                # trap & alarm table will empty
                                cursor.execute("delete from trap_alarms where agent_id = '%s'" % host.ip_address)
                                cursor.execute("delete from trap_alarm_clear where agent_id = '%s'" % host.ip_address)
                                cursor.execute("delete from trap_alarm_current where agent_id = '%s'" % host.ip_address)
                                cursor.execute("delete from system_alarm_table where agent_id = '%s'" % host.ip_address)

                                if host.device_type_id == UNMPDeviceType.odu16:
                                    # odu 16 get table will empty
                                    cursor.execute("delete from get_odu16_nw_interface_statistics_table where host_id = %s" % host.host_id)
                                    cursor.execute("delete from get_odu16_peer_node_status_table where host_id = %s" % host.host_id)
                                    cursor.execute("delete from get_odu16_ra_tdd_mac_statistics_entry where host_id = %s" % host.host_id)
                                    ...
                                    cursor.execute("delete from analyze_get_odu16_peer_node_status_table where host_id = %s" % host.host_id)
                                    cursor.execute("delete from analyze_get_odu16_ra_tdd_mac_statistics_entry where host_id = %s" % host.host_id)
                                    cursor.execute("delete from analyze_get_odu16_synch_statistics_table where host_id = %s" % host.host_id)

                                if host.device_type_id == UNMPDeviceType.odu100:
                                    # odu 100 get table will empty
                                    cursor.execute("delete from odu100_nwInterfaceStatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from odu100_raTddMacStatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from odu100_raScanListTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from odu100_synchStatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from odu100_peerNodeStatusTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from odu100_swStatusTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from odu100_hwDescTable where host_id = %s" % host.host_id)
                                    ...
                                    cursor.execute("delete from analyze_odu100_peerNodeStatusTable where host_id = %s" % host.host_id)
                                ...
                                if host.device_type_id == UNMPDeviceType.idu4:
                                    ...
                                    cursor.execute("delete from idu_e1PortStatusTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from idu_linkStatusTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from idu_portSecondaryStatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from idu_portstatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from idu_swPrimaryPortStatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from idu_iduInfoTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from idu_swStatusTable where host_id = %s" % host.host_id)
                                    # idu 4 analyze table will empty
                                    cursor.execute("delete from analyze_idu_portSecondaryStatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from analyze_idu_portstatisticsTable where host_id = %s" % host.host_id)
                                    cursor.execute("delete from analyze_idu_swPrimaryPortStatisticsTable where host_id = %s" % host.host_id)

                                if(flag_nagios_call==1):
                                    n_bll=NagiosBll()
	                            wnc=n_bll.delete_old_service(host.host_name)
	                            condition_nagios = wnc["success"]==0
	                        
                                if host.device_type_id == UNMPDeviceType.ap25:
                                    # ap get table will empty
                                    cursor.execute("delete from ap25_statisticsTable where host_id = %s" % host.host_id)
                                if device_type_id == UNMPDeviceType.ap25 or device_type_id == UNMPDeviceType.idu4 or device_type_id == UNMPDeviceType.generic or device_type_id == UNMPDeviceType.swt4:
                                    cursor.execute("delete from master_slave_linking where master = %s OR slave = %s" % (host.host_id,host.host_id))
                            ...
                            if device_type_id == UNMPDeviceType.odu16:
                                config_profile_id = obj_reconcilation.odu16_add_current_config_profile(host.host_id,'odu16','odu16_',False)
                            elif device_type_id == UNMPDeviceType.odu100:
                                config_profile_id = obj_reconcilation.reconcilation_controller(host.host_id,'odu100','odu100_',datetime.now(),False,updated_by)
                            elif device_type_id == UNMPDeviceType.idu4:
                                config_profile_id = idu_obj_reconciliation.new_device_reconcilation_controller(host.host_id,'idu4','idu_',False,updated_by)
                            elif device_type_id == UNMPDeviceType.ap25:
                                config_profile_id = ap_obj_reconciliation.default_configuration_added(host.host_id,'ap25',False)
                            ...
                            # service recreation
                            services_list = self.get_services_from_templates(device_type_id)
                            for srv in services_list:
                                # [service_description,check_command,max_check_attempts,normal_check_interval,retry_check_interval]
                                service = HostServices(host.host_id,srv.service_description,srv.check_command,srv.max_check_attempts,srv.normal_check_interval,srv.retry_check_interval,0)
                                session.add(service)
                                ...
                                if(flag_nagios_call==1):
                                    n_bll=NagiosBll()
                                    wnc=n_bll.add_new_service(str(host.host_name),host.host_alias, srv.service_description, srv.max_check_attempts, srv.normal_check_interval, srv.retry_check_interval, srv.check_command)
                                    condition_nagios = wnc["success"]==0
                            # end - service recreation
                        # end - if device type is changed

                                # Firmware change for odu100 than truncate the odu100_raAclConfigTable table 
                        if device_type_id == "odu100":
			    ...
			    if len(result)>0 and result[0][0] != None and result[0][0] != "" and firmware_mapping_id != result[0][0]:
			        ...
			        cursor.execute(del_query)
                        ...
                        if len(hosts) == 1:
                            ...
                            host_assets = session.query(HostAssets).filter(HostAssets.host_asset_id == host.host_asset_id).all()
                            ...
                            host_status = session.query(HostStatus).filter(HostStatus.host_id == host_id).all()
                            if len(host_status) == 1:
                                host_stat = host_status[0]
                                host_stat.status = '0'
                            elif len(host_status) == 0:
                                # add host status default entry
                                host_status = HostStatus(ip_address,host_id,'0',0)
                                session.add(host_status)
                                # END - add host status default entry
                        ...
                        if UNMPDeviceType.odu16 == device_type_id:
                            ...
                            if len(ra_mac_table_obj) > 0:
                                ...
                                ra_status_table = Odu100RaStatusTable(host.host_id,1,None,ra_mac,None,None,None)
                                session.add(ra_status_table)
                            ...
                            if len(node_type_table_obj) > 0:
                                ...
                                if temp_conf_id != None:
                                    ru_conf_table = Odu100RuConfTable(temp_conf_id,1,None,node_type,None,None,None,None,0)
                                    session.add(ru_conf_table)

                            if master_id != "" and master_id != None:
                                master_mac_table_obj = session.query(MasterSlaveLinking).filter(MasterSlaveLinking.slave == host.host_id).all()
                                if len(master_mac_table_obj) > 0:
                            ...
                            if len(vlan_table_obj) > 0:
                                ...
                                if ccu_dhcp_netmask != None:
                                    vlan_table = CcuNetworkConfigurationTable(host.host_id,0,mac_address,ip_address,netmask,"","",ccu_dhcp_netmask,gateway)
                                    session.add(vlan_table)
                            # end - manage vlan
                            session.commit()
                        #ncbll = NagioConfigurationBll()
                        ...
                        if(flag_nagios_call==0):
                            ...
                            n_bll=NagiosBll()
                            wnc=n_bll.edit_old_host(host_name,host_alias,ip_address,parent_name,hostgroup)
                            condition_nagios = wnc["success"]==0
                        if (condition_nagios):
                            if flag_nagios_call:
                                SystemSetting.reload_nagios_config()
                        #if isinstance(wnc, bool):
                            el = EventLog()
                            el.log_event("Edit Host '%s' Details." % (ip_address) ,updated_by)
    ...
    def add_deleted_host(self,host_ids,nms_instance,updated_by):
        ...
        try:
            ...
            for host_i in range(0,len(host_ids)):
                deleted_host = session.query(Host.host_name,Hosts.host_alias,Host.ip_address,Host.mac_address).filter(Hosts.host_id == host_ids[host_i]).all()
                    ...
                    if self.is_duplicate_host_for_add(d_host.host_name, d_host.host_alias,d_host.ip_address,d_host.mac_address):
                        ...
                        if len(hosts) == 1:
                            ...
                            if(flag_nagios_call==0):
                                ...
                                n_bll=NagiosBll()
                                wnc=n_bll.add_new_host(hosts.host_name,hosts.host_alias,hosts.ip_address,hosts.parent_name)
                                condition_nagios = wnc["success"]==0
    ...
    def delete(self,host_ids,nms_instance,updated_by):
        ...
        try:
            session = Session()                 # creating new session object
            # get localhost
            localhost = session.query(Hosts.host_id).filter(Hosts.is_localhost == 1).all()
            localhost_count = len(localhost)
            if localhost_count > 0:
                localhost_id = localhost[0].host_id

                host_childs = session.query(Hosts).filter(Hosts.parent_name.in_(host_ids)).all()
                for hc in host_childs:
                    hc.parent_name = localhost_id
                session.commit()

                hosts = session.query(Hosts).filter(Hosts.host_id.in_(host_ids)).all()          # execute query and fetch data
                host_count = len(hosts)
                if host_count > 0:
                    el = EventLog()
                    for hs in hosts:
                        el.log_event("Host '%s' Deleted [You can view it in deleted host]." % (hs.ip_address) ,updated_by)
                        hs.is_deleted = 1
...
#                    wnc=n_bll.delete_old_host(host_name)
                ...
                if host_count > 0:
                    ...
                    for hst in hosts:
                        ...
                        if hst.config_profile_id != None:
                            cursor.execute("delete from config_profiles where config_profile_id = %s" % hst.config_profile_id)
                        if hst.host_id != None: 
                            # trap & alarm table will empty
                            cursor.execute("delete from trap_alarms where agent_id = '%s'" % hst.ip_address)
                            cursor.execute("delete from trap_alarm_clear where agent_id = '%s'" % hst.ip_address)
                            cursor.execute("delete from trap_alarm_current where agent_id = '%s'" % hst.ip_address)
                            cursor.execute("delete from system_alarm_table where agent_id = '%s'" % hst.ip_address)
                            cursor.execute("delete from host_status where host_ip = '%s'" % hst.ip_address)

                            if hst.device_type_id == UNMPDeviceType.odu16:
                                # odu 16 get table will empty
                                cursor.execute("delete from get_odu16_nw_interface_statistics_table where host_id = %s" % hst.host_id)
                                cursor.execute("delete from get_odu16_peer_node_status_table where host_id = %s" % hst.host_id)
                                cursor.execute("delete from get_odu16_ra_tdd_mac_statistics_entry where host_id = %s" % hst.host_id)
                                cursor.execute("delete from get_odu16_synch_statistics_table where host_id = %s" % hst.host_id)
                                # odu 16 analyze table will empty
                                cursor.execute("delete from analyze_get_odu16_nw_interface_statistics_table where host_id = %s" % hst.host_id)
                                cursor.execute("delete from analyze_get_odu16_peer_node_status_table where host_id = %s" % hst.host_id)
                                cursor.execute("delete from analyze_get_odu16_ra_tdd_mac_statistics_entry where host_id = %s" % hst.host_id)
                            ...
                            if hst.device_type_id == UNMPDeviceType.odu100:
                                # odu 100 get table will empty
                                cursor.execute("delete from odu100_nwInterfaceStatisticsTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from odu100_raTddMacStatisticsTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from odu100_raScanListTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from odu100_synchStatisticsTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from odu100_peerNodeStatusTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from odu100_swStatusTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from odu100_hwDescTable where host_id = %s" % hst.host_id)
                                # odu 100 analyze table will empty
                                cursor.execute("delete from analyze_odu100_nwInterfaceStatisticsTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from analyze_odu100_peerNodeStatusTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from analyze_odu100_raTddMacStatisticsTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from analyze_odu100_synchStatisticsTable where host_id = %s" % hst.host_id)

                            if hst.device_type_id == UNMPDeviceType.idu4:
                                # idu 4 get table will empty
                                cursor.execute("delete from idu_tdmoipNetworkInterfaceStatisticsTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from idu_iduNetworkStatisticsTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from idu_e1PortStatusTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from idu_linkStatusTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from idu_portSecondaryStatisticsTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from idu_portstatisticsTable where host_id = %s" % hst.host_id)
                                cursor.execute("delete from idu_swPrimaryPortStatisticsTable where host_id = %s" % hst.host_id)
                                # idu 4 analyze table will empty
                            ...
                            if hst.device_type_id == UNMPDeviceType.ap25:
                                # ap get table will empty
                                cursor.execute("delete from ap25_statisticsTable where host_id = %s" % hst.host_id)


                        if hst.host_asset_id != None:
                            cursor.execute("delete from host_assets where host_asset_id = %s" % hst.host_asset_id)
                        cursor.execute("delete from master_slave_linking where master = %s OR slave = %s" % (hst.host_id,hst.host_id))
                        cursor.execute("delete from hosts where host_id = %s" % hst.host_id)
                        cursor.close()
                        el.log_event("Host '%s' Deleted." % (hst.ip_address) ,updated_by)
                    db.commit()
                    db.close()
                    #ncbll = NagioConfigurationBll()
                    #wnc = ncbll.write_nagios_config(nms_instance)
    ...
    def is_duplicate_host_for_edit(self,host_id,host_name,host_alias,ip_address,mac_address):
        ...
        try:
            session = Session()                 # creating new session object
            host_count = session.query(Hosts).filter(and_(Hosts.host_id != host_id,or_(Hosts.host_name == host_name, Hosts.host_alias == host_alias,Hosts.ip_address == ip_address, Hosts.mac_address == mac_address),Hosts.is_deleted == 0)).count()
            if host_count == 0:
                return True
            else:
                return False
    ...
    def host_parents(self,host_id=None):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(Hosts.host_id, Hosts.is_localhost, Hosts.host_name, Hosts.host_alias,Hosts.ip_address,Hosts.mac_address).filter(and_(Hosts.is_deleted == 0,Hosts.host_id != host_id)).order_by(Hosts.host_alias).all()          # execute query and fetch data
            return hosts
        except Exception,e:
            return e
        finally:
...
WHERE hosts.is_deleted = '0' AND hosts.host_id IS NOT NULL AND users_groups.user_id = '%s' AND users_groups.group_id = hostgroups_groups.group_id AND hosts_hostgroups.hostgroup_id = hostgroups_groups.hostgroup_id AND hosts.host_id = hosts_hostgroups.host_id OR hosts.ip_address = 'localhost' ORDER BY hosts.host_name"%(user_id)
    ...
    def host_hostgroups(self):
        ...
        try:
            session = Session()                 # creating new session object
            hostgroups = session.query(Hostgroups.hostgroup_id, Hostgroups.hostgroup_name, Hostgroups.hostgroup_alias).filter(Hostgroups.is_deleted == 0).order_by(Hostgroups.hostgroup_alias).all()          # execute query and fetch data
            return hostgroups
        except Exception,e:
            return e
        finally:
    ...
    def host_hostgroups2(self,userid):
        ...
        try:
            session = Session()                 # creating new session object
            hostgroups = session.query(Hostgroups.hostgroup_id, Hostgroups.hostgroup_name, Hostgroups.hostgroup_alias).filter(and_(Hostgroups.is_deleted == 0,UsersGroups.user_id=='%s'%(userid),UsersGroups.group_id==HostgroupsGroups.group_id,HostgroupsGroups.hostgroup_id==Hostgroups.hostgroup_id)).order_by(Hostgroups.hostgroup_alias).all()          # execute query and fetch data
            return hostgroups
        except Exception,e:
            return e
        finally:
    ...
    def host_vendors(self):
        ...
        try:
            session = Session()                 # creating new session object
            vendors = session.query(HostVendor.host_vendor_id, HostVendor.vendor_name).filter(HostVendor.is_deleted == 0).order_by(HostVendor.vendor_name).all()          # execute query and fetch data
            return vendors
        except Exception,e:
            return e
        finally:
    ...
    def host_os(self):
        ...
        try:
            session = Session()                 # creating new session object
            os = session.query(HostOs.host_os_id, HostOs.os_name).filter(HostOs.is_deleted == 0).order_by(HostOs.sequence).all()          # execute query and fetch data
            return os
        except Exception,e:
            return e
        finally:
    ...
    def host_priority(self):
        ...
        try:
            session = Session()                 # creating new session object
            priority = session.query(Priority.priority_id, Priority.priority_name).filter(Priority.is_deleted == 0).order_by(Priority.sequence).all()          # execute query and fetch data
            return priority
        except Exception,e:
            return e
        finally:
    ...
    def host_state(self):
        ...
        try:
            session = Session()                 # creating new session object
            state = session.query(HostStates.host_state_id, HostStates.state_name).filter(HostStates.is_deleted == 0).order_by(HostStates.sequence).all()          # execute query and fetch data
            return state
        except Exception,e:
            return e
        finally:
    ...
    def device_type(self):
        ...
        try:
            session = Session()                 # creating new session object
            device = session.query(DeviceType.device_type_id, DeviceType.device_name).filter(DeviceType.is_deleted == 0).order_by(DeviceType.sequence).all()          # execute query and fetch data
            return device
        except Exception,e:
            return e
        finally:
            ...
            if len(host_details)>0:
                if host_details[0].config_profile_id == "" or host_details[0].config_profile_id == None:
                    ...
                    if device_dict["device_type_id"]==UNMPDeviceType.odu100:
                        ...
                        gateway_oid = ["1.3.6.1.4.1.26149.2.2.9.1.5.1","IpAddress",device_dict["ip_gateway"]]
                        autoIp = ["1.3.6.1.4.1.26149.2.2.9.1.6.1","Integer32",device_dict["dhcp"]]
                        vlan_management = ['1.3.6.1.4.1.26149.2.2.9.1.7.1','Integer32',device_dict["odu100_management_mode"]]
                        vlan_tag = ['1.3.6.1.4.1.26149.2.2.9.1.8.1','Integer32',device_dict["odu100_vlan_tag"]]
                        admin_state = ['1.3.6.1.4.1.26149.2.2.9.1.2.1','Integer32',0]
                        dic_odu = {'ip':ip_oid,'netmask':netmask_oid,'gateway':gateway_oid,'autoip':autoIp,'vlan_management':vlan_management,'vlan_tag':vlan_tag}
                        admin_dic = {'admin-1':admin_state}
                        change_net_config = pysnmp_set(dic_odu,host_details[0].ip_address,host_details[0].snmp_port,host_details[0].snmp_write_community,admin_dic)
                        admin_state = ['1.3.6.1.4.1.26149.2.2.9.1.2.1','Integer32',1]
                        admin_dic = {'admin':admin_state}
                    ...
                    if device_dict["device_type_id"]==UNMPDeviceType.ccu:
                        netmask_oid = ["1.3.6.1.4.1.26149.3.5.1.4.0","OctetString",device_dict["ip_network_mask"]]
                        gateway_oid = ["1.3.6.1.4.1.26149.3.5.1.8.0","OctetString",device_dict["ip_gateway"]]
                        dhcp_netmask = ["1.3.6.1.4.1.26149.3.5.1.7.0","OctetString",device_dict["ccu_dhcp_netmask"]]
                        dic_odu = {'netmask':netmask_oid,'gateway':gateway_oid,'dhcp_netmask':dhcp_netmask}
                        change_net_config = pysnmp_seter(dic_odu,host_details[0].ip_address,int(host_details[0].snmp_port),host_details[0].snmp_write_community)

                    if device_dict["device_type_id"]==UNMPDeviceType.ap25:
                        ip_oid = ["1.3.6.1.4.1.26149.10.1.2.1.0","OctetString",device_dict["ip_address"]]
                        netmask_oid = ["1.3.6.1.4.1.26149.10.1.2.2.0","OctetString",device_dict["ip_network_mask"]]
                        gateway_oid = ["1.3.6.1.4.1.26149.10.1.2.3.0","OctetString",device_dict["ip_gateway"]]
                        primarydns = ["1.3.6.1.4.1.26149.10.1.2.4.0","OctetString",device_dict["primarydns"]]
                        secondarydns = ["1.3.6.1.4.1.26149.10.1.2.5.0","OctetString",device_dict["secondarydns"]]
                        dic_ap = {'ip':ip_oid,'netmask':netmask_oid,'gateway':gateway_oid,'primarydns':primarydns,'secondarydns':secondarydns}
                        change_net_config = pysnmp_set(dic_ap,host_details[0].ip_address,host_details[0].snmp_port,host_details[0].snmp_write_community)
                    for i in range(0,3):
                        if device_dict["device_type_id"]==UNMPDeviceType.ccu:
                    ...
                    if chk == 1:
                        if device_dict["device_type_id"]==UNMPDeviceType.odu16:
                            ...
                            ap25_details = sqlalche_obj.query(Ap25AccesspointIPsettings).filter(Ap25AccesspointIPsettings.config_profile_id == host_details[0].config_profile_id).all()
                            if len(ap25_details)>0:
                                ap25_details[0].lanIPaddress= device_dict["ip_address"]
                                ap25_details[0].lanSubnetMask= device_dict["ip_network_mask"]
                                ap25_details[0].lanGatewayIP= device_dict["ip_gateway"]
                                ap25_details[0].lanPrimaryDNS = device_dict["primarydns"]
                                ap25_details[0].lanSecondaryDNS = device_dict["secondarydns"]
                            else:
                                ap25_detail = Ap25AccesspointIPsettings(host_details[0].config_profile_id,device_dict["ip_address"],device_dict["ip_network_mask"],device_dict["ip_gateway"],device_dict["primarydns"],device_dict["secondarydns"])
    ...
    def odu_master(self,device_type_id):
        ...
        try:
            ...
            device = []
            if device_type_id == UNMPDeviceType.odu16:
                device = session.query(Hosts.ip_address,Hosts.mac_address,Hosts.host_id).filter(and_(Hosts.is_deleted == 0,GetOdu16_ru_conf_table.host_id == Hosts.host_id,or_(GetOdu16_ru_conf_table.default_node_type == 0,GetOdu16_ru_conf_table.default_node_type == 2),Hosts.device_type_id == device_type_id)).all()          # execute query and fetch data
            elif device_type_id == UNMPDeviceType.odu100:
                device = session.query(Hosts.ip_address,Hosts.mac_address,Hosts.host_id).filter(and_(Hosts.is_deleted == 0,Odu100RuConfTable.config_profile_id == Hosts.config_profile_id,or_(Odu100RuConfTable.defaultNodeType == 0,Odu100RuConfTable.defaultNodeType == 2),Hosts.device_type_id == device_type_id)).all()          # execute query and fetch data
            return device
        except Exception,e:
            return e
        finally:
    ...
    def is_duplicate_host_with_ip_address(self,ip_address):
        ...
        try:
            session = Session()                 # creating new session object
            host = session.query(Hosts.host_id).filter(and_(Hosts.is_deleted == 0,Hosts.ip_address == ip_address)).count()          # execute query and fetch data
            if host == 0:
                return True
            else:
                return False
    ...
    def is_duplicate_host_with_host_alias(self,host_alias):
        ...
        try:
            session = Session()                 # creating new session object
            host = session.query(Hosts.host_id).filter(and_(Hosts.is_deleted == 0,Hosts.host_alias == host_alias)).count()          # execute query and fetch data
            if host == 0:
                return True
            else:
                return False
    ...
    def is_duplicate_host_with_mac_address(self,mac_address):
        ...
        try:
            session = Session()                 # creating new session object
            host = session.query(Hosts.host_id).filter(and_(Hosts.is_deleted == 0,Hosts.mac_address == mac_address)).count()          # execute query and fetch data
            if host == 0:
                return True
            else:
                return False
...
class VendorBll(object):
    def grid_view(self):
        ...
        try:
            session = Session()                 # creating new session object
            vendors = session.query(HostVendor.host_vendor_id, HostVendor.vendor_name, HostVendor.description).filter(HostVendor.is_deleted == 0).order_by(HostVendor.vendor_name).all()          # execute query and fetch data
            return vendors
        except Exception,e:
            return e
        finally:
    ...
    def get_vendor_by_id(self,host_vendor_id):
        ...
        try:
            session = Session()                 # creating new session object
            vendors = session.query(HostVendor.host_vendor_id, HostVendor.vendor_name, HostVendor.description).filter(HostVendor.host_vendor_id == host_vendor_id).all()          # execute query and fetch data
            if len(vendors) == 1:
                return vendors[0]
            else:
                err = ErrorMessages()
        ...
        try:
            ...
            if self.validate(vendor_name,description):
                if self.is_duplicate_vendor_for_add(vendor_name):
                    vendor = HostVendor(vendor_name,description,timestamp,created_by,creation_time,updated_by,is_deleted)
                    session.add(vendor)
                    session.commit()
                    el = EventLog()
                    el.log_event("Add New Vendor '%s'." % (vendor_name) ,updated_by)
        ...
        try:
            ...
            if self.validate(vendor_name, description):
                if self.is_duplicate_vendor_for_edit(host_vendor_id, vendor_name):
                    vendors = session.query(HostVendor).filter(HostVendor.host_vendor_id == host_vendor_id).all()
                    if len(vendors) == 1:
                        vendor = vendors[0]
                        ...
                        session.commit()
                        el = EventLog()
                        el.log_event("Vendor '%s' Details Updated." % (vendor_name) ,updated_by)
                        return vendor.host_vendor_id
                    else:
                        err = ErrorMessages()
                        err.error_msg = err.no_record_error
    ...
    def delete(self,vendor_ids,updated_by):
        ...
        try:
            session = Session()                 # creating new session object
            vendors = session.query(HostVendor).filter(HostVendor.host_vendor_id.in_(vendor_ids)).all()          # execute query and fetch data
            vendor_count = len(vendors)
            if vendor_count > 0:
                el = EventLog()
                for vd in vendors:
                    el.log_event("Vendor '%s' Deleted." % (vd.vendor_name) ,updated_by)
                    session.delete(vd)
                session.commit()
                return vendor_count
            else:
    ...
    def is_duplicate_vendor_for_add(self,vendor_name):
        ...
        try:
            session = Session()                 # creating new session object
            vendor_count = session.query(HostVendor).filter(HostVendor.vendor_name == vendor_name).count()
            if vendor_count == 0:
                return True
            else:
                return False
    ...
    def is_duplicate_vendor_for_edit(self,host_vendor_id,vendor_name):
        ...
        try:
            session = Session()                 # creating new session object
            vendor_count = session.query(HostVendor).filter(and_(HostVendor.host_vendor_id != host_vendor_id,HostVendor.vendor_name == vendor_name)).count()
            if vendor_count == 0:
                return True
            else:
                return False
...
class BlackListMacBll(object):
    def grid_view(self):
        ...
        try:
            session = Session()                 # creating new session object
            black_list_macs = session.query(BlackListMacs.black_list_mac_id, BlackListMacs.mac_address, BlackListMacs.description).filter(BlackListMacs.is_deleted == 0).order_by(BlackListMacs.mac_address).all()          # execute query and fetch data
    ...
    def get_black_list_mac_by_id(self,black_list_mac_id):
        ...
        try:
            session = Session()                 # creating new session object
            black_list_macs = session.query(BlackListMacs.black_list_mac_id, BlackListMacs.mac_address, BlackListMacs.description).filter(BlackListMacs.black_list_mac_id == black_list_mac_id).all()          # execute query and fetch data
        ...
        try:
            ...
            if self.validate(mac_address,description):
                if self.is_duplicate_black_list_mac_for_add(mac_address):
                    black_list_mac = BlackListMacs(mac_address,description,timestamp,created_by,creation_time,updated_by,is_deleted)
                    session.add(black_list_mac)
                    session.commit()
                    el = EventLog()
                    el.log_event("Add New Black Listed Host '%s'." % (mac_address) ,updated_by)
        ...
        try:
            ...
            if self.validate(mac_address, description):
                if self.is_duplicate_black_list_mac_for_edit(black_list_mac_id, mac_address):
                    black_list_macs = session.query(BlackListMacs).filter(BlackListMacs.black_list_mac_id == black_list_mac_id).all()
                    if len(black_list_macs) == 1:
                        ...
                        session.commit()
                        el = EventLog()
                        el.log_event("Black Listed Host '%s' Updated." % (mac_address) ,updated_by)
                        return black_list_mac.black_list_mac_id
                    else:
                        err = ErrorMessages()
                        err.error_msg = err.no_record_error
    ...
    def delete(self,black_list_mac_ids,updated_by):
        ...
        try:
            session = Session()                 # creating new session object
            black_list_macs = session.query(BlackListMacs).filter(BlackListMacs.black_list_mac_id.in_(black_list_mac_ids)).all()          # execute query and fetch data
            black_list_mac_count = len(black_list_macs)
            if black_list_mac_count > 0:
                el = EventLog()
                for vd in black_list_macs:
                    el.log_event("Black Listed Host '%s' Deleted." % (mac_address) ,updated_by)
                    session.delete(vd)
                session.commit()
    ...
    def is_duplicate_black_list_mac_for_add(self,mac_address):
        ...
        try:
            session = Session()                 # creating new session object
            black_list_mac_count = session.query(BlackListMacs).filter(BlackListMacs.mac_address == mac_address).count()
            if black_list_mac_count == 0:
                return True
            else:
                return False
    ...
    def is_duplicate_black_list_mac_for_edit(self,black_list_mac_id,mac_address):
        ...
        try:
            session = Session()                 # creating new session object
            black_list_mac_count = session.query(BlackListMacs).filter(and_(BlackListMacs.black_list_mac_id != black_list_mac_id,BlackListMacs.mac_address == mac_address)).count()
            if black_list_mac_count == 0:
                return True
            else:
                return False
...
class ServiceBll(object):
    ...
    def grid_view(self,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars):
        ...
        try:
            #session = Session()                 # creating new session object
            a_columns = ["host_id","is_localhost","host_name","ip_address","host_alias","hostgroup_name","device_name"]
            table_columns=[["host_id","is_localhost","host_name","ip_address","host_alias","host_state_id"],
                           ["device_name"],["hostgroup_name"]]
            #table_classes = [Hosts,DeviceType]
            #table_join = ["user_defined_outerjoin"]
            table_classes = [Hosts,DeviceType,Hostgroups,UsersGroups,HostgroupsGroups,HostsHostgroups]
            table_join = ["user_defined_outerjoin","","user_defined_outerjoin","user_defined_outerjoin","user_defined_outerjoin"]
            hgbll_obj=HostgroupBll()
            join_conditions = [
                [
                    {
            ...
            req_vars["iSortCol_0"]=int(req_vars["iSortCol_0"])+1# if int(req_vars["iSortCol_0"])>2 else int(req_vars["iSortCol_0"])-1 	
            #hosts = session.query(Hosts.host_id, Hosts.is_localhost, Hosts.host_name, Hosts.host_alias,Hosts.ip_address,Hosts.mac_address,Hosts.host_state_id,DeviceType.device_name).filter(and_(Hosts.is_deleted == 0,Hosts.host_state_id == HostState.enable,Hosts.device_type_id == DeviceType.device_type_id)).order_by(Hosts.host_name).all()          # execute query and fetch data
            hosts=hgbll_obj.get_data_table_sqlalchemy(a_columns,table_columns,table_classes,table_join,other_conditions,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,req_vars,join_conditions) 
            return hosts
        except Exception,e:
            output = {
    ...
    def get_host_details(self,host_id):
        ...
        try:
            session = Session()                 # creating new session object
            hosts = session.query(Hosts.host_id,Hosts.host_name,Hosts.host_alias,Hosts.ip_address).filter(and_(Hosts.host_id == host_id)).all()          # execute query and fetch data
            return hosts
        except Exception,e:
            err = ErrorMessages()
            err.error_msg = err.db_error
    ...
    def grid_view_service(self,host_id):
        ...
        try:
            session = Session()                 # creating new session object
            services = session.query(HostServices.service_description,HostServices.normal_check_interval,HostServices.host_id).filter(HostServices.host_id == host_id).order_by(HostServices.service_description).all()          # execute query and fetch data
            return services
        except Exception,e:
            return e
        finally:
    ...
    def set_service_time(self,hosts_list,service_name,service_time,user_name):
        ...
        try:
            ...
            for i in range(len(hosts_list)):                 # creating new session object
                services = session.query(HostServices).filter(and_(HostServices.host_id == hosts_list[i],HostServices.service_description ==service_name[i].replace('_',' '))).update({HostServices.normal_check_interval:service_time[i]})
                if(flag_nagios_call==0):
                    ncbll = NagioConfigurationBll()
                    wnc = ncbll.write_nagios_config(nms_instance)
                    condition_nagios =  isinstance(wnc, bool)
                    ...
                    n_bll=NagiosBll()
                    wnc=n_bll.edit_old_service(hosts_list[i] ,service_name[i].replace('_',' ') , service_time[i])# (host_id, service_description, normal_check_interval)
                    condition_nagios = wnc["success"]==0
	    ...
	    wnc = ncbll.write_nagios_config(nms_instance) 
            ...
            for service in services:
                ...
                #else:
                normal_check_interval=(service.normal_check_interval==None and s_config_dict["normal_check_interval"]) or str(service.normal_check_interval)
                max_check_attempts=(service.max_check_attempts==None and s_config_dict["max_check_attempts"]) or str(service.max_check_attempts)
                retry_check_interval=(service.retry_check_interval==None and s_config_dict["retry_check_interval"]) or str(service.retry_check_interval)
                s_config += ng_cng.service_config(use_template,service.host_name,service.service_description,max_check_attempts,normal_check_interval,retry_check_interval,service.check_command)
...
class NagioConfigurationBll(object):
    def write_nagios_config(self,nms_instance):
        ...
        try:
            session = Session()                 # creating new session object
            ng_cng = NagiosConfiguration()
            hostgroups = session.query(Hostgroups.hostgroup_name, Hostgroups.hostgroup_alias).filter(Hostgroups.is_deleted == 0).all()          # execute query and fetch data
            hg_config = ""
            for hostgroup in hostgroups:
                hg_config += ng_cng.hostgroup_config(hostgroup.hostgroup_name,hostgroup.hostgroup_alias)
            ng_cng.write_hostgroup_config_file(nms_instance,hg_config)
            
            Parents = aliased(Hosts, name='parents')
            hosts = session.query(Hosts.host_name, Hosts.host_alias,Hosts.ip_address,Hosts.priority_id,Hostgroups.hostgroup_name,(Parents.host_name).label("parent")).outerjoin(Parents,Hosts.parent_name == Parents.host_id).filter(and_(Hosts.is_deleted == 0,HostsHostgroups.hostgroup_id==Hostgroups.hostgroup_id,Hosts.host_id==HostsHostgroups.host_id)).all()          # execute query and fetch data
            ...
            for host in hosts:
                h_config += ng_cng.host_config(use_template,host.host_name,host.host_alias,host.ip_address,host.hostgroup_name,check_command,host.parent)
        ...
        if validate == 1:
            while check_ip(first,second) == 1:
                if int(first[3]) != 255 or int(first[3]) != 1 or int(first[3]) != 2:
                    dst_ip_str ="%s.%s.%s.%s" % (first[0],first[1],first[2],first[3])
...
#    ln+=2
    ...
    h = HostBll()
    rvalue = h.add(ip_address,ip_address,ip_address,mac,device_type_id,"255.255.255.0","10.10.10.1",'','','',None,'',None,0,'',None,None,'e',"normal",'1','lenny','','','','public','private','161','162','2c','','UNMP',parent_name,'f',1,'%.2f'%lg,'%.2f'%ln,'','',hostgroup,mac,node_type,master_id)
...
def demo_tree_host(idu,idu_parent):
    ...
    if idu=="m":
        ...
        odu100m_parent = add_demo_host(ip_address,idu_parent,mac,"odu100",0)
        print "odu100 M ",ip_address        
        ip_address,mac = get_ip_mac()
        add_demo_host(ip_address,idu_parent,mac,"ap25",0)
        print "AP  ",ip_address     
        ip_address,mac = get_ip_mac()
        odu100s_parent =  add_demo_host(ip_address,odu100m_parent,mac,"odu100",1,odu100m_parent)
...
def p2mp_setup():
    ...
    for i in m100_li:
        ...
        for k in range(0,odu100):
            ip_address = ip_li_g.pop(0)
            mac = get_mac()
            add_demo_host(ip_address,parent_name,mac,"odu100",1,parent_name)        
    for i in m16_li:
        parent_name = i
        for k in range(0,odu16):
            ip_address = ip_li_g.pop(0)
            mac = get_mac()
            add_demo_host(ip_address,parent_name,mac,"odu16",1,parent_name)        
