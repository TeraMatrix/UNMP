...
from pysnmp_ap import pysnmp_get,bulktable
...
host_status_dic = {0:'No operation', 1:'Firmware download', 2:'Firmware upgrade', 3:'Restore default config', 4:'Flash commit',\
    5:'Reboot', 6:'Site survey', 7:'Calculate BW', 8:'Uptime service', 9:'Statistics gathering', 10:'Reconciliation', 11:'Table reconciliation', 12:'Set operation', 13:'Live monitoring', 14:'Status capturing'}
...
def sync_config_table_get(host_id):
    ...
    sqlalche_obj.sql_alchemy_db_connection_open()
    odu16_profile_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).all() 
    sync_config_table=sqlalche_obj.session.query(SetOdu16SyncConfigTable.raster_time ,SetOdu16SyncConfigTable.num_slaves,SetOdu16SyncConfigTable.sync_loss_threshold,SetOdu16SyncConfigTable.leaky_bucket_timer,\
    SetOdu16SyncConfigTable.sync_lost_timeout,SetOdu16SyncConfigTable.sync_config_time_adjust,SetOdu16SyncConfigTable.sync_config_broadcast_enable).filter(SetOdu16SyncConfigTable.config_profile_id==odu16_profile_id[0][0]).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
        ...
        for i in range(0,len(device_tuple)):
            ...
            if device_tuple[i][6]=="odu16":
                ...
                if len(master_slave_select)>0:
                    if master_slave_select[0][0]!=None and (int(master_slave_select[0][0]) == 0 or int(master_slave_select[0][0])==2):
                        slave_data = "-"
                        master_slave = "UBR (M)"
                    else:
                        slave_data = ""
                        master_host_id = sqlalche_obj.session.query(MasterSlaveLinking.master).filter(MasterSlaveLinking.slave == device_tuple[i][0]).all()
                        #return [" jai shree ram ****** ",str(master_host_id)] 
                        if len(master_host_id)>0:
                            ...
                            master_mac = str(host_asset_data[0].ra_mac if len(host_asset_data)>0 and host_asset_data[0].ra_mac!=None else "")
                            peer_status = sqlalche_obj.session.query(GetOdu16PeerNodeStatusTable.sig_strength).\
                            filter(and_(GetOdu16PeerNodeStatusTable.peer_mac_addr==master_mac,\
                            or_(GetOdu16PeerNodeStatusTable.host_id==device_tuple[i][0],GetOdu16PeerNodeStatusTable.sig_strength==1))).order_by(desc(GetOdu16PeerNodeStatusTable.timestamp)).limit(1).all()
                            if len(peer_status)>0:
                                if peer_status[0].sig_strength==None:
                                    slave_data = str(host_data[0].host_alias)+"(-)"
                                elif int(peer_status[0].sig_strength)==1:
                                    slave_data = str(host_data[0].host_alias)+" ( Link Disconnected )"
                                
                                else:
                                    slave_data = str(host_data[0].host_alias)+" ("+str(peer_status[0].sig_strength)+"dBm)"
                            else:
                                slave_data = str(host_data[0].host_alias)+"(-)"
                        else:
                            slave_data = "-"
                ...
                if len(master_slave_select)>0:
                    if master_slave_select[0][0] == 0 or master_slave_select[0][0] == 2:
                        ...
                        if len(master_host_id)>0:
                            ...
                            if len(peer_status)>0:
                                if int(peer_status[0].sigStrength1)==1 or int(peer_status[0].sigStrength1)==-95:
                                    slave_data = str(host_data[0].host_alias)+" ( Link Disconnected )"
                                elif peer_status[0].sigStrength1==None:
                                    slave_data = str(host_data[0].host_alias)+"(-)"
                                
                                else:
                                    slave_data = str(host_data[0].host_alias)+" ("+str(peer_status[0].sigStrength1)+"dBm)"
                            else:
                                slave_data = str(host_data[0].host_alias)+"(-)"
                        else:
                            slave_data = "(-)"
            ...
            if len(sync_data)>0:
                if sync_data[0][0]==None:
                    sync_state = 1
                    image_sync_title = "RU State UnLocked"
            ...
            live_monitoring = "&nbsp;&nbsp;<a target=\"main\" href=\"live_monitoring.py?host_id=%s\"><img src=\"images/new/star-empty.png\" title=\"Live Monitoring\" style=\"width:16px;height:16px;\" class=\"imgbutton n-reconcile\" /></a>"%(device_tuple[i][0])\
            ...
            "<input type=\"hidden\" value=\"%s\" name=\"host_id\" id=\"host_id\" />"%(device_status_host_id) if i==len(device_tuple)-1 else ""),"<center><img id=\"operation_status\" name=\"operation_status\" src=\"%s\" title=\"%s\" style=\"width:12px;height:12px;\"class=\"imgbutton n-reconcile\"/></center>&nbsp;&nbsp;"%(op_img,op_title)])
        ...
        for i in range(0,len(device_tuple)):
            if device_tuple[i][6]=="odu16":
                ...
                if len(master_slave_select)>0:
                    if master_slave_select[0][0] == 0 or master_slave_select[0][0] == 2:
                        slave_data = "-"
                        master_slave = "UBRe (Master)"
                    else:
                        #slave_data = "0"
                        slave_data = ""
                        master_host_id = sqlalche_obj.session.query(MasterSlaveLinking.master).filter(MasterSlaveLinking.slave == device_tuple[i][0]).all()
                        if len(master_host_id)>0:
                            host_data = sqlalche_obj.session.query(Hosts.host_alias).filter(and_(Hosts.host_id==master_host_id[0][0],Hosts.is_deleted==0)).all()
                            peer_status = sqlalche_obj.session.query(Odu100PeerNodeStatusTable.sigStrength1).\
                            filter(and_(Odu100PeerNodeStatusTable.tunnelStatus==1,Odu100PeerNodeStatusTable.linkStatus.in_([1,2]),\
                            Odu100PeerNodeStatusTable.host_id==device_tuple[i][0])).order_by(desc(Odu100PeerNodeStatusTable.timestamp)).limit(1).all()
                            if len(peer_status)>0:
                                if int(peer_status[0].sigStrength1)==1:
                                    slave_data = str(host_data[0].host_alias)+" ( Link Disconnected )"
                                elif peer_status[0].sigStrength1==None:
                                    slave_data = str(host_data[0].host_alias)+"(-)"
...
def odu100_get_peerconfigtable(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_peer_conf_table = sqlalche_obj.session.query(Odu100PeerConfigTable.peerMacAddress ,Odu100PeerConfigTable.guaranteedUplinkBW,Odu100PeerConfigTable.guaranteedDownlinkBW,Odu100PeerConfigTable.basicrateMCSIndex).filter(Odu100PeerConfigTable.config_profile_id == odu100_profile_id[0]).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
...
def odu100_get_raconfigtable(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            Odu100RaConfTable.guaranteedBroadcastBW,Odu100RaConfTable.acs,Odu100RaConfTable.dfs,Odu100RaConfTable.aclMode,Odu100RaConfTable.antennaPort).filter(Odu100RaConfTable.config_profile_id == odu100_profile_id[0]).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
...
def odu100_get_ruconfigtable(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            Odu100RuConfTable.poeState ,Odu100RuConfTable.alignmentControl,Odu100RuConfTable.defaultNodeType).filter(Odu100RuConfTable.config_profile_id == odu100_profile_id[0]).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
...
class MakeOdu100SelectListWithDic(object):
    ...
    def country_code_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        country_code_select_list_name_value_dic = {'name':['DBG','Denmark(208)','India(356)','Sweden(752)','USA(840)'],'value':['0','208','356','752','840']}
    ...
    def poe_state_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        poe_state_select_list_name_value_dic = {'name':['Disable','Enable','Not Applicable'],'value':['0','1','2']}
    ...
    def acs_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        acs_state_select_list_name_value_dic = {'name':['Disbale','Enable'],'value':['0','1']}
    ...
    def dfs_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        dfs_state_select_list_name_value_dic = {'name':['Disable','Enable'],'value':['0','1']}
    ...
    def preffered_channel_select_list_snmp(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        # this dictionary is used to store the name and value of select list
        if preferred_channel_dic.has_key(str(selected_field)):
            selected_field = 0
    ...
    def preffered_channel_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg,host_id,selected_device):
        ...
        if len(channel_list)>0:
            ...
            if preffered_channel_list_name_value_dic.has_key(str(selected_field)):
                selected_field = 0
    ...
    def acl_index_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        """
        Author - Anuj Samariya
    ...
    def raster_time_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        raster_time_name_value_dic = {'name':['2','4','8'],'value':['2','4','8']}
    ...
    def mcs_index_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        """
        Author - Anuj Samariya
        This function is used to create the select list of acl_index_select_list
        self - It denotes the class object[Type - MakeOdu100SelectListWithDic ]
        ...
        if selected_field!=-1:
            for i in range(0,8):
                mcs_index_name_value_dic.setdefault('name',[]).append(i)
                mcs_index_name_value_dic.setdefault('value',[]).append(i)        
        else:
            for i in range(0,9):
                if i==0:
        ...
        return make_select_list_using_dictionary(mcs_index_name_value_dic,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg)
    
    def mcs_index_select_list_mimo(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        """
        Author - Anuj Samariya
        This function is used to create the select list of acl_index_select_list
        self - It denotes the class object[Type - MakeOdu100SelectListWithDic ]
        ...
        if(j == -1): 
            if len(depend_oid_value)>0:              
                for i in range(0,len(depend_oid_value )):                    
                    result = pysnmp_set(depend_oid_value[i],device_param_list[0],device_param_list[1],device_param_list[2])                   
                    if result["success"] == 0 or result["success"] == '0':
                        success_result["success"] = result["success"]
                        for i in result["result"]:
                            if result["result"][i] != 0:
...
def check_mac(host_id,macaddress,acl_mode):
    try:
        global sqlalche_obj
        sqlalche_obj.sql_alchemy_db_connection_open()
        odu100_profile_id = []
...
def mac_chk_deny(host_id,mac_list):
    try:
        ...
        sqlalche_obj.sql_alchemy_db_connection_close()
#print mac_chk_deny(44,['00:ff:aa:33:33:84', '00:ff:aa:33:33:86', '', ''])
   ...
   # {keys:[query_result[0][0].oid+query_result[0][0].indexes,query_result[0][0].oid_type,dic_result[keys]]}
    try:
        ...
        errorStatus = {0:'noError',
                   ...
                   52:'typeError',
                   53:'No Response From Device.Please Try Again',
        ...
        if save_retry ==1:
            ...
            if result['success']==1:
                for i in result["result"]:                    
                    result["result"] = errorStatus.get(i,"Configuration Failed.Please try again later")
            else:
                ...
                for i in range(0,timeslot_val):
                    ...
                    dic_result['ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.%s'%(i+1)])
    ...
    try:
        ...
        if result_admin["success"]==0:
            ...
            for i in result_admin["result"]:
                    errorStatus.has_key(i)
                    result_admin["result"] = errorStatus.get(i,"Device is busy.Please try again later")
                    sqlalche_obj.sql_alchemy_db_connection_close()
                    return result_admin
...
##            sqlalche_obj.session.add(add_ra_llc)
    ...
    def odu100_add_default_config_profile(self,host_id,device_type_id,table_prefix,current_time,reconcile_chk,user_name):
        try:
            ...
            if len(ru_conf_data)>0:
                for i in range(0,len(ru_conf_data)):
                    ...
                    ru_conf_data_add = Odu100RuConfTable(new_profile_id,ru_conf_data[i].confIndex,ru_conf_data[i].adminstate,\
                                                        ...
                                                        ru_conf_data[i].synchSource,ru_conf_data[i].countryCode,ru_conf_data[i].poeState,ru_conf_data[i].alignmentControl)
            ...
            ru_status_data = Odu100RuStatusTable(host_id,1,None,1,None,None,None,None)
...
##    def update_reconcilation_controller(self,host_id,device_type_id,table_prefix,insert_update,user_name):#'42669d26-1820-11e1-8c26-e069956899a4'
##        #For Insert a new profile
##        #host_id = for new Profile it is Null else It is passed by the function user
##        #device_type_id  = Pass byt the function user
##        # table_prefix = pass by the table user as the database table name prefixes
##        #insert_update = pass by the function user if want to isert a new profile then pass true else pass false
##        try:
##            global errorStatus
##            global sqlalche_obj
##            i = 0
...
##                tables_not_fill = sqlalche_obj.session.query(NmsGraphs.tablename).filter(NmsGraphs.device_type_id == device_type_id).all()
##                for i in range(0,len(tables_not_fill)):
##                    fill_not_table.append(tables_not_fill[i][0])
##                query_result = sqlalche_obj.session.query(Oids.table_name).distinct().filter(not_(Oids.table_name.in_((fill_not_table)))).filter(Oids.device_type_id==device_type_id).all()
##                if insert_update == True:
##                    # For Update the New Row
##                    m = 0
##                    for j in range(0,len(query_result)):         
##                        if query_result[j].table_name == "peerConfigTable" or query_result[j].table_name == "raPreferredRFChannelTable" or query_result[j].table_name == "raAclConfigTable" or query_result[j].table_name == "Odu100RaChannelListTable":
##                            continue
##                        else:    
##                            if query_result[j].table_name=="ipConfigTable" or query_result[j].table_name=="omcConfTable" \
##                            or query_result[j].table_name=="raConfTable" or query_result[j].table_name=="raTddMacConfigTable" \
##                            or query_result[j].table_name=="ruConfTable" or query_result[j].table_name=="syncConfigTable" or query_result[j].table_name=="swStatusTable" or\
##                            query_result[j].table_name=="hwDescTable":
##                                result = sqlalche_obj.session.query(Oids.coloumn_name,Oids.oid+Oids.indexes).filter(Oids.table_name==query_result[j].table_name).all()
##                                
##                                form_dict = {}
##                                for i in range(0,len(result)):
##                                    form_dict[result[i][0]] = result[i][1]                
##                                getvalue = pysnmp_geter(form_dict,device_param_list[0],device_param_list[1],device_param_list[2])                
##                                time.sleep(5)
##                                tablename = table_prefix+query_result[j].table_name
##                                table_name = rename_tablename(tablename)
##                                try:
##                                    exec  "table_result = sqlalche_obj.session.query(%s).filter(%s.config_profile_id == '%s').all()"%(table_name,table_name,device_param_list[3])
##                                    
##                                except Exception as e:
##                                    exec "table_result = sqlalche_obj.session.query(%s).filter(%s.host_id == '%s').all()"%(table_name,table_name,host_id)
##                                
##                                if table_result == []:
##                                    flag = 1
##                                else:
##                                    if getvalue["success"] == 0 and getvalue["result"] == {}:
##                                        flag_empty = flag_empty + 1
##                                        if flag_empty == 4:
##                                            break
##                                        else:
##                                            continue
##                                    elif getvalue["success"] == 0:
##                                        if tablename == "odu100_ipConfigTable":
##                                            rec = rec + 1
##                                        elif tablename == "odu100_omcConfTable":
##                                            rec = rec + 1
##                                        elif tablename == "odu100_raConfTable":
##                                            rec = rec + 1
##                                        elif tablename == "odu100_raTddMacConfigTable":
##                                            rec = rec + 1
##                                        elif tablename == "odu100_ruConfTable":
##                                            rec = rec + 1
##                                        elif tablename == "odu100_syncConfigTable":
##                                            rec = rec + 1
##                                        elif tablename == "odu100_swStatusTable":
##                                            rec = rec + 1
##                                        elif tablename == "odu100_hwDescTable":
##                                            rec = rec + 1
##                                        for k in getvalue["result"]:
##                                            if k in form_dict:
##                                                exec "table_result[0].%s = '%s'"%(k,getvalue["result"][k])                    
##                                            else:
##                                                exec "table_result[0].%s = None"%(k)
##                                        sqlalche_obj.session.commit()
##                                    elif getvalue["success"] == 1:
##                                        if tablename == "odu100_ipConfigTable":
##                                            rec = rec - 1
##                                        elif tablename == "odu100_omcConfTable":
##                                            rec = rec - 1
...
##                                        elif tablename == "odu100_hwDescTable":
##                                            rec = rec - 1
##                                        for i in getvalue["result"]:
##                                            if errorStatus.has_key(i):
##                                                flag = 2
##                                                m = m+1
##                                                getvalue["result"] = errorStatus[i]
##                                        if flag == 4 and m == 5:
##                                            break
##                                        else:
##                                            
##                                            continue
##                if flag_empty != 4:        
##                    result_acl = pysnmp_acl_reconcile("1.3.6.1.4.1.26149.2.2.13.5",device_param_list.ip_address,int(device_param_list.snmp_port),device_param_list.snmp_read_community)
##                    if len(result_acl)==1:
##                        rec = rec - 1 
##                    elif len(result_acl)>1:
##                        rec = rec + 1
##                        acl_config = sqlalche_obj.session.query(Odu100RaAclConfigTable).filter(Odu100RaAclConfigTable.config_profile_id==device_param_list.config_profile_id).delete()
##                        sqlalche_obj.session.commit()
##                        if len(result_acl['mac'])==0 and len(result_acl['row'])==0:
##                            result_acl = {'success':0,'result':"Acl Reconciliation Done Successfully"}
##                        elif (len(result_acl['mac'])==len(result_acl['row'])):
##                            mac = result_acl['mac']
##                            row = result_acl['row']
##                            for i in row:
##                                acl_row = Odu100RaAclConfigTable(device_param_list.config_profile_id,1,i,mac[i],i)
##                                sqlalche_obj.session.add(acl_row)
##
##                    result_peer = pysnmp_peer_reconcile('1.3.6.1.4.1.26149.2.2.13.9.1',device_param_list.ip_address,int(device_param_list.snmp_port),device_param_list.snmp_read_community)
##                    if result_peer["success"]==0:
##                        rec = rec + 1
##                        peer_config = sqlalche_obj.session.query(Odu100PeerConfigTable).filter(Odu100PeerConfigTable.config_profile_id == device_param_list.config_profile_id).delete()        
##                        sqlalche_obj.session.commit()
##                        m = 0
##                        for i in result_peer["result"]:
##                            peer_add = Odu100PeerConfigTable(device_param_list.config_profile_id,1,1,result_peer["result"][i][0],result_peer["result"][i][1],result_peer["result"][i][2],result_peer["result"][i][3])
##                            sqlalche_obj.session.add(peer_add)
##                        sqlalche_obj.session.commit()
##                    else:
##                        rec = rec - 1
##                    
##                    result_channel = pysnmp_get_table('1.3.6.1.4.1.26149.2.2.13.7.4',device_param_list.ip_address,int(device_param_list.snmp_port),device_param_list.snmp_read_community)
##                    if result_channel["success"]==0:
##                        rec = rec + 1
##                        channel_result = sqlalche_obj.session.query(Odu100RaPreferredRFChannelTable).filter(Odu100RaPreferredRFChannelTable.config_profile_id == device_param_list.config_profile_id).delete()        
##                        sqlalche_obj.session.commit()
##                        m = 0
##                        if len(result_channel["result"])==10:
##                            for i in result_channel["result"]:
##                                channel_add = Odu100RaPreferredRFChannelTable(device_param_list.config_profile_id,1,result_channel["result"][i][0],result_channel["result"][i][1])
...
##                        rec = rec - 1
##                   
##                    result_ra_channel_list = pysnmp_get_table('1.3.6.1.4.1.26149.2.2.13.3',device_param_list.ip_address,int(device_param_list.snmp_port),device_param_list.snmp_read_community)
##                    if result_ra_channel_list["success"]==0:
##                        rec = rec + 1
##                        if len(result_ra_channel_list["result"])>0:
##                            channel_result = sqlalche_obj.session.query(Odu100RaChannelListTable).filter(Odu100RaChannelListTable.host_id == host_id).delete()        
##                            sqlalche_obj.session.commit()
##                            m = 0
##                            for i in result_ra_channel_list["result"]:
##                                channel_list_add = Odu100RaChannelListTable(host_id,1,i+1,result_ra_channel_list["result"][i][0],result_ra_channel_list["result"][i][1])
##                                sqlalche_obj.session.add(channel_list_add)
##                                m = m+1
##                            sqlalche_obj.session.commit()
##                    
##                    tdd_mac_status_table = pysnmp_get_table('1.3.6.1.4.1.26149.2.2.13.7.2',device_param_list.ip_address,int(device_param_list.snmp_port),device_param_list.snmp_read_community)
##                    if tdd_mac_status_table["success"]==0:
##                        rec = rec + 1
##                        if len(tdd_mac_status_table["result"])>0:
##                            tdd_mac_status = sqlalche_obj.session.query(Odu100RaTddMacStatusTable).filter(Odu100RaTddMacStatusTable.host_id == host_id).delete()        
...
##                    host_data[0].reconcile_status = 0
##                    el = EventLog()
##                    el.log_event("Device Reconcilation Not Done.SNMP Agent Down","%s"%(user_name))
##                    sqlalche_obj.session.commit()   
##                    return -1
##            else:
##                host_data[0].reconcile_health = 0
##                host_data[0].reconcile_status = 0
##                el = EventLog()
##                el.log_event("Device Reconcilation Not Done.Device not ping","%s"%(user_name))
##                sqlalche_obj.session.commit()   
##                return ""
##                
##        except ProgrammingError as e:
...
##    def reconcilation_controller(self,host_id,device_type_id,table_prefix,reconcile_chk=True):#'42669d26-1820-11e1-8c26-e069956899a4'
##        #For Insert a new profile
##        #host_id = for new Profile it is Null else It is passed by the function user
##        #device_type_id  = Pass byt the function user
##        # table_prefix = pass by the table user as the database table name prefixes
...
##                    query_result = sqlalche_obj.session.query(Oids.table_name).distinct().filter(not_(Oids.table_name.in_((fill_not_table)))).filter(Oids.device_type_id==device_type_id).all()
##                    m = 0   
##                    profile_id = Odu16ConfigProfiles(device_type_id,None,'Master',None,datetime.now(),None,datetime.now(),None,0)
##                    sqlalche_obj.session.add(profile_id)
##                    sqlalche_obj.session.flush()
##                    sqlalche_obj.session.refresh(profile_id)
...
##                            or query_result[j].table_name=="ruConfTable" or query_result[j].table_name=="syncConfigTable" or query_result[j].table_name=="swStatusTable" or\
##                            query_result[j].table_name=="hwDescTable" :
##                                result = sqlalche_obj.session.query(Oids.coloumn_name,Oids.oid+Oids.indexes,Oids.indexes_name).filter(Oids.table_name==query_result[j].table_name).all()
##                                form_dict = {}
##                                oids_indexes_name = {}
##                                oids_index = []
##                                for i in range(0,len(result)):
##                                    form_dict[result[i][0]] = result[i][1] 
##                                    oids_indexes_name[result[i][0]] = result[i][2]
##                                
##                                getvalue = pysnmp_geter(form_dict,device_param_list[0],device_param_list[1],device_param_list[2])
##                                table = query_result[j].table_name
##                                tablename = table_prefix+query_result[j].table_name
##                                
##                                if getvalue["success"] == 0 and getvalue["result"] == {}:
##                                    flag_empty = flag_empty + 1
...
##                                            if k!='preindex':
##                                                add_column.append(k)
##                                                add_row.append(("'"+getvalue["result"].get(k)+"'"))
##                                            oids_index = oids_indexes_name[k].split(",")
##                                            for p in range(0,len(oids_index)):
##                                                if oids_index[p] not in add_column:
##                                                    add_column.append(oids_index[p])
##                                                    add_row.append('1')
##                                                else:
##                                                    continue
##                                        else:
...
##                                        config = "config_profile_id"
##                                        host = "host_id"
##                                    try: 
##                                        sqlalche_obj.db.execute("insert into %s(%s,%s,%s) values('',%s,%s)"%(tablename,primary_key,config,(",".join(add_column)),config_profile_id,(",".join(add_row))))
##                                    except Exception as e:   
##                                        
##                                        sqlalche_obj.db.execute("insert into %s(%s,%s) values(%s,%s)"%(tablename,host,(",".join(add_column)),host_id,(",".join(add_row))))
##                                    
##                                elif getvalue["success"] == 1:
##                                    if tablename == "odu100_ipConfigTable":
##                                        rec = rec - 1
...
##                                row = result_acl['row']
##                                for i in row:
##                                    acl_row = Odu100RaAclConfigTable(config_profile_id,1,i,mac[i],i)
##                                    sqlalche_obj.session.add(acl_row)
##                                    
##                        result_peer = pysnmp_peer_reconcile('1.3.6.1.4.1.26149.2.2.13.9.1',device_param_list.ip_address,int(device_param_list.snmp_port),device_param_list.snmp_read_community)
##                        if result_peer["success"]==0:
...
##                            m = 0
##                            for i in result_peer["result"]:
##                                peer_add = Odu100PeerConfigTable(config_profile_id,1,1,result_peer["result"][i][0],result_peer["result"][i][1],result_peer["result"][i][2],result_peer["result"][i][3])
##                                sqlalche_obj.session.add(peer_add)
##                            sqlalche_obj.session.commit()
##                        else:
##                            rec = rec - 1
...
##                            if len(result_channel["result"])==10:
##                                for i in result_channel["result"]:
##                                    channel_add = Odu100RaPreferredRFChannelTable(config_profile_id,1,result_channel["result"][i][0],result_channel["result"][i][1])
##                                    sqlalche_obj.session.add(channel_add)
##                                    m = m+1
##                                sqlalche_obj.session.commit()
##                            else:
...
##                        sqlalche_obj.session.commit()
##            ##            if flag == 1:
##            ##                getvalue["result"] = "Profile Updated SuccessFully.But Some Fields are missing."
##            ##            elif flag == 2:
##            ##                getvalue["result"] = "Request is Timeout.Please Retry Again" 
##            ##            else:
##            ##                getvalue["result"] = "Profile Updated Successfully"
##                        sqlalche_obj.session.commit()            
##                        sqlalche_obj.sql_alchemy_db_connection_close()  
...
##                        sqlalche_obj.session.commit()
##                        reconcile_per = 0
##                        config_profile_id,reconcile_per =  self.odu100_add_default_config_profile(host_id,device_type_id,table_prefix,reconcile_chk)
##                        return str(config_profile_id),reconcile_per
##                else:
##                    host_data[0].reconcile_status = 0
##                    sqlalche_obj.session.commit()
                                ...
                                if len(table_result)>0:
                                    ...
                                    if len(rec_table_dic)>0:
                                        ...
                                        for j in rec_table_dic:
                                            ...
                                            time.sleep(5)
                                            result = bulktable(str(rec_table_dic[j])+str(".1"),host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community,int(table_var_bind.get(j)))
                                                ...
                                                for key in result['result']:
                                                    if int(key)==53:
                                                        ...
                                                        if key in errorStatus:
                                                            result['result']=errorStatus.get(key,"UNMP Server is busy.Please try after some time,2003") 
                                                        ...
                                                        if len(sqlalche_table_result)>0:
                                                            for row in result['result']:
                                                                for val in range(0,len(result['result'][row])): 
                                                                    setattr(sqlalche_table_result[row-1],column_list[val],str(result['result'][row][val]))
                                                                    if timestamp==1:
        ...
        if len(host_param)>0:
            ...
            if snmp_get_result['success']==0:
                ...
                if 53 in snmp_get_result["result"]:
                    ...
                    result = {"success":1,"result":"Device is busy.Please Retry again","flag":"1"}      
                else:
...
#print obj.update_reconcilation_controller(34,'odu100','odu100_',True,"anuj")
#print obj.reconcilation_controller(66,'odu100','odu100_',True)
...
class SiteSurvey(object):
        ...
        try:
            ...
            if int(snmp_get_result['success'])==0:
                ...
                if int(snmp_get_result['result'].values()[0]) == 2 or int(snmp_get_result['result'].values()[0]) == 0 or int(snmp_get_result['result'].values()[0]) == 3:
                    ...
                    #print snmp_result
                    if ((snmp_result['success']==1 and 53 in int(snmp_result['result'])) or snmp_result['success']==0):
                        time.sleep(2)
                            ...
                            if int(node_type)==1 or int(node_type)==3:
                                ...
                                if snmp_get_result['success']==1:
                                    if 53 in snmp_get_result['result']:
                                        ...
                                        if 51 in snmp_get_result["result"]:
                                            result =  {"success":1,"result":"Network is unreachable"}
                                            return result
    ...
    def chk_site_survey_status(self,host_id):
        ...
        try:
            ...
            sqlalche_obj.sql_alchemy_db_connection_close()
#obj = SiteSurvey()
#print obj.chk_site_survey_status(79)
    ...
    def admin_state_change(self,host_id,device_type,admin_state_name,state):
        ...
        try:
            ...
            if snmp_result['success']==0:
                ...
                for i in snmp_result["result"]:
                    if snmp_result["result"][i] != 0:
                        snmp_result["result"] = str(host_data[0].host_alias)+" ("+str(host_data[0].ip_address)+") "+str(errorStatus.get(i," Unkmown snmp error, code : "+str(i)))
        except Exception,e:
        ...
        try:
            ...
            for i in range(0,len(host_id_list)):
                ...
                if len(host_data)>0:
                    if host_data[0].device_type_id == UNMPDeviceType.odu16:
                        ...
                        if len(get_default_node_type)>0:
                            if int(get_default_node_type[0].default_node_type) ==1 or int(get_default_node_type[0].default_node_type) ==3:
                                ...
                                if len(master_host_id)>0:
                                    ...
                                    peer_status = sqlalche_obj.session.query(GetOdu16PeerNodeStatusTable.sig_strength).\
                                    filter(and_(GetOdu16PeerNodeStatusTable.peer_mac_addr==master_mac,\
                                    or_(GetOdu16PeerNodeStatusTable.host_id==host_id_list[i],GetOdu16PeerNodeStatusTable.sig_strength==1))).order_by(desc(GetOdu16PeerNodeStatusTable.timestamp)).limit(1).all()
                        ...
                        if int(ru_admin_data[0].defaultNodeType) == 1 or int(ru_admin_data[0].defaultNodeType) == 3:
                            ...
                            if len(master_host_id)>0:
                                ...
                                master_mac = str(host_asset_data[0].ra_mac if len(host_asset_data)>0 else "")+","
                                peer_status = sqlalche_obj.session.query(Odu100PeerNodeStatusTable.sigStrength1).\
                                filter(and_(Odu100PeerNodeStatusTable.host_id==host_id_list[i],or_(Odu100PeerNodeStatusTable.peerMacAddr==master_mac,Odu100PeerNodeStatusTable.sigStrength1==1))).order_by(desc(Odu100PeerNodeStatusTable.timestamp)).limit(1).all()
                    ...
                    if host_data[0].device_type_id == UNMPDeviceType.odu16:
                        if len(peer_status)>0:
                            if int(peer_status[0].sig_strength)==1:
                                temp_list.append(str(master_host_data[0].host_alias)+" ( Link Disconnected )")
                            elif peer_status[0].sig_strength==None:
...
def get_modulation_rate(host_id):
    ...
    host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id == host_id).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
    modulation_rate_result = pysnmp_get_table_ap('1.3.6.1.4.1.26149.2.2.13.10.1',host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community)
    if modulation_rate_result['success']==1:
        for i in modulation_rate_result['result']:
            modulation_rate_result['result'] = errorStatus.get(i,"UNMP Server Encountered an error")
    return modulation_rate_result

def refresh_channel_freq_list(host_id,device_type):
    global sqlalche_obj
    global errorStatus
    result = {'success':0,'result':{}}
    try:
        ...
        if len(host_data)>0:
            ...
            while(i<5):
                time.sleep(5)
                snmp_get_result = pysnmp_get_table_ap('1.3.6.1.4.1.26149.2.2.13.3.1',host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community)
                if snmp_get_result['success']==0:
                    ...
                    if 53 in snmp_get_result['result']:
                        ...
                        result[result]=errorStatus.get(53)
                        result['success']=1
                    else:
                        for i in snmp_get_result['result']:
                            if snmp_get_result[i]!=0:
                                snmp_result['result'] = errorStatus.get(snmp_get_result[i],"SNMP unknown Error,error code %s"%(snmp_get_result[i]))
                        result['success']=1
    ...
    try:
        ...
        if len(device_param_list)>0:
            ...
            if snmp_get_result['success']==0:
                        ...
                        if int(snmp_set_result['success'])==0:
                            for i in snmp_set_result['result']:
                                if snmp_set_result['result'][i]!=0:
                                    result = {'success':1,'result':errorStatus.get(i,"SNMP Unknown error,error code %s"%(i))}
                                    return
                                else:
                                    continue
                ...
                #print "######################################"
                for i in snmp_get_result['result']:
                    result['result'] = errorStatus.get(i,"Unknown SNMP Error,error Code,%s"%(i))
                    result['success']=1
                    return
