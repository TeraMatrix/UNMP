...
from py_module import pysnmp_setAcl,pysnmp_seter,pysnmp_geter,snmp_ping,pysnmp_reconcile,pysnmp_acl_reconcile,pysnmp_peer_reconcile,pysnmp_get_table,pysnmp_set1
...
oid_type={'RU.OMCConfTable.omcIPAddress':'a','RU.OMCConfTable.periodicStatisticsTimer':'u',\
...
###############################################################################
host_status_dic = {0:'No operation', 1:'Firmware download', 2:'Firmware upgrade', 3:'Restore default config', 4:'Flash commit',\
...
def omc_conf_table_get(host_id):
    ...
    sqlalche_obj.sql_alchemy_db_connection_open()
    #session=session_db()
    odu16_profile_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).all() 
    odu16_omc_conf_table=sqlalche_obj.session.query(SetOdu16OmcConfTable).filter(SetOdu16OmcConfTable.config_profile_id==odu16_profile_id[0][0]).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
...
def omc_conf_set(host_id,omc_fields,omc_config,user_name):
    ...
    odu16_omc_conf_table= sqlalche_obj.session.query(SetOdu16OmcConfTable).filter(SetOdu16OmcConfTable.config_profile_id==device_param_list[0][4]).all()
    ...
    for i in range(len(omc_fields)):
        ...
        result+=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],omc_oid,omc_type,omc_type_val)
    err=error_odu16(result,param,err1)
    try:
        el = EventLog()
        #el.log_event( "description detail" , "user_name" )
        if 1 in err1:
            el.log_event("Values Updated in UBR UNMP Form" ,"%s"%(user_name))
        if int(err1[0])==1:
...
def ru_config_table_get(host_id):
    ...
    sqlalche_obj.sql_alchemy_db_connection_open()
    odu16_profile_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).all() 
    ru_config_get= sqlalche_obj.session.query(SetOdu16RUConfTable.channel_bandwidth,SetOdu16RUConfTable.sysnch_source,SetOdu16RUConfTable.country_code).filter(SetOdu16RUConfTable.config_profile_id==odu16_profile_id[0][0]).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
...
def ru_config_table_set(host_id,ru_config_fields,ru_config_param,user_name):
    ...
    device_param_list = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_write_community,Hosts.ip_address,Hosts.snmp_port,Hosts.config_profile_id).\
        filter(Hosts.host_id == host_id).all() 
    ru_config_set=sqlalche_obj.session.query(SetOdu16RUConfTable).filter(SetOdu16RUConfTable.config_profile_id==device_param_list[0][4]).first()
    admin_state=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.1.1.2.1','i','0')
    var=admin_state.find('adminstate.1')

    if var!=-1:
        ru_config_set.adminstate=0
        result+=snmp_setmultiple(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],oid_name[ru_config_fields[0]],oid_type[ru_config_fields[0]],ru_config_param[0],oid_name[ru_config_fields[1]],oid_type[ru_config_fields[1]],ru_config_param[1])
        err=error_odu16(result,param,err1)
        try:
            el = EventLog()
            if 1 in err1:
                el.log_event("Values Updated in UBR Radio Unit Form" ,"%s"%(user_name))
            ...
            admin_state = snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.1.1.2.1','i','1')
            var=admin_state.find('adminstate.1')
            if var!=-1:
...
def ru_date_time_table_get(host_id):
    ...
    sqlalche_obj.sql_alchemy_db_connection_open()
    odu16_profile_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).all() 
    ru_date_time=sqlalche_obj.session.query(SetOdu16RUDateTimeTable.year,SetOdu16RUDateTimeTable.month,SetOdu16RUDateTimeTable.day,SetOdu16RUDateTimeTable.hour,SetOdu16RUDateTimeTable.min,SetOdu16RUDateTimeTable.sec).filter(SetOdu16RUDateTimeTable.config_profile_id==odu16_profile_id[0][0]).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
...
def ru_date_time_table_set(host_id,date_time_fields,date_time_param,user_name):
    ...
    device_param_list = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_write_community,Hosts.ip_address,Hosts.snmp_port,Hosts.config_profile_id).\
        filter(Hosts.host_id == host_id).all() 
    odu16_date_time_table= sqlalche_obj.session.query(SetOdu16RUDateTimeTable).filter(SetOdu16RUDateTimeTable.config_profile_id==device_param_list[0][4]).all()
    for i in range(len(date_time_fields)):
        ...
        result+=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],oidname,oidtype,oidvalue)
    err=error_odu16(result,param,err1)
    try:
        el = EventLog()
        if 1 in err1:
            el.log_event("Values Updated in UBR RU Date Time Form" ,"%s"%(user_name))
...
def network_interface_config_get():
    ...
    sqlalche_obj.sql_alchemy_db_connection_open()
    odu16_profile_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).all() 
    network_interface_config= sqlalche_obj.session.query(SetOdu16NetworkInterfaceConfig.ssid,SetOdu16NetworkInterfaceConfig.index).filter(SetOdu16NetworkInterfaceConfig.config_profile_id==odu16_profile_id[0][0]).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
...
def network_interface_config_set(host_id,network_interface_config_fields,network_interface_config_param,user_name):
    ...
    device_param_list = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_write_community,Hosts.ip_address,Hosts.snmp_port,Hosts.config_profile_id).\
        filter(Hosts.host_id == host_id).all() 
    network_interface_config_table=sqlalche_obj.session.query(SetOdu16NetworkInterfaceConfig).filter(SetOdu16NetworkInterfaceConfig.config_profile_id==device_param_list[0][4]).all()
    ...
    try:
        el = EventLog()
        if 1 in err1:
            el.log_event("Values Updated in UBR Netwrok Interface Form" ,"%s"%(user_name))
        if err!='':
            raise Set_exception
        else:
            for i in range(len(network_interface_config_param)):
                network_interface_config_table[i].ssid=network_interface_config_param[i]
...
def ra_acl_config_table_get(host_id):
    ...
    sqlalche_obj.sql_alchemy_db_connection_open()
    odu16_profile_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).all() 
    ra_conf_table = sqlalche_obj.session.query(SetOdu16RAConfTable.acl_mode).filter(SetOdu16RAConfTable.config_profile_id==odu16_profile_id[0][0]).all()
    ra_acl_config=sqlalche_obj.session.query(SetOdu16RAAclConfigTable.mac_address,SetOdu16RAAclConfigTable.index).filter(SetOdu16RAAclConfigTable.config_profile_id==odu16_profile_id[0][0]).order_by(SetOdu16RAAclConfigTable.index).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
...
def sync_config_table_get(host_id):
    ...
    sqlalche_obj.sql_alchemy_db_connection_open()
    odu16_profile_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).all() 
    sync_config_table=sqlalche_obj.session.query(SetOdu16SyncConfigTable.raster_time ,SetOdu16SyncConfigTable.num_slaves,SetOdu16SyncConfigTable.sync_loss_threshold,SetOdu16SyncConfigTable.leaky_bucket_timer,SetOdu16SyncConfigTable.sync_lost_timeout,SetOdu16SyncConfigTable.sync_config_time_adjust,SetOdu16SyncConfigTable.sync_config_broadcast_enable).filter(SetOdu16SyncConfigTable.config_profile_id==odu16_profile_id[0][0]).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
...
def ra_llc_conf_table_get(host_id):
    ...
    sqlalche_obj.sql_alchemy_db_connection_open()
    odu16_profile_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).all() 
    ra_llc_conf_table = sqlalche_obj.session.query(SetOdu16RALlcConfTable.llc_arq_enable ,SetOdu16RALlcConfTable.arq_win,SetOdu16RALlcConfTable.frame_loss_threshold,SetOdu16RALlcConfTable.leaky_bucket_timer_val,SetOdu16RALlcConfTable.frame_loss_timeout).filter(SetOdu16RALlcConfTable.config_profile_id==odu16_profile_id[0][0]).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
...
def ra_llc_configuration_set(host_id,llc_configuration_fields,llc_configuration_param,user_name):
    ...
    result=''
    err1=[0,0,0,0,0]
    form_name=['ARQ Mode','ArqWin(Retransmit Window Size)','Frame Loss Threshold','Leaky Bucket Timer','Frame Loss Time Out']
    param=[]
    dictarr=[]
    resultarray={}
    ...
    device_param_list = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_write_community,Hosts.ip_address,Hosts.snmp_port,Hosts.config_profile_id).\
        filter(Hosts.host_id == host_id).all() 
    ra_llc_config = sqlalche_obj.session.query(SetOdu16RALlcConfTable).filter(SetOdu16RALlcConfTable.config_profile_id==device_param_list[0][4]).first()
...
def ra_tdd_mac_config_get(host_id):
    ...
    sqlalche_obj.sql_alchemy_db_connection_open()
    odu16_profile_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).all() 
    ra_tdd_mac_conf_table = sqlalche_obj.session.query(SetOdu16RATddMacConfig.rf_channel_frequency,SetOdu16RATddMacConfig.pass_phrase,\
        ...
        filter(SetOdu16RATddMacConfig.config_profile_id==odu16_profile_id[0][0]).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
...
def ra_tdd_mac_config_set(host_id,tdd_mac_fields,tdd_mac_param,user_name):
    ...
    dictarr=[]
    resultarray={}
    form_name=['TDD1 MAC RF Frequency','Pass Phrase','TDD 1 MAC RF Coding','TDD 1 MAC TX Power','Max Crc Errors','Leaky Bucket Timer']
    err1=[0,0,0,0,0,0]
    ...
    device_param_list = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_write_community,Hosts.ip_address,Hosts.snmp_port,Hosts.config_profile_id).\
        filter(Hosts.host_id == host_id).all() 
    ra_tdd_mac_configtable = sqlalche_obj.session.query(SetOdu16RATddMacConfig).filter(SetOdu16RATddMacConfig.config_profile_id==device_param_list[0][4]).first()
    ra_config = sqlalche_obj.session.query(SetOdu16RAConfTable).filter(SetOdu16RAConfTable.config_profile_id==device_param_list[0][4]).first()
    admin_state=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.13.1.1.2.1','i','0')
    ...
    if var!=-1:
        ...
        try:
            el = EventLog()
            if 1 in err1:
                el.log_event("Values Updated in UBR Radio Frequency Form" ,"%s"%(user_name))
            ...
            if err1[5]==1:
                ra_tdd_mac_configtable.leaky_bucket_timer_value=tdd_mac_param[5]
            sqlalche_obj.session.commit()
            if err!='':
                admin_state = snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.13.1.1.2.1','i','1')
                var=admin_state.find('raAdminState.1')
                if var!=-1:
...
def peer_config_table_get(host_id):
    ...
    peer_config_table = sqlalche_obj.session.query(SetOdu16PeerConfigTable.peer_mac_address ,SetOdu16PeerConfigTable.index).\
        filter(SetOdu16PeerConfigTable.config_profile_id==odu16_profile_id[0][0]).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
...
def ra_conf_table_get(host_id):
    ...
    ra_conf_table = sqlalche_obj.session.query(SetOdu16RAConfTable.acl_mode).\
        filter(SetOdu16RAConfTable.config_profile_id==odu16_profile_id[0][0]).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
...
def sys_omc_registration_table_get(host_id):
    ...
    sys_omc_registration_table = sqlalche_obj.session.query(SetOdu16SysOmcRegistrationTable).\
        filter(SetOdu16SysOmcRegistrationTable.config_profile_id==odu16_profile_id[0][0]).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
...
def omc_registration_configuration_set(host_id,omc_registration_fields,omc_registration_param,user_name):
    ...
    err1=[0,0,0,0,0]
    resultarray={}
    form_name=['Address','Contact Person','Mobile','AlternateContact','Email']
    ...
    device_param_list = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_write_community,Hosts.ip_address,Hosts.snmp_port,Hosts.config_profile_id).\
        filter(Hosts.host_id == host_id).all() 
    omc_registration_configuration = sqlalche_obj.session.query(SetOdu16SysOmcRegistrationTable).filter(SetOdu16SysOmcRegistrationTable.config_profile_id==device_param_list[0][4]).first()
    ...
    try:
        el = EventLog()
        if 1 in err1:
            el.log_event("Values Updated in UBR OMC Registration Form" ,"%s"%(user_name))
        for j in range(0,len(omc_registration_fields)):
            ...
            dictarr.append(dict)
        if err1[0]==1:
            omc_registration_configuration.sys_omc_register_contact_addr = omc_registration_param[0]
        if err1[1]==1:
        ...
        if err1[4]==1:
            omc_registration_configuration.sys_omc_register_contact_email  = omc_registration_param[4]
        sqlalche_obj.session.commit()
        sqlalche_obj.sql_alchemy_db_connection_close()
        if err!='':
            raise Set_exception
...
def syn_configuration_set(host_id,syn_configuration_fields,syn_configuration_param,user_name):
    ...
    dictarr=[]
    resultarray={}
    form_name=['Raster Time','Number of Slaves','SyncLossThreshold','Leaky Bucket Timer','Sync Loss Time Out','Sync Timer Adjust']
    err1=[0,0,0,0,0,0]
    ...
    device_param_list = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_write_community,Hosts.ip_address,Hosts.snmp_port,Hosts.config_profile_id).\
        filter(Hosts.host_id == host_id).all() 
    syn_configuration = sqlalche_obj.session.query(SetOdu16SyncConfigTable).filter(SetOdu16SyncConfigTable.config_profile_id==device_param_list[0][4]).first()
    admin_state=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.11.1.1.2.1','i','0')
    ...
    if var!=-1:
        ...
        try:
            el = EventLog()
            if 1 in err1:
                el.log_event("Values Updated in UBR Synchronization Registration Form" ,"%s"%(user_name))
            ...
            if err1[2]==1:
                syn_configuration.sync_loss_threshold=syn_configuration_param[2]
            if err1[3]==1:
                syn_configuration.leaky_bucket_timer=syn_configuration_param[3]
            if err1[4]==1:
                syn_configuration.sync_lost_timeout=syn_configuration_param[4]
            if err1[5]==1:
                syn_configuration.sync_config_time_adjust=syn_configuration_param[5]

            admin_state = snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.11.1.1.2.1','i','1')
            var=admin_state.find('adminStatus.1')
            if var!=-1:
            ...
            resultarray['formAction']='Syn_Cancel_Omc_Registration_Configuration.py'
...
def ra_config_set(host_id,ra_config_fields,ra_config_param,user_name):
    ...
    device_param_list = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_write_community,Hosts.ip_address,Hosts.snmp_port,Hosts.config_profile_id).\
        filter(Hosts.host_id == host_id).all() 
    ra_config = sqlalche_obj.session.query(SetOdu16RAConfTable).filter(SetOdu16RAConfTable.config_profile_id==device_param_list[0][4]).first()
    snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.13.1.1.2.1','i','0')
    ...
    sqlalche_obj.session.commit()
    snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.13.1.1.2.1','i','1')
    sqlalche_obj.sql_alchemy_db_connection_close()
    return result
...
def peer_config_set(host_id,status,peer_config_fields,peer_config_param,time_slot_param,user_name):
    ...
    if status==1:
        ...
        device_param_list = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_write_community,Hosts.ip_address,Hosts.snmp_port,Hosts.config_profile_id).\
            filter(Hosts.host_id == host_id).all() 
        syn_configuration = sqlalche_obj.session.query(SetOdu16SyncConfigTable).filter(SetOdu16SyncConfigTable.config_profile_id==device_param_list[0][4]).all()
        admin_state=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.11.1.1.2.1','i','0')
        var=admin_state.find('adminStatus.1')
        if int(var)!=-1:        
            numslaves=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.11.1.1.5.1','i',time_slot_param)
            val=numslaves.find('numSlaves.1')
            if int(val)!=-1:
                syn_configuration[0].num_slaves=time_slot_param
                sqlalche_obj.session.commit()
                peer_config_table=sqlalche_obj.session.query(SetOdu16PeerConfigTable).filter(SetOdu16PeerConfigTable.config_profile_id==device_param_list[0][4]).all()
                for i in range(0,len(peer_config_fields)):
                ...
                try:
                    el = EventLog()
                    if 1 in err1:
                        el.log_event("Values Updated in UBR Peer MAC Form" ,"%s"%(user_name))
                    ...
                    for i in range(0,len(peer_config_param)):
                        if err1[i]==1:
                            peer_config_table[i].peer_mac_address=peer_config_param[i]
...
def peer_slaves(host_id,peermac,syn_configuration_fields,syn_configuration_param):
    ...
    if var!=-1:
        for i in range(int(syn_configuration_param[1]),peermac):
            ...
            count.append(i)
            peer_config_table[i].index=i+1
            ss=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.13.9.1.1.2.1.%s'%(i+1),'s',"")
            count.append(ss)
        sqlalche_obj.session.commit()
        for i in range(len(syn_configuration_fields)):
            oidname=oid_name[syn_configuration_fields[i]]
        ...
        try:
            ...
            sqlalche_obj.sql_alchemy_db_connection_close()
            if err!='':
                snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.11.1.1.2.1','i','1')
                raise Set_exception
        except Set_exception as e:
            resultarray["result"]=dictarr
            resultarray["tableName"]= 'SetOdu16PeerConfigTable'
...
def acl_config_set(host_id,acl_field,acl_param,acl_config_fields,acl_config_param,user_name):
    ...
    device_param_list = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_write_community,Hosts.ip_address,Hosts.snmp_port,Hosts.config_profile_id,Hosts.snmp_read_community).\
        filter(Hosts.host_id == host_id).all() 
    acl_table = sqlalche_obj.session.query(SetOdu16RAConfTable).filter(SetOdu16RAConfTable.config_profile_id==device_param_list[0][4]).all()
    ra_acl_config = sqlalche_obj.session.query(SetOdu16RAAclConfigTable).filter(SetOdu16RAAclConfigTable.config_profile_id==device_param_list[0][4]).order_by(SetOdu16RAAclConfigTable.index).all()
    acl_oid=oid_name[acl_field]
    acl_oid_type=oid_type[acl_field]
    result+=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],acl_oid,acl_oid_type,acl_param)
    ...
    result += snmp_get(device_param_list[0][0],device_param_list[0][5],device_param_list[0][2],device_param_list[0][3],name_get) 
    ...
    if int(check_result)==-1:
        ...
        for i in range(0,count):
            if i<10:
                ...
                result+=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],name,type,"%s "%(oidvalue))
            elif i>=10:
                ...
                if len(ra_acl_config)>len(acl_config_fields):
                    for j in range(len(ra_acl_config),len(acl_config_fields),-1):
                        ...
                        type=oidtype.replace('#','s')
                        result+=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.13.5.1.3.1.%s'%(j),'i','6')                        
                    ...
                    for k in range(len(ra_acl_config),count):
                        ...
                        if acl_config_param[k]=="":
                            ...
                            oidvalue=acl_config_param[k]
                        result+=snmp_setmultiple(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.13.5.1.3.1.%s'%(k+1),'i','4',name,type,"%s "%(oidvalue))
    ...
    try:
        ...
        for j in range(0,len(acl_config_fields)):
            ...
            dict["status"]=err1[j]
            dictarr.append(dict)
        del_acl_config = sqlalche_obj.session.query(SetOdu16RAAclConfigTable).filter(between(SetOdu16RAAclConfigTable.index,11,int(len(ra_acl_config)))).all()
        if del_acl_config==[]:
            val=1
        else:
        ...
        for i in range(0,len(acl_config_fields)):
            ...
            if i>=10:
                if err1[i]==1:
                    sqlalche_obj.session.add(SetOdu16RAAclConfigTable('%s'%(device_param_list[0][4]),'%s'%(acl_config_param[i]),'%s'%(i+1)))                    
        sqlalche_obj.session.commit()
        if err!='':
            raise Set_exception
    except Set_exception as e:
...
def retry_set_one(host_id,table_name,textbox,textbox_value,textbox_field,index_value,user_name):
    ...
    device_param_list = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_write_community,Hosts.ip_address,Hosts.snmp_port,Hosts.config_profile_id).filter(Hosts.host_id == host_id).all() 
    if textbox=='RU.SyncClock.SyncConfigTable.rasterTime' or textbox=='RU.SyncClock.SyncConfigTable.numSlaves' or textbox=='RU.SyncClock.SyncConfigTable.timerAdjust':
        ...
        if admin_state=="":
            ...
            result=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],oidname,oidtype,textbox_value)
            if result=="":
                result="1"
            else:
                el = EventLog()
                el.log_event("Values Updated in UBR Synchronization Form in Retry Mode" ,"%s"%(user_name))
                snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.11.1.1.2.1','i','1')
                str_table_obj = "table_result=sqlalche_obj.session.query(%s).filter(%s.config_profile_id=='%s').all()" % (table_name,table_name,device_param_list[0][4])
                exec str_table_obj
                str_table_obj = "table_result[0]." + textbox_field + " = '"+ textbox_value +"'"
                exec str_table_obj
                sqlalche_obj.session.commit()
                result="0"
        sqlalche_obj.sql_alchemy_db_connection_close()
        ...
        if admin_state=="":
            ...
            el = EventLog()
            el.log_event("Values Updated in UBR Radio Frequency Form in Retry Mode" ,"%s"%(user_name))
            result=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],oidname,oidtype,textbox_value)
            if result=="":
                result="1"
            else:
        ...
        type=oidtype.replace('#','s')
        result=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],name,type,"%s "%(textbox_value))
        if result=="":
            result="1"
        else:
            el = EventLog()
            el.log_event("Values Updated in UBR ACL Form in Retry Mode" ,"%s"%(user_name))
            str_table_obj = "table_result=sqlalche_obj.session.query(%s).filter(%s.config_profile_id=='%s').all()" % (table_name,table_name,device_param_list[0][4])
            exec str_table_obj
            str_table_obj = "table_result["+str(int(index_value)-1)+"]." + textbox_field + " = '"+ textbox_value +"'"
            exec str_table_obj
            str_table_obj= "table_result["+str(int(index_value)-1)+"].index" + " = '"+ index_value +"'"
            sqlalche_obj.session.commit()
            result="0"
        sqlalche_obj.sql_alchemy_db_connection_close()
        return result
        ...
        if result=="":
            ...
            el = EventLog()
            el.log_event("Values Updated in UBR Peer MAC Form in Retry Mode" ,"%s"%(user_name))
            str_table_obj = "table_result=sqlalche_obj.session.query(%s).filter(%s.config_profile_id=='%s').all()" % (table_name,table_name,device_param_list[0][4])
            exec str_table_obj
            str_table_obj = "table_result["+str(int(index_value)-1)+"]." + textbox_field + " = '"+ textbox_value +"'"
            exec str_table_obj
        ...
        if admin_state=="":
            ...
            el = EventLog()
            el.log_event("Values Updated in UBR Radio Unit Form in Retry Mode" ,"%s"%(user_name))
            result=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],oidname,oidtype,textbox_value)
            if result=="":
                result="1"
            else:
                snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.1.1.2.1','i','1')
                str_table_obj = "table_result=sqlalche_obj.session.query(%s).filter(%s.config_profile_id=='%s').all()" % (table_name,table_name,device_param_list[0][4])
                exec str_table_obj
                str_table_obj = "table_result[0]." + textbox_field + " = '"+ textbox_value +"'"
                exec str_table_obj
        ...
        if result=="":
            ...
            el = EventLog()
            el.log_event("Values Updated in UBR Form in Retry Mode","%s"%(user_name))
            str_table_obj = "table_result=sqlalche_obj.session.query(%s).filter(%s.config_profile_id=='%s').all()" % (table_name,table_name,device_param_list[0][4])
...
def commit_to_flash(host_id,field,user_name):
    try:
        ...
        result_commit=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],oidname,'i','4') 
        time.sleep(5)
        if (result_commit=='' or result_commit=='None'):
            return 'Device is not connected or Device is Rebooting.Please Retry Again.'
        else:
            el = EventLog()
            el.log_event("Device Data is Commited To Flash" ,"%s"%(user_name))
    ...
    if 'SetOdu16SyncConfigTable' in table_name_list:
        ...
        if admin_state=='':
            ...
            for i in range(len(textbox_list)):
                ...
                result=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],oidname,oidtype,textbox_value_list[i])
                if result=="":
                    dic[textbox_list[i]] = 1
                    snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.11.1.1.2.1','i','1')
                else:
                    el = EventLog()
                    el.log_event("Values Updated in UBR Synchronization Form in Retry All Mode" ,"%s"%(user_name))
                    dic[textbox_list[i]]= 0
                    str_table_obj="table_result=sqlalche_obj.session.query(%s).filter(%s.config_profile_id=='%s').all()" %(table_name_list[i],table_name_list[i],device_param_list[0][4])
                    exec str_table_obj
                    str_table_obj= "table_result[0]." + textbox_field_list[i] + " = '"+ textbox_value_list[i] +"'"
                    exec str_table_obj
                    sqlalche_obj.session.commit();
                    snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.11.1.1.2.1','i','1')
        sqlalche_obj.sql_alchemy_db_connection_close()
        ...
        if admin_state=='':
            ...
            for i in range(len(textbox_list)):
                ...
                if result=="":
                    ...
                    el = EventLog()
                    el.log_event("Values Updated in UBR Radio Frequency Form in Retry All Mode" ,"%s"%(user_name))
                    dic[textbox_list[i]]= 0
                    str_table_obj="table_result=sqlalche_obj.session.query(%s).filter(%s.config_profile_id=='%s').all()" %(table_name_list[i],table_name_list[i],device_param_list[0][4])
                    exec str_table_obj
                    str_table_obj= "table_result[0]." + textbox_field_list[i] + " = '"+ textbox_value_list[i] +"'"
        ...
        if admin_state=='':
            ...
            result=snmp_setmultiple(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],oid_multi_name[0],oid_multi_type[0],textbox_value_list[0],oid_multi_name[1],oid_multi_type[1],textbox_value_list[1])
            for i in range(len(textbox_list)):
                if result=="":
                    dic[textbox_list[i]] = 1
                    snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.1.1.2.1','i','1')
                    ...
                    el = EventLog()
                    el.log_event("Values Updated in UBR Radio Unit Form in Retry All Mode" ,"%s"%(user_name))
                    dic[textbox_list[i]]= 0
                    str_table_obj="table_result=sqlalche_obj.session.query(%s).filter(%s.config_profile_id=='%s').all()" %(table_name_list[i],table_name_list[i],device_param_list[0][4])
                    exec str_table_obj
                    str_table_obj= "table_result[0]." + textbox_field_list[i] + " = '"+ textbox_value_list[i] +"'"
        ...
        for i in range(0,len(textbox_list)):
            ...
            if result == ""  or result == None:
                ...
                el = EventLog()
                el.log_event("Values Updated in UBR Peer MAC Form in Retry All Mode" ,"%s"%(user_name))
                dic[textbox_list[i]]= 0
                str_table_obj="table_result=sqlalche_obj.session.query(%s).filter(%s.config_profile_id=='%s').order_by(%s.index).all()" %(table_name_list[i],table_name_list[i],device_param_list[0][4],table_name_list[i])
                exec str_table_obj
                str_table_obj= "table_result["+str(int(index_value_list[i])-1)+"]." + textbox_field_list[i] + " = '"+ textbox_value_list[i] +"'"
                exec str_table_obj
                str_table_obj="table_result["+str(int(index_value_list[i])-1)+"].index"+ " = "+ str(index_value_list[i])
                exec str_table_obj
            sqlalche_obj.session.commit()
        sqlalche_obj.sql_alchemy_db_connection_close()
        return str(dic)
        ...
        for i in range(0,len(textbox_list)):
            ...
            if textbox_list[i]=='RU.RA.1.RAConfTable.aclMode':
                ...
                if result=="":
                    ...
                    el = EventLog()
                    el.log_event("Values Updated in UBR RAConf Form in Retry All Mode" ,"%s"%(user_name))
                    dic[textbox_list[i]]= 0
                    str_table_obj="table_result=sqlalche_obj.session.query(%s).filter(%s.config_profile_id=='%s').all()" %(table_name_list[i],table_name_list[i],device_param_list[0][4])
                    exec str_table_obj
                    str_table_obj= "table_result[0]." + textbox_field_list[i] + " = '"+ textbox_value_list[i] +"'"
                ...
                type=oidtype.replace('#','s')
                result=snmp_set(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],name,type,"%s "%(textbox_value_list[i]))
                if result=="":
                    dic[textbox_list[i]]=1
                else:
                    el = EventLog()
        ...
        for i in range(len(textbox_list)):
            ...
            if result=="":
                ...
                el = EventLog()
                el.log_event("Values Updated in UBR Form in Retry All Mode","%s"%(user_name))
                dic[textbox_list[i]]= 0
                str_table_obj="table_result=sqlalche_obj.session.query(%s).filter(%s.config_profile_id=='%s').all()" %(table_name_list[i],table_name_list[i],device_param_list[0][4])
                exec str_table_obj
                str_table_obj= "table_result[0]." + textbox_field_list[i] + " = '"+ textbox_value_list[i] +"'"
...
#        .order_by(Hosts.host_alias).order_by(Hosts.ip_address).all()
        ...
        device_dict = data_table_data_sqlalchemy(ip_address,mac_address,selected_device,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,userid,html_var)
        #return device_dict
    ...
    #[36, "172.22.0.111", "Default", "172.22.0.111", "FF:FF:FF:FF:FF:FF", " ", "odu16", 0, 304]
        ...
        for i in range(0,len(device_tuple)):
            if device_tuple[i][6]=="odu16":
                master_slave_select = sqlalche_obj.session.query(GetOdu16_ru_conf_table.default_node_type).filter(GetOdu16_ru_conf_table.host_id==device_tuple[i][0]).all()
                if len(master_slave_select)>0:
                    if int(master_slave_select[0][0]) == 0 or int(master_slave_select[0][0])==2:
                        ...
                        master_host_id = sqlalche_obj.session.query(MasterSlaveLinking.master,Hosts.host_alias,Hosts.host_asset_id).\
                            outerjoin(Hosts,MasterSlaveLinking.slave == Hosts.host_id).filter(MasterSlaveLinking.slave == device_tuple[i][0]).all()

                        #master_host_id = sqlalche_obj.session.query(MasterSlaveLinking.master).filter(MasterSlaveLinking.slave == device_tuple[i][0]).all()
                        if len(master_host_id)>0:
                            #host_data = sqlalche_obj.session.query(Hosts.host_alias,Hosts.host_asset_id).filter(and_(Hosts.host_id==master_host_id[0][0],Hosts.is_deleted==0)).all()
...
##                            master_mac = str(host_asset_data[0].ra_mac if len(host_asset_data)>0 else "")
                            ...
                            peer_status = sqlalche_obj.session.query(GetOdu16PeerNodeStatusTable.sig_strength,GetOdu16PeerNodeStatusTable.link_status).\
                                ...
                                order_by(desc(GetOdu16PeerNodeStatusTable.get_odu16_peer_node_status_table_id)).limit(1).all()
                            if len(peer_status)>0:
                                if peer_status[0].sig_strength==None:
                                    ...
                                    slave_data = str(host_alias)+" (Device Unreachable)"

                                else:
                                    if peer_status[0].link_status==1:
                                        slave_data = str(host_alias)+"( Link Disconnected )"
                                    else:
                                        slave_data = str(host_alias)+" ("+str(peer_status[0].sig_strength)+"dBm)"
                            else:
                ...
                ru_data = sqlalche_obj.session.query(Odu100RuConfTable.adminstate,Odu100RaConfTable.raAdminState,Odu100SyncConfigTable.adminStatus).\
                    ...
                    filter(Odu100RuConfTable.config_profile_id==device_tuple[i][8]).all()
...
#                sync_data = sqlalche_obj.session.query(SetOdu16SyncConfigTable.adminStatus).filter(SetOdu16SyncConfigTable.config_profile_id==device_tuple[i][8]).all()
                ...
                master_slave_select = sqlalche_obj.session.query(Odu100RuConfTable.defaultNodeType).filter(Odu100RuConfTable.config_profile_id==device_tuple[i][8]).all()
                if len(master_slave_select)>0:
                    if master_slave_select[0][0] == 0 or master_slave_select[0][0] == 2:
                        ...
                        if len(master_host_id)>0:
                            #host_data = sqlalche_obj.session.query(Hosts.host_alias,Hosts.host_asset_id).filter(Hosts.host_id==master_host_id[0][0]).all()
                            host_alias = master_host_id[0].host_alias
                        ...
                        if len(peer_status)>0:
                            if peer_status[0].sigStrength1==None:
                                ...
                                if int(peer_status[0].linkStatus)==1:
                                    ...
                                    slave_data = str(host_alias)+" ("+str(peer_status[0].sigStrength1)+"dBm)"
                        else:
                            if host_alias != "" and host_alias != None:
                                slave_data = str(host_alias)+"(-)"
##                    else:
...
#                sync_data = sqlalche_obj.session.query(Odu100SyncConfigTable.adminStatus).filter(Odu100SyncConfigTable.config_profile_id==device_tuple[i][8]).all()

            #host_count = session.query(Hosts).filter(and_(or_(Hosts.host_alias == host_alias,Hosts.ip_address == ip_address, Hosts.mac_address == mac_address),Hosts.is_deleted == 0)).count()


                ru_status = sqlalche_obj.session.query(Odu100RuStatusTable.ruoperationalState).filter(Odu100RuStatusTable.host_id==device_tuple[i][0]).all()
                ra_status = sqlalche_obj.session.query(Odu100RaStatusTable.raoperationalState).filter(Odu100RaStatusTable.host_id==device_tuple[i][0]).order_by(desc(Odu100RaStatusTable.timestamp)).all()
                sync_status = sqlalche_obj.session.query(Odu100SynchStatusTable.syncoperationalState).filter(Odu100SynchStatusTable.host_id==device_tuple[i][0]).order_by(desc(Odu100SynchStatusTable.timestamp)).all()
...
##                        'sync_admin':1 if sync_data[0][0]==None else int(sync_data[0][0]) if len(sync_data)>0 else 1}
            ...
            snmp_up_time_data = sqlalche_obj.db.execute("select trap_event_id,timestamp from system_alarm_table where host_id='%s' order by timestamp desc limit 1"%(device_tuple[i][0]))
            snmp_up_down_time=""
...
#                device_status_image_path = "images/temp/green_dot.png"
            ...
            device_list.append(["<center><img id=\"device_status\" name=\"device_status\" src=\"%s\" title=\"%s\" style=\"width:8px;height:8px;\" class=\"imgbutton w-reconcile\" original-title=\"%s\" /></center>&nbsp;&nbsp;"%(device_status_image_path,device_status,device_status)\
                                ,device_tuple[i][1],device_tuple[i][2],device_tuple[i][3],device_tuple[i][4],"-" if device_tuple[i][5]==" " else device_tuple[i][5],master_slave,slave_data,\
                ...
                <li>\
                    <a class=\"%s n-reconcile\" id=\"ru.ra.raConfTable.raAdminState\" name=\"ru.ra.raConfTable.raAdminState\" title=\"%s\" state=\"%s\" onclick=\"adminStateCheck(event,this,'%s','%s','ru.ra.raConfTable.raAdminState')\">RA</a>\
                                ...
                                %("red" if ru_state==0 else "green",image_ru_title,ru_state,device_tuple[i][0],device_tuple[i][6],\
            ...
            %s"
                                %(device_tuple[i][0],device_tuple[i][6],device_list_state,\
                                  device_tuple[i][0],'sp_dashboard_profiling.py' if device_tuple[i][6] == "odu100" else 'sp_dashboard_profiling.py',\
                                  device_tuple[i][0],device_tuple[i][6],device_list_state,device_tuple[i][3],\
                                  device_tuple[i][0],device_tuple[i][6],device_list_state,images,device_tuple[i][7],device_tuple[i][0],device_tuple[i][6],\
                                  live_monitoring,monitoring_status,device_tuple[i][0],\
...
##                            master_mac = str(host_asset_data[0].ra_mac if len(host_asset_data)>0 else "")
                            peer_status = sqlalche_obj.session.query(GetOdu16PeerNodeStatusTable.sig_strength,GetOdu16PeerNodeStatusTable.link_status).\
                                filter(GetOdu16PeerNodeStatusTable.host_id==device_tuple[i][0]).order_by(desc(GetOdu16PeerNodeStatusTable.timestamp)).limit(1).all()
                            if len(peer_status)>0:
                                if peer_status[0].sig_strength==None:
                                    slave_data = str(host_alias)+" ( )"
                                elif int(peer_status[0].sig_strength)==1111111:
...
##                        slave_data = "(-)"
            ...
            admin_dic = {'ru_admin':1 if ru_data[0][0]==None else int(ru_data[0][0]) if len(ru_data)>0 else 1,\
                         'ra_admin':1 if ra_data[0][0]==None else int(ra_data[0][0]) if len(ra_data)>0 else 1,\
                         'sync_admin':1 if sync_data[0][0]==None else int(sync_data[0][0]) if len(sync_data)>0 else 1}            
            snmp_up_time_data = sqlalche_obj.db.execute("select trap_event_id from system_alarm_table where host_id='%s' order by timestamp desc limit 1"%(device_tuple[i][0]))
            snmp_up_down_time=""
            for row in snmp_up_time_data:
                snmp_up_down_time = row['trap_event_id']
            if snmp_up_down_time=="":
            ...
            device_list.append(["<center><img id=\"device_status\" name=\"device_status\" src=\"%s\" title=\"%s\" style=\"width:8px;height:8px;\" class=\"imgbutton w-reconcile\"/></center>&nbsp;&nbsp;"%(device_status_image_path,device_status)\
                                ,device_tuple[i][1],device_tuple[i][2],device_tuple[i][3],device_tuple[i][4],"-" if device_tuple[i][5]==" " else device_tuple[i][5],master_slave,slave_data,\
...
def get_device_list_odu_profiling(ip_address,mac_address,selected_device): 
    ...
    try:
        ...
        sqlalche_obj.sql_alchemy_db_connection_open()
        #this is the query which returns the multidimensional array of hosts table and store in device_tuple
        device_tuple = sqlalche_obj.session.query(Hosts.host_id,Hosts.host_alias,Hosts.ip_address,Hosts.mac_address).filter(and_(Hosts.is_deleted == 0,Hosts.ip_address.like('%s%%'%(ip_address)),\
...
def odu100_get_ipconfigtable(host_id):
    ...
    try:
        ...
        odu100_profile_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).one()
        if odu100_profile_id == None or odu100_profile_id == []:
            return {"success":1,"result":"","detail":""}
        else: 
            odu100_ip_conf_table = sqlalche_obj.session.query(Odu100IpConfigTable.ipAddress,Odu100IpConfigTable.ipNetworkMask,Odu100IpConfigTable.ipDefaultGateway,Odu100IpConfigTable.autoIpConfig).filter(Odu100IpConfigTable.config_profile_id == odu100_profile_id[0]).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
...
def odu100_get_omcconfigtable(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_omc_conf_table = sqlalche_obj.session.query(Odu100OmcConfTable.omcIpAddress,Odu100OmcConfTable.periodicStatsTimer).filter(Odu100OmcConfTable.config_profile_id == odu100_profile_id[0]).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
...
def odu100_get_peerconfigtable(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_peer_conf_table = sqlalche_obj.session.query(Odu100PeerConfigTable.peerMacAddress ,Odu100PeerConfigTable.guaranteedUplinkBW,Odu100PeerConfigTable.guaranteedDownlinkBW,Odu100PeerConfigTable.basicrateMCSIndex,\
...
def odu100_get_aclconfigtable(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_acl_conf_table = sqlalche_obj.session.query(Odu100RaAclConfigTable.aclIndex,Odu100RaAclConfigTable.macaddress,Odu100RaAclConfigTable.odu100_raAclConfigTable_id,Odu100RaAclConfigTable.raIndex).filter(Odu100RaAclConfigTable.config_profile_id == odu100_profile_id[0]).order_by(Odu100RaAclConfigTable.aclIndex).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
...
def odu100_get_raconfigtable(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_ra_conf_table = sqlalche_obj.session.query(Odu100RaConfTable.numSlaves,Odu100RaConfTable.ssID,Odu100RaConfTable.acm,Odu100RaConfTable.dba,\
                                                              ...
                                                              Odu100RaConfTable.linkDistance,Odu100RaConfTable.anc).filter(Odu100RaConfTable.config_profile_id == odu100_profile_id[0]).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
...
def odu100_get_tddmacconfigtable(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_tddmac_conf_table = sqlalche_obj.session.query(Odu100RaTddMacConfigTable.encryptionType,Odu100RaTddMacConfigTable.passPhrase,Odu100RaTddMacConfigTable.txPower,\
...
def odu100_get_ruconfigtable(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_ru_conf_table = sqlalche_obj.session.query(Odu100RuConfTable.channelBandwidth,Odu100RuConfTable.synchSource,Odu100RuConfTable.countryCode,\
...
def odu100_get_llcconfigtable(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_llc_conf_table = sqlalche_obj.session.query(Odu100RaLlcConfTable).filter(Odu100RaLlcConfTable.config_profile_id == odu100_profile_id[0]).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
...
def odu100_ip_packet_table(host_id):
    ...
    try:
        global sqlalche_obj
        sqlalche_obj.sql_alchemy_db_connection_open()
        odu100_ip_filter_table = sqlalche_obj.session.query(Odu100IpFilterTable).filter(Odu100IpFilterTable.host_id == host_id).all()
...
def totalPacketIpMAC(host_id):
    ...
    try:
        global sqlalche_obj
        sqlalche_obj.sql_alchemy_db_connection_open()
        odu100_mac_result = sqlalche_obj.session.query(Odu100MacFilterTable.filterMacAddress).filter(and_(Odu100MacFilterTable.host_id == host_id,Odu100MacFilterTable.filterMacAddress != "")).all()
        odu100_ip_result = sqlalche_obj.session.query(Odu100IpFilterTable.ipFilterIpAddress).filter(and_(Odu100IpFilterTable.host_id == host_id,Odu100IpFilterTable.ipFilterIpAddress != "")).all()
...
def odu100_mac_packet_table(host_id):
    ...
    try:
        global sqlalche_obj
        sqlalche_obj.sql_alchemy_db_connection_open()
        odu100_mac_filter_table = sqlalche_obj.session.query(Odu100MacFilterTable).filter(Odu100MacFilterTable.host_id == host_id).all()
...
def odu100_get_sysconfigtable(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_sys_conf_table = sqlalche_obj.session.query(Odu100SysOmcRegistrationTable).filter(Odu100SysOmcRegistrationTable.config_profile_id == odu100_profile_id[0]).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
...
def odu100_get_syncconfigtable(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_sync_conf_table = sqlalche_obj.session.query(Odu100SyncConfigTable.rasterTime,Odu100SyncConfigTable.syncLossThreshold,Odu100SyncConfigTable.leakyBucketTimer,\
...
def odu100_get_channelConfig(host_id):
    ...
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_channel_conf_table = sqlalche_obj.session.query(Odu100RaPreferredRFChannelTable.rafrequency).filter(Odu100RaPreferredRFChannelTable.config_profile_id == odu100_profile_id[0]).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
...
def odu100_get_status(host_id,class_name,time_stamp=0):
    try:
        ...
        if host_id == "" or host_id == None:
            return []
        if time_stamp==0:
            exec "get_data = sqlalche_obj.session.query(%s).filter(%s.host_id==%s).all()"%(class_name,class_name,host_id)
        else:
            exec "get_data = sqlalche_obj.session.query(%s).filter(%s.host_id==%s).order_by(desc(%s.timestamp)).limit(1).all()"%(class_name,class_name,host_id,class_name)
        if len(get_data)>0:
            return get_data
        else:
            return []
...
def acl_edit_bind(uuid):
    try:
        ...
        odu100_acl_edit_data = sqlalche_obj.session.query(Odu100RaAclConfigTable.aclIndex,Odu100RaAclConfigTable.macaddress,Odu100RaAclConfigTable.config_profile_id).filter(Odu100RaAclConfigTable.odu100_raAclConfigTable_id == uuid).all()
        odu100_ra_conf_data = sqlalche_obj.session.query(Odu100RaConfTable.aclMode).filter(Odu100RaConfTable.config_profile_id == odu100_acl_edit_data[0][2]).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
        dic_result = {"success":0,"result":{}}
...
def select_table_prefix(device_type_id):
    try:
        global sqlalche_obj
        sqlalche_obj.sql_alchemy_db_connection_open()
        table_prefix = sqlalche_obj.session.query(DeviceType.table_prefix).filter(DeviceType.device_type_id == device_type_id).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
...
class MakeOdu100SelectListWithDic(object):
    ...
    def channel_bandwidth_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        channel_bandwidth_select_list_name_value_dic = {}
        channel_bandwidth_select_list_name_value_dic = {'name':['5','10','20','40','40 Short GI'],'value':['0','1','2','3','4']}
    ...
    def packet_filter_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        # this dictionary is used to store the name and value of select list
        packet_filter_select_list_name_value_dic = {}
        packet_filter_select_list_name_value_dic = {'name':['Disable','IP Filter','MAC Filter'],'value':['0','1','2']}
        # call the function of common_controller
        # This returns the select list string in html format
    ...
    def sync_source_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        """
        # this dictionary is used to store the name and value of select list
        sync_source_select_list_name_value_dic = {}
        sync_source_select_list_name_value_dic = {'name':['Internal clock (0)','Radio 1 (1)','External (3)'],'value':['0','1','3']}
    ...
    def country_code_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        """
        # this dictionary is used to store the name and value of select list
        country_code_select_list_name_value_dic = {}
        country_code_select_list_name_value_dic = {'name':['DBG(0)','Denmark(208)','India(356)','Sweden(752)','USA(840)'],'value':['0','208','356','752','840']}
    ...
    def poe_state_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        """
        # this dictionary is used to store the name and value of select list
        poe_state_select_list_name_value_dic = {}
        poe_state_select_list_name_value_dic = {'name':['Disable','Enable'],'value':['0','1']}
    ...
    def poe_state_select_list_cpu(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        # this dictionary is used to store the name and value of select list
        poe_state_select_list_name_value_dic = {}
        poe_state_select_list_name_value_dic = {'name':['Not Applicable'],'value':['2']}
    ...
    def timeslot_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        for i in range(0,timeslot_total_select_name_value):
            if i==0:
                timeslot_select_list_name_value_dic.setdefault('name',[]).append(str(i+1)+" Timeslot(T1/T2)")
                timeslot_select_list_name_value_dic.setdefault('value',[]).append(i+1)
            else:
                timeslot_select_list_name_value_dic.setdefault('name',[]).append(str(i+1)+" Timeslot(T2)")
                timeslot_select_list_name_value_dic.setdefault('value',[]).append(i+1)
    ...
    def encryption_type_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        # this dictionary is used to store the name and value of select list
        encryption_type_select_list_name_value_dic = {}
        encryption_type_select_list_name_value_dic = {'name':['None','AES'],'value':['0','1']}
    ...
    def acm_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        acm_state_select_list_name_value_dic = {}
        acm_state_select_list_name_value_dic = {'name':['Disable','Enable'],'value':['0','1']}
    ...
    def dba_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        dba_state_select_list_name_value_dic = {}
        dba_state_select_list_name_value_dic = {'name':['Disable','Enable'],'value':['0','1']}
    ...
    def acs_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        acs_state_select_list_name_value_dic = {}
        acs_state_select_list_name_value_dic = {'name':['Disable','Enable'],'value':['0','1']}
    ...
    def ans_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        ans_state_select_list_name_value_dic = {}
        ans_state_select_list_name_value_dic = {'name':['Disable','Enable'],'value':['0','1']}
    ...
    def dfs_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        dfs_state_select_list_name_value_dic = {}
        dfs_state_select_list_name_value_dic = {'name':['Disable','Enable','NA'],'value':['0','1','2']}
    ...
    def preffered_channel_select_list10(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        for i in range(5180,5241,5):
            preffered_channel_list_name_value_dic.setdefault('name',[]).append(i)
            preffered_channel_list_name_value_dic.setdefault('value',[]).append(i)
    ...
    def preffered_channel_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg,host_id,selected_device):
        ...
        global sqlalche_obj
        sqlalche_obj.sql_alchemy_db_connection_open()
        channel_list = sqlalche_obj.session.query(Odu100RaChannelListTable).filter(Odu100RaChannelListTable.host_id == host_id).all()
        preffered_channel_list_name_value_dic = {}
        preffered_channel_list_name_value_dic.setdefault('name',[]).append(0)
        preffered_channel_list_name_value_dic.setdefault('value',[]).append(0)
        if len(channel_list)>0:
            ...
            for i in range(0,len(channel_list)):
                preffered_channel_list_name_value_dic.setdefault('name',[]).append(channel_list[i].frequency)
                preffered_channel_list_name_value_dic.setdefault('value',[]).append(channel_list[i].frequency)

        else:

            host_data = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).all() 
            #return host_data
            preferred_channel_list = sqlalche_obj.session.query(Odu100RaPreferredRFChannelTable).filter(Odu100RaPreferredRFChannelTable.config_profile_id == host_data[0].config_profile_id).all()
            for i in range(0,len(preferred_channel_list)):
                preffered_channel_list_name_value_dic.setdefault('name',[]).append(preferred_channel_list[i].rafrequency)
                preffered_channel_list_name_value_dic.setdefault('value',[]).append(preferred_channel_list[i].rafrequency)
        sqlalche_obj.sql_alchemy_db_connection_close()
        return make_select_list_using_dictionary(preffered_channel_list_name_value_dic,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg)
    ...
    def dhcp_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        return the select list string in html format
        """
        dhcp_select_list_using_dictionary = {}
        dhcp_select_list_using_dictionary = {'name':['DISABLE','ENABLE'],'value':['0','1']}
    ...
    def acl_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        return the select list string in html format
        """
        acl_select_list_using_dictionary = {}
        acl_select_list_using_dictionary = {'name':['Disable','Accept','Deny'],'value':['0','1','2']}
    ...
    def antenna_port_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        # this dictionary is used to store the name and value of select list
        antenna_name_value_dic = {}
        antenna_name_value_dic = {}
        antenna_name_value_dic = {'name':['Port1','Port2','MIMO'],'value':['1','2','3']}
...
##                    mcs_index_name_value_dic.setdefault('value',[]).append(i-1)        
    ...
    def link_distance_select_list(self,selected_field,selected_list_state,selected_list_id,is_readonly,select_list_initial_msg):
        ...
        return the select list string in html format
        """
        link_distance_name_value_dic = {}
        link_distance_name_value_dic = {'name':['UPTO 20','GT 20'],'value':['0','1']}
...
def odu100_set_config(host_id,device_type_id,dic_result):
    ...
    try:  
        ...
        if len(dependent_oid)>0: 
            for i in range(0,len(dependent_oid)):
                j+=1                
                #print depend_oid_value[i] ,'-------=++++++++++',dependent_oid[i],'depend_oid_value for %s '%i 
                if dependent_oid[i].has_key('ru.ruConfTable.adminstate-1'):
                    result = (pysnmp_set(depend_oid_value[i],device_param_list[0],device_param_list[1],device_param_list[2],dependent_oid[i]))
                ...
                if result["success"] == 0 or result["success"] == '0':                    
                    #print result["result"],'??????????????????/((((((((((())))))))',success_result
                    success_result["success"] = result["success"]
                    for i in result["result"]:
                        if result["result"][i] != 0:
                            result["result"][i] = errorStatus[result["result"][i]]
                        else:
                            oid_list_table_field_value = sqlalche_obj.session.query(Oids.table_name,Oids.coloumn_name).filter(and_(Oids.oid_name == i,Oids.device_type_id == device_type_id)).all()
                    ...
                    for i in result["result"]:
                        ...
                        success_result["result"] = errorStatus.get(i,"SNMP Agent Down")

        if len(dependent_oid)>0:  
            for i in range(0,len(dependent_oid)):               
                query_admin_result = sqlalche_obj.session.query(Oids.oid,Oids.oid_type,Oids.indexes).filter(Oids.oid_name == "ru.ra.raConfTable.raAdminState").one()                            
                if dependent_oid[i].has_key('ru.ruConfTable.adminstate-1'):                    
                    oid_list_table_field_value = []
                    table_name = ""
                    admin_state = "ru.ruConfTable.adminstate"
                    query_admin_result = sqlalche_obj.session.query(Oids.oid,Oids.oid_type,Oids.indexes).filter(Oids.oid_name == "ru.ruConfTable.adminstate").one()
                    result_admin = {}
                    ...
                    if len(query_admin_result) > 0:                       
                        result_admin = pysnmp_set(dic_admin_value,device_param_list[0],device_param_list[1],device_param_list[2])
                        oid_list_table_field_value = sqlalche_obj.session.query(Oids.table_name,Oids.coloumn_name).filter(and_(Oids.oid_name == "ru.ruConfTable.adminstate",Oids.device_type_id == device_type_id)).all()
                        if len(oid_list_table_field_value) != 0:                                        
                            table_name = "odu100_" + oid_list_table_field_value[0][0]
                            table_name = rename_tablename(table_name)
                            exec "table_result = sqlalche_obj.session.query(%s).filter(%s.config_profile_id == \"%s\").all()"%(table_name,table_name,device_param_list[3])
                        if result_admin["success"] == 0 or result_admin["success"] == '0':
                            ...
                            exec "table_result[0].%s = '%s'"%(oid_list_table_field_value[0][1],1)
                            #success_result["result"].update(result["result"])
                        else:
                            exec "table_result[0].%s = '%s'"%(oid_list_table_field_value[0][1],1)
                            for i in result_admin["result"]:    
                                if i==553:
                                    result_admin["result"] = str(errorStatus.get(i,"SNMP agent Unknown Error Occured"))
                                elif i==551:
                                    result_admin["result"] = str(errorStatus.get(i,"SNMP agent Unknown Error Occured")) 
                                elif result_admin["result"][i] != 0:
                                    result_admin["result"] = str(errorStatus.get(result_admin["result"][i],"SNMP agent Unknown Error Occured"))
                            result_admin['success']=1
                        result_admin['admin_name'] = 'ru.ruConfTable.adminstate'
                        success_result['result_admin']=result_admin


                elif dependent_oid[i].has_key('ru.ipConfigTable.adminState-1'):                  
                    query_admin_result = sqlalche_obj.session.query(Oids.oid,Oids.oid_type,Oids.indexes).filter(Oids.oid_name == "ru.ipConfigTable.adminState").one()
                    dic_admin_value = {"ru.ipConfigTable.adminState":[query_admin_result[0]+query_admin_result[2],query_admin_result[1],'1']}
                    if len(query_admin_result) > 0:                        
                        result = pysnmp_set(dic_admin_value,device_param_list[0],device_param_list[1],device_param_list[2])                        
                        if result["success"] == 0 or result["success"] == '0':
                            success_result["success"] = result["success"]
                            for i in result["result"]:
                                if result["result"][i] != 0:
                    ...
                    if len(query_admin_result) > 0:  
                        ...
                        result_admin = pysnmp_set(dic_admin_value,device_param_list[0],device_param_list[1],device_param_list[2])  
                        #return result_admin
                        oid_list_table_field_value = sqlalche_obj.session.query(Oids.table_name,Oids.coloumn_name).filter(and_(Oids.oid_name == "ru.ra.raConfTable.raAdminState",Oids.device_type_id == device_type_id)).all()
                        table_name = "odu100_" + oid_list_table_field_value[0][0]
                        table_name = rename_tablename(table_name)
                        if result_admin["success"] == 0 or result_admin["success"] == '0':
                            if len(oid_list_table_field_value) != 0:                                        
                            ...
                            #exec "table_result[0].%s = '%s'"%(oid_list_table_field_value[0][1],0)
                            for i in result_admin["result"]:
                                if i==553:
                                    result_admin["result"] = str(errorStatus.get(i,"SNMP agent Unknown Error Occured"))
                                elif i==551:
                                    ...
                                    result_admin["result"] = str(errorStatus.get(result_admin["result"][i],"SNMP agent Unknown Error Occured"))
                            result_admin['success']=1
                        result_admin['admin_name'] = 'ru.ra.raConfTable.raAdminState'
                        success_result['result_admin']=result_admin
                    break                
                else:
                    continue
        ...
        if(j == -1): 
            if len(depend_oid_value)>0:
                ...
                for k in range(0,len(depend_oid_value)):
                    pysnm_result = pysnmp_set(depend_oid_value[k],device_param_list[0],device_param_list[1],device_param_list[2])
                    if pysnm_result["success"] == 0 or pysnm_result["success"] == '0':
                        ...
                        for i in pysnm_result["result"]:
                            if pysnm_result["result"][i] != 0:
                                pysnm_result["result"][i] = errorStatus[pysnm_result["result"][i]]
                            else:
                                oid_list_table_field_value = sqlalche_obj.session.query(Oids.table_name,Oids.coloumn_name).filter(and_(Oids.oid_name == i,Oids.device_type_id == device_type_id)).all()
                                if len(oid_list_table_field_value) == 0:
                                    continue
                        ...
                        if success_result.has_key("success") and success_result["success"] == 0:
                            pass
                        else:
                            success_result["success"] = 1
                        ...
                        for i in pysnm_result["result"]:
                            if i==553:
                                fail_result = str(errorStatus.get(i,"SNMP agent Unknown Error Occured"))
                            elif i==551:
                                fail_result = str(errorStatus.get(i,"SNMP agent Unknown Error Occured")) 
                            else:
                                if pysnm_result["result"][i] > 95:
                                    fail_result = errorStatus.get(pysnm_result["result"][i],"Device is not respoding")
                        if len(fail_result) > 0:
                            break_found = 1
                            break
                        for j in depend_oid_value[k]:
                            if pysnm_result["result"].has_key(j):
                                fail_dic[j] = str(errorStatus.get(pysnm_result["result"][j],"SNMP agent Unknown Error Occured")) 
                            else:
                ...
                if break_found and success_result['success'] == 0:
                    for k in depend_oid_value:
                        if success_result['result'].has_key(k):
                            pass
                        else:
                    ...
                    if len(success_result['result']) == 1:
                        success_result['result'] = success_result['result'].values()[0]
                    else:
...
def controller_validation(host_id,device_type_id,dic_result):
    ...
    try:
        ...
        if dic_result["success"] == '0' or dic_result["success"] == 0:
            ...
            for keys in dic_result.iterkeys():
                if keys == "success":
                    ...
                    oid_list_min_max_value = sqlalche_obj.session.query(Oids.min_value,Oids.max_value).filter(and_(Oids.oid_name == keys,Oids.device_type_id == device_type_id)).all()
            ...
            if flag == 1:
                ...
                sqlalche_obj.sql_alchemy_db_connection_close()   
                dic_result = odu100_set_config(host_id,device_type_id,dic_result)
                return dic_result
        else:
            sqlalche_obj.sql_alchemy_db_connection_close()  
            return dic_result
        ...
        sqlalche_obj.sql_alchemy_db_connection_close()
#print controller_validation(4,'odu100',{'ru.omcConfTable.periodicStatsTimer': '12', 'success': 0, 'ru.omcConfTable.omcIpAddress': '172.22.0.95'})
...
def check_mac(host_id,macaddress,acl_mode,submit_btn_name,acl_index):
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_acl_mac_available = sqlalche_obj.session.query(Odu100RaAclConfigTable.macaddress,Odu100RaAclConfigTable.aclIndex).filter(Odu100RaAclConfigTable.config_profile_id == odu100_profile_id[0]).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
        if len(odu100_acl_mac_available) == 0:
            ...
            if submit_btn_name.lower() == "add":
                ...
                acl_mac_dict = dict((row[1],row[0]) for row in odu100_acl_mac_available)
                second_mac_list = list(row[0] for row in odu100_acl_mac_available if row[1] != int(acl_index))
...
def check_acl_accept(host_id,macaddress,acl_mode):
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_peer_conf_table = sqlalche_obj.session.query(Odu100PeerConfigTable.peerMacAddress).filter(Odu100PeerConfigTable.config_profile_id == odu100_profile_id[0]).all()
            odu100_acl_mac_available = sqlalche_obj.session.query(Odu100RaAclConfigTable.macaddress,Odu100RaAclConfigTable.aclIndex).filter(Odu100RaAclConfigTable.config_profile_id == odu100_profile_id[0]).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
        if len(odu100_acl_mac_available) == 0:
            return 2
...
def check_acl_mac_accept(host_id,mac_address):
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_acl_mode = sqlalche_obj.session.query(Odu100RaConfTable.aclMode).filter(Odu100RaConfTable.config_profile_id == odu100_profile_id[0]).all()
            odu100_peer_conf_table = sqlalche_obj.session.query(Odu100PeerConfigTable.peerMacAddress).filter(Odu100PeerConfigTable.config_profile_id == odu100_profile_id[0]).all()
...
def check_acl_mac(host_id,mac_address):
    try:
        ...
        if odu100_profile_id == None or odu100_profile_id == []:
            ...
            odu100_ra_conf_table = sqlalche_obj.session.query(Odu100RaConfTable.aclMode).filter(Odu100RaConfTable.config_profile_id == odu100_profile_id[0]).all()
            odu100_peer_conf_table = sqlalche_obj.session.query(Odu100PeerConfigTable.peerMacAddress).filter(Odu100PeerConfigTable.config_profile_id == odu100_profile_id[0]).all()
        ...
        if dic_result["success"] == 0:
            for keys in dic_result["result"].iterkeys():
                if keys == "success" or keys == "aclindex":
                    ...
                    oid_list_table_field_value = sqlalche_obj.session.query(Oids.table_name,Oids.coloumn_name).filter(and_(Oids.oid_name == keys,Oids.device_type_id == device_type_id)).all()
                    table_name = "odu100_" + oid_list_table_field_value[0][0]
                    table_name = rename_tablename(table_name)
                    if table_name == "Odu100RaAclConfigTable":
                        str_table_obj = "table_result = sqlalche_obj.session.query(%s.%s).filter(%s.config_profile_id == \"%s\").first()"%(table_name,oid_list_table_field_value[0][1],table_name,odu100_profile_id[0])
                    else:
                        str_table_obj = "table_result = sqlalche_obj.session.query(%s.%s).filter(%s.config_profile_id == \"%s\").one()"%(table_name,oid_list_table_field_value[0][1],table_name,odu100_profile_id[0])
                    exec str_table_obj
...
def acl_delete_bind(uuid,host_id):
    try:  
        ...
        device_param_list = sqlalche_obj.session.query(Hosts.ip_address,Hosts.snmp_port,Hosts.snmp_write_community,Hosts.config_profile_id).\
            filter(Hosts.host_id == host_id).one()
        odu100_acl_conf_table = sqlalche_obj.session.query(Odu100RaAclConfigTable).filter(Odu100RaAclConfigTable.odu100_raAclConfigTable_id == '%s'%(uuid)).one()

        rowSts.append({"ru.ra.raAclConfigTable.rowSts":["1.3.6.1.4.1.26149.2.2.13.5.1.3"+"."+str(odu100_acl_conf_table.raIndex)+"."+str(odu100_acl_conf_table.aclIndex),"Integer32","6"]})

        result = pysnmp_setAcl(device_param_list[0],device_param_list[1],device_param_list[2],{},{},{},rowSts[0])
...
##rint acl_delete_bind("ba52de08-ea94-11e0-8978-e069956899a4","9fad8c64-de92-11e0-b146-e069956899a4")
        ...
        if keys == "success":
            ...
            if len(query_result) > 0:
                ...
                if query_result[0][0].dependent_id == "" or query_result[0][0].dependent_id == None:
                    dependent_oid.append({keys:[query_result[0][0].oid + query_result[0][0].indexes,query_result[0][0].oid_type,dic_result[keys]]})
                    ...
                    #result = (pysnmp_set(depend_oid_value[i],device_param_list[0],device_param_list[1],device_param_list[2],dependent_oid[i]))
    for i in range(0,len(dependent_oid)):
        result = (pysnmp_set(dependent_oid[i],device_param_list[0],device_param_list[1],device_param_list[2]))
    sqlalche_obj.sql_alchemy_db_connection_close()  
    for i in result["result"]:
        if result["result"][i] != 0:
            result["result"][i] = errorStatus[result["result"][i]]
...
def peer_set(host_id,device_type_id,timeslot_val,dic_result,save_retry):
        ...
        if save_retry ==1:
            for i in range(timeslot_val,0,-1):
                if 'ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s'%(i) in dic_result:
                    dic .update({"ru.ra.peerNode.peerConfigTable.peerMacAddress.%s"%(i):["1.3.6.1.4.1.26149.2.2.13.9.1.1.2.1.%s"%(i),"OctetString",dic_result["ru.ra.peerNode.peerConfigTable.peerMacAddress.%s"%(i)]],\
                    ...
                    dic = {"ru.ra.peerNode.peerConfigTable.peerMacAddress.%s"%(i):["1.3.6.1.4.1.26149.2.2.13.9.1.1.2.1.%s"%(i),"OctetString",dic_result["ru.ra.peerNode.peerConfigTable.peerMacAddress.%s"%(i)]]}
            ...
            result = pysnmp_set(dic,device_param_list[0].ip_address,device_param_list[0].snmp_port,device_param_list[0].snmp_write_community,{'raadmin-1':['1.3.6.1.4.1.26149.2.2.13.1.1.2.1','Integer32',0]})
            if result['success']==1:
                for i in result["result"]:
                    if i==553:
                        result["result"] = str(errorStatus.get(i,"SNMP agent Unknown Error Occured"))
                    elif i==551:
                        result["result"] = str(errorStatus.get(i,"SNMP agent Unknown Error Occured")) 
                    else:
                        if result["result"][i] != 0:
                            result["result"] = str(errorStatus.get(result["result"][i],"Device Unresponsive"))
                        else:
                            result["result"] = str(errorStatus.get(i,"Device Unresponsive"))

            else:
                peer_config = sqlalche_obj.session.query(Odu100PeerConfigTable).filter(Odu100PeerConfigTable.config_profile_id == device_param_list[0].config_profile_id).delete()        
                sqlalche_obj.session.commit()
                for i in range(0,timeslot_val):
                    if 'ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s'%(i+1) in dic_result:
                        peer_add = Odu100PeerConfigTable(device_param_list[0].config_profile_id,1,1,dic_result["ru.ra.peerNode.peerConfigTable.peerMacAddress.%s"%(i+1)],\
                                                         dic_result['ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s'%(i+1)],dic_result['ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s'%(i+1)],\
                        ...
                        peer_add = Odu100PeerConfigTable(device_param_list[0].config_profile_id,1,1,dic_result["ru.ra.peerNode.peerConfigTable.peerMacAddress.%s"%(i+1)],512,512,-1,100000,100000)
                        sqlalche_obj.session.add(peer_add)
                admin_result = pysnmp_set({'raadmin':['1.3.6.1.4.1.26149.2.2.13.1.1.2.1','Integer32',1]},device_param_list[0].ip_address,device_param_list[0].snmp_port,device_param_list[0].snmp_write_community)
                ra_admin=sqlalche_obj.session.query(Odu100RaConfTable).filter(Odu100RaConfTable.config_profile_id==device_param_list[0].config_profile_id).all()
                print admin_result
                if admin_result['success']==1:
                    for i in admin_result['result']:
                        admin_result['result']=errorStatus.get(admin_result['result'][i],"")
            ...
            for k in dic_result:
                if k== "success":
                    ...
                    if "peerMacAddress" in k:
                        name = "ru.ra.peerNode.peerConfigTable.peerMacAddress.%s"%(k.split(".")[-1])
                        li.append(k.split(".")[-1])
                        oid = "1.3.6.1.4.1.26149.2.2.13.9.1.1.2.1.%s"%(k.split(".")[-1])
                        oid_type = "OctetString"
                        oidvalue = dic_result["ru.ra.peerNode.peerConfigTable.peerMacAddress.%s"%(k.split(".")[-1])]
                        dic ={name:[oid,oid_type,oidvalue]}

                    elif "guaranteedUplinkBW" in k:
                        li.append(k.split(".")[-1])
                        name = "ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s"%(k.split(".")[-1])
                        oid = "1.3.6.1.4.1.26149.2.2.13.9.1.1.3.1.%s"%(k.split(".")[-1])
                        oid_type = "Integer32"
                        oidvalue = dic_result["ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.%s"%(k.split(".")[-1])]
                        dic ={name:[oid,oid_type,oidvalue]}
                    elif "guaranteedDownlinkBW" in k:
                        li.append(k.split(".")[-1])
                        name = "ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s"%(k.split(".")[-1])
                        oid = "1.3.6.1.4.1.26149.2.2.13.9.1.1.4.1.%s"%(k.split(".")[-1])
                        oid_type = "Integer32"
                        oidvalue = dic_result["ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.%s"%(k.split(".")[-1])]
                        dic ={name:[oid,oid_type,oidvalue]}
                    elif "basicRateMCSIndex" in k:
                        li.append(k.split(".")[-1])
                        name = "ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.%s"%(k.split(".")[-1])
                        oid = "1.3.6.1.4.1.26149.2.2.13.9.1.1.5.1.%s"%(k.split(".")[-1])
                        oid_type = "Integer32"
                        oidvalue = dic_result["ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.%s"%(k.split(".")[-1])]
                        dic ={name:[oid,oid_type,oidvalue]}
                    elif "maxUplinkBW" in k:
                        li.append(k.split(".")[-1])
                        name = "ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s"%(k.split(".")[-1])
                        oid = "1.3.6.1.4.1.26149.2.2.13.9.1.1.6.1.%s"%(k.split(".")[-1])
                        oid_type = "Integer32"
                        oidvalue = dic_result["ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s"%(k.split(".")[-1])]
                        dic ={name:[oid,oid_type,oidvalue]}
                    elif "maxDownlinkBW" in k:
                        li.append(k.split(".")[-1])
                        name = "ru.ra.peerNode.peerConfigTable.maxDownlinkBW.%s"%(k.split(".")[-1])
                        oid = "1.3.6.1.4.1.26149.2.2.13.9.1.1.7.1.%s"%(k.split(".")[-1])
                        oid_type = "Integer32"
                        oidvalue = dic_result["ru.ra.peerNode.peerConfigTable.maxUplinkBW.%s"%(k.split(".")[-1])]
                        dic ={name:[oid,oid_type,oidvalue]}
                    dic_r.update(dic)
    ...
    ##            dic_r.update(i)
            ...
            result = pysnmp_set(dic_r,device_param_list[0].ip_address,device_param_list[0].snmp_port,device_param_list[0].snmp_write_community,{'adminState-1':['1.3.6.1.4.1.26149.2.2.13.1.1.2.1','Integer32','1']})
            peer_config = sqlalche_obj.session.query(Odu100PeerConfigTable).filter(Odu100PeerConfigTable.config_profile_id == device_param_list[0].config_profile_id).all()
            if result['success']==1:
                for i in result["result"]:
                ...
                if len(peer_config)>0:
                    for i in dic_result:
                        if ('peermac' in i) and (i.split(".")[-1] in li):
                            peer_config[int((i.split(".")[-1]))-1].peerMacAddress = dic_result[i]
                        elif ('guaranteedUplinkBW' in i) and (i.split(".")[-1] in li):
                            peer_config[int((i.split(".")[-1]))-1].guaranteedUplinkBW = dic_result[i]
                        elif ('guaranteedDownlinkBW' in i) and (i.split(".")[-1] in li):
                            peer_config[int((i.split(".")[-1]))-1].guaranteedDownlinkBW = dic_result[i]
                        elif ('basicRateMCSIndex' in i) and (i.split(".")[-1] in li):
                            peer_config[int((i.split(".")[-1]))-1].basicrateMCSIndex = dic_result[i]
                        elif ('maxUplinkBW' in i) and (i.split(".")[-1] in li):
                            peer_config[int((i.split(".")[-1]))-1].maxUplinkBW = dic_result[i]
                        elif ('maxDownlinkBW' in i) and (i.split(".")[-1] in li):
                            peer_config[int((i.split(".")[-1]))-1].maxDownlinkBW = dic_result[i]
            admin_result = pysnmp_set({'raadmin':['1.3.6.1.4.1.26149.2.2.13.1.1.2.1','Integer32',1]},device_param_list[0].ip_address,device_param_list[0].snmp_port,device_param_list[0].snmp_write_community)
            ra_admin=sqlalche_obj.session.query(Odu100RaConfTable).filter(Odu100RaConfTable.config_profile_id==device_param_list[0].config_profile_id).all()
        ...
        sqlalche_obj.sql_alchemy_db_connection_close()
#
#print peer_set(28,'odu100',2,{'ru.ra.peerNode.peerConfigTable.peerMacAddress.2': '00:80:48:71:86:94', 'ru.ra.peerNode.peerConfigTable.maxUplinkBW.2': '3200', 'ru.ra.peerNode.peerConfigTable.peerMacAddress.1': '00:80:48:71:86:91', 'ru.ra.peerNode.peerConfigTable.maxDownlinkBW.2': '3200', 'ru.ra.peerNode.peerConfigTable.maxDownlinkBW.1': '4000', 'ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.1': '512', 'ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.1': '512', 'ru.ra.peerNode.peerConfigTable.guaranteedDownlinkBW.2': '512', 'ru.ra.peerNode.peerConfigTable.guaranteedUplinkBW.2': '512', 'ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.1': '5', 'success': 0, 'ru.ra.peerNode.peerConfigTable.basicRateMCSIndex.2': '0', 'ru.ra.peerNode.peerConfigTable.maxUplinkBW.1': '4000'},1)
...
def channel_config_set(host_id,device_type_id,dic_result):
    try:
        ...
        for i in dic_result:
            k = i.split(".")[-1]
            dic.update({i:('1.3.6.1.4.1.26149.2.2.13.7.4.1.2.1.%s'%str(k),'Gauge32',dic_result[i])})
        admin_state = {'admin':['1.3.6.1.4.1.26149.2.2.13.1.1.2.1','Integer32','0']}

        result_admin = pysnmp_set(admin_state,device_param_list[0].ip_address,device_param_list[0].snmp_port,device_param_list[0].snmp_write_community)
        ...
        if result_admin["success"]==0:
            ...
            for i in result_admin["result"]:
                errorStatus.has_key(i)
                result_admin["result"] = errorStatus.get(i,"SNMP Agent gone away")
                sqlalche_obj.sql_alchemy_db_connection_close()
                return result_admin

        admin_state = {'admin':['1.3.6.1.4.1.26149.2.2.13.1.1.2.1','Integer32',1]}
        result_admin = pysnmp_set(admin_state,device_param_list[0].ip_address,device_param_list[0].snmp_port,device_param_list[0].snmp_write_community)

        channel_config = sqlalche_obj.session.query(Odu100RaPreferredRFChannelTable).filter(Odu100RaPreferredRFChannelTable.config_profile_id == device_param_list[0].config_profile_id).all()
        ...
        time.sleep(5)
        tdd_mac_status_table = pysnmp_get_table('1.3.6.1.4.1.26149.2.2.13.7.2',device_param_list[0].ip_address,int(device_param_list[0].snmp_port),device_param_list[0].snmp_read_community)
        if tdd_mac_status_table["success"]==0:
            if len(tdd_mac_status_table["result"])>0:
                tdd_mac_status = sqlalche_obj.session.query(Odu100RaTddMacStatusTable).filter(Odu100RaTddMacStatusTable.host_id == host_id).delete()        
                sqlalche_obj.session.commit()
                m = 0
                for i in tdd_mac_status_table["result"]:
                    tdd_mac_status_add = Odu100RaTddMacStatusTable(host_id,1,tdd_mac_status_table["result"][i][0],datetime.now())
    ...
    try:
        ...
        for i in dic_result:
            ...
            if k1 == 'ipFilterIpAddress':
                dic.update({i:('1.3.6.1.4.1.26149.2.2.17.1.1.2.%s'%str(k),'OctetString',dic_result[i])})
            elif k1 == 'ipFilterNetworkMask':
                dic.update({i:('1.3.6.1.4.1.26149.2.2.17.1.1.3.%s'%str(k),'IpAddress',dic_result[i])})
            elif k1 == 'filterMacAddress':
                dic.update({i:('1.3.6.1.4.1.26149.2.2.17.2.1.2.%s'%str(k),'OctetString',dic_result[i])})

        result = pysnmp_set(dic,device_param_list[0].ip_address,int(device_param_list[0].snmp_port),device_param_list[0].snmp_write_community)
        ...
        if result["success"] == 0 or result["success"] == '0':
                ...
                if result["result"][i] != 0:
                    ...
                    if i in dic_result:
                        if ip_mac_statue == 0: # 0 for ipPacketFilter and 1 for macPacketFilter
                            filter_result = sqlalche_obj.session.query(Odu100IpFilterTable).filter(and_(Odu100IpFilterTable.host_id == host_id,Odu100IpFilterTable.ipFilterIndex == int(index_value))).all()
                            setattr(filter_result[0], column_name, dic_result[i])
                        else:
                            filter_result = sqlalche_obj.session.query(Odu100MacFilterTable).filter(and_(Odu100MacFilterTable.host_id == host_id,Odu100MacFilterTable.macFilterIndex == int(index_value))).all()
                            setattr(filter_result[0], column_name, dic_result[i])
            sqlalche_obj.session.commit()
            ...
            for i in result["result"]:
                if i==553:
                    ...
                    if result["result"][i] != 0:
                        result["result"] = errorStatus.get(result["result"][i],"Device is not respoding")
                success_result["result"] = result['result']
...
#dic = {'ru.packetFilters.ipFilterTable.ipFilterIpAddress.5': '', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.4': '172.22.0.104', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.7': '', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.6': '', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.1': '172.22.0.1', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.3': '', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.2': '172.22.0.2', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.8': '', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.1': '255.255.255.255', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.2': '255.255.255.255', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.3': '255.255.255.255', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.4': '255.255.255.255', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.5': '255.255.255.255', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.6': '255.255.255.255', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.7': '255.255.255.255', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.8': '255.255.255.255'}

#print packet_filter_set(11,'odu100','ip',dic)
#aa={'ru.packetFilters.ipFilterTable.ipFilterIpAddress.5': '', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.4': '172.22.0.161', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.7': '', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.6': '172.22.0.165', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.1': '172.22.0.156', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.3': '172.22.0.154', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.2': '172.22.0.158', 'ru.packetFilters.ipFilterTable.ipFilterIpAddress.8': '172.22.0.166', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.1': '255.255.255.255', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.2': '255.255.255.255', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.3': '255.255.255.255', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.4': '255.255.255.255', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.5': '', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.6': '255.255.255.255', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.7': '', 'ru.packetFilters.ipFilterTable.ipFilterNetworkMask.8': '255.255.255.255'}        
...
class OduReconcilation(object):# 
    def odu100_acl_reconciliation(self,host_id,device_type):
        try:
            ...
            result = pysnmp_acl_reconcile("1.3.6.1.4.1.26149.2.2.13.5",device_param_list[0].ip_address,port,device_param_list[0].snmp_read_community)
            if len(result)==1:
                if 'err' in result:
                    for i in result['err']:
                        if i in errorStatus:
                            result = {'success':1,'result':str(errorStatus[i])}
            elif len(result)>1:
                acl_config= sqlalche_obj.session.query(Odu100RaAclConfigTable).filter(Odu100RaAclConfigTable.config_profile_id==device_param_list[0][4]).delete()
                sqlalche_obj.session.commit()
                if len(result['mac'])==0 and len(result['row'])==0:
                    con_table_output = com_obj.reconciliation_model(host_id,device_type,'raConfTable',0)
                    if con_table_output["success"] == 0 :
                        result = {'success':0,'result':"Acl Reconciliation Done Successfully"}
                    else :
                        result = {'success':0,'result':con_table_output["result"]}
                elif (len(result['mac'])==len(result['row'])):
                    mac = result['mac']
                    row = result['row']
                    for i in row:
                        acl_row = Odu100RaAclConfigTable(device_param_list[0][4],1,i,mac[i],i)
    ...
    def odu16_add_default_config_profile(self,host_id,device_type_id,table_prefix,insert_update):
        global sqlalche_obj
        sqlalche_obj.sql_alchemy_db_connection_open()
        new_profile = Odu16ConfigProfiles(device_type_id,"Odu16config","Master",None,datetime.now(),None,datetime.now(),None,0)
        ...
        try:
            default_config_profile_id = sqlalche_obj.session.query(Odu16ConfigProfiles.config_profile_id).filter(and_(Odu16ConfigProfiles.device_type_id == device_type_id,Odu16ConfigProfiles.config_profile_type_id == "Default")).all()

            omc_conf_data = sqlalche_obj.session.query(SetOdu16OmcConfTable.omc_ip_address,SetOdu16OmcConfTable.periodic_stats_timer).filter(SetOdu16OmcConfTable.config_profile_id == default_config_profile_id[0][0]).all()
            omc_conf_add_row = SetOdu16OmcConfTable(profile_id,omc_conf_data[0][0],omc_conf_data[0][1])
            sqlalche_obj.session.add(omc_conf_add_row)

            peer_config_data = sqlalche_obj.session.query(SetOdu16PeerConfigTable.peer_mac_address,SetOdu16PeerConfigTable.index).filter(SetOdu16PeerConfigTable.config_profile_id == default_config_profile_id[0][0]).all()
            for i in range(0,len(peer_config_data)):
                add_peer_config_data = SetOdu16PeerConfigTable(profile_id,peer_config_data[i][0],peer_config_data[i][1])
                sqlalche_obj.session.add(add_peer_config_data)

            acl_config = sqlalche_obj.session.query(SetOdu16RAAclConfigTable.mac_address,SetOdu16RAAclConfigTable.index).filter(SetOdu16RAAclConfigTable.config_profile_id == default_config_profile_id[0][0]).all()
            for i in range(0,len(acl_config)):
                add_acl_config = SetOdu16RAAclConfigTable(profile_id,acl_config[i][0],acl_config[i][1])
                sqlalche_obj.session.add(add_acl_config)

            ra_conf = sqlalche_obj.session.query(SetOdu16RAConfTable.raAdminState,SetOdu16RAConfTable.acl_mode,SetOdu16RAConfTable.ssid).filter(SetOdu16RAConfTable.config_profile_id == default_config_profile_id[0][0]).all()
            add_ra_config = SetOdu16RAConfTable(profile_id,ra_conf[0][0],ra_conf[0][1],ra_conf[0][2])
            sqlalche_obj.session.add(add_ra_config)

            ra_llc = sqlalche_obj.session.query(SetOdu16RALlcConfTable.llc_arq_enable,SetOdu16RALlcConfTable.arq_win,SetOdu16RALlcConfTable.frame_loss_threshold,\
            ...
            add_ra_llc = SetOdu16RALlcConfTable(profile_id,ra_llc[0][0],ra_llc[0][1],ra_llc[0][2],ra_llc[0][3],ra_llc[0][4])
            sqlalche_obj.session.add(add_ra_llc)

            ra_tdd_mac = sqlalche_obj.session.query(SetOdu16RATddMacConfig.rf_channel_frequency,SetOdu16RATddMacConfig.pass_phrase,SetOdu16RATddMacConfig.rfcoding,\
            ...
            add_ra_tdd_mac = SetOdu16RATddMacConfig(profile_id,ra_tdd_mac[0][0],ra_tdd_mac[0][1],ra_tdd_mac[0][2],ra_tdd_mac[0][3],ra_tdd_mac[0][4],ra_tdd_mac[0][5])
            sqlalche_obj.session.add(add_ra_tdd_mac)

            ru_conf = sqlalche_obj.session.query(SetOdu16RUConfTable.adminstate,SetOdu16RUConfTable.channel_bandwidth,SetOdu16RUConfTable.sysnch_source,SetOdu16RUConfTable.country_code).filter(SetOdu16RUConfTable.config_profile_id == default_config_profile_id[0][0]).all()
            add_ru_conf = SetOdu16RUConfTable(profile_id,ru_conf[0][0],ru_conf[0][1],ru_conf[0][2],ru_conf[0][3])
            sqlalche_obj.session.add(add_ru_conf)

            ru_date_time = sqlalche_obj.session.query(SetOdu16RUDateTimeTable.year,SetOdu16RUDateTimeTable.month,SetOdu16RUDateTimeTable.day,SetOdu16RUDateTimeTable.hour,\
                                                      SetOdu16RUDateTimeTable.min,SetOdu16RUDateTimeTable.sec).all()
            add_ru_date_time = SetOdu16RUDateTimeTable(profile_id,ru_date_time[0][0],ru_date_time[0][1],ru_date_time[0][2],ru_date_time[0][3],ru_date_time[0][4],ru_date_time[0][5])
            sqlalche_obj.session.add(add_ru_date_time)

            sync_config = sqlalche_obj.session.query(SetOdu16SyncConfigTable.adminStatus,SetOdu16SyncConfigTable.raster_time,SetOdu16SyncConfigTable.num_slaves,\
            ...
            add_sync_config = SetOdu16SyncConfigTable(profile_id,sync_config[0][0],sync_config[0][1],sync_config[0][2],sync_config[0][4],sync_config[0][5],sync_config[0][6],\
                                                      sync_config[0][7])
            sqlalche_obj.session.add(add_sync_config)

            sys_register = sqlalche_obj.session.query(SetOdu16SysOmcRegistrationTable.sys_omc_register_contact_addr,\
                ...
                filter(SetOdu16SysOmcRegistrationTable.config_profile_id == default_config_profile_id[0][0]).all()

            add_sys_register = SetOdu16SysOmcRegistrationTable(profile_id,sys_register[0][0],sys_register[0][1],sys_register[0][2],sys_register[0][3],\
                                                               sys_register[0][4],sys_register[0][5],sys_register[0][6],sys_register[0][7],sys_register[0][8],\
                                                               sys_register[0][9],sys_register[0][10],sys_register[0][11],sys_register[0][12],sys_register[0][13],\
            ...
            get_sw_status_table = GetOdu16SWStatusTable(host_id,None,None,None)
    ...
    def odu100_add_default_config_profile(self,host_id,device_type_id,table_prefix,current_time,reconcile_chk,user_name):
        ...
        try:
            ...
            new_profile = Odu16ConfigProfiles(device_type_id,"Odu100config","Master",None,datetime.now(),None,datetime.now(),None,0)
            sqlalche_obj.session.add(new_profile)
            sqlalche_obj.session.flush()
            sqlalche_obj.session.refresh(new_profile)
            ...
            host_data = sqlalche_obj.session.query(Hosts).\
                filter(Hosts.host_id == host_id).all()           
            #print "hello"
            ru_conf_data = sqlalche_obj.session.query(Odu100RuConfTable).filter(Odu100RuConfTable.config_profile_id==default_profile_id[0].config_profile_id).all()

            if len(ru_conf_data)>0:
                for i in range(0,len(ru_conf_data)):

                    ru_conf_data_add = Odu100RuConfTable(new_profile_id,ru_conf_data[i].confIndex,ru_conf_data[i].adminstate,\
                                                         ru_conf_data[i].defaultNodeType,ru_conf_data[i].channelBandwidth,\
                                                         ru_conf_data[i].synchSource,ru_conf_data[i].countryCode,ru_conf_data[i].poeState,\
            ...
            #print "hello12"
            unmp_conf_data = sqlalche_obj.session.query(Odu100OmcConfTable).filter(Odu100OmcConfTable.config_profile_id==default_profile_id[0].config_profile_id).all()
            if len(unmp_conf_data)>0:
                for i in range(0,len(unmp_conf_data)):
                    unmp_conf_data_add = Odu100OmcConfTable(new_profile_id,unmp_conf_data[i].omcConfIndex,unmp_conf_data[i].omcIpAddress,unmp_conf_data[i].periodicStatsTimer)
                    sqlalche_obj.session.add(unmp_conf_data_add)
            #print "hello13"
            registration_conf_data = sqlalche_obj.session.query(Odu100SysOmcRegistrationTable).filter(Odu100SysOmcRegistrationTable.config_profile_id==default_profile_id[0].config_profile_id).all()
            if len(registration_conf_data)>0:
                for i in range(0,len(registration_conf_data)):
                    registration_conf_data_add = Odu100SysOmcRegistrationTable(new_profile_id,registration_conf_data[i].sysOmcRegistrationIndex,\
                                                                               registration_conf_data[i].sysOmcRegisterContactAddr,\
                                                                               registration_conf_data[i].sysOmcRegisterContactPerson,registration_conf_data[i].sysOmcRegisterContactMobile,\
            ...
            #print "hello14"
            synchronization_conf_data = sqlalche_obj.session.query(Odu100SyncConfigTable).filter(Odu100SyncConfigTable.config_profile_id==default_profile_id[0].config_profile_id).all()
            if len(synchronization_conf_data)>0:
                for i in range(0,len(synchronization_conf_data)):
                    synchronization_conf_data_add = Odu100SyncConfigTable(new_profile_id,\
                                                                          synchronization_conf_data[i].syncConfigIndex,synchronization_conf_data[i].adminStatus,\
                                                                          synchronization_conf_data[i].synchState,synchronization_conf_data[i].rasterTime,\
                                                                          synchronization_conf_data[i].syncLossThreshold,synchronization_conf_data[i].leakyBucketTimer,\
                                                                          synchronization_conf_data[i].syncLostTimeout,\
            ...
            #print "hello15"
            ra_conf_data = sqlalche_obj.session.query(Odu100RaConfTable).filter(Odu100RaConfTable.config_profile_id==default_profile_id[0].config_profile_id).all()
            if len(ra_conf_data)>0:
                for i in range(0,len(ra_conf_data)):
                    ra_conf_data_add = Odu100RaConfTable(new_profile_id,\
                                                         ra_conf_data[i].raIndex,ra_conf_data[i].raAdminState,ra_conf_data[i].aclMode,\
                                                         ra_conf_data[i].ssID,ra_conf_data[i].guaranteedBroadcastBW,ra_conf_data[i].dba,\
                                                         ra_conf_data[i].acm,ra_conf_data[i].acs,ra_conf_data[i].dfs,ra_conf_data[i].numSlaves,ra_conf_data[i].antennaPort,\
                                                         ra_conf_data[i].linkDistance,ra_conf_data[i].anc)
                    sqlalche_obj.session.add(ra_conf_data_add)
            #print "hello16"
            tdd_mac_conf_data = sqlalche_obj.session.query(Odu100RaTddMacConfigTable).filter(Odu100RaTddMacConfigTable.config_profile_id==default_profile_id[0].config_profile_id).all()
            if len(tdd_mac_conf_data)>0:
                for i in range(0,len(tdd_mac_conf_data)):
                    tdd_mac_conf_data_add = Odu100RaTddMacConfigTable(new_profile_id,\
                                                                      tdd_mac_conf_data[i].raIndex,tdd_mac_conf_data[i].passPhrase,tdd_mac_conf_data[i].txPower,\
                                                                      tdd_mac_conf_data[i].maxPower,tdd_mac_conf_data[i].maxCrcErrors,tdd_mac_conf_data[i].leakyBucketTimerValue,\
                                                                      tdd_mac_conf_data[i].encryptionType)
                    sqlalche_obj.session.add(tdd_mac_conf_data_add)
            #print "hello17"
            ra_llc_conf_data = sqlalche_obj.session.query(Odu100RaLlcConfTable).filter(Odu100RaLlcConfTable.config_profile_id==default_profile_id[0].config_profile_id).all()
            if len(ra_llc_conf_data)>0:
                for i in range(0,len(ra_llc_conf_data)):
                    ra_llc_conf_data_add = Odu100RaLlcConfTable(new_profile_id,\
                                                                ra_llc_conf_data[i].raIndex,ra_llc_conf_data[i].arqWinLow,ra_llc_conf_data[i].arqWinHigh,\
                                                                ra_llc_conf_data[i].frameLossThreshold,ra_llc_conf_data[i].leakyBucketTimerVal,ra_llc_conf_data[i].frameLossTimeout
                                                                )
                    sqlalche_obj.session.add(ra_llc_conf_data_add)
            #print "hello19"
            channel_conf_data = sqlalche_obj.session.query(Odu100RaPreferredRFChannelTable).filter(Odu100RaPreferredRFChannelTable.config_profile_id==default_profile_id[0].config_profile_id).all()
            if len(channel_conf_data)>0:
                for i in range(0,len(channel_conf_data)):
                    channel_conf_data_add = Odu100RaPreferredRFChannelTable(new_profile_id,\
            ...
            #print "hello22"
            # ra mac is removed
            ru_status_data = Odu100RuStatusTable(host_id,1,None,1,None,None,None,None,0,0,0)
    ...
    def update_reconcilation_controller(self,host_id,device_type,table_prefix,current_time,user_name):
        ...
        try:
            ...
            if len(firmware_result):
                firmware = firmware_result[0].firmware_mapping_id
                if firmware=='7.2.25':
                    table_name = "odu100_7_2_25_oid_table"
                else:
                ...
                if host_op_status!=None or host_op_status!="":
                    ...
                    if int(host_op_status)==0:
                        ...
                        if len(host_data)>0:
                            ...
                            ping_chk = snmp_ping(host_data[0].ip_address,host_data[0].snmp_read_community,int(host_data[0].snmp_port))
                            if int(ping_chk)==0:
                                host_data[0].reconcile_status = 1
                                sqlalche_obj.session.commit()
                                if device_type=="odu100":   # table change according to firmware version
                                    if firmware=="7.2.25":
                                        table_result = sqlalche_obj.session.query(Odu1007_2_25_oid_table).filter(Odu1007_2_25_oid_table.is_recon==1).all()
                                    else:
                                        table_result = sqlalche_obj.session.query(Odu1007_2_20_oid_table).filter(Odu1007_2_20_oid_table.is_recon==1).all()
                                if len(table_result)>0:
                                    for i in range(0,len(table_result)):
                                        table_oid_dic.update({table_result[i].table_name:[table_result[i].status,table_result[i].timestamp,table_result[i].table_oid]})
                                        table_var_bind.update({table_result[i].table_name:table_result[i].varbinds})

                                    rec_table_dic = self.time_diff_rec_table(table_oid_dic,current_time)
                                    if len(rec_table_dic)>0:
                                        ...
                                        for j in rec_table_dic:
                                            ...
                                            sqlalche_tablename = rename_tablename(tablename) 
                                            ...
                                            if j=="swStatusTable" or j=="omcConfTable":
                                                result = pysnmp_get_table_ap(str(rec_table_dic[j])+str(".1"),host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community)
                                            else:
                                                result = bulktable(str(rec_table_dic[j])+str(".1"),host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community,table_var_bind[j])
                                            #print result
                                            ...
                                            if result['success']==1:
                                                if j=="omcConfTable" and result['success']==1:
                                                    ...
                                                    for key in result['result']:
                                                        if int(key)==553:
                                                            agent_start(host_data[0].ip_address,'root','public')
                                                            result['result']=errorStatus.get(key,"UNMP Server is busy.Please try after some time,2000")
                                                            result['success']=1 
                                                        elif int(key)==551:
                                                            result['result']=errorStatus.get(key,"UNMP Server is busy.Please try after some time,2002")
                                                            result['success']=1
                                                            return
                                                            ...
                                                            agent_start(host_data[0].ip_address,'root','public')                          
                                                            result['result']=errorStatus.get(key,"Device is busy.Please try again later")
                                                            result['success']=1
                                                            return 
                                                            ...
                                                            if result['result'][key]!=0:
                                                                agent_start(host_data[0].ip_address,'root','public')            
                                                                result['result']=errorStatus.get(result['result'][key],"Device is not responding")
                                                                result['success']=1
                                                                return 
                                                ...
                                                if len(result['result'])==0:
                                                    ...
                                                    if "config_profile_id" in column_list:
                                                        ...
                                                        sqlalche_obj.db.execute("delete from %s where host_id = '%s' "%(tablename,host_data[0].host_id))
                                                else:
                                                    rec_done.append(str(j))
                                                    rec = rec+1
                                                    if "config_profile_id" in column_list:
                                                    ...
                                                    if j=="raAclConfigTable" or j=="peerConfigTable":
                                                        ...
                                                        for row in result['result']:
                                                            sqlalche_obj.db.execute("Insert into %s values(NULL,%s,%s)"%(tablename,host_data[0].config_profile_id,str(result["result"][row])[1:-1]))
                                                    else:
...
#					    				print column_list[j],result['result'][row][j],'equal'
                                                                        setattr(sqlalche_table_result[row-1],column_list[k],result['result'][row][k])
                                                                        if timestamp==1:
...
#									    print column_list[j],result_dic['result'][row][index_val],'not equal'
                                                                            setattr(sqlalche_table_result[row-1],column_list[k],result['result'][row][index_val])
                                                                            index_val += 1
                                                                        if timestamp == 1:
                                                                            setattr(sqlalche_table_result[row-1],"timestamp",time_stamp[:time_stamp.find('.')-1])

#								if len(result['result'][row]) == len(odu100_table_model[firmware][j]):
#                                                                new_column_list=list(zip(odu100_table_model[firmware][j],result['result'][row]))
...
#                                                                    setattr(sqlalche_table_result[row-1],column_list[val],str(result['result'][row][val]) if column_list[val] in odu100_table_model[firmware][j] else None)
#                                                                    if timestamp==1:
#                                                                        setattr(sqlalche_table_result[row-1],"timestamp",time_stamp[:time_stamp.find('.')-1])
                                                        else:
                                                            for row in result['result']:
                                                                if config_type ==1:
                                                                    sqlalche_obj.db.execute("Insert into %s (%s,config_profile_id,%s) values(NULL,%s,%s)"%(tablename,primary_key_id,",".join(column_list),host_data[0].config_profile_id,str(result['result'][row])[1:-1]))
                                                                else:
                                                                    if timestamp == 0:
                                                                        sqlalche_obj.db.execute("Insert into %s (%s,host_id,%s) values(NULL,%s,%s)"%(tablename,primary_key_id,",".join(column_list),host_id,str(result['result'][row])[1:-1]))
                                                                    else:

                                                                        aa="Insert into %s (%s,host_id,%s,timestamp) values(NULL,%s,%s,'%s')"%(tablename,primary_key_id,",".join(column_list),host_id,str(result['result'][row])[1:-1],time_stamp[:time_stamp.find('.')-1])
                                                                        sqlalche_obj.db.execute(aa)


                                        sqlalche_obj.db.execute("Update %s set status = 0 where table_name in ('%s')"%(table_name,"\',\'".join(rec_not_done)))
                                        a = str(current_time)
                                        sqlalche_obj.db.execute("Update %s set status = 1,timestamp='%s' where table_name in ('%s')"%(table_name,a[:a.find('.')-1],"\',\'".join(rec_done)))
                                        ...
                                        if rec_per !=0:
                                            sqlalche_obj.db.execute("delete from odu100_raChannelListTable where host_id='%s'"%(host_id))
                                        sqlalche_obj.session.commit()
                                        el = EventLog()
                                        el.log_event("Device Reconcilation Done","%s"%(user_name))   
                                        result = {"success":0,"result":{rec_per:[host_data[0].host_alias,host_data[0].ip_address]}}
                                        return 
                                    else:
                                    ...
                                    result = {'success':1,'result':"UNMP Server is busy.Please try after some time,1200"}
                                    host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id == host_id).all()
                                ...
                                if int(ping_chk)==2:
                                    ...
                                    result = {'success':1,'result':"Reconciliation not done.No response from device "+str(host_data[0].host_alias)+"("+str(host_data[0].ip_address+")")}
                                return
                        else:
                            result = {'success':1,'result':"UNMP Server is busy.Please try after some time,1100"}
                            return
                    else:
                        result = {'success':1,'result':"Device is busy, Device "+host_status_dic[int(host_op_status)]+" is in progress. please wait ..."}
                        return
                else:
                    result = {'success':1,'result':"UNMP Server is busy.Please try after some time,1000"}
                    return
            else:
            ...
            import traceback
            result = {"success":1,"result":"UNMP Server is busy.Please try after some time"+str(e)}
            print traceback.format_exc()
            return 
        finally:
            essential_obj.host_status(host_id,0,None,10)
                ...
                if len(host_data)>0:
                    if is_reconcile==True:
                        if snmp_ping(host_data[0].ip_address,host_data[0].snmp_read_community,int(host_data[0].snmp_port))==0:
                            profile_id = Odu16ConfigProfiles(device_type,None,'Master',None,datetime.now(),None,datetime.now(),None,0)
                            sqlalche_obj.session.add(profile_id)
                            sqlalche_obj.session.flush()
                            config_profile_id = profile_id.config_profile_id
                            sqlalche_obj.session.commit()
                            if device_type=="odu100":
                                table_result = sqlalche_obj.session.query(Odu100Oid_table).filter(Odu100Oid_table.is_recon==1).all()
                            if len(table_result)>0:
                                for i in range(0,len(table_result)):
                                    rec_table_dic.update({table_result[i].table_name:table_result[i].table_oid})
                                    table_var_bind.update({table_result[i].table_name:table_result[i].varbinds})
                                ...
                                for j in rec_table_dic:
                                    ...
                                    for columns in result_db:
                                        column_list.append(columns["column_name"])
                                    time.sleep(5)
                                    result = pysnmp_get_table(str(rec_table_dic[j])+str(".1"),host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community,int(table_var_bind.get(j)))
                                    if result['success']==1:
                                        for key in result['result']:
                                            if int(key)==553:
                                                ...
                                                result['result']=errorStatus.get(key,"UNMP Server is busy.Please try after some time,2001")
                                                result['success']=1
                                                return
                                            elif int(key)==97 or int(key)==98 or int(key)==99 or int(key)==102:
                                                result['result']=errorStatus.get(key,"UNMP Server is busy.Please try after some time,2002")
                                                ...
                                                temp_result = self.odu100_add_default_config_profile(host_id,device_type,table_prefix,is_reconcile)
                                                result = {'success':0,'result':[str(temp_result[0]),0]}
                                                return 
                                            else:
                                                if result['result'][key]!=0:
                                                    result['result']=errorStatus.get(result['result'][key],"UNMP Server is busy.Please try after some time,2002")
                                                    result['success']=1
                                        rec_not_done.append(str(j))
                                    else:
                                        if len(result['result'])==0:
                                            ...
                                            for row in result['result']:
                                                if config_type ==1:
                                                    sqlalche_obj.db.execute("Insert into %s values(NULL,%s,%s)"%(tablename,config_profile_id,str(result["result"][row])[1:-1]))
                                                else:
                                                    if timestamp == 0:                                                        
                                                        sqlalche_obj.db.execute("Insert into %s values(NULL,%s,%s)"%(tablename,host_id,str(result["result"][row])[1:-1]))                                                
                                                    else:
                                                        time_stamp = str(datetime.now())
                                                        sqlalche_obj.db.execute("Insert into %s values(NULL,%s,%s,'%s')"%(tablename,host_id,str(result["result"][row])[1:-1],time_stamp[:time_stamp.find('.')-1]))                                        

                                sqlalche_obj.db.execute("Update odu100_oid_table set status = 0 where table_name in ('%s')"%("\',\'".join(rec_not_done)))
                                a = str(current_time)
                                sqlalche_obj.db.execute("Update odu100_oid_table set status = 1,timestamp='%s' where table_name in ('%s')"%(a[:a.find('.')-1],"\',\'".join(rec_done)))
                                rec_per = (float(rec)/float(total_rec))
                                rec_per = int(rec_per * 100)
                                host_data[0].reconcile_health = rec_per
                                host_data[0].reconcile_status = 2
                                ...
                                el = EventLog()
                                el.log_event("Device Reconcilation Done","%s"%(user_name))
                                result = {'success':0,'result':[str(config_profile_id),rec_per]}
                            else:
                                result = {'success':1,'result':[str(config_profile_id),0]}
                                return
                        else:
                            temp_result = self.odu100_add_default_config_profile(host_id,device_type,table_prefix,is_reconcile)
                            result = {'success':0,'result':[str(temp_result[0]),0]}
    ...
    def odu16_add_current_config_profile(self,host_id,device_type_id,table_prefix,reconcile_chk=True):
        ...
        host_data = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_read_community,Hosts.ip_address,Hosts.snmp_port).filter(Hosts.host_id == host_id).all()
        #host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id == host_id).all()
        ...
        if reconcile_chk==True:
            if snmp_ping(host_data[0].ip_address,host_data[0].snmp_read_community,int(host_data[0].snmp_port))==0:
                config_id = Odu16ConfigProfiles(device_type_id,None,'Master',None,datetime.now(),None,datetime.now(),None,0)
                ...
                sqlalche_obj.session.commit()
                walk=snmp_walk(host_data[0][0],host_data[0][1],host_data[0][2],host_data[0][3],'.1.3.6.1.4.1.26149.2','-On','-OQ')
                ### less_count count
        ...
        ############################################################################################        
                ...
                if walk.get('.1.3.6.1.4.1.26149.2.2.1.1.7.1')=='raBW10Mhz':
                    ...
                    ru_channel_bandwidth = walk.get('.1.3.6.1.4.1.26149.2.2.1.1.7.1',-1)
                ...
                if walk.get('.1.3.6.1.4.1.26149.2.2.1.1.8.1')=='internal':
                    ...
                    ru_sync_source = walk.get('.1.3.6.1.4.1.26149.2.2.1.1.8.1',-1)
                ...
                ru_config = SetOdu16RUConfTable(profile_id,1 if walk.get('.1.3.6.1.4.1.26149.2.2.9.1.2.1')=='unlocked' else 0,\
                                                ru_channel_bandwidth,ru_sync_source,walk.get('.1.3.6.1.4.1.26149.2.2.1.1.9.1'))
                sqlalche_obj.session.add(ru_config)
                date_time = SetOdu16RUDateTimeTable(profile_id,walk.get('.1.3.6.1.4.1.26149.2.2.2.1.2.1',2000),walk.get('.1.3.6.1.4.1.26149.2.2.2.1.3.1',1),\
                                                    walk.get('.1.3.6.1.4.1.26149.2.2.2.1.4.1',1),walk.get('.1.3.6.1.4.1.26149.2.2.2.1.5.1',0),walk.get('.1.3.6.1.4.1.26149.2.2.2.1.6.1',20),\
                ...
                ra_config = SetOdu16RAConfTable(profile_id,1 if walk.get('.1.3.6.1.4.1.26149.2.2.13.1.1.2.1')=='unlocked' else 0,\
                                                acl_val,walk.get('.1.3.6.1.4.1.26149.2.2.13.1.1.5.1','Shyam'))
                sqlalche_obj.session.add(ra_config)


                omc_config= SetOdu16OmcConfTable(profile_id,walk.get('.1.3.6.1.4.1.26149.2.2.7.1.2.1',''),walk.get('.1.3.6.1.4.1.26149.2.2.7.1.3.1',30))
                sqlalche_obj.session.add(omc_config)

                #in this some fileds are missing which we added later
                omc_registration =  SetOdu16SysOmcRegistrationTable(profile_id,walk.get('.1.3.6.1.4.1.26149.2.2.8.1.2.1','not populated'),walk.get('.1.3.6.1.4.1.26149.2.2.8.1.3.1','not populated'),\
                                                                    walk.get('.1.3.6.1.4.1.26149.2.2.8.1.4.1','not populated'),walk.get('.1.3.6.1.4.1.26149.2.2.8.1.5.1','not populated'),walk.get('.1.3.6.1.4.1.26149.2.2.8.1.6.1','not populated'),\
                                                                    None,None,None,None,None,None,None,None,None,None,None,None)
                sqlalche_obj.session.add(omc_registration)

                syn_config = SetOdu16SyncConfigTable(profile_id,1 if walk.get('.1.3.6.1.4.1.26149.2.2.11.1.1.2.1')=='unlocked' else 0 ,walk.get('.1.3.6.1.4.1.26149.2.2.11.1.1.4.1',2),\
                                                     walk.get('.1.3.6.1.4.1.26149.2.2.11.1.1.5.1',1),\
                                                     walk.get('.1.3.6.1.4.1.26149.2.2.11.1.1.6.1',2),walk.get('.1.3.6.1.4.1.26149.2.2.11.1.1.7.1',5),walk.get('.1.3.6.1.4.1.26149.2.2.11.1.1.8.1',60),\
                                                     walk.get('.1.3.6.1.4.1.26149.2.2.11.1.1.9.1',10))
                sqlalche_obj.session.add(syn_config)

                llc_config=SetOdu16RALlcConfTable(profile_id,walk.get('.1.3.6.1.4.1.26149.2.2.13.6.1.1.1.1'),walk.get('.1.3.6.1.4.1.26149.2.2.13.6.1.1.2.1'),\
                ...
                if walk.get('.1.3.6.1.4.1.26149.2.2.13.7.1.1.3.1')=='racs24Mbps':
                    ...
                    rf_coding = 1
                tdd_mac_config=SetOdu16RATddMacConfig(profile_id,walk.get('.1.3.6.1.4.1.26149.2.2.13.7.1.1.1.1',5180),walk.get('.1.3.6.1.4.1.26149.2.2.13.7.1.1.2.1',''),\
                                                      rf_coding,walk.get('.1.3.6.1.4.1.26149.2.2.13.7.1.1.4.1',10),\
                ...
                for i in range(0,8):
                    ...
                    if (walk.get(".1.3.6.1.4.1.26149.2.2.13.9.1.1.2.1.%s"%(i+1))=='""') or (walk.get(".1.3.6.1.4.1.26149.2.2.13.9.1.1.2.1.%s"%(i+1))==''):
                        peer_config=SetOdu16PeerConfigTable(profile_id,'',int(i+1))
                        sqlalche_obj.session.add(peer_config)
                    else:
                        peer_config=SetOdu16PeerConfigTable(profile_id,walk.get(".1.3.6.1.4.1.26149.2.2.13.9.1.1.2.1.%s"%(i+1)),int(i+1))
                        sqlalche_obj.session.add(peer_config)
                j=0
                if walk.has_key(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1)):
                    while(walk.has_key(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))):
                        if (walk.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))=='"                  "') or (walk.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))=='""') or (walk.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))==''):
                            acl_config=SetOdu16RAAclConfigTable(profile_id,'',int(j+1))
                            sqlalche_obj.session.add(acl_config)

                        else:
                            acl_config=SetOdu16RAAclConfigTable(profile_id,walk.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1)),int(j+1))
                            sqlalche_obj.session.add(acl_config)
                    ...
                    for k in range(0,10):
                        acl_config=SetOdu16RAAclConfigTable(profile_id,'',int(k+1))
                        sqlalche_obj.session.add(acl_config)
                    #extra_count = extra_count + 1
                    #less_count = less_count + 1
                ...
                if walk.get('.1.3.6.1.4.1.26149.2.2.1.1.5.1') == "rootRU":
                    ...
                    node_type = 3
                get_ru_conf_add = GetOdu16_ru_conf_table(host_id,op_state,walk.get('.1.3.6.1.4.1.26149.2.2.1.1.4.1'),\
                                                         node_type,walk.get('.1.3.6.1.4.1.26149.2.2.1.1.6.1'))

                sqlalche_obj.session.add(get_ru_conf_add)

                get_hw_desc_table = GetOdu16HWDescTable(host_id,walk.get('.1.3.6.1.4.1.26149.2.2.10.1.2.1'),walk.get('.1.3.6.1.4.1.26149.2.2.10.1.3.1'))
                sqlalche_obj.session.add(get_hw_desc_table)

                get_sw_status_table = GetOdu16SWStatusTable(host_id,walk.get('.1.3.6.1.4.1.26149.2.2.6.1.2.1'),walk.get('.1.3.6.1.4.1.26149.2.2.6.1.3.1'),walk.get('.1.3.6.1.4.1.26149.2.2.6.1.4.1'))
                sqlalche_obj.session.add(get_sw_status_table)
                ...
                host_data[0].reconcile_status = 0
                sqlalche_obj.session.commit()
                config_profile_id,reconcile_per =  self.odu16_add_default_config_profile(host_id,device_type_id,table_prefix,reconcile_chk)
    ...
    def odu16_reconcilation_controller_update(self,host_id,device_type_id,table_prefix,insert_update,user_name):
        ...
        host_data[0].reconcile_status = 1
        sqlalche_obj.session.commit()
        result=snmp_walk(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2','-On','-OQ')
...
##                less_count = less_count + 1
            ...
            ru_conf = sqlalche_obj.session.query(SetOdu16RUConfTable).filter(SetOdu16RUConfTable.config_profile_id == device_param_list[0][4]).all()
            ...
            if result.get(".1.3.6.1.4.1.26149.2.2.1.1.9.1"):
                ru_conf[0].country_code = result.get(".1.3.6.1.4.1.26149.2.2.1.1.9.1")

            datetime = sqlalche_obj.session.query(SetOdu16RUDateTimeTable).filter(SetOdu16RUDateTimeTable.config_profile_id==device_param_list[0][4]).all()
            if result.get(".1.3.6.1.4.1.26149.2.2.2.1.2.1"):
                datetime[0].year = result.get(".1.3.6.1.4.1.26149.2.2.2.1.2.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.2.1.3.1"):
                datetime[0].month = result.get(".1.3.6.1.4.1.26149.2.2.2.1.3.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.2.1.4.1"):
            ...
            omc_config= sqlalche_obj.session.query(SetOdu16OmcConfTable).filter(SetOdu16OmcConfTable.config_profile_id==device_param_list[0][4]).all()
            if result.get(".1.3.6.1.4.1.26149.2.2.7.1.2.1"):
                omc_config[0].omc_ip_address = result.get(".1.3.6.1.4.1.26149.2.2.7.1.2.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.7.1.3.1"):
                omc_config[0].periodic_stats_timer = result.get(".1.3.6.1.4.1.26149.2.2.7.1.3.1")

            omc_registration= sqlalche_obj.session.query(SetOdu16SysOmcRegistrationTable).filter(SetOdu16SysOmcRegistrationTable.config_profile_id==device_param_list[0][4]).all()
            if result.get(".1.3.6.1.4.1.26149.2.2.8.1.2.1"):
                omc_registration[0].sys_omc_register_contact_addr =	result.get(".1.3.6.1.4.1.26149.2.2.8.1.2.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.8.1.3.1"):
                omc_registration[0].sys_omc_register_contact_person = result.get(".1.3.6.1.4.1.26149.2.2.8.1.3.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.8.1.4.1"):
                omc_registration[0].sys_omc_register_contact_mobile = result.get(".1.3.6.1.4.1.26149.2.2.8.1.4.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.8.1.5.1"):
                omc_registration[0].sys_omc_register_alternate_contact = result.get(".1.3.6.1.4.1.26149.2.2.8.1.5.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.8.1.6.1"):
                omc_registration[0].sys_omc_register_contact_email = result.get(".1.3.6.1.4.1.26149.2.2.8.1.6.1")

            syn_config= sqlalche_obj.session.query(SetOdu16SyncConfigTable).filter(SetOdu16SyncConfigTable.config_profile_id==device_param_list[0][4]).all()
            if result.get(".1.3.6.1.4.1.26149.2.2.11.1.1.4.1"):
                syn_config[0].raster_time  = result.get(".1.3.6.1.4.1.26149.2.2.11.1.1.4.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.11.1.1.5.1"):
                syn_config[0].num_slaves = 	result.get(".1.3.6.1.4.1.26149.2.2.11.1.1.5.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.11.1.1.6.1"):
                syn_config[0].sync_loss_threshold = result.get(".1.3.6.1.4.1.26149.2.2.11.1.1.6.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.11.1.1.7.1"):
                syn_config[0].leaky_bucket_timer = result.get(".1.3.6.1.4.1.26149.2.2.11.1.1.7.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.11.1.1.8.1"):
                syn_config[0].sync_lost_timeout = result.get(".1.3.6.1.4.1.26149.2.2.11.1.1.8.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.11.1.1.9.1"):
                syn_config[0].sync_config_time_adjust = result.get(".1.3.6.1.4.1.26149.2.2.11.1.1.9.1")

            llc_config= sqlalche_obj.session.query(SetOdu16RALlcConfTable).filter(SetOdu16RALlcConfTable.config_profile_id==device_param_list[0][4]).all()
            llc_config[0].llc_arq_enable = result[".1.3.6.1.4.1.26149.2.2.13.6.1.1.1.1"] 
            llc_config[0].arq_win = result[".1.3.6.1.4.1.26149.2.2.13.6.1.1.2.1"]
            llc_config[0].frame_loss_threshold = result[".1.3.6.1.4.1.26149.2.2.13.6.1.1.3.1"]
            llc_config[0].leaky_bucket_timer_val = result[".1.3.6.1.4.1.26149.2.2.13.6.1.1.4.1"]
            llc_config[0].frame_loss_timeout = result[".1.3.6.1.4.1.26149.2.2.13.6.1.1.5.1"]

            tddmac_config= sqlalche_obj.session.query(SetOdu16RATddMacConfig).filter(SetOdu16RATddMacConfig.config_profile_id==device_param_list[0][4]).all()
            if result.get('.1.3.6.1.4.1.26149.2.2.13.7.1.1.3.1')=='racs24Mbps':
                tddmac_config[0].rfcoding = 1
            else:
                tddmac_config[0].rfcoding = 2
            if result.get(".1.3.6.1.4.1.26149.2.2.13.7.1.1.1.1"):
                tddmac_config[0].rf_channel_frequency = result.get(".1.3.6.1.4.1.26149.2.2.13.7.1.1.1.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.13.7.1.1.4.1"):
                tddmac_config[0].tx_power = result.get(".1.3.6.1.4.1.26149.2.2.13.7.1.1.4.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.13.7.1.1.5.1"):
                tddmac_config[0].max_power = result.get(".1.3.6.1.4.1.26149.2.2.13.7.1.1.5.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.13.7.1.1.6.1"):
                tddmac_config[0].max_crc_errors = result.get(".1.3.6.1.4.1.26149.2.2.13.7.1.1.6.1")
            if result.get(".1.3.6.1.4.1.26149.2.2.13.7.1.1.7.1"):
                tddmac_config[0].leaky_bucket_timer_value = result.get(".1.3.6.1.4.1.26149.2.2.13.7.1.1.7.1")

            peer_config= sqlalche_obj.session.query(SetOdu16PeerConfigTable).filter(SetOdu16PeerConfigTable.config_profile_id==device_param_list[0][4]).all()
            for i in range(0,len(peer_config)):
                if (result.get(".1.3.6.1.4.1.26149.2.2.13.9.1.1.2.1.%s"%(i+1))=='""') or (result.get(".1.3.6.1.4.1.26149.2.2.13.9.1.1.2.1.%s"%(i+1))==''):
                    ...
                    peer_config[i].peer_mac_address = result.get(".1.3.6.1.4.1.26149.2.2.13.9.1.1.2.1.%s"%(i+1))
                    peer_config[i].index=int(i+1)
                else:
                    less_count = less_count + 1


            acl_config= sqlalche_obj.session.query(SetOdu16RAAclConfigTable).filter(SetOdu16RAAclConfigTable.config_profile_id==device_param_list[0][4]).all()


            j=0
            if result.has_key(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1)):
                while(result.has_key(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))):                    
                    if j<len(acl_config):
                        #print result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))
                        if (result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))=='"                  "') or (result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))=='""') or (result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))==''):
                            ...
                            acl_config[j].mac_address = result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))
                            acl_config[j].index=int(j+1)
                        else:                            
                            less_count = less_count + 1
                    else:
                        if (result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))=='"                  "') or (result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))=='""') or (result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1))==''):
                            add_acl_config=SetOdu16RAAclConfigTable(device_param_list[0][4],'',int(j+1))
                            sqlalche_obj.session.add(add_acl_config)
                        else:                            
                            add_acl_config=SetOdu16RAAclConfigTable(device_param_list[0][4],result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(j+1)),int(j+1))
                            sqlalche_obj.session.add(add_acl_config)
                        extra_count = extra_count + 1
            ...
            get_odu16_ru_conf = sqlalche_obj.session.query(GetOdu16_ru_conf_table).filter(Hosts.host_id == host_id).all()
            ...
            if result.get('.1.3.6.1.4.1.26149.2.2.1.1.4.1'):
                get_odu16_ru_conf[0].object_model_version = result.get('.1.3.6.1.4.1.26149.2.2.1.1.4.1')

            if result.get('.1.3.6.1.4.1.26149.2.2.1.1.6.1'):
                get_odu16_ru_conf[0].no_radio_interfaces = result.get('.1.3.6.1.4.1.26149.2.2.1.1.6.1')

            get_odu16_sw_status = sqlalche_obj.session.query(GetOdu16SWStatusTable).filter(Hosts.host_id == host_id).all()
            if len(get_odu16_sw_status)>0:
                get_odu16_sw_status[0].active_version = result.get('.1.3.6.1.4.1.26149.2.2.6.1.2.1')
                get_odu16_sw_status[0].passive_version = result.get('.1.3.6.1.4.1.26149.2.2.6.1.3.1')
                get_odu16_sw_status[0].bootloader_version = result.get('.1.3.6.1.4.1.26149.2.2.6.1.4.1')

            get_odu16_hw_desc = sqlalche_obj.session.query(GetOdu16HWDescTable).filter(Hosts.host_id == host_id).all()
            if len(get_odu16_hw_desc)>0:
                get_odu16_hw_desc[0].hw_version = result.get('.1.3.6.1.4.1.26149.2.2.10.1.2.1')
                get_odu16_hw_desc[0].hw_serial_no = result.get('.1.3.6.1.4.1.26149.2.2.10.1.3.1')
            ...
            el.log_event("Device Reconcilation Done","%s"%(user_name))
            sqlalche_obj.session.commit()            
            result = {"success":0,"result":{reconcile_per:[host_data[0].host_alias,host_data[0].ip_address]}}
            sqlalche_obj.sql_alchemy_db_connection_close()
            return result
...
##                    return "%s Reconcilation Done.Please Reconcile Again"%(reconcile_per)
    ...
    def list_reconciliation(self):
        try:
            ...
            rec_dir = {}
            sqlalche_obj.sql_alchemy_db_connection_open()
            rec_list = sqlalche_obj.session.query(Hosts.host_id,Hosts.reconcile_status,Hosts.reconcile_health).filter(Hosts.device_type_id.like("odu%")).all()
            sqlalche_obj.sql_alchemy_db_connection_close()
    ...
    def reboot(self,host_id,device_type_id):
        ...
        sqlalche_obj.sql_alchemy_db_connection_close()
        if len(host_param)>0:
            snmp_get_result = pysnmp_get('1.3.6.1.4.1.26149.2.2.5.1.7',host_param[0].ip_address,int(host_param[0].snmp_port),host_param[0].snmp_read_community)
            if snmp_get_result['success']==0:
                if int(snmp_get_result['result'].values()[0]) == 1:
                    snmp_get_om_operation = pysnmp_get('1.3.6.1.4.1.26149.2.2.5.1.2',host_param[0].ip_address,int(host_param[0].snmp_port),host_param[0].snmp_read_community)
                    if snmp_get_om_operation['success']==0:
                        ...
                        if 553 in snmp_get_om_operation["result"]:
                            result = {"success":1,"result":"No Response From Device.Please Try Again","flag":"1"}
                        elif 551 in snmp_get_om_operation["result"]:
                            result =  {"success":1,"result":"Network is unreachable","flag":"1"}
                        elif 99 in snmp_get_om_operation["result"]:
                            result = {"success":1,"result":"UNMP Server is busy.Please Retry again","flag":"1"}      
                        else:
                            return snmp_get_om_operation
                        return result
                else:
                    result = pysnmp_set1(host_param[0].ip_address,int(host_param[0].snmp_port),host_param[0].snmp_write_community,oid_dict[1])
                    time.sleep(5)
                    result['flag']=0
...
#print obj.reconcilation_controller(26,'odu100','odu100_',datetime.now(),True,"anuj")
    ...
    def site_survey(self,host_id,node_type,list_of_channels):
        ...
        try:
            ...
            if int(snmp_get_result['success'])==0:
                ...
                #print snmp_get_result['result'].values()[0],"^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
                if int(snmp_get_result['result'].values()[0]) == 2 or int(snmp_get_result['result'].values()[0]) == 0 or int(snmp_get_result['result'].values()[0]) == 3:
                    snmp_result = pysnmp_set({'omoperation':('1.3.6.1.4.1.26149.2.2.5.1.2.1','Integer32',6),'loc':('1.3.6.1.4.1.26149.2.2.5.1.9.1','OctetString',list_of_channels)},device_param_list[0].ip_address,int(device_param_list[0].snmp_port),device_param_list[0].snmp_write_community)        
                    ...
                    if ((snmp_result['success']==1 and 553 in int(snmp_result['result'])) or snmp_result['success']==0):
                        ...
                        for value in temp_result:
                            if temp_result[value] == 0:
                                ...
                                result = {"success":1,"result":"  "+str(errorStatus.get(temp_result[value]))} 
                                return result
                            ...
                            if int(node_type)==1 or int(node_type)==3:
                                ...
                                if snmp_get_result['success']==1:
                                    if 553 in snmp_get_result['result']:
                                        ...
                                        if 551 in snmp_get_result["result"]:
                                            result =  {"success":1,"result":"Device is unreachable at the moment."}
                                            return result
                                        elif 99 in result["result"]:
                                            result = {"success":1,"result":"UNMP has encountered an unexpected error. Please Retry"} 
                                            return result
                                    ...
                                    for key in snmp_get_result['result']:
                                        if int(snmp_get_result['result'][key]) == 1:
                                            ...
                                            if 553 in snmp_get_result["result"]:
                                                ...
                                                result = {"success":1,"result": errorStatus.get(snmp_get_result["result"][key],"")}
                                            result_node_type = False
                                            break
                                    if flag == 0:
                                        time.sleep(10)
...
##                                time.sleep(1)
                            ...
                            if 553 in result["result"]:
                                agent_start(device_param_list[0].ip_address,'root','public')
                                result = {"success":1,"result":"No Response From Device.Please Try Again"}
                            elif 551 in result["result"]:
                                result =  {"success":1,"result":"Network is unreachable"}
                            elif 99 in result["result"]:
                                result = {"success":1,"result":"UNMP has encountered an expected error. Please Retry "}                        
                            else:
                                for i in result['result']:
                                    if result['result'][i] != 0:
                                        result['result'] = str(errorStatus.get(result['result'][i],"SNMP agent Unknown Error Occured"))
                        ...
                        if 553 in snmp_result["result"]:
                            ...
                            result = {"success":1,"result":"UNMP has encountered an eeexpected error. Please Retry"}      
                        else:
                            for i in snmp_result["result"]:
                                if snmp_result["result"][i] != 0:
                                    result = {"success":1,"result":str(errorStatus.get(snmp_result["result"][i],"SNMP agent Unknown Error Occured %s"%(snmp_get_result)))} 
                        return result
                else:
                    #{'result': {'1.3.6.1.4.1.26149.2.2.5.1.7': '2'}, 'success': 0}
                    snmp_get_om_operation = pysnmp_get('1.3.6.1.4.1.26149.2.2.5.1.2',device_param_list[0].ip_address,int(device_param_list[0].snmp_port),device_param_list[0].snmp_read_community)
                        ...
                        if 553 in snmp_get_om_operation["result"]:
                            ...
                            for i in snmp_get_om_operation["result"]:
                                if snmp_get_om_operation["result"][i] != 0:
                                    result = {"success":1,"result":str(errorStatus.get(snmp_get_om_operation["result"][i],"SNMP agent Unknown Error Occured"))} 
                            #result['result'] = str(errorStatus.get(snmp_get_result["result"][i],"SNMP agent Unknown Error Occured"))
                    return result
            else:
                if 553 in snmp_get_result["result"]:
                    agent_start(device_param_list[0].ip_address,'root','public')
                    ...
                    result = {"success":1,"result":"UNMP has encountered an unexpected error. Please retry again"}      
                    return result
                else:
                    for i in snmp_get_result["result"]:
                        if snmp_get_result["result"][i] != 0:
                            result = {"success":1,"result":str(errorStatus.get(snmp_get_result["result"][i],"SNMP agent Unknown Error Occured"))} 
                            #result['result'] = str(errorStatus.get(snmp_get_result["result"][i],"SNMP agent Unknown Error Occured"))
                    return result
        except OperationalError as e:
            return {"success":1,"result":"UNMP Database Server is Busy at the moment, please try again later.","detail":str(e)}
...
def acl_reconcile(host_id,device_type_id,table_prefix,insert_update):
    ...
    if host_id != "" or host_id != None:
        device_param_list = sqlalche_obj.session.query(Hosts.snmp_version_id,Hosts.snmp_read_community,Hosts.ip_address,Hosts.snmp_port,Hosts.config_profile_id,Hosts.reconcile_health,Hosts.snmp_write_community).\
            filter(Hosts.host_id == host_id).all()
    result={}
    result=snmp_walk(device_param_list[0][0],device_param_list[0][1],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.13.5.1.2','-On','-OQ')
    if result=={}:
        result=""
        return result
    else:
        acl_config= sqlalche_obj.session.query(SetOdu16RAAclConfigTable).filter(SetOdu16RAAclConfigTable.config_profile_id==device_param_list[0][4]).delete()
        ...
        if len(result)>0:
            ...
            for i in range(0,j):
                ...
                if result.has_key(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(i+1)):
                    if (result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(i+1))=='"                  "') or (result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(i+1))=='""') or (result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(i+1))==''):
                        add_acl_config = SetOdu16RAAclConfigTable(device_param_list[0][4],'',i+1)
                    else:
                        add_acl_config = SetOdu16RAAclConfigTable(device_param_list[0][4],result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(i+1)) if result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(i+1))==None else result.get(".1.3.6.1.4.1.26149.2.2.13.5.1.2.1.%s"%(i+1)).replace(" ",""),i+1)
                    result_final = 0
                    sqlalche_obj.session.add(add_acl_config)
                else:
                    name=oidname.replace('#',str(i+1))
                    type=oidtype.replace('#','s')
                    oidvalue="                 "
                    set_result = snmp_setmultiple(device_param_list[0][0],device_param_list[0][6],device_param_list[0][2],device_param_list[0][3],'.1.3.6.1.4.1.26149.2.2.13.5.1.3.1.%s'%(i+1),'i','4',name,type,"%s "%(oidvalue))
...
def acl_controller_add_edit(host_id,device_type_id,dic_result,raindex):
    try:  
        ...
        device_param_list = sqlalche_obj.session.query(Hosts.ip_address,Hosts.snmp_port,Hosts.snmp_write_community,Hosts.config_profile_id).\
            filter(Hosts.host_id == host_id).one()        
        odu100_acl_conf_table = sqlalche_obj.session.query(Odu100RaAclConfigTable).filter(Odu100RaAclConfigTable.config_profile_id == device_param_list[3]).order_by(Odu100RaAclConfigTable.aclIndex).all()        
        for keys in dic_result.iterkeys():
            if keys == "success":
                continue
            else:
                ...
                if len(query_result) > 0:                
                    if query_result[0][0].dependent_id == "" or query_result[0][0].dependent_id == None:
                        if keys == "ru.ra.raAclConfigTable.macaddress":                            
                            mac_address_dict.append({keys:[query_result[0][0].oid+"."+str(raindex)+"."+str(dic_result["aclindex"]),query_result[0][0].oid_type,dic_result[keys]]})
                            rowSts.append({"ru.ra.raAclConfigTable.rowSts":["1.3.6.1.4.1.26149.2.2.13.5.1.3"+"."+str(raindex)+"."+str(dic_result["aclindex"]),"Integer32","4"]})                             
                    else:                        
                        if keys == "ru.ra.raConfTable.aclMode":
                            dependent_oid.append({query_result[0][1]:[query_result[0][2]+query_result[0][3]]})
                            acl.append({keys:[query_result[0][0].oid+query_result[0][0].indexes,query_result[0][0].oid_type,dic_result[keys]]})                                       
        if len(odu100_acl_conf_table)>0:
            ...
            add = 0
        if add == 0:   
            result = pysnmp_setAcl(device_param_list[0],device_param_list[1],device_param_list[2],{} if len(acl)==0 else acl[0],{} if len(dependent_oid)==0 else dependent_oid[0],{} if len(mac_address_dict)==0 else mac_address_dict[0],{} if len(rowSts)==0 else rowSts[0])                     
            ...
            if result["success"] == 0 or result["success"] == '0':                                    
                ...
                if len(add_acl_dic)>0:                   
                    sqlalche_obj.session.add(Odu100RaAclConfigTable(device_param_list[3],raindex,dic_result["aclindex"],add_acl_dic[0],1))
                sqlalche_obj.session.commit()
                success_result["result"].update(result["result"])
            else:
                success_result["success"] = 1
            ...
            result = pysnmp_setAcl(device_param_list[0],device_param_list[1],device_param_list[2],{} if len(acl)==0 else acl[0],{} if len(dependent_oid)==0 else dependent_oid[0],{} if len(mac_address_dict)==0 else mac_address_dict[0],{})         
            if result["success"] == 0 or result["success"] == '0':                                
                success_result["success"] = result["success"]
                for i in result["result"]:                    
                    if result["result"][i] != 0:                       
                        ...
                        if len(oid_list_table_field_value) == 0:
                            ...
                            if i in dic_result:
                                ...
                                if table_name == "Odu100RaConfTable":
                                    exec "table_result = sqlalche_obj.session.query(%s).filter(and_(%s.config_profile_id == \"%s\")).all()"%(table_name,table_name,device_param_list[3])   
                                else:
                                    exec "table_result = sqlalche_obj.session.query(%s).filter(and_(%s.config_profile_id == \"%s\",%s.aclIndex == \"%s\")).all()"%(table_name,table_name,device_param_list[3],table_name,int(dic_result["aclindex"]))                                
                                exec "table_result[0].%s = '%s'"%(oid_list_table_field_value[0][1],dic_result[i])
                                sqlalche_obj.session.commit()
                success_result["result"].update(result["result"])
            else:
...
class OduStatus(object):
    ...
    def hw_sw_frequecy_status_chk(self,host_id,device_type):
        ...
        try:
            ...
            sqlalche_obj.sql_alchemy_db_connection_open()
            config_profile_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id==host_id).all()
            if device_type=="odu100":
                sw_status = sqlalche_obj.session.query(Odu100SwStatusTable.activeVersion).filter(Odu100SwStatusTable.host_id==host_id).all()
                hw_status = sqlalche_obj.session.query(Odu100HwDescTable.hwSerialNo).filter(Odu100HwDescTable.host_id==host_id).all()
                channel_list = sqlalche_obj.session.query(Odu100RaPreferredRFChannelTable.rafrequency).\
                    filter(Odu100RaPreferredRFChannelTable.config_profile_id==config_profile_id[0][0]).all()
                ...
                for row in ra_preffered_list:
                    if row['rafrequency']!=0:
                        preffered_channel_dic.update({row['channelNumber']:row['rafrequency']})
            else:
                sw_status = sqlalche_obj.session.query(GetOdu16SWStatusTable.active_version).filter(GetOdu16SWStatusTable.host_id==host_id).all()
                hw_status = sqlalche_obj.session.query(GetOdu16HWDescTable.hw_serial_no).filter(GetOdu16HWDescTable.host_id==host_id).all()
                frequency = sqlalche_obj.session.query(SetOdu16RATddMacConfig.rf_channel_frequency).filter(SetOdu16RATddMacConfig.config_profile_id==config_profile_id[0][0]).all()

            #tddmac_status = sqlalche_obj.session.query(Odu100RaTddMacStatusTable.rfChanFreq).filter(Odu100RaTddMacStatusTable.host_id==host_id).all()
    ...
    def peer_status_chk(self,host_id,device_type):
        ...
        try:
            ...
            peer_dic = {}
            if device_type=="odu100":
                total_peer_count = sqlalche_obj.session.query(func.count(Odu100PeerNodeStatusTable.timeSlotIndex)).filter(Odu100PeerNodeStatusTable.host_id==host_id).group_by(Odu100PeerNodeStatusTable.timestamp).order_by(desc(Odu100PeerNodeStatusTable.timestamp)).limit(1).all()
                peer_status = sqlalche_obj.session.query(Odu100PeerNodeStatusTable.peerMacAddr,Odu100PeerNodeStatusTable.sigStrength1,Odu100PeerNodeStatusTable.timestamp).\
                    filter(and_(Odu100PeerNodeStatusTable.tunnelStatus==1,Odu100PeerNodeStatusTable.linkStatus.in_([1,2]),\
                ...
                total_peer_count = sqlalche_obj.session.query(func.count(GetOdu16PeerNodeStatusTable.timeslot_index)).filter(GetOdu16PeerNodeStatusTable.host_id==host_id).group_by(GetOdu16PeerNodeStatusTable.timestamp).order_by(desc(GetOdu16PeerNodeStatusTable.timestamp)).limit(1).all()
                peer_status = sqlalche_obj.session.query(GetOdu16PeerNodeStatusTable.peer_mac_addr,GetOdu16PeerNodeStatusTable.sig_strength,GetOdu16PeerNodeStatusTable.timestamp).\
                    filter(and_(GetOdu16PeerNodeStatusTable.tunnel_status==1,GetOdu16PeerNodeStatusTable.link_status.in_([1,2]),\
            ...
            if len(peer_status)>0:
                for i in range(0,len(peer_status)):
                    peer_dic.update({'timeslot%s'%(i+1):{'mac_address':peer_status[i].peerMacAddr,'signalstrength':peer_status[i].sigStrength1,'timestamp':datetime.strftime(peer_status[i].timestamp,"%d-%b-%Y %A %I:%M:%S %p")}})
    ...
    def admin_states_data(self,host_id,device_type):
        ...
        try:
            ...
            config_profile_id = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id==host_id).all()
            if device_type=="odu100":
                ru_data = sqlalche_obj.session.query(Odu100RuConfTable.adminstate).filter(Odu100RuConfTable.config_profile_id==config_profile_id[0][0]).all()
                ra_data = sqlalche_obj.session.query(Odu100RaConfTable.raAdminState).filter(Odu100RaConfTable.config_profile_id==config_profile_id[0][0]).all()
                sync_data = sqlalche_obj.session.query(Odu100SyncConfigTable.adminStatus).filter(Odu100SyncConfigTable.config_profile_id==config_profile_id[0][0]).all()
            else:
                ru_data = sqlalche_obj.session.query(SetOdu16RUConfTable.adminstate).filter(SetOdu16RUConfTable.config_profile_id==config_profile_id[0][0]).all()
                ra_data = sqlalche_obj.session.query(SetOdu16RAConfTable.ra_admin_state).filter(SetOdu16RAConfTable.config_profile_id==config_profile_id[0][0]).all()
                sync_data = sqlalche_obj.session.query(SetOdu16SyncConfigTable.admin_status).filter(SetOdu16SyncConfigTable.config_profile_id==config_profile_id[0][0]).all()
    ...
    def all_lock_unlocked(self,host_id,device_type,admin_state_name,state):
        ...
        try:
            ...
            for i in range(0,len(admin_state_list)):
                oid_data = sqlalche_obj.session.query(Oids.oid,Oids.indexes,Oids.oid_type).filter(Oids.oid_name==admin_state_list[i]).all()
                if admin_state_list[i]=="ru.ruConfTable.adminstate":
                    ru_admin = 1                    
                    ru_dic = {admin_state_list[i]:[str(oid_data[0].oid)+str(oid_data[0].indexes),oid_data[0].oid_type,state]}
                else:
                    oid_dic.update({admin_state_list[i]:[str(oid_data[0].oid)+str(oid_data[0].indexes),oid_data[0].oid_type,state]})
            snmp_result = pysnmp_seter_ap(oid_dic,host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_write_community)
            if snmp_result['success']==0:
                for i in snmp_result['result']:
                    if snmp_result['result'][i]==0:
                        if i=="ru.syncClock.syncConfigTable.adminStatus":
                            ...
                            ra_data[0].raAdminState = state
                if ru_admin == 1:
                    ru_result = pysnmp_seter_ap(ru_dic,host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_write_community)
                    if int(state) == 1:
                        if int(ru_result['success'])==1:
                            ...
                            snmp_result['result'].update({'ru.ruConfTable.adminstate':ru_result["result"]}) 
                        else:
                            if device_type==UNMPDeviceType.odu100:
                                ru_data = sqlalche_obj.session.query(Odu100RuConfTable).filter(Odu100RuConfTable.config_profile_id==host_data[0].config_profile_id).all()
                            else:
                            ...
                            for i in ru_result['result']:
                                if ru_result['result'][i]==0:
                                    if device_type==UNMPDeviceType.odu100:
                                        ...
                                        ru_data = sqlalche_obj.session.query(SetOdu16RUConfTable).filter(SetOdu16RUConfTable.config_profile_id==host_data[0].config_profile_id).all()
                                    snmp_result['result'].update({'ru.ruConfTable.adminstate':0})
                                else:
                                    for i in ru_result["result"]:
                                        if ru_result["result"][i] != 0:
                                            if ru_result["result"].get(i) in errorStatus:                                                
                                                ru_result["result"] = errorStatus[ru_result["result"][i]]
                                    snmp_result['result'].update({'ru.ruConfTable.adminstate':ru_result["result"]}) 
                        else:
                            for i in ru_result["result"]:
                                if ru_result["result"][i] != 0:
                                    if i in errorStatus:
                                        ...
                                        ru_result["result"] = "SNMP unknown error occurred"
                            snmp_result['result'].update({'ru.ruConfTable.adminstate':ru_result["result"]}) 
    ...
    def global_admin_request(self,host_id):
        try:
            ...
            for i in range(0,len(host_id_list)):
                ...
                peer_status = []
                master_host_data = []
                host_data = sqlalche_obj.session.query(Hosts.config_profile_id,Hosts.device_type_id,Hosts.host_asset_id).filter(Hosts.host_id==host_id_list[i]).all()
                if len(host_data)>0:
                    if host_data[0].device_type_id == UNMPDeviceType.odu16:
                        ...
                        get_default_node_type = sqlalche_obj.session.query(GetOdu16_ru_conf_table.default_node_type).\
                            filter(GetOdu16_ru_conf_table.host_id==host_id_list[i]).all()
                        ru_admin_data = sqlalche_obj.session.query(SetOdu16RUConfTable.adminstate).\
                            filter(SetOdu16RUConfTable.config_profile_id==host_data[0].config_profile_id).all()

                        sync_admin_data = sqlalche_obj.session.query(SetOdu16SyncConfigTable.adminStatus).\
                            filter(SetOdu16SyncConfigTable.config_profile_id==host_data[0].config_profile_id).all()

                        ra_admin_data = sqlalche_obj.session.query(SetOdu16RAConfTable.raAdminState).\
                            filter(SetOdu16RAConfTable.config_profile_id==host_data[0].config_profile_id).all()
                        ...
                        if len(get_default_node_type)>0:
                            if int(get_default_node_type[0].default_node_type) ==1 or int(get_default_node_type[0].default_node_type) ==3:
                                master_host_id = sqlalche_obj.session.query(MasterSlaveLinking.master).filter(MasterSlaveLinking.slave == host_id_list[i]).all()
                                if len(master_host_id)>0:
                                    master_host_data = sqlalche_obj.session.query(Hosts.host_alias,Hosts.host_asset_id).filter(Hosts.host_id==master_host_id[0][0]).all()
                                    host_asset_data = sqlalche_obj.session.query(HostAssets.ra_mac).filter(HostAssets.host_asset_id==master_host_data[0].host_asset_id).all()
                                    master_mac = str(host_asset_data[0].ra_mac if len(host_asset_data)>0 else "")
                                    peer_status = sqlalche_obj.session.query(GetOdu16PeerNodeStatusTable.sig_strength,GetOdu16PeerNodeStatusTable.link_status).\
                                        filter(GetOdu16PeerNodeStatusTable.host_id==host_id_list[i]).order_by(desc(GetOdu16PeerNodeStatusTable.timestamp)).limit(1).all()


                    else:
                        ru_admin_data = sqlalche_obj.session.query(Odu100RuConfTable.adminstate,Odu100RuConfTable.defaultNodeType).\
                            filter(Odu100RuConfTable.config_profile_id==host_data[0].config_profile_id).all()
                        print ru_admin_data
                        sync_admin_data = sqlalche_obj.session.query(Odu100SyncConfigTable.adminStatus).\
                            filter(Odu100SyncConfigTable.config_profile_id==host_data[0].config_profile_id).all()
                        print sync_admin_data

                        ra_admin_data = sqlalche_obj.session.query(Odu100RaConfTable.raAdminState).\
                            filter(Odu100RaConfTable.config_profile_id==host_data[0].config_profile_id).all()
                        print ra_admin_data

                        ru_status = sqlalche_obj.session.query(Odu100RuStatusTable.ruoperationalState).filter(Odu100RuStatusTable.host_id==host_id_list[i]).limit(1).all()
                        print ru_status,"^^^^"
                        ra_status = sqlalche_obj.session.query(Odu100RaStatusTable.raoperationalState).filter(Odu100RaStatusTable.host_id==host_id_list[i]).order_by(desc(Odu100RaStatusTable.timestamp)).limit(1).all()
                        print ra_status,"^&*"
                        sync_status = sqlalche_obj.session.query(Odu100SynchStatusTable.syncoperationalState).filter(Odu100SynchStatusTable.host_id==host_id_list[i]).order_by(desc(Odu100SynchStatusTable.timestamp)).limit(1).all()
                        print sync_status,"%^&*"
                        if len(ru_status)>0:
                            if ru_status[0].ruoperationalState==None:
                                ru_op_state = 1
                        ...
                        if int(ru_admin_data[0].defaultNodeType) == 1 or int(ru_admin_data[0].defaultNodeType) == 3:
                            ...
                            if len(master_host_id)>0:
                                ...
                                master_mac = str(host_asset_data[0].ra_mac if len(host_asset_data)>0 else "")+","
                                peer_status = sqlalche_obj.session.query(Odu100PeerNodeStatusTable.sigStrength1,Odu100PeerNodeStatusTable.linkStatus).\
                                    filter(Odu100PeerNodeStatusTable.host_id==host_id_list[i]).order_by(desc(Odu100PeerNodeStatusTable.timestamp)).limit(1).all()
                    ...
                    if host_data[0].device_type_id == UNMPDeviceType.odu16:
                        ...
                        if len(peer_status)>0:
                            if peer_status[0].sig_strength==None:
                                temp_list.append(str(master_host_data[0].host_alias)+"(-)")
                            elif int(peer_status[0].sig_strength)==1111111:
                                temp_list.append(str(master_host_data[0].host_alias)+" (Device Unreachable)")
                            else:
                                if peer_status[0].link_status==1:
                                    temp_list.append(str(master_host_data[0].host_alias)+"( Link Disconnected )")
                                else:
                                    temp_list.append(str(master_host_data[0].host_alias)+" ("+str(peer_status[0].sig_strength)+"dBm)")
                        else:
                            temp_list.append("")
                    else:
                        if len(ru_admin_data)>0:
                        ...
                        if len(peer_status)>0:
                            if peer_status[0].sigStrength1==None:
                                ...
                                if int(peer_status[0].linkStatus)==1:
                                    ...
                                    temp_list.append(str(master_host_data[0].host_alias)+" ("+str(peer_status[0].sigStrength1)+"dBm)")
                        else:
                            temp_list.append("")
...
def get_modulation_rate(host_id):
    ...
    host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id == host_id).all()
    sqlalche_obj.sql_alchemy_db_connection_close()
    modulation_rate_result = bulktable('1.3.6.1.4.1.26149.2.2.13.10.1',host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community,20)
    if modulation_rate_result['success']==1:
        for i in modulation_rate_result['result']:
            modulation_rate_result['result'] = errorStatus.get(i,"Device is busy.Please try again")
...
def refresh_channel_freq_list_table(host_id,device_type):
    ...
    try:
        ...
        if len(host_data)>0:
            ...
            while(i<8):
                time.sleep(2)
                #snmp_get_result = bulktable('1.3.6.1.4.1.26149.2.2.13.3.1',host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community,20)
                flag = 0
                if snmp_get_result['success']==0:
                    channel_list = sqlalche_obj.session.query(Odu100RaChannelListTable).filter(Odu100RaChannelListTable.host_id==host_id).delete()
                    sqlalche_obj.session.commit()
                    for i in snmp_get_result['result']:                        
                        channel_add = Odu100RaChannelListTable(host_id,snmp_get_result['result'][i][0],snmp_get_result['result'][i][1],snmp_get_result['result'][i][2],snmp_get_result['result'][i][3]) 
                        preferred_channel_dic.setdefault('name',[]).append(snmp_get_result['result'][i][3])
                        preferred_channel_dic.setdefault('value',[]).append(snmp_get_result['result'][i][3])
                    ...
                    if 553 in snmp_get_result['result']:
                        ...
                        for i in snmp_get_result['result']:
                            if snmp_get_result[i]!=0:
                                snmp_result['result'] = errorStatus.get(i,"Device is busy.Please try again Later")
                        result['success']=1
            if flag == 1:
                result['success'] = 1 
                result['result'] =  flag                  
    ...
    try:
        ...
        if snmp_get_result['success']==0:
            ...
            for i in snmp_get_result["result"]:
                    #return snmp_result
                if i==553:
                    result["result"] = str(host_data[0].host_alias)+" ("+str(host_data[0].ip_address)+") "+str(errorStatus.get(i,"SNMP agent Unknown Error Occured"))
                elif i==551:
                    result["result"] = str(host_data[0].host_alias)+" ("+str(host_data[0].ip_address)+") "+str(errorStatus.get(i,"SNMP agent Unknown Error Occured")) 
                elif snmp_get_result["result"][i] != 0:
                    result["result"] = str(host_data[0].host_alias)+" ("+str(host_data[0].ip_address)+") "+str(errorStatus.get(snmp_get_result["result"][i],"SNMP agent Unknown Error Occured"))
            result['success']=1
    except Exception as e:
        result = {'result':"UNMP has encountered an error.Please retry again %s"%(e),'success':1}
    finally:
        sqlalche_obj.sql_alchemy_db_connection_close()
        return result
...
def bw_get_value(host_id,device_type):
    ...
    try:
        sqlalche_obj.sql_alchemy_db_connection_open()
        device_param_list = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id==host_id).all()
        get_dic = {'tx_rate':'1.3.6.1.4.1.26149.2.2.5.1.11.1','tx_time':'1.3.6.1.4.1.26149.2.2.5.1.10.1','tx_bw':'1.3.6.1.4.1.26149.2.2.5.1.12.1'}
        snmp_get = pysnmp_geter(get_dic,device_param_list[0].ip_address,int(device_param_list[0].snmp_port),device_param_list[0].snmp_read_community)
        ...
        if int(snmp_get['success'])==0:                                    
            ...
            for i in snmp_get['result']:
                result = {'success':1,'result':errorStatus.get(i,"SNMP Agent Down ")}
                return
    except Exception as e:
        result['success']=1
        result['result']=str(e)
    ...
    try:
        ...
        if len(device_param_list)>0:
            ...
            if snmp_get_result['success']==0:
                    ...
                    if int(snmp_get_result['result'][i])==1:
                        ...
                        for i in dic_result:
                            if i=="success":
                                continue
                            if i=="tx_rate":
                                oid_dic.update({i:['1.3.6.1.4.1.26149.2.2.5.1.11.1','Integer32',dic_result[i]]})
                            if i=="tx_time":
                                oid_dic.update({i:['1.3.6.1.4.1.26149.2.2.5.1.10.1','Integer32',dic_result[i]]})
                        snmp_set_result =  pysnmp_set(oid_dic,device_param_list[0].ip_address,int(device_param_list[0].snmp_port),device_param_list[0].snmp_write_community)
                        #print snmp_set_result,"&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"
                        if int(snmp_set_result['success'])==0:
                            for i in snmp_set_result['result']:
                                if snmp_set_result['result'][i]!=0:
                                    ...
                                    continue
                            snmp_om_set_result =  pysnmp_set({'om_operation':['1.3.6.1.4.1.26149.2.2.5.1.2.1','Integer32',7]},device_param_list[0].ip_address,device_param_list[0].snmp_port,device_param_list[0].snmp_write_community)
                            #print snmp_om_set_result,"^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
                ...
                for i in snmp_get_result['result']:
                    if int(i)==553:
                        result['result'] = errorStatus.get(i,"Device is busy,please try again later")
                    elif int(i)==551:
                        result['result'] = errorStatus.get(i,"Device is busy,please try again later")
                    elif snmp_get_result['result'][i]!=0:
                        result['result'] = errorStatus.get(snmp_get_result['result'][i],"Device is busy,please try again later ")
                    else:
                        result['result'] = errorStatus.get(i,"Device is not responding")
                    result['success']=1
                    return
...
def get_site_survey_bll(host_id):
    ...
    try:
        ...
        result = bulktable('1.3.6.1.4.1.26149.2.2.13.11.1',device_param_list[0].ip_address,int(device_param_list[0].snmp_port),device_param_list[0].snmp_read_community)
        #print result
        #result = {'result': {1: ['1', '1', '1', '5180', '0', '-115', '0', '-115', '-115', '36', '-115']}, 'success': 0}
        if result['success']==0: 
            sqlalche_obj.db.execute("delete from odu100_raSiteSurveyResultTable where host_id='%s'"%(host_id))
            sqlalche_obj.session.commit()
            time.sleep(1)
            for i in range(0,len(result["result"])):                
                ...
                #print "Insert into %s values (NULL,%s,%s)"%('odu100_raSiteSurveyResultTable',host_id,str(result["result"][i+1])[1:-1])
                sqlalche_obj.db.execute("Insert into %s values (NULL,%s,%s)"%('odu100_raSiteSurveyResultTable',host_id,str(result["result"][i+1])[1:-1]))
            sqlalche_obj.session.commit()
            result={'success':0}
        else:
        ...
        result =  {"success":1,"result":"UNMP Database Server is Busy at the moment, please try again later.","detail":str(e)}
    except DisconnectionError as e:
        result = {"success":1,"result":"Database Disconnected","detail":""}
    except Exception as e:
        result = {'success':1,'result':"UNMP Web Server is Busy at the moment, please try again later %s"%(e)}
