...
def history_report(h):
    ...
    js_list = ["js/calendrical.js","js/jquery-ui-1.8.6.custom.min.js","js/pages/jquery.multiselect.min.js","js/pages/jquery.multiselect.filter.js","js/jquery.dataTables.min.js","js/pages/history_reporting2.js"]
    html.new_header("Backup & Restore","history_report.py","",css_list,js_list)
    r=HistoryReportBll()
...
def main_report(h):
    ...
    css_list = ["css/demo_table_jui.css","css/calendrical.css","css/fcbkcomplete.css","css/style12.css","css/jquery.multiselect.css","css/jquery.multiselect.filter.css","css/jquery-ui-1.8.4.custom.css"]
    js_list = ["js/calendrical.js","js/jquery-ui-1.8.6.custom.min.js","js/pages/jquery.multiselect.min.js","js/pages/jquery.multiselect.filter.js","js/jquery.dataTables.min.js","js/pages/history_reporting.js"]
    html.new_header("Backup & Restore","history_report.py","",css_list,js_list)
    ...
    if(hostgroup_id_list==[]):
        ...
        result=r.get_hostgroup_device(user_id,hostgroup_id_list,db_historical)
        if (result["success"]==0):
            html.write(Report.list_form(result["result"],result["host_data"],device_type_user_selected_id,device_type_user_selected_name,month_var))
        else:
            pass
    html.new_footer()
...
def history_reporting_make_db(h):
    ...
    result=r.history_reporting_make_db(user_id,user_name,month_var,db_historical)
...
def history_reporting_get_host_data(h):
    ...
    #db_historical=html.req.session["db_historical"]
    db_historical=user_id.replace("-","_")+"_db"
    result=r.history_reporting_get_host_data(hostgroup_id,device_type_id,db_historical)
    column_result=r.history_reporting_get_column_template(device_type_id,report_type,db_historical)
    selected_columns=[]
...
#    no_of_devices=str(html.var("no_of_devices"))
    ...
    ####
    result=r.history_reporting_get_excel(date_start,date_end,time_start,time_end,all_host,report_type,column_value,device_type,hostgroup,view_type,
