...
oid_dict = {'peerNodeStatus':'1.3.6.1.4.1.26149.2.2.13.9.2.1','peerconfig':'1.3.6.1.4.1.26149.2.2.13.9.1.1'}
...
class FirmwareUpdate(object):
    ...
    def get_node_type(self,host_id,device_type):
        ...
        if len(host_data)>0:
            config_profile_id = "" if host_data[0].config_profile_id == None else host_data[0].config_profile_id
        if device_type == UNMPDeviceType.odu16:
            default_node_type = sqlalche_obj.session.query(GetOdu16_ru_conf_table.default_node_type).filter(GetOdu16_ru_conf_table.host_id == host_id).all()
        elif device_type == UNMPDeviceType.odu100:
            default_node_type = sqlalche_obj.session.query(Odu100RuConfTable.defaultNodeType).filter(Odu100RuConfTable.config_profile_id == config_profile_id).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
    ...
    def get_master_host_id(self,host_id):
        ...
        sqlalche_obj.sql_alchemy_db_connection_open()
        master_host_id = []
        master_host_id = sqlalche_obj.session.query(MasterSlaveLinking.master).filter(MasterSlaveLinking.slave==host_id).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
    ...
    def get_master_slave_dic(self,master_host_id,master_slave_linking):
        ...
        if len(master_slave_linking)>0:
            for i in range(0,len(master_slave_linking)):
                slave_host_data = sqlalche_obj.session.query(Hosts.ip_address,Hosts.mac_address,Hosts.host_name,Hosts.host_asset_id).filter(Hosts.host_id==master_slave_linking[i][0]).all()
                slave_firmware_data = sqlalche_obj.session.query(HostAssets.firmware_status,HostAssets.firmware_type,HostAssets.firmware_file_name,HostAssets.firmware_file_path).filter(HostAssets.host_asset_id == slave_host_data[0].host_asset_id).all()
                master_slave_list.append({'node_type':1,'ip_address':slave_host_data[0].ip_address if len(slave_host_data)>0 else "--",\
        ...
        host_data = sqlalche_obj.session.query(Hosts.ip_address,Hosts.snmp_port,Hosts.snmp_read_community,Hosts.mac_address,Hosts.host_name).filter(Hosts.host_id == master_host_id).all()
        request_ping = snmp_ping(host_data[0].ip_address,host_data[0].snmp_read_community,int(host_data[0].snmp_port))
        ...
        master_host_data = sqlalche_obj.session.query(Hosts.ip_address,Hosts.mac_address,Hosts.host_name,Hosts.host_asset_id).filter(Hosts.host_id==master_host_id).all()
        master_firmware_data = sqlalche_obj.session.query(HostAssets.firmware_status,HostAssets.firmware_type,HostAssets.firmware_file_name,HostAssets.firmware_file_path).filter(HostAssets.host_asset_id == master_host_data[0].host_asset_id).all()
        master_slave_list.append({'node_type':0,'ip_address':master_host_data[0].ip_address if len(master_host_data)>0 else "--",\
    ...
    def get_master_slave_linking(self,master_id):
        ...
        master_slave_linking = sqlalche_obj.session.query(MasterSlaveLinking.slave).filter(MasterSlaveLinking.master== master_id).all()
        master_slave_list = self.get_master_slave_dic(master_id,master_slave_linking)
        return master_slave_list
    ...
    def get_master_slave_snmp(self,master_host_id,device_type):
        ...
        result = 1
        macAddress = ""
        host_data = sqlalche_obj.session.query(Hosts.ip_address,Hosts.snmp_port,Hosts.snmp_read_community,Hosts.mac_address,Hosts.host_name,Hosts.mac_address).filter(Hosts.host_id == master_host_id).all()
        if len(host_data)>0:
            request_ping = snmp_ping(host_data[0].ip_address,host_data[0].snmp_read_community,int(host_data[0].snmp_port))
            if request_ping == 0:
                result = 1
            ...
            if len(master_slave_list)>0:
                result_peer_node_status = pysnmp_get_table(oid_dict['peerNodeStatus'],host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community)                    
                if result_peer_node_status["success"]==0:
                                ...
                                if j["node_type"]==1:
                                    if result_peer_node_status["result"][i][5]!="":
                                        ...
                                        if macAddress in result_peer_node_status["result"][i]:
                                            mac_address_dic[j["mac_address"]] =[result_peer_node_status["result"][i][2],result_peer_node_status["result"][i][3]]
                                        else:
                                            continue
                        ...
                        for i in master_slave_list:
                            if i["node_type"]==1:
                                if i["mac_address"] in mac_address_dic:
                                    if device_type == UNMPDeviceType.odu16:
                                        slave_host_id = sqlalche_obj.session.query(GetOdu16RaStatusTable.host_id).filter(GetOdu16RaStatusTable.ra_mac_address == '%s'%(i["mac_address"])).all()
                                    else:
                                        slave_host_id = sqlalche_obj.session.query(Odu100RaStatusTable.host_id).filter(Odu100RaStatusTable.raMacAddress == '%s'%(i["mac_address"])).all()
                                if len(slave_host_id)>0:
                                        slave_host_data = sqlalche_obj.session.query(Hosts.ip_address,Hosts.mac_address,Hosts.host_name,Hosts.host_asset_id).filter(Hosts.host_id==slave_host_id[0].host_id).all()
                                        slave_firmware_data = sqlalche_obj.session.query(HostAssets.firmware_status,HostAssets.firmware_type,HostAssets.firmware_file_name,HostAssets.firmware_file_path).filter(HostAssets.host_asset_id == slave_host_data[0].host_asset_id).all()                                
                                        slave_list.append({'node_type':1,'ip_address':slave_host_data[0].ip_address if len(slave_host_data)>0 else "--",\
                                                        'mac_address':slave_host_data[0].mac_address if len(slave_host_data)>0 else "--",\
                                    ...
                                    discovered_host_exist = sqlalche_obj.session.query(DiscoveredHosts).filter(DiscoveredHosts.mac_address=='%s'%(i["mac_address"])).all()
                                    tcp_discovery_host_exist = sqlalche_obj.session.query(TcpDiscovery).filter(TcpDiscovery.site_mac=='%s'%(i["mac_address"])).all()
                                    if len(discovered_host_exist)>0 or len(tcp_discovery_host_exist)>0:
                                        found = 1
                                    else:
                                        found = 2
                                    slave_list.append({'node_type':1,'ip_address':'',\
                                                        'mac_address':'',\
                                                        'link_status':mac_address_dic[i["mac_address"]][0],\
                                                        'tunnel_status':mac_address_dic[i["mac_address"]][1],\
                                                        'firmware_status':"--",\
                    ...
                    master_slave_update_list = self.get_master_slave_update(master_host_id,master_slave_list,slave_list)
                else:
                    flag = 1
    ...
    def select_firmware_table(self,device_type):
        ...
        sqlalche_obj.sql_alchemy_db_connection_open()
        firmware_table=[]
        firmware_table = sqlalche_obj.session.query(FirmwareListTable.firmware_file_name,FirmwareListTable.firmware_file_path).filter(FirmwareListTable.device_type == device_type).all()
        sqlalche_obj.sql_alchemy_db_connection_close()
    ...
    def ftp_upload(self,file_path,device_type,filename):
        ...
        try:
            try:
                ...
                #f = ftplib.FTP(ftp_credentials["ip"],ftp_credentials["username"],ftp_credentials["password"])
            except ftplib.all_errors, e:
                errorcode_string = str(e).split(None, 1)
                final_result = {"success":1,"result":errorcode_string[1]+" Please Retry Again"}
            else:
                try:
                ...
                try:
                    result = f.storbinary('STOR /unmp-ftp/%s/%s'%(device_type,filename), fp)
                except ftplib.all_errors, e:
                    final_result = {"success":1,"result":"File not uploaded successfully.Please retry again"}
                if result == "226 Transfer complete.":
                    final_result = {"success":0,"result":"File Uploaded SuccessFully"}
                    ftp_table = sqlalche_obj.session.query(FirmwareListTable).filter(and_(FirmwareListTable.device_type==device_type,\
                                                           FirmwareListTable.firmware_file_name==filename)).all()
                    if len(ftp_table)==0:
                        table_insert = FirmwareListTable(device_type,filename,file_path)
...
##print obj.ftp_upload("/omd/sites/nms3/share/check_mk/web/htdocs/download/firmware_downloads/odu100/aa.img","odu100","aa.img")