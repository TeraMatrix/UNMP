...
class SPDashboardView(object):
    ...
    def sp_ap_client_table_view(result):
        ...
        for i in new_result:
            ...
            Client_table+='<td>%s</td>'%str("--" if i[1]==None or i[1]=="" else i[1])
            Client_table+='<td>%s</td>'%str("--" if i[2]==None or i[2]=="" else i[2])
            Client_table+='<td>%s</td>'%str("--" if i[3]==None or i[3]=="" else 0-int(i[3]))
            Client_table+='<td>%s</td>'%str("--" if i[4]==None or i[4]=="" else i[4])
            Client_table+='<td>%s</td>'%str("--" if i[5]==None or i[5]=="" else i[5])
            Client_table+='<td>%s</td>'%str("--" if i[6]==None or i[6]=="" else i[6])
            Client_table+='<td>%s</td>'%str("--" if i[7]==None or i[7]=="" else i[7])
            Client_table+='<td>%s</td>'%str("--" if i[8]==None or i[8]=="" else i[8])
            Client_table+='<td>%s</td>'%str("--" if i[9]==None or i[9]=="" else i[9])
            Client_table+='<td><img src="images/%s" alt="Yes" title="Status"/></td></tr>'%str("host_status_ap_1.png" if i[10]==None or i[10]=="" or i[10]=="No"  else "host_status_ap_0.png")
            count_no+=1
        if len(new_result)<1:
    ...
    def sp_event_alarm_table_view(result,ip_address):
        ...
        image_dic={0:"images/gr.png",1:"images/lb.png",2:"images/gr.png",3:"images/yel.png",4:"images/or.png",5:"images/red.png"}
        ...
        for i in range(length):
            if i<6:
                ...
                history_trap+='<td>%s</td>'%all_traps[i][2]
                history_trap+='<td>%s</td>'%all_traps[i][3]
                history_trap+='<td>%s&nbsp&nbsp&nbsp&nbsp%s</td></tr>'%(all_traps[i][6],(("<a href=\"status_snmptt.py?trap_status=history&ip_address="+ip_address+"\">more>></a>" if len(all_traps) >6 else "" )))
