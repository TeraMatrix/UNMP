...
import shelve
import subprocess,os,tarfile,calendar,time,csv
...
#            cur_db="nmsp"
            ...
            path_backup='/omd/daemon/mysql_backup/%s_%s.tar.bz2' %(year,month) 
	    ...
	    path_sh='/omd/daemon/mysql_scripts/mysql_restore.sh' 
            proc=subprocess.Popen(['sh',path_sh,path_temp,db_backup,cur_db], stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
...
#            cur_db="nmsp"
            ...
            proc=subprocess.Popen(['sh',path_sh,path_temp,str(str_time),cur_db], stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
	    output, err = proc.communicate()
            buf_data = output
            #code = proc.wait()
            #bufData = proc.stdout.read()
...
#	    time.sleep(5)
    ...
    def check_db_status_historical(self,user_id):
    	try: 
    	    ...
    	    cursor=db.cursor()
    	    time.sleep(1)
    	    sql="select status from historical_info where user_id='%s' "%(user_id)
    	    cursor.execute(sql)
    ...
    def update_db_status_historical(self,user_id):
    	try: 
    	    db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
    	    cursor=db.cursor()
    	    sql="update historical_info set status=3 where user_id='%s' "%(user_id)
    	    cursor.execute(sql)
    	    db.commit()
    	    db.close()
            return {'success':0,'result':'0'}
    ...
    def set_last_access_time_historical(self,user_id):
    	try: 
    	    db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
    	    cursor=db.cursor()
    	    sql="update historical_info set last_access_time='%s' where user_id='%s' "%(datetime.now(),user_id)
    	    cursor.execute(sql)
    	    db.commit()
    	    db.close()
            return {'success':0,'result':'0'}
    ...
    def get_year_month_historical(self,user_id):
    	try: 
    	    db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
    	    cursor=db.cursor()
    	    sql="select `year_month` from historical_info where user_id='%s' "%(user_id)
    	    cursor.execute(sql)
    	    result=cursor.fetchall()
    	    db.close()
    	    if result!=():
    ...
    def clear_data_historical(self,user_id):
    	try: 
    	    db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
    	    cursor=db.cursor()
    	    sql="select database_name from historical_info where user_id='%s' "%(user_id)
    	    cursor.execute(sql)
    	    result=cursor.fetchall()
    	    if result!=():
    	    	try:
    ...
    def get_year_month_status_historical(self,user_id):
    	try: 
    	    db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
    	    cursor=db.cursor()
    	    sql="select `year_month`,status from historical_info where user_id='%s' "%(user_id)
    	    cursor.execute(sql)
    	    result=cursor.fetchall()
    	    db.close()
    	    if result!=():
    ...
    def history_reporting_make_db(self,user_id,user_name,month_var,db_name):
		...
		return {'success':5,'result':filename}
    	    sql="INSERT INTO historical_info (historical_info_id, user_id, user_name, database_name, `year_month`, start_time, status, last_access_time) VALUES(NULL,'%s','%s','%s','%s','%s','%s','%s')"%(user_id,user_name,db_name,month_var,datetime.now(),0,datetime.now())
    	    cursor.execute(sql)
    	    db.commit()
            res=self.restore_backup(month_var,db_name)
            
            if res.find("ok")!=-1:
                sql="UPDATE historical_info set status=1 , last_access_time='%s' where user_id='%s' " %(datetime.now(),user_id)
                cursor.execute(sql)
                db.commit()
                db.close()
                return {'success':0,'result':'1','data':str(res)}
            else:
                sql="UPDATE historical_info set status=2 , last_access_time='%s' where user_id='%s' " %(datetime.now(),user_id)
                cursor.execute(sql)
                db.commit()            
            	db.close()
    ...
    view_type,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,new_user_report_dict,db_historical,username):
	    ...
	    nms_instance = __file__.split("/")[3]       # it gives instance name of nagios system
            ...
            result_columns=re.history_reporting_get_mapping_column(device_type,report_type,db_historical)
            column_value=[]
            column_key=[]
            key_user=[]
            data_report={}
		...
		# but we use column_user for generating report headings
			...
			outage_data=get_outage(3000,date1,date2,time1,time2,all_group,all_host,db_historical)
			...
			trap_data=get_total_data_trap(3000,date1,date2,time1,time2,all_group,all_host,db_historical)	
			    ...
			    return trap_data
                    ...
                    query_dict=re.get_query_dict(device_type,report_type,date_temp_1,date_temp_2,all_host,db_historical)
	            if(str(query_dict["success"])=="0"):
	                ...
	                if(view_type=="data_table"):
	                    ...
	                    if flag_dict==True:
	                        ...
	                        res_sql=re.get_sql_data(query_dict["result"],date_temp_1,date_temp_2,db_historical)
			...
			#######################################-----FORM THE DEFAULT DATA
	                if str(query_dict["default_data"])=="1":
                            ...
                            if(len(var_default)==4):
                            	#4 . find the delta
                        	res_default=re.form_the_default_data(data_report,int(var_default[0]),int(var_default[1]),int(var_default[2]),int(var_default[3]))
