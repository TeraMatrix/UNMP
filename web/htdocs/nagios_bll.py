...
import MySQLdb , subprocess , json, shelve, os
    ...
    def nagios_force_sync(self):
        try:
            ...
            for db_hg in hostgroup_db.keys():
                if hostgroup_config.has_key(db_hg):# hostgroup is in .cfg and DB. update name and alias
                    hostgroup_config[db_hg].update(hostgroup_db[db_hg])
                else:
                    hostgroup_config[db_hg]=hostgroup_db[db_hg]# hostgroup is in DB. Add in cfg
                
            for config_hg in hostgroup_config.keys():
                if hostgroup_db.has_key(config_hg):# hostgroup is in .cfg and DB. pass
                    pass
                else:
                    hostgroup_config.pop(config_hg)# hostgroup is in .cfg only. remove.        
                
            shelve_name='hostgroup'
            file_name='hostgroups.cfg'
            updated_dict=hostgroup_config
            attribute="hostgroup"
            rt = nagios_force_sync_parser(shelve_name,file_name,updated_dict,attribute)
            if rt['success']==1:
            ...
            if(hosttemplate_config.has_key('generic-host')):
                pass
            else:
                ...
                # hosttemplate_config
                # hosttemplate_db
            default_template_list=['generic-host','check_mk_host','check_mk_cluster','host-pnp',
                                   'check_mk_default','check_mk_host-summary','check_mk_cluster-summary']
            for db_hg in hosttemplate_db.keys():
                if hosttemplate_config.has_key(db_hg):# hosttemplate is in .cfg and DB. update name
                    hosttemplate_config[db_hg].update({'name':db_hg})
                else:
                    hosttemplate_config[db_hg]=generic_host.copy()# hostgroup is in DB. Add in cfg
                    hosttemplate_config[db_hg]['name']=db_hg
                    
            for config_hg in hosttemplate_config.keys():
                if hosttemplate_db.has_key(config_hg):# hostgroup is in .cfg and DB. pass
                    pass
                elif(config_hg not in default_template_list):
                    hosttemplate_config.pop(config_hg)# hostgroup is in .cfg only. remove.        
            ...
            for db_hg in hosts_db.keys():
                if hosts_config.has_key(db_hg):# host is in .cfg and DB. update name and alias,etc
                    ...
                    hosts_config[db_hg]=hosts_db[db_hg]# host is in DB. Add in cfg
            ...
            for db_hg in hosts_db.keys():
                ...
                for config_hg in hosts_config.keys():
                    ...
                    if hosts_config_dict.get('host_name','') == hosts_db_dict.get('host_name','0') and hosts_config_dict.get('service_description','') == hosts_db_dict.get('service_description','0'): 
                        ...
                        flag_service = 0
                if flag_service:
                    hosts_config[count_service]=hosts_db_dict# host is in DB. Add in cfg
...
#                    hosts_config.pop(config_hg)# host is in .cfg only. remove.        
            ...
            for config_hg in hosts_config.keys():
                hosts_config_dict = hosts_config[config_hg]
                flag_service = 0
                temp_tuple = (hosts_config_dict.get('host_name',''),hosts_config_dict.get('service_description',''))
                for db_hg in hosts_db.keys():
                    count_service+=1
                    hosts_db_dict = hosts_db[db_hg]
                #if hosts_config_dict.get('host_name','') == hosts_db_dict.get('host_name','0') and hosts_config_dict.get('service_description','') == hosts_db_dict.get('service_description','0'): 
    ...
    def get_nagios_hostgroup_inventory(self,hostgroup_name):
        try:
            ...
            if(service_result["success"]==0):
                ...
                service_details={}
            host_result={}
            if host_template_name!=None and service_result["data"].has_key(host_template_name):
                host_result["data"] = service_result["data"][host_template_name]
            ...
            host_result["options"]={}
            file_name_list = ["timeperiod.cfg","commands.cfg","hostgroups.cfg","hosts.cfg","contactgroup.cfg","contacts.cfg"]
            corresponding_dict=["timeperiod","check_command","hostgroups","parents","contactgroup","contacts"]
...
#                                hosttemplate_list.append([load_result["data"][key]["name"],load_result["data"][key]["name"]])
                        if file_name=="hosts.cfg":
                            hosttemplate_list.append([load_result["data"][key]["host_name"],load_result["data"][key]["alias"]])
                        else:
                            hosttemplate_list.append([key,key])
                host_result["options"][corresponding_dict[i]] = hosttemplate_list
                
            file_name_list = ["timeperiod.cfg","commands.cfg","contactgroup.cfg"]
            corresponding_dict=["timeperiod","check_command","contactgroup"]
            ...
            #host_result["options"]["notification_options"]=[["d","d (DOWN)"],["u","u (UP)"],["r","r (RECOVERY)"],["f","f (FLAPPING)"],["s","s (SCHEDULED DOWNTIME)"],["n","n (NONE)"]]
            host_result["options"]["notification_options"]=[["d","d (DOWN)"],["u","u (UNREACHABLE)"],["r","r (RECOVERY/OK)"],["f","f (FLAPPING)"],["s","s (SCHEDULED)"],["n","n (NONE)"]]
            host_result["options"]["notifications_enabled"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
            host_result["options"]["initial_state"]=[["o","o (OK)"],["d","d (DOWN)"],["u","u (UNREACHABLE)"]]
            host_result["options"]["active_checks_enabled"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
            host_result["options"]["passive_checks_enabled"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
            host_result["options"]["check_freshness"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
            host_result["options"]["process_perf_data"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
            host_result["options"]["flap_detection_enabled"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
            #host_result["options"]["flap_detection_options"]=[["o","o (OK)"],["u","u (UNREACHABLE)"]]#[["o","o (OK)"],["w","w (WARNING)"],["c","c (CRITICAL)"],["u","u (UNREACHABLE)"]]
            host_result["options"]["flap_detection_options"]=[["o","o (UP/OK)"],["d","d (DOWN)"],["u","u (UNREACHABLE)"]]
            host_result["options"]["event_handler_enabled"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
    ...
    def save_nagios_edit_hostgroup_inventory(self,req_vars={}):
        try:
            ...
            var_list=["action_url", "active_checks_enabled", "address", "alias", "check_command", "check_freshness", "check_interval",
            ...
            write_result = set_attribute_by_name_to_shelve(attribute_data, "hosttemplate" , template_name)
            comment = "hostgroup %s updated at time : %s ."%(unique_name,str(datetime.now())[:22])
            file_name="host_template.cfg"
    ...
    def get_advanced_host_settings_nagios(self, host_id):
        try:
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            query="select service_description,max_check_attempts,normal_check_interval,retry_check_interval from host_services where host_id='%s' "%(host_id)
            cursor.execute(query)
            ...
            if result:
                for row in result:
                    if row[0].lower().find('snmp uptime')!=-1:
                        ...
                        service_dict['statistics_service'] = [row[1],row[2],row[3]]
            cursor.close()
            db.close()
    ...
    def get_host_name_from_host_id(self,host_id):
        try:
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            query="select host_name from hosts where host_id='%s' "%(host_id)
            cursor.execute(query)
            result=cursor.fetchall()
            cursor.close()
            db.close()
    ...
    def get_hostgroup_name_from_hostgroup_id(self,hostgroup_id):
        try:
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            query="select hostgroup_name from hostgroups where hostgroup_id='%s' "%(hostgroup_id)
            cursor.execute(query)
            result=cursor.fetchall()
            cursor.close()
            db.close()
    ...
    def apply_advanced_host_settings_nagios(self, req_vars):
        try:
            ...
            # snmp uptime
            max_check_attempts_snmp_uptime = req_vars.get("max_check_attempts_snmp_uptime","")
            check_interval_snmp_uptime = req_vars.get("check_interval_snmp_uptime","")
            retry_interval_snmp_uptime = req_vars.get("retry_interval_snmp_uptime","")
            # statistics service
            max_check_attempts_statistics_service = req_vars.get("max_check_attempts_statistics_service","")
            check_interval_statistics_service = req_vars.get("check_interval_statistics_service","")
            retry_interval_statistics_service = req_vars.get("retry_interval_statistics_service","")
            ...
            if(host_id!=""):
                ...
                if max_check_attempts_snmp_uptime!="":
                    query_list.append("max_check_attempts = '%s' , "%(max_check_attempts_snmp_uptime))
                if check_interval_snmp_uptime!="":
                    query_list.append("normal_check_interval = '%s' , "%(check_interval_snmp_uptime))
                if retry_interval_snmp_uptime!="":
                    query_list.append("retry_check_interval = '%s' , "%(retry_interval_snmp_uptime))     
                
                if query_list != ["update host_services set  "]:
                    query=''.join(query_list)[:-2] + " where host_id='%s' and service_description like 'snmp uptime%%' "%host_id
                    cursor.execute(query)
                    db.commit()
                ...
                if max_check_attempts_statistics_service!="":
                    query_list.append("max_check_attempts = '%s' , "%(max_check_attempts_statistics_service))
                if check_interval_statistics_service!="":
                    query_list.append("normal_check_interval = '%s' , "%(check_interval_statistics_service))
                if retry_interval_statistics_service!="":
                    query_list.append("retry_check_interval = '%s' , "%(retry_interval_statistics_service))     
    ...
    def get_pagination_data(self, dict_name = "",list_files=[],a_columns=[],req_vars={},extra_dict=[],host_index=-1,hostgroup_index=-1):
        try:
            ...
            for file_name in list_files:
                modified_result = check_files_if_modified(file_name)  # file_name = file_name,is_cfg=1 
            ...
            if hostgroup_index!=-1 and host_index!=-1:          # code for hostgroup
                for row in range(len(result_list)):
                    ...
                    host_name_list = [i.strip() for i in result_list[row][host_index].split(',')]
                    host_aliases = ""
                    ...
                    if load_result["success"]==0:
                        for key in load_result["data"]:
                            if load_result["data"][key].has_key("hostgroups"):
                                ...
                                if host_hostgroup == unique_name and load_result["data"][key]["host_name"] not in already_members:
                                    assigned_hosts += load_result["data"][key]["alias"]+", "
            ...
            i_display_length = i_display_start + int(req_vars.get("iDisplayLength",0))
            ...
            if(str(sSortDir_0)=="asc"):
               result_list = sorted(result_list, key=lambda result_list: result_list[int(i_sort_col_0)],reverse=False)
            else:
               result_list = sorted(result_list, key=lambda result_list: result_list[int(i_sort_col_0)],reverse=True)
    ...
    def get_log_data(self,req_vars):
    	try:
            ...
            a_columns=["host_name", "alias","address" , "parents", "hostgroups"]
            list_files=['hosts.cfg']
            parent_host_index = 3
            # get the data table result
            host_result = self.get_pagination_data("host",list_files ,a_columns ,req_vars)
            aaData = host_result["aaData"]
            unique_index = 0 # index of host_name
            if not host_result.has_key("exception"):
    ...
    def get_nagios_host_by_name(self,host_name):
        try:
            ...
            # declare a dict "options" which will contain all options of the nagios fields
            host_result["options"]={}

            if host_result["data"].has_key("parents"):
                parent_name = host_result["data"]["parents"]
                parent_info = get_attribute_by_name(parent_name,'host')
                if parent_info["success"]==0:
                    host_result["data"]["parents"] = parent_info["data"]["alias"]
            
            # load options of various files and nagios fields
            file_name_list = ["host_template.cfg","timeperiod.cfg","commands.cfg","hostgroups.cfg","hosts.cfg","contactgroup.cfg","contacts.cfg"]
            corresponding_dict=["use","timeperiod","check_command","hostgroups","parents","contactgroup","contacts"]
    ...
    def save_nagios_edit_host(self,req_vars={}):
        try:
            ...
            # attribute_data will contain the various options present in the request sent for saving
            attribute_data = {}
            for var in var_list:
                res = req_vars.get(var,"")
                if res!="" and str(res).lower()!="null":
            ...
            if attribute_data.has_key("parents"):
                es = Essential()
                parent_id=1
                child_id=1
                ...
                cursor=db.cursor()
                # write hostgroups file 
                query="select host_id,host_name from hosts where host_name in ('%s','%s')"%(parent_name,child_name)
                hosts_id_db = {}
                cursor.execute(query)
            ...
            if attribute_data.has_key("hostgroups"):
                hostgroup_name_list=[i.strip() for i in attribute_data["hostgroups"].split(',')]
                if attribute_data['hostgroups']=='':
                    attribute_data['hostgroups'] = self.get_hostgroup_name_from_host_name(unique_name)
            else:
                attribute_data['hostgroups'] = self.get_hostgroup_name_from_host_name(unique_name)
            if attribute_data.has_key("parents"):
                hostgroup_name_list=[i.strip() for i in attribute_data["parents"].split(',')]
                if attribute_data['parents']=='':
                    attribute_data['parents'] = self.get_parent_name_from_host_name(unique_name)
            else:
                attribute_data['parents'] = self.get_parent_name_from_host_name(unique_name)
            template_result = get_attribute_by_name(attribute_data["hostgroups"], 'hosttemplate')
            if(template_result)['success']==1:
                # append new hostgroup template to host_templates.cfg
                rt = self.add_new_host_template_hostgroup(attribute_data["hostgroups"], True)
                if rt['success'] == 0:
                    use_li = use.split(',')
                    use_li=list(set(use_li))
                    use_index = use_li.index(old_hostgroup) if use_li.count(old_hostgroup)  else -1
                    if use_index!=-1:
            ...
            write_result = complete_write_attribute_by_name_to_shelve(attribute_data, attribute , unique_name)
            ...
            if all_hostgroups["success"]==0:
                ...
                for key in hostgroup_data:
                    if hostgroup_data[key]["hostgroup_name"] not in hostgroup_name_list:
                        if hostgroup_data[key].has_key("members"):
                            old_members=hostgroup_data[key]["members"]
                            members_list = [i.strip() for i in old_members.split(',')]
                            flag=0
                            if unique_name in members_list:
                                 members_list.remove(unique_name)
                                 set_attribute_by_name_to_shelve({"members":",".join(members_list)}, "hostgroup" , hostgroup_data[key]["hostgroup_name"])
                            
            if write_result["success"]==0:
                comment = "host %s updated at time : %s ."%(attribute_data["alias"],str(datetime.now())[:22])
                file_name="hosts.cfg"
                write_cfg_result=write_configuration_file(file_name,comment)
                comment = "hostgroups updated due to changes in hosts %s at time : %s "%(attribute_data["alias"],str(datetime.now())[:22])
                file_name="hostgroups.cfg"
                write_cfg_result=write_configuration_file(file_name,comment)
            else:
            ...
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            if attribute_data.has_key("hostgroups") and attribute_data["hostgroups"]!="" and attribute_data["hostgroups"]!=old_hostgroup:
            ...
            where host_id=(select host_id from hosts where host_name='%s')"%(attribute_data["hostgroups"],unique_name)
                cursor.execute(query)
                    ...
                    #result = edit_for_inventory_object(host_name, file_name, attribute, dict_name, new_host_dict, comment)
            if old_host_alias!=attribute_data["alias"] and attribute_data["alias"]!="":
                query = "update hosts set host_alias='%s' where host_name='%s'"%(attribute_data["alias"], unique_name)
                cursor.execute(query)     
            if attribute_data.has_key("parents") and attribute_data["parents"]!="":
    ...
    def get_log_data_host_template(self,req_vars):
        try:
            ...
            a_columns=["name","notification_interval", "max_check_attempts", "check_command"]
            list_files=['host_template.cfg']
            parent_host_index = 0
            host_result = self.get_pagination_data("hosttemplate",list_files ,a_columns ,req_vars)
...
#                        aaData[row][parent_host_index] = parent_info["data"]["alias"]
    ...
    def get_nagios_host_template_by_name(self,host_template_name):
        try:
            ...
            service_result=load_db_by_name(attribute)#get_attribute_by_name(service_description, attribute)
            if(service_result["success"]==0):
                service_details = service_result["data"]
            else:
                service_details={}
...
#            host_result=get_attribute_by_name(host_template_name, attribute)
    ...
    def save_nagios_edit_host_template(self,req_vars={}):
        try:
            ...
            var_list=["action_url", "active_checks_enabled",  "check_command", "check_freshness", "check_interval",
                       "check_period", "contact_groups", "contacts", "event_handler", "event_handler_enabled", "first_notification_delay", 
            ...
            if write_result["success"]==0:
                comment = "host template %s updated at time : %s ."%(attribute_data["name"],str(datetime.now())[:22])
                file_name="host_template.cfg"
                write_cfg_result=write_configuration_file(file_name,comment)
                return JSONEncoder().encode(write_cfg_result)
            else:
    ...
    def nagios_delete_host_template(self,req_vars={}):
        try:
            load_result = load_configuration(['host_template.cfg'])
            unique_names_li = req_vars.get("host_template_names",'').split(',')
            attribute = "hosttemplate"
            unique_names_str=", ".join(unique_names_li)
            for unique_name in unique_names_li:
                write_result= delete_attribute_by_name_from_shelve(attribute,unique_name)
           
            if write_result["success"]==0:
                comment = "host_template(s) %s deleted at time : %s ."%(unique_names_str,str(datetime.now())[:22])
                file_name="host_template.cfg"
                write_cfg_result=write_configuration_file(file_name,comment)
    ...
    def get_log_data_service(self,req_vars):
        try:
            a_columns=["service_description", "use", "host_name", "normal_check_interval"]
            list_files=['services.cfg','hosts.cfg']
            host_index = 3
            
            host_result = self.get_pagination_data("service",list_files ,a_columns ,req_vars,{},host_index)
            #if host_result.has_key("exception"):
            aaData = host_result["aaData"]
            unique_index = 0 # index of host_name
            service_index = 1
            ...
            if not host_result.has_key("exception"):
                for row in range(len(aaData)):
                    #unique_name = aaData[row][service_index] + (aaData[row][host_index] if aaData[row][host_index]!="-" else "")
...
#            
            ...
            if service_description!=None and service_result["data"].has_key(service_description):
                host_result["data"] = service_result["data"][service_description]
            ...
            host_result["options"]={}
            file_name_list = ["service_template.cfg","timeperiod.cfg","commands.cfg","hostgroups.cfg","contactgroup.cfg","hosts.cfg","contacts.cfg"]
            corresponding_dict=["use","timeperiod","check_command","hostgroups","contactgroup","host_name","contacts"]
            for i in range(len(file_name_list)):
                file_name = file_name_list[i]
                hosttemplate_list=[]
                result_list =[]
            ...
            host_result["options"]["parallelize_check"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
            host_result["options"]["failure_prediction_enabled"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
            host_result["options"]["stalking_options"]=[["o","o (OK)"],["w","w (WARNING)"],["c","c (CRITICAL)"],["u","u (UNREACHABLE)"]]
            host_result["options"]["retain_status_information"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
            host_result["options"]["retain_nonstatus_information"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
            host_result["options"]["obsess_over_service"]=[["0","0 (Don't obsess)"],["1","1 (obsess)"]]
            host_result["options"]["is_volatile"]=[["0","0 (not volatile)"],["1","1 (volatile)"]]
    ...
    def save_nagios_edit_service(self,req_vars={}):
        try:
            var_list=["action_url",  "active_checks_enabled",  "check_command",  "check_freshness",  "check_interval",  "check_period",  
            ...
            if unique_name=="":
                ...
                if li_host_dependency==[]:
                    ...
                    unique_name = "service" + str(int(last_host_dependency[len('service'):])+1)
            ...
            if write_result["success"]==0:
                comment = "service %s updated at time : %s ."%(attribute_data["service_description"],str(datetime.now())[:22])
                file_name="services.cfg"
                write_cfg_result=write_configuration_file(file_name,comment)
            ...
            if(attribute_data["service_description"].lower().find('snmp uptime')!=-1):
                max_check_attempts_snmp_uptime = attribute_data.get("max_check_attempts","")
                check_interval_snmp_uptime = attribute_data.get("normal_check_interval","")
                retry_interval_snmp_uptime = attribute_data.get("retry_check_interval","")
            ...
            (attribute_data["service_description"].lower().find('statictics service')!=-1)):
                max_check_attempts_statistics_service = attribute_data.get("max_check_attempts","")
                check_interval_statistics_service = attribute_data.get("normal_check_interval","")
                retry_interval_statistics_service = attribute_data.get("retry_check_interval","")
            ...
            if(attribute_data.get("host_name","")!=""):
                ...
                if flag_service:
                    query=''.join(query_list)[:-2] + " where service_description like 'snmp uptime%%' "
                    cursor.execute(query)
                    db.commit()
    ...
    def nagios_delete_service(self,req_vars={}):
        try:
            ...
            if write_result["success"]==0:
                comment = "service(s) %s deleted at time : %s ."%(unique_names_str,str(datetime.now())[:22])
                file_name="services.cfg"
                write_cfg_result=write_configuration_file(file_name,comment)
                return JSONEncoder().encode(write_cfg_result)
            else:
    ...
    def get_log_data_service_template(self,req_vars):
        try:
            ...
            #req_vars["iSortCol_0"] =req_vars.get("iSortCol_0",0) if req_vars.get("iSortCol_0",0)!=0 else 1 
            host_result = self.get_pagination_data("servicetemplate",list_files ,a_columns ,req_vars)
            #if host_result.has_key("exception"):
            aaData = host_result["aaData"]
            unique_index = 0 # index of host_name
    ...
    def get_nagios_service_template_by_name(self,service_description):
        try:
            ...
            host_result["options"]["process_perf_data"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
            host_result["options"]["flap_detection_enabled"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
            #host_result["options"]["flap_detection_options"]=[["o","o (UP/OK)"],["u","u (UNREACHABLE)"]]#[["o","o (OK)"],["w","w (WARNING)"],["c","c (CRITICAL)"],["u","u (UNREACHABLE)"]]
            host_result["options"]["flap_detection_options"]=[["o","o (UP/OK)"],["d","d (DOWN)"],["u","u (UNREACHABLE)"]]
            host_result["options"]["event_handler_enabled"]=[["0","0 (Disable)"],["1","1 (Enable)"]]
    ...
    def save_nagios_edit_service_template(self,req_vars={}):
        try:
            ...
            write_result = complete_write_attribute_by_name_to_shelve(attribute_data, attribute , unique_name)
            if write_result["success"]==0:
                comment = "service template %s updated at time : %s ."%(attribute_data["name"],str(datetime.now())[:22])
    ...
    def nagios_delete_service_template(self,req_vars={}):
        try:
            ...
            unique_names_li = req_vars.get("service_template_names",'').split(',')
            attribute = "servicetemplate"
            unique_names_str=", ".join(unique_names_li)
            for unique_name in unique_names_li:
                write_result= delete_attribute_by_name_from_shelve(attribute,unique_name)
            ...
            if write_result["success"]==0:
                comment = "service template(s) %s deleted at time : %s ."%(unique_names_str,str(datetime.now())[:22])
                file_name="service_template.cfg"
                write_cfg_result=write_configuration_file(file_name,comment)
    ...
    def get_log_data_hostgroup(self,req_vars):
        try:
            ...
            host_index = 2
            hostgroup_index = 0
            host_result = self.get_pagination_data("hostgroup",list_files ,a_columns ,req_vars,{} ,host_index, hostgroup_index )
            #if host_result.has_key("exception"):
            aaData = host_result["aaData"]
            unique_index = 0 # index of host_name
            if not host_result.has_key("exception"):
    ...
    def get_nagios_hostgroup_by_name(self,hostgroup_name):
        try:
            attribute = "hostgroup"
            hostgroup_result=load_db_by_name(attribute)#get_attribute_by_name(service_description, attribute)
            if(hostgroup_result["success"]==0):
            ...
            load_result=load_return_attribute("hosts.cfg")
            assigned_hosts=""
            if not host_result["data"].has_key("members"):
                host_result["data"]["members"]=""
            already_members=[i.strip() for i in host_result["data"]["members"].split(',')]
                ...
                for key in load_result["data"]:
                    if load_result["data"][key].has_key("hostgroups"):
                        ...
                        if host_hostgroup == hostgroup_name and load_result["data"][key]["host_name"] not in already_members:
                            assigned_hosts += load_result["data"][key]["host_name"]+","
            if assigned_hosts!="":
                ...
                if host_result["data"]["members"]!="":
                    ...
                    host_result["data"]["members"]=assigned_hosts
            host_result["options"]={}
            file_name_list = ["host_template.cfg","hostgroups.cfg","hosts.cfg"]
            ...
            for i in range(len(file_name_list)):
                ...
                if load_result["success"]==0:
                    for key in load_result["data"]:
                        if file_name=="hosts.cfg":
                            if load_result["data"][key].has_key("hostgroups") and load_result["data"][key]["hostgroups"]==hostgroup_name:
                                hosttemplate_list.append([load_result["data"][key]["host_name"],load_result["data"][key]["alias"]])
                            elif not load_result["data"][key].has_key("hostgroups")  :
                                hosttemplate_list.append([load_result["data"][key]["host_name"],load_result["data"][key]["alias"]])
                        else:
                            hosttemplate_list.append([key,key])
                host_result["options"][corresponding_dict[i]] = hosttemplate_list
    ...
    def save_nagios_edit_hostgroup(self,req_vars={}):
        try:
            ...
            var_list= ['action_url', 'alias', 'hostgroup_members', 'hostgroup_name', 'members', 'notes', 'notes_url']
            attribute_data = {}
            for var in var_list:
                res = req_vars.get(var,"")
                if res!="" and str(res).lower()!="null":
            ...
            ## new code
            host_name_list=[]
            if attribute_data.has_key("members"):
                host_name_list = [i.strip() for i in attribute_data["members"].split(',')]
                for host_name in host_name_list:  
                    host_info = set_attribute_by_name_to_shelve({"hostgroups":attribute_data["hostgroup_name"]}, "host" , host_name)
                
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            #query="delete  hosts_hostgroups from hosts_hostgroups join hostgroups as hg on hg.hostgroup_id = hosts_hostgroups.hostgroup_id and hg.hostgroup_name='%s' "%(unique_name)
            ...
            if new_name!=unique_name:
                query = " update hostgroups set hostgroup_name='%s' where hostgroup_name='%s'"%(new_name, unique_name)  
                cursor.execute(query)  
            db.commit()
            cursor.close()
            db.close()
            ...
            if all_hosts["success"]==0:
                ...
                for key in host_data:
                    if host_data[key]["host_name"] not in host_name_list:
                        if host_data[key].has_key("hostgroups"):
                            old_hostgroup=host_data[key]["hostgroups"]
                            hostgroup_list = [i.strip() for i in old_hostgroup.split(',')]
                            flag=0
                            if unique_name in hostgroup_list:
                                 hostgroup_list.remove(unique_name)
                                 set_attribute_by_name_to_shelve({"hostgroups":",".join(hostgroup_list)}, "host" , host_data[key]["host_name"])
                            
            #if not attribute_data.has_key("members"):
            #    attribute_data["members"]=""
            write_result = complete_write_attribute_by_name_to_shelve(attribute_data, attribute , unique_name)              
            if write_result["success"]==0:
                comment = "hostgroup: %s updated at time : %s ."%(new_name,str(datetime.now())[:22])
                file_name="hostgroups.cfg"
                write_cfg_result=write_configuration_file(file_name,comment)
                comment = "hosts updated due to changes in hostgroup %s at time : %s "%(new_name,str(datetime.now())[:22])
                file_name="hosts.cfg"
                write_cfg_result=write_configuration_file(file_name,comment)
    ...
    def reload_nagios_hostgroup(self):
        try:
            ...
            for hostgroup_name,hostgroup_id in result_hg:
                host_name_list=[]
                if hostgroup_result.has_key(hostgroup_name):
                    attribute_data = hostgroup_result[hostgroup_name] # get the info of individual hostgroup
                    #host_name_list = [i.strip() for i in attribute_data["members"].split(',')]
                    ...
                    for host_name in host_name_list:  
                        ...
                        host_info = set_attribute_by_name_to_shelve(replace_dict, "host" , host_name[0],extra_data)
                        
                    hostgroup_info = set_attribute_by_name_to_shelve({"members":""}, "hostgroup" , hostgroup_name)
            comment = "hostgroups updated at time : %s ."%(str(datetime.now())[:22])
            file_name="hostgroups.cfg"
            write_cfg_result=write_configuration_file(file_name,comment)
            comment = "hosts updated due to changes in hostgroups at time : %s "%(str(datetime.now())[:22])
            file_name="hosts.cfg"
            write_cfg_result=write_configuration_file(file_name,comment)
            return JSONEncoder().encode(write_cfg_result)
        except Exception,e:
    ...
    def get_log_data_command(self,req_vars):
        try:
            ...
            host_result = self.get_pagination_data("command",list_files ,a_columns ,req_vars)
            #if host_result.has_key("exception"):
            aaData = host_result["aaData"]
            unique_index = 0 # index of host_name
            if not host_result.has_key("exception"):
    ...
    def save_nagios_edit_command(self,req_vars={}):
        try:
            ...
            if write_result["success"]==0:
                comment = "command %s %s at time : %s ."%(attribute_data["command_name"],updated,str(datetime.now())[:22])
                write_cfg_result=write_configuration_file(file_name,comment)
                return JSONEncoder().encode(write_cfg_result)
            else:
                return JSONEncoder().encode(write_result)
    ...
    def nagios_delete_command(self,req_vars={}):
        try:
            ...
            if write_result["success"]==0:
                comment = "command(s) %s deleted at time : %s ."%(unique_names_str,str(datetime.now())[:22])
                file_name="commands.cfg"
                write_cfg_result=write_configuration_file(file_name,comment)
    ...
    def get_log_data_hostdependency(self,req_vars):
        try:
            ...
            host_index = 1
            #req_vars["iSortCol_0"] =req_vars.get("iSortCol_0",0) if req_vars.get("iSortCol_0",0)!=0 else 1 
            host_result = self.get_pagination_data(dict_name,list_files ,a_columns ,req_vars,extra_dict_name_li)
            #if host_result.has_key("exception"):
            aaData = host_result["aaData"]
            unique_index = 0 # index of host_name
            dependent_index = 2
    ...
    def get_nagios_hostdependency_by_name(self,hostdependency_name):
        try:
            ...
            if(service_result["success"]==0):
                ...
                service_details={}
            host_result={}
            if hostdependency_name!=None and service_result["data"].has_key(hostdependency_name):
                host_result["data"] = service_result["data"][hostdependency_name]
            elif hostdependency_name==None:
                host_result["data"]={}
            else:
                output = {"success":1,"exception":hostdependency_name+" not found."}
                return JSONEncoder().encode(output)
                
            host_result["options"]={}
            file_name_list = ["timeperiod.cfg","hostgroups.cfg","hosts.cfg","hosts.cfg"]
            corresponding_dict=["dependency_period","dependent_hostgroup_name","host_name","dependent_host_name"]
            for i in range(len(file_name_list)):
                file_name = file_name_list[i]
                hosttemplate_list=[]
                result_list =[]
            ...
            host_result["options"]["execution_failure_criteria"]=[["n","n (none, always)"],["p","p (pending)"],["u","u (host Unreachable state)"],["o","o (host Up state)"],
                                                                     ["d","d (host Down state)"]]
            host_result["options"]["inherits_parent"]=[["0","0"],["1","1"]]
            host_result["success"]=0
            return JSONEncoder().encode(host_result)
        except Exception,e:
            output = {
    ...
    def save_nagios_edit_hostdependency(self,req_vars={}):
        try:
            ...
            var_list=['dependent_host_name', 'host_name', 'dependency_period','dependent_hostgroup_name', 'execution_failure_criteria','inherits_parent' ,'notification_failure_criteria',]
            attribute_data = {}
            for var in var_list:
                res = req_vars.get(var,"")
                if res!="" and str(res).lower()!="null":
            ...
            if unique_name=="":
                ...
                if li_host_dependency==[]:
                    ...
                    li_host_dependency.sort()
                    last_host_dependency=li_host_dependency[-1]
                    unique_name = "hostdependency" + str(int(last_host_dependency[len('hostdependency'):])+1)
            ...
            if write_result["success"]==0:
                ...
                comment = "hostdependency of host(s) %s %s at time : %s ."%(host_aliases,updated,str(datetime.now())[:22])
    ...
    def nagios_delete_hostdependency(self,req_vars={}):
        try:
            ...
            unique_names_li = req_vars.get("host_dependency_names",[]).split(',')
            attribute = "hostdependency"
            db_result=load_db_by_name(attribute)
            unique_names_str=", ".join(unique_names_li)
            host_aliases = ""
            for unique_name in unique_names_li:
                ######
                if db_result['data'].has_key(unique_name):    
            ...
            if write_result["success"]==0:
                comment = "host dependency for host(s) %s deleted at time : %s ."%(host_aliases,str(datetime.now())[:22])
                file_name="hostdependency.cfg"
                write_cfg_result=write_configuration_file(file_name,comment)
    ...
    def get_log_data_servicedependency(self,req_vars):
        try:
            a_columns=["host_name","service_description", "dependent_host_name","dependent_service_description"]
    ...
    def get_nagios_servicedependency_by_name(self,servicedependency_name):
        try:
            ...
            if(service_result["success"]==0):
                ...
                service_details={}
            host_result={}
            if servicedependency_name!=None and service_result["data"].has_key(servicedependency_name):
                host_result["data"] = service_result["data"][servicedependency_name]
            elif servicedependency_name==None:
                host_result["data"]={}
            else:
                output = {"success":1,"exception":servicedependency_name+" not found."}
                return JSONEncoder().encode(output)
                
            host_result["options"]={}
            file_name_list = ["timeperiod.cfg","hostgroups.cfg","hosts.cfg","hosts.cfg","services.cfg","services.cfg","contacts.cfg"]
            corresponding_dict=["dependency_period","dependent_hostgroup_name","host_name","dependent_host_name","service_description",
                ...
                if load_result["success"]==0:
                    for key in load_result["data"]:
                        if file_name=="hosts.cfg":
                            ...
                            hosttemplate_list.append([key.replace(' ','___'),load_result["data"][key]["service_description"]])
                        else:
                            hosttemplate_list.append([key,key])
                host_result["options"][corresponding_dict[i]] = hosttemplate_list
            ...
            for key in load_service_result["data"]:
                ...
                if di_key.has_key("host_name"):
                    if host_service_dict.has_key(di_key["host_name"]):
                        ...
                        host_service_dict[di_key["host_name"]] = [di_key["service_description"]]
    ...
    def save_nagios_edit_servicedependency(self,req_vars={}):
        try:
            ...
            var_list=['dependent_host_name', 'host_name', 'dependency_period','dependent_hostgroup_name', 'execution_failure_criteria',
            ...
            if unique_name=="":
                ...
                if li_host_dependency==[]:
                    ...
                    li_host_dependency.sort()
                    last_host_dependency=li_host_dependency[-1]
                    unique_name = "servicedependency" + str(int(last_host_dependency[len('servicedependency'):])+1)
            ...
            if write_result["success"]==0:
                ...
                if host_aliases!="":
                    host_aliases = host_aliases[:-2]
                comment = "service dependency of host(s) %s and service %s %s at time : %s ."%(host_aliases,attribute_data.get("service_description",""),updated,str(datetime.now())[:22])
    ...
    def nagios_delete_servicedependency(self,req_vars={}):
        try:
            ...
            unique_names_li = req_vars.get("service_dependency_names",[]).split(',')
            attribute = "servicedependency"
            unique_names_str=", ".join(unique_names_li)
            host_aliases = ""
            service_aliases=""
...
#            if write_result["success"]==0:
            comment = "service dependency of hosts(s) %s and servie(s) %s deleted at time : %s ."%(host_aliases,service_aliases,str(datetime.now())[:22])
            file_name="servicedependency.cfg"
            write_cfg_result=write_configuration_file(file_name,comment)
            return JSONEncoder().encode(write_cfg_result)
#            else:
#                return JSONEncoder().encode(write_result)
    ...
    def get_nagios_status(self):
        try:
            #proc = subprocess.Popen(["omd status nms nagios"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            process = subprocess.Popen('omd status nagios', shell=True, stdout=subprocess.PIPE)
    ...
    def do_action_nagios(self,action):
        try:
            process = subprocess.Popen('omd %s nagios'%(action), shell=True, stdout=subprocess.PIPE)
            #process = subprocess.Popen('/omd/sites/nms/bin/nagios -v /omd/sites/nms/tmp/nagios/nagios.cfg', shell=True, stdout=subprocess.PIPE)
            output, _ = process.communicate()
            code = process.wait()
            output_action = {
            "success":0,
    ...
    def do_verify_nagios(self,action):
        try:
            process = subprocess.Popen('/omd/sites/%s/bin/nagios -v /omd/sites/%s/tmp/nagios/nagios.cfg'%(nms_instance,nms_instance), shell=True, stdout=subprocess.PIPE)
            output, _ = process.communicate()
            code = process.wait()
            ...
            while True:
                ...
                nindex=output.find('\n',pos+1)
                if nindex!=-1:
                    output=output[:pos]+' <B> '+output[pos:nindex]+' </B> '+output[nindex:]
                else:
                    break
                ipos=nindex+1
            ...
            for i in range(len(li)):
                if li[i].startswith("'host"):
                    li[i] =li[i]+" ("+ hosts_di["data"][li[i][1:-1]]["alias"]+")"
            output=' '.join(li)     
    ...
    def add_new_host(self,host_name,alias,address,parent_name="",hostgroup_id="",use="generic-host"):
        try:
            ...
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            query="select host_name from hosts where host_id='%s' "%(parent_name)
            cursor.execute(query)
            res=cursor.fetchall()
            
            ## if hostgroup_id is not given
            if hostgroup_id=="":
                query="select hostgroup_id from hosts_hostgroups where host_id=(select host_id from hosts where host_name='%s') "%(host_name)
                cursor.execute(query)
            ...
            load_result = load_configuration(['host_template.cfg'])
            template_result = get_attribute_by_name(hostgroup_id, 'hosttemplate')
            if(template_result)['success']==1:
                # append new hostgroup template to host_templates.cfg
                ...
                use = hostgroup_id + "," + use
            new_host_dict={"host_name":host_name,"alias":alias,"address":address,"use":use,"parents":parent_name,"hostgroups":hostgroup_id}
            comment = "Adding new host(%s) at time : %s"%(alias,str(datetime.now())[:22])
    ...
    def edit_old_host(self,host_name,alias,address,parent_name="",hostgroup_id="",use="generic-host"):
        try:
            ...
            dict_name="host"
            attribute="host"
            rt = load_configuration(['services.cfg','hosts.cfg','host_template.cfg'])
            comment="Updating host(%s) at time : %s"%(alias,str(datetime.now())[:22])
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            query="select host_name from hosts where host_id='%s' "%(parent_name)
            cursor.execute(query)
    ...
    def delete_old_host(self,host_name):
        try:
            ...
            if host_info["success"]==0:
                #host_aliases += host_info["data"]["alias"]+", 
                comment="Deleting host(%s) at time : %s"%(host_info["data"]["alias"],str(datetime.now())[:22])
            replace_dict={"parents":"localhost"}
            result = delete_for_inventory_object(host_name, file_name, attribute, dict_name,replace_dict, comment)
    ...
    def add_new_hostgroup(self,hostgroup_name,alias):
        try:
            ...
            comment = "Adding new hostgroup(%s) at time : %s"%(hostgroup_name,str(datetime.now())[:22])
            result=append_for_inventory_new_object(file_name,dict_name,attribute,new_host_dict,comment)
    ...
    def edit_old_hostgroup(self,old_hostgroup_name,hostgroup_name,alias):
        try:
            ...
            dict_name="hostgroup"
            attribute="hostgroup"
            comment="Updating hostgroup(%s) at time : %s"%(hostgroup_name,str(datetime.now())[:22])
            new_host_dict={"hostgroup_name":hostgroup_name,"alias":alias}
            load_result = load_configuration(['hostgroups.cfg'])
            result = edit_for_inventory_object(old_hostgroup_name, file_name, attribute, dict_name, new_host_dict, comment)
            rt = self.edit_host_template_hostgroup(old_hostgroup_name,hostgroup_name)
            rt = self.edit_host_hostgroup_inventory(old_hostgroup_name,hostgroup_name)
            return result
        except Exception,e:
            return {"success":1,"data":str(e)}
        
    # function for editing and saving host template through hostgroup via inventory    
...
#            replace_name="name"
            ...
            try:
                ...
                pass
            load_result = load_configuration(['host_template.cfg'])
            comment="Updating host template for hostgroup(%s) at time : %s"%(new_hostgroup_name,str(datetime.now())[:22])
            result = edit_for_inventory_object_without_unique(other_dict,file_name,attribute,dict_name,old_hostgroup_name,
    ...
    def edit_host_hostgroup_inventory(self,old_hostgroup_name,new_hostgroup_name):
        ...
        try:
            ...
            comment="Updating hosts for hostgroup(%s) at time : %s"%(new_hostgroup_name,str(datetime.now())[:22])
            result = edit_for_inventory_object_without_unique(other_dict,file_name,attribute,dict_name,old_hostgroup_name,
                                                              new_hostgroup_name,comment)
            return result
        except Exception,e:
    ...
    def delete_old_hostgroup(self,hostgroup_name):
        try:
            ...
            attribute="hostgroup"
            load_result = load_configuration(['hostgroups.cfg'])
            comment="Deleting hostgroup(%s) at time : %s"%(hostgroup_name,str(datetime.now())[:22])
            replace_dict={"hostgroups":"Default"}
            result = delete_for_inventory_object(hostgroup_name, file_name, attribute, dict_name,replace_dict, comment)
            replace_dict_template = {"use":"Default", "hostgroups":"Default"}
            old_dict_template = {"use":hostgroup_name,"hostgroups":hostgroup_name}
            try:
                eval('host').clear()
            except:
                pass
            result_hosts = delete_for_inventory_object_modify_hosts(old_dict_template, 'hosts.cfg', 'host', 'host',replace_dict_template, comment)
            d = load_configuration(['host_template.cfg'])
            attribute = "host"
            replace_dict_template = {"use":"generic-host"}
            result_template = delete_for_inventory_object(hostgroup_name, "host_template.cfg", attribute, "hosttemplate",replace_dict_template,comment,0)
            return result
        except Exception,e:
    ...
    def add_new_host_template_hostgroup(self,host_template_name,use_default_template=True,default_template_name="generic-host"):
        try:
            ...
            if use_default_template:
                host_template_result = get_attribute_by_name(default_template_name, attribute)
            ...
            write_result = complete_write_attribute_by_name_to_shelve(host_template_result, attribute , host_template_name)
            if write_result["success"]==0:
                comment = "host template %s updated at time : %s ."%(host_template_name,str(datetime.now())[:22])
                file_name="host_template.cfg"
                write_cfg_result=write_configuration_file('host_template.cfg',comment,False)
    ...
    def add_new_service(self,host_name, host_alias, service_description, max_check_attempts, normal_check_interval, retry_check_interval, check_command,use="generic-service"):
        ...
        new_host_dict={"host_name":host_name,"service_description":service_description,"max_check_attempts":max_check_attempts,"use":use,
                       "normal_check_interval":normal_check_interval, "retry_check_interval":retry_check_interval,"check_command":check_command}
        
        comment = "Adding new service for host(%s) at time : %s"%(host_alias,str(datetime.now())[:22])
        result=append_for_inventory_new_object(file_name,dict_name,attribute,new_host_dict,comment)
        return result
    ...
    def edit_old_service(self, host_id, service_description, normal_check_interval):
        try:
            ...
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            query="select host_name, host_alias from hosts where host_id='%s' "%(host_id)
            cursor.execute(query)
            res=cursor.fetchall()
            db.close()
            load_result = load_configuration(['service.cfg'])
            ...
            if res:
                host_name=res[0][0]
                host_alias=res[0][1]
            comment="Updating service %s for host(%s) at time : %s"%(service_description, host_alias, str(datetime.now())[:22])
            unique_name = service_description + host_name
            new_host_dict={ "host_name":host_name,"service_description":service_description, "normal_check_interval":normal_check_interval}
    ...
    def delete_old_service(self,host_name):
        try:
            ...
            comment="Deleting service for host (%s) at time : %s"%(host_alias,str(datetime.now())[:22])
            replace_dict={}
            result = delete_for_inventory_object_without_unique_name(unique_attribute,unique_name, file_name, attribute, dict_name,replace_dict, comment)
            return result
        except Exception,e:
            return {"success":1,"data":str(e)}
    ...
    def reload_nagios_service_times(self):
        try:
            ...
            attribute = "service"
            rt = load_configuration(['services.cfg','hosts.cfg','hostgroups.cfg'])
            service_result=load_db_by_name(attribute)
            if service_result["success"]==0:
            ...
            for host_name,host_id in result_host_tuple:
                ...
                for key in service_result.keys():
                    if service_result[key].get("host_name","")==host_name:
                        attribute_data = service_result[key] # get the info of individual hosts
                        query = "select normal_check_interval,service_description from host_services where host_id='%s' "%(host_id)
                        cursor.execute(query)
                        ...
                        for normal_check_interval,service_description in service_time_list:
                            if service_description.upper().find("SNMP UPTIME")!=-1 and service_result[key].get("service_description","").upper().find("SNMP UPTIME")!=-1:
                                host_info = set_attribute_by_name_to_shelve({"normal_check_interval":normal_check_interval}, "service" , key)
            ...
            db.close()            
            comment = "services updated at time : %s "%(str(datetime.now())[:22])
            file_name="services.cfg"
            write_cfg_result=write_configuration_file(file_name,comment)
            return JSONEncoder().encode(write_cfg_result)
        except Exception,e:
