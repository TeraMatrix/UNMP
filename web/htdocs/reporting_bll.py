    ...
    def get_avg_data_for_two_dates(self,no_of_devices,date1,date2,all_group,all_host):
        try:
            ...
            if all_host=="" and all_group=="":
                ...
                while(d1<=d2):
                    date_temp=str(d1)[:10]
		    host_data=str(all_host.split(',')).replace('[','(').replace(']',')')
                    query="Select date(go16.timestamp),hosts.host_name,hosts.ip_address, AVG(go16.rx_phy_error) , AVG(go16.rx_crc_errors),hostgroups.hostgroup_name\
        	               from get_odu16_ra_tdd_mac_statistics_entry  as go16\
        	               join ( select host_id,host_name,ip_address,mac_address,host_alias from hosts where device_type_id like 'ODU16%%' group by host_id) as hosts\
                           INNER JOIN hosts_hostgroups ON hosts_hostgroups.host_id = hosts.host_id\
...
#			    time_stamp_signal1.append(str((signal_strength[k][0]).strftime('%d-%m-%Y %H:%M')))
				...
				default_list[int(index)-1]=int(signal_strength[k][3])
			    else:
				default_list[int(index)-1]=int(signal_strength[k][3])
				grp_name.append(signal_strength[k][5])
				host_name.append(signal_strength[k][1])
			...
			if signal_strength[0][6]=='-1' or str(signal_strength[0][3]).strip()=='' or str(signal_strength[0][4]).strip()=='':
				...
				signal_interface7.append(0)
				signal_interface8.append(0)
				time_stamp_signal1.append(str((signal_strength[0][0]).strftime('%d-%m-%Y %H:%M')))
			else:
				default_list[int(signal_strength[0][4])-1]=int(signal_strength[0][3])
				grp_name.append(signal_strength[0][5])
				host_name.append(signal_strength[0][1])
				ip_address.append(signal_strength[0][2])
				signal_interface1.append(default_list[0])
			...
			if signal_strength[i+1][6]=='-1' or str(signal_strength[i+1][3]).strip()=='' or str(signal_strength[i+1][4]).strip()=='':
				...
				signal_interface7.append(0)
				signal_interface8.append(0)
				time_stamp_signal1.append(str((signal_strength[i+1][0]).strftime('%d-%m-%Y %H:%M')))
			else:
				default_list[int(signal_strength[i+1][4])-1]=int(signal_strength[0][3])
				grp_name.append(signal_strength[i+1][5])
				host_name.append(signal_strength[i+1][1])
				ip_address.append(signal_strength[i+1][2])
				signal_interface1.append(default_list[0])
                ...
                for k in range(0,len(time_stamp_signal1)):
                        average_list.append([time_stamp_signal1[k],host_name[k],ip_address[k],signal_interface1[k],signal_interface2[k],signal_interface3[k],signal_interface4[k],signal_interface5[k],signal_interface6[k],signal_interface7[k],signal_interface8[k],grp_name[k]])
            else:
    		    host_data=str(all_host.split(',')).replace('[','(').replace(']',')')
    		    while(d1<=d2):
			flag=0
...
# 		sig_strength=3 , timeslot_index=4 , ssidentifier=6
		...
		for k in range(0,len(signal_strength)-1):
			i=k
			flag=1
#			if signal_strength[k][6]=='-1' or str(signal_strength[k][3]).strip()=='-110' or str(signal_strength[0][4]).strip()=='':
...
#			    pass
		...
		if len(signal_strength)>0 and flag==1: # loop sey out , for last entry
			if signal_strength[i+1][6]=='-1' or str(signal_strength[i+1][3]).strip()=='' or str(signal_strength[i+1][4]).strip()=='':
                ...
                for k in range(0,len(time_stamp_signal1)):
                    total_list.append([time_stamp_signal1[k],host_name[k],ip_address[k],signal_interface1[k],signal_interface2[k],signal_interface3[k],signal_interface4[k],signal_interface5[k],signal_interface6[k],signal_interface7[k],signal_interface8[k],grp_name[k]])
            else:
                host_data=str(all_host.split(',')).replace('[','(').replace(']',')')
                query="Select go16.timestamp , hst.host_name , hst.ip_address , go16.sig_strength, go16.timeslot_index,hostgroups.hostgroup_name,go16.ssidentifier from get_odu16_peer_node_status_table  as go16\
                join ( select host_id,host_name,ip_address from hosts where device_type_id like 'ODU16%%' group by host_id) as hst ON go16.host_id=hst.host_id\
...
# TOTAL DATA FOR A GIVEN DATE PERIOD FOR NETWORK USAGE
    def get_total_data_network_usage(self,no_of_devices,date1,date2,time1,time2,all_group,all_host):
	    ...
	    tr=[]
		    ...
		    host_data=str(all_host.split(',')).replace('[','(').replace(']',')')
		    query="SELECT  go16.index,go16.timestamp,hosts.host_name,hosts.ip_address,go16.rx_bytes,go16.tx_bytes,hostgroups.hostgroup_name FROM get_odu16_nw_interface_statistics_table AS 				go16 JOIN (SELECT host_id, host_name,ip_address FROM hosts WHERE device_type_id LIKE 'ODU16%%' GROUP BY host_id) AS hosts  INNER JOIN hosts_hostgroups ON 				hosts_hostgroups.host_id = hosts.host_id  INNER JOIN hostgroups ON hostgroups.hostgroup_id = hosts_hostgroups.hostgroup_id WHERE go16.host_id = hosts.host_id and 				go16.timestamp between '%s' AND '%s' AND hosts.host_id IN %s order by go16.timestamp asc ,hosts.host_name asc, go16.index asc"%(date_temp1,date_temp2,host_data)
		    cursor.execute (query)
		    res = cursor.fetchall()
		    if(len(res)==0):
			return result_dict
...
# TOTAL DATA FOR A GIVEN DATE PERIOD FOR NETWORK OUTAGE
	...
	try:
	    main_res=get_outage(no_of_devices,date1,date2,time1,time2,all_group,all_host)
	    return main_res
	    conn = MySQLdb.connect (*SystemConfig.get_mysql_credentials())
	    cursor = conn.cursor ()
	    date1=date1.replace("/","-")
	    ...
	    if all_group=='' and all_host=='':
			...
			while(host==str(res[i][2]) and i<length):
				...
				sec2=int(time2[17:])
	   			time_diff=float((h1)*3600+(m1)*60+(sec1))
	      			s1=str(res[i][1])                
				time_diff=float((h2-h1)*3600+(m2-m1)*60+(sec2-sec1))
				s1=str(res[i][1])
				s2=str(res[i+1][1])
				if(s1=='0'):
				    uptime+=time_diff
				...
				i=i+1
				    ...
				    m1=int(time1[14:16])
		                    sec1=int(time1[17:])
		                    s1=str(res[i][1])
				    time_diff=float((23-h1)*3600+(59-m1)*60+(59-sec1))
	   	                else:
		                    time_diff=float((chour-h1)*3600+(cminute-m1)*60+(csecond-sec1))
		                if(s1=='0'):
					uptime+=time_diff
		                else:
					downtime+=time_diff
				...
				uptime_perc=(100*(uptime)/(uptime+downtime))
				if(uptime_perc<0):
				    uptime_perc=0.0
				elif(uptime_perc>100):
				    uptime_perc=100.0
				downtime_perc=100-uptime_perc
			    ls.append(str(uptime_perc)[:str(uptime_perc).find(".")+2])
			    ls.append(str(downtime_perc)[:str(downtime_perc).find(".")+2])
			    ls.append(hstgroup)
		 	    tr.append(ls)
			    ls=[]
		            if(host!=str(res[i][2])):
...
#	    style1.font = font # Add Bold Font to Style
		...
		if flag==0:
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Average_Data),'%(crc_avg[k][5]), cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,4,"Average Error Information",style)
		    xls_sheet.write_merge(1,1,0,4,"%s(Group Name)"%(crc_avg[k][5]),style)
...
#		    xls_sheet.write(3,3,"No. of crc error",style1)
		    i=4
#		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center,color grey25')
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		    headings = ['Data', 'Hostname','IP Address','No. of phy error', 'No. of crc error']
		...
		if crc_avg[len(crc_avg)-1][5]==crc_avg[len(crc_avg)-2][5]:
		    ...
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Average_Data),'%(crc_avg[0][5]), cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,4,"Average Error Information",style)
		    xls_sheet.write_merge(1,1,0,4,"%s(Group Name)"%(crc_avg[0][5]),style)
		    xls_sheet.write_merge(2,2,0,4,"")
		    i=4
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		    headings = ['Data', 'Hostname','IP Address','No. of phy error', 'No. of crc error']
		...
		if flag==0:
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Total_Data)'%(crc_total[k][5]))
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,4,"Total Error Information",style)
		    xls_sheet.write_merge(1,1,0,4,"%s(Group Name)"%(crc_total[k][5]),style)
		    xls_sheet.write_merge(2,2,0,4,"")
		    i=4
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		    headings = ['Data', 'Hostname','IP Address','No. of phy error', 'No. of crc error']
		...
		if crc_total[len(crc_total)-1][5]==crc_total[len(crc_total)-2][5]:
			for j in range(len(crc_total[len(crc_total)-1])-1):
			    width = 5000
			    xls_sheet.write(i,j,crc_total[len(crc_total)-1][j],style1)
			    xls_sheet.col(j).width = width 
	    elif len(crc_total)>0:
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Total_Data)'%(crc_total[0][5]))
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,4,"Total Error Information",style)
		    xls_sheet.write_merge(1,1,0,4,"%s(Group Name)"%(crc_total[0][5]),style)
		    xls_sheet.write_merge(2,2,0,4,"")
		    i=4
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		    headings = ['Data', 'Hostname','IP Address','No. of phy error', 'No. of crc error']
	    ...
	    xls_book.save('/omd/sites/%s/share/check_mk/web/htdocs/download/error_excel.xls' % nms_instance)
	    return '0'
        except Exception,e:
            return str(e)
    ...
    def nw_bandwith_excel_creating(self,nw_total):
		...
		if flag==0:
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Bandwith_data),'%(nw_total[k][9]), cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,8,"Bandwith Information",style)
		    xls_sheet.write_merge(1,1,0,8,"%s(Group Name)"%(nw_total[k][9]),style)
...
#		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center,color grey25')
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		    headings = ['Data', 'Hostname','IP Address','Eth0(Rx)','Eth0(Tx)','Eth1(Rx)','Eth1(Tx)','Br0(Rx)','Br0(Tx)']
		...
		if nw_total[len(nw_total)-1][9]==nw_total[len(nw_total)-2][9]:
			for j in range(len(nw_total[len(nw_total)-1])-1):
			    width = 5000
			    xls_sheet.write(i,j,nw_total[len(nw_total)-1][j],style1)
			    xls_sheet.col(j).width = width
	    elif len(nw_total)>0:
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Bandwith_data),'%(nw_total[0][9]), cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,3,"Bandwith Information",style)
		    xls_sheet.write_merge(1,1,0,3,"%s(Group Name)"%(nw_total[0][9]),style)
...
#		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center,color grey25')
		    ...
		    for j in range(len(nw_total[len(nw_total)-1])-1):
		        ...
		        xls_sheet.write(i,j,nw_total[len(nw_total)-1][j],style1)
		        xls_sheet.col(j).width = width 
	    xls_book.save('/omd/sites/%s/share/check_mk/web/htdocs/download/nw_bandwidth_excel.xls' % nms_instance)
	    return '0'
        except Exception,e:
            return str(e[-1])
    ...
    def rssi_excel_creating(self,rssi_avg,rssi_total):
		...
		if flag==0:
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Aerage_rssi),'%(rssi_avg[k][11]), cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,10,"Average RSSI Information",style)
		    xls_sheet.write_merge(1,1,0,10,"%s(Group Name)"%(rssi_avg[k][11]),style)
		    xls_sheet.write_merge(2,2,0,10,"")
		    i=4
#		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center,color grey25')
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		    headings = ['Data', 'Hostname','IP Address','Peer1','Peer2','Peer3','Peer4','Peer5','Peer6','Peer7','Peer8',]
		...
		if rssi_avg[len(rssi_avg)-1][11]==rssi_avg[len(rssi_avg)-2][11]:
			for j in range(len(rssi_avg[len(rssi_avg)-1])-1):
			    width = 5000
			    xls_sheet.write(i,j,rssi_avg[len(rssi_avg)-1][j],style1)
			    xls_sheet.col(j).width = width
	    elif len(rssi_avg)>0:
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Average_rssi),'%(rssi_avg[0][11]), cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,10,"Average RSSI Information",style)
		    xls_sheet.write_merge(1,1,0,10,"%s(Group Name)"%(rssi_avg[0][11]),style)
		    xls_sheet.write_merge(2,2,0,10,"")
		    i=4
#		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center,color grey25')
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		...
		if flag==0:
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Total_rssi),'%(rssi_total[k][11]), cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,10,"Total RSSI Information",style)
		    xls_sheet.write_merge(1,1,0,10,"%s(Group Name)"%(rssi_total[k][11]),style)
		    xls_sheet.write_merge(2,2,0,10,"")
		    i=4
#		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center,color grey25')
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		...
		if rssi_total[len(rssi_total)-1][11]==rssi_total[len(rssi_total)-2][11]:
			for j in range(len(rssi_total[len(rssi_total)-1])-1):
			    width = 5000
			    xls_sheet.write(i,j,rssi_total[len(rssi_total)-1][j],style1)
			    xls_sheet.col(j).width = width
	    elif len(rssi_total)>0:
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Total_rssi),'%(rssi_total[0][11]), cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,10,"Total RSSI Information",style)
		    xls_sheet.write_merge(1,1,0,10,"%s(Group Name)"%(rssi_total[0][11]),style)
		    xls_sheet.write_merge(2,2,0,10,"")
		    i=4
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		    headings = ['Data', 'Hostname','IP Address','Peer1','Peer2','Peer3','Peer4','Peer5','Peer6','Peer7','Peer8',]
            ...
            xls_book.save('/omd/sites/%s/share/check_mk/web/htdocs/download/rssi_excel.xls' % nms_instance)
            return '0'
        except Exception,e:
            return str(e)
    ...
    def outage_excel_creating(self,outage_total):
		...
		if flag==0:
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Network_Outage),'%(outage_total[k][5]), cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,4,"Devcie Outage Information",style)
		    xls_sheet.write_merge(1,1,0,4,"%s(Group Name)"%(outage_total[k][5]),style)
		    xls_sheet.write_merge(2,2,0,4,"")
#		    xls_sheet.write(3,0,"Date",style1)
#		    xls_sheet.write(3,1,"Hostname",style1)
#		    xls_sheet.write(3,2,"No. of phy error",style1)
...
#		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center,color grey25')
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		    headings = ['Data', 'Hostname','IP Address','Up Time(%)','Down Time(%)']
		...
		if outage_total[len(outage_total)-1][9]==outage_total[len(outage_total)-2][9]:
			for j in range(len(outage_total[len(outage_total)-1])-1):
			    width = 5000
			    xls_sheet.write(i,j,outage_total[len(outage_total)-1][j],style1)
			    xls_sheet.col(j).width = width
	    elif len(outage_total)>0:
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Network_Outage),'%(outage_total[0][5]), cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,4,"Devcie Outage Information",style)
		    xls_sheet.write_merge(1,1,0,4,"%s(Group Name)"%(outage_total[0][5]),style)
		    xls_sheet.write_merge(2,2,0,4,"")
		    i=4
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		    headings = ['Data', 'Hostname','IP Address','Up Time(%)','Down Time(%)']
	    ...
	    xls_book.save('/omd/sites/%s/share/check_mk/web/htdocs/download/outage_excel.xls' % nms_instance)
	    return '0'
        except Exception,e:
            return str(e)
...
# TOTAL TRAP DATA EXCEL FOR A GIVEN DATE PERIOD BY SERVITY
    ...
    def event_excel_creating(self,event_total,alarm_result):
	    ...
	    count=0
	    event_name=['Informational(0)','Normal(1)','Informational(2)','Minor(3)','Major(4)','Critical(5)']
            for k in range(len(event_total)-1):
		if flag==0:
		    i=0
		    #count+=1
		    xls_sheet=xls_book.add_sheet('%s,'%(str(event_name[int(event_total[k][10])])),cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,9,"%s Events Information"%(str(event_name[int(event_total[k][10])])),style)
		    xls_sheet.write_merge(2,2,0,9,"")
		    i=4
#		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center,color grey25')
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		    headings = ['system receive date','system receive time','Event receive date','device type','group name','host name','agent id','event id','event type', 'description']
		...
		for j in range(len(event_total[len(event_total)-1])-1):
		    ...
		    i=0
		    xls_sheet=xls_book.add_sheet('%s,'%(str(event_name[int(event_total[len(event_total)-1][10])])),cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,9,"%s Events Information"%(str(event_name[int(event_total[k][10])])),style)
		    xls_sheet.write_merge(2,2,0,9,"")
		    ...
		    for j in range(len(event_total[len(event_total)-1])-1):
			    if j<8:
				    width = 7000
				    xls_sheet.write(i,j,event_total[len(event_total)-1][j],style1)
				    xls_sheet.col(j).width = width 
			    elif j==8:
				    width = 10000
				    xls_sheet.write(i,j,event_total[len(event_total)-1][j],style1)
		...
		if event_total[len(event_total)-1][10]==event_total[len(event_total)-2][10]:
		    ...
		    i=0
		    xls_sheet=xls_book.add_sheet('%s(Total_event)%s,'%(event_total[k][10],str(event_name[int(event_total[k][10])])),cell_overwrite_ok=True)
		    xls_sheet.row(0).height = 521
		    xls_sheet.row(1).height = 421
		    xls_sheet.write_merge(0,0,0,9,"Informational Events",style)
		    xls_sheet.write_merge(2,2,0,9,"")
		    i=4
#		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center,color grey25')
		    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
	    ...
	    i=0
	    xls_sheet=xls_book.add_sheet("Alarm Information",cell_overwrite_ok=True)
	    xls_sheet.row(0).height = 521
	    xls_sheet.row(1).height = 421
		...
		xls_sheet.write(i-1, colx, value, heading_xf)
			...
			for j in range(len(alarm_result[k])-1):
			    if j<8:
				    ...
				    xls_sheet.col(j).width = width 
			i=i+1
	    xls_book.save('/omd/sites/%s/share/check_mk/web/htdocs/download/Event_excel_report.xls' % nms_instance)
	    return '0'
        except Exception,e:
            return str(e)
    ...
    def inventory_excel_report_creation(self,user_id):
		...
		all_hosts = hst_bll.grid_view_active_host_report()     # fetching all hosts data from database
		...
		for hst in all_hosts:
			if (hst.device_name).strip()=='UBR' or (hst.device_name).strip()=='RM18':
				...
				if len(result)>0:
					ra_mac='--' if result[0][0]==None or result[0][0]==''   else result[0][0]
					freq='--' if result[0][1]==None or result[0][1]==''   else result[0][1]
					gr_name='--' if result[0][2]==None or result[0][2]==''   else result[0][2]
					ac_ver='--' if result[0][3]==None or result[0][3]==''   else result[0][3]
					hw_ver='--' if result[0][4]==None or result[0][4]==''   else result[0][4]
					hw_sr_no='--' if result[0][5]==None or result[0][5]==''   else result[0][5]
					tdmoip='N/A'

			elif (hst.device_name).strip()=='UBRe' or (hst.device_name).strip()=='RM':
				sql="SELECT ra.raMacAddress,fm.rfChanFreq,hostgroups.hostgroup_name,sw.activeVersion ,hw.hwVersion,hw.hwSerialNo FROM hosts LEFT JOIN \
						 odu100_raStatusTable as ra ON ra.host_id=hosts.host_id \
						LEFT JOIN odu100_raTddMacStatusTable as fm ON fm.host_id = hosts.host_id \
						LEFT JOIN hosts_hostgroups ON hosts_hostgroups.host_id = hosts.host_id\
						LEFT JOIN hostgroups ON hostgroups.hostgroup_id = hosts_hostgroups.hostgroup_id\
						LEFT JOIN  odu100_swStatusTable as sw ON sw.host_id=hosts.host_id LEFT JOIN odu100_hwDescTable as hw ON hw.host_id=hosts.host_id\
						 WHERE hosts.ip_address='%s'"%str(hst.ip_address)
				cursor.execute(sql)
				result=cursor.fetchall()
				if len(result)>0:
				...
				if len(result)>0:
					ra_mac='N/A'
					freq='N/A'
					gr_name='--' if result[0][0]==None or result[0][0]==''   else result[0][0]
					ac_ver='--' if result[0][1]==None or result[0][1]==''   else result[0][1]
					hw_ver='--' if result[0][2]==None or result[0][2]==''   else result[0][2]
					hw_sr_no='--' if result[0][3]==None or result[0][3]==''   else result[0][3]
					tdmoip='--' if result[0][4]==None or result[0][4]==''   else result[0][4]
			...
			hst.creation_time != None and hst.creation_time.strftime("%d-%m-%Y %H:%M") or "-",\
			hst.created_by != None and hst.created_by or "-",ra_mac,freq,tdmoip])       # creating 2D List of host details

		xls_sheet=xls_book.add_sheet('Active Host(s)',cell_overwrite_ok=True)
		xls_sheet.row(0).height = 521
		xls_sheet.row(1).height = 421
		xls_sheet.write_merge(0,0,0,10,"Active Host(s) Information",style)
		xls_sheet.write_merge(1,1,0,10,"")
		xls_sheet.write_merge(2,2,0,10,"")
		i=4
		heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		#headings = ['Host Alias','IP Address','Device Name','MAC','RA MAC','Frequency','Group Name','Active Software Version','Hardware Version','Hardware Serial No.','Active Since (Date - Time)','Added By']
		headings = ['Host Alias','IP Address','Device Name','MAC','Group Name','Active Software Version','Hardware Version','Hardware Serial No.','Active Since (Date - Time)','Added By','RA MAC','Frequency','TDMOIP MAC']
		xls_sheet.set_panes_frozen(True) # frozen headings instead of split panes
		xls_sheet.set_horz_split_pos(i) # in general, freeze after last heading row
		xls_sheet.set_remove_splits(True) # if user does unfreeze, don't leave a split there
		for colx, value in enumerate(headings):
		...
		all_hosts = hst_bll.grid_view_disable_host_report()     # fetching all hosts data from database
		disable_host = []                                 # creating empty list [we will use this in datatables]
		for hst in all_hosts:
			if (hst.device_name).strip()=='UBR':
				...
				sql="SELECT ra.raMacAddress,fm.frequency,hostgroups.hostgroup_name,sw.activeVersion ,hw.hwVersion,hw.hwSerialNo FROM hosts LEFT JOIN \
						 odu100_raStatusTable as ra ON ra.host_id=hosts.host_id \
						LEFT JOIN odu100_raChannelListTable as fm ON fm.host_id = hosts.host_id \
						LEFT JOIN hosts_hostgroups ON hosts_hostgroups.host_id = hosts.host_id\
						LEFT JOIN hostgroups ON hostgroups.hostgroup_id = hosts_hostgroups.hostgroup_id\
						LEFT JOIN  odu100_swStatusTable as sw ON sw.host_id=hosts.host_id LEFT JOIN odu100_hwDescTable as hw ON hw.host_id=hosts.host_id\
						 WHERE hosts.ip_address='%s'"%str(hst.ip_address)
			...
			hst.timestamp != None and hst.timestamp.strftime("%d-%m-%Y %H:%M") or "-",\
			hst.updated_by != None and hst.updated_by or "-"])       # creating 2D List of host details
		xls_sheet=xls_book.add_sheet('Disabled Host(s)',cell_overwrite_ok=True)
		xls_sheet.row(0).height = 521
		xls_sheet.row(1).height = 421
		xls_sheet.write_merge(0,0,0,11,"Disabled Host(s) Information",style)
		xls_sheet.write_merge(1,1,0,11,"")
		xls_sheet.write_merge(2,2,0,11,"")
		i=4
		heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		headings = ['Host Alias','IP Address','Device Name','MAC','RA MAC','Frequency','Group Name','Active Software Version','Hardware Version','Hardware Serial No.','Disabled Since (Date - Time)','Disabled By']
		xls_sheet.set_panes_frozen(True) # frozen headings instead of split panes
		xls_sheet.set_horz_split_pos(i) # in general, freeze after last heading row
		xls_sheet.set_remove_splits(True) # if user does unfreeze, don't leave a split there
		for colx, value in enumerate(headings):
		...
		all_hosts = hst_bll.grid_view_deleted_host_report()     # fetching all hosts data from database
		deleted_host = []                                 # creating empty list [we will use this in datatables]
		for hst in all_hosts:
			deleted_host.append([\
		...
		xls_sheet=xls_book.add_sheet('Deleted Host(s)',cell_overwrite_ok=True)
		xls_sheet.row(0).height = 521
		xls_sheet.row(1).height = 421
		xls_sheet.write_merge(0,0,0,6,"Deleted Host(s) Information",style)
		xls_sheet.write_merge(1,1,0,6,"")
		xls_sheet.write_merge(2,2,0,6,"")
		i=4
		heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		headings = ['Host Alias','IP Address','Device Name','MAC Address','Updated By','Time']
		xls_sheet.set_panes_frozen(True) # frozen headings instead of split panes
		xls_sheet.set_horz_split_pos(i) # in general, freeze after last heading row
		xls_sheet.set_remove_splits(True) # if user does unfreeze, don't leave a split there
		for colx, value in enumerate(headings):
		...
		all_hosts = hst_bll.grid_view_tcp_discovered_host_report()     # fetching all hosts data from database
		hosts_list = []                                 # creating empty list [we will use this in datatables]
		s_no = 0
		temp_list = []
		for hst in all_hosts:
		...
		all_hosts2 = hst_bll.grid_view_discovered_host_report()     # fetching all hosts data from database
...
#			ra_mac=cursor.fetchall()
			discovered_host.append([str(hst[1]),str(hst[2]),str(hst[3]),str(hst[4]),str(hst[5])])       # creating 2D List of host details
		xls_sheet=xls_book.add_sheet('Discovered Host(s)',cell_overwrite_ok=True)
		xls_sheet.row(0).height = 521
		xls_sheet.row(1).height = 421
		xls_sheet.write_merge(0,0,0,4,"Discovered Host(s) Information",style)
		xls_sheet.write_merge(1,1,0,4,"")
		xls_sheet.write_merge(2,2,0,4,"")
		i=4
		heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
		headings = ['Discover Type','IP Address','Site MAC','Type','Time']
		xls_sheet.set_panes_frozen(True) # frozen headings instead of split panes
		xls_sheet.set_horz_split_pos(i) # in general, freeze after last heading row
		xls_sheet.set_remove_splits(True) # if user does unfreeze, don't leave a split there
		for colx, value in enumerate(headings):
		...
		xls_book.save('/omd/sites/%s/share/check_mk/web/htdocs/download/inventory_report.xls' % nms_instance)
		return 0
		# Exception Handling
...
def outage_graph(host_id,start_date,end_date):
    ...
    try:
        current_date=datetime.date(datetime.strptime(start_date,'%Y-%m-%d %H:%M:%S'))
...
#            raise SelfException(cursor)
	...
	ip_address=""
        #sel_query="SELECT trap_event_id,trap_event_type,timestamp FROM system_alarm_table WHERE timestamp>='%s' and timestamp<='%s' and agent_id='%s' order by timestamp "%(start_date,end_date,ip_address)
	...
	where host.host_id='%s' and go16.timestamp>='%s' and go16.timestamp<='%s'"%(host_id,start_date,end_date)
        ...
        sel_query="SELECT trap_event_id,trap_event_type FROM system_alarm_table WHERE timestamp<='%s' and agent_id='%s' order by timestamp limit 1"%(start_date,ip_address)
        cursor.execute(sel_query)
	...
	total=0
		...
		if int(last_status)==50002:
		    total_up_time+=abs((result[no][2]-start_time).days*1440+(result[no][2]-start_time).seconds/60)
		    start_time=result[no][2]
		    last_status=result[no][0]
		else:
		    total_down_time+=abs((result[no][2]-start_time).days*1440+(result[no][2]-start_time).seconds/60)
		    start_time=result[no][2]
		    last_status=result[no][0]
	    else:
		if flag==1 and (int(last_status)==50001 or int(last_status)==50002):
		    temp_datetime=datetime.strptime(str(datetime.date(start_time))+" 23:59:59",'%Y-%m-%d %H:%M:%S')
		    start_time=temp_datetime
		    if int(last_status)==50002:
		        total_up_time+=abs((temp_datetime-result[no-1][2]).days*1440+(temp_datetime-result[no-1][2]).seconds/60)
		    else:
		        total_down_time+=abs((temp_datetime-result[no-1][2]).days*1440+(temp_datetime-result[no-1][2]).seconds/60)
		    ...
		    total_up_time=0
		    if int(last_status)==50002:
		        total_up_time+=abs((result[no][2]-temp_datetime).days*1440+(result[no][2]-temp_datetime).seconds/60)
		        start_time=result[no][2]
		        last_status=result[no][0]
		    else:
		        total_down_time+=abs((result[no][2]-temp_datetime).days*1440+(result[no][2]-temp_datetime).seconds/60)
		        start_time=result[no][2]
		        last_status=result[no][0]

		if flag==0 and (int(last_status)==50001 or int(last_status)==50002):
		    temp_datetime=datetime.strptime(str(datetime.date(start_time))+" 23:59:59",'%Y-%m-%d %H:%M:%S')
		    temp_start_datetime=datetime.strptime(str(datetime.date(start_time))+" 00:00:00",'%Y-%m-%d %H:%M:%S')
		    start_time=temp_datetime
		    if int(last_status)==50002:
		        total_up_time+=abs((temp_datetime-temp_start_datetime).days*1440+(temp_datetime-temp_start_datetime).seconds/60)
		    else:
		        total_down_time+=abs((temp_datetime-temp_start_datetime).days*1440+(temp_datetime-temp_start_datetime).seconds/60)
		    date_days.append((temp_date+timedelta(days=(i))))
		    total=total_down_time+total_up_time
		    up_state.append(round((total_up_time*100)/float(total),2))
		    down_state.append(round((total_down_time*100)/float(total),2))
	...
	for j in range(i,(datetime.strptime(end_date,'%Y-%m-%d %H:%M:%S')-datetime.strptime(start_date,'%Y-%m-%d %H:%M:%S')).days):
        ...
        cursor.close()
        db.close()
        output_dict={'success':0,'UpTime':up_state,'DownTime':down_state,'date_days':date_days,'list':total_li}
        return output_dict 
    # Exception Handling
    except Exception as e:
		    ...
		    if temp_date.month > main_date.month or temp_date.day > main_date.day:
		        ...
		        if day_diff > 1:
		            for i in range(1,day_diff+1):
		                ...
		                if prev_value == '50002':
		                    main_list.append([leftout_date,main_ip,tpl_temp[4],tpl_temp[5],tpl_temp[6],timedelta(0, 86399),None])
		                elif prev_value == '50001':
		                    main_list.append([leftout_date,main_ip,tpl_temp[4],tpl_temp[5],tpl_temp[6],None,timedelta(0, 86399)])
		...
		if is_date:
		    if uptime == None or downtime == None:
		        ...
		        if prev_value == '50002':
		            ...
		            downtime = delta
		    main_list.append([main_date,main_ip,tpl_temp[4],tpl_temp[5],tpl_temp[6],uptime,downtime])
		    main_date = temp_date
		    uptime = None
		    downtime = None
		    is_date = 0
...
def get_outage(no_of_devices,date1,date2,time1,time2,all_group,all_host):
	...
	for j in all_host:
	    ...
	    if status_result!=() and result!=():
	        ...
	        t_list=((status_result[0][0],status_result[0][1],t_date,status_result[0][3],status_result[0][4],status_result[0][5],status_result[0][6]),)
	        result=t_list+result
	    temp_res= main_outage(result,datetime.strptime(end_date ,"%Y-%m-%d %H:%M:%S"))   
	    if str(temp_res['success'])=="0":
