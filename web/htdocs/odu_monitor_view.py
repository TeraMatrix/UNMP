...
def get_dashboard_data():
    ...
    get_data=DashboardConfig.get_config_attributes('odu16_dashboard',devcie_type_attr)
    refresh_time=10   # default time
    total_count=10    # default count showing record
    if get_data is not None:
        if get_data[0][0]=='odu16_D':
...
def odu_profiling(h):
    ...
    if es.is_host_allow(user_id,host_id) == 0:    
        ...
        javascript_list = ["js/highcharts.js","js/oduMonitorDashboard.js","js/calendrical.js"]
        html.new_header("UBR Dashboard","","",css_list,javascript_list)
        html.write('<div class=\"form-div\">')
#        host_id = ""
        extr_button={'tag':'button','id':'adSrhODU16','value':'Advance Graph','name':'adSrhODU16'}
        ...
        html.header("UBR Monitoring")
        if device_list_param == [] or device_list_param == None:
            flag=1
        #        html.write(page_header_search(host_id,"","UBR,UBRe",device_type,"enabled","device_type",extr_button))
            ...
            if  int(output)==1:
                ...
                html.write(page_header_search(host_id,mac_address,"UBR,UBRe",device_type,"enabled","device_type",extr_button))
        else:
            html.write(page_header_search(device_list_param[0][0],device_list_param[0][1],"UBR,UBRe",device_list_param[0][2],device_list_state,"device_type",extr_button))
        if host_id == "" or host_id == "None":
            val = ""
            flag=1
            html.write("<div id=\"odu16_show_msg\"></div><div id=\"tab_yo\">There is no profile selected</div>")
        else:
            html.write("<div id=\"odu16_show_msg\"></div><div id=\"tab_yo\" style=\"display:block;overflow:hidden\">")
            flag=0
        ...
        if flag==0:
            ...
            <td style="text-align:right"><button type=\"submit\" class=\"yo-button\" id=\"odu_report_btn\" style=\"margin-top:5px;\" onclick="oduReportGeneration();"><span class=\"save\">Report</span></button></td>\
     <td style="text-align:left"><button type=\"submit\" class=\"yo-button\" style=\"margin-top:5px;\" onclick="oduExcelReportGeneration();"><span class=\"report\">Report</span></button></td>\
            </tr></table>\
	    </div></div>')
        else:
            html.write('</div>')
...
def welcome_odu_monitor_page(h):
    ...
    try:
        ...
        cursor.close()
        db.close()		
        html.write(str(dashboard_table(ip_address,odu_start_date,odu_start_time,odu_end_date,odu_end_time,str(refresh_time),str(total_count))))
...
def dashboard_table(ip_address,odu_start_date,odu_start_time,odu_end_date,odu_end_time,refresh_time,total_count):
	...
	<input class=\"yo-button yo-small\" type=\"button\" name=\"odu_graph_show\" value=\"graph\" id=\"odu_graph_show\" style=\"width:30px;\"/>\
    </div></div>\
    <div id="host_info_div"></div>\
    <table id="device_graph" cellspacing="10px" cellpadding="0" width="100%%">\
        <colgroup>\
    ...
    </table>'% (refresh_time,ip_address,total_count,odu_start_date,odu_start_time,odu_end_date,odu_end_time)
    return dash_str
...
def odu_network_interface_table_graph(h):
    ...
    try:
        ...
        end_time=datetime.strptime(odu_end_date+' '+odu_end_time,"%d/%m/%Y %H:%M")
        cursor = db.cursor()
        if int(limitFlag)==0:
            limit_data=''
        else:
            limit_data=' limit 16'
#		cursor.callproc("odu_network_interface",(start_time,end_time,get_refresh_time(),int(interface_value),ip_address))
#		result=cursor.fetchall()
#		if result is not None:
#			if len(result)>1:
#				for row in result :
#					rx0.append(int(row[1])/1024)
#					tx0.append(int(row[2])/1024)
#					time_stamp0.append(row[3].strftime('%H:%M'))
        sel_query="select odu.index,(odu.rx_bytes),(odu.tx_bytes),odu.timestamp from get_odu16_nw_interface_statistics_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where  h.ip_address='%s' AND  odu.timestamp <= '%s' AND odu.timestamp >='%s' and odu.index = '%s' order by timestamp desc"%(ip_address,end_time,start_time,interface_value)
        sel_query+=limit_data
#		html.write(str(sel_query))
        cursor.execute(sel_query)
        result=cursor.fetchall()
        if result is not None:
            if len(result)>0:
                for i in range(len(result)-1):
                    rx_byte=((int(result[i][1])-int(result[i+1][1]))/1024)
                    tx_byte=((int(result[i][2])-int(result[i+1][2]))/1024)
                    rx0.append( 0 if rx_byte<0 else rx_byte)
                    tx0.append(0 if tx_byte<0 else tx_byte)
                    time_stamp0.append(result[i][3].strftime('%H:%M'))
        ...
        db.close()
        output_dict={'success':0,'interface_rx':rx0,'interface_tx':tx0,'time_stamp0':time_stamp0,'ip_address':ip_address}
        html.write(str(output_dict))
...
def odu_error_graph(h):
    ...
    odu_end_date=html.var('end_date')
    odu_end_time=html.var('end_time')
    count_val=html.var('total_count')
    crc_error=[]
    phy_error=[]
    error_time_stamp=[]
    ip_address=html.var("ip_address")
    ...
    try:
        ...
        if int(limitFlag)==0:
            ...
            limit_data=' limit 16'
        sel_query="select IFNULL((odu.rx_crc_errors),0),IFNULL((odu.rx_phy_error),0),odu.timestamp from  get_odu16_ra_tdd_mac_statistics_entry as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >='%s' AND odu.timestamp <= '%s' order by odu.timestamp desc"%(ip_address,start_time,end_time)
        sel_query+=limit_data
        cursor.execute(sel_query)
        result=cursor.fetchall()
        cursor.close()
        ...
        if result is not None:
            for i in range(len(result)-1) :
                crc_error.append(0 if (int(result[i][0])-int(result[i+1][0]))<0 else (int(result[i][0])-int(result[i+1][0])))
                phy_error.append(0 if (int(result[i][0])-int(result[i+1][1]))<0 else (int(result[i][0])-int(result[i+1][1])))
                error_time_stamp.append(result[i][2].strftime('%H:%M'))
        # reverse the value in list
        error_time_stamp.reverse()
        crc_error.reverse()
        crc_error.reverse()
        output_dict={'success':0,'crc_error':crc_error,'phy_error':phy_error,'time_stamp':error_time_stamp,'time':str(start_time),'end_time':str(end_time)}
        html.write(str(output_dict))
...
def odu_sync_lost_graph(h):
    ...
    try:
        ...
        start_time=datetime.strptime(odu_start_date+' '+odu_start_time,"%d/%m/%Y %H:%M")
        end_time=datetime.strptime(odu_end_date+' '+odu_end_time,"%d/%m/%Y %H:%M")
        sel_query="select IFNULL((odu.sysc_lost_counter),0),odu.timestamp from  get_odu16_synch_statistics_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' order by odu.timestamp desc"%(ip_address,start_time,end_time)
        if int(limitFlag)==0:
            limit_data=''
        else:
            limit_data=' limit 16'
        ...
        if result is not None:
            for i in range(len(result)-1):
                sync_lost.append(0 if (int(result[i][0])-int(result[i+1][0]))<0 else (int(result[i][0])-int(result[i+1][0])))
                sync_time_stamp.append(result[i][1].strftime('%H:%M'))
        # reverse the list
        sync_time_stamp.reverse()
        sync_lost.reverse()
        output_dict={'success':0,'sync_lost':sync_lost,'sync_time_stamp':sync_time_stamp}
        html.write(str(output_dict))
    # Exception Handling  
    except MySQLdb as e:
        output_dict={'success':3,'output':str(e[-1])}
...
def odu_signal_strength_graph(h):
    ...
    limitFlag=html.var("limitFlag")
    ip_address=html.var("ip_address")
    limitFlag=int(limitFlag)
    signal_strength=["peer1","peer2","peer3","peer4","peer5","peer6","peer7","peer8"]
    try:
        # Open database connection
        db,cursor=mysql_connection()
        if db ==1:
        ...
        start_time=datetime.strptime(odu_start_date+' '+odu_start_time,"%d/%m/%Y %H:%M")
        end_time=datetime.strptime(odu_end_date+' '+odu_end_time,"%d/%m/%Y %H:%M")
        # This query calculate the master or slave status
        ...
        status_name='Slave' if int(status)==1 else 'Master'
        ...
        sel_query="select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status from  get_odu16_peer_node_status_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' AND link_status<>'notAssigned' AND tunnel_status<>'notAvailable' order by odu.timestamp desc"%(ip_address,start_time,end_time)
        sel_query+=limit_data
        cursor.execute(sel_query)
        ...
        for k in range(0,len(signal_strength)-1):
            ...
            if count==5 and signal_flag==0:
                break
            if signal_strength[k][3]=='-1' or signal_strength[k][1]==0 or signal_strength[k][0]=='' or signal_strength[k][1]=='':
                count+=1
                ...
                time_stamp_signal1.append(str((signal_strength[k][2]).strftime('%H:%M')))
            else:
        ...
        if len(signal_strength)>0 and flag==0 :
            if signal_strength[0][3]=='-1' or signal_strength[0][1]==0 or signal_strength[0][0]=='' or signal_strength[0][1]=='':
                signal_interface1.append(0)
                signal_interface2.append(0)
                signal_interface3.append(0)
                signal_interface4.append(0)
                ...
                signal_interface7.append(0)
                signal_interface8.append(0)
                time_stamp_signal1.append(str((signal_strength[0][2]).strftime('%H:%M')))
            else:
                default_list[int(signal_strength[0][0])-1]=int(signal_strength[0][1])
                signal_interface1.append(default_list[0])
                signal_interface2.append(default_list[1])
                signal_interface3.append(default_list[2])
                signal_interface4.append(default_list[3])
        ...
        if status_name=='Master':
            ...
            for i in range(peer_count):
                signal_json_data+="{name:'peer%s',data:%s}%s"%(i+1,signal_list[i],',' if i<peer_count-1 else '')
            signal_json_data+="]"
        else:
            signal_json_data="[{name:'peer1',data:%s}]"%(signal_interface1)

#		if status_name=='Master':
#			signal_json_data="[{name:'peer1',data:%s},{name:'peer2',data:%s},{name:'peer3',data:%s},{name:'peer4',data:%s},{name:'peer5',data:%s},{name:'peer6',data:%s},{name:'peer7',data:%s},{name:'peer8',data:%s}]"%(signal_interface1,signal_interface2,signal_interface3,signal_interface4,signal_interface5,signal_interface6,signal_interface7,signal_interface8)
#		else:
#			signal_json_data="[{name:'peer1',data:%s}]"%(signal_interface1)
#
        # close the connection
            ...
            if result is not None:
                if len(result)>0:
                    ...
                    device_detail+='<tbody>\
								...
								<td class="cell-info">'+str('--' if result[5]==None or result[5]==""  else channel.get(int(result[5]),""))+'</td>\
							    </tr>\
							    <tr>\
								<td class="cell-label">\
								    Operation state\
...
def odu_trap_graph(h):
    ...
    try:
        ...
        # set the default value 5 for no of days
        #call.proc("procedure name",(no_of_day,odu ip_address))
        sql="SELECT count(ta.trap_event_id),date(ta.timestamp) ,ta.serevity FROM trap_alarms as ta  where  date(ta.timestamp)<=current_date() and  date(ta.timestamp)>current_date()-%s AND ta.agent_id='%s'  group by serevity,date(ta.timestamp) order by  timestamp desc"%(trap_days,ip_address)
        cursor.execute(sql)
        trap_result=cursor.fetchall()
        if trap_result is not None:
            # store the information in list.
            for i in range(0,trap_days):
                normal1=0
                inforamtional1=0
                minor1=0
                major1=0
                ...
                for row in trap_result:
                    if datetime.date(datetime.now()+timedelta(days=-i))==row[1]:
                        if int(row[2])==0 :
                            ...
                            critical1+=int(row[0])
                time_stamp.append(datetime.date(datetime.now()+timedelta(days=-i)).strftime('%d %b %Y'))
        ...
        cursor.close()
        db.close()
        odu_trap_detail={'success':0,'output':{'graph':[normal,inforamtional,minor,major,critical],'timestamp':time_stamp}}
    ...
    try:
        ...
        if table_option.strip() == "trap": 
            ...
            for i in range(length):
                if i<4:
                    history_trap+='<tr><td class="vertline"><img src=\"%s\" alt=\"%s\" title=\"%s\" class=\"imgbutton\" style=\"width:13px;\"/></td>'% (image_dic[all_traps[i][0]],image_title_name[all_traps[i][0]],image_title_name[all_traps[i][0]])
                    history_trap+='<td>%s</td>'%all_traps[i][1]
                    history_trap+='<td>%s</td>'%all_traps[i][2]
                    history_trap+='<td style="text-align:left;">%s</td></tr>'%all_traps[i][3]
                else:
                    history_trap+='<tr><td class="vertline"><img src=\"%s\" alt=\"%s\" title=\"%s\" class=\"imgbutton\" style=\"width:13px;\"/></td>'% (image_dic[all_traps[i][0]],image_title_name[all_traps[i][0]],image_title_name[all_traps[i][0]])
                    history_trap+='<td >%s</td>'%all_traps[i][1]
                    history_trap+='<td>%s</td>'%all_traps[i][2]
                    history_trap+='<td style="text-align:left;">%s&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp%s</td></tr>'%(all_traps[i][3],(("<a href=\"status_snmptt.py?trap_status=history&ip_address="+ip_address+"\">more>></a>" if len(all_traps) >5 else "" )))
            cursor.close()
            ...
            for i in range(length):
                if i<4:
                    current_alarm_html+='<tr><td class="vertline"><img src=\"%s\" alt=\"%s\" title=\"%s\" class=\"imgbutton\" style=\"width:13px;\"/></td>'% (image_dic[current_alarm[i][0]],image_title_name[current_alarm[i][0]],image_title_name[current_alarm[i][0]])
                    current_alarm_html+='<td>%s</td>'%current_alarm[i][1]
                    current_alarm_html+='<td>%s</td>'%current_alarm[i][2]
                    current_alarm_html+='<td style="text-align:left;">%s</td></tr>'%current_alarm[i][3]
                else:
                    current_alarm_html+='<tr><td class="vertline"><img src=\"%s\" alt=\"%s\" title=\"%s\" class=\"imgbutton\" style=\"width:13px;\"/></td>'% (image_dic[current_alarm[i][0]],image_title_name[current_alarm[i][0]],image_title_name[current_alarm[i][0]])
                    current_alarm_html+='<td>%s</td>'%current_alarm[i][1]
                    current_alarm_html+='<td>%s</td>'%current_alarm[i][2]
                    current_alarm_html+='<td style="text-align:left;">%s&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp%s</td></tr>'%(current_alarm[i][3],(("<a href=\"status_snmptt.py?trap_status=history&ip_address="+ip_address+"\">more>></a>" if len(current_alarm) >5 else "" )))
            cursor.close()
...
# this function take one argument and return the dictionary of outage graph information. 
def odu_outage_graph(h):
    ...
    last_status_end_time=datetime.strptime(str(current_date+timedelta(days=-5))+" 23:59:59",'%Y-%m-%d %H:%M:%S') # convert the string in  datetime. 
    try:
        ...
        if result is not None:
            if len(result)>0:
                for i in range(5):
                                    ...
                                    if last_status=='' and up_flag==0:
                                        if temp_time is not '':
                                            if result[j-1][2]==0 or result[j-1][2]=="0":
                                                total_up_time+=abs((row[1]-temp_time).days*1440+(row[1]-temp_time).seconds/60)
                                                temp_up_time=row[1]
                                            else:
                                                total_down_time+=abs((row[1]-temp_time).days*1440+(row[1]-temp_time).seconds/60)
                                                temp_down_time=row[1]
                                        up_flag=1
                                    elif last_status is not '' and up_flag==0:
                                        up_flag=1
                                        if last_status==0:
                                            total_up_time=abs((row[1]-(temp_date+timedelta(days=-(i+1)))).days*1440+(row[1]-(temp_date+timedelta(days=-(i+1)))).seconds/60)
                                        else:
                                            total_down_time=abs((row[1]-(temp_date+timedelta(days=-(i+1)))).days*1440+(row[1]-(temp_date+timedelta(days=-(i+1)))).seconds/60)
                                else:
                                    if result[j-1][2]==0 or result[j-1][2]=="0":
                                        total_up_time+=abs((row[1]-temp_time).days*1440+(row[1]-temp_time).seconds/60)
                                        temp_up_time=row[1]
                    ...
                    if flag==1:
                        if result[j-1][2]==0:
                            total_up_time=abs((result[j-1][1]-(temp_date+timedelta(days=-i))).days*1440+(result[j-1][1]-(temp_date+timedelta(days=-i))).seconds/60)
                        else:
                            total_down_time=abs((result[j-1][1]-(temp_date+timedelta(days=-i))).days*1440+(result[j-1][1]-(temp_date+timedelta(days=-i))).seconds/60)
                    date_days.append((temp_date+timedelta(days=-(i))).strftime("%d %b %Y"))
                    total=total_up_time+total_down_time
                    if flag==1 and total>0:
                        up_state.append(round((total_up_time*100)/float(total),2))
                        down_state.append(round((total_down_time*100)/float(total),2))
                    else:
                        up_state.append(0)
                        down_state.append(0)
            else:
        ...
        output_dict={'success':0,'up_state':up_state,'down_state':down_state,'date_days':date_days}
        html.write(str(output_dict))
...
def odu_device_report(h):
    ...
    try:
        # create the database connection and check the connection created or not
        db,cursor=mysql_connection()
        if db ==1:
            raise SelfException(cursor)

        # import the report modules 
        ...
        data=[]
        data.append(['UBR '+str(ip_address),str(start_time)+'--'+str(end_time)])
        ...
        t.setStyle(TableStyle([
            ...
            ('ALIGN', (1, 0), (1, 0), 'RIGHT')]))
        odu_report.append(t)
        odu_report.append(Spacer(11,11))

        ########################################### ODU Device Information ########################################
        odu_report.append(Spacer(21,21))
        result1='' 
        # create the cursor
        cursor=db.cursor()
        # get the host id correspondence.	
        ...
        if len(result)>0:
            ...
            table_output=device_information_function(result1,'ODU Device Information')
            #---- Query for get the last reboot time of particular device ------#
            sql="SELECT trap_receive_date from trap_alarms where trap_event_type = 'NODE_UP' and agent_id='%s' order by timestamp desc limit 1"%ip_address
        ...
        cursor.close()
        data1=[]
        data1.append(['','UBR Device Information','',''])
        t = Table(data1, [.021*inch,1.8 * inch, 2.75 * inch, 2.4*inch])
        ...
        odu_report.append(Spacer(31,31))
        cursor=db.cursor()
        result1=''

        if int(select_option)==0:
            sel_query="select IFNULL((odu.rx_crc_errors),0),IFNULL((odu.rx_phy_error),0),odu.timestamp from  get_odu16_ra_tdd_mac_statistics_entry as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp <='%s' AND odu.timestamp >= '%s' order by odu.timestamp desc"%(ip_address,end_time,start_time)
            if int(limitFlag)==0:
                limit_data=''
            else:
                limit_data=' limit 16'
            ...
            sel_query="select IFNULL((odu.rx_crc_errors),0),IFNULL((odu.rx_phy_error),0),odu.timestamp from  get_odu16_ra_tdd_mac_statistics_entry as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s order by odu.timestamp desc"%(ip_address,total_days)
        cursor.execute(sel_query)
        result1=cursor.fetchall()
        table_output=table_list_creation(result1,'CRC/PHY ERROR','Crc Error(error count)','Phy Error(error count)','Time(HH:MM:SS)',)
        # close the database and cursor connection.
        cursor.close()
        data1=[]
        ...
        if int(select_option)==0:
            ...
            sel_query="select IFNULL((odu.sysc_lost_counter),0),odu.timestamp from  get_odu16_synch_statistics_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s order by odu.timestamp desc"%(ip_address,total_days)
        cursor.execute(sel_query)
        result1=cursor.fetchall()
        table_output=sync_table_list_creation(result1,'Sync Lost','Sync Lost(error count)','Time(HH:MM:SS)',)
        # close the database and cursor connection.
        cursor.close()
        data1=[]
        ...
        ##################################### Signal Strength Graph Table Report ######################################
        odu_report.append(Spacer(31,31))
        cursor=db.cursor()
        status=0
        signal_flag=1
        ...
        if int(select_option)==0:
            ...
            sel_query="select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status from  get_odu16_peer_node_status_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s AND link_status<>'notAssigned' AND tunnel_status<>'notAvailable' order by odu.timestamp desc"%(ip_address,total_days)
        cursor.execute(sel_query)
        signal_strength=cursor.fetchall()
        count=0
        flag=0
        ...
        for k in range(0,len(signal_strength)-1):
            ...
            if signal_strength[k][3]=='-1' or signal_strength[k][1]==0 or signal_strength[k][0]=='' or signal_strength[k][1]=='':
                ...
                signal_interface7.append(0)
                signal_interface8.append(0)
                time_stamp_signal1.append(str((signal_strength[k][2]).strftime('%d-%m-%Y %H:%M')))
            else:
                if signal_strength[k][2]==signal_strength[k+1][2]:
                    default_list[int(signal_strength[k][0])-1]=int(signal_strength[k][1])
                else:
        ...
        if len(signal_strength)>0 and flag==0 :
            if signal_strength[0][3]=='-1' or signal_strength[0][1]==0 or signal_strength[0][0]=='' or signal_strength[0][1]=='':
                ...
                signal_interface7.append(0)
                signal_interface8.append(0)
                time_stamp_signal1.append(str((signal_strength[0][2]).strftime('%d-%m-%Y %H:%M')))
            else:
                default_list[int(signal_strength[0][0])-1]=int(signal_strength[0][1])
                signal_interface1.append(default_list[0])
                signal_interface2.append(default_list[1])
        ...
        data1=[]
        data1.append(['','RSSI','',''])
        t = Table(data1, [.021*inch,.51 * inch, 3.87 * inch, 2.57*inch])
        t.setStyle(TableStyle([('BACKGROUND',(1,0),(1,0),(0.35,0.35,0.35)),('FONT', (0, 0), (1, 0), 'Helvetica', 11),('TEXTCOLOR',(1,0),(2,0),colors.white)]))
        odu_report.append(t)
        if status_name=='Master':
            table_output=signal_table_creation('RSSI',time_stamp_signal1,signal_interface1,signal_interface2,signal_interface3,signal_interface4,signal_interface5,signal_interface6,signal_interface7,signal_interface8,status_name)
            t = Table(table_output,[1.5*inch,.7*inch,.7*inch,.7*inch,.7*inch,.7*inch,.7*inch,.7*inch,.7*inch])
            t.setStyle(TableStyle([('FONT', (0, 0), (8, 0), 'Helvetica-Bold', 10),        
                                   ('FONT', (0, 1), (8, int(len(table_output))-1), 'Helvetica', 9),        
                                   ('ALIGN', (1,0), (8, int(len(table_output))-1), 'CENTER'),
                                   ('BACKGROUND',(0,0),(9,0),(0.9,0.9,0.9)),
                                   ('LINEABOVE',(0,0), (9, 0), 1.21,(0.35,0.35,0.35)),
                                   ('GRID',(0,0),(8,int(len(table_output))-1),0.31,(0.75,0.75,0.75))]))
            for i in range(1,len(table_output)):
                if i%2==1:
                    t.setStyle(TableStyle([('BACKGROUND',(1,i),(8,i),(0.95,0.95,0.95)),
                                           ('BACKGROUND',(0,i-1),(0,i-1),(0.98,0.98,0.98)),
                                           ('BACKGROUND',(1,i-1),(8,i-1),(0.9,0.9,0.9))
                                           ]))
            odu_report.append(t)
            data1=[]
            if len(table_output)>1:
            ...
            table_output=signal_table_creation('RSSI',time_stamp_signal1,signal_interface1,status_name)
        ...
        if int(select_option)==0:
            ...
            sql="SELECT ta.serevity,ta.trap_event_id,ta.trap_event_type,ta.trap_receive_date FROM trap_alarm_current as ta WHERE ta.agent_id='%s' AND date(ta.timestamp)<=current_date() and  date(ta.timestamp)>=current_date()-%s order by ta.timestamp "%(ip_address,total_days)

#	sql="SELECT ta.serevity,ta.trap_event_id,ta.trap_event_type,ta.trap_receive_date FROM trap_alarm_current as ta WHERE ta.agent_id='%s' order by ta.timestamp desc limit 5 "%ip_address
        cursor.execute(sql)
        result1=cursor.fetchall()
        ...
        t.setStyle(TableStyle([('FONT', (0, 0), (3, 0), 'Helvetica-Bold', 10),        
                               ('FONT', (0, 1), (3, int(len(table_output))-1), 'Helvetica', 9),        
                               ('ALIGN', (1,0), (3, int(len(table_output))-1), 'CENTER'),
                               ('BACKGROUND',(0,0),(3,0),(0.9,0.9,0.9)),
                               ('LINEABOVE',(0,0), (3, 0), 1.21,(0.35,0.35,0.35)),
                               ('GRID',(0,0),(8,int(len(table_output))-1),0.31,(0.75,0.75,0.75))]))
        for i in range(1,len(table_output)):
            if i%2==1:
                t.setStyle(TableStyle([('BACKGROUND',(1,i),(3,i),(0.95,0.95,0.95)),
                                       ('BACKGROUND',(0,i-1),(0,i-1),(0.98,0.98,0.98)),
                                       ]))
            else:
        ...
        if int(select_option)==0:
            ...
            sql="SELECT ta.serevity,ta.trap_event_id,ta.trap_event_type,ta.trap_receive_date FROM trap_alarms as ta WHERE ta.agent_id='%s' AND date(ta.timestamp)<=current_date() and  date(ta.timestamp)>=current_date()-%s order by ta.timestamp "%(ip_address,total_days)
        #sql="SELECT ta.serevity,ta.trap_event_id,ta.trap_event_type,ta.trap_receive_date FROM trap_alarms as ta WHERE ta.agent_id='%s' order by ta.timestamp desc limit 5 "%ip_address

        # create the cursor
        cursor=db.cursor()
        cursor.execute(sql)
        result1=cursor.fetchall()
        table_output=table_list_trap(result1,'Latest 5 Traps')
        # close the database and cursor connection.
        cursor.close()
        data1=[]
        ...
        result1=cursor.fetchall()
        table_output=table_list_alarm(result1,'Last 10 Days Events Information')
        # close the database and cursor connection.
        cursor.close()
        data1=[]
        ...
        odu_report.append(t)
        data=table_output
        t = Table(data, [1.45 * 1.1*inch,1.1*inch,1.1*inch,1.1*inch,1.1*inch,1.1*inch])
        t.setStyle(TableStyle([('FONT', (0, 0), (5, 0), 'Helvetica-Bold', 10),        
                               ('FONT', (0, 1), (5, int(len(table_output))-1), 'Helvetica', 9),        
                               ('ALIGN', (1,0), (5, int(len(table_output))-1), 'CENTER'),
                               ('BACKGROUND',(0,0),(5,0),(0.9,0.9,0.9)),
                               ('LINEABOVE',(0,0), (5, 0), 1.21,(0.35,0.35,0.35)),
                               ('GRID',(0,0),(8,int(len(table_output))-1),0.31,(0.75,0.75,0.75))]))
        ...
        for i in range(1,len(table_output)):
            if i%2==1:
                t.setStyle(TableStyle([('BACKGROUND',(1,i),(5,i),(0.95,0.95,0.95)),
                                       ('BACKGROUND',(0,i-1),(0,i-1),(0.98,0.98,0.98)),
                                       ]))
            else:
        ...
        ########################################### ODU INTERFACE ######################################## 
        nw_interface=['NETWORK BANDWIDTH RATE(eth0)','NETWORK BANDWIDTH RATE(bro)','NETWORK BANDWIDTH RATE(eth1)']
        interface_index=1
        for interface in nw_interface:
            ...
            result1=''
            if int(select_option)==0:
                sel_query="select odu.index,(odu.rx_bytes),(odu.tx_bytes),odu.timestamp from get_odu16_nw_interface_statistics_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where  h.ip_address='%s' AND  odu.timestamp >='%s' AND odu.timestamp <='%s' and odu.index = '%s' order by timestamp desc"%(ip_address,start_time,end_time,interface_index)
                if int(limitFlag)==0:
                    limit_data=''
                else:
                    limit_data=' limit 16'
                ...
                sel_query="select odu.index,(odu.rx_bytes),(odu.tx_bytes),odu.timestamp from get_odu16_nw_interface_statistics_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where  h.ip_address='%s' AND  date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s and odu.index = '%s' order by timestamp desc"%(ip_address,total_days,interface_index)
            # prepare SQL query to get total number of access points in this system
            cursor.execute(sel_query)
            result1=cursor.fetchall()
            table_output=nw_table_list_creation(result1,interface,'Receving Bytes','Transmitting Bytes','Time(HH:MM:SS)',)
            # close the database and cursor connection.
            cursor.close()

            data1=[]
            data1.append(['',interface,'',''])
            t = Table(data1, [.021*inch,2.75 * inch, 2.12 * inch, 2.12*inch])
            t.setStyle(TableStyle([('BACKGROUND',(1,0),(1,0),(0.35,0.35,0.35)),('FONT', (0, 0), (1, 0), 'Helvetica', 11),('TEXTCOLOR',(1,0),(2,0),colors.white)]))
            odu_report.append(t)
            data=table_output
            t = Table(data, [2.7 * inch, 2.2 * inch, 2.2*inch])
            t.setStyle(TableStyle([('FONT', (0, 0), (2, 0), 'Helvetica-Bold', 10),        
                                   ...
                                   ('LINEABOVE',(0,0), (2, 0), 1.21,(0.35,0.35,0.35)),
                                   ('GRID',(0,0),(8,int(len(table_output))-1),0.31,(0.75,0.75,0.75))]))
        ...
        up_flag=0
        current_date=datetime.date(datetime.now())
        current_datetime=datetime.strptime(str(current_date+timedelta(days=-(total_days)))+" 00:00:00",'%Y-%m-%d %H:%M:%S') # convert the string in  datetime. 
        last_datetime=datetime.strptime(str(current_date)+" 23:59:59",'%Y-%m-%d %H:%M:%S')
        temp_date=last_datetime

        # this datetime last status calculation
        last_status_current_time=datetime.strptime(str(current_date+timedelta(days=-(total_days+1)))+" 00:00:00",'%Y-%m-%d %H:%M:%S') # convert the string in  datetime.
        last_status_end_time=datetime.strptime(str(current_date+timedelta(days=-(total_days)))+" 23:59:59",'%Y-%m-%d %H:%M:%S') # convert the string in  datetime. 
	   ...
	   where nagios_statehistory.state_time between '%s'  and '%s' and nagios_hosts.address='%s'\
        ...
        pdf_doc.build(odu_report)
        output_dict={'success':0,'output':'Report Generated Successfully'}
        html.write(str(output_dict))
        # Exception Handling  
    except MySQLdb as e:
        output_dict={'success':3,'output':str(e[-1])}
...
def table_list_creation(result,table_name,first_header,second_header,time):
    ...
    for i in range(0,len(result)-1):
        temp_list=[]
        temp_list=[result[i][2].strftime('%d-%m-%Y %H:%M'),0 if (int(result[i][0])-int(result[i+1][0]))<0 else (int(result[i][0])-int(result[i+1][0])),0 if (int(result[i][1])-int(result[i+1][1]))<0 else (int(result[i][1])-int(result[i+1][1]))]
...
def nw_table_list_creation(result,table_name,first_header,second_header,time):
    ...
    for i in range(0,len(result)-1):
        temp_list=[]
        temp_list=[result[i][3].strftime('%d-%m-%Y %H:%M'),0 if (int(result[i][1])-int(result[i+1][1]))<0 else (int(result[i][1])-int(result[i+1][1])),0 if (int(result[i][2])-int(result[i+1][2]))<0 else (int(result[i][2])-int(result[i+1][2]))]
        output.append(temp_list)
    return output  
...
def table_list_alarm(result,table_name):
    ...
    for i in range(0,len(result)):
        temp_list=[]
        temp_list=[result[i][6].strftime('%d %b %Y'),result[i][1],result[i][2],result[i][3],result[i][4],result[i][5]]
        output.append(temp_list)
    return output  
...
def table_list_trap(result,table_name):
    ...
    for i in range(0,length):
        temp_list=[]
        temp_list=[result[i][0],result[i][1],result[i][2],datetime.strptime(str(result[i][3]),'%a %b %d %H:%M:%S %Y').strftime('%d %b %Y')]
        output.append(temp_list)
    return output  
...
def device_information_function(result,table_name):
    ...
    default_node_type={0:'rootRU',1:'t1TDN',2:'t2TDN',3:'t2TEN'}
    operation_state={0:'disabled',1:'enabled'}
    channel={0:'raBW5Mhz',1:'raBW10Mhz',2:'raBW20Mhz',-1:''}
    device_field=['Frequency','Slaves','Active Version','Hardware Version','Interfaces','Last Reboot Reason','Channel','Operation state','Node Type','MAC Address','Last Reboot Time']
    output=[]
    output.append([device_field[0],str('--' if result[0][0]==None or result[0][0]==""  else result[0][0])])
    output.append([device_field[1],str('--' if result[0][1]==None or result[0][1]==""  else result[0][1])])
    output.append([device_field[2],str('--' if result[0][2]==None or result[0][2]==""  else result[0][2])])
    output.append([device_field[3],str('--' if result[0][3]==None or result[0][3]==""  else result[0][3])])
    output.append([device_field[5],str('--' if result[0][4]==None or result[0][4]==""  else last_reboot_resion[result[0][4]])])
    output.append([device_field[6],str('--' if result[0][5]==None or result[0][5]==""  else channel[result[0][5]])])
    output.append([device_field[7],str('--' if result[0][6]==None or result[0][6]==""  else operation_state[result[0][6]])])
    output.append([device_field[8],str('--' if result[0][7]==None or result[0][7]==""  else default_node_type[result[0][7]])])
    output.append([device_field[9],str('--' if result[0][8]==None or result[0][8]==""  else result[0][8])])
    return output  
...
def signal_table_creation(table_name,time_stamp,peer1=[],peer2=[],peer3=[],peer4=[],peer5=[],peer6=[],peer7=[],peer8=[],status=''):
    output=[]
    if status=='Master':
        output.append(['Time(HH:MM)','peer1','peer2','peer3','peer4','peer5','peer6','peer7','peer8'])
        k=0
        for time_field in time_stamp:
            output.append([str(time_field),str(peer1[k]),str(peer2[k]),str(peer3[k]),str(peer4[k]),str(peer5[k]),str(peer6[k]),str(peer7[k]),str(peer8[k])])
            k=k+1
    else:
        output.append(['Time(HH:MM)','peer1'])
        k=0
        for time_field in time_stamp:
            output.append([str(time_field),str(peer1[k])])
            k=k+1
    return output
...
def outage_graph_generation(date_time,down_state,up_state):
    ...
    for date_time1 in date_time:
        output.append([date_time1,round(up_state[i],2),round(down_state[i],2)])
        i+=1
    return output
...
def sync_table_list_creation(result,table_name,first_header,time):
    ...
    for i in range(0,len(result)-1):
        temp_list=[]
        temp_list=[result[i][1].strftime('%d-%m-%Y %H:%M'),0 if (int(result[i][0])-int(result[i+1][0]))<0 else (int(result[i][0])-int(result[i+1][0]))]
        output.append(temp_list)
    return output  
...
######################################## ODU device spacific Excel report generating #################################
...
def odu_excel_report_genrating(h):
    ...
    try:
        ...
        if len(host_result)>0:
            device_type=('--' if host_result[0][0]=='' or host_result[0][0]==None else host_result[0][0])
            device_name=('--' if host_result[0][1]=='' or host_result[0][1]==None else host_result[0][1])
        ...
        xls_sheet=xls_book.add_sheet('device_information',cell_overwrite_ok=True)
        xls_sheet.row(0).height = 521
        xls_sheet.row(1).height = 421
        xls_sheet.write_merge(0,0,0,2,"UBR Device Information",style)
        xls_sheet.write_merge(1,1,0,2,str(device_type)+'       '+str(device_name)+ '         '+str(ip_address) ,style)
...
#		heading=[table_output[0][1],table_output[0][2],table_output[0][3],table_output[0][4],table_output[0][5],table_output[0][6],table_output[0][7]]
        ...
        for i in range(0,len(crc_result)-1):
            temp_list=[]
            temp_list=[crc_result[i][2].strftime('%d-%m-%Y %H:%M'),0 if (int(crc_result[i][0])-int(crc_result[i+1][0]))<0 else (int(crc_result[i][0])-int(crc_result[i+1][0])),0 if (int(crc_result[i][1])-int(crc_result[i+1][1]))<0 else (int(crc_result[i][1])-int(crc_result[i+1][1]))]
        ...
        for i in range(0,len(sync_result)-1):
            temp_list=[]
            temp_list=[sync_result[i][1].strftime('%d-%m-%Y %H:%M'),0 if (int(sync_result[i][0])-int(sync_result[i+1][0]))<0 else (int(sync_result[i][0])-int(sync_result[i+1][0]))]
...
#		sync_list=sorted(sync_list, key=itemgetter(0))
        ...
        network_bandwidth=['eth0','br0','eth1']
        for index in range(1,4):
            if int(select_option)==0:
                sel_query="select odu.index,(odu.rx_bytes),(odu.tx_bytes),odu.timestamp from get_odu16_nw_interface_statistics_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where  h.ip_address='%s' AND  odu.timestamp >='%s' AND odu.timestamp <='%s' and odu.index = '%s' order by timestamp desc"%(ip_address,start_time,end_time,index)
                if int(limitFlag)==0:
                    limit_data=''
                else:
                    limit_data=' limit 16'
                ...
                sel_query="select odu.index,(odu.rx_bytes),(odu.tx_bytes),odu.timestamp from get_odu16_nw_interface_statistics_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where  h.ip_address='%s' AND  date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s and odu.index = '%s' order by timestamp desc"%(ip_address,total_days,index)
            cursor.execute(sel_query)
            ...
            for i in range(0,len(nw_result)-1):
                temp_list=[]
                temp_list=[nw_result[i][3].strftime('%d-%m-%Y %H:%M'),0 if (int(nw_result[i][1])-int(nw_result[i+1][1]))<0 else (int(nw_result[i][1])-int(nw_result[i+1][1])),0 if (int(nw_result[i][2])-int(nw_result[i+1][2]))<0 else (int(nw_result[i][2])-int(nw_result[i+1][2]))]
                network_list.append(temp_list)

            xls_sheet=xls_book.add_sheet('network_bandwidth(%s)'%network_bandwidth[index-1],cell_overwrite_ok=True)
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
            xls_sheet.write_merge(0,0,0,2,"Network Bandwidth Information (%s)"%network_bandwidth[index-1],style)
            xls_sheet.write(1,0,device_type,style)
            xls_sheet.write(1,1,device_name,style)
            xls_sheet.write(1,2,ip_address,style)
            xls_sheet.write_merge(2,2,0,2,"")
        ...
        if int(select_option)==0:
            ...
            sel_query="select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status from  get_odu16_peer_node_status_table as odu INNER JOIN hostatus=0 if status_result[0][0]==None  else status_result[0][0]sts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s AND link_status<>'notAssigned' AND tunnel_status<>'notAvailable' order by odu.timestamp desc"%(ip_address,total_days)
        cursor.execute(sel_query)
        signal_strength=cursor.fetchall()
        count=0
        flag=0
        ...
        if status_name=='Master':
            for time_field in time_stamp_signal1:
                signal_list.append([str(time_field),str(signal_interface1[k]),str(signal_interface2[k]),str(signal_interface3[k]),str(signal_interface4[k]),str(signal_interface5[k]),str(signal_interface6[k]),str(signal_interface7[k]),str(signal_interface8[k])])
                k=k+1

            xls_sheet=xls_book.add_sheet('Signal_strength',cell_overwrite_ok=True)
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
            xls_sheet.write_merge(0,0,0,8,"Signal Strength Information",style)
            xls_sheet.write_merge(1,1,0,8,str(device_type)+'       '+str(device_name)+ '         '+str(ip_address) ,style)
            xls_sheet.write_merge(2,2,0,2,"")
            i=4
            heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
            headings = ['Time','Peer1','Peer2','Peer3','Peer4','Peer5','Peer6','Peer7','Peer8']
            xls_sheet.set_panes_frozen(True) # frozen headings instead of split panes
            xls_sheet.set_horz_split_pos(i) # in general, freeze after last heading row
            xls_sheet.set_remove_splits(True) # if user does unfreeze, don't leave a split there
            for colx, value in enumerate(headings):
            ...
            for time_field in time_stamp_signal1:
                signal_list.append([str(time_field),str(signal_interface1[k])])
                k=k+1
            signal_list=sorted(signal_list, key=itemgetter(0))
            xls_sheet=xls_book.add_sheet('Signal_strength',cell_overwrite_ok=True)
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
            xls_sheet.write_merge(0,0,0,2,"Signal Strength Information",style)
            xls_sheet.write_merge(1,1,0,2,str(device_type)+'       '+str(device_name)+ '         '+str(ip_address) ,style)
            xls_sheet.write_merge(2,2,0,2,"")
            i=4
            heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
        ...
        sql="SELECT count(ta.trap_event_id),date(ta.timestamp) ,ta.serevity FROM trap_alarms as ta  where  date(ta.timestamp)<=current_date() and  date(ta.timestamp)>=current_date()-%s AND ta.agent_id='%s'  group by serevity,date(ta.timestamp) order by  timestamp desc"%(total_days if int(total_days)>5 else 5,ip_address)
        cursor.execute(sql)
        trap_result=cursor.fetchall()
        if trap_result is not None:
            # store the information in list.
        ...
        for time in time_stamp:
            event_list.append([int(inforamtional[k]),int(normal[k]),int(minor[k]),int(major[k]),int(critical[k]),str(time_stamp[k])])
            k+=1

        xls_sheet=xls_book.add_sheet('event_count_information',cell_overwrite_ok=True)
        xls_sheet.row(0).height = 521
        xls_sheet.row(1).height = 421
        xls_sheet.write_merge(0,0,0,5," %s Days Event Information"%'Current' if total_days==0 else total_days,style)
        xls_sheet.write_merge(1,1,0,5,str(device_type)+'       '+str(device_name)+ '         '+str(ip_address) ,style)
        xls_sheet.write_merge(2,2,0,5,"")
        i=4
        heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
        headings = ['Informational','Normal','Minor','Major','Critical','Time']
        xls_sheet.set_panes_frozen(True) # frozen headings instead of split panes
        xls_sheet.set_horz_split_pos(i) # in general, freeze after last heading row
        xls_sheet.set_remove_splits(True) # if user does unfreeze, don't leave a split there
        for colx, value in enumerate(headings):
        ...
        trap_days=((end_time-start_time).days)
        if int(select_option)==0:
#			sql="SELECT ta.serevity,ta.trap_event_id,ta.trap_event_type,ta.trap_receive_date FROM trap_alarm_current as ta WHERE ta.agent_id='%s' AND date(ta.timestamp)=current_date()  order by ta.timestamp "%(ip_address)
            sql="SELECT ta.serevity,ta.trap_event_id,ta.trap_event_type,ta.trap_receive_date FROM trap_alarm_current as ta WHERE ta.agent_id='%s' AND ta.timestamp>='%s' AND ta.timestamp<='%s' order by ta.timestamp "%(ip_address,start_time,end_time)
        ...
        cursor.execute(sql)
        trap_result=cursor.fetchall()
        severity_list=['Informational','Normal','Informational','Minor','Major','Critical']

        xls_sheet=xls_book.add_sheet('alarm_information',cell_overwrite_ok=True)
        xls_sheet.row(0).height = 521
        xls_sheet.row(1).height = 421
        xls_sheet.write_merge(0,0,0,3,"%s Days Alarm Information"%'Current' if trap_days==0 else trap_days,style)
        xls_sheet.write_merge(1,1,0,3,str(device_type)+'       '+str(device_name)+ '         '+str(ip_address) ,style)
        ...
        for k in range(len(trap_result)):
            for j in range(len(trap_result[k])):
                if j==0:
                    width = 5000
                    xls_sheet.write(i,j,severity_list[trap_result[k][j]],style1)
                    xls_sheet.col(j).width = width 
                else:
                    width = 5000
        ...
        if int(select_option)==0:
            ...
            sql="SELECT ta.serevity,ta.trap_event_id,ta.trap_event_type,ta.trap_receive_date FROM trap_alarms as ta WHERE ta.agent_id='%s' AND date(ta.timestamp)<=current_date() and  date(ta.timestamp)>=current_date()-%s order by ta.timestamp "%(ip_address,trap_days)
        cursor.execute(sql)
        trap_result=cursor.fetchall()
        severity_list=['Informational','Normal','Informational','Minor','Major','Critical']

        xls_sheet=xls_book.add_sheet('event_information',cell_overwrite_ok=True)
        xls_sheet.row(0).height = 521
        xls_sheet.row(1).height = 421
        xls_sheet.write_merge(0,0,0,3,"%s Days Event Information"%'Current' if trap_days==0 else trap_days,style)
        xls_sheet.write_merge(1,1,0,3,str(device_type)+'       '+str(device_name)+ '         '+str(ip_address) ,style)
        xls_sheet.write_merge(2,2,0,3,"")
        i=4
        heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
        ...
        xls_sheet=xls_book.add_sheet('outage_information',cell_overwrite_ok=True)
        xls_sheet.row(0).height = 521
        xls_sheet.row(1).height = 421
        xls_sheet.write_merge(0,0,0,2,"%s Days Outage Information"%'Current' if total_days==0 else total_days,style)
        xls_sheet.write_merge(1,1,0,2,str(device_type)+'       '+str(device_name)+ '         '+str(ip_address) ,style)
        xls_sheet.write_merge(2,2,0,2,"")
        i=4
        heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
...
#		if len(crc_result)>0:
        xls_book.save('/omd/sites/%s/share/check_mk/web/htdocs/download/odu_specific_excel_report.xls' % nms_instance)
        output_dict={"success":0,'output':'Report Generated Successfully.'}
        html.write(str(output_dict))
    ...
    # Exception Handling  
        ...
        output_dict={'success':1,'output':str(e[-1]),'flag':flag}
        html.write(str(output_dict))
    finally:
        if db.open:
            db.close()
