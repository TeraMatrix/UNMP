...
#        host_id = ""
        extr_button = {'tag': 'button', 'id': 'adSrhODU16',
                       'value': 'Advance Graph', 'name': 'adSrhODU16'}
        ...
        if host_id == "" or host_id == "None":
            ...
            flag = 1
            html.write(
                "<div id=\"odu16_show_msg\"></div><div id=\"tab_yo\">There is no profile selected</div>")
        else:
            html.write(
                "<div id=\"odu16_show_msg\"></div><div id=\"tab_yo\" style=\"display:block;overflow:hidden\">")
            flag = 0
    ...
    try:
        ...
        html.write(
            str(dashboard_table(ip_address, odu_start_date, odu_start_time,
                                odu_end_date, odu_end_time, str(refresh_time), str(total_count))))
...
#					time_stamp0.append(row[3].strftime('%H:%M'))
        sel_query = "select odu.index,(odu.rx_bytes),(odu.tx_bytes),odu.timestamp from get_odu16_nw_interface_statistics_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where  h.ip_address='%s' AND  odu.timestamp <= '%s' AND odu.timestamp >='%s' and odu.index = '%s' order by timestamp desc" % (
        ...
        if result is not None:
            if len(result) > 0:
                for i in range(len(result) - 1):
                    rx_byte = (
                        (int(result[i][1]) - int(result[i + 1][1])) / 1024)
                    tx_byte = (
                        (int(result[i][2]) - int(result[i + 1][2])) / 1024)
...
def odu_error_graph(h):
    ...
    try:
        ...
        if int(limitFlag) == 0:
            ...
            limit_data = ' limit 16'
        sel_query = "select IFNULL((odu.rx_crc_errors),0),IFNULL((odu.rx_phy_error),0),odu.timestamp from  get_odu16_ra_tdd_mac_statistics_entry as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >='%s' AND odu.timestamp <= '%s' order by odu.timestamp desc" % (
        ...
        if result is not None:
            for i in range(len(result) - 1):
                crc_error.append(0 if (int(result[i][0]) - int(
                    result[i + 1][0])) < 0 else (int(result[i][0]) - int(result[i + 1][0])))
                phy_error.append(0 if (int(result[i][0]) - int(
                    result[i + 1][1])) < 0 else (int(result[i][0]) - int(result[i + 1][1])))
...
def odu_sync_lost_graph(h):
    ...
    try:
        ...
        end_time = datetime.strptime(
            odu_end_date + ' ' + odu_end_time, "%d/%m/%Y %H:%M")
        sel_query = "select IFNULL((odu.sysc_lost_counter),0),odu.timestamp from  get_odu16_synch_statistics_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' order by odu.timestamp desc" % (
            ip_address, start_time, end_time)
        if int(limitFlag) == 0:
            limit_data = ''
        else:
...
def odu_signal_strength_graph(h):
    ...
    signal_strength = ["peer1", "peer2", "peer3", "peer4", "peer5",
                       "peer6", "peer7", "peer8"]
    try:
        # Open database connection
        db, cursor = mysql_connection()
        ...
        sel_query = "select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status from  get_odu16_peer_node_status_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' AND link_status<>'notAssigned' AND tunnel_status<>'notAvailable' order by odu.timestamp desc" % (
            ip_address, start_time, end_time)
        sel_query += limit_data
        cursor.execute(sel_query)
        ...
        if table_option.strip() == "trap":
            ...
            for i in range(length):
                if i < 4:
                    ...
                    history_trap += '<td style="text-align:left;">%s</td></tr>' % all_traps[
                        i][3]
                else:
                    history_trap += '<tr><td class="vertline"><img src=\"%s\" alt=\"%s\" title=\"%s\" class=\"imgbutton\" style=\"width:13px;\"/></td>' % (
                        image_dic[all_traps[i][0]], image_title_name[all_traps[i][0]], image_title_name[all_traps[i][0]])
                    history_trap += '<td >%s</td>' % all_traps[i][1]
                    history_trap += '<td>%s</td>' % all_traps[i][2]
                    history_trap += '<td style="text-align:left;">%s&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp%s</td></tr>' % (
            ...
            for i in range(length):
                if i < 4:
                    ...
                    current_alarm_html += '<td>%s</td>' % current_alarm[i][1]
                    current_alarm_html += '<td>%s</td>' % current_alarm[i][2]
                    current_alarm_html += '<td style="text-align:left;">%s&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp%s</td></tr>' % (
...
def odu_device_report(h):
    ...
    try:
        ...
        data = []
        data.append(
            ['UBR ' + str(ip_address), str(start_time) + '--' + str(end_time)])
        ...
        if len(result) > 0:
            ...
            table_output = device_information_function(
                result1, 'ODU Device Information')
            #---- Query for get the last reboot time of particular device ------#
            sql = "SELECT trap_receive_date from trap_alarms where trap_event_type = 'NODE_UP' and agent_id='%s' order by timestamp desc limit 1" % ip_address
        ...
        if int(select_option) == 0:
            ...
            sel_query = "select IFNULL((odu.rx_crc_errors),0),IFNULL((odu.rx_phy_error),0),odu.timestamp from  get_odu16_ra_tdd_mac_statistics_entry as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s order by odu.timestamp desc" % (
        ...
        if int(select_option) == 0:
            ...
            sel_query = "select IFNULL((odu.sysc_lost_counter),0),odu.timestamp from  get_odu16_synch_statistics_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s order by odu.timestamp desc" % (
                ip_address, total_days)
        cursor.execute(sel_query)
        result1 = cursor.fetchall()
        ...
        if int(select_option) == 0:
            ...
            sel_query = "select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status from  get_odu16_peer_node_status_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s AND link_status<>'notAssigned' AND tunnel_status<>'notAvailable' order by odu.timestamp desc" % (
                ip_address, total_days)
        cursor.execute(sel_query)
        signal_strength = cursor.fetchall()
        count = 0
        ...
        odu_report.append(t)
        if status_name == 'Master':
            table_output = signal_table_creation(
                'RSSI', time_stamp_signal1, signal_interface1, signal_interface2, signal_interface3, signal_interface4, signal_interface5,
                signal_interface6, signal_interface7, signal_interface8, status_name)
            t = Table(
            ...
            table_output = signal_table_creation(
                'RSSI', time_stamp_signal1, signal_interface1, status_name)
...
# FROM trap_alarm_current as ta WHERE ta.agent_id='%s' order by
# ta.timestamp desc limit 5 "%ip_address
        cursor.execute(sql)
        result1 = cursor.fetchall()
        ...
        # sql="SELECT
        # ta.serevity,ta.trap_event_id,ta.trap_event_type,ta.trap_receive_date
        # FROM trap_alarms as ta WHERE ta.agent_id='%s' order by ta.timestamp
        # desc limit 5 "%ip_address
        ...
        ########################################### ODU INTERFACE #############
        nw_interface = ['NETWORK BANDWIDTH RATE(eth0)', 'NETWORK BANDWIDTH RATE(bro)',
                                                'NETWORK BANDWIDTH RATE(eth1)']
        interface_index = 1
        for interface in nw_interface:
            ...
            if int(select_option) == 0:
                ...
                sel_query = "select odu.index,(odu.rx_bytes),(odu.tx_bytes),odu.timestamp from get_odu16_nw_interface_statistics_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where  h.ip_address='%s' AND  date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s and odu.index = '%s' order by timestamp desc" % (
                    ip_address, total_days, interface_index)
            # prepare SQL query to get total number of access points in this
            # system
            cursor.execute(sel_query)
            result1 = cursor.fetchall()
            table_output = nw_table_list_creation(
                result1, interface, 'Receving Bytes', 'Transmitting Bytes', 'Time(HH:MM:SS)',)
            # close the database and cursor connection.
            cursor.close()
...
def nw_table_list_creation(result, table_name, first_header, second_header, time):
    ...
    for i in range(0, len(result) - 1):
        temp_list = []
        temp_list = [result[i][3].strftime('%d-%m-%Y %H:%M'), 0 if (int(
            result[i][1]) - int(result[i + 1][1])) < 0 else (int(result[i][1]) - int(result[i + 1][1])), 0 if (int(result[i][2]) - int(result[i + 1][2])) < 0 else (int(result[i][2]) - int(result[i + 1][2]))]
        output.append(temp_list)
    return output
...
def table_list_alarm(result, table_name):
    ...
    for i in range(0, len(result)):
        temp_list = []
        temp_list = [result[i][6].strftime('%d %b %Y'), result[i][1], result[i][
                                           2], result[i][3], result[i][4], result[i][5]]
        output.append(temp_list)
    return output
...
def table_list_trap(result, table_name):
    ...
    for i in range(0, length):
        ...
        temp_list = [result[i][0], result[i][1], result[i][2], datetime.strptime(str(
            result[i][3]), '%a %b %d %H:%M:%S %Y').strftime('%d %b %Y')]
        output.append(temp_list)
    return output
...
def device_information_function(result, table_name):
    ...
    output.append([device_field[1], str('--' if result[0][1]
...
def signal_table_creation(table_name, time_stamp, peer1=[], peer2=[], peer3=[], peer4=[], peer5=[], peer6=[], peer7=[], peer8=[], status=''):
    ...
    if status == 'Master':
        ...
        for time_field in time_stamp:
            output.append([str(time_field), str(peer1[k]), str(peer2[k]), str(
                peer3[k]), str(peer4[k]), str(peer5[k]), str(peer6[k]), str(peer7[k]), str(peer8[k])])
            k = k + 1
    else:
...
#		sync_list=sorted(sync_list, key=itemgetter(0))
        ...
        for index in range(1, 4):
            ...
            for i in range(0, len(nw_result) - 1):
                temp_list = []
                temp_list = [nw_result[i][3].strftime('%d-%m-%Y %H:%M'), 0 if (int(nw_result[i][1]) - int(
                    nw_result[i + 1][1])) < 0 else (int(nw_result[i][1]) - int(nw_result[i + 1][1])), 0 if (int(nw_result[i][2]) - int(nw_result[i + 1][2])) < 0 else (int(nw_result[i][2]) - int(nw_result[i + 1][2]))]
        ...
        if status_name == 'Master':
            ...
            xls_sheet.write_merge(
                0, 0, 0, 8, "Signal Strength Information", style)
            xls_sheet.write_merge(1, 1, 0, 8, str(device_type) + '       ' + str(
                device_name) + '         ' + str(ip_address), style)
            xls_sheet.write_merge(2, 2, 0, 2, "")
            i = 4
            heading_xf = xlwt.easyxf(
                'font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
            headings = ['Time', 'Peer1', 'Peer2', 'Peer3',
                'Peer4', 'Peer5', 'Peer6', 'Peer7', 'Peer8']
            xls_sheet.set_panes_frozen(
                True)  # frozen headings instead of split panes
            xls_sheet.set_horz_split_pos(
        ...
        sql = "SELECT count(ta.trap_event_id),date(ta.timestamp) ,ta.serevity FROM trap_alarms as ta  where  date(ta.timestamp)<=current_date() and  date(ta.timestamp)>=current_date()-%s AND ta.agent_id='%s'  group by serevity,date(ta.timestamp) order by  timestamp desc" % (
            total_days if int(total_days) > 5 else 5, ip_address)
        cursor.execute(sql)
        trap_result = cursor.fetchall()
        if trap_result is not None:
            # store the information in list.
