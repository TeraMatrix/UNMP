	...
	try:
		...
		for j in range(start_index,end_index):
		    if(str(li[j])==str(default_value)):
		    	count=count+1
		    if count==(end_index-start_index) or ((report_type=="RSL" or report_type=="RSSI") and count!=0):
	    		    if(do_repeat==1):
    ...
    def get_excel_sheet(self,*params):
		...
		name_report=main_title+"_"+dat1+dat2+"_excel.xls"
                if data_report==[]:
                    ...
                    flag=1
                xls_sheet=xls_book.add_sheet(sheet_name, cell_overwrite_ok=True)
...
#		return {"succcess" : 1 , "result":str(e) }
    ...
    i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,new_user_report_dict,username,hostgroup_id_list):
	    ...
	    if(str(trap_data["success"])=="0" or trap_data["success"]==0):
	        report_data_list=deepcopy(trap_data["result"])
	    else:
		return trap_data # in case of error return
            if(view_type=="data_table"):
                if(trap_data["result"]==[]):
    		        status={"success":0,"aaData":trap_data["result"],
	            			"sEcho":1,"iTotalRecords":0,"iTotalDisplayRecords":0}
    		        return status
    	        report_data_list=trap_data["result"]
		if(str(sSortDir_0)=="asc"):
                    report_data_list2=sorted(report_data_list, key=lambda report_data_list: report_data_list[int(iSortCol_0)],reverse=False)
                else:
                    report_data_list2=sorted(report_data_list, key=lambda report_data_list: report_data_list[int(iSortCol_0)],reverse=True)
	    ...
	    if view_type=="excel":    
		    sheet_dict={
		        ...
		        "main_title":"Event Report" ,
		        "second_title":device_name,
		        "headings":["Date","Host Alias","IP Address","Hostgroup","Normal","Informational","Minor","Major","Critical"],
		        "path_report":'/omd/sites/%s/share/check_mk/web/htdocs/download/' % nms_instance,
		...
		for i in range(1,day_diff+1):
            ...
            if day_diff > 1:
       		...
       		if prev_value == '50002':
       		      ...
       		      main_list.append([leftout_date,tpl_temp[4],tpl_temp[5],tpl_temp[6],None,timedelta(0, 86399)])
	    #if len(main_list) == 0 and len(result_tuple) > 0:
            #    leftout_date=main_date
       	#	if prev_value == '50002':
       	#	      main_list.append([leftout_date,tpl_temp[4],tpl_temp[5],tpl_temp[6],timedelta(0, 86399),None])
       	#	elif prev_value == '50001':
       	#	      main_list.append([leftout_date,tpl_temp[4],tpl_temp[5],tpl_temp[6],None,timedelta(0, 86399)]) 		      
...
def trap_get_total_data_trap(report_type,no_of_devices,date1,date2,time1,time2,all_group,all_host,hostgroup_id_list,device_type):
	    ...
	    if all_host!="":
	        query="SELECT date(ta.timestamp) ,hosts.host_alias,hosts.ip_address,count(ta.trap_event_id),ta.serevity,hostgroups.hostgroup_name\
	        	...
	        	order by  ta.timestamp,hosts.host_name,ta.serevity "%(table_name,','.join(hostgroup_id_list),date_temp1,date_temp2,host_data,device_type)
	    else:
	        query="SELECT date(ta.timestamp) ,hosts.host_alias,hosts.ip_address,count(ta.trap_event_id),ta.serevity,hostgroups.hostgroup_name\
	        FROM %s as ta join (select host_name,host_id,ip_address,host_alias,device_type_id from hosts ) as hosts on hosts.ip_address=ta.agent_id\
	        INNER JOIN hosts_hostgroups ON hosts_hostgroups.host_id = hosts.host_id\
	        ...
	        order by  ta.timestamp,hosts.host_name,ta.serevity "%(table_name,','.join(hostgroup_id_list),date_temp1,date_temp2,device_type)
	    cursor.execute (query)
	    res = cursor.fetchall()
	    if(len(res)==0):
	        return result_dict
