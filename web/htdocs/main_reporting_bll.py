...
class MainReportBll(object):
    ...
    def main_reporting_get_column_template(self,device_type_id,report_type):
        try:
            ...
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            sql=" select column_selected,column_non_selected from report_template where device_type='%s' and report_type='%s'"%(device_type_id,report_type)
            cursor.execute(sql)
            result=cursor.fetchall()
            lis=[]
    ...
    def get_query_dict(self,device_type,report_type,date_temp_1,date_temp_2,all_host):
        try:
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            sql=" select * from report_query_dict where report_type='%s' and device_type='%s'"%(report_type,device_type)
            cursor.execute(sql)
            result=cursor.fetchall()
            query_dict={
    ...
    # get host group name and id with device type and their report type ..this is called initially
    ...
    # Function to form Default Data set ie fill values like '0' when values are not present
            ...
            for i in range(len(res)):
                ...
                # if host are same
                    if str(res[i][host_index])==str(res[i-1][host_index]) and str(res[i][time_index])[:16]==str(res[i-1][time_index])[:16]:
                        ...
                        values[int(res[i][key])*val1+val2]=int(res[i][values1_index])
                        # fill the second value in the value list
                        if(int(values2_index)>=0):
                            values[int(res[i][key])*val3+val4]=int(res[i][values2_index])
...
#   Function to find Delta between Successive Values
    def get_delta_for_values(self,inpt,host_index,data_length,values_length,default_value,is_append,off_value,port_index=-1):
        try:
            ...
            result_dict["success"]=0
            if port_index!=-1:
                result_list=sorted(result_list, key=itemgetter(host_index,0,port_index))
    ...
    # Function to form Default Data set like if values contain default data value ie '1111111' then it will be replaced by 'Device Unreachable'
...
#   Function to get the description of report generated
    def get_description(self,all_host,report_name):
        try:
            db = MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor = db.cursor()
            sql = "SELECT host_alias FROM hosts WHERE host_id IN ('%s')"%("' , '".join(all_host.split(',')))
            cursor.execute(sql)
            result=cursor.fetchall()
    ...
    def main_reporting_get_excel(self,date1,date2,time1,time2,all_host,report_type,column_user,device_type,all_group,view_type,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,new_user_report_dict,username):
        try:
            ...
            result_columns=re.main_reporting_get_mapping_column(device_type,report_type)
            column_value=[]
            column_key=[]
            key_user=[]
            data_report={}
            ...
            if(result_columns["success"]==0 or result_columns["success"]=="0"):
                ...
                #column_key+=result_columns["result"][3].split(";")
                # for i in range column in table if user selected the column then add in key_user
                ...
                # here is code for special reports like Device Reachability,Traps,Clients,Configuration.
                if (str(result_columns["result"][8])=='1'):
                    ...
                    if(report_type.upper()=="DEVICE REACHABILITY" or report_type.upper()=="NETWORK OUTAGE"):
                        ...
                        outage_data=get_outage(3000,date1,date2,time1,time2,all_group,all_host)
                        ...
                        trap_data=get_total_data_trap(3000,date1,date2,time1,time2,all_group,all_host)	
                        ...
                        configuration_data = get_configuration_details(all_host.split(','))
                    ...
                    query_dict=re.get_query_dict(device_type,report_type,date_temp_1,date_temp_2,all_host)
                    if(str(query_dict["success"])=="0"):
                        # 2 . get the data for the report
                        #res_sql=re.get_sql_data(query_dict["result"])
                        if(view_type=="data_table"):
                            dict_shelve=shelve.open('/omd/sites/%s/share/check_mk/web/htdocs/download/%s.db' %(nms_instance,new_user_report_dict["user_id"]))
                            dict_shelve_data=shelve.open('/omd/sites/%s/share/check_mk/web/htdocs/download/%s_data.db' %(nms_instance,new_user_report_dict["user_id"]))
                            if(dict_shelve.has_key("user_info_dict")):
                                user_report_dict=dict_shelve["user_info_dict"]
                            else:
                                user_report_dict={}
                            flag_dict=(user_report_dict!={} and user_report_dict==new_user_report_dict)
                            if flag_dict==True and str(sEcho)!="1":
                                res_sql=dict_shelve_data["res_sql"]
                                dict_shelve.close()
                                dict_shelve_data.close()
                            else:
                                res_sql=re.get_sql_data(query_dict["result"],date_temp_1,date_temp_2)
                                dict_shelve_data["res_sql"]=res_sql
                                dict_shelve["user_info_dict"]=new_user_report_dict
                                dict_shelve.close()
                                dict_shelve_data.close()
                        else:
                            res_sql=re.get_sql_data(query_dict["result"],date_temp_1,date_temp_2)
                        ...
                        if(str(query_dict["group_the_data"])=="1"):
                            ...
                            var=str(query_dict["group_the_data_variables"]).split(",")
                            ...
                            if(len(var)==12):
                                ...
                                #return data_report
                                res_group=re.get_and_form_data_default(data_report,int(var[0]),int(var[1]),int(var[2]),int(var[3]),int(var[4]),int(var[5]),int(var[6]),int(var[7]),int(var[8]),int(var[9]),int(var[10]),int(var[11]))
                        ...
                        if str(query_dict["find_delta"])=="1":
                            ...
                            var_delta=str(query_dict["find_delta_variables"]).split(",")
                            if(len(var_delta)==7):
                                #4 . find the delta
                                #return data_report
                                res_delta=re.get_delta_for_values(data_report,int(var_delta[0]),int(var_delta[1]),int(var_delta[2]),int(var_delta[3]),int(var_delta[4]),int(var_delta[5]),int(var_delta[6]))
                                #return res_delta
                        ...
                        if str(query_dict["default_data"])=="1":
                            #get_delta_for_values(self,inpt,host_index,data_length,values_length):
                            var_default=str(query_dict["default_data_variables"]).split(",")
                            if(len(var_default)==4):
                                #4 . find the delta
                                res_default=re.form_the_default_data(data_report,int(var_default[0]),int(var_default[1]),int(var_default[2]),int(var_default[3]),report_type)
                                #return res_default
                ...
                if view_type=="excel":    
                    ...
                    if str(status["success"])=="0":
                        ...
                        desc=self.get_description(all_host,result_columns["result"][4])
            ...
            for par in params:
                ...
                for row in data_report:
                    ...
                    if i==60000:
                        i=4
                        sheet_no+=1
                        xls_sheet=xls_book.add_sheet(str(sheet_name)+str(sheet_no), cell_overwrite_ok=True)
                        xls_sheet.row(0).height = 521
                        xls_sheet.row(1).height = 421
    ...
    def pagination_create_table(self,query_dict,i_display_start,i_display_length,s_search,sEcho):
        ...
        try:
            ...
            cursor = db.cursor()
            sql_query1 = "SELECT SQL_CALC_FOUND_ROWS %s FROM %s %s %s %s %s" % (" , ".join(a_columns),s_table,s_join,s_where,s_group_by,s_order)#,s_limit)
            ...
            # query to get data according to searching condition given by user but with limit like 0,10 or 10,10
            cursor = db.cursor()
...
#		return {"succcess" : 1 , "result":str(e) }
    ...
    def trap_main_reporting_get_excel(self,date1,date2,time1,time2,all_host,report_type,device_type,all_group,view_type,i_display_start,
                                      i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,new_user_report_dict,username,hostgroup_id_list):
        try:
            if device_type!="":
                db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
                cursor=db.cursor()
                sql="SELECT `device_name` FROM `device_type` WHERE `device_type_id`='%s' "%(device_type)
                cursor.execute(sql)
                result=cursor.fetchall()
            ...
            re=MainReportBll()      
            #report_obj=UbrReportBll()
            trap_data=trap_get_total_data_trap(report_type,3000,date1,date2,time1,time2,all_group,all_host,hostgroup_id_list,device_type)
            ...
            if report_type!="all":    
                ...
                if view_type=="excel":
                    ...
                    sheet_dict_history={
                        ...
                        "time2":time2,
                        "device_type":device_type}    
                    status=re.get_excel_sheet(sheet_dict_current,sheet_dict_clear,sheet_dict_history)
                    if str(status["success"])=="0":
                        el = EventLog()
                        desc=self.get_description(all_host,"Event report")
                        if str(desc["success"])=="0":
                    ...
                    sheet_dict_history={
                        ...
                        "time2":time2,
                        "device_type":device_type}    
                    status=re.get_csv_file(sheet_dict_current,sheet_dict_clear,sheet_dict_history)
                    if str(status["success"])=="0":
                        el = EventLog()
                        #desc="Excel Report Generated for Hosts  "
                        desc=self.get_description(all_host,"Event report")
    ...
    try:
        ...
        for tpl in result_tuple:
            ...
            if is_date:
                ...
                main_list.append([main_date,tpl_temp[4],tpl_temp[5],tpl_temp[6],uptime,downtime])
                uptime = None
                downtime = None
...
#                    day_diff=(temp_date-main_date).days
                ...
                if day_diff > 1:
                    for i in range(1,day_diff):
                        ...
                        if prev_value == '50002':
                            main_list.append([leftout_date,tpl_temp[4],tpl_temp[5],tpl_temp[6],timedelta(0, 86399),None])
                        elif prev_value == '50001':
                            main_list.append([leftout_date,tpl_temp[4],tpl_temp[5],tpl_temp[6],None,timedelta(0, 86399)])
		...
		INNER JOIN (select hostgroup_id , hostgroup_name from hostgroups) as  hostgroups ON hostgroups.hostgroup_id = hosts_hostgroups.hostgroup_id \
		 WHERE timestamp<='%s' and host.host_id='%s' order by timestamp desc limit 1"%(start_date,j)
            ...
            if status_result!=() and result!=():
                #t_date=result[0][2]
                t_date=datetime.strptime(start_date,"%Y-%m-%d %H:%M:%S")#status_result[0][2]
            ...
            if str(temp_res['success'])=="0":
                ...
                for i in li_res:
                    ...
                    if total!=0:
                        i[0]=str(i[0])[:11]
                        i[-2]= int(round(float(uptime)*100/float(total)))  # (uptime*100)/total # int(round(float(a)*100/float(a+b)))
...
def trap_get_total_data_trap(report_type,no_of_devices,date1,date2,time1,time2,all_group,all_host,hostgroup_id_list,device_type):
    try:
        ...
        result_dict["result"]=tr
        host_data=str(all_host.split(',')).replace('[','(').replace(']',')')
        table_dict={"clear":"trap_alarm_clear","current":"trap_alarm_current","history":"trap_alarms"}
        if device_type!="":
