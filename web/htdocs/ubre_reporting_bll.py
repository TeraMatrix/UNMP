...
# %H:%M')))
                ...
                for k in range(0, len(time_stamp_signal1)):
                    average_list.append(
                        [time_stamp_signal1[k], host_name[k], ip_address[k], signal_interface1[k], signal_interface2[k], signal_interface3[k], signal_interface4[k], signal_interface5[k], signal_interface6[k], signal_interface7[k], signal_interface8[k], signal_interface9[k], signal_interface10[k],
                         signal_interface11[k], signal_interface12[k], signal_interface13[k], signal_interface14[k], signal_interface15[k], signal_interface16[k], grp_name[k]])
            else:
                host_data = str(
                    all_host.split(',')).replace('[', '(').replace(']', ')')
                while(d1 <= d2):
...
# horiz center,color grey25')
                    heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
                    headings = [
                        'Data', 'Hostname', 'IP Address', 'Peer1', 'Peer2', 'Peer3', 'Peer4', 'Peer5', 'Peer6', 'Peer7', 'Peer8', 'Peer9',
