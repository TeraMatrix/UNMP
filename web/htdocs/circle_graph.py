	...
	try:
		...
		if len(result)>0:
			...
			for row in result:
				...
				sevices_status+="<table  style=\"width:300px;font-size:11px; margin-top:0px;\"><colgroup><col width=\"25%%\"/><col width=\"25%%\"/><col width=\"25%%\"/><col width=\"25%%\"/></colgroup>"
				...
				some_link+="<table style=\"width:300px;font-size:11px; margin-top:0px;\"><colgroup><col width=\"33%%\"/><col width=\"33%%\"/><col/></colgroup>"
				#some_link+="<tr><td ><lable style=\" width:100px;\"><a href=http://172.22.0.95/nms2/check_mk/odu_profiling.py?host_id=%s&device_type=%s&device_list_state=enabled alt=\"Configuration\">Configuration</td>"%(' ' if row[1]=="" or row[5]==None else row[5],' ' if row[6]=="" or row[6]==None else row[6])
				if redirect_page=='localhost_dashboard':
				    pass
				else:
				    some_link+="<td style=\"width:30px\"><lable style=\" width:100px;\"><a href=status_snmptt.py?ip_address=%s- alt=\"Alarms\">Alarms</td>"%('' if row[1]=="" or row[1]==None else row[1])
				some_link+="<tr><td style=\"width:30px\"><lable style=\" width:100px;\"><a href=%s.py?host_id=%s&device_type=%s&device_list_state=enabled alt=\"Dashboard\">Dashboard</td></tr><table>"%(redirect_page,' ' if row[5]=="" or row[5]==None else row[5],' ' if row[6]=="" or row[6]==None else row[6])
