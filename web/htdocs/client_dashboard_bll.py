...
class ClientDashboardBll(object):
    ...
    def client_device_information(self,mac_address):
        ...
        try:
            ...
            if db ==1 or db =='1' :
                raise SelfException(cursor)
            sql="SELECT hosts.host_alias,client_name,UPPER(mac),rssi,vap_id,ap.total_tx,ap.total_rx FROM ap_client_details as ap inner join hosts on\
                 hosts.host_id=ap.last_seen_ap_id inner join ap_client_ap_data as ap_data on ap.client_id=ap_data.client_id where ap.mac='%s' and hosts.device_type_id='ap25' and hosts.is_deleted=0"%(mac_address)
            cursor.execute(sql)
            result=cursor.fetchall()
...
#		graph_json.append(graph_dict)
    ...
    def client_graph_json(self,display_type,user_id,table_name,x_axis_value,index_name,graph_id,flag,start_date,end_date,start,limit,mac_address,graph,update_field_name,interface=1,value_cal=1,*column_name):
		...
		default_value='DU'
            ...
            if str(update_field_name).strip()=='calType':
                up_query="update  graph_templet_table set graph_cal_id =%s where user_id='%s' and graph_display_id='%s' and dashboard_type=2"%(value_cal,user_id,graph_id)
		...
		up_query="update graph_field_table set is_checked=1 where user_id='%s' and graph_name='%s' and graph_field_value IN ('%s')"%(user_id,graph_id,columns)
            ...
            sel_query="select graph_title,graph_subtitle from graph_templet_table where graph_display_id='%s' and user_id='%s' and dashboard_type=2"%(graph_id,user_id)
            cursor.execute(sel_query)
            ...
            if graph_id.strip()=='apClientRssi123': # this is not using
                odu100_rssi_result=self.sp_signal_strength_graph(display_type,graph_id,mac_address,start_date,end_date,table_name,column_name[0])
		    ...
		    if  len(column_name[0][0])>0:
		        ...
		        sel_query+=column_list+","+"gp_tab.%s from %s as gp_tab where UPPER(gp_tab.addressMAC)='%s' AND gp_tab.timestamp >= '%s' AND gp_tab.timestamp <='%s' order by gp_tab.timestamp desc"%(x_axis_value,table_name,mac_address,start_date,end_date)
		    else:
		        sel_query+="gp_tab.%s from %s as gp_tab where UPPER(gp_tab.addressMAC)='%s' AND gp_tab.timestamp >= '%s' AND gp_tab.timestamp <='%s' order by gp_tab.timestamp desc"%(x_axis_value,table_name,mac_address,start_date,end_date)
                else:
		    sel_query="Select "
		    if  len(column_name[0][0])>0:
		        column_list=',gp_tab.'.join(column_name[0])
                ...
                if calculation[int(value_cal)-1]=='normal':
                    for row in result:
                        for i  in range(len(row)-1):
                            data_list[i].append(0 if row[i]==None or row[i]==None else int(str(row[i][:-1]) if 'M' in str(row[i]) else row[i]))
    ...
    def update_show_graph_table(self,device_type_id,user_id,show_graph):
        try:
            ...
            if show_graph.strip()!='':
                sql="UPDATE graph_templet_table SET is_disabled=1 WHERE user_id='%s' and device_type_id='%s' and dashboard_type=2"%(user_id,device_type_id)
                cursor.execute(sql)
                db.commit()
                sql1="UPDATE graph_templet_table SET is_disabled=0 WHERE user_id='%s' AND device_type_id='%s' AND graph_display_id IN ('%s') and dashboard_type=2"%(user_id,device_type_id,"','".join(graph_list))
                cursor.execute(sql1)
                db.commit()
    ...
    def client_dashboard_get_graph_name(self,user_id,device_type):
        try:
            ...
            cursor=db.cursor()
            sql=" select graph_display_name,is_disabled,graph_display_id from graph_templet_table where device_type_id='%s' and user_id='%s' and dashboard_type=2"%(device_type,user_id)
            cursor.execute(sql)
            result=cursor.fetchall()
            selected_graph=[]
            non_selected_graph=[]
    ...
    def client_total_display_graph(self,device_type_id,user_id):
        try:
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            sql=" select graph_display_id from graph_templet_table where device_type_id='%s' and user_id='%s' and is_disabled=0 and dashboard_type=2"%(device_type_id,user_id)
            cursor.execute(sql)
            result=cursor.fetchall()
	    count=0
            if len(result)>0:
...
#        check_var=''
        try:
            device_name={'ap25':'AP25','odu16':'RM18','odu100':'RM','idu4':'IDU'}
            table_output=[]
		...
		result=self.client_device_information(mac_address)
		    ...
		    device_detail=['AP alias','Client Alias','MAC Address','RSL','VAP','Total Tx','Total Rx']
                    for i in result:
                        table_output.append([device_detail[0],'--' if result[0][0]==None or result[0][0]=='' else result[0][0]])
                        table_output.append([device_detail[1],'--' if result[0][1]==None or result[0][1]=='' else result[0][1]])
                        ...
                        table_output.append([device_detail[6],'--' if result[0][6]==None or result[0][6]=='' else result[0][6]])
            
            # create the excel sheet for devcice information .
            xls_sheet=xls_book.add_sheet('Client_information',cell_overwrite_ok=True)
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
            xls_sheet.write_merge(0,0,0,2,"AP25 Client Information (%s--%s)"%(str(start_date),str(end_date)),style)
            ...
            for i in range(len(tab_list)):
                ...
                limit=0
                output_result=self.client_graph_json('excel',user_id,table_name,x_axis,index_name,graph_id,limitFlag,start_date,end_date,start,limit,mac_address,graph_list[i],update_field_name,tab_list[i],cal_list[i],field_column)
                d1_list=[]
		...
		if len(headings)<2:
		    continue
                ...
                xls_sheet.write_merge(1,1,0,len(headings)-1,device_name[device_type]+'   '+str(mac_address)+'  Display Information -: (%s)'%str(cal_type) ,style)
            ...
            if device_type=='ap25':
                ...
                headings = ['Client Alias','MAC Address','Total Tx','Total Rx','First Seen Time','First Seen AP','Last Seen Time','Last Seen AP','Current State']
                xls_sheet=xls_book.add_sheet('Client Details',cell_overwrite_ok=True)
                xls_sheet.row(0).height = 521
                xls_sheet.row(1).height = 421
                xls_sheet.write_merge(0,0,0,len(headings)-1,'Client State Information (%s--%s)'%(start_date,end_date),style)
                xls_sheet.write_merge(1,1,0,len(headings)-1,device_name[device_type],style)
                xls_sheet.write_merge(2,2,0,len(headings)-1,"")
                i=4
                heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
                for colx, value in enumerate(headings):
...
#            raise SelfException('This devices not exists so excel report can not be generated.')  # Check msg 
		...
		result=self.client_device_information(mac_address)
		    ...
		    device_detail=['AP alias','Client Alias','MAC Address','RSL','VAP','Total Tx','Total Rx']
                    for i in result:
                        ...
                        table_output.append([device_detail[4],'--' if result[0][4]==None or result[0][4]=='' else result[0][4]])
                        table_output.append([device_detail[5],'--' if result[0][5]==None or result[0][5]=='' else result[0][5]])
                        table_output.append([device_detail[6],'--' if result[0][6]==None or result[0][5]=='' else result[0][6]])
            ...
            for i in range(len(tab_list)):
                ...
                start=0 # default value for start and end here .
                limit=0
                output_result=self.client_graph_json(display_type,user_id,table_name,x_axis,index_name,graph_id,limitFlag,start_date,end_date,start,limit,mac_address,graph_list[i],update_field_name,tab_list[i],cal_list[i],field_column)
                d1_list=[]
                first_column=x_axis.capitalize()
                headings = [first_column]
                if     int(output_result['success'])==0:
        ...
        # if no column select so report not created.
                ...
                second_row=[device_name[device_type]+' Client State Information -:'+str(cal_type)]
                blank_row=["","",""]
