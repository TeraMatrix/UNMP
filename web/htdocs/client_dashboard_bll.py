                    ...
                    for row in result:
                        for i  in range(len(row)-1):
                            if graph_id=="apClientRssi":
                            	data_list[i].append(0 if row[i]==None or row[i]==None else (int(str(row[i][:-1])-95) if 'M' in str(row[i]) else row[i]))
                            else:
...
#        check_var=''
		...
		result=self.client_device_information(mac_address)
		    ...
		    device_detail=['AP alias','Client Alias','MAC Address','RSSI','VAP','Total Tx(Mbps)','Total Rx(Mbps)']
                    for i in result:
		...
		if len(headings)<2:
            ...
            if device_type=='ap25':
                ...
                headings = ['Client Alias','MAC Address','Total Tx(Mbps)','Total Rx(Mbps)','First Seen Time','First Seen AP','Last Seen Time','Last Seen AP','Current State']
