...
def ap_listing(h):
    try:
        ...
        javascript_list = ["js/jquery.dataTables.min.js",'js/ccpl_jquery_autocomplete.js',"js/ap_listing.js"]
        ...
        html.new_header("AP Listing","ap_listing.py","",css_list,javascript_list)
        ...
        html.write(str(page_header_search(ip_address,mac_address,"UBR,UBRe,Access Point,IDU4Port",selected_device_type,"enabled","device_type")))
...
def ap_profiling(h):
    ...
    css_list = ['css/demo_table_jui.css','css/jquery-ui-1.8.4.custom.css','css/ccpl_jquery_combobox.css']
    jss_list = ['js/jquery.dataTables.min.js','js/ccpl_jquery_autocomplete.js','js/ap_controller.js','js/jquery-ui-personalized-1.6rc2.min.js']
    snapin_list = ["reports","views","Alarm","Inventory","Settings","NetworkMaps","user_management","schedule","Listing"]
    ...
    device_list_parameter = device_parameter_bll_obj.get_device_parameter(host_id)# call the idu_profiling_controller function get_device_param returns the list.List Contains ipaddress,macaddress,devicetypeid,config_profile_id
    if isinstance(device_list_parameter,list):
        html.new_header("%s %s Configuration"%("AP",device_list_parameter[0].ip_address),"ap_listing.py","",css_list,jss_list,snapin_list)
        if device_list_parameter == [] or device_list_parameter == None:
            html.write(page_header_search("","","RM18,RM,Access Point,IDU",None,"enabled","device_type"))#call the function of common_controller , it is used for listing the Devices based on IPaddress,Macaddress,DeviceTy
        else:
            html.write(page_header_search(device_list_parameter[0][0],device_list_parameter[0][1],"RM18,RM,Access Point,IDU",device_type,device_list_state,"device_type"))
            html.write(APProfiling.ap_div(device_list_parameter[0][0],device_list_parameter[0][1],host_id))
...
def ap_device_listing_table(h):
    ...
    try:
        ...
        ###########################
        ap_mode = {0:'Standard',1:'Root AP',2:'Repeater',3:'Client',4:'Multi AP',5:'Multi VLAN',6:'Dynamic VLAN'}
        ...
        device_dict = ap_device_list_bll_obj.ap_device_list(ip_address,mac_address,selected_device,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,userid,html_req)
        ...
        if isinstance(device_list,list):
            for i in range(0,len(device_list)):
                ...
                snmp_up_time_data = obj_device_status.common_list_device_status(device_list[i][0])
                device_status = snmp_up_time_data['result']['device_status'][0]
                device_status_image_path = snmp_up_time_data['result']['device_status'][1]
                
                ap_admin_state_data = obj_get_data.common_get_data("Ap25RadioSetup",device_list[i][0])
                ...
                if i==len(device_list)-1:
                    ...
                    device_status_host_id+=str(device_list[i][0])+","
                monitoring_status = "<img src=\"images/new/info1.png\" style=\"width:16px;height:16px;\" title=\"Current Device Status\" class=\"imgbutton n-reconcile\"/>" if device_list[i][5]=="ap25" else "<img src=\"images/new/info1.png\" style=\"width:16px;height:16px;\" title=\"Current Device Status\" class=\"imgbutton n-reconcile\"/>"
                if html.req.session["role"] == "admin" or html.req.session["role"] == "user":
                    result_device_list.append(["<center><img id=\"ap_device_status\" name=\"ap_device_status\" src=\"%s\" title=\"%s\" style=\"width:8px;height:8px;\" class=\"imgbutton n-reconcile imgEditodu16\" /></center>"\
                    ...
                    "<div class=\"listing-icon\"><img src=\"images/new/wifi.png\" title=\"Wireless\" style=\"width:16px;height:16px;\" class=\"imgbutton n-reconcile imgEditodu16\" onclick=\"wirelessStatus(event,this,'%s','%s');\"/></div>&nbsp;\
                    <a target=\"main\" href=\"ap_profiling.py?host_id=%s&device_type=%s&device_list_state=%s\"><img id=\"%s\" src=\"images/new/edit.png\" title=\"Edit Profile\" style=\"width:16px;height:16px;\" class=\"imgbutton imgEditodu16 n-reconcile\"/></a>&nbsp;&nbsp;\
                    <a target=\"main\" href=\"%s?host_id=%s&device_type=%s&device_list_state=%s\"><img src=\"images/new/graph.png\" style=\"width:16px;height:16px;\" title=\"Performance Monitoring\" class=\"imgbutton imgEditodu16 n-reconcile\"/></a>&nbsp;&nbsp;\
                    <a target=\"main\" href=\"status_snmptt.py?ip_address=%s-\"><img src=\"images/new/alert.png\" style=\"width:16px;height:16px;\" title=\"Device Alarms\" class=\"imgbutton imgEditodu16 n-reconcile\"/></a>&nbsp;&nbsp;\
                    ...
                    device_list[i][3],\
                    device_list[i][0],device_list[i][5],device_list_state,\
                    images,device_list[i][6],device_list[i][0],device_list[i][5],monitoring_status,\
                    "<input type=\"hidden\" value=\"%s\" name=\"host_id\" id=\"host_id\" />"%(device_status_host_id) if i==len(device_list)-1 else "")\
                    ])
                else:
                    result_device_list.append(["<center><img id=\"ap_device_status\" name=\"ap_device_status\" src=\"%s\" title=\"%s\" style=\"width:8px;height:8px;\" class=\"imgbutton n-reconcile\" /></center>"\
                    %(device_status_image_path,device_status)\
                    ,device_list[i][1],device_list[i][2],device_list[i][3],device_list[i][4],ap_mode[int(device_list[i][8])],\
                    "<ul class=\"button_group\" style=\"width:30px\"><li><a class=\"%s n-reconcile imgEditodu16\" state=\"%s\" title=\"%s\" onclick=\"radio_enable_disable(event,this,'%s','radioSetup.radioState');\"/>AP</a></li></ul>"
                    %(image_class,state,title,device_list[i][0]),\
                    ...
                    <a target=\"main\" ><img id=\"%s\" src=\"images/new/edit.png\" title=\"Edit Profile\" style=\"width:16px;height:16px;\" class=\"imgbutton imgEditodu16 n-reconcile\"/></a>&nbsp;&nbsp;\
                    <a target=\"main\" href=\"%s?host_id=%s&device_type=%s&device_list_state=%s\"><img src=\"images/new/graph.png\" style=\"width:16px;height:16px;\" title=\"Performance Monitoring\" class=\"imgbutton imgEditodu16 n-reconcile\"/></a>&nbsp;&nbsp;\
                    <a target=\"main\" href=\"status_snmptt.py?ip_address=%s-\"><img src=\"images/new/alert.png\" style=\"width:16px;height:16px;\" title=\"Device Alarms\" class=\"imgbutton imgEditodu16 n-reconcile\"/></a>&nbsp;&nbsp;\
...
def get_device_list_ap_profiling(h): 
    try:
        ...
        device_list = ap_device_list_bll_obj.ap_device_list_profiling(ip_address,mac_address,selected_device)
        ...
        if result==0 or result==1 or result==2:
            ...
            device_parameter_bll_obj = DeviceParameters()
            device_list_parameter = device_parameter_bll_obj.get_device_parameter(result)# call the ap_profiling_controller function get_device_param returns the list.List Contains ipaddress,macaddress,devicetypeid,config_profile_id
            #html.write(str(device_list_parameter))
            html.write(APProfiling.ap_profile_call(result,selected_device,device_list_parameter))
...
def update_reconciliation(h):
    ...
    obj = Reconciliation()
    result = obj.update_configuration(host_id,device_type_id)
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(result)))
...
def ap25_reconcilation(h):
    ...
    obj_reconcilation = Reconciliation()
    result = obj_reconcilation.update_configuration(host_id,device_type)
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(result))) 
...
def ap_scan(h):
    ...
    host_id = html.var('host_id')
    obj_ap_scan = APScan()
    result = obj_ap_scan.ap_scan(host_id)
    ap_scan_table = ""
    ...
    if result["success"] == 0:        
        ...
        ap_scan_table+="<tr><td colspan=\"8\">%s</td></tr>"%(result["result"])
    ap_scan_table+="</table>"
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("radioSetup.radioState")!=None:
            if Validation.is_required(html.var("radioSetup.radioState")):
                dic_result["radioSetup.radioState"]=html.var("radioSetup.radioState")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("radioSetup.radioAPmode")!=None:
            if Validation.is_required(html.var("radioSetup.radioAPmode")):
                dic_result["radioSetup.radioAPmode"]=html.var("radioSetup.radioAPmode")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("radioSetup.radioManagementVLANstate")!=None:
            if Validation.is_required(html.var("radioSetup.radioManagementVLANstate")):
                dic_result["radioSetup.radioManagementVLANstate"]=html.var("radioSetup.radioManagementVLANstate")
            else:
                flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Radio State is required"
        ...
        if html.var("radioSetup.radioCountryCode")!=None:
            if Validation.is_required(html.var("radioSetup.radioCountryCode")):
                dic_result["radioSetup.radioCountryCode"]=html.var("radioSetup.radioCountryCode")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("radioSetup.numberOfVAPs")!=None:
            if Validation.is_required(html.var("radioSetup.numberOfVAPs")):
                dic_result["radioSetup.numberOfVAPs"]=html.var("radioSetup.numberOfVAPs")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("radioSetup.radioChannel")!=None:
            if Validation.is_required(html.var("radioSetup.radioChannel")):
                dic_result["radioSetup.radioChannel"]=html.var("radioSetup.radioChannel")
            else:
                flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Radio channel is required"
        ...
        if html.var("radioSetup.radioTxPower")!=None:
            if Validation.is_required(html.var("radioSetup.radioTxPower")):
                dic_result["radioSetup.radioTxPower"]=html.var("radioSetup.radioTxPower")
            else:
                flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Radio Wifi Mode is required"
        ...
        if html.var("radioSetup.radioGatingIndex")!=None:
            if Validation.is_required(html.var("radioSetup.radioGatingIndex")):
                dic_result["radioSetup.radioGatingIndex"]=html.var("radioSetup.radioGatingIndex")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("radioSetup.radioAggregation")!=None:
            if Validation.is_required(html.var("radioSetup.radioAggregation")):
                dic_result["radioSetup.radioAggregation"]=html.var("radioSetup.radioAggregation")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("radioSetup.radioAggFrames")!=None:
            if Validation.is_required(html.var("radioSetup.radioAggFrames")):
                if Validation.is_number(html.var("radioSetup.radioAggFrames")):
                    dic_result["radioSetup.radioAggFrames"]=html.var("radioSetup.radioAggFrames")
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if html.var("radioSetup.radioAggSize")!=None:
            if Validation.is_required(html.var("radioSetup.radioAggSize")):
                if Validation.is_number(html.var("radioSetup.radioAggSize")):
                    dic_result["radioSetup.radioAggSize"]=html.var("radioSetup.radioAggSize")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "The value is not a number.Please Enter a Number in Aggsize"
            else:
                flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Aggregation Size is required"
        ...
        if html.var("radioSetup.radioAggMinSize")!=None:
            if Validation.is_required(html.var("radioSetup.radioAggMinSize")):
                if Validation.is_number(html.var("radioSetup.radioAggMinSize")):
                    dic_result["radioSetup.radioAggMinSize"]=html.var("radioSetup.radioAggMinSize")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "The value is not a number.Please Enter a Number in minsize"
            else:
                flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Aggregation Minimum Size is required"
                
        if html.var("radioSetup.radioChannelWidth")!=None:
            if Validation.is_required(html.var("radioSetup.radioChannelWidth")):
                dic_result["radioSetup.radioChannelWidth"]=html.var("radioSetup.radioChannelWidth")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("radioSetup.radioTXChainMask")!=None:
            if Validation.is_required(html.var("radioSetup.radioTXChainMask")):
                dic_result["radioSetup.radioTXChainMask"]=html.var("radioSetup.radioTXChainMask")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("radioSetup.radioRXChainMask")!=None:
            if Validation.is_required(html.var("radioSetup.radioRXChainMask")):
                dic_result["radioSetup.radioRXChainMask"]=html.var("radioSetup.radioRXChainMask")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if flag == 1:
            ...
            result = obj_bll_set_valid.common_validation(host_id,device_type_id,dic_result)
            html.req.content_type = 'application/json'
            html.req.write(str(JSONEncoder().encode(result)))
        ...
        result = obj_bll_set_valid.ap_cancel_form(host_id,device_type_id,dic_result)
        html.req.content_type = 'application/json'
        html.req.write(str(JSONEncoder().encode(result)))
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("services.upnpServerStatus")!=None:
            if Validation.is_required(html.var("services.upnpServerStatus")):
                dic_result["services.upnpServerStatus"]=html.var("services.upnpServerStatus")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("services.systemLogStatus")!=None:
            if Validation.is_required(html.var("services.systemLogStatus")):
                dic_result["services.systemLogStatus"]=html.var("services.systemLogStatus")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("services.systemLogIP")!=None:
            if Validation.is_required(html.var("services.systemLogIP")):
                if Validation.is_valid_ip(html.var("services.systemLogIP")):
                    dic_result["services.systemLogIP"]=html.var("services.systemLogIP")
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if html.var("services.systemLogPort")!=None:
            if Validation.is_required(html.var("services.systemLogPort")):
                if Validation.is_number(html.var("services.systemLogPort")):
                    dic_result["services.systemLogPort"]=html.var("services.systemLogPort")
                else:
                    flag = 1
                    dic_result["success"] = 1
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("vapSelection.selectVap")!=None :
            if Validation.is_required(html.var("vapSelection.selectVap")):
                dic_result["vapSelection.selectVap"] = html.var("vapSelection.selectVap")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("vapWPAsecuritySetup.aclState")!=None:
            dic_result["vapWPAsecuritySetup.aclState"]=html.var("vapWPAsecuritySetup.aclState")
        else:
            flag = 1
            dic_result["success"] = 1
            dic_result["result"] = "Please Select a Acl State"
            
        if html.var("vapWPAsecuritySetup.aclMode")!=None:
            dic_result["vapWPAsecuritySetup.aclMode"]=html.var("vapWPAsecuritySetup.aclMode")
        ...
        if flag == 1:
            ...
            result = obj_bll_set_valid.basic_acl_set(host_id,device_type_id,dic_result)
            html.req.content_type = 'application/json'
            html.req.write(str(JSONEncoder().encode(result)))
...
def acl_add_form_action(h):
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if vap_selection_id == None:
            flag = 1
            dic_result["success"]=1
            dic_result["result"] = "Unmp has Encountered an Error.Please Reconcile the device"
        ...
        if html.var("mac_add")!=None:
                    ...
                    if len(mac_add)==0:
                        ...
                        if len(final_mac_list)>0:
                            ...
                            if valid == 1:
                                flag = 1
                                dic_result["success"] = 1
                                dic_result["result"] = "Please Enter the valid mac addresses"
                            elif duplicate == 1:
                                flag = 1
                                dic_result["success"] = 1
                                dic_result["result"] = "Mac Addresses Already Exists"
                            else:
                                flag = 1
                                dic_result["success"] = 1
                                dic_result["result"] = "You entered the duplicate mac"
                else:
                    flag = 1
                    dic_result["success"] = 1
            ...
            flag = 1
            dic_result["success"] = 1
            dic_result["result"] = "Please Choose Option of single and multiple added"
        ...
        if flag == 1:
            ...
            result = obj_bll_mac_operations.add_acl(host_id,device_type_id,dic_result,vap_selection_id,selected_vap)
            html.req.content_type = 'application/json'
            html.req.write(str(JSONEncoder().encode(result)))
            #html.req.write(str(JSONEncoder().encode(dic_result)))
...
def delete_all_mac(h):
    ...
    if flag == 1:
        ...
        result = obj_bll_mac_operations.delete_all_mac(host_id,device_type_id,selected_vap,vap_selection_id)
        html.req.content_type = 'application/json'
        html.req.write(str(JSONEncoder().encode(result)))
...
def delete_single_mac(h):
    ...
    if flag == 1:
        ...
        result = obj_bll_mac_operations.delete_acl(host_id,device_type_id,dic_result,selected_vap,vap_selection_id)
        html.req.content_type = 'application/json'
        html.req.write(str(JSONEncoder().encode(result)))
...
def ap_vap_form_action(h):
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("basicVAPsetup.vapESSID")!=None:
            if Validation.is_required(html.var("basicVAPsetup.vapESSID")):
                dic_result["basicVAPsetup.vapESSID"]=html.var("basicVAPsetup.vapESSID")
            else:
                flag = 1
                dic_result["success"]=1
                dic_result["result"]="Essid is required"
                
        if html.var("basicVAPsetup.vapHiddenESSIDstate")!=None:
            dic_result["basicVAPsetup.vapHiddenESSIDstate"] = html.var("basicVAPsetup.vapHiddenESSIDstate")
            
        if html.var("rts_mode")!=None:
            if Validation.is_required(html.var("rts_mode")):
                if int(html.var("rts_mode"))==1:
                    if html.var("basicVAPsetup.vapRTSthresholdValue")!=None:
                        if Validation.is_required(html.var("basicVAPsetup.vapRTSthresholdValue")):
                            if Validation.is_number(html.var("basicVAPsetup.vapRTSthresholdValue")):
                                dic_result["basicVAPsetup.vapRTSthresholdValue"]=html.var("basicVAPsetup.vapRTSthresholdValue")
                            else:
                                flag = 1
                                dic_result["success"]=1
                                dic_result["result"]="RTS Threshold must be number"
                        else:
                            flag = 1
                            dic_result["success"]=1
        ...
        if html.var("frag_mode")!=None:
            if Validation.is_required(html.var("frag_mode")):
                if int(html.var("frag_mode"))==1:
                    if html.var("basicVAPsetup.vapFragmentationThresholdValue")!=None:
                        if Validation.is_required(html.var("basicVAPsetup.vapFragmentationThresholdValue")):
                            if Validation.is_number(html.var("basicVAPsetup.vapFragmentationThresholdValue")):
                                dic_result["basicVAPsetup.vapFragmentationThresholdValue"]=html.var("basicVAPsetup.vapFragmentationThresholdValue")
                            else:
                                flag = 1
                                dic_result["success"]=1
                                dic_result["result"]="Fragmentation threshold must be number"
                        else:
                            flag = 1
                            dic_result["success"]=1
                            dic_result["result"]="Fragmentation Threshold is required"
            else:
                flag = 1
                dic_result["success"]=1
                dic_result["result"]="Please Select The Fragmentation Mode"
        if html.var("basicVAPsetup.vapBeaconInterval")!=None:
            if Validation.is_required(html.var("basicVAPsetup.vapBeaconInterval")):
                if Validation.is_number(html.var("basicVAPsetup.vapBeaconInterval")):
                    if int(html.var("basicVAPsetup.vapBeaconInterval"))>=100 and int(html.var("basicVAPsetup.vapBeaconInterval"))<=1000:
                        dic_result["basicVAPsetup.vapBeaconInterval"]=html.var("basicVAPsetup.vapBeaconInterval")
                    else:
                        flag = 1
                        dic_result["success"]=1
                        dic_result["result"]="Beacon Interval must be in Between 100 and 1000"
                else:
                    flag = 1
                    dic_result["success"]=1
        ...
        if html.var("basicVAPsecurity.vapSecurityMode")!=None:
            if int(html.var("basicVAPsecurity.vapSecurityMode"))==0:
                dic_result["basicVAPsecurity.vapSecurityMode"] = html.var("basicVAPsecurity.vapSecurityMode")
            elif int(html.var("basicVAPsecurity.vapSecurityMode"))==1:
                dic_result["basicVAPsecurity.vapSecurityMode"] = html.var("basicVAPsecurity.vapSecurityMode")
                if html.var("vapWEPsecuritySetup.vapWEPmode")!=None:
                    dic_result["vapWEPsecuritySetup.vapWEPmode"] = html.var("vapWEPsecuritySetup.vapWEPmode")
                
                if html.var("vapWEPsecuritySetup.vapWEPprimaryKey")!=None:
                    if int(html.var("vapWEPsecuritySetup.vapWEPprimaryKey"))==1:
                        if html.var("vapWEPsecuritySetup.vapWEPkey1")!=None:
                            if Validation.is_required(html.var("vapWEPsecuritySetup.vapWEPkey1")):
                                if (len(html.var("vapWEPsecuritySetup.vapWEPkey1"))==5 or len(html.var("vapWEPsecuritySetup.vapWEPkey1"))==13 or len(html.var("vapWEPsecuritySetup.vapWEPkey1"))==16) :
                                    dic_result["vapWEPsecuritySetup.vapWEPkey1"] = html.var("vapWEPsecuritySetup.vapWEPkey1")
                                else:
                                    flag = 1
                                    dic_result["success"]=1
                                    dic_result["result"] = "WEP key1 must be the length of 5,13 or 16"
                            else:
                                flag = 1
                                dic_result["success"] = 1
                                dic_result["result"]="WEP Key1 is required"
                        
                    elif int(html.var("vapWEPsecuritySetup.vapWEPprimaryKey"))==2:
                        if html.var("vapWEPsecuritySetup.vapWEPkey2")!=None:
                            if Validation.is_required(html.var("vapWEPsecuritySetup.vapWEPkey2")):                                
                                if len(html.var("vapWEPsecuritySetup.vapWEPkey2"))==5 or len(html.var("vapWEPsecuritySetup.vapWEPkey2"))==13 or len(html.var("vapWEPsecuritySetup.vapWEPkey2"))==16:
                                    dic_result["vapWEPsecuritySetup.vapWEPkey2"] = html.var("vapWEPsecuritySetup.vapWEPkey2")
                                else:
                                    flag = 1
                                    dic_result["success"]=1
                                    dic_result["result"] = "WEP key2 must be the length of 5,13 or 16"
                            else:
                                flag = 1
                                dic_result["success"] = 1
                                dic_result["result"]="WEP Key2 is required"
                        
                    elif int(html.var("vapWEPsecuritySetup.vapWEPprimaryKey"))==3:
                        if html.var("vapWEPsecuritySetup.vapWEPkey3")!=None:
                            if Validation.is_required(html.var("vapWEPsecuritySetup.vapWEPkey3")):
                                if len(html.var("vapWEPsecuritySetup.vapWEPkey3"))==5 or len(html.var("vapWEPsecuritySetup.vapWEPkey3"))==13 or len(html.var("vapWEPsecuritySetup.vapWEPkey3"))==16:
                                    dic_result["vapWEPsecuritySetup.vapWEPkey3"] = html.var("vapWEPsecuritySetup.vapWEPkey3")
                                else:
                                    flag = 1
                                    dic_result["success"]=1
                                    dic_result["result"] = "WEP key3 must be the length of 5,13 or 16"
                            else:
                                flag = 1
                                dic_result["success"] = 1
                        ...
                        if html.var("vapWEPsecuritySetup.vapWEPkey4")!=None:
                            if Validation.is_required(html.var("vapWEPsecuritySetup.vapWEPkey4")):
                                if len(html.var("vapWEPsecuritySetup.vapWEPkey4"))==5 or len(html.var("vapWEPsecuritySetup.vapWEPkey4"))==13 or len(html.var("vapWEPsecuritySetup.vapWEPkey4"))==16:
                                    dic_result["vapWEPsecuritySetup.vapWEPkey4"] = html.var("vapWEPsecuritySetup.vapWEPkey4")
                                else:
                                    flag = 1
                                    dic_result["success"]=1
                                    dic_result["result"] = "WEP key4 must be the length of 5,13 or 16"
                            else:
                                flag = 1
                                dic_result["success"] = 1
                    ...
                    if len(html.var("vapWEPsecuritySetup.vapWEPkey1"))==0:
                        dic_result["vapWEPsecuritySetup.vapWEPkey1"]= ""
                    else:
                        if (len(html.var("vapWEPsecuritySetup.vapWEPkey1"))==5 or len(html.var("vapWEPsecuritySetup.vapWEPkey1"))==13 or len(html.var("vapWEPsecuritySetup.vapWEPkey1"))==16):
                            dic_result["vapWEPsecuritySetup.vapWEPkey1"]= html.var("vapWEPsecuritySetup.vapWEPkey1")
                        else:
                            ...
                            dic_result["success"]=1
                            dic_result["result"] = "WEP key1 must be the length of 5,13 or 16"
                    if len(html.var("vapWEPsecuritySetup.vapWEPkey2"))==0:
                        dic_result["vapWEPsecuritySetup.vapWEPkey2"]= ""
                    else:
                        if (len(html.var("vapWEPsecuritySetup.vapWEPkey2"))==5 or len(html.var("vapWEPsecuritySetup.vapWEPkey2"))==13 or len(html.var("vapWEPsecuritySetup.vapWEPkey2"))==16):
                            dic_result["vapWEPsecuritySetup.vapWEPkey2"]= html.var("vapWEPsecuritySetup.vapWEPkey2")
                        else:
                    ...
                    if len(html.var("vapWEPsecuritySetup.vapWEPkey3"))==0:
                        dic_result["vapWEPsecuritySetup.vapWEPkey3"]= ""
                    else:
                        if (len(html.var("vapWEPsecuritySetup.vapWEPkey3"))==5 or len(html.var("vapWEPsecuritySetup.vapWEPkey3"))==13 or len(html.var("vapWEPsecuritySetup.vapWEPkey3"))==16):
                            dic_result["vapWEPsecuritySetup.vapWEPkey3"]= html.var("vapWEPsecuritySetup.vapWEPkey3")
                        else:
                    ...
                    if len(html.var("vapWEPsecuritySetup.vapWEPkey4"))==0:
                        dic_result["vapWEPsecuritySetup.vapWEPkey4"]= ""
                    else:
                        if (len(html.var("vapWEPsecuritySetup.vapWEPkey4"))==5 or len(html.var("vapWEPsecuritySetup.vapWEPkey4"))==13 or len(html.var("vapWEPsecuritySetup.vapWEPkey4"))==16):
                            dic_result["vapWEPsecuritySetup.vapWEPkey4"]= html.var("vapWEPsecuritySetup.vapWEPkey4")
                        else:
                    ...
                    dic_result["vapWEPsecuritySetup.vapWEPprimaryKey"] = html.var("vapWEPsecuritySetup.vapWEPprimaryKey")
            else:    
                dic_result["basicVAPsecurity.vapSecurityMode"] = html.var("basicVAPsecurity.vapSecurityMode")
                if html.var("vapWPAsecuritySetup.vapWPAmode")!=None:
                    dic_result["vapWPAsecuritySetup.vapWPAmode"] = html.var("vapWPAsecuritySetup.vapWPAmode")
                
                if html.var("vapWPAsecuritySetup.vapWPAcypher")!=None:
                    dic_result["vapWPAsecuritySetup.vapWPAcypher"]= html.var("vapWPAsecuritySetup.vapWPAcypher")
                    
                if html.var("vapWPAsecuritySetup.vapWPArekeyInterval")!=None:
                    if Validation.is_required(html.var("vapWPAsecuritySetup.vapWPArekeyInterval")):
                        if Validation.is_number(html.var("vapWPAsecuritySetup.vapWPArekeyInterval")):
                            dic_result["vapWPAsecuritySetup.vapWPArekeyInterval"]=html.var("vapWPAsecuritySetup.vapWPArekeyInterval")
                        else:
                            flag = 1
                            dic_result["success"]=1
                            dic_result["result"]="WPA Rekey Interval must be number"
                    else:
                        flag = 1
                        dic_result["success"]=1
                        dic_result["result"]="WPA Rekey Interval is required"
                
                if html.var("vapWPAsecuritySetup.vapWPAmasterReKey")!=None:
                    if Validation.is_required(html.var("vapWPAsecuritySetup.vapWPAmasterReKey")):
                        if Validation.is_number(html.var("vapWPAsecuritySetup.vapWPAmasterReKey")):
                            dic_result["vapWPAsecuritySetup.vapWPAmasterReKey"]=html.var("vapWPAsecuritySetup.vapWPAmasterReKey")
                        else:
                            flag = 1
                            dic_result["success"]=1
                            dic_result["result"]="WPA Master Rekey Interval must be number"
                    else:
                        flag = 1
                        dic_result["success"]=1
                        dic_result["result"]="WPA Master Rekey Interval is required"
                    
                if html.var("vapWPAsecuritySetup.vapWEPrekeyInt")!=None:
                    if Validation.is_required(html.var("vapWPAsecuritySetup.vapWEPrekeyInt")):
                        if Validation.is_number(html.var("vapWPAsecuritySetup.vapWEPrekeyInt")):
                            dic_result["vapWPAsecuritySetup.vapWEPrekeyInt"]=html.var("vapWPAsecuritySetup.vapWEPrekeyInt")
                        else:
                            flag = 1
                            dic_result["success"]=1
                            dic_result["result"]="WPA Pre Key Interval must be number"
                    else:
                        flag = 1
                        dic_result["success"]=1
                        dic_result["result"]="WPA Master Rekey Interval is required"
                        
                if html.var("vapWPAsecuritySetup.vapWPAkeyMode")!=None:
                    
                    if int(html.var("vapWPAsecuritySetup.vapWPAkeyMode"))==0:
                        dic_result["vapWPAsecuritySetup.vapWPAkeyMode"] = html.var("vapWPAsecuritySetup.vapWPAkeyMode")
                        if Validation.is_required(html.var("vapWPAsecuritySetup.vapWPAconfigPSKPassPhrase")):
                            dic_result["vapWPAsecuritySetup.vapWPAconfigPSKPassPhrase"]=html.var("vapWPAsecuritySetup.vapWPAconfigPSKPassPhrase")
                        else:
                            flag = 1
                            dic_result["vapWPAsecuritySetup.vapWPAconfigPSKPassPhrase"] = "Pass Phrase is required"
                    else:
                        dic_result["vapWPAsecuritySetup.vapWPAkeyMode"] = html.var("vapWPAsecuritySetup.vapWPAkeyMode")
                        if html.var("vapWPAsecuritySetup.vapWPArsnPreAuth")!=None:
                            dic_result["vapWPAsecuritySetup.vapWPArsnPreAuth"]=html.var("vapWPAsecuritySetup.vapWPArsnPreAuth")
                        if html.var("vapWPAsecuritySetup.vapWPArsnPreAuthInterface")!=None:
                            dic_result["vapWPAsecuritySetup.vapWPArsnPreAuthInterface"]=html.var("vapWPAsecuritySetup.vapWPArsnPreAuthInterface")
                        
                        if html.var("vapWPAsecuritySetup.vapWPAeapReAuthPeriod")!=None:
                            if Validation.is_required(html.var("vapWPAsecuritySetup.vapWPAeapReAuthPeriod")):
                                if Validation.is_number(html.var("vapWPAsecuritySetup.vapWPAeapReAuthPeriod")):
                                    dic_result["vapWPAsecuritySetup.vapWPAeapReAuthPeriod"]=html.var("vapWPAsecuritySetup.vapWPAeapReAuthPeriod")
                                else:
                                    flag = 1
                                    dic_result["success"] = 1
                                    dic_result["result"] = "WPA Reauth period must be number"
                            else:
                                flag = 1
                                dic_result["success"] = 1
                                dic_result["result"] = "WPA Reauth period is required"
                        
                    if html.var("vapWPAsecuritySetup.vapWPAserverIP")!=None:
                        if Validation.is_required(html.var("vapWPAsecuritySetup.vapWPAserverIP")):
                            if Validation.is_valid_ip(html.var("vapWPAsecuritySetup.vapWPAserverIP")):
                                dic_result["vapWPAsecuritySetup.vapWPAserverIP"]=html.var("vapWPAsecuritySetup.vapWPAserverIP")
                            else:
                                flag = 1
                                dic_result["success"] = 1
                                dic_result["result"] = "Invalid WPA Server IPAddress"
                        else:
                            flag = 1
                            dic_result["success"] = 1
                            dic_result["result"] = "WPA Server IPAddress is required"
                    
                    if html.var("vapWPAsecuritySetup.vapWPAserverPort")!=None:
                        if Validation.is_required(html.var("vapWPAsecuritySetup.vapWPAserverPort")):
                            if Validation.is_number(html.var("vapWPAsecuritySetup.vapWPAserverPort")):
                                dic_result["vapWPAsecuritySetup.vapWPAserverPort"]=html.var("vapWPAsecuritySetup.vapWPAserverPort")
                            else:
                                flag = 1
                                dic_result["success"] = 1
                                dic_result["result"] = "WPA Server Port must be number"
                        else:
                            flag = 1
                            dic_result["success"] = 1
                            dic_result["result"] = "WPA Server Port is required"
                            
                    if html.var("vapWPAsecuritySetup.vapWPAsharedSecret")!=None:
                        if Validation.is_required(html.var("vapWPAsecuritySetup.vapWPAsharedSecret")):
                            dic_result["vapWPAsecuritySetup.vapWPAsharedSecret"]=html.var("vapWPAsecuritySetup.vapWPAsharedSecret")
                        else:
                            flag = 1
                            dic_result["success"] = 1
                            dic_result["result"] = "WPA Shared Secret is required"
        ...
        if flag == 1:
            ...
            #html.write(str(dic_result))
            result = obj_bll_set_valid.vap_set(host_id,device_type,dic_result,selectedvap,vap_id)
            html.req.content_type = 'application/json'
            html.req.write(str(JSONEncoder().encode(result)))
...
def show_wireless_status(h):
    global html
    html = h
    wifi_mode = {0:'wifi11g',1:'wifi11gnHT20',2:'wifi11gnHT40plus',3:'wifi11gnHT40minus'}
    ...
    basic_vap_data = obj_ap_data.common_get_data('Ap25BasicVAPsetup',host_id)
    basic_vap_security_data = obj_ap_data.common_get_data('Ap25BasicVAPsecurity',host_id)
    html_str = ""
    if len(wireless_data)>0:
        result_dic['result'].update({'channel':"-" if wireless_data[0].radioChannel==None else wireless_data[0].radioChannel})
        result_dic['result'].update({'wifimode':"-" if wireless_data[0].wifiMode==None else wifi_mode[int(wireless_data[0].wifiMode)]})
        result_dic['result'].update({'txpower':"-" if wireless_data[0].radioTxPower==None else wireless_data[0].radioTxPower})
        result_dic['result'].update({'txrxchain':"-" if wireless_data[0].radioTXChainMask==None and wireless_data[0].radioRXChainMask==None else str(wireless_data[0].radioTXChainMask)+"x"+str(wireless_data[0].radioRXChainMask)})
        ...
        if wireless_data[0].radioAPmode!=None:
            if int(wireless_data[0].radioAPmode)==1 or int(wireless_data[0].radioAPmode)==3:
                ...
                activevap = 0 if wireless_data[0].numberofVAPs==None else wireless_data[0].numberofVAPs
        else:
            activevap = 0
           
        for i in range(0,activevap):
            if len(basic_vap_data)>0 and len(basic_vap_security_data)>0:
                vap_data.update({'ssid%s'%(i):"-" if basic_vap_data[i].vapESSID==None else basic_vap_data[i].vapESSID,\
...
def system_info(h):
    ...
    ip_setting_data = obj_ap_data.common_get_data('Ap25AccesspointIPsettings',host_id)
    html_str = ""
    html_str+="<table id=\"table_status\" name=\"table_status\" class=\"yo-table\"  style=\"border:2px;border-style:solid;border-color:#6b6969;z-index:1px\">"
...
def show_radio_admin(h):
    ...
    host_id = html.var("host_id")
    device_type = html.var("device_type_id")    
    main_admin_list = obj_idu_get_data.common_get_data('Ap25RadioSetup',host_id)
    ...
    if len(main_admin_list)>0:
        for i in range(0,len(main_admin_list)):
            ...
            %("images/temp/green_dot.png" if int(main_admin_list[i].radioState)==1 else "images/temp/red_dot.png",\
            "Radio Enable" if int(main_admin_list[i].radioState)==1 else "Radio Disable",\
...
def disable_enable_radio(h):
    ...
    obj = APRadioState()
    result_dic = obj.radio_enable_disable(host_id,admin_state_name,int(state))
    html.req.content_type = 'application/json'
