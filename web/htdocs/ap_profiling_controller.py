...
def ap_listing(h):
    try:
        ...
        html.new_header("AP Listing","ap_listing.py",add_btn,css_list,javascript_list)
...
def get_client_data_table(h):
    ...
    client_list=ap_device_list_bll_obj.pagination_create_table(i_display_start,i_display_length,s_search,sEcho,req_vars)
...
def ap_profiling(h):
    ...
    if isinstance(device_list_parameter,list):
        ...
        if device_list_parameter == [] or device_list_parameter == None:
            ...
            html.write(page_header_search(device_list_parameter[0][0],device_list_parameter[0][1],"RM18,RM,IDU,Access Point,CCU",device_type,device_list_state,"device_type"))
...
def edit_ap_client_details(h):
    ...
    result = APConnectedClients().edit_ap_client_details(client_id,client_name,client_ip)
    html.req.content_type = 'application/json'
    ...
    try:
        ...
        if isinstance(device_list,list):
            for i in range(0,len(device_list)):
                ...
                op_status = essential_obj.get_hoststatus(device_list[i][0]) # Get the host status and this code use for op state status 
                if op_status==None:
                ...
                chk_status = obj_status.chk_radio_status(str(device_list[i][0]))
                ...
                total_clients_connected = obj_clients_connected.total_connected_clients(device_list[i][0])                
                ...
                if i==len(device_list)-1:
                    ...
                    device_status_host_id+=str(device_list[i][0])+","
                live_monitoring = "&nbsp;&nbsp;<a target=\"main\" href=\"live_monitoring.py?host_id=%s&device_type=%s\"><img src=\"images/new/star-empty.png\" title=\"Live Monitoring\" style=\"width:16px;height:16px;\" class=\"imgbutton n-reconcile\" /></a>"%(device_list[i][0],device_list[i][5])
                
                #monitoring_status = "<img src=\"images/new/info1.png\" style=\"width:16px;height:16px;\" title=\"Current Device Status\" class=\" n-reconcile\"/>" if device_list[i][5]=="ap25" else "<img src=\"images/new/info1.png\" style=\"width:16px;height:16px;\" title=\"Current Device Status\" class=\"imgbutton n-reconcile\"/>"
                monitoring_status = ""                
                if html.req.session["role"] == "admin" or html.req.session["role"] == "user":
                    ...
                    %(device_list[i][0],device_status_image_path,device_status)\
                    ,device_list[i][1],device_list[i][2],device_list[i][3],device_list[i][4],connected_clients,ap_mode[radio_ap_mode],\
                    ...
                    device_list[i][3],\
                    device_list[i][0],device_list[i][5],device_list_state,\
                    images,device_list[i][6],device_list[i][0],device_list[i][5],live_monitoring,monitoring_status,\
                    ...
                    device_list[i][0],'sp_dashboard_profiling.py' if device_list[i][5] == "ap25" else 'sp_dashboard_profiling.py',device_list[i][0],device_list[i][5],device_list_state,\
                    device_list[i][3],\
                    images,device_list[i][6],live_monitoring,monitoring_status,\
                    "<input type=\"hidden\" value=\"%s\" name=\"host_id\" id=\"host_id\" />"%(device_status_host_id) if i==len(device_list)-1 else ""),"<center><img id=\"operation_status\" name=\"operation_status\" src=\"%s\" title=\"%s\" style=\"width:12px;height:12px;\" class=\"n-reconcile\"/></center>&nbsp;&nbsp;"%(op_img,op_title)])
...
def get_device_list_ap_profiling(h): 
    try:
        ...
        if result==0 or result==1 or result==2:
            ...
            group_name=html.req.session['group']
            html.write(APProfiling.ap_profile_call(group_name,result,selected_device,device_list_parameter))
...
def update_reconciliation(h):
    ...
    if username==None:
        username = ""
    result = obj_reconcilation.update_configuration(host_id,device_type,table_prefix,current_time,username)
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(result)))
        ...
        if len(result["result"])>0:
            ...
            for i in result["result"]:
                ...
                for j in result["result"][i]:
                    ap_dhcp_client_table +="<td>%s</td>"%(str(j) if str(j).find("\n")==-1 else "" if str(j)=="\n" else str(j)[0:-1])
                k+=1
                ap_dhcp_client_table+="</tr>"
        else:
            ap_dhcp_client_table+="<tr><td colspan=\"5\">No Data Exists.</tr>"
    else:
        ap_dhcp_client_table+="<tr><td colspan=\"5\">%s</td></tr>"%(result["result"])
...
def ap_scan(h):
    ...
    if result["success"] == 0:        
        ...
        ap_scan_table+="<tr><td colspan=\"9\">%s</td></tr>"%(result["result"])
    ap_scan_table+="</table>"
                ...
                if int(html.var("radioSetup.radioAggregation")) == 1 or True:
                    ...
                    if html.var("radioSetup.radioAggFrames")!=None:
                        if Validation.is_required(html.var("radioSetup.radioAggFrames")):
                            ...
                            flag = 1
                            dic_result["success"] = 1
                            dic_result["result"] = "Aggregation Frame is required"
            ...
            if Validation.is_required(html.var("services.systemLogPort")):
                if Validation.is_number(html.var("services.systemLogPort")):
                    if int(html.var("services.systemLogPort"))>=1 and int(html.var("services.systemLogPort"))<=65535:
                        ...
                        flag = 1
                        dic_result["success"] = 1
                        dic_result["result"] = "Log Port Number should be 1 to 65535"
                else:
                    flag = 1
                    dic_result["success"] = 1
...
def ap_dhcp_form_action(h):
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("dhcpServer.dhcpServerStatus")!=None:
            if Validation.is_required(html.var("dhcpServer.dhcpServerStatus")):
                dic_result["dhcpServer.dhcpServerStatus"]=html.var("dhcpServer.dhcpServerStatus")
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("dhcpServer.dhcpStartIPaddress")!=None:
            if Validation.is_required(html.var("dhcpServer.dhcpStartIPaddress")):
                if Validation.is_valid_ip(html.var("dhcpServer.dhcpStartIPaddress")) and int(html.var("dhcpServer.dhcpStartIPaddress").split('.')[0]) not in [0,255] and int(html.var("dhcpServer.dhcpStartIPaddress").split('.')[-1]) not in [0,255]:
                    dic_result["dhcpServer.dhcpStartIPaddress"]=html.var("dhcpServer.dhcpStartIPaddress")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Please enter the valid DHCP start IP Address"
            else:
                flag = 1
                dic_result["success"] = 1
        ...
        if html.var("dhcpServer.dhcpEndIPaddress")!=None:
            if Validation.is_required(html.var("dhcpServer.dhcpEndIPaddress")):
                if Validation.is_valid_ip(html.var("dhcpServer.dhcpEndIPaddress")) and int(html.var("dhcpServer.dhcpEndIPaddress").split('.')[0]) not in [0,255] and int(html.var("dhcpServer.dhcpEndIPaddress").split('.')[-1]) not in [0,255]:
                    dic_result["dhcpServer.dhcpEndIPaddress"]=html.var("dhcpServer.dhcpEndIPaddress")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Please enter the valid DHCP end IP Address"
            else:
                flag = 1
                dic_result["success"] = 1
	...
	if dic_result["success"] == 0 and html.var("dhcpServer.dhcpStartIPaddress")!=None and html.var("dhcpServer.dhcpEndIPaddress")!=None:
	    if dic_result["dhcpServer.dhcpStartIPaddress"] > dic_result["dhcpServer.dhcpEndIPaddress"]:
	        flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Start DHCP IP Address can't be greater than end DHCP IP Address"
	    elif int(dic_result["dhcpServer.dhcpStartIPaddress"].split('.')[0]) != int(ip_address.split('.')[0]) :
	        flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Please check the domain of start DHCP IP Address"
	    elif int(dic_result["dhcpServer.dhcpEndIPaddress"].split('.')[0]) != int(ip_address.split('.')[0]):
	        flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Please check the domain of end DHCP IP Address"
	    elif dic_result["dhcpServer.dhcpStartIPaddress"] <= ip_address and ip_address <= dic_result["dhcpServer.dhcpEndIPaddress"]:
	        flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "Device IP address can't be exists in DHCP IP range"
        ...
        if html.var("dhcpServer.dhcpSubnetMask")!=None:
            if Validation.is_required(html.var("dhcpServer.dhcpSubnetMask")):
                if Validation.is_valid_ip(html.var("dhcpServer.dhcpSubnetMask")) and int(html.var("dhcpServer.dhcpSubnetMask").split('.')[0]) not in [0]:
                    dic_result["dhcpServer.dhcpSubnetMask"]=html.var("dhcpServer.dhcpSubnetMask")
                else:
                    flag = 1
                    dic_result["success"] = 1
                    dic_result["result"] = "Please enter the valid DHCP subnet Mask IP Address"
            else:
                flag = 1
                dic_result["success"] = 1
                dic_result["result"] = "DHCP Network Mask IP Address is required"
            ...
            if Validation.is_required(html.var("dhcpServer.dhcpClientLeaseTime")):
                if Validation.is_positive_number(html.var("dhcpServer.dhcpClientLeaseTime")):
                    if int(html.var("dhcpServer.dhcpClientLeaseTime"))>=10 and int(html.var("dhcpServer.dhcpClientLeaseTime"))<=6000:
                        dic_result["dhcpServer.dhcpClientLeaseTime"]=html.var("dhcpServer.dhcpClientLeaseTime")
                    else:
                        flag = 1
                        dic_result["success"] = 1
                        dic_result["result"] = "Lease Time must be in 10 to 6000"
                else:
                    flag = 1
                    dic_result["success"] = 1
        ...
        if flag == 1:
            ...
            #html.write(str(dic_result))
            time.sleep(2)
            result = obj_bll_set_valid.common_set_config(host_id,device_type_id,dic_result)
            html.req.content_type = 'application/json'
            html.req.write(str(JSONEncoder().encode(result)))
...
def ap_acl_form_action(h):
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("basicACLconfigTable.aclState")!=None:
            dic_result["basicACLconfigTable.aclState"]=html.var("basicACLconfigTable.aclState")
        else:
            flag = 1
            dic_result["success"] = 1
            dic_result["result"] = "You can not do this operation because ACL Mode is disable."
            
        if html.var("basicACLconfigTable.aclMode")!=None:
            dic_result["basicACLconfigTable.aclMode"]=html.var("basicACLconfigTable.aclMode")
...
def ap_vap_form_action(h):
    ...
    if html.var("host_id") == "" or html.var("host_id") == None:
        ...
        if html.var("basicVAPconfigTable.vapESSID")!=None:
            if Validation.is_required(html.var("basicVAPconfigTable.vapESSID")):
                if len(html.var("basicVAPconfigTable.vapESSID"))>=0 and len(html.var("basicVAPconfigTable.vapESSID"))<33:
                    dic_result["basicVAPconfigTable.vapESSID"]=html.var("basicVAPconfigTable.vapESSID")
                else:
                    flag = 1
                    dic_result["success"]=1
        ...
        if html.var("essid")!=None:
            dic_result["basicVAPconfigTable.vapHiddenESSIDstate"] = html.var("essid")
            ...
            if Validation.is_required(html.var("basicVAPconfigTable.vlanId")):
                if Validation.is_number(html.var("basicVAPconfigTable.vlanId")):
                    if int(html.var("basicVAPconfigTable.vlanId"))>0 and int(html.var("basicVAPconfigTable.vlanId"))<4096:
                        dic_result["basicVAPconfigTable.vlanId"] = html.var("basicVAPconfigTable.vlanId")
                    else:
                        flag = 1
                        dic_result["success"]=1
            ...
            if Validation.is_required(html.var("basicVAPconfigTable.vlanPriority")):
                if Validation.is_number(html.var("basicVAPconfigTable.vlanPriority")):
                    if int(html.var("basicVAPconfigTable.vlanPriority"))>=0 and int(html.var("basicVAPconfigTable.vlanPriority"))<8:
                        dic_result["basicVAPconfigTable.vlanPriority"] = html.var("basicVAPconfigTable.vlanPriority")
                    else:
                        flag = 1
                        dic_result["success"]=1
                        dic_result["result"]="VLAN Priority must be in 0 to 7"
                else:
                    flag = 1
                    dic_result["success"]=1
        ...
        if html.var("basicVAPconfigTable.vapRadioMac")!=None:
            if Validation.is_required(html.var("basicVAPconfigTable.vapRadioMac")):
                if Validation.is_valid_mac(html.var("basicVAPconfigTable.vapRadioMac")):
                    dic_result["basicVAPconfigTable.vapRadioMac"] = html.var("basicVAPconfigTable.vapRadioMac")
                else:
                    flag = 1
                    dic_result["success"]=1
        ...
        if html.var("basicVAPconfigTable.vapMode")!=None:
            if Validation.is_required(html.var("basicVAPconfigTable.vapMode")):
                dic_result["basicVAPconfigTable.vapMode"] = html.var("basicVAPconfigTable.vapMode")
                        ...
                        if Validation.is_required(html.var("basicVAPconfigTable.vapRTSthresholdValue")):
                            if Validation.is_number(html.var("basicVAPconfigTable.vapRTSthresholdValue")):
                                if int(html.var("basicVAPconfigTable.vapRTSthresholdValue"))>=256 and int(html.var("basicVAPconfigTable.vapRTSthresholdValue"))<=2346:
                                    dic_result["basicVAPconfigTable.vapRTSthresholdValue"]=html.var("basicVAPconfigTable.vapRTSthresholdValue")
                                else:
                                    flag = 1
                                    dic_result["success"]=1
                                    dic_result["result"]="RTS Threshold should be between 256 to 2346"
        ...
        if html.var("frag_mode")!=None:
            if Validation.is_required(html.var("frag_mode")):
                if int(html.var("frag_mode"))==1:
                        ...
                        if Validation.is_required(html.var("basicVAPconfigTable.vapFragmentationThresholdValue")):
                            if Validation.is_number(html.var("basicVAPconfigTable.vapFragmentationThresholdValue")):
                                if int(html.var("basicVAPconfigTable.vapFragmentationThresholdValue"))>=256 and int(html.var("basicVAPconfigTable.vapFragmentationThresholdValue"))<=2346:
                                    dic_result["basicVAPconfigTable.vapFragmentationThresholdValue"]=html.var("basicVAPconfigTable.vapFragmentationThresholdValue")
                                else:
                                    flag = 1
                                    dic_result["success"]=1
                    ...
                    dic_result["basicVAPconfigTable.vapFragmentationThresholdValue"]=0
            else:
                flag = 1
                dic_result["success"]=1
            ...
            if Validation.is_required(html.var("basicVAPconfigTable.vapBeaconInterval")):
                if Validation.is_number(html.var("basicVAPconfigTable.vapBeaconInterval")):
                    if int(html.var("basicVAPconfigTable.vapBeaconInterval"))>=40 and int(html.var("basicVAPconfigTable.vapBeaconInterval"))<=1000:
                        dic_result["basicVAPconfigTable.vapBeaconInterval"]=html.var("basicVAPconfigTable.vapBeaconInterval")
                    else:
                        flag = 1
                        dic_result["success"]=1
                        dic_result["result"]="Beacon Interval must be in Between 40 and 1000"
                else:
                    flag = 1
                    dic_result["success"]=1
        ...
        if html.var("basicVAPconfigTable.vapSecurityMode")!=None:
            if int(html.var("basicVAPconfigTable.vapSecurityMode"))==0:
                ...
                dic_result["basicVAPconfigTable.vapSecurityMode"] = html.var("basicVAPconfigTable.vapSecurityMode")
            elif int(html.var("basicVAPconfigTable.vapSecurityMode"))==1:
                
                dic_result["basicVAPconfigTable.vapSecurityMode"] = html.var("basicVAPconfigTable.vapSecurityMode")
                if html.var("vapWEPsecurityConfigTable.vapWEPmode")!=None:
                    dic_result["vapWEPsecurityConfigTable.vapWEPmode"] = html.var("vapWEPsecurityConfigTable.vapWEPmode")
                ...
                if html.var("vapWEPsecurityConfigTable.vapWEPprimaryKey")!=None:
                        ...
                        if html.var("vapWEPsecurityConfigTable.vapWEPkey1")!=None:
                            if Validation.is_required(html.var("vapWEPsecurityConfigTable.vapWEPkey1")):
                                if (len(html.var("vapWEPsecurityConfigTable.vapWEPkey1"))==5 or len(html.var("vapWEPsecurityConfigTable.vapWEPkey1"))==13 or len(html.var("vapWEPsecurityConfigTable.vapWEPkey1"))==16):
                                    dic_result["vapWEPsecurityConfigTable.vapWEPkey1"] = html.var("vapWEPsecurityConfigTable.vapWEPkey1")
                                    wep_flag=1
                        ...
                        if html.var("vapWEPsecurityConfigTable.vapWEPkey2")!=None:
                            if Validation.is_required(html.var("vapWEPsecurityConfigTable.vapWEPkey2")):                                
                                if len(html.var("vapWEPsecurityConfigTable.vapWEPkey2"))==5 or len(html.var("vapWEPsecurityConfigTable.vapWEPkey2"))==13 or len(html.var("vapWEPsecurityConfigTable.vapWEPkey2"))==16:
                                    dic_result["vapWEPsecurityConfigTable.vapWEPkey2"] = html.var("vapWEPsecurityConfigTable.vapWEPkey2")
                                    wep_flag=2
                                    ...
                                    #flag = 1
                                    #dic_result["success"]=1
                                    #ic_result["result"] = "WEP key2 must be the length of 5,13 or 16"
                            else:
                                wep_flag_error=2
                                #flag = 1
                                #dic_result["success"] = 1
                        ...
                        if html.var("vapWEPsecurityConfigTable.vapWEPkey3")!=None:
                            if Validation.is_required(html.var("vapWEPsecurityConfigTable.vapWEPkey3")):
                                if len(html.var("vapWEPsecurityConfigTable.vapWEPkey3"))==5 or len(html.var("vapWEPsecurityConfigTable.vapWEPkey3"))==13 or len(html.var("vapWEPsecurityConfigTable.vapWEPkey3"))==16:
                                    dic_result["vapWEPsecurityConfigTable.vapWEPkey3"] = html.var("vapWEPsecurityConfigTable.vapWEPkey3")
                                    wep_flag=3
                        ...
                        if html.var("vapWEPsecurityConfigTable.vapWEPkey4")!=None:
                            if Validation.is_required(html.var("vapWEPsecurityConfigTable.vapWEPkey4")):
                                if len(html.var("vapWEPsecurityConfigTable.vapWEPkey4"))==5 or len(html.var("vapWEPsecurityConfigTable.vapWEPkey4"))==13 or len(html.var("vapWEPsecurityConfigTable.vapWEPkey4"))==16:
                                    dic_result["vapWEPsecurityConfigTable.vapWEPkey4"] = html.var("vapWEPsecurityConfigTable.vapWEPkey4")
                                    wep_flag=4
                    ...
                    if wep_flag>0 and wep_field_flag>0:
                        flag = 1
                        dic_result["success"]=1
                        dic_result["result"] = "WEP key%s Length can be 5,13,16 for ASCII and 10,26,32 for HEX"%wep_field_flag
                        ...
                        flag = 1
                        dic_result["success"]=1
                        dic_result["result"] = "WEP key%s is required"%html.var("vapWEPsecurityConfigTable.vapWEPprimaryKey")

                    dic_result["vapWEPsecurityConfigTable.vapWEPprimaryKey"] = html.var("vapWEPsecurityConfigTable.vapWEPprimaryKey")
            else:    
                
                dic_result["basicVAPconfigTable.vapSecurityMode"] = html.var("basicVAPconfigTable.vapSecurityMode")
                if html.var("vapWPAsecurityConfigTable.vapWPAmode")!=None:
                    dic_result["vapWPAsecurityConfigTable.vapWPAmode"] = html.var("vapWPAsecurityConfigTable.vapWPAmode")
                    
                if html.var("vapWPAsecurityConfigTable.vapWPAcypher")!=None:
                    dic_result["vapWPAsecurityConfigTable.vapWPAcypher"]= html.var("vapWPAsecurityConfigTable.vapWPAcypher")
                ...
                if html.var("vapWPAsecurityConfigTable.vapWPArekeyInterval")!=None:
                    if Validation.is_required(html.var("vapWPAsecurityConfigTable.vapWPArekeyInterval")):
                        if Validation.is_number(html.var("vapWPAsecurityConfigTable.vapWPArekeyInterval")):
                            dic_result["vapWPAsecurityConfigTable.vapWPArekeyInterval"]=html.var("vapWPAsecurityConfigTable.vapWPArekeyInterval")
                        else:
                            flag = 1
                            dic_result["success"]=1
                ...
                if html.var("vapWPAsecurityConfigTable.vapWPAmasterReKey")!=None:
                    if Validation.is_required(html.var("vapWPAsecurityConfigTable.vapWPAmasterReKey")):
                        if Validation.is_number(html.var("vapWPAsecurityConfigTable.vapWPAmasterReKey")):
                            dic_result["vapWPAsecurityConfigTable.vapWPAmasterReKey"]=html.var("vapWPAsecurityConfigTable.vapWPAmasterReKey")
                        else:
                            flag = 1
                            dic_result["success"]=1
                ...
                if html.var("vapWPAsecurityConfigTable.vapWPAmode")=='0' or int(html.var("vapWPAsecurityConfigTable.vapWPAmode")==0): 
                    if html.var("vapWPAsecurityConfigTable.vapWEPrekeyInt")!=None:
                        if Validation.is_required(html.var("vapWPAsecurityConfigTable.vapWEPrekeyInt")):
                            if Validation.is_number(html.var("vapWPAsecurityConfigTable.vapWEPrekeyInt")):
                                dic_result["vapWPAsecurityConfigTable.vapWEPrekeyInt"]=html.var("vapWPAsecurityConfigTable.vapWEPrekeyInt")
                            else:
                                flag = 1
                                dic_result["success"]=1
                    ...
                    if html.var("vapWPAsecurityConfigTable.vapWEPrekeyInt")!=None:
                        if Validation.is_number(html.var("vapWPAsecurityConfigTable.vapWEPrekeyInt")):
                            dic_result["vapWPAsecurityConfigTable.vapWEPrekeyInt"]=600
                        else:
                            flag = 1
                            dic_result["success"]=1
                            dic_result["result"]="WPA Pre Key Interval must be number"
                ...
                if html.var("vapWPAsecurityConfigTable.vapWPAkeyMode")!=None:
                    ...
                    if int(html.var("vapWPAsecurityConfigTable.vapWPAkeyMode"))==0:
                        dic_result["vapWPAsecurityConfigTable.vapWPAkeyMode"] = html.var("vapWPAsecurityConfigTable.vapWPAkeyMode")
                        if Validation.is_required(html.var("vapWPAsecurityConfigTable.vapWPAkeyMode")):
                            if int(len(html.var("vapWPAsecurityConfigTable.vapWPAconfigPSKPassPhrase")))>=8 and int(len(html.var("vapWPAsecurityConfigTable.vapWPAconfigPSKPassPhrase")))<=32:
                                dic_result["vapWPAsecurityConfigTable.vapWPAconfigPSKPassPhrase"]=html.var("vapWPAsecurityConfigTable.vapWPAconfigPSKPassPhrase")
                            else:
                                flag = 1
                                dic_result["success"] = 1
                                dic_result["result"] = "Key length should be in 8 to 32 characters"
                        else:
                            flag = 1
                            dic_result["success"] = 1
                        ...
                        if html.var("vapWPAsecurityConfigTable.vapWPArsnPreAuth")!=None:
                            dic_result["vapWPAsecurityConfigTable.vapWPArsnPreAuth"]=html.var("vapWPAsecurityConfigTable.vapWPArsnPreAuth")
                        if html.var("vapWPAsecurityConfigTable.vapWPArsnPreAuthInterface")!=None:
                            dic_result["vapWPAsecurityConfigTable.vapWPArsnPreAuthInterface"]=html.var("vapWPAsecurityConfigTable.vapWPArsnPreAuthInterface")
                        ...
                        if html.var("vapWPAsecurityConfigTable.vapWPAeapReAuthPeriod")!=None:
                            if Validation.is_required(html.var("vapWPAsecurityConfigTable.vapWPAeapReAuthPeriod")):
                                if Validation.is_number(html.var("vapWPAsecurityConfigTable.vapWPAeapReAuthPeriod")):
                                    dic_result["vapWPAsecurityConfigTable.vapWPAeapReAuthPeriod"]=html.var("vapWPAsecurityConfigTable.vapWPAeapReAuthPeriod")
                                else:
                                    flag = 1
                                    dic_result["success"] = 1
                    ...
                    if html.var("vapWPAsecurityConfigTable.vapWPAserverIP")!=None:
                        if Validation.is_required(html.var("vapWPAsecurityConfigTable.vapWPAserverIP")):
                            if Validation.is_valid_ip(html.var("vapWPAsecurityConfigTable.vapWPAserverIP")) and html.var("vapWPAsecurityConfigTable.vapWPAserverIP") not in ["0.0.0.0"] and int(html.var("vapWPAsecurityConfigTable.vapWPAserverIP").split('.')[-1]) not in [0,255]:
                                dic_result["vapWPAsecurityConfigTable.vapWPAserverIP"]=html.var("vapWPAsecurityConfigTable.vapWPAserverIP")
                            else:
                                flag = 1
                                dic_result["success"] = 1
                    ...
                    if html.var("vapWPAsecurityConfigTable.vapWPAserverPort")!=None:
                        if Validation.is_required(html.var("vapWPAsecurityConfigTable.vapWPAserverPort")):
                            if Validation.is_number(html.var("vapWPAsecurityConfigTable.vapWPAserverPort")):
                                dic_result["vapWPAsecurityConfigTable.vapWPAserverPort"]=html.var("vapWPAsecurityConfigTable.vapWPAserverPort")
                            else:
                                flag = 1
                                dic_result["success"] = 1
                    ...
                    if html.var("vapWPAsecurityConfigTable.vapWPAsharedSecret")!=None:
                        if Validation.is_required(html.var("vapWPAsecurityConfigTable.vapWPAsharedSecret")):
                            dic_result["vapWPAsecurityConfigTable.vapWPAsharedSecret"]=html.var("vapWPAsecurityConfigTable.vapWPAsharedSecret")
                        else:
                            flag = 1
                            dic_result["success"] = 1
...
def show_wireless_status(h):
    ...
    basic_vap_data = obj_ap_data.common_get_data('Ap25BasicVAPconfigTable',host_id)
