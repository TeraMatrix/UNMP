...
def get_dashboard_data():
    ...
    get_data=DashboardConfig.get_config_attributes('odu100_dashboard',devcie_type_attr)
...
def odu_profiling(h):
    ...
    if es.is_host_allow(user_id,host_id) == 0:    
        ...
        javascript_list = ["js/highcharts.js","js/odu100Dashboard.js","calendrical/calendrical.js"]
        html.new_header("UBRe Dashboard","","",css_list,javascript_list)
        html.write('<div class=\"form-div\">')
        host_id = ""
        extr_button={'tag':'button','id':'adSrhODU100','value':'Advance Graph','name':'adSrhODU100'}
        host_id = html.var("host_id")
        ...
        if host_id == "" or host_id == "None":
            val = ""
            flag=1
            html.write("<div id=\"odu100_show_msg\"></div><div id=\"tab_yo\">There is no profile selected</div>")
        else:
            html.write("<div id=\"odu100_show_msg\"></div><div id=\"tab_yo\" style=\"display:block;overflow:hidden\">")
            flag=0
            odu100_dashboard(h)
            html.write("</div>")
        if flag==0:
...
def odu100_dashboard(h):
    ...
    try:
        ...
        cursor.close()
        db.close()	
        html.write(str(dashboard_table(ip_address,odu_start_date,odu_start_time,odu_end_date,odu_end_time,str(odu_refresh_time),str(total_count))))
...
def odu100_network_interface_table_graph(h):
    ...
    try:
        ...
        if int(limitFlag)==0:
            ...
            limit_data=' limit 16'
        sel_query="select (odu.rxBytes),(odu.txBytes),odu.timestamp from odu100_nwInterfaceStatisticsTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where  h.ip_address='%s' AND  odu.timestamp >= '%s' AND odu.timestamp <='%s' and odu.nwStatsIndex = '%s' order by odu.timestamp desc"%(ip_address,start_time,end_time,interface_value)
...
def odu100_error_graph(h):
    ...
    try:
        ...
        if int(limitFlag)==0:
            ...
            limit_data=' limit 16'
        sel_query="select IFNULL((odu.rxCrcErrors),0),IFNULL((odu.rxPhyError),0),odu.timestamp from  odu100_raTddMacStatisticsTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp <='%s' AND odu.timestamp >= '%s' order by odu.timestamp"%(ip_address,end_time,start_time)
        sel_query+=limit_data
        cursor.execute(sel_query)
        result=cursor.fetchall()
        cursor.close()
...
def odu100_sync_lost_graph(h):
    ...
    try:
        ...
        if int(limitFlag)==0:
            ...
            limit_data=' limit 16'
        sel_query="select IFNULL((odu.syncLostCounter),0),odu.timestamp from  odu100_synchStatisticsTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' order by odu.timestamp"%(ip_address,start_time,end_time)
        sel_query+=limit_data
        cursor.execute(sel_query)
        result=cursor.fetchall()
        if result is not None:
...
def odu100_signal_strength_graph(h):
    ...
    limitFlag=html.var("limitFlag")
    ip_address=html.var("ip_address")
    signal_strength=["peer1","peer2","peer3","peer4","peer5","peer6","peer7","peer8","peer9","peer10","peer11","peer12","peer13","peer14","peer15","peer16"]
    try:
        # Open database connection
        ...
        sel_query="SELECT defaultNodeType FROM odu100_ruConfTable as def INNER JOIN hosts ON hosts.config_profile_id=def.config_profile_id WHERE hosts.ip_address='%s'"%(ip_address)
        cursor.execute(sel_query)
        ...
        if int(limitFlag)==0:
            ...
            if status_name=='Master':
                ...
                limit_data=' limit 5'
        sel_query="select odu.timeSlotIndex,(IFNULL((odu.sigStrength1),0)),odu.timestamp,odu.ssIdentifier,odu.linkStatus,odu.tunnelStatus,odu.peerMacAddr from  odu100_peerNodeStatusTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' order by odu.timestamp desc"%(ip_address,start_time,end_time)
        sel_query+=limit_data
        cursor.execute(sel_query)
...
def odu100_trap_graph(h):
    ...
    try:
        ...
        # convert the string to datetime
        # convert the string to datetime
        end_time=datetime.strptime(odu_start_date+' '+odu_start_time,"%d/%m/%Y %H:%M")
        start_time=datetime.strptime(odu_end_date+' '+odu_end_time,"%d/%m/%Y %H:%M")
        if start_time > datetime.now():
            start_time=datetime.now()
        # calculate the total days
        #total_days=((start_time-end_time).days)  check after the consult to peeyush sir
                ...
                for row in trap_result:
                    if datetime.date(datetime.now()+timedelta(days=-i))==row[1]:
                        if int(row[2])==0 :
                            ...
                            critical1+=int(row[0])
                time_stamp.append(datetime.date(datetime.now()+timedelta(days=-i)).strftime('%x'))
        ...
        if table_option.strip() == "trap": 
            ...
            for i in range(length):
                if i<4:
                    ...
                    history_trap+='<td>%s</td>'%all_traps[i][1]
                    history_trap+='<td>%s</td>'%all_traps[i][2]
                    history_trap+='<td>%s&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp%s</td></tr>'%(all_traps[i][3],(("<a href=\"status_snmptt.py?trap_status=history&ip_address="+ip_address+"\">more>></a>" if len(all_traps) >5 else "" )))
            cursor.close()
            ...
            for i in range(length):
                if i<4:
                    ...
                    current_alarm_html+='<td>%s</td>'%current_alarm[i][1]
                    current_alarm_html+='<td>%s</td>'%current_alarm[i][2]
                    current_alarm_html+='<td>%s&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp%s</td></tr>'%(current_alarm[i][3],(("<a href=\"status_snmptt.py?trap_status=history&ip_address="+ip_address+"\">more>></a>" if len(current_alarm) >5 else "" )))
            cursor.close()
...
def odu100_device_information(h):
    ...
    try:
        ...
        channel={0:'raBW5Mhz',1:'raBW10Mhz',2:'raBW20Mhz',3:'raBW5	0Mhz'}
        ...
        if result is not None:
            ...
            if len(slave)>1:
                no_of_slave='--' if slave[0][0]==None or slave[0][0]=='' else slave[0][0]
            else:
                no_of_slave='--'
...
def odu100_device_report(h):
    ...
    try:
        ...
        data=[]
        data.append(['UBRe  '+str(ip_address),str(start_time)+'--'+str(end_time)])
        ...
        if len(result)>0:
            ...
            table_output=device_information_function(result,'ODU Device Information',no_of_slave)
        ...
        if int(select_option)==0:
            sel_query="select IFNULL((odu.rxCrcErrors),0),IFNULL((odu.rxPhyError),0),odu.timestamp from  odu100_raTddMacStatisticsTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp <='%s' AND odu.timestamp >= '%s' order by odu.timestamp desc"%(ip_address,end_time,start_time)
            if int(limitFlag)==0:
                limit_data=''
            else:
                limit_data=' limit 16'
            ...
            sel_query="select IFNULL((odu.rxCrcErrors),0),IFNULL((odu.rxPhyError),0),odu.timestamp from  odu100_raTddMacStatisticsTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s order by odu.timestamp desc"%(ip_address,total_days)
        cursor.execute(sel_query)
        ...
        result1=''
        if int(select_option)==0:
            sel_query="select IFNULL((odu.syncLostCounter),0),odu.timestamp from  odu100_synchStatisticsTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' order by odu.timestamp desc"%(ip_address,start_time,end_time)
        ...
        if int(select_option)==0:
            sel_query="select odu.raScanIndex,(IFNULL((odu.signalStrength),0)),odu.timestamp from  odu100_raScanListTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' order by odu.timestamp desc"%(ip_address,start_time,end_time)
            signal_flag=0
            if int(limitFlag)==0:
                limit_data=''
            else:
        ...
        if status_name=='Master':
            ...
            odu100_report.append(t)
            table_output=[]
            table_output.append(['Time(HH:MM)','peer1','peer2','peer3','peer4','peer5','peer6','peer7','peer8'])
            k=0
            for time_field in time_stamp_signal1:
                table_output.append([str(time_field),str(signal_interface1[k]),str(signal_interface2[k]),str(signal_interface3[k]),str(signal_interface4[k]),str(signal_interface5[k]),str(signal_interface6[k]),str(signal_interface7[k]),str(signal_interface8[k])])
                k=k+1
            ...
            odu100_report.append(t)
            table_output=[]
            table_output.append(['Time(HH:MM)','peer9','peer10','peer11','peer12','peer13','peer14','peer15','peer16'])
            k=0
            for time_field in time_stamp_signal1:
                table_output.append([str(time_field),str(signal_interface9[k]),str(signal_interface10[k]),str(signal_interface11[k]),str(signal_interface12[k]),str(signal_interface13[k]),str(signal_interface14[k]),str(signal_interface15[k]),str(signal_interface16[k])])
                k=k+1
            t = Table(table_output,[1.5*inch,.7*inch,.7*inch,.7*inch,.7*inch,.7*inch,.7*inch,.7*inch,.7*inch])
            t.setStyle(TableStyle([('FONT', (0, 0), (8, 0), 'Helvetica-Bold', 10),        
                                   ('FONT', (0, 1), (8, int(len(table_output))-1), 'Helvetica', 9),        
            ...
            k=0
            for time_field in time_stamp_signal1:
                table_output.append([str(time_field),str(signal_interface1[k])])
                k=k+1
        ...
        ########################################### ODU INTERFACE ######################################## 
        nw_interface=['NETWORK BANDWIDTH RATE(eth0)','NETWORK BANDWIDTH RATE(eth1)']
        interface_index=1
        for interface in nw_interface:
            ...
            #result1=cursor.fetchall()
            if int(select_option)==0:
                sel_query="select (odu.rxBytes),(odu.txBytes),odu.timestamp from odu100_nwInterfaceStatisticsTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where  h.ip_address='%s' AND  odu.timestamp >= '%s' AND odu.timestamp <='%s' and odu.nwStatsIndex = '%s' order by timestamp desc"%(ip_address,start_time,end_time,interface_index)
                if int(limitFlag)==0:
                    limit_data=''
                else:
                    limit_data=' limit 16'
                ...
                sel_query="select (odu.rxBytes),(odu.txBytes),odu.timestamp from odu100_nwInterfaceStatisticsTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id where  h.ip_address='%s' AND  date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s and odu.nwStatsIndex = '%s' order by timestamp desc"%(ip_address,total_days,interface_index)
            cursor.execute(sel_query)
            result1=cursor.fetchall()
            table_output=nw_table_list_creation(result1,interface,'Receving Bytes(Rx)','Transmitting Bytes(Tx)','Time(HH:MM:SS)',)
            # close the database and cursor connection.
            cursor.close()
...
def nw_table_list_creation(result,table_name,first_header,second_header,time):
    ...
    if result is not None:
        for i in range(len(result)-1):
            ...
            temp_list=[]
            temp_list=[result[i][2].strftime('%d-%m-%Y %H:%M'),( 0 if rx_byte <= 0 else rx_byte),( 0 if tx_byte <= 0 else tx_byte)]
...
def table_list_alarm(result,table_name):
    ...
    for i in range(0,len(result)):
        temp_list=[]
        temp_list=[result[i][6].strftime('%x'),result[i][1],result[i][2],result[i][3],result[i][4],result[i][5]]
        output.append(temp_list)
    return output  
...
def table_list_trap(result,table_name):
    ...
    for i in range(0,length):
        temp_list=[]
        temp_list=[result[i][0],result[i][1],result[i][2],datetime.strptime(str(result[i][3]),'%a %b %d %H:%M:%S %Y').strftime('%x')]
        output.append(temp_list)
    return output  
...
def device_information_function(result,table_name,slave):
    ...
    default_node_type={0:'rootRU',1:'t1TDN',2:'t2TDN',3:'t2TEN'}
    operation_state={0:'disabled',1:'enabled'}
    channel={0:'raBW5Mhz',1:'raBW10Mhz',2:'raBW20Mhz',3:'raBW50Mhz',3:'raBW100Mhz'}
    ...
    try:
        ...
        if len(result)>0:
            ...
            if len(slave)>1:
                ...
                no_of_slave='--'
            table_output=device_information_function(result1,'ODU Device Information',no_of_slave)
        #---- Query for get the last reboot time of particular device ------#
        sql="SELECT trap_receive_date from trap_alarms where trap_event_type = 'NODE_UP' and agent_id='%s' order by timestamp desc limit 1"%ip_address
...
#		heading=[table_output[0][1],table_output[0][2],table_output[0][3],table_output[0][4],table_output[0][5],table_output[0][6],table_output[0][7]]
        ...
        for index in range(1,3):
            if int(select_option)==0:
                sel_query="select (odu.rxBytes),(odu.txBytes),odu.timestamp from odu100_nwInterfaceStatisticsTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where  h.ip_address='%s' AND  odu.timestamp >= '%s' AND odu.timestamp <='%s' and odu.nwStatsIndex = '%s' order by timestamp desc"%(ip_address,start_time,end_time,index)
                if int(limitFlag)==0:
                    limit_data=''
                else:
                    limit_data=' limit 16'
                ...
                sel_query="select (odu.rxBytes),(odu.txBytes),odu.timestamp from odu100_nwInterfaceStatisticsTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id where  h.ip_address='%s' AND  date(odu.timestamp) <=current_date() AND date(odu.timestamp) >= current_date()-%s and odu.nwStatsIndex = '%s' order by timestamp desc"%(ip_address,total_days,index)
            cursor.execute(sel_query)
            ...
            for i in range(0,len(nw_result)-1):
                temp_list=[]
                temp_list=[nw_result[i][2].strftime('%d-%m-%Y %H:%M'),0 if (int(nw_result[i][0])-int(nw_result[i+1][0]))<0 else (int(nw_result[i][0])-int(nw_result[i+1][0])),0 if (int(nw_result[i][1])-int(nw_result[i+1][1]))<0 else (int(nw_result[i][1])-int(nw_result[i+1][1]))]
        ...
        if status_name=='Master':
            ...
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
            xls_sheet.write_merge(0,0,0,16,"Signal Strength Information",style)
            xls_sheet.write_merge(1,1,0,16,str(device_type)+'       '+str(device_name)+ '         '+str(ip_address) ,style)
            xls_sheet.write_merge(2,2,0,16,"")
            i=4
            heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
            headings = ['Time','Peer1','Peer2','Peer3','Peer4','Peer5','Peer6','Peer7','Peer8','peer9','peer10','peer11','peer12','peer13','peer14','peer15','peer16']
            xls_sheet.set_panes_frozen(True) # frozen headings instead of split panes
            xls_sheet.set_horz_split_pos(i) # in general, freeze after last heading row
            xls_sheet.set_remove_splits(True) # if user does unfreeze, don't leave a split there
            for colx, value in enumerate(headings):
...
#		if len(crc_result)>0:
        xls_book.save('/omd/sites/%s/share/check_mk/web/htdocs/download/ubre_specific_report.xls' % nms_instance)
...
############################################  ODU Device Type dashboard creation by new theam ########################
