                        ...
                        for varBindTableRow in varBindTable:
                            for name, val in varBindTableRow:
                                ...
                                if isinstance(val,v2c.IpAddress):
                                    ...
                                    value = val.prettyPrint() #str(val) #str(val) #
...
#print table_bulk("1.3.6.1.4.1.26149.2.2.13.5.1","172.22.0.120",161,"public",5) 
                        ...
                        for varBindTableRow in varBindTable:
                            for name, val in varBindTableRow:
                                ...
                                if isinstance(val,v2c.IpAddress):
                                    ...
                                    value = str(val) #val.prettyPrint() val.prettyPrint() #str(val) #val.prettyPrint() #str(val)
                                    #if value.find('0x'):
                                    #    value = str(value[2:])
                ...
                for varBindTableRow in varBindTable:
                    for name, val in varBindTableRow:
                        ...
                        if isinstance(val,v2c.IpAddress):
                            ...
                            value = val.prettyPrint() #str(val) #val.prettyPrint() #str(val)
    ...
    try:
        if isinstance(ip_address,str) and isinstance(oid,str) and isinstance(community,str) and isinstance(port,int):
            try:
                ...
                while 1:
                    print " *%%**%*%*%*%*%*%*  ",first_value
                    return_dict = bulkget(ip_address, port, community, oid, first_value,max_value,varBindTable)
...
#print pysnmp_get_table('1.3.6.1.4.1.26149.2.1.2.2.1', '172.22.0.104', 8001, 'public')
...
#print pysnmp_get_table('1.3.6.1.4.1.26149.10.2.3.4.2.1.1', '172.22.0.101', 161, 'public')

#print pysnmp_get_node('1.3.6.1.4.1.26149.2.2.13.11.1', '172.22.0.102', 161, 'public')

#print pysnmp_get_node('1.3.6.1.4.1.26149.10.2.2', '172.22.0.101', 161, 'public')
...
#print pysnmp_get_table('1.3.6.1.4.1.26149.10.2.3.4.2.1.1','172.22.0.101',161,'private')

#print single_set('172.22.0.101', 161,'private',['1.3.6.1.4.1.26149.10.2.3.4.1.1.0','Integer32',1])
            ...
            if len(vap_selc_list) > 1:
                ...
                for vap in range(1,total_vap+1):
                    ...
                    if set_dict['success'] == 0:
                        result2_dict = pysnmp_get_node(basicVAPsetup_oid, ip_address, port, get_community) # basicVapSetup
                        ...
                        if result2_dict['success'] == 0:
                            ...
                            result2_dict = pysnmp_get_node(basicVAPsecurity_oid,ip_address, port, get_community) # basicVapSecurity
                            ...
                            if result2_dict['success'] == 0:
                                ...
                                result2_dict = pysnmp_get_node(vapWPAsecuritySetup_oid,ip_address, port, get_community) #vapWAP
                                if result2_dict['success'] == 0:
                                    list2 = result2_dict['result'][1] if len(result2_dict['result']) > 0 else []
                                    vap_data_list.append(list2)
                                else:
                                ...
                                result2_dict = pysnmp_get_node(basicACLsetup_oid,ip_address, port, get_community) #basicACLmode
                                if result2_dict['success'] == 0:
                                    list2 = result2_dict['result'][1] if len(result2_dict['result']) > 0 else []
                                    list2 = list2[:2] 
                                    list3 = []
                                    flag_acl_mode = 0
                                    if list2[0] == '0':
                                        acl_dict = single_set(ip_address, port,set_community,[aclState_oid,'Integer32',1])
                                        #print " @",acl_dict
                                        if acl_dict['success'] == 0:
                                            flag_acl_mode = 1
                                    time.sleep(1)
                                    result2_dict = pysnmp_get_table(aclMACsTable_oid,ip_address, port, get_community) #ACLmacTable
                                    #print " >> ",result2_dict
                                    if result2_dict['success'] == 0:
                                        for i in range(1,len(result2_dict['result'])+1):
                                            list3.append(result2_dict['result'][i])                                    
                                    if flag_acl_mode:
                                        acl_dict = single_set(ip_address, port,set_community,[aclState_oid,'Integer32',0]) 
                                        if acl_dict['success'] == 0:
                                            pass
                                        else:
                                            list2.pop(0)
    ...
    try:
        if isinstance(ip_address,str) and isinstance(oid,str) and isinstance(community,str) and isinstance(port,int):
            try:
                ...
                errorIndication, errorStatus, errorIndex, varBindTable = cmdgen.CommandGenerator().nextCmd(cmdgen.CommunityData('get-agent', community),cmdgen.UdpTransportTarget((ip_address, port)),make_tuple(oid))
                if errorIndication:
                        response_dict[553] = str(errorIndication)
                        success = 1
                        return  
                    ...
                    if errorStatus > 0 and errorIndex != None:
                        response_dict[int(errorStatus)] = errorStatus.prettyPrint()
                        success = 1  
                        return                                                     
                    elif errorStatus == 0:
                        #print varBindTable          
                        if len(varBindTable) < 1: success = 1;response_dict[102] = 'NO VALUE ASSOCIATED WITH OID FOR SNMP GET';
                        for varBindTableRow in varBindTable:
                            for name, val in varBindTableRow:
                                if isinstance(val,v2c.IpAddress):
                                    value = str(val.prettyPrint())
            ...
            response_dict = {}
            success = 1
            response_dict[96] = "Arguments_are_not_proper : oid as str ( don't include first dot as .1.3 must be 1.3),ip_address as str ,port as int,community as str"
...
#print pysnmp_get('1.3.6.1.4.1.26149.2.2.5.1.7', '172.22.0.102', 161, 'public')
...
def pysnmp_seter(received_dict = {},ip_address=None,port=None,community=None,admin_state = {}, is_recursive=0,response_dict={},temp_dict = {},error_dict = {},state=0):
    ...
    try:
        ...
        aoid_str = None
        snmp_args = None
        datatypes_dict = {'Integer32':v2c.Integer32, 'Integer':v2c.Integer32, 'Unsigned32':v2c.Gauge32, 'OctetString':v2c.OctetString, 'DisplayString':v2c.OctetString,   'Gauge32':v2c.Gauge32, 'Gauge':v2c.Gauge32, 'IpAddress':v2c.IpAddress, 'Counter32':v2c.Counter32, 'Counter':v2c.Counter32, 'Counter64':v2c.Counter64}
        if is_recursive:
            print " **** recursive \n",port
            is_recursive = 0
            ...
            for i in received_dict.keys():
                ...
                if error_dict.has_key(oid_tuple):
                    pass
                else :
                    snmp_args.append((oid_tuple,datatypes_dict[datatype](value)))
                ...
                if errorIndication:
                    ...
                    if errorStatus > 0 and errorIndex != None:
                        ...
                        if first_varBind == aoid_str:
                            success = 1
                            response_dict[54] = "NOT ABLE TO SET FIRST DEFIND VALUE"
                            return 
                        else:
                            state = 0
                            response_dict[temp_dict[first_varBind]] = int(errorIndex)
                        ...
                        if len(error_dict) >= len(received_dict):
                            ...
                            is_recursive = 1
                            #print " *********** ",error_dict,response_dict,temp_dict
                    ...
                    if errorIndication:
                        ...
                        if errorStatus > 0 and errorIndex != None:
                            ...
                            if first_varBind == aoid_tuple:
                                ...
                                response_dict[temp_dict[first_varBind]] = int(errorStatus)
                                error_dict[first_varBind] = 1
