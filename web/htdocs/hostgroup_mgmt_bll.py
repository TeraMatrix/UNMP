...
class HostgroupMgmtBll(object):
	...
	'''
    	try:
            ...
            for i in range(0,len(lis)):
                id=lis[i].pop(0)
                lis[i].insert(0,'<div id="ck_box"><input type="checkbox" name="group_check" value="%s" />' %(id))
                lis[i].append('<img class=\"img-link\" src=\"images/new/info.png\" title=\"View User Details\" onclick=\"viewGroupDetails(\'%s\');\">' %(id))
            #return lis
            #if(len(lis)==0):
            #    return []
            if flag_grp == 1:
            ...
            for i in lism:
                ...
                if lis_temp.count(i[0])==0:
                    tr=[]
                    id=i[1]
                    tr.append('<div id="ck_box"><input type="checkbox" name="group_check" value="%s" />' %(id))
                    tr.append(i[0])
                    tr.append(" ")
                    tr.append('<img class=\"img-link\" src=\"images/new/info.png\" title=\"View User Details\" onclick=\"viewGroupDetails(\'%s\');\">' %(id))
                    lis_group.append(tr)
