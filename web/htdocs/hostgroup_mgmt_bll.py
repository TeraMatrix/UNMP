...
class HostgroupMgmtBll(object):
    ...
    def get_usergroup_data(self,host_id,flag_grp=0):
        ...
        try:
            ...
            for i in range(0,len(lis)):
                sid=lis[i].pop(0)
                lis[i].insert(0,'<div id="ck_box"><input type="checkbox" name="group_check" value="%s" />' %(sid))
                lis[i].append('<img class=\"img-link\" src=\"images/new/info.png\" title=\"View User Details\" onclick=\"viewGroupDetails(\'%s\');\">' %(sid))
            #return lis
            #if(len(lis)==0):
            #    return []
            if flag_grp == 1:
            ...
            for i in lism:
                ...
                if lis_temp.count(i[0])==0:
                    tr=[]
                    sid=i[1]
                    tr.append('<div id="ck_box"><input type="checkbox" name="group_check" value="%s" />' %(sid))
                    tr.append(i[0])
                    tr.append(" ")
                    tr.append('<img class=\"img-link\" src=\"images/new/info.png\" title=\"View User Details\" onclick=\"viewGroupDetails(\'%s\');\">' %(sid))
                    lis_group.append(tr)
