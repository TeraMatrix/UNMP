...
#	html.req.write(str(JSONEncoder().encode(output_dic)))
    ...
    def all_graph_json(self, device_type_id, user_id):
        ...
        try:
            ...
            for row in result:
                ...
                graph_dict = {'name': row[0], 'displayName': row[1], 'fields': option_list, 'calType': cal_list, 'ajax': ajax_json, 'tabList': {'value': interface_value, 'name': interface_name, 'selected': check_val}, 'type': graph_type, 'otherOption': {'showOption': True if int(row[8]) == 1 else False, 'showRefreshButton': True if int(row[9]) == 1 else False, 'showNextPreButton': True if int(
                    row[10]) == 1 else False, 'width': row[13], 'height': str(row[14]) + "px", 'showType': showType, 'showFields': showFields, 'showCalType': showCalType, 'showTabOption': showTabOption, 'autoRefresh': row[20]}, 'startFrom': row[11], 'itemLimit': row[12]}
    ...
    #        print table_field_dict
            ...
            if table_name == 'ap25_vapClientStatisticsTable':
                ...
                sel_query += column_list + "," + "gp_tab.%s from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' AND gp_tab.timestamp >= '%s' AND gp_tab.timestamp <='%s' and gp_tab.index = '%s' order by gp_tab.timestamp desc" % (x_axis_value, table_name, ip_address, start_date,
                                                                                                                                                                                                                                                                               end_date, interface)
                if int(flag) == 0:
                    limit_data = ''
                else:
    ...
    # heading=[table_output[0][1],table_output[0][2],table_output[0][3],table_output[0][4],table_output[0][5],table_output[0][6],table_output[0][7]]
        ...
        # this is code for bandwidth
            ...
            output_result = self.common_graph_json(
                user_id, table_name, x_axis, limitFlag, start_date, end_date, ip_address, graph1, update_field_name, tab1, cal1, field_column)
            d1_list = []
            headings = [x_axis]
            ...
            output_result = self.common_graph_json(
                user_id, table_name, x_axis, limitFlag, start_date, end_date, ip_address, graph1, update_field_name, tab2, cal2, field_column)
            d1_list = []
            headings = ['VAPS']
            if int(output_result['success']) == 0:
                d1_list.append(output_result['timestamp'])
