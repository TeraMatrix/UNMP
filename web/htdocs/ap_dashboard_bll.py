...
#                    ip_address=''
    ...
    def get_dashboard_data(self):
        ...
        get_data=DashboardConfig.get_config_attributes('ap_dashboard',devcie_type_attr)
    ...
    def ap_interface(self,odu_start_date,odu_start_time,odu_end_date,odu_end_time,interface_value,ip_address,limitFlag):
        rx0=[]
        tx0=[]
        time_stamp0=[]
        try:
...
#	html.req.write(str(JSONEncoder().encode(output_dic)))
    ...
    def ap_device_information(self,ip_address):
        ...
        try:
            ...
            output_dict={'success':0,'result':result,'no_of_uesr':no_of_user}
            return output_dict 
        # Exception Handling
        except MySQLdb as e:
            output_dict={'success':3,'output':str(e[-1])}
            ...
            output_dict={'success':2,'output':'UNMP database Server or Services not running so please contact your Administrator.'}
            return  output_dict
        except Exception as e:
            output_dict={'success':1,'output':str(e[-1])}
            return output_dict
    ...
    def all_graph_json(self,device_type_id,user_id):
        ...
        try:
            ...
            for row in result:
                ...
                query1="SELECT graph_field_value,graph_field_display_name,is_checked FROM graph_field_table WHERE graph_name='%s' and user_id='%s'"%(row[6],user_id)
                cursor.execute(query1)
                ...
                query2="SELECT interface_value,interface_display_name,is_selected FROM  graph_interface_table WHERE graph_name='%s' and user_id='%s'"%(row[6],user_id)
                ...
                query3="SELECT graph_cal_id,graph_cal_name FROM  graph_calculation_table where user_id='%s' and table_name='%s'"%(user_id,row[6])
                ...
                query4="SELECT url,method,other_data FROM  graph_ajax_call_information where user_id='%s' and graph_id='%s'"%(row[2],row[6])
                ...
                graph_dict={'name':row[0],'displayName':row[1],'fields':option_list,'calType':cal_list,'ajax':ajax_json,'tabList': {'value':interface_value,'name':interface_name,'selected':check_val},'type':graph_type,'otherOption':{'showOption':True if int(row[8])==1 else False,'showRefreshButton':True if int(row[9])==1 else False,'showNextPreButton':True if int(row[10])==1 else False,'width':row[13],'height':str(row[14])+"px",'showType': showType,'showFields':showFields,'showCalType':showCalType,'showTabOption':showTabOption,'autoRefresh':row[20]},'startFrom':row[11],'itemLimit':row[12]}

                #graph_dict={'name':row[0],'graphName':row[1],'fields':option_list,'calType':cal_list,'ajax':ajax_json,'tabList': {'value':interface_value,'name':interface_name,'selected':check_val},'type':graph_type,'otherOption':{'graph_tab_option':row[8],'refresh_button':row[9],'showNextPreButton':row[10],'start_value':row[11],'end_value':row[12],'width':row[13],'height':row[14]}}
    ...
    def common_graph_json(self,user_id,table_name,x_axis_value,flag,start_date,end_date,ip_address,graph,update_field_name,interface=1,value_cal=1,*column_name):
        data_list=[]
        time_list=[]
    ...
    #        print table_field_dict
            ...
            if str(update_field_name).strip()=='calType':
                up_query="update  graph_templet_table set graph_cal_id =%s where user_id='%s' and graph_name_field='%s'"%(value_cal,user_id,table_name)
                cursor.execute(up_query)
                conn.commit()
                
            elif str(update_field_name).strip()=='fields':
                up_query="update graph_field_table set is_checked=0 where user_id='%s' and graph_name='%s'"%(user_id,table_name)
                cursor.execute(up_query)
                conn.commit()

                columns="','".join(column_name[0])
                up_query="update graph_field_table set is_checked=1 where user_id='%s' and graph_name='%s' and graph_field_value IN ('%s')"%(user_id,table_name,columns)
                cursor.execute(up_query)
                conn.commit()
            
            elif str(update_field_name).strip()=='type':
                up_query="update  graph_templet_table set graph_id =%s where user_id='%s' and graph_name_field='%s'"%(graph,user_id,table_name)
                cursor.execute(up_query)
                conn.commit()
            ...
            get_coloum="SELECT graph_field_value,graph_field_display_name,tool_tip_title FROM graph_field_table WHERE graph_name='%s' AND user_id='%s'"%(table_name,user_id)
            ...
            sel_query="select graph_title,graph_subtitle from graph_templet_table where graph_name_field='%s' and user_id='%s'"%(table_name,user_id)
            cursor.execute(sel_query)
		...
		time_list=vap_list
                ...
                sel_query+=column_list+","+"gp_tab.%s from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' AND gp_tab.timestamp >= '%s' AND gp_tab.timestamp <='%s' and gp_tab.index = '%s' order by gp_tab.timestamp desc"%(x_axis_value,table_name,ip_address,start_date,end_date,interface)
                if int(flag)==0:
                    limit_data=''
                else:
                   limit_data=' limit 16'
		        ...
		        for k in range(len(result)-1):
		            ...
		            for i  in range(len(result[k])-1):
		                if result[k][i]==1 or str(result[k][i])=='1':
		                    ...
		                    data_list[i].append('0' if result[k][i]==None or result[k][i]=='' else int(result[k][i])-int(result[k+1][i]))
		                #print row[i]
    ...
    def ap_excel_report(self,user_id,ip_address,cal1,cal2,tab1,tab2,field1,field2,table_name1,table_name2,graph_name1,graph_name2,start_date,end_date,select_option,limitFlag,graph1,graph2):
	    ...
	    table_output.append(['No of Connected User',0 if no_of_user[0][0]==None or no_of_user[0][0]=='' else no_of_user[0][0]])
            ...
            xls_sheet.write_merge(1,1,0,2,str('AP25')+'       '+str(ip_address) ,style)
	...
	# this is code for bandwidth
		...
		da=datetime.now()+timedelta(days=-2)
		da1=datetime.now()+timedelta(days=-1)
		start_date=da.strftime('%Y %m %d 00:00:00')
		end_date=da1.strftime('%Y %m %d 24:00:00')
#		start_date=datetime.strptime(date1,'%Y %m %d %H:%M:%S')
#		end_date=datetime.strptime(date2,'%Y %m %d %H:%M:%S')
	    elif int(select_option)==2:	
		da=datetime.now()+timedelta(days=-3)
		da1=datetime.now()+timedelta(days=-1)
		start_date=da.strftime('%Y %m %d 00:00:00')
		end_date=da1.strftime('%Y %m %d 24:00:00')
	    elif int(select_option)==3:	
		da=datetime.now()+timedelta(days=-4)
		da1=datetime.now()+timedelta(days=-1)
		start_date=da.strftime('%Y %m %d 00:00:00')
		end_date=da1.strftime('%Y %m %d 24:00:00')
	    elif int(select_option)==4:
		da=datetime.now()+timedelta(days=-8)
		da1=datetime.now()+timedelta(days=-1)
		start_date=da.strftime('%Y %m %d 00:00:00')
		end_date=da1.strftime('%Y %m %d 24:00:00')

	    output_result=self.common_graph_json(user_id,table_name,x_axis,limitFlag,start_date,end_date,ip_address,graph1,update_field_name,tab1,cal1,field_column)
	    d1_list=[]
	    headings = [x_axis]
	    ...
	    interface=['eth0','eth1','br0','ath1','ath2','ath3','ath4','ath5','ath6','ath7']
            xls_sheet=xls_book.add_sheet('%s'%(interface[int(tab1)]),cell_overwrite_ok=True)
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
            xls_sheet.write_merge(0,0,0,2,"AP Information",style)
            xls_sheet.write_merge(1,1,0,2,str('AP25')+'       '+str(ip_address) ,style)
	    ...
	    output_result=self.common_graph_json(user_id,table_name,x_axis,limitFlag,start_date,end_date,ip_address,graph1,update_field_name,tab2,cal2,field_column)
	    d1_list=[]
	    headings = ['VAPS']
	    if 	int(output_result['success'])==0:
		d1_list.append(output_result['timestamp'])
		...
		merge_result=merge_list(d1_list)
            ...
            xls_sheet=xls_book.add_sheet('Connected Client',cell_overwrite_ok=True)
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
	    ...
	    xls_sheet.set_remove_splits(True) # if user does unfreeze, don't leave a split there
            ...
            for k in range(len(merge_result)):
                for j in range(len(merge_result[k])):
                    ...
                    xls_sheet.col(j).width = width 
                i=i+1
            xls_book.save('/omd/sites/%s/share/check_mk/web/htdocs/download/AP_excel.xls' % nms_instance)
