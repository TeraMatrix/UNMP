...
logging.basicConfig(filename='/omd/daemon/log/error_log.log',format='%(levelname)s: %(asctime)s : %(module)s >> %(message)s', level=logging.DEBUG)
log = logging.getLogger('AP_PROFILING_BLL')
if debug_start>0:
    log.info(" lggin start: ")
...
##            .order_by(Hosts.host_alias).order_by(Hosts.ip_address).all()
    ...
    def get_time_ago(self,date1):
        ...
        date1=datetime.strptime(date1,"%Y-%m-%d %H:%M:%S") # mysql datetime syntax
    ...
    def ap_client_list(self):
        ...
        try:
            ...
            for row in device_list:
                client_list.append([row["state"],row["client_name"],row["mac"],row["client_ip"] != None and row["client_ip"] or "",str(row["total_tx"]),str(row["total_rx"]),row["host_alias2"],str(row["rssi"]),\
                row["ssid"],self.get_time_ago(str(row["last_seen_time"])),row["host_alias2"],"<a href=\"javascript:editClient('%s');\"><img alt=\"edit\" src=\"images/new/edit.png\" class=\"host_opr\" title=\"Edit Client Details\"/></a>&nbsp;<a href=\"client_dashboard_profiling.py?client_mac=%s&device_type=%s\"><img alt=\"Performance Monitoring\" src=\"images/new/graph.png\" class=\"host_opr\" title=\"Client Performance Monitoring\"/></a>"%(row["client_id"],row["mac"],"ap25")\
                ])
...
##                    s_where += "%s LIKE '%%%s%%' " % (a_columns[i],MySQLdb.escape_string(s_search_i))
            ...
            # SQL queries
            # Get data to display
            sql_query = "SELECT SQL_CALC_FOUND_ROWS %s FROM %s %s %s %s %s" % (", ".join(a_columns),s_table,s_join,s_where,s_order,s_limit)
            #return sql_query
            ...
            sql_query = "SELECT COUNT(%s) FROM %s %s %s " % (s_index_column,s_table,s_join,s_where)
                ...
                if len(host_param)>0:
                    ...
                    if result["success"]==0:
                        ...
                        if result["success"]==0:
                            time.sleep(20)
                            while(i<5):
                                get_result = pysnmp_get('1.3.6.1.4.1.26149.10.5.3',host_param[0].ip_address,int(host_param[0].snmp_port),'public')
                            ...
                            if k != 553 and k != '553':                                    
                                if errorStatus.has_key(k):
                                    ...
                                    try:
                                	result = {"success":1,"result":result['result'].get(oid_dict[1][0],' Unknown snmp error , Please try again later')}
                                    except:
                                	result = {"success":1,"result":" Unknown snmp error , Please try again later"}
                            else:
                                result = {'success':1,'result':'Device SNMP agent is Not Responding'}
		    else:
                    	k = result['result'].keys()[0]
                        if k != 553 and k != '553':                                    
                            if errorStatus.has_key(k):
        ...
        try:
            ...
            if int(host_op_state)==0:
                if len(host_param)>0:
                    result = single_set(host_param[0].ip_address,int(host_param[0].snmp_port),host_param[0].snmp_write_community,oid_dict[1])
                    if result["success"]==0:
                    	result = {"success":0,"result":"Configuration data saved permanently in the device"} # check
                    	return
                    else:
                    	k = result['result'].keys()[0]
                        if k != 553 and k != '553':                                    
...
##            sqlalche_obj.sql_alchemy_db_connection_close()
    ...
    def update_configuration(self,host_id,device_type,table_prefix,current_time,user_name):
        ...
        try:
                ...
                if host_op_status!=None or host_op_status!="":
                    ...
                    if int(host_op_status)==0:
                        ...
                        if len(host_data)>0:
                            ...
                            if debug_start > 0:
                                log.info(" ping_chk: "+str(ping_chk))
                            if int(ping_chk)==0:
                                ...
                                # mac address updation in table
                                    try:
                                        ...
                                        get_result=pysnmp_geter(oid_dic,host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community)
					if get_result['success']==0:
					    get_result = get_result['result']
					    sqlalche_obj.db.execute("Update hosts set mac_address = '%s' where ip_address='%s'"%(get_result['mac_address'],\
				    ...
				    # mac address updation
                                    ...
                                    table_result = sqlalche_obj.session.query(Ap25Oid_table).filter(Ap25Oid_table.is_recon==1).all()
                                    print table_result
                                    if debug_start > 0:
                                        log.info(" table_result: "+str(table_result))
                                if len(table_result)>0:
                                    ...
                                    if len(rec_table_dic)>0:
                                        ...
                                        for j in rec_table_dic:
                                            ...
                                            if debug_start > 0:
                                                log.info(" column_list: "+str(column_list))
                                            if debug_start > 0:
                                                log.info(" database_name: "+str(database_name))
                                            time.sleep(5)
                                            
                                            result = bulktable(rec_table_dic[j],host_data[0].ip_address,int(host_data[0].snmp_port),host_data[0].snmp_read_community,int(table_var_bind.get(j)))
                                            if debug_start > 0:
                                                log.info(" resut: "+str(result))
                                            if result['success']==1:
                                                ...
                                                if len(result['result'])==0:
                                                    ...
                                                    rec = rec+1
                                                    if debug_start > 0:
                                                        log.info(" sqlalchetablename \n\n",str(sqlalche_tablename))
                                                    
                                                    if "config_profile_id" in column_list:
                                                        if debug_start > 0:
                                                            log.info(" debug isi mein : ")
                                                        table_form = "123"
                                                        table_form = eval(sqlalche_tablename)
                                                        ...
                                                        if debug_start > 0:
                                                            log.info(" sqlalche_table_result : "+str(sqlalche_table_result))
                                        ...
                                        if len(rec_not_done)>0:
                                            sqlalche_obj.db.execute("Update ap25_oid_table set status = 0 where table_name in ('%s')"%("\',\'".join(rec_not_done)))
                                        
                                        
                                        if len(rec_done)>0:
                                            a = str(current_time)
                                            sqlalche_obj.db.execute("Update ap25_oid_table set status = 1,timestamp='%s' where table_name in ('%s')"%(a[:a.find('.')-1],"\',\'".join(rec_done)))
                        ...
                        result = {'success':1,'result':"Device is busy, Device "+host_status_dic.get(int(host_op_status),'Other Operation')+" is in progress. please wait ..."}
                        return
                else:
            ...
            result = {"success":1,"result":"UNMP Server is busy.Please try after some time %s"%str(e)}
            return 
        finally:
            if chcek_flag==1:
    ...
    def default_configuration_added(self,host_id,device_type_id,table_prefix,current_time,user_name,reconcile_chk=True):
        ...
        if len(default_profile_id)==0:
            ...
            #print "hello12"    
            basicvapsetup_data = sqlalche_obj.session.query(Ap25BasicVAPconfigTable).filter(Ap25BasicVAPconfigTable.config_profile_id == default_profile_id[0].config_profile_id).all()   
            if len(basicvapsetup_data)>0:
                for i in range(0,len(basicvapsetup_data)):
                    basicvapsetup_add_row = Ap25BasicVAPconfigTable(new_profile_id,vap_selection_id[i],basicvapsetup_data[i].vapESSID,\
            ...
            #print "hello123"    
            wepsecurity_data = sqlalche_obj.session.query(Ap25VapWEPsecurityConfigTable).filter(Ap25VapWEPsecurityConfigTable.config_profile_id == default_profile_id[0].config_profile_id).all()
            if len(wepsecurity_data)>0:
                for i in range(0,len(wepsecurity_data)):
                    wepsecurity_add_row = Ap25VapWEPsecurityConfigTable(new_profile_id,wepsecurity_data[i].vapWEPmode,1,\
            ...
            #print "hello15"        
            basicacl_data = sqlalche_obj.session.query(Ap25BasicACLconfigTable).filter(Ap25BasicACLconfigTable.config_profile_id == default_profile_id[0].config_profile_id).all()  
            if len(basicacl_data)>0:
                for i in range(0,len(basicacl_data)):
                    basicacl_add_row = Ap25BasicACLconfigTable(new_profile_id,vap_selection_id[i],basicacl_data[i].aclState,basicacl_data[i].aclMode)
            ...
            if len(radiosetup_data)>0:
                for i in range(0,len(radiosetup_data)):
                    radiosetup_row_add = Ap25RadioSetup(new_profile_id,radiosetup_data[i].radioState,radiosetup_data[i].radioAPmode,\
                                                        radiosetup_data[i].radioManagementVLANstate,radiosetup_data[i].radioCountryCode,radiosetup_data[i].numberOfVAPs,\
            ...
            #print "hello90"
            wpa_data = sqlalche_obj.session.query(Ap25VapWPAsecurityConfigTable).filter(Ap25VapWPAsecurityConfigTable.config_profile_id == default_profile_id[0].config_profile_id).all()
            if len(wpa_data)>0:
                for i in range(0,len(wpa_data)):
                    wpa_add_row = Ap25VapWPAsecurityConfigTable(new_profile_id,vap_selection_id[i],wpa_data[i].vapWPAmode,wpa_data[i].vapWPAcypher,\
    ...
    def reconciliation_chk_status(self,host_id):
        ...
        if host_id!="" or host_id!=None:
            ...
            if reconcile_status[0].reconcile_status == 2:
                ...
                sqlalche_obj.session.commit()
            sqlalche_obj.sql_alchemy_db_connection_close()
            result = {"result":[status,reconcile_status[0].reconcile_health,op_img,0 if op_status==None else op_status],"success":0}
            return result             
        else:
            sqlalche_obj.sql_alchemy_db_connection_close()
        ...
        try:
            ...
            if int(host_op_state)==0:
                try:
                    ...
                    #sqlalche_obj.session.flush()
                    if int(calculate)==0:
                        host_param = sqlalche_obj.session.query(Ap25DhcpClientsTable.ap25_dhcpClientsTable_id,Ap25DhcpClientsTable.dhcpClientsMACaddress,Ap25DhcpClientsTable.dhcpClientsIPaddress,Ap25DhcpClientsTable.dhcpClientsExpiresIn,Ap25DhcpClientsTable.timestamp).filter(Hosts.host_id==host_id).all()
                        result['success']=0
                        temp_dict={}
                        for i in range(len(host_param)):
                            temp_dict[i+1]=[host_param[i].dhcpClientsMACaddress,host_param[i].dhcpClientsIPaddress,host_param[i].dhcpClientsExpiresIn,host_param[i].timestamp]
                        result['result']=temp_dict
                        return result
                    else:
                        host_param = sqlalche_obj.session.query(Hosts.ip_address,Hosts.snmp_port,Hosts.snmp_write_community,Hosts.snmp_read_community).filter(Hosts.host_id==host_id).all()
                        if len(host_param)>0:
                            client_table = sqlalche_obj.session.query(Ap25DhcpClientsTable).filter(Ap25DhcpClientsTable.host_id==host_id).all()
                            for client in client_table:
                                 sqlalche_obj.session.delete(client)
                            sqlalche_obj.session.commit()
                            result = bulktable('1.3.6.1.4.1.26149.10.1.3.2.1.1',host_param[0].ip_address,int(host_param[0].snmp_port),host_param[0].snmp_read_community)
                            if result["success"]==0:
                                for k in result['result']:
                                    result["result"][k].append(datetime.now().strftime('%Y %m %d %H:%M:%S'))
                                    dhcp_client_data = Ap25DhcpClientsTable(host_id,result['result'][k][0],result['result'][k][1],result['result'][k][2])
        ...
        try:
            ...
            if int(host_op_state)==0:
                try:
                    ...
                    #sqlalche_obj.session.flush()
                    if int(calculate)==0:
                        host_param = sqlalche_obj.session.query(Ap25ApScanDataTable.macAddress,Ap25ApScanDataTable.essid,Ap25ApScanDataTable.frequency,Ap25ApScanDataTable.quality,\
                        ...
                        temp_dict={}
                        for i in range(len(host_param)):
                            temp_dict[i+1]=[str(i),host_param[i].macAddress,host_param[i].essid,host_param[i].frequency,host_param[i].quality,host_param[i].signalLevel,host_param[i].noiseLevel,host_param[i].beconIntervel,str(host_param[i].Timestamp)]
                        result['result']=temp_dict
                        return result
                    else:
                        host_param = sqlalche_obj.session.query(Hosts.ip_address,Hosts.snmp_port,Hosts.snmp_write_community,Hosts.snmp_read_community).filter(Hosts.host_id==host_id).all()
                        if len(host_param)>0:
                            scan_table = sqlalche_obj.session.query(Ap25ApScanDataTable).filter(Ap25ApScanDataTable.host_id==host_id).all()
                            for scan_row in scan_table:
                                 sqlalche_obj.session.delete(scan_row)
                            sqlalche_obj.session.commit()
                            result = bulktable('1.3.6.1.4.1.26149.10.4.5.1.1.1',host_param[0].ip_address,int(host_param[0].snmp_port),host_param[0].snmp_read_community)
                            if result["success"]==0:
                                for k in result["result"]:
...
class APCommonSetValidation(object):
            ...
            if int(host_op_state)==0:
                ...
                if(j == -1):
                    if len(depend_oid_value)>0:
                        ...
                        #return {'success':1,'reew':str(result),'dic_oid':dic_oid}
                        if result["success"] == 0 or result["success"] == '0':
                            success_result["success"] = result["success"]
                            for i in result["result"]:
                                if result["result"][i] != 0:
                                    ...
                                    oid_list_table_field_value = sqlalche_obj.session.query(Ap25Oids.table_name,Ap25Oids.coloumn_name).filter(and_(Ap25Oids.oid_name == i,Ap25Oids.device_type_id == device_type_id)).all()
                                    oid_list_table_field_value1 = sqlalche_obj.session.query(Ap25Oids.table_name,Ap25Oids.coloumn_name).filter(and_(Ap25Oids.oid_name == i,Ap25Oids.device_type_id == device_type_id))
                                     #if i=='radioSetup.radioCountryCode':
                                     #   return {'success':1,'devi':str(oid_list_table_field_value),'qy':str(oid_list_table_field_value1)}
                                    if len(oid_list_table_field_value) == 0:
                                        continue
                                    else:
                                        if i in dic_result:
                                            ...
                                            #return {'success':1,'res':str(table_result)}
                                            if len(table_result)>0:
            ...
            if int(host_op_state)==0:
                ...
                for keys in dic_result.iterkeys():
                    if keys == "success" or keys == "vap_selection_id":
                        ...
                        query_result = sqlalche_obj.session.query(o2,o1.oid_name,o1.oid,o1.indexes).outerjoin(o1,o1.oid_id == o2.dependent_id).filter(and_(o2.oid_name == keys,o2.device_type_id == device_type_id)).all()
                        if len(query_result)>0:
                            independent_oid.append({keys:[query_result[0][0].oid + str(".")+str(dic_result["vapSelection.selectVap"]),query_result[0][0].oid_type,dic_result[keys]]})
                ...
                result = pysnmp_seter(basic_acl,host_data[0].ip_address,host_data[0].snmp_port,host_data[0].snmp_write_community)
    ...
    ##                                        select_vap_data[j].selectVap = dic_result[i]
                                    ...
                                    basic_acl_data = sqlalche_obj.session.query(Ap25BasicACLconfigTable).filter(and_(Ap25BasicACLconfigTable.vapselection_id==dic_result['vap_selection_id'],Ap25BasicACLconfigTable.config_profile_id==host_data[0].config_profile_id)).all()
                    ...
                    if 53 in result["result"] or 553 in result["result"]:
                        ...
                        for i in result['result']:
                            result = {"success":1,"result":errorStatus.get(result['result'][i],"Device is busy.Please try again later")}
                return result
            else:
                return {"success":1,"result":"Device is busy, Device "+str(host_status_dic.get(int(host_op_state),"other operation"))+" is in progress. please wait ..."}
    ...
    def vap_set(self,host_id,device_type_id,dic_result,selectedvap,vap_id):
        try:
            ...
            if int(host_op_state)==0:
                    ...
                    if keys == "success":
                        ...
                        if len(query_result)>0:
                            ...
                            or keys=="vapWEPsecurityConfigTable.vapWEPkey3" or keys=="vapWEPsecurityConfigTable.vapWEPkey4" or keys=="vapWEPsecurityConfigTable.vapWEPprimaryKey":
                                independent_oid.append({keys:[str(query_result[0][0].oid) + str(query_result[0][0].indexes),query_result[0][0].oid_type,key]})
                            else:
                                independent_oid.append({keys:[str(query_result[0][0].oid) + "."+str(selectedvap),query_result[0][0].oid_type,key]})
                ...
                result = pysnmp_seter(vap_data,host_data[0].ip_address,host_data[0].snmp_port,host_data[0].snmp_write_community)
                        ...
                        if result["result"][i] != 0:
                            ...
                            if i in dic_result:
                                ...
                                if i == "vapWEPsecurityConfigTable.vapWEPmode" or i=="vapWEPsecurityConfigTable.vapWEPkey1" or i=="vapWEPsecurityConfigTable.vapWEPkey2" or i=="vapWEPsecurityConfigTable.vapWEPkey3" or i=="vapWEPsecurityConfigTable.vapWEPkey4" or i=="vapWEPsecurityConfigTable.vapWEPprimaryKey":
                                    ...
                                    exec "table_result=sqlalche_obj.session.query(%s).filter(and_(%s.vapselection_id=='%s',%s.config_profile_id=='%s')).all()"%(sql_table_name,sql_table_name,vap_id,sql_table_name,host_data[0].config_profile_id)
                                    exec "table_result[0].%s='%s'"%(oid_list_table_field_value[0].coloumn_name,dic_result[i])    
...
class SelectVap(object):
    ...
    def select_vap_vap(self,host_id,device_type):
        ...
        result_data = sqlalche_obj.session.query(Ap25BasicACLconfigTable.aclState,Ap25BasicACLconfigTable.aclMode,Ap25BasicACLconfigTable.vapselection_id).filter(Ap25BasicACLconfigTable.config_profile_id==host_data[0].config_profile_id).all()
    ...
    def select_vap_change(self,host_id,device_type):
        ...
        host_data = sqlalche_obj.session.query(Hosts.config_profile_id).filter(Hosts.host_id == host_id).all()
        result_basic_vap_data = sqlalche_obj.session.query(Ap25BasicVAPconfigTable.vapselection_id,Ap25BasicVAPconfigTable.vapESSID,Ap25BasicVAPconfigTable.vapHiddenESSIDstate,\
        ...
        result_wep_security_data = sqlalche_obj.session.query(Ap25VapWEPsecurityConfigTable.vapWEPmode,\
        ...
        result_wpa_security_data = sqlalche_obj.session.query(Ap25VapWPAsecurityConfigTable.vapWPAmode,\
        ...
        result_basic_vap_security_mode = sqlalche_obj.session.query(Ap25BasicVAPconfigTable.vapSecurityMode,Ap25BasicVAPconfigTable.vapMode,\
            ...
            try:
                ...
                for i in dic_result:
                    ...
                    for j in range(0,len(dic_result[i])): 
                        result = pysnmp_seter({i:['1.3.6.1.4.1.26149.10.2.3.4.1.1.4.'+str(selected_vap),'OctetString',dic_result[i][j]]},host_data[0].ip_address,host_data[0].snmp_port,host_data[0].snmp_write_community)
                        #result = pysnmp_seter({i:['1.3.6.1.4.1.26149.10.2.3.4.1.1.4.1','OctetString',dic_result[i][j]]},host_data[0].ip_address,host_data[0].snmp_port,host_data[0].snmp_write_community)
                        if result["success"] == 0 or result["success"] == '0':
                            for k in result["result"]:
                                if k=='vap_select':
                                    continue
                                if result["result"][k] != 0:                            
                                    errorIndex = 1
                                    result["result"][i] = errorStatus.get(result["result"][i],'undefied snmp error')
                                    if len(mac_length)>0:
        ...
        if int(host_op_state)==0:
            try:
                ...
                for i in dic_result:
                    ...
                    mac_length = len(dic_result[i])
                    for j in range(0,len(dic_result[i])):
                        result = pysnmp_seter({i:['1.3.6.1.4.1.26149.10.2.3.4.1.1.5.'+str(selected_vap),'OctetString',dic_result[i][j]]},host_data[0].ip_address,host_data[0].snmp_port,host_data[0].snmp_write_community)
                        if result["success"] == 0 or result["success"] == '0':
                            for k in result["result"]:
                                if k=='vap_select':
                                    continue
                            ...
                            if 53 in result["result"] or 553 in result["result"]:
                                ...
                                final_result = {"success":1,"result":"%s"%(errorStatus)}
                            break
                    sqlalche_obj.sql_alchemy_db_connection_close()
                    if errorIndex == 1:
                        return result
    ...
    def delete_all_mac(self,host_id,device_type_id,selected_vap,vap_selection_id):
        try:
            ...
            if int(host_op_state)==0:
                try:
                    host_data = sqlalche_obj.session.query(Hosts).filter(Hosts.host_id == host_id).all()
                    result = pysnmp_seter({'delete_mac':['1.3.6.1.4.1.26149.10.2.3.4.1.1.6.'+str(selected_vap),'Integer32',1]},host_data[0].ip_address,host_data[0].snmp_port,host_data[0].snmp_write_community)
                    if result["success"] == 0 or result["success"] == '0':
                        for k in result["result"]:
                            if k=='vap_select':
                                continue
                            if result["result"][k] != 0:
                                result["result"][k] = errorStatus.get(result["result"][k],'undefied snmp error')
                                errorIndex = 1
                            else:
                ...
                snmp_result={"success":1,"result":"Device is busy, Device "+str(host_status_dic.get(int(host_op_state),"other operation"))+" is in progress. please wait ..."}
        except Exception as e:
    ...
    def radio_enable_disable(self,host_id,admin_state,state):
        ...
        try:
            ...
            if int(host_op_state)==0:
                ...
                snmp_result={"success":1,"result":"Device is busy, Device "+str(host_status_dic.get(int(host_op_state),"other operation"))+" is in progress. please wait ...",'image':'images/host_status1.png'}
        except:
            ...
            if host_id==None or host_id == "" or host_id=="undefined":
                ...
                for i in range(0,len(host_id_list)):
                    ...
                    if len(host_data_list)>0:
                        radio_state = sqlalche_obj.session.query(Ap25RadioSetup.radioState,Ap25RadioSetup.radioAPmode).filter(Ap25RadioSetup.config_profile_id==host_data_list[0].config_profile_id).all()
                        radio_status.update({host_id_list[i]:[radio_state[0].radioState if len(radio_state)>0 else "",radio_state[0].radioAPmode if len(radio_state)>0 else "",op_img,0 if op_status==None else op_status]})
...
class APConnectedClients(object):
    def total_connected_clients(self,host_id):
        try:
            global sqlalche_obj
            sqlalche_obj.sql_alchemy_db_connection_open()
            host_data = sqlalche_obj.session.query(Hosts.ip_address).filter(Hosts.host_id==host_id).all()            
            if len(host_data)>0:
                ip_address = host_data[0].ip_address
            result = {'success':0,'result':""}
            flag = 1
            connected_clients = 1
            connected_clients = sqlalche_obj.db.execute("SELECT count(*) as client,host_id FROM ap_connected_client WHERE host_id='%s' and state='1'"%(host_id))
    ...
    def client_details(self,client_id):
        try:
            ...
            sqlalche_obj.sql_alchemy_db_connection_open()
            client_data = sqlalche_obj.session.query(ApClient_details.client_name,ApClient_details.mac,ApClient_details.client_ip).filter(ApClient_details.client_id==client_id).all()
    ...
    def edit_ap_client_details(self,client_id,client_name,client_ip):
        try:
            global sqlalche_obj
            result = {"success":1,"result":"UNMP Server is busy, try again later."}
            sqlalche_obj.sql_alchemy_db_connection_open()
            client_data = sqlalche_obj.session.query(ApClient_details).filter(ApClient_details.client_id==client_id).all()
            if len(client_data) > 0:
                ...
                result["result"] = "Client Deatils can't update. please try again later."
        except Exception as e:
            result['success']=1
            result['result'] = str(e)
        finally:
