import shelve, datetime , os , tarfile
# NMS Instance
nms_instance = __file__.split("/")[3] 
#nms_instance = "nms"   
# Nagios path & path of shelve files
nagios_path = "/omd/sites/%s/etc/nagios/conf.d/"%(nms_instance)
db_path = "/omd/sites/%s/share/check_mk/web/htdocs/download/nag_db/"%(nms_instance)
# Path of .cfg files & tar files with log
file_folder_to_be_backed = "/omd/sites/%s/etc/nagios/conf.d/"%(nms_instance)
backup_store_path = "/omd/sites/%s/share/check_mk/web/htdocs/download/nag_db/"%(nms_instance)
backup_log = "/omd/sites/%s/share/check_mk/web/htdocs/download/nag_db/backup_log.log"%(nms_instance)
# Extraction path of tar files
tarfile_path = "/omd/sites/%s/share/check_mk/web/htdocs/download/nag_db/"%(nms_instance) 
...
#filepath_dict={"host_cfg" :"/omd/sites/nms/etc/nagios/conf.d/check_mk_templates.cfg"               }

dicts_name=["command","contact","contactgroup","host","hosttemplate","hostdependency",
...
count_maintain_list = ["service","servicedependency","hostdependency" , "hostescalation", "serviceescalation","hostextinfo","serviceextinfo"]
...
def create_backup(comment = "",flag_write_to_log_file = True):
    try:
        if flag_write_to_log_file:
            ...
            out.add(file_folder_to_be_backed, arcname=os.path.basename(file_folder_to_be_backed))
            out.close()
...
def restore_backup(file_name):
    global tarfile_path
    try:
        comment="Backup before config restore at time : %s"%(str(datetime.datetime.now())[:22])
...
def store_file_modification_time(file_name=""):
    try:
        ...
        if new_stat!={} and new_stat.has_key("data"):
            stats = new_stat["data"]
...
def check_files_if_modified(file_name=""):
    ...
    try:
        ...
        if old_stat!={} and old_stat.has_key("data"):
            stats = old_stat["data"]
            if stats!={} and stats.has_key(file_name):
    ...
    try:
        for key in source_dict:
            if source_dict[key]=="r": # required value
                if not check_dict.has_key(key): # its not present in check_dict
                    return key+" value not present in input file %s"%(dict_name)
        for key in check_dict:        # check validity of keys in check_dict
            if not source_dict.has_key(key): # its not present in source_dict
...
#                li.append(line)
            ...
            while(i<len(li)):
                ...
                if li[i].startswith('define'):        # li[i] = define host{
                    dict_name = li[i].split()[1]      # get the name of definition
                    dict_name = dict_name.replace('{','')# replace any { if present
                    di_data={}                          # initialize data dict
                    while(j<len(li) and li[j].find('}')==-1):# run till we reach end of file or find }
                        if li[j].strip()=="{":
                            j=j+1
                            continue
                        data = li[j].split()       
                        di_data[data[0]] = ' '.join(data[1:]) # make the key value pair
                        j=j+1
                    if j==len(li):
                        raise Exception("Invalid configuration file.. invalid open & closing braces")
                i=j+1
                if di_data.has_key("name"):         # its a template definition for host or service
...
#                        else:
                ...
                if dict_name not in exception_cfg and verify_configuration:
                    check_valid_config_file = check_valid_template(eval(dict_name+"_dict"),di_data,dict_name)
...
#                        else:
        ...
        stats = str(time_var.seconds)+"."+str(time_var.microseconds)+" seconds"   
        count_list = [1,1,1,1,1,1,1] 
...
def load_db_by_name(dict_name,is_cfg=0):
    # get a specific .cfg file definition from shelve... no need of parsing the .cfg files as we already have them in shelve
    try:
        if is_cfg:
...
def get_service_by_name(service_name):
    try:
        ...
        di = load_db_by_name(dict_name)
        if di["success"]==0:
            if di["data"].has_key(service_name):
...
def get_hostgroup_by_name(hostgroup_name):
    try:
        ...
        di = load_db_by_name(dict_name)
        if di["success"]==0:
            if di["data"].has_key(hostgroup_name):
...
def get_attribute_by_name(attribute_name, attribute):
    try:
        ...
        if di["success"]==0:
            if di["data"].has_key(attribute_name):
...
def delete_attribute_by_name_from_shelve( attribute , unique_name):
    ...
    try:
        ...
        if di.has_key(unique_name):
            di.pop(unique_name)
        if di_eval.has_key(unique_name):
...
# write nagios config files(.cfg) from memory of a given attribute like 'host','service'       
def write_config_from_memory(attribute):
    # write objects to .cfg files from memory after modification to any attributes
    try:
        ...
        for data in attribute_dict:
            ...
            for key in attribute_dict[data]:
                write_li.append("\t%s \t %s{"%(key,attribute_dict[data][key]))
            write_li.append("}")
        cfg_file.close()
        store_file_modification_time(filepath_dict["%s_cfg"%(attribute)])
...
# write nagios config files(.cfg) by loading from shelve file of a given attribute like 'host','service'          
...
#                li.append(line)
            ...
            while(i<len(li)):
                ...
                if di_data.has_key("name"):         # its a template definition for host or service
                    ...
                    #    definition_name = definition_name + "_description"
                    if definition_name=="service":
                        definition_name_service=di_data["service_description"]+di_data["host_name"]
                    else:
                        definition_name = definition_name + "_name"
                dict_name=dict_names[file_name]
                if definition_name=="service":
...
def edit_for_inventory_object(unique_name="localhost",file_name="hosts.cfg",attribute="host",dict_name="host",new_host_dict={},
...
def edit_for_inventory_object_without_unique(other_dict={},file_name="hosts.cfg",attribute="host",dict_name="host",
...
def delete_for_inventory_object_without_unique_name(unique_attribute="host_name",unique_name="host3",file_name="hosts.cfg",attribute="host",dict_name="host",replace_dict={},comment = "nagios backup"):
    ...
    try:
        ...
        for key in di.keys():
            di_data = di[key]
            if di_data.has_key(unique_attribute):
                li_data=[i.strip() for i in di_data[unique_attribute].split(',')]
