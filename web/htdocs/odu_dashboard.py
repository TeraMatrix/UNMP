...
def odu_dashboard_page(h):
	...
	javascript_list = ["js/highcharts.js","js/oduDashboard.js","js/common.js"]
...
def odu_network_interface_graph(h):
	...
	try:
		...
		cursor.callproc("odu_interface_graph",(int(start),(int(start)+int(limit)),interface_value))
...
def odu_tdd_mac_error(h):
	...
	try:
		...
		if db ==1:
			raise SelfException(cursor)
		cursor.callproc("odu_tdd_mac_error",(int(start),(int(start)+int(limit))))
		result=cursor.fetchall()
				...
				if len(result[0])>1 and len(result)>1:
					for i in range(5):
						if i*2+1 >= len(result):
							...
							crc_error.append(int(result[i*2][0]))#-int(result[2*i+1][0]))
							phy_error.append(int(result[i*2][1]))#-int(result[2*i+1][1])) ## Doubt (Peeyush) Arithmatic Operation on wron values for Delta
...
def odu_peer_node_signal(h):
	...
	try:
		...
		cursor.callproc("odu_peer_node_signal",(int(start),(int(start)+int(limit)),interface_value))
		result=cursor.fetchall()
		if result is not None:
			if len(result)>0:
				if len(result[0])>1:
...
def odu_synslost_counter(h):
	...
	try:
		...
		cursor.callproc("odu_sync_lost_counter",(int(start),(int(start)+int(limit))))
		result=cursor.fetchall()
                cursor.close()
                db.close()
		if result is not None:
				...
				if len(result[0])>1 and len(result)>1:
					...
					for i in range(5):
						if i*2+1 >= len(result):
							...
							sync_lost.append(int(result[i*2][0]))#-int(result[i*2+1][0])) ## Doubt (Peeyush) Why is there an Aritmatic operation being performed here ?
...
#    result_dic_bll=rpt_bll.get_total_data_network_outage(5,date1,date2,time1,time2,all_group,all_host)
    ...
    try:
        ...
        if db ==1:
            raise SelfException(cursor)
	sel_query="SELECT ip_address FROM hosts WHERE device_type_id='odu16' order by ip_address limit %s,%s"%(start,limit)
...
def odu_common_dashboard_report_generating(h):
    ...
    try:
        ...
        data=[]
        data.append(['UBR Common Dashboard',(str(datetime.now().strftime('%d %b %Y'))+'-'+str(datetime.now().strftime('%d %b %Y')))])
	...
	odu_common_report.append(Spacer(21,21))
        ...
        crc_error=[]
        table_output=[]
	netwok_table_name=['UBR Network Bandwidth(eth0)','UBR Network Bandwidth(eth1)','UBR Network Bandwidth(br0)']
	# prepare SQL query to get total number of access points in this system
	cursor.callproc("odu_interface_graph",(int(nw_start),(int(nw_start)+int(nw_end)),nw_interface))
	result=cursor.fetchall()
	# function calling
	...
	cursor=db.cursor()
        # prepare SQL query to get total number of access points in this system
	cursor.callproc("odu_tdd_mac_error",(int(error_start),(int(error_start)+int(error_end))))
        result=cursor.fetchall()
	# function calling
        #table_output=table_list_creation(result1,'ODU Crc//Phy Error','Crc Error(error count)','Phy Error(error count)')
        # close the database and cursor connection.
	table_output=[]
	table_output.append(['UBR Devices','Crc Error(error count)','Phy Error(error count)'])
        if result is not None:
            if len(result)>0:
                if len(result[0])>1 and len(result)>1:
                    for i in range(5):
	...
	for i in range(len(odu_name)):
        ...
        cursor.close()
	data1=[]
	data1.append(['UBR Crc/Phy Error Graph','',''])
        t = Table(data1, [1.51 * inch, 2.8 * inch, 2.77*inch])
	t.setStyle(TableStyle([('BACKGROUND',(0,0),(0,0),(0.35,0.35,0.35)),('FONT', (0, 0), (1, 0), 'Helvetica', 11),('TEXTCOLOR',(0,0),(0,0),colors.white)]))
	odu_common_report.append(t)
        data= table_output
        t = Table(data, [2.7 * inch, 2.2 * inch, 2.2*inch])
	t.setStyle(TableStyle([('FONT', (0, 0), (2, 0), 'Helvetica-Bold', 10),        
	...
	cursor=db.cursor()
        # prepare SQL query to get total number of access points in this system
	cursor.callproc("odu_peer_node_signal",(int(ss_start),(int(ss_start)+int(ss_end)),ss_interface))
	...
	# function calling
        ...
        cursor.close()
	data1=[]
	data1.append(['','UBR RSSI'+str(signal_interface[int(ss_interface)]),'',''])
	...
	cursor=db.cursor()
        # prepare SQL query to get total number of access points in this system
	cursor.callproc("odu_sync_lost_counter",(int(sync_start),(int(sync_start)+int(sync_end))))
        result=cursor.fetchall()
	# function calling
	...
	# function calling
        #table_output=table_list_signal_sync(result1,'ODU Sync Lost','Sync Lost')
	table_output.append(['UBR Devices','Sync Lost'])
