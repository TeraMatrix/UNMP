...
def odu_outage_graph(h):
    ...
    try:
        ...
        output_dict=ubr_outage_calculation(current_datetime,last_datetime,ip_address_list)
        cursor.close()
        db.close()
        html.write(str(output_dict))
    # Exception Handling Part
...
def odu_common_dashboard_report_generating(h):
    ...
    try:
        ...
        netwok_table_name=['UBR Network Bandwidth(eth0)','UBR Network Bandwidth(br0)','UBR Network Bandwidth(eth1)']
        ...
        if length>0:
            if len(result[0])>1:
                for i in range(int(length)):
                    table_output.append([(result[i*2+1][2]),int(result[i*2][0])-int(result[i*2+1][0]),int(result[i*2][1])-int(result[i*2+1][1])])
        ...
        cursor.close()
        data1=[]
        data1.append(['','UBR Signal Strength'+str(signal_interface[int(ss_interface)]),'',''])
        ...
        if length>0:
            if len(result[0])>1:
                for i in range(int(length)):
                    table_output.append([(result[i*2+1][1]),int(result[i*2][0])-int(result[i*2+1][0])])
        # close the database and cursor connection.
        cursor.close()
        data1=[]
        ...
        current_datetime=datetime.strptime(str(current_date)+" 00:00:00",'%Y-%m-%d %H:%M:%S') # convert the string in  datetime. 
        last_datetime=datetime.strptime(str(current_date)+" 23:59:59",'%Y-%m-%d %H:%M:%S')
        sel_query="SELECT ip_address FROM hosts WHERE device_type_id='odu16' order by ip_address limit %s,%s"%(int(start),int(limit))
        cursor.execute(sel_query)
        ip_result=cursor.fetchall()
        for ip in ip_result:
            ip_address_list.append(ip[0])
        ...
        if int(output_dict['success'])==1:
            ...
            table_output=outage_graph_generation(output_dict['odu_name'],output_dict['down_state'],output_dict['up_state'])
        data1=[]
