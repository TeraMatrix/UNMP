...
def start_page(h):
    ...
    #"css/demo_table_jui.css
        ...
        html.write("<td style=\"width:60px;\">")
        ...
        html.write("<td style=\"width:70px;\">")
        ...
        html.write("<td style=\"width:90px;\">")
        ...
        html.write("<img src=\"images/lb.png\"  name=\"select_option_div\" alt=\"Informational\" title=\"Informational\" style=\"width:12px\" class=\"imgbutton\" /><span style=\"line-height:20px; padding:0px 12px;cursor:pointer;cursor:hand;\">Informational</span>")
        html.write("</div>")
        html.write("</td>")
...
def trap_filter_function(h):
    ...
    if flag_value == 0:
        ...
        try:
            ...
            device_type_dict  = dict((row[0],row[1]) for row in device_type_result)
            ...
            s_where= "WHERE  hostgroups.hostgroup_id IN (%s) and %s AND ta.timestamp >= '%s' and  ta.timestamp <='%s'"%(','.join(hostgroup_id_list),sql,start_date,end_date)
            if agent_id == "" or agent_id == None:
                s_where="WHERE  hostgroups.hostgroup_id IN (%s) and  %s AND ta.timestamp >= '%s' and  ta.timestamp <='%s'"%(','.join(hostgroup_id_list),sql,start_date,end_date)
            else:
                if "-" in agent_id:
                    s_where="WHERE  hostgroups.hostgroup_id IN (%s) AND agent_id='%s' AND %s AND ta.timestamp >= '%s' and  ta.timestamp <='%s'"%(','.join(hostgroup_id_list),(agent_id.replace("-","")),sql,start_date,end_date)
                else:
                    s_where="WHERE hostgroups.hostgroup_id IN (%s) AND agent_id like  '%s%%' AND %s AND ta.timestamp >= '%s' and  ta.timestamp <='%s'"%(','.join(hostgroup_id_list),agent_id,sql,start_date,end_date)
...
def trap_detail_information(h):
    ...
    try:
        ...
        if len(result)>0:
            ...
            html.write("</td>")
            html.write("<td>%s</td>"%(datetime.strptime(str(result[0][3]),'%a %b %d %H:%M:%S %Y').strftime('%d %b %y %I:%M:%S %p')))
            html.write("</tr>")
    ...
    if flag_value == 0:
        try:
            ...
            for option in range(1,5):
                ...
                report_option_dict={1:'Current Alarms',2:'Clear Alarms',3:'History',4:'Reachability Alarms'}
                ...
                if report_type=='excelReport':
                    xls_sheet=xls_book.add_sheet('%s %s'%(report_option_dict[int(option)],sheet_count),cell_overwrite_ok=True)
                    ...
                    for k in range(len(merge_result)):
                        ...
                        if i==60000:
                            ...
                            xls_sheet.row(0).height = 521
                            xls_sheet.row(1).height = 421
                            xls_sheet.write_merge(0,0,0,len(headings)-1,"%s"%report_option_dict[int(option)],style)
                            xls_sheet.write_merge(1,1,0,len(headings)-1," %s -- %s "%(str(start_date)[:11],str(end_date)[:11]),style)
                            xls_sheet.write_merge(2,2,0,len(headings)-1,"",style)
                            i=4
                            heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
                    ...
                    blank_row=["","",""]
                    main_row=["%s    %s -- %s "%(report_option_dict[int(option)],str(start_date)[:11],str(end_date)[:11])]
                    second_row=["","",""]
