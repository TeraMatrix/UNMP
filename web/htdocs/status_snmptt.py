...
def trap_filter_function(h):
	...
	if flag_value == 0:
		...
		if sql_j > 1:
		    sql=sql+" and ("+sql_1+")"
		try:
		    a_columns=["STR_TO_DATE(ta.trap_receive_date,'%a %b %e %H:%i:%s %Y')","STR_TO_DATE(ta.trap_receive_date,'%a %b %e %H:%i:%s %Y')","ta.trap_event_type","ta.event_id","hosts.device_type_id","hosts.host_alias",
...
def trap_report_creating(h):
	...
	if flag_value == 0:
		try:
			...
			if db ==1:
				raise SelfException(cursor)
				# import the csv and excel module
			import csv
			import xlwt
			...
			for option in range(1,4):
				...
				merge_result=[]
				for row in result:
	#				merge_result.append([(datetime.strptime(str(row[1]),'%a %b %d %H:%M:%S %Y').strftime('%d %b %y %I:%M:%S %p')),device_type_dict[row[2]],row[3],row[4],severity_name[int(row[5])],row[6],row[7],row[8],row[9],row[10],row[11]," ".join([ele.capitalize() for ele in row[12].split(" ")])])
				...
				if report_type=='excelReport':
					xls_sheet=xls_book.add_sheet('%s Event Report'%report_option_dict[int(option)],cell_overwrite_ok=True)
		...
		# Exception Handling  
		    ...
		    output_dict={'success':1,'error_msg':'Error No : 105 '+str(err_obj.get_error_msg(105)),'main_msg':str(e[-1]),'dc':check_var}
		    h.req.write(str(JSONEncoder().encode(output_dict)))
		finally:
		    if db.open:
			db.close()
