...
$(function(){
	...
	$("div.jstree").find("a").find("ins").click(function(){ $(this).parent().click();})
...
function createHost(discoveryType,hostList,devType)
{
	...
	$.ajax({
		...
		async:false,
		success:function(result){
						setTimeout(function(){ createService(discoveryType,hostList,result) },3000);
					}
	})
}
	...
	if(parseInt(service) == 2)
	{
		$.ajax({
			...
			success:function(result){
							...
							{
								setTimeout(function(){viewDiscoveredHostDetails("ping");},3000);
							}
							else if(discoveryType == "snmp")
							{
								setTimeout(function(){viewDiscoveredHostDetails("snmp");},3000);
							}
							else if(discoveryType == "upnp")
							{
								setTimeout(function(){viewDiscoveredHostDetails("upnp");},3000);
							}
							else if(discoveryType == "sdmc")
							{
								setTimeout(function(){viewDiscoveredHostDetails("sdmc");},3000);
							}
							discoveredHostDetailsForAll();
							$("div.loading").hide();
							//reload nms tree
