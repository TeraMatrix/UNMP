...
//	apRecursionVar=setTimeout(function (){apAddDateTime();},refresh_time*60000);
	apRecursionVar=setTimeout(function (){updateDateTime();},60000);
}
...
function graphInitiator(){
	...
	{	
        ...
        if(apIpAddress==undefined || apIpAddress=='' || apIpAddress==null){
		$("#tab_yo").hide();
		$("#ap_show_msg").html('Data Not exists for graph.');
		$("#adSrhAP").hide(); // show the button
		$("#report_button_div").hide();
		$("#ap_host_info_div").html("");
		$("#ap_device_graph").hide();
		return ;
	}
	var refresh_time=$("#refresh_time").val();
	$("#adSrhAP").show(); // show the button
	apNWBandwidth=$("#dashboard1").yoDashboard({
		title:"Network Bandwidth",
		showRefreshButton:true,
		ajaxRequest:function(div_obj){
				apNetworkInterfacesTable(div_obj.attr("id"),0);
				return true;
		},
		showTabOption:true,
		tabList: {value:[0,1,2,3],name:["eth0","bro","ath0","ath1"],selected:0},
		height:"180px"
	});
	apRecursionVar=setTimeout(function (){graphInitiator();
					apAddDateTime();},refresh_time*60000);
}




function apNetworkInterfacesTable(divId,interface_value)
{
	...
	var end_time=$("#odu_end_time").val();
            $.ajax({
                type:"post",
                url:"ap_network_interface_graph.py?ip_address="+apIpAddress+"&start_date="+start_date+"&start_time="+start_time+"&end_date="+end_date+"&end_time="+end_time+"&interface_value="+interface_value+"&limitFlag="+limitFlag,
		data:$(this).serialize(), // $(this).text?
		cache:false,
                success:function(result){
					 try
				...
				$.yoDashboard.hideLoading(apNWBandwidth);
...
function deviceList()
{
	...
	$.ajax({
		...
		success:function(result){
					if (result == 0 || result == "0")
					{
						 $("#ap_show_msg").html("No Data exist.")
						 $("#adSrhap").hide(); // hide the button
						 $("#tab_yo").hide();
						 $("#ap_device_graph").hide();
						 $("#ap_host_info_div").hide();
