...
/*function iduHwSwFrequencyStatus(event,obj,hostId,deviceTypeId)
{
        $(".listing-icon").removeClass("listing-icon-selected");
        event.stopPropagation();
        $.ajax({
	                type: "get",
	                url : "idu_hw_sw_frequency_status.py?host_id=" + hostId +"&device_type_id="+ deviceTypeId,
	                success:function(result){
...
function iduTrapStatus(event,obj,hostId,deviceTypeId,ipAddress)
{
        $(".listing-icon").removeClass("listing-icon-selected");
        event.stopPropagation();
        $.ajax({
...
function show_e1_admin_state(event,obj,hostId,deviceTypeId)
{
        $(".listing-icon").removeClass("listing-icon-selected");
        event.stopPropagation();
        $.ajax({
	                type: "get",
	                url : "e1_admin_state_show.py?host_id=" + hostId +"&device_type_id="+ deviceTypeId,
	                success:function(result){
	                        $(obj).parent().addClass("listing-icon-selected");
	                        obj = $("#status_div");
	                        obj.html("");
	                        obj.html("<div class='sm-loading'></div><div class='sm-spin' style='top:35%;left:36%'></div>"+result);
                                obj.css({'top':event.pageY-90});
                                obj.css({'left':event.pageX-parseInt(((obj.width())*2)/3)});
...
/*function show_main_admin_state(event,obj,hostId,deviceTypeId)
{
        $(".listing-icon").removeClass("listing-icon-selected");
        event.stopPropagation();
        $.ajax({
	                type: "get",
	                url : "main_admin_state_show.py?host_id=" + hostId +"&device_type_id="+ deviceTypeId,
	                success:function(result){
	                        $(obj).parent().addClass("listing-icon-selected");
	                        obj = $("#status_div");
	                        obj.html("");
...
/*
function lock_unlock_admin(event,obj,hostId,adminStateName,state)
{
        ...
        $.ajax({
	                type: "get",
	                url : "all_locked_unlocked.py?host_id=" + hostId +"&admin_state_name="+ adminStateName+"&state="+ state,
	                success:function(result){
	                        if(parseInt(result.success)==0)
	                        {
	                                for(var node in result.result)
	                                {
	                                        if(result.result[node]==0 || result.result[node]=='0')
	                                        {
	                                                if(state==0)
	                                                {
	                                                        $("#"+node).attr({"src":"images/temp/red_dot.png"});
	                                                        $("#"+node).attr({"state":state});
	                                                }       
	                                                else
	                                                {
	                                                        $("#"+node).attr({"src":"images/temp/green_dot.png"});
	                                                        $("#"+node).attr({"state":state});
	                                                }
	                                                $("#"+node).attr({"original-title":"Set successfully"});
	                                                
	                                        }
	                                        else
	                                        {
	                                                $("#"+node).attr({"original-title":result.result[node]});
	                                        }
