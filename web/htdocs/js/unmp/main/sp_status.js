...
//	$("#page_tip").colorbox(
//	{
//	href:"page_tip_sp_status_table.py",
    ...
    $("#sp_show_graph_list").toggle(
        function () {
            $('#tab_yo').slideDown('slow', function () {
                $("#sp_show_graph_list").val('Configuration')
            });
        },
        function () {
...
function deviceList() {
    ...
    $.ajax({
        ...
        cache: false,
        success: function (result) {
            $("#header3_text").html(apIpAddress + " " + selectListElem + " Status Dashboard");
...
function spDeviceDetail() {
    ...
    $.ajax({
        type: "post",
        url: "sp_status_device_details.py?ip_address=" + spIpAddress + "&device_type_id=" + deviceTypeId,
        data: $(this).serialize(),
        cache: false,
        success: function (result) {
...
function spExcelStatusReportGeneration() {
    spCommonReportCreating('sp_excel_status_report_genrating.py', 'UBR_excel.xls');
}
...
function spCSVStatusReportGeneration() {
    spCommonReportCreating('sp_csv_status_report_genrating.py', 'UBR_CSV_Report.csv');
}
...
function spCommonReportCreating(redirectPath, file_name) {
    ...
    if (select_option == 0) {
        ...
        $.ajax({
            type: "post",
            url: redirectPath + "?ip_address=" + spIpAddress + "&start_date=" + start_date + "&start_time=" + start_time + "&end_date=" + $("#sp_end_date").val() + "&end_time=" + $("#sp_end_time").val() + "&device_type_id=" + deviceTypeId + "&select_option=" + select_option + "&limitFlag=" + limitFlag + graphQuerySrting,
            data: $(this).serialize(),
            cache: false,
            success: function (result) {
                if (result.success >= 1 || result.success >= "1") {
...
function show_graph_click() {
    ...
    $.ajax({
        type: "post",
        url: "update_show_graph_status.py?device_type_id=" + deviceTypeId + "&selected_graph=" + $("#sp").val(),
