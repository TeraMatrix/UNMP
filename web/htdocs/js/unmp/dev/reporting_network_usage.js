...
// network bandwidth reporting function start
    $("#excel_rpt_nw").click(function () {
        spinStart($spinLoading, $spinMainLoading);
        var cur_date = new Date();
        var d = cur_date.getDate();
        var y = cur_date.getFullYear();
        ...
        var start_time = $("#start_time").val();
        var end_time = $("#end_time").val();
        var url = "nw_bandwidth_excel_reporting.py?no_of_devices=" + no_of_devices + "&start_date=" + start_date + "&end_date=" + end_date + "&start_time=" + start_time + "&end_time=" + end_time + '&all_host=' + filterHoststring() + '&all_group=' + filterGroupstring();
        $.ajax({
            type: method,
            url: url,
            cache: false,
            success: function (result) {
                if (result == 0 || result == '0') {
                    ...
                    window.location = "download/nw_bandwidth_excel.xls";
                }
...
// network bandwidth reporting function end
//	$("#page_tip").colorbox(
//	    {
//		href:"help_network_usage.py",
//		title: "Page Tip",
//		opacity: 0.4,
//		maxWidth: "80%",
//		width:"650x",
...
//	create data table object
    oTableTotal = $('#total_report_table').dataTable({
        ...
        "aoColumns": [
            { "sTitle": "TIME", "sWidth": "15%" },
            { "sTitle": "HOST NAME", "sClass": "center", "sWidth": "15%"},
            { "sTitle": "IP ADDRESS", "sClass": "center", "sWidth": "15%" },
            { "sTitle": "Eth(0) RX", "sClass": "center", "sWidth": "9%" },
            { "sTitle": "Eth(0) TX", "sClass": "center", "sWidth": "9%" },
            { "sTitle": "Eth(1) RX", "sClass": "center", "sWidth": "9%" },
            { "sTitle": "Eth(1) TX", "sClass": "center", "sWidth": "9%" },
            { "sTitle": "BR(0)  RX", "sClass": "center", "sWidth": "9%" },
            { "sTitle": "BR(0)  TX", "sClass": "center", "sWidth": "9%" },
            { "sTitle": "Group Name", "sClass": "center", "sWidth": "9%" }
        ]
    });
    $("#total_header").show();
...
function hostGroupSearch() {
    $("#group_search").fcbkcomplete({
        isID: 'fb_gp',
        width: '380px',
        json_url: "show_group_result.py?common=false",
        addontab: false,
        maxitems: 10,
        maxshownitems: 10,
        input_min_size: 1,
        ...
        onselect: function () {
            ...
            $.ajax({
                type: "post",
                url: "show_host_data_of_group.py?select_value=" + selected + "&common=false",
                cache: false,
                success: function (result) {
                    for (var h in result) {
...
function HostSearch() {
    ...
    $("#host_search").fcbkcomplete({
        isID: 'fb_host',
        json_url: "show_host_result.py?common=false",
        width: '380px',
        addontab: false,
        maxitems: 10,
        maxshownitems: 10,
