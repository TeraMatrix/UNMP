...
var callB=null;
var ping_chk = 0;
var opStatusDic = {0:'No operation', 1:'Firmware download', 2:'Firmware upgrade', 3:'Restore default config', 4:'Flash commit', 5:'Reboot', 6:'Site survey', 7:'Calculate BW', 8:'Uptime service', 9:'Statistics gathering', 10:'Reconciliation', 11:'Table reconciliation', 12:'Set operation', 13:'Live monitoring', 14:'Status capturing'}
	...
	$.ajax({
		...
		success:function(result){
			if(result == 0 || result == "0")
			{
				...
				retryBtnObj.attr({"src":"images/done.png","title":"Done","alt":"Done"});
				checkAllDone(objId);
		...
		success:function(result){
			...
			{	
				if(result[textBoxArray[tbI]] == "0" || result[textBoxArray[tbI]] == 0)
				{
					...
					imgBtn.attr({"src":"images/done.png","title":"Done","alt":"Done"});
...
function ruConfigurationconfirmCheck()
{
	$("#ru_configuration_form").submit(function(){	
		...
		{
			ruConfigForm = $(this);
			$.prompt('Setting these values will cause Reboot.\n Are you sure you want to do this?',{ buttons:{Ok:true,Cancel:false}, prefix:'jqismooth',callback:ruConfigurationForm });	
...
function aclAddMore()
{
	$("#aclAddMore").click(function(e){
		...
		divStr.find("input").attr({"name":textBoxName,"id":textBoxId,"fact":textBoxFact,"value":textBoxValue,"index":textBoxIndex});
            ...
            try
            {
		        ...
		        success:function(result){
			        ...
			        else
			        {
			         ...
			         setTimeout(function(){spinStop($spinLoading,$spinMainLoading);},5000);    
		            }
		        });    
            }
            catch(err)
							...
							{  
							        ...
							        if(parseInt(json[node][0]) == 1)
								{
								        var imgRec = $(recTableObj).find("a:eq(0)"); 
               						                var imgbtn = $(imgRec);
								        imgbtn.html("Radio Unit Unlocked");
							        ...
							        if(parseInt(json[node][1]) == 1)
								{
							        ...
							        else
							        {
						                ...
						                if(parseInt(json[node][2]) == 1)
							        {
								        var imgRec = $(recTableObj).find("a:eq(2)"); 
               						                var imgbtn = $(imgRec);
								        imgbtn.html("SYN Unlocked");
							                imgbtn.attr({"class":"green"});
							        }
							        else
							        {
	                        ...
	                        success:function(result){
	                                if(parseInt(result.success)==0)
	                                {
	                                        ...
	                                        else
	                                        {
	                                                ...
	                                                if(singleAdminStateName=="ru.ruConfTable.adminstate")
	                                                {
        	                                                $(singleObj).html("Radio Unit Locked");
	                                                }
	                                                else if(singleAdminStateName=="ru.ra.raConfTable.raAdminState")
	                                                {
	                                                     $(singleObj).html("Radio Access Locked");   
	                                                }
	                                                else
	                                                {
	                                                        $(singleObj).html("SYN Locked");
	                                                }
	                                        }
	                                }
	                                else
...
$(function(){
	...
	globalAdminStatus();
        $("#filterOptions").hide();
	$("#hide_search").show();
	$("#odu16_form_div").css({'margin-top':'20px'});
	$("#hide_search").toggle(function(){
		...
		$this.css({
		        'background-color': "#F1F1F1",
                        ...
                        'overflow': 'hidden',
                        'width': "100%"});
               $("#odu16_form_div").css({'margin-top':'76px'});
	},
	function(){
		var $this = $(this);
		var $span = $this.find("span").eq(0);
...
//	$("#odu100_ra_configuration_form").find("input[id='ru.ra.tddMac.raTddMacConfigTable.passPhrase']").attr({"disabled":true});
	...
	{
		$("#odu100_ra_configuration_form").find("input[id='ru.ra.tddMac.raTddMacConfigTable.passPhrase']").removeAttr("disabled");
	}
	$("#odu100_ra_configuration_form").find("select[id='ru.ra.tddMac.raTddMacConfigTable.encryptionType']").change(function(){
	var encryptionValue = $("#odu100_ra_configuration_form").find("select[id='ru.ra.tddMac.raTddMacConfigTable.encryptionType']").val();
	if (encryptionValue == 0 || encryptionValue == '0')
	{
		$("#odu100_ra_configuration_form").find("input[id='ru.ra.tddMac.raTddMacConfigTable.passPhrase']").attr({"disabled":true});
		$("#odu100_ra_configuration_form").find("input[id='ru.ra.tddMac.raTddMacConfigTable.passPhrase']").val("");
	}
	...
	$("#bw_calculator_form").click(function(){
	        if(reconcile_chk_status_btn == 0 || reconcile_chk_status_btn == null)
		{
				...
				success:function(result)
					{	result = eval("(" + result + ")");
						if(result.success == 0)	
						{
							...
							{
								reconcile_chk_status_btn = 0;
								$.prompt('Device Configuration data would be Synchronized with the UNMP server Database',{ buttons:{Ok:true,Cancel:false}, 									prefix:'jqismooth',callback: odu16Reconcilation});
								spinStop($spinLoading,$spinMainLoading);
							}
							else if(json[0]==1)
							{
...
function rebootconfirm()
{
	if(reconcile_chk_status_btn == 0 || reconcile_chk_status_btn == null)
	{
		$.prompt('Are you sure you want to reboot this device? \n Click Ok to confirm',{ buttons:{Ok:true,Cancel:false},prefix:'jqismooth',callback:reboot});			
	}
	else
	{
		$.prompt('Reconciliation is Running.Please Wait',{ buttons:{Ok:true}, prefix:'jqismooth'});
	...
	$.ajax({
		...
		success:function(result)	
		{
			if(result==0)
			{
				...
				/*url = "refresh_channel_list.py?host_id="+host_id+"&selected_device="+device_type_id;
				method = "get";
				showdiv = "#content_8";
				$(showdiv).html("");
...
function acl_reconciliation()
{
	$("input[id='reconcileAcl']").click(function(){
		...
		$.prompt('Are you sure you want to reconcile the ACL MAC?',{ buttons:{Ok:true,Cancel:false}, prefix:'jqismooth',callback: aclReconciliation});
	});
}
	...
	if(v != undefined && v==true && aclClickReconcile)
	{
		...
		$.ajax({
			...
			success:function(result){
						...
						if(result == "")
						{
							$.prompt('Device is Time Out.Please Try Again',{ buttons:{Ok:true}, prefix:'jqismooth'});
							spinStop($spinLoading,$spinMainLoading);
						}
						if(result == 1)
						{
							$.prompt('Problem Creating MAC Addresses.Please Reconcile The ACL MAC Again.',{ buttons:{Ok:true}, prefix:'jqismooth'});
							spinStop($spinLoading,$spinMainLoading);
						}
						if(result == 2)
						{
							$.prompt('There is no MAC Available in device',{ buttons:{Ok:true}, prefix:'jqismooth'});
							spinStop($spinLoading,$spinMainLoading);
						}
						if(result == 0)
						{	
								...
								success:function(result)	
											{
												...
												acl_reconciliation();
												spinStop($spinLoading,$spinMainLoading);	
												$.prompt('ACL Reconciliation Done Successfully',{ buttons:{Ok:true}, prefix:'jqismooth'});
											}
								});
...
function deleteAclConfigMacAddress(macAddrNo)
{
	...
	for(macCount=macAddrNo+1;macCount<=maxMac;macCount++)
	{
		...
		divStr.find("input").attr({"name":textBoxName,"id":textBoxId,"fact":textBoxFact});
...
function odu100IpConfigFormSubmitCheck(formObj,btn)
{
	...
	if($("#"+formObj).valid())
	{
		...
		{
		$.prompt('Set These Values Will Cause Reboot.\nAre You Sure You Want to Do this?',{ buttons:{Ok:true,Cancel:false}, prefix:'jqismooth',callback: odu100IpConfigFormSubmit});
		}
		return false;
	}
}
...
function odu100RuConfigFormSubmitCheck(formObj,btn)   
{
	...
	if($("#"+formObj).valid())
	{
		...
		{
		$.prompt('Setting these values will cause Reboot.Are you sure you want to do this?',{ buttons:{Ok:true,Cancel:false}, prefix:'jqismooth',callback: odu100RUConfigurationSubmit});
		}
		return false;
	}
}
...
function odu100AclConfig(formObj,btn)
{
	formObjAcl = formObj
	btnAcl = btn
	$.prompt('Are you sure you want to reconcile the ACL MAC?',{ buttons:{Ok:true,Cancel:false}, prefix:'jqismooth',callback: odu100AclReconciliation});	
}

function odu100AclReconciliation(v,m)
{
	if(v != undefined && v==true && formObjAcl && btnAcl)
	{
		spinStart($spinLoading,$spinMainLoading);
		var host_id = $("input[name='host_id']").val();
		var selected_device_type = $("select[id='device_type']").val();
							...
							if((btnValue == "Save") || (btnValue == "Retry"))
							{
								...
								{	
									...
									else
									{
										...
										"class":"img-submit-button","oid":node,"name":"odu100_submit","onclick":"return odu100PeerForm('"+ formObj 	+"',this)"});
										imageCreate.val("");
...
{	
	...
	if(btnValue == "Ok")
	{
		...
		{
			myForm.find("select[id='ru.ra.raConfTable.acm']").attr({"disabled":true});
			myForm.find("select[id='ru.ra.raConfTable.dba']").attr({"disabled":true});
			myForm.find("select[id='ru.ra.raConfTable.acs']").attr({"disabled":true});
			myForm.find("select[id='ru.syncClock.syncConfigTable.rasterTime']").attr({"disabled":true});
			myForm.find("select[id='ru.ra.raConfTable.numSlaves']").attr({"disabled":true});
                        myForm.find("select[id='ru.ra.raConfTable.dfs']").attr({"disabled":true});
		        myForm.find("input[name='ru.syncClock.syncConfigTable.syncConfigTimerAdjust']").attr({"disabled":true});
		        myForm.find("input[name='ru.syncClock.syncConfigTable.percentageDownlinkTransmitTime']").attr({"disabled":true});
		        myForm.find("input[name='ru.ra.raConfTable.linkDistance']").attr({"disabled":true});
		}
		else
		{
	...
	else if(btnValue == "Cancel ")
	{
		...
		myForm.find("select[id='ru.ra.raConfTable.numSlaves']").attr({"disabled":true});		
		myForm.find("select[id='ru.ra.raConfTable.dba']").attr({"disabled":true});
		myForm.find("select[id='ru.ruConfTable.synchSource']").attr({"disabled":true});
		spinStop($spinLoading,$spinMainLoading);
	}
	else
	{
					...
					try
					{
						...
						if(result.success == 0)
						{
							if((btnValue == "Save") || (btnValue == "Retry") || (btnValue == "Add") || (btnValue == "Edit") || (btnValue == "Retry "))
							{
									...
									{ 
										if(btnValue == "Add" || (btnValue == "Edit") || (btnValue == "Retry"))
										{
											...
											if($("#odu100_acl_add_mac_config_form").find("input[id='id_omc_submit_save']").val() == "Edit")
											{
										 	...
										 	imageCreate.attr({"type":"button","title":json[node],
											"class":"img-submit-button","oid":node,"name":"odu100_submit"});
											selectAclList.attr({"disabled":false});	
											$(imageCreate).insertAfter(selectListName);
										}
										else
										{
								...
								else
								{
									...
									{
									        peerSelectBasicRateMCSIndex();
										url = "refresh_channel_list.py?host_id="+host_id+"&selected_device="+device_type;
										method = "get";
										showdiv = "#content_8";
										$(showdiv).html("");
										$(showdiv).html("Channel list is refreshing.This take few minutes.");
						...
						else
						{
							...
							try
							{
								...
								$("select.img-submit-button").remove();
								...
								for(var node in result)
								{
									...
									var inputTextboxName = $("input[id='"+node+"']");
									var imageCreate = $("<input/>");
									imageCreate.attr({"type":"button","title":result[node],
									"class":"img-submit-button","oid":node,"name":"odu100_submit"});
									$(imageCreate).insertAfter(inputTextboxName);
									$(imageCreate).insertAfter(selectListName);
								}*/
...
function siteSurvey()
{
        if(reconcile_chk_status_btn == 0 || reconcile_chk_status_btn == null)
	{
                spinStart($spinLoading,$spinMainLoading);spinStop($spinLoading,$spinMainLoading);
                ...
                $.ajax({
			        ...
			        success:function(result)
			        {
                			...
                			{
                			        $().toastmessage('showErrorToast',"UNMP has encountered an error,ErrorCode:1001");
                			}
                			else if(parseInt(result)==53)
                			{
                			        $().toastmessage('showErrorToast',"No response from device");
                			}
                			else if(parseInt(result)==51)
                			{
                			        $().toastmessage('showErrorToast',"Network is unreachable");                			
                			}
                			else if(parseInt(result)==99)
                			{
                			        $().toastmessage('showErrorToast',"UNMP has encountered an error.");                			
                			}
                			...
                			else
                			{
                			     ...
                			     {
                			        $().toastmessage('showErrorToast',"Device is busy, Device "+String(opStatusDic[parseInt(result)])+" is in progress. please wait ...");
                			     }
                			     else
                			     {
                        			     siteSnmpSurvey(hostId,nodeType);
...
function siteSnmpSurvey(hostId,nodeType)
{
        ...
        $.ajax({
                        type:"post",
                        url:"site_survey_snmp.py?node_type="+nodeType+"&host_id="+hostId,
                        success:function(result)
                        {
                                if(parseInt(result.success)==0)
                                {
...
function odu100SyncConfigFormSubmit()
{
	$("#odu100_sync_config_form").submit(function(event){
		if($(this).valid())
		{	
			odu100CommonSetAjaxRequest(this,event);
			return false;
		}
	});
}

function odu100RAConfigurationSubmit()
{
	$("#odu100_ra_configuration_form").submit(function(event){
		if($(this).valid())
		{	
			odu100CommonSetAjaxRequest(this,event);
			return false;
...
function odu100PeerConfigurationSubmit()
{
	$("#odu100_peer_configuration_form").submit(function(event){
		odu100CommonSetAjaxRequest(this,event);
		return false;
	});
}
...
function aclAddEdit()
{
	$("#odu100_acl_add_mac_config_form").submit(function(event){
		if($(this).valid())
		{
			odu100CommonSetAjaxRequest(this,event);
			return false;
...
function peerMacValid()
{
	$("#peer_config_form").validate({
			rules: {
					'ru.np.ra.1.peer.1.config.macAddress':{macAddress:true},
					'ru.np.ra.1.peer.2.config.macAddress':{macAddress:true},
					'ru.np.ra.1.peer.3.config.macAddress':{macAddress:true},
					'ru.np.ra.1.peer.4.config.macAddress':{macAddress:true},
					'ru.np.ra.1.peer.5.config.macAddress':{macAddress:true},
					'ru.np.ra.1.peer.6.config.macAddress':{macAddress:true},
					'ru.np.ra.1.peer.7.config.macAddress':{macAddress:true},
					'ru.np.ra.1.peer.8.config.macAddress':{macAddress:true}
				},
			messages:{
					'ru.np.ra.1.peer.1.config.macAddress':{macAddress:"Invalid Mac Address"},
					'ru.np.ra.1.peer.2.config.macAddress':{mmacAddress:"Invalid Mac Address"},
					'ru.np.ra.1.peer.3.config.macAddress':{macAddress:"Invalid Mac Address"},
					'ru.np.ra.1.peer.4.config.macAddress':{macAddress:"Invalid Mac Address"},
					'ru.np.ra.1.peer.5.config.macAddress':{macAddress:"Invalid Mac Address"},
					'ru.np.ra.1.peer.6.config.macAddress':{macAddress:"Invalid Mac Address"},
					'ru.np.ra.1.peer.7.config.macAddress':{macAddress:"Invalid Mac Address"},
					'ru.np.ra.1.peer.8.config.macAddress':{macAddress:"Invalid Mac Address"}
				}
	});
}
...
function odu100peerMacValid()
{
	$("#odu100_peer_configuration_form").validate({
			rules: {
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.1':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.2':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.3':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.4':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.5':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.6':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.7':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.8':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.9':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.10':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.11':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.12':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.13':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.14':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.15':{macAddress:true},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.16':{macAddress:true}				},
			messages:{
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.1':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.2':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.3':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.4':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.5':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.6':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.7':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.8':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.9':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.10':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.11':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.12':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.13':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.14':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.15':{macAddress:"Invalid Mac Address"},
					'ru.ra.peerNode.peerConfigTable.peerMacAddress.16':{macAddress:"Invalid Mac Address"}
				}
	});
}
