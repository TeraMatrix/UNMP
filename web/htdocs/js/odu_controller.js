...
function odu100PeerConfig(formObj,btn)
{
	...
	if(btnValue=="Ok")
	{
		...
		{
		        myForm.find("input:not(input[type=hidden])").attr({"disabled":true});  
        		myForm.find("select:not(input[type=hidden])").attr({"disabled":true});  
	        }
	        else
	        {
							...
							if((btnValue == "Save") || (btnValue == "Retry"))
							{
								...
								{	
									...
									else
									{
										...
										"class":"img-submit-button","oid":node});
										imageCreate.val("");
										        ...
										        $.ajax({
											        ...
											        success:function(result)
											        {
												        ...
												        spinStop($spinLoading,$spinMainLoading);
                                                                      ...
                                                                      odu100_ru_channel_banwidth = $("#odu100_ru_configuration_form").find("select[name='ru.ruConfTable.channelBandwidth']").val();
                	                                                        odu100_ru_country_code = $("#odu100_ru_configuration_form").find("select[name='ru.ruConfTable.countryCode']").val();
                	                                                        $("#channel_configuration_form").find("select").attr({"diabled":true});
									}
...
function alarmRecon(host_id,device_type)
{
	$.colorbox(
	{
		...
		width:"600px",
		height:"300px",
		onClosed: function(){clearTimeout(upVar);}
	});
}
...
//        spinStart($spinLoading,$spinMainLoading);
	...
	return false;
        /*url = "refresh_channel_list.py?host_id="+host_id+"&selected_device="+device_type+"&refresh="+1;
	method = "get";
	showdiv = "#content_8";
	$.ajax({
		type:method,
		...
		{
			$(showdiv).html(result);
			status_msg = parseInt($("input[name='status_msg']").val());
			if(status_msg==1)
			{       
		                $().toastmessage('showErrorToast','Device is not responding,please try again later.');
			}
			spinStop($spinLoading,$spinMainLoading);
		}
