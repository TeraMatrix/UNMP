...
/*function changeStatusMiniSpin(divId,statusOf)
{
	divName = obj1 = $("#"+divId).parent().parent().attr("id");
	obj1 = $("#"+divName);
	if(statusOf == 0)
	{
		obj1.find(".sm-loading").hide();
        	obj1.find(".sm-spin").hide();
	}
	else
	{
		obj1.find(".sm-loading").show();
                obj1.find(".sm-spin").show();
	}
...
function graphInitiator(){
	...
	spinStop($spinLoading,$spinMainLoading);	
	odu16RecursionVar=setTimeout(function (){graphInitiator();oduAddDateTime();},refresh_time*20000);
}
