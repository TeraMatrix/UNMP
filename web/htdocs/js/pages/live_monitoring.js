...
function drawRRATable()
{
	...
	for(var i=0;i<rraCount;i++)
	{
		...
		$("<input/>").keypress(function(event){return isNumberKey(event);}).attr({"name":"rra_dataset","type":"text"}).css({"width":"30px","height":"13px", "padding":"3px 3px 4px"}).val(graphObj[$form["name"].val()]["rra_dataset"][i]!=undefined ? graphObj[$form["name"].val()]["rra_dataset"][i] : "").appendTo($("<td/>").appendTo($tr));
		$tr.appendTo($table);
	}
	$table.appendTo($form["rra_table_div"]);
...
function drawDatasetTable()
{
	...
	{
		var $tr = $("<tr/>");
		$("<input/>").keypress(function(event){return isNumberKey(event);}).attr({"name":"row_index","type":"text"}).css({"width":"30px","height":"13px", "padding":"3px 3px 4px"}).val(tempRow[i]).appendTo($("<td/>").appendTo($tr));
		$("<input/>").keypress(function(event){return isNumberKey(event);}).attr({"name":"column_index","type":"text"}).css({"width":"30px","height":"13px", "padding":"3px 3px 4px"}).val(tempColumn[i]).appendTo($("<td/>").appendTo($tr));
