...
function multiselect_report_func()
{
		...
		$("#multiselect_report").bind("multiselectclose", function(event, ui){
			...
			if(sel!=null)
			{
				...
				$.ajax({ 
				type:"get",
				url:"history_reporting_get_host_data.py"+'?hostgroup_id='+hostgroup_id+'&device_type_id='+device_type_id+'&report_type='+sel,
				data:"",
				cache:false,
...
function multiselect_host_func()
{
		...
		{	/*if(srce.indexOf("sizcache")==-1):
			...
			var checkedHosts = $(this).multiselect("getChecked").map(function(){return this.value;}).get();
...
function multiselect_dates_func()
{
		...
		end_date.setYear(var_month[0]);
		end_date.setMonth(var_month[1]-1);
		end_date.setDate(7);
		end_date.setHours(23);
		end_date.setMinutes(59);
		start_date.setYear(var_month[0]);		
		start_date.setMonth(var_month[1]-1);
...
$(document).ready(function(){
		$("#multiselect_month").multiselect({selectedList: 1,multiple:false,noneSelectedText: 'Select Time Range',header:"Available Time Range",minWidth:290});
		$("#multiselect_month").bind("multiselectclick", function(event, ui){
		});
	...
	$("#get_history_reporting_data").submit(function(){
		...
		var url="history_reporting_get_excel.py?start_date="+start_date+"&end_date="+end_date+"&start_time="+start_time+"&end_time="+end_time+"&all_host="+$("#multiselect_hosts").val()+"&columns="+$("#hd").val();
...
$("#page_tip").colorbox(  			//page tip
	    	{
		href:"view_page_tip_main_history_reporting.py",
		title: "Help",
		opacity: 0.4,
		maxWidth: "80%",
		width:"600px",
