        ...
        $("input[id='btn_filter']").toggle(function()
					...
					,function(){
	                   ...
	                   /*     option=$("input[name='option']:checked").val();
                        	url="trap_filter_function.py?"+$("#alarm_info_form").serialize()+"&option="+option+"&serevity1="+serevity_value[1]+"&serevity2="+serevity_value[2]+"&serevity3="+serevity_value[3]+"&serevity4="+serevity_value[4]+"&serevity5="+serevity_value[5]+"&searchFlag="+searchFlag+"&countTimes="+countTimes+"&last_execution_time="+"";
...
/*		    $.ajax({
			...
			success:function(result){
				...
				else
				{
					...
					var cdate = new Date(endDateStr[2],parseInt(endDateStr[1])-1, endDateStr[0]); */
					var cdate= new Date();
...
//	});
				...
				$.ajax({
					type:"post",
					url:"trap_filter_function.py?"+$(this).serialize()+"&option="+option+"&serevity1="+serevity_value[1]+"&serevity2="+serevity_value[2]+"&serevity3="+serevity_value[3]+"&serevity4="+serevity_value[4]+"&serevity5="+serevity_value[5]+"&searchFlag="+searchFlag+"&countTimes="+countTimes+"&last_execution_time="+"",
					cache:false,
					success:function(result){
				 			try
				 			{
							...
							else if (result.success==4 || result.success=='4')
							{
								$().toastmessage('showNoticeToast', result.output);
								spinStop($spinLoading,$spinMainLoading);
								return;
							 }
							  else
								...
								$('#trap_detail').dataTable({
									...
									"aoColumns": [
											...
											{ "sTitle": "IP Address", "sClass": "center","sWidth": "12%" },
...
//		var eventId=$("#eventId").val();
		...
		spinStop($spinLoading,$spinMainLoading);
	eventRecursion=setTimeout(function (){updateDatatable(last_execution_time);},60000);
