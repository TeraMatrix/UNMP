...
class SPStatusBll(object):
    ...
    def specific_all_graph_json(self,device_type_id,user_id,ip_address):
		...
		if device_type_id=='odu16' or device_type_id=='odu100':
		    ...
		    refresh_json={'url':'update_status_table_in_database.py','method':'get','cache':False,'data':{'table_name':"%s,%s"%(ip_address,ajax[2])}}
    ...
    def common_table_json(self,display_type,user_id,table_name,x_axis_value,index_name,graph_id,flag,start_date,end_date,start,limit,ip_address,graph,update_field_name,interface=1,value_cal=1,*column_name):
        ...
        td=[]
        th=[]
	limit_dict={'odu100NWstatus':2,'odu100syncstatus':1,'odu100Rastatus':1,'idu4e1portstatus':4}
	flag=1
	op_state=1
	...
	if display_type=='table' or display_type=='report':
        ...
        try:
            ...
            sel_query="select graph_title,graph_subtitle from graph_templet_table where graph_display_id='%s' and user_id='%s' and dashboard_type=1"%(graph_id,user_id)
            cursor.execute(sel_query)
		...
		cursor.execute(sel_query)
		result=cursor.fetchall()
		given=['portNum', 'opStatus', 'los', 'lof', 'ais', 'rai', 'rxFrameSlip', 'txFrameSlip', 'bpv', 'adptClkState', 'holdOverStatus', 'timestamp']
		req=[]
		req.append('portNum')
		if "" in column_name[0]:
		...
		for name in column_name[0]:
		    th.append(str(table_field_dict1[table_name][name])+str(graph_field_dict[name]))
		if len(column_name[0])>0 and len(th):
		    th.append('Timestamp')
		    td=manage_list(req,given,result)
	    elif graph_id.strip()=='idu4linkstatustable':
		sel_query="SELECT count(bundleNumber) FROM idu_linkConfigurationTable as idutemp inner join hosts on idutemp.config_profile_id=hosts.config_profile_id where idutemp.portNumber =%s and hosts.ip_address='%s' and hosts.is_deleted=0"%(interface,ip_address)
		cursor.execute(sel_query)
		link_result=cursor.fetchall()
		link_count=0 if link_result[0][0]==None or len(link_result)=="" else link_result[0][0]
		op_state=2
		if display_type.strip()=='report':
		 ...
		 from   idu_linkStatusTable as idu INNER JOIN hosts ON hosts.host_id=idu.host_id where hosts.ip_address='%s' and idu.portNum =%s and idu.timestamp >='%s' and idu.timestamp <='%s' and hosts.is_deleted=0 order by idu.timestamp desc,idu.bundleNum asc "%(ip_address,interface,start_date,end_date)
		else:
		    sel_query="select idu.bundleNum,\
			(if(idu.operationalStatus=0,'Disable',if(idu.operationalStatus=1111111,idu.operationalStatus,'Enable'))) as operationalStatus,\
			minJBLevel,\
			...
			(if(idu.underrunOccured=0,'Bit Clear',if(idu.underrunOccured=1111111,idu.underrunOccured,'Bit Set'))) as underrunOccured,\
			(if(idu.overrunOccured=0,'Bit Clear',if(idu.overrunOccured=1111111,idu.overrunOccured,'Bit Set'))) as overrunOccured ,idu.timestamp\
		 from   idu_linkStatusTable as idu INNER JOIN hosts ON hosts.host_id=idu.host_id where hosts.ip_address='%s' and idu.portNum =%s and hosts.is_deleted=0  order by idu.timestamp desc,idu.bundleNum asc limit %s"%(ip_address,interface,link_count)
		cursor.execute(sel_query)
		result=cursor.fetchall()
		given=['bundleNum','operationalStatus', 'minJBLevel', 'maxJBLevel', 'underrunOccured', 'overrunOccured', 'timestamp']
		req=[]
		...
		if len(column_name[0])>0 and len(th)>0:
		    ...
		    if display_type.strip()=='report':
                        new_sel_query=sel_query+column_list+",gp_tab.timestamp from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' and gp_tab.timestamp >='%s' and gp_tab.timestamp <='%s' and hosts.is_deleted=0 order by gp_tab.timestamp desc)"%(table_name,ip_address,start_date,end_date)
		    else:
                        sel_query+=column_list+","+"gp_tab.%s,gp_tab.%s from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' and hosts.is_deleted=0  order by gp_tab.timestamp desc) as gp_tab"%(index_name,x_axis_value,table_name,ip_address)
		        new_sel_query="SELECT "+column_list+",gp_tab.%s FROM "%(x_axis_value)+sel_query+" GROUP BY gp_tab.%s limit %s"%(index_name,limit_dict[graph_id])
		...
		state_index=7
                ...
                for row in result:
                    for i  in range(len(row)-1):
                        data_list[i].append(0 if row[i]==None or row[i]==None else row[i])
		...
		th=[]
			...
			op_state=2
			    ...
			    td[temp_list][state_index]= 'Enable' if int(td[temp_list][state_index])==1 else 'Disable'
	    data_list=[]
	    if len(td)>0:
	    ...
	    table_dict={'td':data_list,'th':th}
            output_dic={'success':0,'timestamp':time_list,'data':table_dict,'graph_title':graph_title,'graph_sub_title':graph_sub_title}
            return  output_dic
        except MySQLdb as e:
            output_dict={'success':1,'error_msg':'Error No : 102 '+str(err_obj.get_error_msg(102)),'main_msg':str(e[-1])}
            return output_dict
    ...
    def update_database_table(self,ip_address,table_name):
        try:
            ...
            if db ==1:
                raise SelfException(cursor)
            hstatus_dict = {0:'No operation', 1:'Firmware download', 2:'Firmware upgrade', 3:'Restore default config', 4:'Flash commit',\
             5:'Reboot', 6:'Site survey', 7:'Calculate BW', 8:'Uptime service', 9:'Statistics gathering', 10:'Reconciliation', \
             11:'Table reconciliation', 12:'Set operation', 13:'Live monitoring', 14:'Status capturing' }
	    ...
	    sel_query="SELECT host_id,snmp_port,snmp_read_community,device_type_id from hosts where ip_address = '%s'"%ip_address
	    cursor.execute(sel_query)
	    result=cursor.fetchall()
	    if len(result)>0:
	        host_id="" if result[0][0]==None else result[0][0]
	        snmp_port="" if result[0][1]==None else result[0][1]
	        read_community="" if result[0][2]==None else result[0][2]
	        device_type_id="" if result[0][3]==None else result[0][3]
	    dd = DeviceStatus()
	    status=dd.insert_status(host_id,ip_address,snmp_port,read_community,device_type_id,table_name)
	    if int(status)==0:
	        output_dict={'success':0,'msg':'status updated successfully.'}
	    elif status== -1 or status== '-1':
	        output_dict={'success':1,'msg':'No response recieved from device'}	    
	    elif int(status)>0 and int(status)<21:
	        output_dict={'success':1,'msg':'Device is busy, Device %s is in progress.'%hstatus_dict.get(int(status),"other operation")}
	    else:
	        output_dict={'success':1,'msg':'UNMP Server or Services not running so please contact your Administrator.'}
            return  output_dict
        # Exception Handling
        except MySQLdb as e:
            output_dict={'success':1,'msg':'Error No : 102 '+str(err_obj.get_error_msg(102)),'main_msg':str(e[-1])}
            return output_dict
        except SelfException:
            output_dict={'success':1,'msg':'Error No : 104 '+str(err_obj.get_error_msg(104)),'main_msg':str(e[-1])}
            return output_dict
        except Exception as e:
            output_dict={'success':1,'msg':'Error No : 105 '+str(err_obj.get_error_msg(105)),'main_msg':str(e[-1])}
            return output_dict
        finally:
            if db.open:
                db.close()
    ...
    def update_show_graph_table(self,device_type_id,user_id,show_graph):
        try:
            ...
            if show_graph.strip()!='':
                sql="UPDATE graph_templet_table SET is_disabled=1 WHERE user_id='%s' and device_type_id='%s' and dashboard_type=1"%(user_id,device_type_id)
                cursor.execute(sql)
                db.commit()
                sql1="UPDATE graph_templet_table SET is_disabled=0 WHERE user_id='%s' AND device_type_id='%s' AND graph_display_id IN ('%s') and dashboard_type=1"%(user_id,device_type_id,"','".join(graph_list))
                cursor.execute(sql1)
                db.commit()
    ...
    def sp_dashboard_get_graph_name(self,user_id,device_type,ip_address):
        try:
            ...
            cursor=db.cursor()
            sql=" select graph_display_name,is_disabled,graph_display_id from graph_templet_table where device_type_id='%s' and user_id='%s' and dashboard_type=1"%(device_type,user_id)
            cursor.execute(sql)
            result=cursor.fetchall()
            selected_graph=[]
            non_selected_graph=[]
    ...
    def sp_total_display_graph(self,device_type_id,user_id,ip_address):
        try:
           # check the deivce is master or not.
            ...
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            sql=" select count(graph_display_id) from graph_templet_table where device_type_id='%s' and user_id='%s' and is_disabled=0 and dashboard_type=1"%(device_type_id,user_id)
            cursor.execute(sql)
            result=cursor.fetchall()
	    count=0
            if len(result)>0:
...
#            save_file_name=str(start_date)+'_'str(end_date)+'_'+str(device_name[device_type])+'(%s)_excel.xls'%ip_address
    ...
    #        heading=[table_output[0][1],table_output[0][2],table_output[0][3],table_output[0][4],table_output[0][5],table_output[0][6],table_output[0][7]]
            ...
            for i in range(len(tab_list)):
                ...
                limit=0
                output_result=self.common_table_json(display_type,user_id,table_name,x_axis,index_name,graph_id,limitFlag,start_date,end_date,start,limit,ip_address,graph_list[i],update_field_name,tab_list[i],cal_list[i],field_column)
...
#                    merge_result=merge_list(d1_list)
		...
		if len(headings)<2:
		    continue
                ...
                xls_sheet.row(0).height = 521
                xls_sheet.row(1).height = 421
                xls_sheet.write_merge(0,0,0,len(headings)-1,"%s (%s--%s)"%(str(graph_name_list[i]),str(start_date),str(end_date)),style)
                xls_sheet.write_merge(1,1,0,len(headings)-1,device_name[device_type]+'   '+str(ip_address),style)
...
# CSV reporting start here.
    def sp_csv_report(self,device_type,user_id,ip_address,cal_list,tab_list,field_list,table_name_list,graph_name_list,start_date,end_date,select_option,limitFlag,graph_list,start_list,limit_list):
        ...
        try:
            ...
            headings=['Fields','Fields Value']
            blank_row=["","",""]
            main_row=['Device Information  (%s-%s)'%(str(start_date),str(end_date))]
    ...
    def sp_pdf_report(self,device_type,user_id,ip_address,cal_list,tab_list,field_list,table_name_list,graph_name_list,start_date,end_date,select_option,limitFlag,graph_list,start_list,limit_list):
	    ...
	    display_type='report'
            ...
            if db ==1:
                raise SelfException(cursor)
            device_name={'ap25':'AP25','odu16':'RM18','odu100':'RM','idu4':'IDU','ccu':'CCU'}
	    idu4_port={'0':'(odu)','1':'(eth0)','2':'(eth1)','3':'(cpu)','4':'(maxima)'}
...
'odu100_peerNodeStatusTable':{'1':'(Link1)','2':'(Link2)','3':'(Link3)','4':'(Link4)','5':'(Link5)','6':'(Link6)','7':'(Link7)','8':'(Link8)','9':'(Link9)','16':'(Link16)'}}
            ...
            # we are geting the login user name here. 
            login_user_name=''
            sel_query="SELECT user_name FROM user_login WHERE user_id='%s' and hosts.is_deleted=0"%(user_id)
            cursor.execute(sel_query)
            uesr_information=cursor.fetchall()
            if len(uesr_information)>0:
                login_user_name=uesr_information[0][0]
            # finished
            # save file name.
            save_file_name=str(login_user_name)+'_'+str(device_name[device_type])+'_PDF_Report.pdf'
...
#            pdfdoc="/omd/sites/%s/share/check_mk/web/htdocs/download/IDU4_PDF_Report.pdf"%(nms_instance,start_time,end_time)
            ...
            if device_type.strip()=='odu16' or device_type.strip()=='odu100':
                ...
                if len(result)>0 and result!=None:
                    table_output.append([device_field[0],str('--' if result[0]==None or result[0]==""  else result[0])])
                    table_output.append([device_field[1],str('--' if result[1]==None or result[1]==""  else result[1])])
                    table_output.append([device_field[2],str('--' if result[2]==None or result[2]==""  else result[2])])
                    table_output.append([device_field[3],str('--' if result[3]==None or result[3]==""  else result[3])])
                    table_output.append([device_field[4],str('--' if result[4]==None or result[4]==""  else last_reboot_resion[int(result[4])])])
                    table_output.append([device_field[5],str('--' if result[5]==None or result[5]==""  or result[5]<0 else channel[int(result[5])])])
...
#                        headings.append(data_list['name'][0])
        ...
        #                t = Table(data,[2.7 * inch, 2.2 * inch, 1.5*inch, .7*inch])
            ...
            headings = ['Received Date','Device Type','Event Name','Event ID','Event Type','Description']
            table_output.append(headings)
