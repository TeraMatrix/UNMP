...
class SPStatusBll(object):
        ...
        try:
            ...
            if Validation.is_valid_ip(host_id):
                ...
                if host_id == "" or host_id == None or str(host_id) == 'None':
                    ...
                    if cursor.execute(
                            "SELECT ip_address from hosts where mac_address = '%s' and device_type_id = '%s' and is_deleted=0") % (
                    mac_address, selected_device):
                        result = cursor.fetchall()
                    elif cursor.execute(
                            "SELECT ip_address from hosts where ip_address = '%s' and device_type_id = '%s' and is_deleted=0") % (
    ...
    def specific_all_graph_json(self, device_type_id, user_id, ip_address):
        ...
        try:
            ...
            FROM graph_templet_table WHERE user_id='%s' AND device_type_id='%s' AND is_disabled=0 and dashboard_type=1 " % (
            ...
            for row in result:
                ...
                graph_dict = {'name': row[0], 'displayName': row[1], 'fields': option_list, 'ajax': ajax_json,
                              'tabList': {'value': interface_value, 'name': interface_name, 'selected': check_val},
                              'otherOption': {'showOption': True if int(row[6]) == 1 else False,
                                              'showRefreshButton': True if int(row[7]) == 1 else False,
                                              'showNextPreButton': True if int(row[8]) == 1 else False,
                                              'width': row[11],
                                              'height': (row[12] == -1 and "auto" or str(row[12]) + "px"), 'showType':
                                  showType, 'showFields': showFields, 'showCalType': showCalType,
                                              'showTabOption': showTabOption, 'autoRefresh': row[18]},
                              'startFrom': row[9], 'itemLimit': row[10], 'totalItemAjax': total_item_json,
                              'onRefreshAjax': refresh_json}
    ...
    def common_table_json(self, display_type, user_id, table_name, x_axis_value, index_name, graph_id, flag, start_date,
                          end_date, start, limit, ip_address, graph, update_field_name, interface=1, value_cal=1,
                          *column_name):
        """
        ...
        try:
		 ...
		 from  idu_e1PortStatusTable as idu INNER JOIN hosts ON hosts.host_id=idu.host_id  where hosts.ip_address='%s' and idu.timestamp >='%s' and idu.timestamp <='%s' and hosts.is_deleted=0 order by idu.timestamp desc,idu.portNum asc" % (
			...
			(if(idu.adptClkState=0,'State Idle', \
		       ...
		       as adptClkState, \
		     (if(idu.holdOverStatus=0,'In Normal Mode',if(idu.holdOverStatus=1111111,idu.holdOverStatus,'In Hold Over Mode'))) as holdOverStatus,idu.timestamp \
		 from  idu_e1PortStatusTable as idu INNER JOIN hosts ON hosts.host_id=idu.host_id  where hosts.ip_address='%s' and hosts.is_deleted=0 order by idu.timestamp desc,idu.portNum asc limit 4" % (
                    ip_address)
                cursor.execute(sel_query)
                result = cursor.fetchall()
		...
		and hosts.is_deleted=0" % (interface, ip_address)
		 ...
		 and idu.portNum =%s and hosts.is_deleted=0  order by idu.timestamp desc,idu.bundleNum asc limit %s" % (
                    ip_address, interface, link_count)
                cursor.execute(sel_query)
                result = cursor.fetchall()
                ...
                if len(column_name[0][0]) > 0:
                    ...
                    if display_type.strip() == 'report':
                        ...
                        where hosts.ip_address='%s' and hosts.is_deleted=0  order by gp_tab.timestamp desc) as gp_tab" % (
                        index_name, x_axis_value, table_name, ip_address)
                        new_sel_query = "SELECT " + column_list + ",gp_tab.%s FROM " % (
                            x_axis_value) + sel_query + " GROUP BY gp_tab.%s limit %s" % (
                                        index_name, limit_dict[graph_id])
                ...
                if len(column_name[0][0]) > 0:
                    ...
                    for name in column_name[0]:
                        ...
                        i += 1
                    th.append('Timestamp')
                    if flag == 0 and len(time_list) > 0 and len([True for column_val in column_name[0] if
                                                                 column_val.strip() in ["operationalState",
    ...
    def sp_excel_report(self, device_type, user_id, ip_address, cal_list, tab_list, field_list, table_name_list,
                        graph_name_list, start_date, end_date, select_option, limitFlag, graph_list, start_list,
                        limit_list):
        """
        ...
        try:
            ...
            if device_type.strip() == 'ap25':
                channel = [
                    ...
                    'channel-08', 'channel-09', 'channel-10',
                ...
                on ap25_versions.host_id=hosts.host_id where ap25_radioSetup.config_profile_id=hosts.config_profile_id order by hosts.ip_address" % (
                ip_address)
                cursor.execute(sql)
                result = cursor.fetchall()
                ...
                if len(result) > 0:
                    for i in result:
                        ...
                        table_output.append([device_detail[1], '--' if result[0][1]
                                                                       == None or result[0][1] == '' else channel[
                            int(result[0][1]) - 1]])
            ...
            if device_type.strip() == 'odu16' or device_type.strip() == 'odu100':
                ...
                if len(last_reboot_time) > 0:
                    table_output.append([device_field[9], '--' if last_reboot_time[0][0]
                                                                  == None or last_reboot_time[0][0] == '' else
            ...
            for i in range(len(tab_list)):
                ...
                output_result = self.common_table_json(
                    display_type, user_id, table_name, x_axis, index_name, graph_id, limitFlag, start_date, end_date,
                    start, limit,
    ...
    def sp_csv_report(self, device_type, user_id, ip_address, cal_list, tab_list, field_list, table_name_list,
                      graph_name_list, start_date, end_date, select_option, limitFlag, graph_list, start_list,
                      limit_list):
        """
    ...
    def sp_pdf_report(self, device_type, user_id, ip_address, cal_list, tab_list, field_list, table_name_list,
                      graph_name_list, start_date, end_date, select_option, limitFlag, graph_list, start_list,
                      limit_list):
        """
        ...
        try:
            ...
            interface_list = {'get_odu16_nw_interface_statistics_table': {'1': '(eth0)', '2': '(br0)', '3': '(eth1)'},
                              ...
                              'odu100_peerNodeStatusTable': {'1': '(Link1)', '2': '(Link2)', '3': '(Link3)',
                                                             '4': '(Link4)', '5': '(Link5)', '6': '(Link6)',
                                                             '7': '(Link7)', '8': '(Link8)', '9': '(Link9)',
                                                             '16': '(Link16)'}}
