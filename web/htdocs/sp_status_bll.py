...
#                        option_list.append({'name':option_tuple[0][0],'displayName':option_tuple[0][1],'isChecked':option_tuple[0][2]})
	...
	if display_type=='table' or display_type=='report':
		...
		if display_type.strip()=='report':
		 ...
		 from  idu_e1PortStatusTable as idu INNER JOIN hosts ON hosts.host_id=idu.host_id  where hosts.ip_address='%s' and idu.timestamp >='%s' and idu.timestamp <='%s' order by idu.timestamp desc,idu.portNum asc"%(ip_address,start_date,end_date)
		else:
		    sel_query="select  idu.portNum, \
			(if(idu.opStatus=0,'Disable',if(idu.opStatus=1111111,idu.opStatus,'Enable'))) as opStatus, \
			(if(idu.los=0,'Bit Clear',if(idu.los=1111111,idu.los,'Bit Set'))) as los, \
			...
			(if(idu.adptClkState=0,'State Idle', \
		       ...
		       as adptClkState, \
		     (if(idu.holdOverStatus=0,'In Normal Mode',if(idu.holdOverStatus=1111111,idu.holdOverStatus,'In Hold Over Mode'))) as holdOverStatus,idu.timestamp \
		 from  idu_e1PortStatusTable as idu INNER JOIN hosts ON hosts.host_id=idu.host_id  where hosts.ip_address='%s' order by idu.timestamp desc,idu.portNum asc limit 4"%(ip_address)
		cursor.execute(sel_query)
		result=cursor.fetchall()
		...
		sel_query="SELECT count(bundleNumber) FROM idu_linkConfigurationTable as idutemp inner join hosts on idutemp.config_profile_id=hosts.config_profile_id where idutemp.portNumber =%s and hosts.ip_address='%s'"%(interface,ip_address)
		cursor.execute(sel_query)
		...
		if display_type.strip()=='report':
		 ...
		 from   idu_linkStatusTable as idu INNER JOIN hosts ON hosts.host_id=idu.host_id where hosts.ip_address='%s' and idu.portNum =%s and idu.timestamp >='%s' and idu.timestamp <='%s' order by idu.timestamp desc,idu.bundleNum asc "%(ip_address,interface,start_date,end_date)
		else:
		    sel_query="select idu.bundleNum,\
			(if(idu.operationalStatus=0,'Disable',if(idu.operationalStatus=1111111,idu.operationalStatus,'Enable'))) as operationalStatus,\
			minJBLevel,\
			...
			(if(idu.underrunOccured=0,'Bit Clear',if(idu.underrunOccured=1111111,idu.underrunOccured,'Bit Set'))) as underrunOccured,\
			(if(idu.overrunOccured=0,'Bit Clear',if(idu.overrunOccured=1111111,idu.overrunOccured,'Bit Set'))) as overrunOccured ,idu.timestamp\
		 from   idu_linkStatusTable as idu INNER JOIN hosts ON hosts.host_id=idu.host_id where hosts.ip_address='%s' and idu.portNum =%s order by idu.timestamp desc,idu.bundleNum asc limit %s"%(ip_address,interface,link_count)
		cursor.execute(sel_query)
		result=cursor.fetchall()
		...
		if len(column_name[0])>0 and len(th)>0:
		    ...
		    if display_type.strip()=='report':
                        new_sel_query=sel_query+column_list+",gp_tab.timestamp from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' and gp_tab.timestamp >='%s' and gp_tab.timestamp <='%s' order by gp_tab.timestamp desc)"%(table_name,ip_address,start_date,end_date)
		    else:
                        sel_query+=column_list+","+"gp_tab.%s,gp_tab.%s from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' order by gp_tab.timestamp desc) as gp_tab"%(index_name,x_axis_value,table_name,ip_address)
...
'odu100_peerNodeStatusTable':{'1':'(Link1)','2':'(Link2)','3':'(Link3)','4':'(Link4)','5':'(Link5)','6':'(Link6)','7':'(Link7)','8':'(Link8)','9':'(Link9)','16':'(Link16)'}}
            ...
            # we are geting the login user name here. 
            login_user_name=''
            sel_query="SELECT user_name FROM user_login WHERE user_id='%s'"%(user_id)
            cursor.execute(sel_query)
            uesr_information=cursor.fetchall()
            if len(uesr_information)>0:
                login_user_name=uesr_information[0][0]
