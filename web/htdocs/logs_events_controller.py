...
def manage_logs(h):
    ...
    html.new_header("Global Logs","manage_logs.py",header_btn,css_list,js_list)
...
def search_logs(h):
    ...
    for i in range(len(query_events_data)):
            ...
            query_events_data[i][1] = "<img src=\"images/new/status-%s.png\" alt=\"%s\"/>" % (query_events_data[i][1],query_events_data[i][1])
            query_events_data[i][2] = LogsEventsBll().convert_time(query_events_data[i][2])
...
def manage_events(h):
    ...
    header_btn = LogsEvents().header_buttons()
    html.new_header("Hosts and Service Events","manage_events.py",header_btn,css_list,js_list)
    html.write(LogsEvents().manage_events())
    html.new_footer()
