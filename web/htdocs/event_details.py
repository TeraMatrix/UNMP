...
def start_page(h):
	...
	global html
	html=h
	css_list = ["css/custom.css","css/demo_table_jui.css","css/jquery-ui-1.8.4.custom.css","facebox/facebox.css","css/calendrical.css"]
	js_list = ["facebox/facebox.js","js/eventDetails.js","js/ccpl_utility.js","js/jquery.dataTables.min.js","js/calendrical.js"]
	...
	html.write("<table class=\"addform\" style=\"border:0px None\"><tr class=\"odd\">")

        html.write("<td style=\"width:150px; font-weight: bold;\">Alarm Type")
        html.write("</td>")

        html.write("<td>")
        html.write("<input type=\"radio\" value=\"1\" name=\"option\" id=\"option1\" checked=\"checked\" class=\"table_option\" width=\"12px\"/>")
        html.write("</td>")
        ...
        html.write("<td>")
        html.write("<input type=\"radio\" value=\"2\" name=\"option\" id=\"option2\" class=\"table_option\" width=\"12px\"/>")
        html.write("</td>")
	...
	html.write("<img src=\"images/normal1.png\"  name=\"select_option_div\" alt=\"Informational\" title=\"Informational\" style=\"width:12px\" class=\"imgbutton\" /><span class=\"inactive-linkclass\" style=\"line-height:20px; padding:0px 12px;cursor:pointer;cursor:hand;\">Informational</span>")
	html.write("</div>")
	html.write("</td>")

        html.write("<td>")
	html.write("<div class=\"trap_select_option_div\" style=\" width:100px;\" id=\"normal_div\">")
	html.write("<img src=\"images/normal1.png\"  name=\"select_option_div\" alt=\"Normal\" title=\"Normal\" style=\"width:12px\" class=\"imgbutton\"/><span class=\"inactive-linkclass\" style=\"line-height:20px; padding:0px 12px;cursor:pointer;cursor:hand; \" >Normal</span>")
	html.write("</div>")
	html.write("</td>")

        html.write("<td>")
	html.write("<div class=\"trap_select_option_div\" style=\" width:100px;\" id=\"minor_div\">")
	html.write("<img src=\"images/minor1.png\" alt=\"Minor\" name=\"select_option_div\" title=\"Minor\" class=\"imgbutton\" style=\"width:12px\" /><span class=\"inactive-linkclass\" style=\"line-height:20px; padding:0px 12px;cursor:pointer;cursor:hand; \">Minor</span>")
	html.write("</div>")
	html.write("</td>")

        html.write("<td>")
	html.write("<div class=\"trap_select_option_div\" style=\" width:100px;\" id=\"major_div\">")
	html.write("<img src=\"images/status-1.png\" alt=\"Major\" name=\"select_option_div\" title=\"Major\"  class=\"imgbutton\" style=\"width:12px\"  /><span style=\"line-height:20px; padding:0px 12px;cursor:pointer;cursor:hand; \">Major</span>")
	html.write("</div>")
	html.write("</td>")
...
def trap_filter_function(h):
	...
	event_id=html.var("event_id") # event_id take Event Type name id by html object(Trap_information page).
	...
	serevity1=html.var("serevity1") # serevity variable take serevity value by html object(Trap_information page).
	...
	if agent_id == "" or agent_id == None:
	        sql="SELECT ta."+primary_key_id+",ta.event_id,ta.agent_id,ta.trap_receive_date,ta.serevity,ta.trap_event_id,ta.trap_event_type,hosts.device_type_id,hosts.host_alias From "+table_name+" as ta INNER JOIN hosts ON ta.agent_id=hosts.ip_address WHERE "+sql
	else:
		if "-" in agent_id:
		        sql="SELECT ta."+primary_key_id+",ta.event_id,ta.agent_id,ta.trap_receive_date,ta.serevity,ta.trap_event_id,ta.trap_event_type,hosts.device_type_id,hosts.host_alias From "+table_name+" as ta INNER JOIN hosts ON ta.agent_id=hosts.ip_address WHERE agent_id='"+agent_id.replace("-","")+"' and "+sql
		else:
		        sql="SELECT "+primary_key_id+",ta.event_id,ta.agent_id,ta.trap_receive_date,ta.serevity,ta.trap_event_id,ta.trap_event_type,hosts.device_type_id,hosts.host_alias From "+table_name+" as ta INNER JOIN hosts ON ta.agent_id=hosts.ip_address WHERE agent_id like  '"+agent_id+"%' and "+sql
	...
	try:
		...
		sql+="AND ta.timestamp BETWEEN '%s' AND '%s' order by trap_receive_date desc"%(start_datetime,end_datetime)
		cursor.execute(sql)
		...
		if result is not None:
			if len(result)>0:
				for row in result:
					i+=1
					img = '<label style="display:none;">%s</label><img src="%s" alt="%s" title="%s" class="imgbutton" style="width:12px" onclick="alarmDetail(\'%s\')"/>'%(row[4],image_dic[int(row[4])],image_title_name[int(row[4])],image_title_name[int(row[4])],row[0])
...
def page_tip_event_details(h):
        ...
        "<div><strong>Events Details</strong> has shown all Events information for all UBR(devices).</div>"\
        "<br/>"\
	...
	"<div><input class=\"table_option\" type=\"radio\" width=\"12px\" checked=\"checked\"\> This option button show the current,clear,history information on click of option button.</div>"\
        "<div class=\"action-tip\"><div class=\"img-div img-div2\"><img style=\"width:16px;height:16px;\" src=\"images/status-1.png\"/></div><div class=\"txt-div\">Critical / Major events</div></div>"\
        "<div class=\"action-tip\"><div class=\"img-div img-div2\"><img style=\"width:16px;height:16px;\" src=\"images/minor1.png\"/></div><div class=\"txt-div\">Minor events</div></div>"\
        "<div class=\"action-tip\"><div class=\"img-div img-div2\"><img style=\"width:16px;height:16px;\" src=\"images/normal1.png\"/></div><div class=\"txt-div\">Informationl / Normal events</div></div>"\
	...
	"<div><input  class=\"yo-button yo-small\" type=\"button\" value=\"submit\"\> This button search the event information according to your searching requirement.</div>"\
        "<br/>"\
 	"<div><strong>Note:</strong>This Events Details page show all events information for all UBR devices, this is also provide advance filtering of Events.\
        </div>"\
        "</div>"
