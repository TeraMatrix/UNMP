...
#import #logging
##logging.basicConfig(filename='/omd/daemon/demo_rrd.log',format='%(levelname)s: %(asctime)s >> %(message)s', level=#logging.DEBUG)
...
class RRDGraph(object):
    ...
    def write_tmp(self,file_path,given_table_dict=None,ip=None,community=None,port=None, version=None,action=None,time_out=1):
        ...
        if file_flag:
            ...
            if action == "start":
                ...
                if file_table_dict.has_key(key):
            ...
            if action == "start":
                st += "\ntable_dict = { '%s' : %s } "%(key,given_table_dict[key])
        st += "\ntimeout=%s"%(time_out)
#        if self.stop_it == 0:
        f = open(file_path,'w')
        f.write(st)
    ...
    def rrd(self,table,action='start'):
        ...
        if isfile(RRD_CONF_FILE):
            ...
            exec f.read()
            f.close()
            self.rrd_step = int(rrd_param.get("rrd_step",5))
            self.rrd_start = rrd_param.get("rrd_start","N")
            self.root_path = rrd_param.get("rrd_file_path","/omd/daemon/rrd/" )
            t1 = time()
            ...
            if isinstance(graph, dict) and self.device_type != None and self.ip_address != None:
                # files path
                tmp_file_path =  self.root_path + self.device_type + "/" + ''.join(self.ip_address.split('.')) + '.tmp'
                rrd_file_path = self.root_path + ''.join(self.ip_address.split('.')) + "_" + self.device_type + "_" + graph["rrd_file_name"]
                ...
                if isfile(rrd_file_path):
                    ...
                    self.create_rrd(rrd_file_path,self.rrd_start,self.rrd_step,ds,rra)
                ...
                try:
                    ...
                    if(graph["is_snmp"] == True):
                        table_dict = {str(table):{'row':graph["row_index"],'col':graph["column_index"],'oid':graph["oid_table"],'file':rrd_file_path}}
                        if isfile(tmp_file_path): #'file exist'
                            #logging.info(' file hai '+str(tmp_file_path))
                            self.write_tmp(tmp_file_path, table_dict, self.ip_address, self.community, self.port, self.version, action)
                            if action == 'stop':
                                if self.stop_it:
                                    if os.path.isfile(tmp_file_path):
                                        os.unlink(tmp_file_path)
                                    subprocess.Popen([LIVE_MONITORING_FILE,'stop',tmp_file_path],stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                                    output, err = popen_obj.communicate()
                                    out = output
                                    return
                            popen_obj = subprocess.Popen([LIVE_MONITORING_FILE,'status',tmp_file_path],stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                            output, err = popen_obj.communicate()
                            if output.find('Service with the PID') != -1:
                                pass #logging.info(' is running hai ')
                            else:
                                subprocess.Popen([LIVE_MONITORING_FILE,'start',tmp_file_path],stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                                output, err = popen_obj.communicate()
                                out = output
                                return
                        else:
                            #logging.info(' no file so start '+tmp_file_path+action)
                            self.write_tmp(tmp_file_path, table_dict, self.ip_address, self.community, self.port, self.version,action)
                            popen_obj = subprocess.Popen([LIVE_MONITORING_FILE,'start',tmp_file_path],stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                            output, err = popen_obj.communicate()
                            out = output
                            return
                except Exception,e:
                    ...
                    #popen call    #self.sleep_time # rrd_file_name, graph["timestamp"]
...
#rr = RRDGraph("odu100","172.22.0.34","public",161,"2c")
#rr.rrd("sync_loss","stop")
