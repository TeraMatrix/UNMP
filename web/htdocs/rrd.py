		    ...
		    if isinstance(graph, dict) and self.device_type != None and self.ip_address != None:
		        ...
		        try:
		            ...
		            if(graph["is_snmp"] == True):
		                table_dict = {str(table):{'row':graph["row_index"], 'col':graph["column_index"], 'oid':graph["oid_table"], 'file':rrd_file_path, 'row_count':graph["row_count"], 'unreachable_value':graph["unreachable_value"]}}
		                #logging.info(" in try internal  "+str(table_dict))
		                if isfile(tmp_file_path): #'file exist'
		                    #logging.info(' file hai '+str(tmp_file_path))
		                    self.write_tmp(tmp_file_path, table_dict, self.ip_address, self.community, self.port, self.version, action,self.rrd_step)
		                    if action == 'stop':
		                        if self.stop_it:
