...
def create_backup():
    try:
        ...
        print path_temp 
        str_time="timestamp between '%s-%s-01 00:00:00' and '%s-%s-31 23:59:59' " %(ye,mon,ye,mon)
        path_sh='/omd/sites/%s/share/check_mk/web/htdocs/mysql_backup.sh %s %s' % (nms_instance,path_temp,str_time)
...
def restore_backup():
    try:
        ...
        out.close()
        path_temp='/omd/daemon/mysql_temp/%s_%s' %(ye,mon)
        path_sh='/omd/sites/%s/share/check_mk/web/htdocs/mysql_restore.sh %s' % (nms_instance,path_temp)
        subprocess.Popen("sh %s" % path_sh,shell=True)
        return "done"
    except Exception,e:
        return str(e)
...
class AnalyzedReportBll(object):
    ...
    def analyzed_reporting_get_column_template(self,device_type_id,report_type):
        try:
            ...
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            sql=" select column_selected,column_non_selected from report_template_analyzed where device_type='%s' and report_type='%s' "%(device_type_id,report_type)
            cursor.execute(sql)
            result=cursor.fetchall()
            lis=[]
    ...
    def analyzed_reporting_get_column_range(self,device_type_id,report_type,range_type):
        try:
            ...
            #	range_type="Range"
            sql=" select column_selected,column_non_selected from report_template_analyzed where device_type='%s' and report_type='%s' and type='%s' "%(device_type_id,report_type,range_type)
            cursor.execute(sql)
            result=cursor.fetchall()
            lis=[]
            lis.append(result[0][0])
    ...
    def analyzed_reporting_get_mapping_column(self,device_type_id,report_type,range_type):
        try:
            ...
            sql=" select column_selected,column_non_selected,mapping_selected,mapping_non_selected, \
                  sheet_name,main_title,second_title,report_name,generate_method from report_template_analyzed where device_type='%s' and report_type='%s' and type='%s'"%(device_type_id,report_type,range_type)
            cursor.execute(sql)
            result=cursor.fetchall()
            lis=[]
            #return sql
    ...
    def get_query_dict(self,device_type,report_type,date_temp_1,date_temp_2,all_host,range_type,range_duration):
        try:
            ...
            if((report_type=="RSSI" or report_type=="RSL") and range_type=="Average"):
                range_type="Total" 
            sql=" select * from report_query_dict_analyzed where report_type='%s' and device_type='%s' and range_type='%s'"%(report_type,device_type,range_type)
            cursor.execute(sql)
            result=cursor.fetchall()
            query_dict={
                ...
                "columns":result[0][4],
                "join" :result[0][6],
                "where" : str(result[0][7]) %(date_temp_1,date_temp_2,"' , '".join(all_host.split(',')),range_duration),
                "group_by": result[0][8],
                "order_by" : result[0][9],
            }
    ...
    def pagination_create_table(self,query_dict,i_display_start,i_display_length,s_search,sEcho,req_vars):
        ...
        try:
            ...
            # SQL queries
            # Get data to display
            sql_query = "SELECT SQL_CALC_FOUND_ROWS %s FROM %s %s %s %s %s" % (", ".join(a_columns).replace(" , "," "),s_table,s_join,s_where,s_order,s_limit)
            #return sql_query
...
#		return {"succcess" : 1 , "result":str(e) }
    ...
    def analyzed_reporting_get_excel(self,date1,date2,time1,time2,all_host,report_type,column_user,device_type,all_group,view_type,range_type,range_duration,i_display_start,i_display_length,s_search,sEcho,sSortDir_0,iSortCol_0,html_req,username):
        try:
            direct=""
            ...
            result_columns=re.analyzed_reporting_get_mapping_column(device_type,report_type,range_type)
            column_value=[]
            column_key=[]
            key_user=[]
            data_report={}
            ...
            if(result_columns["success"]==0 or result_columns["success"]=="0"):
                ...
                if 1==1:
                    ...
                    query_dict=re.get_query_dict(device_type,report_type,date_temp_1,date_temp_2,all_host,range_type,range_duration)
                    if(str(query_dict["success"])=="0"):
                        if view_type=="data_table":
                            res_sql=re.pagination_create_table(query_dict["result"],i_display_start,i_display_length,s_search,sEcho,html_req)
                            #return res_sql
                        else:
                        ...
                        if(str(res_sql["success"])=="0"):
                            if(view_type=="data_table"):
                                ...
                                iTotalDisplayRecords=res_sql["iTotalDisplayRecords"]
                            else:
                                data_report=res_sql["result"]
                        else:
    ...
    def get_excel_sheet(self,*params):
        try:
            ...
            result_dict={"success":"0","result":"report successfully generated","file":name_report}
            return result_dict
        except Exception,e:
            result_dict={"success":"1","result":str(e)}
            return result_dict
