...
class SPStatusView(object):
    ...
    def header_buttons():
        ...
        cancel_btn = "<div class=\"header-icon\"><img onclick=\"backListing();\" class=\"n-tip-image\" src=\"images/back.png\" id=\"del_host\" style=\"width: 16px; height: 16px; margin: 6px 20px 6px 10px;\" original-title=\"Back To Lising\"></div>".format(theme)
    ...
    def sp_footer_tab(flag):
        if int(flag)==0:
         ...
         <td style="text-align:left"><button type=\"submit\" id=\"sp_excel_report\" class=\"yo-button\" style=\"margin-top:5px;\" onclick="spExcelStatusReportGeneration();"><span class=\"report\">Excel</span></button></td>\
         <td style="text-align:left"><button type=\"submit\" id=\"sp_csv_report\" class=\"yo-button\" style=\"margin-top:5px;\" onclick="spCSVStatusReportGeneration();"><span class=\"report\">CSV</span></button></td>\
            </tr></table>\
            </div></div>\
            '
        else:
...
# This is for latest alarm
#        <td width="50%%">\
#        <div class="db-box" id=\"alarm_table\">\
#        <div class="db-head">\
#        <span>Device Alarms</span>\
#        </div>\
#        <div class="db-body">\
#        <div class="db-container" style="height: 250px;" id="alarm_dashboard">\
#        </div>\
#       </div>\
#        </div>\
#        </td>\

#<table border=\"0\" cellspacing=\"10px\" cellpadding=\"0\" width=\"100%%\" ><tr><td><div id=\'trap_dashboard\'></div></td></tr></table>\
#        <input type=\"button\" class=\"yo-small  yo-button\" name=\"sp_graph_show\" value=\"graph\" id=\"sp_graph_show\" style=\"width:50px;\"/>\

    @staticmethod
    def device_information_view(result,ip_address):
        # this is common list
        last_reboot_time=''
        column_list={'odu16':['Frequency','Slave','Active Version','Hardware Version','Last Reboot Reason','Channel','Operation state','Node Type','MAC Address','Last Reboot Time'],'odu100':['Frequency','Slave','Active Version','Hardware Version','Last Reboot Reason','Channel','Operation state','Node Type','MAC Address','Last Reboot Time']}
...
#        channel={0:'raBW5Mhz',1:'raBW10Mhz',2:'raBW20Mhz'}
	    ...
	    if int(master_slave_result['success'])==0 and int(master_slave_result['status']) ==0:
	    	master_slave_status=str('--' if result['result'][1]==None or result['result'][1]==""  else result['result'][1])
	    else:
	    	master_slave_status='N/A'
            if  len(result['result'])>0: 
                last_reboot_time='--' if result['last_reboot_time']==None or len(result['last_reboot_time'])==0 else result['last_reboot_time'][0][0]  
                ...
                device_detail+='<tbody>\
                            ...
                            <td class="cell-info">'+str(master_slave_status)+'</td>\
                            </tr>\
                            <tr>\
                            <td class="cell-label">\
                                Active Version\
            ...
            if result!=None and len(result) >0:
                ...
                device_detail+='<tbody>\
                            ...
                            <td class="cell-info" colspan="3">'+connected_user+'</td>\
                            </tr>\
                        <tbody></table>'
            else:
                device_detail='<table class="tt-table" cellspacing="0" cellpadding="0" width="100%">'
    ...
    def sp_event_alarm_table_view(result,ip_address):
        ...
        history_trap +='\
            <colgroup><col style="width:5%;" /><col style="width:15%;" /><col style="width:12%;" /><col style="width:12%;" /><col style="width:12%;" /><col style="width:12%;" /><col style="width:35%;" />\
  </colgroup>\
        ...
        for i in range(length):
            if i<5:
                ...
                history_trap+='<td>%s</td>'%(datetime.strptime(all_traps[i][5],'%a %b %d %H:%M:%S %Y')).strftime('%d %b %Y %H:%M:%S')
                history_trap+='<td>%s</td>'%all_traps[i][4]
                history_trap+='<td>%s</td>'%all_traps[i][1]
                history_trap+='<td>%s</td>'%all_traps[i][2]
                history_trap+='<td>%s</td>'%all_traps[i][3]
                history_trap+='<td style="text-align:left;">%s</td></tr>'%all_traps[i][6]
            else:
                history_trap+='<tr><td class="vertline"><img src=\"%s\" alt=\"%s\" title=\"%s\" class=\"imgbutton\" style=\"width:13px;\"/></td>'% (image_dic[all_traps[i][0]],image_title_name[all_traps[i][0]],image_title_name[all_traps[i][0]])
                history_trap+='<td >%s</td>'%(datetime.strptime(all_traps[i][5],'%a %b %d %H:%M:%S %Y')).strftime('%d %b %Y %H:%M:%S')
                history_trap+='<td>%s</td>'%all_traps[i][4]
                ...
                history_trap+='<td>%s</td>'%all_traps[i][2]
                history_trap+='<td>%s</td>'%all_traps[i][3]
                history_trap+='<td style="text-align:left;">%s&nbsp&nbsp&nbsp&nbsp%s</td></tr>'%(all_traps[i][6],(("<a href=\"status_snmptt.py?trap_status=history&ip_address="+ip_address+"\">more>></a>" if len(all_traps) >6 else "" )))

        if len(all_traps)<1:
            history_trap+='<tr ><td colspan="4"><b>Events does not exists.</b></td></tr>'
        ...
        for i in range(length):
            if i<5:
                ...
                current_alarm_html+='<td>%s</td>'%(datetime.strptime(current_alarm[i][5],'%a %b %d %H:%M:%S %Y')).strftime('%d %b %Y %H:%M:%S')
                current_alarm_html+='<td>%s</td>'%current_alarm[i][3]
                current_alarm_html+='<td style="text-align:left;">%s</td></tr>'%current_alarm[i][6]
            else:
                current_alarm_html+='<tr><td class="vertline"><img src=\"%s\" alt=\"%s\" title=\"%s\" class=\"imgbutton\" style=\"width:13px;\"/></td>'% (image_dic[current_alarm[i][0]],image_title_name[current_alarm[i][0]],image_title_name[current_alarm[i][0]])
                current_alarm_html+='<td>%s</td>'%(datetime.strptime(current_alarm[i][5],'%a %b %d %H:%M:%S %Y')).strftime('%d %b %Y %H:%M:%S')
                current_alarm_html+='<td>%s</td>'%current_alarm[i][3]
                current_alarm_html+='<td style="text-align:left;">%s&nbsp&nbsp%s</td></tr>'%(current_alarm[i][6],(("<a href=\"status_snmptt.py?trap_status=history&ip_address="+ip_address+"\">more>></a>" if len(current_alarm) >6 else "" )))
