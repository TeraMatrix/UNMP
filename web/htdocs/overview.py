...
import config, htmllib, pprint, sidebar, views, time, defaults, os, cgi, xml.dom.minidom, subprocess, MySQLdb
from lib import *
...
def network_overview(h):
     ...
     html.write("<a id=\"pingButton\" href=\"#pingDiv\" class=\"tab-button\">Ping")
     ...
     html.write("</h2>")
     html.write("<a id=\"optionMenuButton\" href=\"#\" class=\"tab-button\" style=\"float:right;\">")
...
def discoveredHostDetailsForAll(h):
     ...
     checkboxState = ""
     host_list = []
     dom_sdm = xml.dom.minidom.parse("/omd/sites/%s/share/check_mk/web/htdocs/xml/sdmcDiscovery.xml" % site)
     dom_snmp = xml.dom.minidom.parse("/omd/sites/%s/share/check_mk/web/htdocs/xml/snmpDiscovery.xml" % site)
     dom_upnp = xml.dom.minidom.parse("/omd/sites/%s/share/check_mk/web/htdocs/xml/upnpDiscovery.xml" % site)
     dom_ping = xml.dom.minidom.parse("/omd/sites/%s/share/check_mk/web/htdocs/xml/pingDiscovery.xml" % site)
     ...
     discoveredHostsString = "<div><table width=\"60%\" class=\"host-table\"><tbody><tr><th align=\"left\"><input type=\"checkbox\" id=\"allChecked\" name=\"allChecked\" value=\"all\"" + checkboxState + "/></th><th align=\"left\">Host Name</th><th align=\"left\">Host Alias</th><th align=\"left\">Host Address</th><th align=\"left\">Device Type</th></tr>"
     ...
     for host in discoveredHosts:
          if list_search(host_list,host.getAttribute("address")) == -1:
               i += 1
               discoveredHostsString += "<tr><td><input type=\"checkbox\" class=\"sdm\" id=\"host" + str(i) + "\" name=\"host\" value=\"" + host.getAttribute("name") + "\" " + checkboxState + "/></td><td>" + host.getAttribute("name") + "</td><td>" + host.getAttribute("alias") + "</td><td>" + host.getAttribute("address") + "</td><td>" + deviceTypeOption(host.getAttribute("type"),i) + "</td></tr>"
               host_list.append(host.getAttribute("address"))
          else:
               dom_sdm.getElementsByTagName("discoveredHosts")[0].removeChild(host)
               fwxml = open("/omd/sites/%s/share/check_mk/web/htdocs/xml/sdmcDiscovery.xml" % site, "w")
     ...
     for host in discoveredHosts:
          if list_search(host_list,host.getAttribute("address")) == -1:
               i += 1
               discoveredHostsString += "<tr><td><input type=\"checkbox\" class=\"snmp\" id=\"host" + str(i) + "\" name=\"host\" value=\"" + host.getAttribute("name") + "\" " + checkboxState + "/></td><td>" + host.getAttribute("name") + "</td><td>" + host.getAttribute("alias") + "</td><td>" + host.getAttribute("address") + "</td><td>" + deviceTypeOption(host.getAttribute("type"),i) + "</td></tr>"
               host_list.append(host.getAttribute("address"))
          else:
               dom_snmp.getElementsByTagName("discoveredHosts")[0].removeChild(host)
               fwxml = open("/omd/sites/%s/share/check_mk/web/htdocs/xml/snmpDiscovery.xml" % site, "w")
     ...
     for host in discoveredHosts:
          if list_search(host_list,host.getAttribute("address")) == -1:
               i += 1
               discoveredHostsString += "<tr><td><input type=\"checkbox\" class=\"upnp\" id=\"host" + str(i) + "\" name=\"host\" value=\"" + host.getAttribute("name") + "\" " + checkboxState + "/></td><td>" + host.getAttribute("name") + "</td><td>" + host.getAttribute("alias") + "</td><td>" + host.getAttribute("address") + "</td><td>" + deviceTypeOption(host.getAttribute("type"),i) + "</td></tr>"
               host_list.append(host.getAttribute("address"))
          else:
               dom_upnp.getElementsByTagName("discoveredHosts")[0].removeChild(host)
               fwxml = open("/omd/sites/%s/share/check_mk/web/htdocs/xml/upnpDiscovery.xml" % site, "w")
     ...
     for host in discoveredHosts:
          if list_search(host_list,host.getAttribute("address")) == -1:
               i += 1
               discoveredHostsString += "<tr><td><input type=\"checkbox\" class=\"ping\" id=\"host" + str(i) + "\" name=\"host\" value=\"" + host.getAttribute("name") + "\" " + checkboxState + "/></td><td>" + host.getAttribute("name") + "</td><td>" + host.getAttribute("alias") + "</td><td>" + host.getAttribute("address") + "</td><td>" + deviceTypeOption(host.getAttribute("type"),i) + "</td></tr>"
               host_list.append(host.getAttribute("address"))
          else:
               dom_ping.getElementsByTagName("discoveredHosts")[0].removeChild(host)
               fwxml = open("/omd/sites/%s/share/check_mk/web/htdocs/xml/pingDiscovery.xml" % site, "w")
     ...
     discoveredHostsString += "<input type=\"hidden\" name=\"totalHost\" value=\"" + str(i) + "\"/></tbody></table></div>"
...
def dicoveredHostDetailsForPing(h):
     ...
     html = h
     site = __file__.split("/")[3]
     dom = xml.dom.minidom.parse("/omd/sites/%s/share/check_mk/web/htdocs/xml/pingDiscovery.xml" % site)
     discoveryString = "<div>"
     ...
     for dis in discovery:
          ...
          if int(dis.getAttribute('active').strip()) == 1:
               checkboxState = " disabled=\"disabled\" "
               discoveryString += "<div class=\"msg-head\"><img src=\"images/loading-small.gif\" alt=\"\" style=\"vertical-align: middle;\" /><span> Discovery " + dis.getAttribute('complete') + "% Completed...</span></div><input type=\"hidden\" id=\"pingStatus\" name=\"pingStatus\" value=\"1\"/>"
          else:
               if float(dis.getAttribute('complete')) < 100:
                    discoveryString += "<div class=\"msg-head\"><img src=\"images/loading-small-stop.gif\" alt=\"\" style=\"vertical-align: middle;\" /><span> Discovery " + dis.getAttribute('complete') + "% Completed... [stopped]</span></div><input type=\"hidden\" id=\"pingStatus\" name=\"pingStatus\" value=\"0\"/>"
               else:
...
def dicoveredHostDetailsForSnmp(h):
     ...
     html = h
     site = __file__.split("/")[3]
     dom = xml.dom.minidom.parse("/omd/sites/%s/share/check_mk/web/htdocs/xml/snmpDiscovery.xml" % site)
     discoveryString = "<div>"
     discovery = dom.getElementsByTagName("discovery")
     discoveredHosts = dom.getElementsByTagName("host")
     checkboxState = ""
     ...
     for dis in discovery:
          ...
          if int(dis.getAttribute('active').strip()) == 1:
               checkboxState = " disabled=\"disabled\" "
               discoveryString += "<div class=\"msg-head\"><img src=\"images/loading-small.gif\" alt=\"\" style=\"vertical-align: middle;\" /><span> Discovery " + dis.getAttribute('complete') + "% Completed...</span></div><input type=\"hidden\" id=\"snmpStatus\" name=\"snmpStatus\" value=\"1\"/>"
          else:
               if float(dis.getAttribute('complete')) < 100:
                    discoveryString += "<div class=\"msg-head\"><img src=\"images/loading-small-stop.gif\" alt=\"\" style=\"vertical-align: middle;\" /><span> Discovery " + dis.getAttribute('complete') + "% Completed... [stopped]</span></div><input type=\"hidden\" id=\"snmpStatus\" name=\"snmpStatus\" value=\"0\"/>"
               else:
...
def dicoveredHostDetailsForUPnP(h):
     ...
     html = h
     site = __file__.split("/")[3]
     dom = xml.dom.minidom.parse("/omd/sites/%s/share/check_mk/web/htdocs/xml/upnpDiscovery.xml" % site)
     discoveryString = "<div>"
     discovery = dom.getElementsByTagName("discovery")
     discoveredHosts = dom.getElementsByTagName("host")
     checkboxState = ""
     ...
     for dis in discovery:
          ...
          if int(dis.getAttribute('active').strip()) == 1:
               checkboxState = " disabled=\"disabled\" "
               discoveryString += "<div class=\"msg-head\"><img src=\"images/loading-small.gif\" alt=\"\" style=\"vertical-align: middle;\" /><span> Discovery " + dis.getAttribute('complete') + "% Completed...</span></div><input type=\"hidden\" id=\"upnpStatus\" name=\"upnpStatus\" value=\"1\"/>"
          else:
               if float(dis.getAttribute('complete')) < 100:
                    discoveryString += "<div class=\"msg-head\"><img src=\"images/loading-small-stop.gif\" alt=\"\" style=\"vertical-align: middle;\" /><span> Discovery " + dis.getAttribute('complete') + "% Completed... [stopped]</span></div><input type=\"hidden\" id=\"upnpStatus\" name=\"upnpStatus\" value=\"0\"/>"
               else:
...
def dicoveredHostDetailsForSDMC(h):
     ...
     html = h
     site = __file__.split("/")[3]
     dom = xml.dom.minidom.parse("/omd/sites/%s/share/check_mk/web/htdocs/xml/sdmcDiscovery.xml" % site)
     discoveryString = "<div>"
     discovery = dom.getElementsByTagName("discovery")
     discoveredHosts = dom.getElementsByTagName("host")
     checkboxState = ""
     ...
     for dis in discovery:
          ...
          if int(dis.getAttribute('active').strip()) == 1:
               checkboxState = " disabled=\"disabled\" "
               discoveryString += "<div class=\"msg-head\"><img src=\"images/loading-small.gif\" alt=\"\" style=\"vertical-align: middle;\" /><span> Discovery " + dis.getAttribute('complete') + "% Completed...</span></div><input type=\"hidden\" id=\"sdmcStatus\" name=\"sdmcStatus\" value=\"1\"/>"
          else:
               if float(dis.getAttribute('complete')) < 100:
                    discoveryString += "<div class=\"msg-head\"><img src=\"images/loading-small-stop.gif\" alt=\"\" style=\"vertical-align: middle;\" /><span> Discovery " + dis.getAttribute('complete') + "% Completed... [stopped]</span></div><input type=\"hidden\" id=\"sdmcStatus\" name=\"sdmcStatus\" value=\"0\"/>"
               else:
     ...
     for dis in discovery:
          ...
          if int(dis.getAttribute('active').strip()) == 1:
               ...
               if float(dis.getAttribute('complete')) < 100:
                    ...
                    discoveryString += "<div class=\"msg-head\"><span>Discovery 100% done</span></div><input type=\"hidden\" id=\"pingStatus\" name=\"pingStatus\" value=\"100\"/>"
          discoveryDetailString += "<tr><td>Start IP Range <td/><td>" + dis.getAttribute('ip') + "." + dis.getAttribute('start') + "</td></tr>"
          discoveryDetailString += "<tr><td>End IP Range <td/><td>" + dis.getAttribute('ip') + "." + dis.getAttribute('end') + "</td></tr>"
          discoveryDetailString += "<tr><td>Timeout<td/><td>" + dis.getAttribute('timeout') + " sec.</td></tr>"
          discoveryDetailString += "<tr><td>Hostgroups<td/><td>" + dis.getAttribute('hostgroup') + "</td></tr>"
          ...
          discoveryDetailString += "<tr><td>User <span style=\"font-size:9px;\">(who set the details)</span><td/><td>" + dis.getAttribute('username') + "</td></tr>"
...
def discoveryDetailsForSnmp(h):
     ...
     for dis in discovery:
          ...
          discoveryDetailString += "<tr><td>Start IP Range <td/><td>" + dis.getAttribute('ip') + "." + dis.getAttribute('start') + "</td></tr>"
          discoveryDetailString += "<tr><td>End IP Range <td/><td>" + dis.getAttribute('ip') + "." + dis.getAttribute('end') + "</td></tr>"
          discoveryDetailString += "<tr><td>Community<td/><td>" + dis.getAttribute('community') + "</td></tr>"
          discoveryDetailString += "<tr><td>Port<td/><td>" + dis.getAttribute('port') + "</td></tr>"
          discoveryDetailString += "<tr><td>Version<td/><td>" + dis.getAttribute('version') + "</td></tr>"
          discoveryDetailString += "<tr><td>Timeout<td/><td>" + dis.getAttribute('timeout') + " sec.</td></tr>"
          discoveryDetailString += "<tr><td>Hostgroups<td/><td>" + dis.getAttribute('hostgroup') + "</td></tr>"
          if int(dis.getAttribute('service')) == 1:
               discoveryDetailString += "<tr><td>Service<td/><td>Create service using port scanning</td></tr>"
...
def discoveryDetailsForSDMC(h):
     ...
     for dis in discovery:
          ...
          discoveryDetailString += "<tr><td>Timeout<td/><td>" + dis.getAttribute('timeout') + " sec.</td></tr>"
          discoveryDetailString += "<tr><td>Hostgroups<td/><td>" + dis.getAttribute('hostgroup') + "</td></tr>"
          discoveryDetailString += "<tr><td>Selected Device<td/><td>" + dis.getAttribute('device') + "</td></tr>"
          if int(dis.getAttribute('service')) == 1:
               discoveryDetailString += "<tr><td>Service<td/><td>Create service using port scanning</td></tr>"
          elif int(dis.getAttribute('service')) == 2:
               discoveryDetailString += "<tr><td>Service<td/><td>Create service using service template</td></tr>"
     ...
     if discoveryXmlFilePath != "":
          ...
          for dis in discovery:
               if discoveryType == "ping":
                    ...
                    timeOut = dis.getAttribute("timeout")
          if discoveryType == "ping":
               args = [ipBase + "." + ipRangeStart, ipBase + "." + ipRangeEnd, timeOut, discoveryXmlFilePath]
               command = [perlFile]
               command.extend(args)
               pipe = subprocess.Popen(command, stdout=subprocess.PIPE).communicate()[0]
          elif discoveryType == "snmp":
               args = [ipBase + "." + ipRangeStart, ipBase + "." + ipRangeEnd, community, timeOut, version, port, snmpUserName, authKey, snmpPassword, authProtocol, privKey, privPassword, privProtocol, discoveryXmlFilePath]
               command = [perlFile]
               command.extend(args)
               pipe = subprocess.Popen(command, stdout=subprocess.PIPE).communicate()[0]
          elif discoveryType == "upnp":
...
def discoveryAndNetworkDetails(h):
     ...
     for site, hostgroup, alias in hostgroups:
          hostgroup_i += 1
          nmsTree +="<li id=\"" + hostgroup + "\" rel=\"folder\" class=\"jstree-open\"><ins class=\"jstree-icon\">&nbsp;</ins><a href=\"#\" class=\"\" style=\"-moz-user-select: none;\"><ins class=\"jstree-icon\">&nbsp;</ins>" + alias + "</a><ul style=\"\">"
          html.live.set_prepend_site(True)
          ...
          for host_site, host, alias in hosts:
               ...
               for service_site, description, state, command in services:
                    service_i = 0
                    nmsTree +="<li id=\"" + host + "_service_" + str(service_i) + "\" rel=\"default\" class=\"jstree-leaf\"><ins class=\"jstree-icon\">&nbsp;</ins><a href=\"#\" class=\"\" style=\"-moz-user-select: none;\"><ins class=\"jstree-icon\">&nbsp;</ins>" + description + "</a></li>"
               nmsTree += "</ul></li>"
...
def createHostConfiguration(h):
     ...
     dom = xml.dom.minidom.parseString("<nmsDB><autoDiscovery><discovery/></autoDiscovery><discoveredHosts></discoveredHosts></nmsDB>")
     if discoveryXmlFilePath != "":
          dom = xml.dom.minidom.parse(discoveryXmlFilePath)
          ...
          for host in hosts:
               ...
               for hDom in hostDom:
                    if hDom.getAttribute("name").strip() == host:
                         ...
                         dom.getElementsByTagName("discoveredHosts")[0].removeChild(hDom)
                         break
               html.live.set_prepend_site(True)
               query = "GET hosts\nColumns: name\nFilter: name = " + host
               ...
               if(checkfile == 1 or countHost == 0):
                    ...
                    for col in result:
                         totalRow = col[0]
                    if totalRow == 0:
                         sql = "INSERT INTO nms_devices (devicetype,ipaddress,username,password,port,hostname,created_by) VALUES ('%s','%s','%s','%s','%s','%s','%s')" % (deviceType[ind],address,"","","",host,config.user)
                         cursor.execute(sql)
                         db.commit()
               ind += 1
     except:
...
def createSeviceConfiguration(h):
     ...
     if serviceManagement.strip() == "1":
          ...
          if discoveryType == "sdmc":
               ...
               shyamDevicesDom = xml.dom.minidom.parseString("<shyamDevices></shyamDevices>")
               if(os.path.isfile("/omd/sites/%s/share/check_mk/web/htdocs/xml/shyamdevices.xml" % site)):
                    shyamDevicesDom = xml.dom.minidom.parse("/omd/sites/%s/share/check_mk/web/htdocs/xml/shyamdevices.xml" % site)
               ...
               if(os.path.isfile("/omd/sites/%s/share/check_mk/web/htdocs/xml/service_template.xml" % site)):
                    templateDom = xml.dom.minidom.parse("/omd/sites/%s/share/check_mk/web/htdocs/xml/service_template.xml" % site)
               ...
               for host in hosts:
                    ...
                    for sdDom in shyamDevicesDom.getElementsByTagName("device"):
                         if sdDom.getAttribute("sdmcDiscoveryValue").strip() == hostType:
                              serviceTemplateId = sdDom.getAttribute("applyedServiceTemplate")
                              break
                    for tDom in templateDom.getElementsByTagName("host"):
                         if tDom.getAttribute("id").strip() == serviceTemplateId:
                              for stDom in tDom.getElementsByTagName("service"):
                                   new_check_command = getText(stDom.getElementsByTagName("check_command")[0].childNodes).strip().replace("$IPADDRESS$",host)
                                   ...
                                   if(checkfile == 1 or countService == 0):
                                        serviceConfig += "\n#service-" + host + "-" + new_check_command + "\n"
                                        serviceConfig += "define service {\n"
                                        serviceConfig += "\tuse\t\t\t" + getText(stDom.getElementsByTagName("use")[0].childNodes).strip() + "\n"
                                        serviceConfig += "\thost_name\t\t" + host + "\n"
                                        serviceConfig += "\tservice_description\t" + getText(stDom.getElementsByTagName("service_description")[0].childNodes).strip() + "\n"
                                        serviceConfig += "\tmax_check_attempts\t\t\t" + getText(stDom.getElementsByTagName("max_check_attempts")[0].childNodes).strip() + "\n"
                                        serviceConfig += "\tnormal_check_interval\t\t\t" + getText(stDom.getElementsByTagName("normal_check_interval")[0].childNodes).strip() + "\n"
                                        serviceConfig += "\tretry_check_interval\t\t\t" + getText(stDom.getElementsByTagName("retry_check_interval")[0].childNodes).strip() + "\n"
                                        serviceConfig += "\tnotification_interval\t\t\t" + getText(stDom.getElementsByTagName("notification_interval")[0].childNodes).strip() + "\n"
                                        serviceConfig += "\tcheck_command\t\t\t" + new_check_command + "\n"
                                        serviceConfig += "}\n"
                                        serviceConfig += "#endservice-" + host + "-" + new_check_command + "\n"
                              break
...
def deviceTypeOption(dtype,dtypeId):
     strDeviceType = "<select name='deviceType' id='deviceType" + str(dtypeId) + "'>"
