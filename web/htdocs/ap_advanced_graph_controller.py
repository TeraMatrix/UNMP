...
def get_ap_advanced_graph_value(h):
    global html 
    html=h
    device_type_dict={'ap25':'AP25','odu16':'RM18','odu100':'RM','idu4':'IDU','ccu':'CCU'}
    ...
    css_list = ["css/style.css","css/custom.css","calendrical/calendrical.css","css/demo_table_jui.css","css/jquery-ui-1.8.4.custom.css"]
    javascript_list = ["js/highcharts.js","js/apAdvancedGraph.js","calendrical/calendrical.js","js/jquery.dataTables.min.js"]
    html.new_header('%s %s Historical Graphs'%(device_type_dict[device_type_id],ip_address.replace("'","")),selected_listing,"",css_list,javascript_list)    
    html_content=APAdvancedView.ap_set_variable(ip_address,device_type_id,user_id)
...
def ap_total_graph_name(h):
    ...
    controller_dict=APAdvancedView.graph_name_listing(result_dict,ip_address)
...
def advanced_graph_json_creation(h):
    ...
    controller_dict=bll_obj.advanced_graph_json(graph_id,device_type_id,user_id,ip_address)
...
def advanced_update_date_time(h):
    ...
    try:
        ...
        output_dict={'success':0,'end_date':end_date,'end_time':end_time}
...
def advanced_graph_creation(h):
    ...
    bll_obj=APAdvancedGraph()
    controller_dict=bll_obj.advanced_graph_data('graph',user_id,table_name[0],table_name[1],table_name[-2],table_name[-1],start,limit,flag,start_date,end_date,ip_address,graph_type,update_field_name,interface_value,cal_type,column_name)
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(controller_dict)))
...
def ap_data_table_creation(h):
    ...
    user_id=html.req.session["user_id"]
    bll_obj=APAdvancedGraph()
    controller_dict=bll_obj.ap_data_table(user_id,ip_address,cal1,tab1,field1,table_name1,graph_name1,start_date,end_date,limitFlag,graph1,start1,limit1)
    html.req.content_type = 'application/json'
    html.req.write(str(JSONEncoder().encode(controller_dict)))
...
def ap_advanced_excel_creating(h):
    ...
    user_id=html.req.session["user_id"]
    bll_obj=APAdvancedGraph()
    controller_dict=bll_obj.advaeced_excel_report(report_type,device_type,user_id,ip_address,cal1,tab1,field1,table_name1,graph_name1,start_date,end_date,limitFlag,graph1,start1,limit1)
...
def page_tip_advanced_dashboard(h):
        ...
        "<br/>"\
        "<div class=\"action-tip\"><div class=\"img-div\"><img style=\"width:16px;height:16px;\" src=\"images/new_icons/round_plus.png\"/></div><div class=\"txt-div\">Show device satistics name.</div></div>"\
        "<br/>"\
        "<div class=\"action-tip\"><div class=\"img-div\"><img style=\"width:16px;height:16px;\" src=\"images/new_icons/round_minus.png\"/></div><div class=\"txt-div\">Hide device satistics name.</div></div>"\
        "<br/>"\
