...
def common_firrmware_controller(h):
    ...
    global html
    html = h
    html.write("<div class=\"form-div\" id=\"odu16_form_div\" style=\"position: relative;\">")
    html.write("</div>")
    ...
    html.write("<div id=\"firmware-update-child-div\" style=\"position:relative;float:left\"><input type=\"button\" value=\"Update Firmware\" id=\"firmware\" class=\"yo-small yo-button\"/></div>")
...
def firmware_listing(h):
    ...
    #html.write("<script type=\"text/javascript\" src=\"js/firmware_update.js\"></script>\n")
    ...
    connect_chk = check_connection()#used to check that the database connection is available or not
    host_id = html.var("host_id")
    if connect_chk == 0 or connect_chk == "0":
        if host_id == None or host_id == "":
            html.write(str(page_header_search(ip_address,mac_address,"UBR,UBRe",selected_device_type,"enabled","device_type")))
        else:
            device_list_param = get_device_param(host_id)
            html.write(str(page_header_search(device_list_param[0][0],device_list_param[0][1],"UBR,UBRe",device_type,"enabled","device_type")))
...
def firmware_process(host_id,selected_device):
    ...
    try:
        ...
        slaves = []
        master = []
        result = firmware_device_check_controller(host_id,selected_device)#check that the device is master or not
        #html.write(str(result))
        if str(result) == "0" or str(result)==0:
            firmware_result = firmware_process_controller(host_id,selected_device)#used to count the number of slaves connected to a master
            ...
            if firmware_result == 8:
                ...
                if firmware_result[2] == 0:
                    ...
                    html.write("<input type=\"hidden\" id=\"result_final\" name=\"result_final\" value=\"%s\"/>"%(str(result_final[0])))
                    html.write("<input type=\"hidden\" id=\"result_msg\" name=\"result_msg\" value=\"%s\"/>"%(str(result_final[1])))
                    if result_final[0] == 0:
                        if len(firmware_result[1]) > 0:
                            master.append({'tr_master':firmware_result[1][0][0] if (firmware_result[0]) == 7 or (firmware_result[0]) == 6 else firmware_result[0][0][0]})
                    ...
                    if result_final[0] == 0:                         
                        for i in range(0,firmware_result[2]):                    
                            slaves.append({"tr_%s"%(i):firmware_result[1][0][i][0]})
                            if i == 0:
                                table_str+= "<tr style=\"background-color:#EEE\" slave_value=%s>\
                                                    ...
                                                    <input class=\"img-loader\" title=\"loading\" style=\"float: right;\" type=\"image\"></td>\
                                                        ...
                                                        "IP Address" if firmware_result[0][0][3] == None or firmware_result[0][0][3] == "" else firmware_result[0][0][3],\
            ...
            html.write("The Selected Device is of type Slave.Please Select The Master Device For Firemware Upgrade")
...
def firmware_update_set(h):
    ...
    if device_type != "":
        result = firmware_final_set(eval(master),eval(slave_value),current_datetime,device_type,file_name)
    else:
        result = "False"
    html.write(str(result))
...
def firmware_file_upload(h):
    ...
    try:
        ...
        # get the ip address of ap correspondence
            sel_query="SELECT ip_address,http_username,http_password FROM hosts WHERE host_id='%s'"%(html.req.session["host_id_session"])
            ...
            if len(result)>0:
                ...
                if int(responseCode)==404:
                    ...
                    if result != -1:
                        html.write("<p style=\"font-size:10px;font-wight:bold;\">Firmware Uploaded Successfully. Device will Reboot now.</p>")
                    result = responseString.find("Device is now being automatically rebooted")
                    if result != -1:
                        html.write("<p style=\"font-size:10px;font-wight:bold;\">Firmware Updated Successfully</p>")
    ...
    try:
        ...
        # get the ip address of ap correspondence
            ...
            if len(result)>0:
                ...
                if int(responseCode)==404:
                    html.write("<p style=\"font-size:10px;font-wight:bold;\">Page not found<br/><a href=\"javascript:history.go(-1)\">Back</a><p/>")
                elif int(responseCode)==401:
                    html.write("<p style=\"font-size:10px;font-wight:bold;\">Username and Password are wrong.<br/><a href=\"javascript:history.go(-1)\">Back</a><p/>")
                elif int(responseCode)==200:
                    result = responseString.find("Firmware image has bad magic number")
                    if result != -1:
                        html.write("<p style=\"font-size:10px;font-wight:bold;\">Wrong File is Uploaded<br/><a href=\"javascript:history.go(-1)\">Back</a><p/>")
                    result = responseString.find("Firmware upgrade complete")
                    result1 = responseString.find("Device is now being automatically rebooted")
                    if result != -1 or result1!=-1:
                        ...
                        if filename.find("7.2.25") >= 0:
                            sel_query="update hosts set firmware_mapping_id = '%s' where host_id = '%s'"%("7.2.25",html.req.session["host_id_session"])
                        else:
                            sel_query="update hosts set firmware_mapping_id = '%s' where host_id = '%s'"%("7.2.20",html.req.session["host_id_session"])
                        cursor.execute(sel_query)
...
def idu_firmware_file_upload(h):
    ...
    try:
        ...
        # get the ip address of ap correspondence
        sel_query="SELECT ip_address,http_username,http_password,http_port FROM hosts WHERE host_id='%s'"%(html.req.session["host_id_session"])
        cursor.execute(sel_query)
        result=cursor.fetchall()
        cursor.close()
        if len(result)>0:
