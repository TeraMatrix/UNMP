...
import os.path,sys,htmllib,config
from lib import *
...
def create_maps(h):
	...
	html.write("<div class=\"loading\" style=\"display:block;\"><img src='images/loading.gif' alt='loading...'/></div>")
	html.write("<script>setTimeout(function(){ parent.main.location='/%s/nagvis/index.php?mod=Map&act=view&show=Test4'; },2000);</script>" % sitename)
	data="""define global {
  context_menu=1
...
#		html.write(str(prafull))
		...
		#os.chmod('/omd/sites/%s/etc/nagvis/maps/Test4.cfg' % sitename, 777)				
		f1 = open('/omd/sites/%s/etc/nagvis/maps/Test4.cfg' % sitename,'w')
		f1.writelines(data)
