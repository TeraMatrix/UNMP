...
#                        graph_json.append(graph_dict)
    ...
    def common_graph_json(self,display_type,user_id,table_name,x_axis_value,index_name,graph_id,flag,start_date,end_date,start,limit,ip_address,graph,update_field_name,interface=1,value_cal=1,*column_name):
	    ...
	    default_value={'y':0,'marker':{'symbol': 'url(images/device-down.gif)'}}
	    #default_value=0
		...
		default_value='DU'
            ...
            # update the graph calculatio id for particular user for particular table
            ...
            if str(graph_id).strip()=='apVAPUserConnect':
            ...
            elif table_name=='ap25_vapClientStatisticsTable':
                sql="select ap25vap.vap_id,count(ap25vap.addressMAC) from ap25_vapClientStatisticsTable as ap25vap\
                join (select host_id,ip_address,config_profile_id from hosts ) as hosts \
                 on ap25vap.host_id=hosts.host_id \
                where hosts.ip_address='%s'\
                and ap25vap.timestamp between now()-INTERVAL 6 MINUTE and now() group by ap25vap.vap_id order by ap25vap.vap_id"%(ip_address)
                cursor.execute(sql)
                vap_result=cursor.fetchall()
    
                vap_list=['VAP1','VAP2','VAP3','VAP4','VAP5','VAP6','VAP7','VAP8']
                time_list=vap_list
                json_data=[]
                ...
                json_data.append({'name':['User Count',' user'],'data':user_list})
		...
		if column_name[0][0]=="":
                ...
                return output_dic
            elif (table_name.strip()=='odu100_peerNodeStatusTable' or table_name.strip()=='get_odu16_peer_node_status_table') and (graph_id.strip()=='odu100rssi' or graph_id.strip()=='odu100link' or graph_id.strip()=='odu16rssi' or graph_id.strip()=='odu100rssi2'):
...
#		    i=0
#		    for time_stamp in odu100_rssi_result['time_stamp']:
#		        odu100_rssi_result['time_stamp'][i]=(datetime.strptime(time_stamp,'%Y-%m-%d %H:%M:%S')).strftime('%H:%M')
#		        i+=1
#		if display_type=='table' or display_type in ['csv','excel','pdf']:
#		    k=0
#		    for data in odu100_rssi_result['display_signal_strength']:
#			for col in range(len(data['data'])):
#			    if odu100_rssi_result['display_signal_strength'][k]['data'][col]==0:
#				odu100_rssi_result['display_signal_strength'][k]['data'][col]=default_value
#			k+=1
		    ...
		    if  len(column_name[0][0])>0:
		        ...
		        sel_query+=column_list+","+"gp_tab.%s from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' AND gp_tab.timestamp >= '%s' AND gp_tab.timestamp <='%s' and gp_tab.%s = '%s' order by gp_tab.timestamp desc"%(x_axis_value,table_name,ip_address,start_date,end_date,index_name,interface)
		    else:
		        sel_query+="gp_tab.%s from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' AND gp_tab.timestamp >= '%s' AND gp_tab.timestamp <='%s' and gp_tab.%s = '%s' order by gp_tab.timestamp desc"%(x_axis_value,table_name,ip_address,start_date,end_date,index_name,interface)
                else:
		    sel_query="Select "
		    if  len(column_name[0][0])>0:
		        column_list=',gp_tab.'.join(column_name[0])
		        column_list='gp_tab.'+column_list
		        sel_query+=column_list+","+"gp_tab.%s from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' AND gp_tab.timestamp >= '%s' AND gp_tab.timestamp <='%s' AND gp_tab.%s = '%s' order by gp_tab.timestamp desc limit 10"%(x_axis_value,table_name,ip_address,start_date,end_date,index_name,interface)
		    else:
		        sel_query+="gp_tab.%s from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' AND gp_tab.timestamp >= '%s' AND gp_tab.timestamp <='%s' AND  gp_tab.%s = '%s' order by gp_tab.timestamp desc limit 10"%(x_axis_value,table_name,ip_address,start_date,end_date,index_name,interface)
                ...
                if calculation[int(value_cal)-1]=='normal':
                    for row in result:
                        ...
                        #time_list.append(time.mktime(row[len(row)-1].timetuple())*1000+(5.5*3600000))
		...
		if display_type in ['graph']:
			...
			time_list[index_val]=time.mktime(time_list[index_val].timetuple())*1000+(5.5*3600000)
    ...
    def sp_advanced_outage_graph(self,display_type,ip_address,start_date,end_date):
		...
		if display_type.strip()=='graph':
		    date_days.append(time.mktime(datetime.strptime(row[0].strip(),"%Y-%m-%d").timetuple())*1000+(5.5*3600000))
		else:
		    #date_days.append(datitime.strftime(datetime.strptime(row[0].strip(),"%Y-%m-%d"),'%d %b %Y'))
		    #date_days.append((row[0]))
    ...
    def sp_signal_strength_graph(self,display_type,graph_id,ip_address,start_time,end_time,table_name,column_list):
        ...
        default_rssi={}
        time_stamp_signal1=[]
        new_default_value={'y':0,'marker':{'symbol': 'url(images/device-down.gif)'}}
        #default_value=0
        #default_value={'y':0,'marker':{'symbol': 'url(http://oriondemo.solarwinds.com/Orion/images/StatusIcons/Small-down-unknown.gif)'}}
        default_value={'y':0,'marker':{'symbol': 'url(images/device-down.gif)'}}
	...
	qaulity='(%)'
	    ...
	    if display_type=='graph':
	        if (graph_id).strip()=='odu100link' or (graph_id).strip()=='odu16link':
                    if table_name.strip()=='odu100_peerNodeStatusTable':
                        sel_query="select odu.timeSlotIndex,(IFNULL((odu.txLinkQuality),0)),odu.timestamp,odu.ssIdentifier,odu.linkStatus,odu.tunnelStatus,odu.peerMacAddr from  odu100_peerNodeStatusTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' and odu.timeSlotIndex IN (%s) AND odu.timestamp >= '%s' AND odu.timestamp <='%s' order by odu.timestamp desc limit %s"%(ip_address,column_field,start_time,end_time,default_count)
                    elif table_name.strip()=='get_odu16_peer_node_status_table':
                        sel_query="select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status,odu.peer_mac_addr from  get_odu16_peer_node_status_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' and odu.timeSlotIndex IN (%s) AND odu.timestamp >= '%s' AND odu.timestamp <='%s' order by odu.timestamp desc limit %s"%(ip_address,column_field,start_time,end_time,default_count)
	        else:
		    qaulity='(dBm)'
                    if table_name.strip()=='odu100_peerNodeStatusTable':
                        sel_query="select odu.timeSlotIndex,(IFNULL((odu.sigStrength%s),0)),odu.timestamp,odu.ssIdentifier,odu.linkStatus,odu.tunnelStatus,odu.peerMacAddr from  odu100_peerNodeStatusTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' and odu.timeSlotIndex IN (%s) AND odu.timestamp >= '%s' AND odu.timestamp <='%s' order by odu.timestamp desc limit %s"%(1 if graph_id.strip()=='odu100rssi' else 2,ip_address,column_field,start_time,end_time,default_count)
                    elif table_name.strip()=='get_odu16_peer_node_status_table':
                        sel_query="select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status,odu.peer_mac_addr from  get_odu16_peer_node_status_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' and odu.timeslot_index IN (%s) AND odu.timestamp >= '%s' AND odu.timestamp <='%s' order by odu.timestamp desc limit %s"%(ip_address,column_field,start_time,end_time,default_count)
	    else:
	        if (graph_id).strip()=='odu100link' or (graph_id).strip()=='odu16link':
                    if table_name.strip()=='odu100_peerNodeStatusTable':
                        sel_query="select odu.timeSlotIndex,(IFNULL((odu.txLinkQuality),0)),odu.timestamp,odu.ssIdentifier,odu.linkStatus,odu.tunnelStatus,odu.peerMacAddr from  odu100_peerNodeStatusTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' and odu.timeSlotIndex IN (%s) order by odu.timestamp desc"%(ip_address,start_time,end_time,column_field)
                    elif table_name.strip()=='get_odu16_peer_node_status_table':
                        sel_query="select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status,odu.peer_mac_addr from  get_odu16_peer_node_status_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' and odu.timeslot_index IN (%s) order by odu.timestamp desc"%(ip_address,start_time,end_time,column_field)
	        else:
		    qaulity='(dBm)'
                    if table_name.strip()=='odu100_peerNodeStatusTable':
                        sel_query="select odu.timeSlotIndex,(IFNULL((odu.sigStrength%s),0)),odu.timestamp,odu.ssIdentifier,odu.linkStatus,odu.tunnelStatus,odu.peerMacAddr from  odu100_peerNodeStatusTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' and odu.timeSlotIndex IN (%s) order by odu.timestamp desc"%(1 if graph_id.strip()=='odu100rssi' else 2,ip_address,start_time,end_time,column_field)
                    elif table_name.strip()=='get_odu16_peer_node_status_table':
                        sel_query="select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status,odu.peer_mac_addr from  get_odu16_peer_node_status_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' and odu.timeslot_index IN (%s) order by odu.timestamp desc"%(ip_address,start_time,end_time,column_field)
            cursor.execute(sel_query)
            ...
            for k in range(0,len(signal_strength)-1):
                ...
                if signal_strength[k][1]==1111111 or str(signal_strength[k][1])=='1111111' :
                    ...
                    #time_stamp_signal1.append(signal_strength[k][2])
                    ...
                    if signal_strength[k][2]==signal_strength[k+1][2]:
                        ...
                        #default_list[int(signal_strength[k][0])-1]=int(signal_strength[k][1])
                    else:
                        default_rssi[int(signal_strength[k][0])]=int(signal_strength[k][1])
                        for i in range(len(rssi_dict)):
            ...
            if display_type.strip()=='graph':
                for time_index in range(len(time_stamp_signal1)):
                    time_stamp_signal1[time_index]=time.mktime(time_stamp_signal1[time_index].timetuple())*1000+(5.5*3600000)
		...
		if str(col).strip()!='':
            ...
            if status_name=='Master':
                ...
                for i in column_list:
                    signal_json_data.append({'name':['Link%s'%i,qaulity],'data':rssi_dict[j][i]})
#                    signal_json_data.append({'name':['Link%s'%i,qaulity],'data':rssi_dict[j][i],'type':'area','pointInterval': 300 * 1000})
                    j+=1
            else:
		if len(rssi_dict)>0:
    ...
    def sp_event_alarm(self,ip_address,device_type):
        try:
            ...
            output_dict={'success':0,'Event':all_traps,'Alarm':current_alarm}
            return  output_dict
        # Exception Handling
        except MySQLdb as e:
            output_dict={'success':1,'error_msg':'Error No : 102 '+str(err_obj.get_error_msg(102)),'main_msg':str(e[-1])}
...
# Excel reporting start here.
    def sp_excel_report(self,device_type,user_id,ip_address,cal_list,tab_list,field_list,table_name_list,graph_name_list,start_date,end_date,select_option,limitFlag,graph_list,start_list,limit_list):
        if ip_address=='' or ip_address==None or ip_address=='undefined' or str(ip_address)=='None':    # if ip_address not received so excel not created
            raise SelfException('This UBR devices not exists so excel report can not be generated.')  # Check msg 
#        check_var=''
        try:
	    display_type='excel'
            master_slave_status=get_master_slave_value(ip_address)
            ...
            device_name={'ap25':'AP25','odu16':'UBR','odu100':'UBRe','idu4':'IDU'}
            ...
            login_user_name=''
            sel_query="SELECT host_alias FROM hosts WHERE ip_address='%s'"%(ip_address)
            cursor.execute(sel_query)
            ...
            if device_type.strip()=='odu16' or device_type.strip()=='odu100':
                device_field=['Frequency','Slaves','Active Version','Hardware Version','Last Reboot Reason','Channel','Operation State','Node Type','MAC Address','Last Reboot Time']
...
#                check_var+=str('ram2')
#                interface_name=interface_list[interface][tab_list[i]] if  len([interface for interface in  interface_list.keys() if interface==table_name])==1 else ''
                table_dic_key=[interface for interface in  interface_list.keys() if interface==table_name]
#                check_var+=str(table_dic_key)+str('raju' if len(table_dic_key)==0 else interface_list[table_dic_key[0]])+str([tab_list[i]])
...
#            sql="SELECT ta.trap_receive_date,ta.trap_event_type,ta.description FROM trap_alarms as ta inner join hosts on ta.agent_id=hosts.ip_address WHERE ta.agent_id='%s' and ta.timestamp>='%s' and ta.timestamp<='%s' order by ta.timestamp desc limit 7 "%(ip_address,start_date,end_date)
            cursor.execute(sql)
            current_alarm=cursor.fetchall()
            merge_result=[]
...
#            merge_result=[]
#            for row in current_alarm:
#                merge_result.append([row[0],row[1],row[2]]) 
#            xls_sheet=xls_book.add_sheet('Event',cell_overwrite_ok=True)
#            xls_sheet.row(0).height = 521
#            xls_sheet.row(1).height = 421
#            xls_sheet.write_merge(0,0,0,len(headings)-1,'Device Events (%s--%s)'%(start_date,end_date),style)
#            xls_sheet.write_merge(1,1,0,len(headings)-1,device_name[device_type]+'       '+str(ip_address)+str(device_postfix) ,style)
...
#                merge_result.append([row[0],row[1],row[2]]) 
#            blank_row=["","",""]
#            main_row=['Device Alarm Statistics']
#            second_row=[device_name[device_type]+str(device_postfix),str(ip_address)]
#            blank_row=["","",""]
#            writer.writerow(['------','--Break--','------'])
#            writer.writerow(blank_row)
    ...
    def sp_pdf_report(self,device_type,user_id,ip_address,cal_list,tab_list,field_list,table_name_list,graph_name_list,start_date,end_date,select_option,limitFlag,graph_list,start_list,limit_list):
	    ...
	    display_type='pdf'
            ...
            if db ==1:
                raise SelfException(cursor)
            device_name={'ap25':'AP25','odu16':'UBR','odu100':'UBRe','idu4':'IDU'}
	    idu4_port={'0':'(odu)','1':'(eth0)','2':'(eth1)','3':'(eth1)','4':'(maxima)'}
            interface_list={'get_odu16_nw_interface_statistics_table':{'1':'(eth0)','2':'(br0)','3':'(eth1)'},'odu100_nwInterfaceStatisticsTable':{'1':'(eth0)','2':'(eth1)'},'ap25_statisticsTable':{'0':'(eth0)','1':'(br0)','2':'(ath0)','3':'(ath1)','4':'(ath2)','5':'(ath3)','6':'(ath4)','7':'(ath5)','8':'(ath6)'},
'idu_e1PortStatusTable':{'1':'(port1)','2':'(port2)','3':'(port3)','4':'(port4)'},'idu_portstatisticsTable':idu4_port,'idu_swPrimaryPortStatisticsTable':idu4_port,'idu_portSecondaryStatisticsTable':idu4_port,
...
#                        headings.append(data_list['name'][0])
		    ...
		    continue
                if table_name.strip()=='get_odu16_peer_node_status_table' or table_name.strip()=='odu100_peerNodeStatusTable':
                    ...
                    if len(headings)==1:
                        ...
                        t = Table(data,[2 * inch, 1.7 * inch, 1.7*inch,1.7*inch])
                    elif len(headings)==5:
                        t = Table(data,[1.7 * inch, 1.35 * inch, 1.35*inch,1.35*inch,1.35*inch])
                    elif len(headings)==6:
                        t = Table(data,[1.65 * inch, 1.09* inch, 1.09*inch,1.09*inch,1.09*inch,1.09*inch,1.09*inch])
                    elif len(headings)==7:
                        t = Table(data,[1.1 * inch, 1* inch, 1*inch,1*inch,1*inch,1*inch,1*inch,1*inch])
                    elif len(headings)==8:
                        t = Table(data,[1 * inch, .87* inch, .87*inch,.87*inch,.87*inch,.87*inch,.87*inch,.87*inch,.87*inch])
                    elif len(headings)==9:
                        t = Table(data,[1.1 * inch, .75* inch, .75*inch,.75*inch,.75*inch,.75*inch,.75*inch,.75*inch,.75*inch])
        #                t = Table(data,[2.7 * inch, 2.2 * inch, 1.5*inch, .7*inch])
                    ...
                    if len(headings1)>0:
                        ...
                        if len(headings1)==1:
                            ...
                            t = Table(data,[1.1 * inch, .75* inch, .75*inch,.75*inch,.75*inch,.75*inch,.75*inch,.75*inch,.75*inch])
            #                t = Table(data,[2.7 * inch, 2.2 * inch, 1.5*inch, .7*inch])
                        t.setStyle(TableStyle([('FONT', (0, 0), (len(headings1)-1, 0), 'Helvetica-Bold', 10),        
                        ('FONT', (0, 1), (len(headings1)-1, int(len(table_output1))-1), 'Helvetica', 9),        
                        ('ALIGN', (1,0), (len(headings1)-1, int(len(table_output1))-1), 'CENTER'),
                        ('BACKGROUND',(0,0),(len(headings1)-1,0),(0.9,0.9,0.9)),
                        ('LINEABOVE',(0,0), (len(headings1)-1, 0), 1.21,(0.35,0.35,0.35)),
                        ('GRID',(0,0),(8,int(len(table_output1))-1),0.31,(0.75,0.75,0.75))]))
            
                        for i in range(1,len(table_output1)):
                            if i%2==1:
                                t.setStyle(TableStyle([('BACKGROUND',(1,i),(len(headings1)-1,i),(0.95,0.95,0.95)),
                                        ('BACKGROUND',(0,i-1),(0,i-1),(0.98,0.98,0.98)),
                                           ]))
                            else:
                        ...
                        data1=[]
                        if len(table_output)>1:
                            data1.append(['1-'+str(len(table_output1)-1)+' of '+str(len(table_output1)-1)])
                        else:
                            data1.append(['0-'+str(len(table_output1)-1)+' of '+str(len(table_output1)-1)])
                        t = Table(data1, [7.10*inch])
                        t.setStyle(TableStyle([('BACKGROUND',(0,0),(0,0),(0.9,0.9,0.9)),('GRID',(0,0),(5,0),0.31,(0.75,0.75,0.75)),('ALIGN', (0,0), (0, 0), 'RIGHT')]))
                        ubr_report.append(t)
                    ...
                    if len(headings)==1:
                        ...
                        t = Table(data,[1.15 * inch, 1.19* inch, 1.19*inch,1.19*inch,1.19*inch,1.19*inch])
                    elif len(headings)==7:
                        t = Table(data,[1.1 * inch, 1* inch, 1*inch,1*inch,1*inch,1*inch,1*inch])
                    elif len(headings)==8:
                        t = Table(data,[1.17 * inch, .97* inch, .97*inch,.97*inch,.97*inch,.97*inch,.97*inch,.97*inch])
                    elif len(headings)==9:
                        t = Table(data,[1.1 * inch, .75* inch, .75*inch,.75*inch,.75*inch,.75*inch,.75*inch,.75*inch,.75*inch])
        ...
        #                t = Table(data,[2.7 * inch, 2.2 * inch, 1.5*inch, .7*inch])
            ...
            table_output.append(headings)
            for row in current_alarm:
		table_output.append([row[5],serevity_list[int(row[0])],row[1],row[2],row[3],row[6]]) 
	    ...
	    t = Table(data,[1.15 * inch, 1.19* inch, 1.19*inch,1.19*inch,1.19*inch,1.19*inch])
            ...
            t.setStyle(TableStyle([('BACKGROUND',(0,0),(0,0),(0.51,0.51,0.51)),('FONT', (0, 0), (0, 0), 'Helvetica', 11),('TEXTCOLOR',(0,0),(0,0),colors.white)]))
            ubr_report.append(t)
#            ubr_report.append(Spacer(21,21))                
#            sql="SELECT ta.trap_receive_date,ta.trap_event_type,ta.description FROM trap_alarm_current as ta inner join hosts on ta.agent_id=hosts.ip_address WHERE ta.agent_id='%s' and ta.timestamp>='%s' and ta.timestamp<='%s'  order by ta.timestamp desc "%(ip_address,start_date,end_date)
#            cursor.execute(sql)
...
def get_master_slave_value(ip_address): 
	...
	if len(device_type)>0 and  device_type[0][0]=='odu100':
	    sel_query="SELECT defaultNodeType FROM odu100_ruConfTable as def INNER JOIN hosts ON hosts.config_profile_id=def.config_profile_id WHERE hosts.ip_address='%s' limit 1"%(ip_address)
	    cursor.execute(sel_query)
	    status_result=cursor.fetchall()
	elif len(device_type)>0 and  device_type[0][0]=='odu16':
	    sel_query="SELECT default_node_type FROM  get_odu16_ru_conf_table as def INNER JOIN hosts ON hosts.host_id=def.host_id WHERE hosts.ip_address='%s' limit 1"%(ip_address)
	    cursor.execute(sel_query)
	    status_result=cursor.fetchall()
	elif len(device_type)>0 and  device_type[0][0]=='idu4':
	    sel_query="SELECT hwType FROM   idu_iduInfoTable as def INNER JOIN hosts ON hosts.host_id=def.host_id WHERE hosts.ip_address='%s' limit 1"%(ip_address)
	    cursor.execute(sel_query)
	    status_result=cursor.fetchall()
        status=0
        ...
        if 1==1:
            ...
            if str(temp_res['success'])=="0":
                ...
                for i in li_res:
                    ...
                    if total!=0:
                        i[0]=str(i[0])[:11]
                        i[-2]=int(round(float(uptime)*100/float(total)))#(uptime*100)/total
                        i[-1]=int(round(float(downtime)*100/float(total)))#(downtime*100)/total
