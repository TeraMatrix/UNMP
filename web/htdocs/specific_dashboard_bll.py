#!/usr/bin/python2.6
# import the packeges
import config, time, cgi,MySQLdb,sys,csv
...
class SPDashboardBll(object):
    ...
    def get_dashboard_data(self):
        ...
        get_data=DashboardConfig.get_config_attributes('odu_dashboard',devcie_type_attr)
    ...
    def sp_device_information(self,ip_address,device_type):
        ...
        try:
            ...
            if device_type.strip()=='ap25':
                ...
                output_dict={'success':0,'result':result,'no_of_uesr':no_of_user,'device_type':device_type}
                ...
                output_dict={'success':0,'result':result,'last_reboot_time':last_reboot_time,'device_type':device_type}
                ...
                cursor.execute(sql)
                result=cursor.fetchall()
                output_dict={'success':0,'result':result,'device_type':device_type}
			...
			status.ccuSDLastRebootReason,net.ccuNCMACAddress,soft.ccuSICommunicationProtocolVersion\
			from ccu_StatusDataTable as status inner join hosts on status.host_id=hosts.host_id\
			inner join ccu_softwareinformationtable as soft on soft.host_id=hosts.host_id\
			inner join ccu_NetworkConfigurationTable as net on net.host_id=hosts.host_id\
			inner join ccu_InformationTable as info on info.host_id=hosts.host_id\
        ...
        try:
            ...
            for row in result:
                ...
                for value in graph_tuple:
                    if value[0]==row[5]:
                        graph_type.append({'value':value[0],'name':value[1],'isChecked':1})
                    else:
                        graph_type.append({'value':value[0],'name':value[1],'isChecked':0})
                query1="SELECT graph_field_value,graph_field_display_name,is_checked FROM graph_field_table WHERE graph_name='%s' and user_id='%s'"%(row[0],user_id)
                cursor.execute(query1)
                ...
                if master_slave_status['success']==0 and int(master_slave_status['status'])>0:
                    if len(option_tuple)>0 and (row[0]=='odu100rssi' or row[0]=='odu16rssi' or row[0]=='odu100rssi2' or row[0]=='odu100link'):
                        option_list.append({'name':option_tuple[0][0],'displayName':'Master-Link','isChecked':option_tuple[0][2]})
                    else:
		    ...
		    for interface in interface_tuple:
                ...
                for cal in cal_tuple:
                    if int(cal[0])==int(row[13]):
                        cal_list.append({'name':str(cal[0]),'displayName':cal[1],'isChecked':int(1)})
                    else:
                        cal_list.append({'name':str(cal[0]),'displayName':cal[1],'isChecked':int(0)})

                ajax_json={}
                query4="SELECT url,method,other_data FROM  graph_ajax_call_information where user_id='%s' and graph_id='%s'"%(user_id,row[0])
                cursor.execute(query4)
                ajax_info=cursor.fetchall()
                for ajax in ajax_info:
                    ajax_json={'url':ajax[0],'method':ajax[1],'cache':False,'data':{'table_name':ajax[2]}}
                ...
                for count_info in total_count_info:
                    total_item_json={'url':count_info[0],'method':count_info[1],'cache':False,'data':{'table_name':count_info[2]}}
                ...
                graph_dict={'name':row[0],'displayName':row[1],'fields':option_list,'calType':cal_list,'ajax':ajax_json,'tabList': {'value':interface_value,'name':interface_name,'selected':check_val},'type':graph_type,'otherOption':{'showOption':True if int(row[6])==1 else False,'showRefreshButton':True if int(row[7])==1 else False,'showNextPreButton':True if int(row[8])==1 else False,'width':row[11],'height':str(row[12])+"px",'showType': showType,'showFields':showFields,'showCalType':showCalType,'showTabOption':showTabOption,'autoRefresh':row[18]},'startFrom':row[9],'itemLimit':row[10],'totalItemAjax':total_item_json}
...
#		graph_json.append(graph_dict)
    ...
    def common_graph_json(self,display_type,user_id,table_name,x_axis_value,index_name,graph_id,flag,start_date,end_date,start,limit,ip_address,graph,update_field_name,interface=1,value_cal=1,*column_name):
		...
		default_value='DU'
            if str(update_field_name).strip()=='calType':
                up_query="update  graph_templet_table set graph_cal_id =%s where user_id='%s' and graph_display_id='%s' and dashboard_type=0"%(value_cal,user_id,graph_id)
                cursor.execute(up_query)
                conn.commit()
                
            elif str(update_field_name).strip()=='fields':
                up_query="update graph_field_table set is_checked=0 where user_id='%s' and graph_name='%s'"%(user_id,graph_id)
                cursor.execute(up_query)
                conn.commit()

                columns="','".join(column_name[0])
                up_query="update graph_field_table set is_checked=1 where user_id='%s' and graph_name='%s' and graph_field_value IN ('%s')"%(user_id,graph_id,columns)
                cursor.execute(up_query)
                conn.commit()
                ...
                cursor.execute(up_query)
                conn.commit()
            get_coloum="SELECT graph_field_value,graph_field_display_name,tool_tip_title FROM graph_field_table WHERE graph_name='%s' AND user_id='%s'"%(graph_id,user_id)
            cursor.execute(get_coloum)
            coloum_result=cursor.fetchall()
            clm_dict=dict(zip([coloum_result[i][0] for i in range(len(coloum_result))],[coloum_result[i][1] for i in range(len(coloum_result))]))
            table_field_dict1={table_name:clm_dict}
            graph_field_dict=dict((field[0],field[2]) for field in coloum_result)
            
            sel_query="select graph_title,graph_subtitle from graph_templet_table where graph_display_id='%s' and user_id='%s' and dashboard_type=0"%(graph_id,user_id)
            cursor.execute(sel_query)
            graph_name_result=cursor.fetchall()
            graph_title='' if graph_name_result[0][0]==None or graph_name_result[0][0]=='' else graph_name_result[0][0]  
            graph_sub_title='' if graph_name_result[0][1]==None or graph_name_result[0][1]=='' else graph_name_result[0][1]  
            ...
            if str(graph_id).strip()=='apVAPUserConnect123':
                if column_name[0][0]=="rssi":
                    ...
                    column_list=''
                sql="select addressMAC %s from ap25_vapClientStatisticsTable as ap inner join hosts on ap.host_id=hosts.host_id where hosts.ip_address='%s' order by ap25_vapClientStatisticsTable_id limit %s,%s"%(column_list,ip_address,start,limit)
                cursor.execute(sql)
                ...
                time_list=mac_list
                if var_count==2:
                    json_data.append({'name':['RSL',' dbm'],'data':rssi_list})
                else:
		...
		if (end_date-start_date).days < 5:
			start_date=datetime.strptime(str(datetime.date(datetime.now())+timedelta(days=-4))+' '+'00:00:00',"%Y-%m-%d %H:%M:%S")		
		else:
			start_date=datetime.strptime(str(datetime.date(start_date))+' '+'00:00:00',"%Y-%m-%d %H:%M:%S")
                outage_result=self.sp_advanced_outage_graph(display_type,ip_address,start_date,end_date)
                if int(outage_result['success'])!=0:
                    return outage_result
		if column_name[0][0]=="":
                    output_dic={'success':0,'timestamp':outage_result['date_days'],'data':[],'graph_title':graph_title,'graph_sub_title':graph_sub_title}
		else:
                    for name in column_name[0]:
                        json_data.append({"data":outage_result[name],"name":[name,'(%)']})
                    time_list=outage_result['date_days']
                    output_dic={'success':0,'timestamp':time_list,'data':json_data,'graph_title':graph_title,'graph_sub_title':graph_sub_title}
                return output_dic
                ...
                odu100_rssi_result=self.sp_signal_strength_graph(display_type,graph_id,ip_address,start_date,end_date,table_name,column_name[0])
                if int(odu100_rssi_result['success'])!=0:
                    return odu100_rssi_result
                output_dic={'success':0,'timestamp':odu100_rssi_result['time_stamp'],'data':odu100_rssi_result['display_signal_strength'],'graph_title':graph_title,'graph_sub_title':graph_sub_title}
                return output_dic
            else:
		    ...
		    if  len(column_name[0][0])>0:
		        ...
		        sel_query+=column_list+","+"gp_tab.%s from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' AND gp_tab.timestamp >= '%s' AND gp_tab.timestamp <='%s' and gp_tab.%s = '%s' and hosts.is_deleted=0 order by gp_tab.timestamp desc"%(x_axis_value,table_name,ip_address,start_date,end_date,index_name,interface)
		    else:
		        sel_query+="gp_tab.%s from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' AND gp_tab.timestamp >= '%s' AND gp_tab.timestamp <='%s' and gp_tab.%s = '%s' and hosts.is_deleted=0 order by gp_tab.timestamp desc"%(x_axis_value,table_name,ip_address,start_date,end_date,index_name,interface)
                else:
		    sel_query="Select "
		    if  len(column_name[0][0])>0:
		        column_list=',gp_tab.'.join(column_name[0])
		        column_list='gp_tab.'+column_list
		        sel_query+=column_list+","+"gp_tab.%s from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' AND gp_tab.timestamp >= '%s' AND gp_tab.timestamp <='%s' AND gp_tab.%s = '%s' and hosts.is_deleted=0 order by gp_tab.timestamp desc limit 10"%(x_axis_value,table_name,ip_address,start_date,end_date,index_name,interface)
		    else:
		        sel_query+="gp_tab.%s from %s as gp_tab INNER JOIN hosts ON hosts.host_id=gp_tab.host_id where hosts.ip_address='%s' AND gp_tab.timestamp >= '%s' AND gp_tab.timestamp <='%s' AND  gp_tab.%s = '%s' and hosts.is_deleted=0 order by gp_tab.timestamp desc limit 10"%(x_axis_value,table_name,ip_address,start_date,end_date,index_name,interface)
                ...
                if calculation[int(value_cal)-1]=='normal':
                    for row in result:
                        for i  in range(len(row)-1):
                            data_list[i].append(0 if row[i]==None or row[i]==None else int(row[i]))
                        time_list.append(datetime.strptime(str(row[len(row)-1])[:19],'%Y-%m-%d %H:%M:%S'))
			    ...
			    for d3 in range(len(result[k])-1):
			        data_list[d3].append(default_value)
                            ...
                            for i  in range(len(result[k])-1):
                                val=(0 if int(result[k][i])==1111111 else int(result[k][i]))-(0 if int(result[k+1][i])==1111111 else int(result[k+1][i]))
                                main_val=val if val>0 else 0
                                data_list[i].append(0 if result[k][i]==None or result[k][i]=='' else main_val)
                        time_list.append(datetime.strptime(str(result[k][len(result[k])-1])[:19],'%Y-%m-%d %H:%M:%S'))

		if display_type in ['graph']:
		    for index_val in range(len(time_list)):
			time_list[index_val]=time.mktime(time_list[index_val].timetuple())*1000
		...
		if len(data_list)>0 and calculation[int(value_cal)-1]=='normal':
		    for d1 in range(len(data_list[0])):
                ...
                if len(column_name[0][0])>0:
                    i=0
                    for name in column_name[0]:
                        json_data.append({'name':['%s'%table_field_dict1[table_name][name],graph_field_dict[name]],'data':data_list[i]})
                        i+=1
            output_dic={'success':0,'timestamp':time_list,'data':json_data,'graph_title':graph_title,'graph_sub_title':graph_sub_title}
            return  output_dic
        except MySQLdb as e:
    ...
    def sp_total_display_graph(self,device_type_id,user_id,ip_address):
        try:
           ...
           # check the deivce is master or not.
            ...
            cursor=db.cursor()
            sql=" select graph_display_id from graph_templet_table where device_type_id='%s' and user_id='%s' and is_disabled=0 and dashboard_type=0"%(device_type_id,user_id)
            cursor.execute(sql)
            result=cursor.fetchall()
	    count=0
            if len(result)>0:
                if master_slave_status['success']==0 and master_slave_status['status']==0: 
		    graph_id_list=[row[0] for row in result if row[0] not in ['odu100synclost','odu16synclost']]
    ...
    def sp_advanced_outage_graph(self,display_type,ip_address,start_date,end_date):
        ...
        down_state=[]  # Its store the total down state of each day in percentage.
		...
		if display_type.strip()=='graph':
		    date_days.append(time.mktime(datetime.strptime(row[0].strip(),"%Y-%m-%d").timetuple())*1000)
		else:
		    date_days.append(datetime.strftime(datetime.strptime(row[0].strip(),'%Y-%m-%d'),'%d %b %Y'))
                up_state.append(row[2])
                down_state.append(row[3])
	    date_days.reverse()
	    up_state.reverse()
	    down_state.reverse()
            output_dict={'success':0,'Reachable':up_state,'Unreachable':down_state,'date_days':date_days}
            return output_dict 
    ...
    def sp_signal_strength_graph(self,display_type,graph_id,ip_address,start_time,end_time,table_name,column_list):
        ...
        default_rssi={}
        time_stamp_signal1=[]
        new_default_value={'y':0,'marker':{'symbol': 'url(images/ab.png)'}}
        default_value={'y':0,'marker':{'symbol': 'url(images/ab.png)'}}
        link_name="Link"
	qaulity='(%)'
        try:
		...
		if str(col).strip()!='':
	        ...
	        sel_query="select DISTINCT(timeslot_index) from get_odu16_peer_node_status_table where timestamp >= '%s' AND timestamp <='%s'"%(start_time,end_time)
	        cursor.execute(sel_query)
	        count_result=cursor.fetchall()
	    ...
	    if display_type=='graph':
	        if (graph_id).strip()=='odu100link' or (graph_id).strip()=='odu16link':
                    if table_name.strip()=='odu100_peerNodeStatusTable':
                        sel_query="select odu.timeSlotIndex,(IFNULL((odu.txLinkQuality),0)),odu.timestamp,odu.ssIdentifier,odu.linkStatus,odu.tunnelStatus,odu.peerMacAddr from  odu100_peerNodeStatusTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' and odu.timeSlotIndex IN (%s) AND odu.timestamp >= '%s' AND odu.timestamp <='%s' and h.is_deleted=0 order by odu.timestamp desc limit %s"%(ip_address,column_field,start_time,end_time,default_count)
                    elif table_name.strip()=='get_odu16_peer_node_status_table':
                        sel_query="select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status,odu.peer_mac_addr from  get_odu16_peer_node_status_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' and odu.timeSlotIndex IN (%s) AND odu.timestamp >= '%s' AND odu.timestamp <='%s' and h.is_deleted=0 order by odu.timestamp desc limit %s"%(ip_address,column_field,start_time,end_time,default_count)
	        elif table_name.strip()=='ap25_vapClientStatisticsTable':
                    qaulity='(count)'
                    link_name="VAP"
                    if (graph_id).strip()=='vapConnectedClient':
                        sel_query="select ap.vap_id,count(ap.addressMAC),ap.timestamp from  ap25_vapClientStatisticsTable as ap INNER JOIN hosts as h on ap.host_id = h.host_id  where h.ip_address='%s' and ap.vap_id IN (%s) AND ap.addressMAC!='' AND ap.timestamp >= '%s' AND ap.timestamp <='%s' and h.is_deleted=0 group by ap.vap_id,ap.timestamp order by ap.timestamp desc limit %s"%(ip_address,column_field,start_time,end_time,default_count)
                    elif (graph_id).strip()=='totalConnectedClient':
                        link_name="Total Client"
                        sel_query="select ap.vap_id,count(ap.addressMAC),ap.timestamp from  ap25_vapClientStatisticsTable as ap INNER JOIN hosts as h on ap.host_id = h.host_id  where h.ip_address='%s' and ap.vap_id IN (1,2,3,4,5,6,7,8) AND ap.addressMAC!='' AND ap.timestamp >= '%s' AND ap.timestamp <='%s' and h.is_deleted=0 group by ap.timestamp order by ap.timestamp desc limit %s"%(ip_address,start_time,end_time,default_count)
	        else:
		    qaulity='(dBm)'
                    if table_name.strip()=='odu100_peerNodeStatusTable':
                        sel_query="select odu.timeSlotIndex,(IFNULL((odu.sigStrength%s),0)),odu.timestamp,odu.ssIdentifier,odu.linkStatus,odu.tunnelStatus,odu.peerMacAddr from  odu100_peerNodeStatusTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' and odu.timeSlotIndex IN (%s) AND odu.timestamp >= '%s' AND odu.timestamp <='%s' and h.is_deleted=0 order by odu.timestamp desc limit %s"%(1 if graph_id.strip()=='odu100rssi' else 2,ip_address,column_field,start_time,end_time,default_count)
                    elif table_name.strip()=='get_odu16_peer_node_status_table':
                        sel_query="select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status,odu.peer_mac_addr from  get_odu16_peer_node_status_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' and odu.timeslot_index IN (%s) AND odu.timestamp >= '%s' AND odu.timestamp <='%s' and h.is_deleted=0 order by odu.timestamp desc limit %s"%(ip_address,column_field,start_time,end_time,default_count)
	    else:
	        if (graph_id).strip()=='odu100link' or (graph_id).strip()=='odu16link':
                    if table_name.strip()=='odu100_peerNodeStatusTable':
                        sel_query="select odu.timeSlotIndex,(IFNULL((odu.txLinkQuality),0)),odu.timestamp,odu.ssIdentifier,odu.linkStatus,odu.tunnelStatus,odu.peerMacAddr from  odu100_peerNodeStatusTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' and odu.timeSlotIndex IN (%s) and h.is_deleted=0 order by odu.timestamp desc"%(ip_address,start_time,end_time,column_field)
                    elif table_name.strip()=='get_odu16_peer_node_status_table':
                        sel_query="select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status,odu.peer_mac_addr from  get_odu16_peer_node_status_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' and odu.timeslot_index IN (%s) and h.is_deleted=0 order by odu.timestamp desc"%(ip_address,start_time,end_time,column_field)
	        elif table_name.strip()=='ap25_vapClientStatisticsTable':
                    qaulity='(count)'
                    link_name="VAP"
                    if (graph_id).strip()=='vapConnectedClient':
                        sel_query="select ap.vap_id,count(ap.addressMAC),ap.timestamp from  ap25_vapClientStatisticsTable as ap INNER JOIN hosts as h on ap.host_id = h.host_id  where h.ip_address='%s' and ap.vap_id IN (%s) AND ap.addressMAC!='' AND ap.timestamp >= '%s' AND ap.timestamp <='%s' and h.is_deleted=0 group by ap.vap_id,ap.timestamp order by ap.timestamp desc "%(ip_address,column_field,start_time,end_time)
                    elif (graph_id).strip()=='totalConnectedClient':
                        link_name="Total Client"
                        sel_query="select ap.vap_id,count(ap.addressMAC),ap.timestamp from  ap25_vapClientStatisticsTable as ap INNER JOIN hosts as h on ap.host_id = h.host_id  where h.ip_address='%s' and ap.vap_id IN (1,2,3,4,5,6,7,8) AND ap.addressMAC!='' AND ap.timestamp >= '%s' AND ap.timestamp <='%s' and h.is_deleted=0 group by ap.timestamp order by ap.timestamp desc "%(ip_address,start_time,end_time)
	        else:
		    qaulity='(dBm)'
                    if table_name.strip()=='odu100_peerNodeStatusTable':
                        sel_query="select odu.timeSlotIndex,(IFNULL((odu.sigStrength%s),0)),odu.timestamp,odu.ssIdentifier,odu.linkStatus,odu.tunnelStatus,odu.peerMacAddr from  odu100_peerNodeStatusTable as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' and odu.timeSlotIndex IN (%s) and h.is_deleted=0 order by odu.timestamp desc"%(1 if graph_id.strip()=='odu100rssi' else 2,ip_address,start_time,end_time,column_field)
                    elif table_name.strip()=='get_odu16_peer_node_status_table':
                        sel_query="select odu.timeslot_index,(IFNULL((odu.sig_strength),0)),odu.timestamp,odu.ssidentifier,odu.link_status,odu.tunnel_status,odu.peer_mac_addr from  get_odu16_peer_node_status_table as odu INNER JOIN hosts as h  on  odu.host_id = h.host_id  where h.ip_address='%s' AND odu.timestamp >= '%s' AND odu.timestamp <='%s' and odu.timeslot_index IN (%s) and h.is_deleted=0 order by odu.timestamp desc"%(ip_address,start_time,end_time,column_field)
            cursor.execute(sel_query)
            ...
            for k in range(0,len(signal_strength)-1):
                ...
                if signal_strength[k][1]==1111111 or str(signal_strength[k][1])=='1111111' :
                    for i in range(len(rssi_dict)):
                        rssi_dict[i][rssi_dict[i].keys()[0]].append(default_value)
                    time_stamp_signal1.append(datetime.strptime(str(signal_strength[k][2])[:19],'%Y-%m-%d %H:%M:%S'))

                else:
                    if signal_strength[k][2]==signal_strength[k+1][2]:
                        default_rssi[int(signal_strength[k][0])]=int(signal_strength[k][1])
                    else:
                        default_rssi[int(signal_strength[k][0])]=int(signal_strength[k][1])
                        for i in range(len(rssi_dict)):
                            rssi_dict[i][rssi_dict[i].keys()[0]].append(default_rssi[rssi_dict[i].keys()[0]])
                        time_stamp_signal1.append(datetime.strptime(str(signal_strength[k][2])[:19],'%Y-%m-%d %H:%M:%S'))
            ...
            if len(signal_strength)>0 and flag==0 :
                if signal_strength[0][1]==1111111 or str(signal_strength[0][1])=='1111111':
                    for i in range(len(rssi_dict)):
                        rssi_dict[i][rssi_dict[i].keys()[0]].append(default_value)
                    time_stamp_signal1.append(datetime.strptime(str(signal_strength[0][2])[:19],'%Y-%m-%d %H:%M:%S'))
                else:
                        default_rssi[int(signal_strength[0][0])]=int(signal_strength[0][1])
                        for i in range(len(rssi_dict)):
                            rssi_dict[i][rssi_dict[i].keys()[0]].append(default_rssi[rssi_dict[i].keys()[0]])
                        time_stamp_signal1.append(datetime.strptime(str(signal_strength[0][2])[:19],'%Y-%m-%d %H:%M:%S'))
            ...
            if flag==1 and len(signal_strength)>0:
                if signal_strength[k+1][1]==1111111 or str(signal_strength[k+1][1])=='1111111':
                    for i in range(len(rssi_dict)):
                        rssi_dict[i][rssi_dict[i].keys()[0]].append(default_value)
                    time_stamp_signal1.append(datetime.strptime(str(signal_strength[k+1][2])[:19],'%Y-%m-%d %H:%M:%S'))
                else:
                    default_rssi[int(signal_strength[k+1][0])]=int(signal_strength[k+1][1])
                    for i in range(len(rssi_dict)):
                        rssi_dict[i][rssi_dict[i].keys()[0]].append(default_rssi[rssi_dict[i].keys()[0]])
                    time_stamp_signal1.append(datetime.strptime(str(signal_strength[k+1][2])[:19],'%Y-%m-%d %H:%M:%S'))
            ...
            if display_type.strip()=='graph':
                for time_index in range(len(time_stamp_signal1)):
                    time_stamp_signal1[time_index]=time.mktime(time_stamp_signal1[time_index].timetuple())*1000
		...
		if str(col).strip()!='':
            ...
            if status_name=='Master':
                ...
                for i in column_list:
                    signal_json_data.append({'name':['%s%s'%(link_name,'' if link_name=='Total Client' else i),qaulity],'data':rssi_dict[j][i]})
                    j+=1
            else:
		if len(rssi_dict)>0:
		    signal_json_data.append({'name':['Master-Link',qaulity],'data':rssi_dict[0][1]})
            db.close()    
            output_dict={'success':0,'time_stamp':time_stamp_signal1,'display_signal_strength':signal_json_data}
            return output_dict
        # Exception Handling
        except MySQLdb as e:
            output_dict={'success':1,'error_msg':'Error No : 102 '+str(err_obj.get_error_msg(102)),'main_msg':str(e[-1])}
    ...
    def sp_event_alarm(self,ip_address,device_type):
        try:
            ...
            if db ==1:
                raise SelfException(cursor)
            sql="SELECT ta.serevity,ta.event_id,ta.trap_event_id,ta.trap_event_type,hosts.device_type_id,ta.trap_receive_date,description FROM trap_alarms as ta inner join hosts on ta.agent_id=hosts.ip_address WHERE ta.agent_id='%s' order by ta.timestamp desc limit 7 "%ip_address
            cursor.execute(sql)
...
#            current_alarm=cursor.fetchall()
            ...
            output_dict={'success':0,'Event':all_traps,'Alarm':all_traps}
            return  output_dict
        # Exception Handling
        except MySQLdb as e:
            output_dict={'success':1,'error_msg':'Error No : 102 '+str(err_obj.get_error_msg(102)),'main_msg':str(e[-1])}
    ...
    def update_show_graph_table(self,device_type_id,user_id,show_graph):
        try:
            ...
            if show_graph.strip()!='':
                sql="UPDATE graph_templet_table SET is_disabled=1 WHERE user_id='%s' and device_type_id='%s' and dashboard_type=0"%(user_id,device_type_id)
                cursor.execute(sql)
                db.commit()
                sql1="UPDATE graph_templet_table SET is_disabled=0 WHERE user_id='%s' AND device_type_id='%s' AND graph_display_id IN ('%s') and dashboard_type=0"%(user_id,device_type_id,"','".join(graph_list))
                cursor.execute(sql1)
                db.commit()
    ...
    def sp_dashboard_get_graph_name(self,user_id,device_type,ip_address):
        try:
            ...
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            sql=" select graph_display_name,is_disabled,graph_display_id from graph_templet_table where device_type_id='%s' and user_id='%s' and dashboard_type=0"%(device_type,user_id)
            cursor.execute(sql)
            result=cursor.fetchall()
            selected_graph=[]
            non_selected_graph=[]
            ...
            db.close()
            result_dict={"success":"0","selected":selected_graph,"non_selected":non_selected_graph}
            return result_dict
        except MySQLdb as e:
            output_dict={'success':1,'error_msg':'Error No : 102'+str(err_obj.get_error_msg(102)),'main_msg':str(e[-1])}
            return output_dict
    ...
    def sp_ap_client_information(self,device_type_id,ip_address):
        try:
            db=MySQLdb.connect(*SystemConfig.get_mysql_credentials())
            cursor=db.cursor()
            sql="SELECT client_name,mac,rssi,vap_id,ap.total_tx,ap.total_rx,hosts1.host_alias,first_seen_time,hst.host_alias,last_seen_time,if(ap_connected_client.state='1','Yes','No') as state FROM ap_client_details as ap inner join (select host_id,host_alias,ip_address from hosts where is_deleted=0) as hosts1  on hosts1.host_id=ap.first_seen_ap_id inner join (select host_id,host_alias,ip_address from hosts where hosts.is_deleted=0) as hst on hst.host_id=ap.last_seen_ap_id inner join ap_client_ap_data as ap_data on ap.client_id=ap_data.client_id join (select client_id,state,host_id from ap_connected_client) as ap_connected_client on ap_connected_client.host_id=hst.host_id and ap_connected_client.client_id=ap.client_id and ap_connected_client.state='1' where hst.ip_address='%s'"%(ip_address)
            cursor.execute(sql)
            result=cursor.fetchall()

            sql="SELECT host_id FROM hosts where hosts.ip_address='%s' and hosts.device_type_id='%s' and is_deleted='0' limit 1"%(ip_address,device_type_id)
            cursor.execute(sql)
            host_id_result=cursor.fetchall()
            host_id=host_id_result[0][0]
            db.close()
            result_dict={"success":"0","result":result,'host_id':host_id}
            return result_dict
        except MySQLdb as e:
            output_dict={'success':1,'error_msg':'Error No : 102 '+str(err_obj.get_error_msg(102)),'main_msg':str(e[-1])}
            return output_dict
...
# Excel reporting start here.
    def sp_excel_report(self,device_type,user_id,ip_address,cal_list,tab_list,field_list,table_name_list,graph_name_list,start_date,end_date,select_option,limitFlag,graph_list,start_list,limit_list):
        if ip_address=='' or ip_address==None or ip_address=='undefined' or str(ip_address)=='None':
            raise SelfException('This UBR devices not exists so excel report can not be generated.')
        try:
	    display_type='excel'
            master_slave_status=get_master_slave_value(ip_address)
            ...
            device_name={'ap25':'AP25','RM18':'UBR','odu100':'RM','idu4':'IDU','ccu':'CCU'}
            ...
            login_user_name=''
            sel_query="SELECT host_alias FROM hosts WHERE ip_address='%s' and hosts.is_deleted=0"%(ip_address)
            cursor.execute(sel_query)
            uesr_information=cursor.fetchall()
            if len(uesr_information)>0:
                login_user_name=uesr_information[0][0]

            save_file_name=str(device_name[device_type])+'_'+str(login_user_name)+'_'+str(start_date)+'.xls'
            wifi=['wifi11g','wifi11gnHT20','wifi11gnHT40plus','wifi11gnHT40minus']
            radio=['disabled','enabled']
            device_detail=['Radio Status','Radio Channel','No Of VAPs','Software Version','Hardware Version','BootLoader Version','WiFi Mode','MAC Address','No Of Connected User']

            last_reboot_resion={0:'Power Cycle',1:'Watchdog Reset',2:'Normal',3:'Kernel Crash Reset',4:'Radio Count Mismatch Reset',5:'Unknown Soft',6:'Unknown Reset'}
            default_node_type={0:'rootRU',1:'t1TDN',2:'t2TDN',3:'t2TEN'}
            operation_state={0:'disabled',1:'enabled'}
            channel={0:'raBW5MHz',1:'raBW10MHz',2:'raBW20MHz',3:'raBW40MHz',4:'raBW40SGIMHz'}
            ...
            if device_type.strip()=='ap25':
                ...
                sql="SELECT count(*) FROM ap_connected_client inner join hosts on ap_connected_client.host_id=hosts.host_id  WHERE state='1' and hosts.ip_address='%s' and hosts.is_deleted=0 "%(ip_address)
                cursor.execute(sql)
                no_of_user=cursor.fetchall()
                if len(result)>0:
                    for i in result:
                        table_output.append([device_detail[0],'--' if result[0][0]==None or result[0][0]=='' else radio[int(result[0][0])]])
                        table_output.append([device_detail[1],'--' if result[0][1]==None or result[0][1]=='' else channel[int(result[0][1])-1]])
                        table_output.append([device_detail[2],'--' if result[0][2]==None or result[0][2]=='' else result[0][2]])
                        table_output.append([device_detail[3],'--' if result[0][3]==None or result[0][3]=='' else result[0][3]])
                        table_output.append([device_detail[4],'--' if result[0][4]==None or result[0][4]=='' else result[0][4]])
                        table_output.append([device_detail[5],'--' if result[0][5]==None or result[0][5]=='' else result[0][5]])
                        table_output.append([device_detail[6],'--' if result[0][6]==None or result[0][6]=='' else wifi[int(result[0][6])]])
                        table_output.append([device_detail[7],'--' if result[0][7]==None or result[0][7]=='' else result[0][7]])
                if len(no_of_user)>0:
                    table_output.append(['No of Connected User',0 if no_of_user[0][0]==None or no_of_user[0][0]=='' else no_of_user[0][0]])
		...
		device_field=['CCU Type','Serial Number','Hardware Version','Software Version','BootLoader Version','Last Reboot Reason','MAC Address','Protocol Version']
		if len(result)>0 and result!=None:
                    table_output.append([device_field[0],str('--' if result[0][0]==None or result[0][0]==""  else ccu_type[int(result[0][0])])])
                    table_output.append([device_field[1],str('--' if result[0][1]==None or result[0][1]==""  else result[0][1])])
                    table_output.append([device_field[2],str('--' if result[0][2]==None or result[0][2]==""  else result[0][2])])
                    table_output.append([device_field[3],str('--' if result[0][3]==None or result[0][3]==""  else result[0][3])])
                    table_output.append([device_field[4],str('--' if result[0][4]==None or result[0][4]==""  else result[0][4])])
                    table_output.append([device_field[5],str('--' if result[0][5]==None or result[0][5]==""  else result[0][5])])
                    table_output.append([device_field[6],str('--' if result[0][6]==None or result[0][6]==""  else result[0][6])])
                    table_output.append([device_field[7],str('--' if result[0][7]==None or result[0][7]==""  else result[0][7])])
            ...
            if device_type.strip()=='odu16' or device_type.strip()=='odu100':
                device_field=['Frequency','Time Slot','Active Version','Hardware Version','Last Reboot Reason','Channel','Operation State','Node Type','MAC Address','Last Reboot Time']
                if len(result)>0 and result!=None:
		    if device_postfix=='(Master)':
		    	master_slave_result=str('--' if result[1]==None or result[1]==""  else result[1])
		    else:
		    	master_slave_result='N/A'
                    table_output.append([device_field[0],str('--' if result[0]==None or result[0]==""  else result[0])])
                    table_output.append([device_field[1],str(master_slave_result)])
                    table_output.append([device_field[2],str('--' if result[2]==None or result[2]==""  else result[2])])
                    table_output.append([device_field[3],str('--' if result[3]==None or result[3]==""  else result[3])])
                    table_output.append([device_field[4],str('--' if result[4]==None or result[4]==""  else last_reboot_resion[int(result[4])])])
                    table_output.append([device_field[5],str('--' if result[5]==None or result[5]==""  or result[5]<0 else channel[int(result[5])])])
                    table_output.append([device_field[6],str('--' if result[6]==None or result[6]==""  else operation_state[int(result[6])])])
                    table_output.append([device_field[7],str('--' if result[7]==None or result[7]==""  else default_node_type[int(result[7])])])
                    table_output.append([device_field[8],str('--' if result[8]==None or result[8]==""  else result[8])])
		if len(last_reboot_time)>0:
                    table_output.append([device_field[9],'--' if last_reboot_time[0][0]==None or last_reboot_time[0][0]=='' else last_reboot_time[0][0]])
		else:
            ...
            if device_type.strip()=='idu4':
                ...
                second=0
                device_detail=''
                device_field=['H/W Serial Number','System MAC','TDMOIP Interface MAC','Active Version','Passive Version','BootLoader Version','Temperature(C)','Uptime']
                if len(result)>0 and result!=None:
                    if  result!=None and result[0][7] !=None:
                        hour=result[0][7]/3600
                        minute=(result[0][7]/60)%60
                        second=result[0][7]%60
                    table_output.append([device_field[0],str('--' if result[0][0]==None or result[0][0]==""  else result[0][0])])
                    table_output.append([device_field[1],str('--' if result[0][1]==None or result[0][1]==""  else result[0][1])])
                    table_output.append([device_field[2],str('--' if result[0][2]==None or result[0][2]==""  else result[0][2])])
                    table_output.append([device_field[3],str('--' if result[0][3]==None or result[0][3]==""  else result[0][3])])
                    table_output.append([device_field[4],str('--' if result[0][4]==None or result[0][4]==""  else result[0][4])])
                    table_output.append([device_field[5],str('--' if result[0][5]==None or result[0][5]==""  else result[0][5])])
                    table_output.append([device_field[6],str('--' if result[0][6]==None or result[0][6]==""  else result[0][6])])
                    table_output.append([device_field[7],(str(str(hour)+"Hr "+str(minute)+"Min "+str(second)+"Sec"))])

            
            # create the excel sheet for devcice information .
            xls_sheet=xls_book.add_sheet('Device Information',cell_overwrite_ok=True)
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
            xls_sheet.write_merge(0,0,0,2,"%s Information (%s--%s)"%(device_name[device_type],str(start_date),str(end_date)),style)
            xls_sheet.write_merge(1,1,0,2,device_name[device_type]+'       '+str(ip_address)+str(device_postfix) ,style)
	    ...
	    idu4_port={'0':'(odu)','1':'(eth0)','2':'(eth0)','3':'(eth1)','4':'(maxima)'}
...
'idu_e1PortStatusTable':{'1':'(port1)','2':'(port2)','3':'(port3)','4':'(port4)'},'idu_portstatisticsTable':idu4_port,'idu_swPrimaryPortStatisticsTable':idu4_port,'idu_portSecondaryStatisticsTable':idu4_port}
		...
		if graph_id.strip()=="odu100peernode":
		    ...
		    [interface_list.pop('odu100_peerNodeStatusTable') for ke in interface_list.keys() if ke.strip()=='odu100_peerNodeStatusTable']

                table_dic_key=[interface for interface in  interface_list.keys() if interface==table_name]
                interface_name='' if  len(table_dic_key)==0 else interface_list[table_dic_key[0].strip()][tab_list[i]] 
                output_result=self.common_graph_json(display_type,user_id,table_name,x_axis,index_name,graph_id,limitFlag,start_date,end_date,start,limit,ip_address,graph_list[i],update_field_name,tab_list[i],cal_list[i],field_column)
                d1_list=[]
                ...
                if     int(output_result['success'])==0:
                    ...
                    for data_list in output_result['data']:
                        headings.append(str(data_list['name'][0])+str(data_list['name'][1]))
                        d1_list.append(data_list['data'])
                    merge_result=merge_list(d1_list)

		if len(headings)<2:
		    continue
                xls_sheet=xls_book.add_sheet(str(graph_name_list[i]),cell_overwrite_ok=True)
                xls_sheet.row(0).height = 521
                xls_sheet.row(1).height = 421
                xls_sheet.write_merge(0,0,0,len(headings)-1,"%s%s (%s--%s)"%(str(graph_name_list[i]),interface_name,str(start_date),str(end_date)),style)
                xls_sheet.write_merge(1,1,0,len(headings)-1,device_name[device_type]+'   '+str(ip_address)+str(device_postfix)+'  Display Information -: (%s)'%str(cal_type) ,style)
                ...
                xls_sheet.set_panes_frozen(True)  # frozen headings instead of split panes
                xls_sheet.set_horz_split_pos(l)   # in general, freeze after last heading row
                xls_sheet.set_remove_splits(True) # if user does unfreeze, don't leave a split there
            ...
            if device_type=='ap25':
                merge_result=()
                client_result=self.sp_ap_client_information(device_type,ip_address)
                if client_result['success']=='0' or client_result['success']=='0':
                    merge_result=client_result['result']
                headings = ['Client Alias','MAC Address','RSL','VAP','Total Tx','Total Rx','First AP','First Seen Time','Last AP','Last Seen Time','IS Connected']
                xls_sheet=xls_book.add_sheet('Client Detail',cell_overwrite_ok=True)
                xls_sheet.row(0).height = 521
                xls_sheet.row(1).height = 421
                xls_sheet.write_merge(0,0,0,len(headings)-1,'Client Details (%s--%s)'%(start_date,end_date),style)
                xls_sheet.write_merge(1,1,0,len(headings)-1,device_name[device_type]+'       '+str(ip_address)+str(device_postfix) ,style)
                xls_sheet.write_merge(2,2,0,len(headings)-1,"")
                i=4
                heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
                for colx, value in enumerate(headings):
            ...
            sql="SELECT ta.serevity,ta.event_id,ta.trap_event_id,ta.trap_event_type,hosts.device_type_id,ta.trap_receive_date,description FROM trap_alarms as ta inner join hosts on ta.agent_id=hosts.ip_address WHERE ta.agent_id='%s' and ta.timestamp>='%s' and ta.timestamp<='%s' order by ta.timestamp desc"%(ip_address,start_date,end_date)
            ...
            for row in current_alarm:
                merge_result.append([row[5],serevity_list[int(row[0])],row[1],row[2],row[3],row[6]])
            headings = ['Received Date','Severity','Event Name','Event ID','Event Type','Description']
            xls_sheet=xls_book.add_sheet('Events',cell_overwrite_ok=True)
            xls_sheet.row(0).height = 521
            xls_sheet.row(1).height = 421
            xls_sheet.write_merge(0,0,0,len(headings)-1,'Device Events (%s--%s)'%(start_date,end_date),style)
            xls_sheet.write_merge(1,1,0,len(headings)-1,device_name[device_type]+'       '+str(ip_address)+str(device_postfix) ,style)
            xls_sheet.write_merge(2,2,0,len(headings)-1,"")
            i=4
            heading_xf = xlwt.easyxf('font: bold on; align: wrap on, vert centre, horiz center;pattern: pattern solid, fore_colour light_green;')
  ...
  #          # Event and Alarm report End Here. ---->
            xls_book.save('/omd/sites/%s/share/check_mk/web/htdocs/download/%s' %(nms_instance,save_file_name))
            output_dict={'success':0,'output':'report created successfully.','file_name':save_file_name}
            return output_dict
...
# CSV reporting start here.
    def sp_csv_report(self,device_type,user_id,ip_address,cal_list,tab_list,field_list,table_name_list,graph_name_list,start_date,end_date,select_option,limitFlag,graph_list,start_list,limit_list):
        if ip_address=='' or ip_address==None or ip_address=='undefined' or str(ip_address)=='None':
            raise SelfException('This devices not exists so excel report can not be generated.') 
        try:
            display_type='csv'
            master_slave_status=get_master_slave_value(ip_address)
            device_postfix='(Master)'
            ...
            device_name={'ap25':'AP25','odu16':'RM18','odu100':'RM','idu4':'IDU','ccu':'CCU'}
            ...
            if len(uesr_information)>0:
                login_user_name=uesr_information[0][0]
            save_file_name=str(device_name[device_type])+'_'+str(login_user_name)+'_'+str(start_date)+'.csv'
		...
		if len(result)>0 and result!=None:
            ...
            second_row=[str(device_name[device_type])+' Information',str(start_date),str(end_date)]
...
'idu_e1PortStatusTable':{'1':'(port1)','2':'(port2)','3':'(port3)','4':'(port4)'},'idu_portstatisticsTable':idu4_port,'idu_swPrimaryPortStatisticsTable':idu4_port,'idu_portSecondaryStatisticsTable':idu4_port}
		...
		if graph_id.strip()=="odu100peernode":
        	...
        	# if no column select so report not created.
                ...
                second_row=[device_name[device_type]+str(device_postfix),str(ip_address)+' Display Information -:'+str(cal_type)]
                blank_row=["","",""]
            ...
            if device_type=='ap25':
                ...
                second_row=[device_name[device_type]+str(device_postfix),str(ip_address)]
                blank_row=["","",""]
                writer.writerow(['------','--Break--','------'])
                writer.writerow(blank_row)
                writer.writerow(blank_row)
...
'odu100_peerNodeStatusTable':{'1':'(Link1)','2':'(Link2)','3':'(Link3)','4':'(Link4)','5':'(Link5)','6':'(Link6)','7':'(Link7)','8':'(Link8)','9':'(Link9)','16':'(Link16)'}}
            ...
            if len(uesr_information)>0:
                login_user_name=uesr_information[0][0]
            save_file_name=str(device_name[device_type])+'_'+str(login_user_name)+'_'+str(start_date)+'.pdf'
            ...
            PAGE_SIZE = A4
            nms_instance = __file__.split("/")[3]
            pdfdoc="/omd/sites/%s/share/check_mk/web/htdocs/download/%s"%(nms_instance,save_file_name)
            ...
            data=[]
            data.append([device_name[device_type]+str(device_postfix)+str(ip_address),str(start_date)+'--'+str(end_date)])
            ...
            if device_type.strip()=='ap25':
                ...
                sql="SELECT count(*) FROM ap_connected_client inner join hosts on ap_connected_client.host_id=hosts.host_id  WHERE state='1' and hosts.ip_address='%s'"%(ip_address)
                cursor.execute(sql)
                no_of_user=cursor.fetchall()
                if len(result)>0:
                    for i in result:
		...
		if len(last_reboot_time)>0:
            ...
            data1=[]
            data1.append(['','%s Device Information'%device_name[device_type],'',''])
            t = Table(data1, [.021*inch,2.51 * inch, 2.26 * inch, 2.22*inch])
		...
		if graph_id.strip()=="odu100peernode":
		    ...
		    [interface_list.pop('odu100_peerNodeStatusTable') for ke in interface_list.keys() if ke.strip()=='odu100_peerNodeStatusTable']
                ...
                table_dic_key=[interface for interface in  interface_list.keys() if interface==table_name]
                interface_name='' if  len(table_dic_key)==0 else interface_list[table_dic_key[0]][tab_list[i]] 

                output_result=self.common_graph_json(display_type,user_id,table_name,x_axis,index_name,graph_id,limitFlag,start_date,end_date,start,limit,
		...
		data1=[]
		data1.append(['','%s%s'%(graph_name_list[i],interface_name),'',''])
		t = Table(data1, [.021*inch,2.51 * inch, 2.26 * inch, 2.22*inch])
		t.setStyle(TableStyle([('BACKGROUND',(1,0),(1,0),(0.91,0.91,0.91)),
		('FONT', (0, 0), (1, 0), 'Helvetica', 11),('TEXTCOLOR',(1,0),(2,0),colors.black),
		('BOX',(1,0), (1, 0), .38,(0.29,0.29,0.29))]))

		# this is for permission to download folder
		import os
		os.system('chmod 777 /omd/sites/UNMP/share/check_mk/htdocs/web/download/* -R')
		# this is for permission to download 

		ubr_report.append(Spacer(11,11))
		im = Image("/omd/sites/%s/share/check_mk/web/htdocs/download/%s.png"%(nms_instance,graph_id_list[i]), width=7.1*inch, height=3*inch )
		im.hAlign = 'LEFT'
		ubr_report.append(im)
		ubr_report.append(Spacer(11,11))
		ubr_report.append(t)
		...
		if table_name.strip()=='outage':
		    table_output.insert(0,headings)
                    ...
                    t.setStyle(TableStyle([('FONT', (0, 0), (len(headings)-1, 0), 'Helvetica', 10),        
                    ('FONT', (0, 1), (len(headings)-1, int(len(table_output))-1), 'Helvetica', 9),
                    ('ALIGN', (1,0), (len(headings)-1, int(len(table_output))-1), 'CENTER'),
                    ('BACKGROUND',(0,0),(len(headings)-1,0),(0.91,0.91,0.91)),
                    ('LINEABOVE',(0,0), (len(headings)-1, 0), .29,(.29,.29,.29)),
                    ...
                    t.setStyle(TableStyle([('BACKGROUND',(0,0),(0,0),(0.91,0.91,0.91))]))
                    ubr_report.append(t)
                    if len(table_output)==1:
                        data1=[]
                        data1.append(['No data exists.'])
                        t = Table(data1, [7.10*inch])
                        t.setStyle(TableStyle([('BACKGROUND',(0,0),(1,0),(0.91,0.91,0.91)),
                        ('FONT', (0, 0), (0, 0), 'Helvetica', 9),('TEXTCOLOR',(0,0),(0,0),colors.black),
                        ('BOX',(0,0), (0, 0), .29,(0.56,0.56,0.56))]))
                        ubr_report.append(t)
		else:
                    table_output=conversion_to_pdf(table_output,headings)
                    data=table_output
                    t = Table(data,[2.5*inch, 1.15* inch, 1.15*inch,1.15*inch,1.15*inch])
                    t.setStyle(TableStyle([('FONT', (0, 0), (4, 0), 'Helvetica', 10),        
                    ('FONT', (0, 1), (4, int(len(table_output))-1), 'Helvetica', 9),
                    ('ALIGN', (1,0), (4, int(len(table_output))-1), 'CENTER'),
                    ('BACKGROUND',(0,0),(4,0),(0.91,0.91,0.91)),
                    ('LINEABOVE',(0,0), (4, 0), .29,(.29,.29,.29)),
                    ('GRID',(0,0),(8,int(len(table_output))-1),0.21,(0.74,0.74,0.74))]))
                    
                    for i in range(1,len(table_output)):
                        if i%2==1:
                            t.setStyle(TableStyle([('BACKGROUND',(1,i),(4,i),(0.95,0.95,0.95)),
                            ('BACKGROUND',(0,i-1),(0,i-1),(0.98,0.98,0.98)),
                               ]))
                        else:
            ...
            t.setStyle(TableStyle([('BACKGROUND',(0,0),(0,0),(0.91,0.91,0.91)),('FONT', (0, 0), (0, 0), 'Helvetica', 11),
            ('TEXTCOLOR',(0,0),(0,0),colors.black),('BOX',(0,0), (0, 0), .29,(0.56,0.56,0.56))]))
            ubr_report.append(t)

            pdf_doc.build(ubr_report)
            output_dict={'success':0,'output':'pdf downloaded','file_name':save_file_name}
            return output_dict
...
def get_master_slave_value(ip_address): 
    try:
        ...
        if conn ==1:
            raise SelfException(cursor)
        sel_query="SELECT device_type_id FROM hosts WHERE hosts.ip_address='%s' limit 1"%(ip_address)
        cursor.execute(sel_query)
        device_type=cursor.fetchall()
        status_result=()
	if len(device_type)>0 and  device_type[0][0]=='odu100':
	    sel_query="SELECT defaultNodeType FROM odu100_ruConfTable as def INNER JOIN hosts ON hosts.config_profile_id=def.config_profile_id WHERE hosts.ip_address='%s' and hosts.is_deleted=0  limit 1"%(ip_address)
	    cursor.execute(sel_query)
	    status_result=cursor.fetchall()
	elif len(device_type)>0 and  device_type[0][0]=='odu16':
	    sel_query="SELECT default_node_type FROM  get_odu16_ru_conf_table as def INNER JOIN hosts ON hosts.host_id=def.host_id WHERE hosts.ip_address='%s' and hosts.is_deleted=0  limit 1"%(ip_address)
	    cursor.execute(sel_query)
	    status_result=cursor.fetchall()
	elif len(device_type)>0 and  device_type[0][0]=='idu4':
	    sel_query="SELECT hwType FROM   idu_iduInfoTable as def INNER JOIN hosts ON hosts.host_id=def.host_id WHERE hosts.ip_address='%s' and hosts.is_deleted=0  limit 1"%(ip_address)
	    cursor.execute(sel_query)
	    status_result=cursor.fetchall()
        status=0
        if len(status_result)>0:
            if status_result[0][0]==None or status_result[0][0]=='':
            	...
            	if int(status_result[0][0])==0 or int(status_result[0][0])==2:
                    ...
                    status=1
	output_dict={'success':0,'status':status,'ip_address':ip_address}
        return output_dict 
    except Exception,e:
    ...
    try:
		...
		if is_date:
		    if uptime == None or downtime == None:
		        mid_date = datetime(main_date.year,main_date.month,main_date.day,23,59,59)
...
#                    day_diff=(temp_date-main_date).days
                    #day_diff = round(float(d.seconds + d.days * 86400)/float(86400))
                    day_diff = temp_date.day - main_date.day
                    if day_diff > 1:
                        for i in range(1,day_diff):
                            leftout_date=main_date+timedelta(days=i)
                            if prev_value == '50002':
                                main_list.append([leftout_date,tpl_temp[3],timedelta(0, 86399),None])
                            elif prev_value == '50001':
                                main_list.append([leftout_date,tpl_temp[3],None,timedelta(0, 86399)])
		        ...
		        if prev_value == '50002':
		            ...
		            if downtime == None:
                        ...
                        mid_date = datetime(temp_date.year,temp_date.month,temp_date.day,00,00,00)
    ...
    try:
        ...
        if 1==1:
            ...
            if status_result!=() and result!=():
                ...
                t_list=((status_result[0][0],status_result[0][1],t_date,status_result[0][3]),)
                result=t_list+result
            else:
	    	if len(result) < 1 and status_result!=():
	    	    #result = status_result
	    	    t_date=datetime.strptime(start_date ,"%Y-%m-%d %H:%M:%S")
	            t_date=t_date.replace(hour=0,minute=0,second=0)
	            t_list=((status_result[0][0],status_result[0][1],t_date,status_result[0][3]),)
	            result=t_list
            temp_res= main_outage(result,datetime.strptime(end_date[:18] ,"%Y-%m-%d %H:%M:%S"))
            if str(temp_res['success'])=="0":
                ...
                for i in li_res:
                    ...
                    if total!=0:
                        ...
                        i[-1]=int(round(float(downtime)*100/float(total)))
    ...
    if len(data_list)>0:
        for i in range(1,len(data_list[0])):
            ...
            for j in range(len(data_list)):
                temp_list.append(0 if data_list[j][i]=='DU' or data_list[j][i]=='--' else data_list[j][i])
            print temp_list
            new_list.append([heading[i],min(temp_list),max(temp_list),float(sum(temp_list)/len(data_list)),sum(temp_list)])
