...
#		graph_json.append(graph_dict)
            ...
            if str(graph_id).strip()=='apVAPUserConnect123':
                ...
                if len(column_name[0][0])>0:
                    ...
                    for name in column_name[0]:
                        ...
                        i+=1
            output_dic={'success':0,'timestamp':time_list,'data':json_data,'graph_title':graph_title,'graph_sub_title':graph_sub_title,'range_min':0,'range_max':0}
            return  output_dic
        except MySQLdb as e:
            output_dict={'success':1,'error_msg':'Error No : 102 '+str(err_obj.get_error_msg(102)),'main_msg':str(e[-1])}
            return output_dict
    ...
    def sp_signal_strength_graph(self,display_type,graph_id,ip_address,start_time,end_time,table_name,column_list):
        ...
        try:
            ...
            db.close()    
            output_dict={'success':0,'time_stamp':time_stamp_signal1,'display_signal_strength':signal_json_data,'range_min':min_value,'range_max':max_value}
            return output_dict
        # Exception Handling
        except MySQLdb as e:
            output_dict={'success':1,'error_msg':'Error No : 102 '+str(err_obj.get_error_msg(102)),'main_msg':str(e[-1])}
...
# Excel reporting start here.
    def sp_excel_report(self,device_type,user_id,ip_address,cal_list,tab_list,field_list,table_name_list,graph_name_list,start_date,end_date,select_option,limitFlag,graph_list,start_list,limit_list):
        ...
        try:
            display_type='excel'
            master_slave_status=get_master_slave_value(ip_address)
            ap_mode_dic={0:'standard',1:'rootAP',2:'repeater',3:'client',4:'multiAP',5:'multiVLAN',6:'dynamicVLAN'}
            device_postfix='(Master)'
            if master_slave_status['success']==0:
                if master_slave_status['device_type']=='ap25':
                    device_postfix=' Mode :- %s '%ap_mode_dic[master_slave_status['status']]
                else:
                    device_postfix='(Slave)' if master_slave_status['status']>0 else '(Master)'
            ...
            device_detail=['Radio Status','Radio Channel','No Of VAPs','Software Version','Hardware Version','BootLoader Version','WiFi Mode','MAC Address',
                           'No Of Connected User']

            last_reboot_resion={0:'Power Cycle',1:'Watchdog Reset',2:'Normal',3:'Kernel Crash Reset',4:'Radio Count Mismatch Reset',5:'Unknown Soft',
                                6:'Unknown Reset'}
            ...
            if device_type.strip()=='ap25':
                channel=['channel-01','channel-02','channel-03','channel-04','channel-05','channel-06','channel-07','channel-08',
                         'channel-09','channel-10','channel-11','channel-12','channel-13','channel-14']

                device_detail=['Radio Status','Radio Channel','No of VAPs','Software Version','Hardware Version','BootLoader Version',
			...
			where hosts.ip_address='%s' and hosts.is_deleted=0 and hosts.device_type_id='ccu' limit 1"%ip_address
                ...
                device_field=['CCU Type','Serial Number','Hardware Version','Active Software Version','Backup Software Version','BootLoader Version','Last Reboot Reason','MAC Address','Protocol Version']
                if len(result)>0 and result!=None:
                    ...
                    table_output.append([device_field[8],str('--' if result[0][8]==None or result[0][8]==""  else result[0][7])])
...
'idu_e1PortStatusTable':{'1':'(port1)','2':'(port2)','3':'(port3)','4':'(port4)'},'idu_portstatisticsTable':idu4_port,'idu_swPrimaryPortStatisticsTable':idu4_port,'idu_portSecondaryStatisticsTable':idu4_port}
            ...
            if device_type=='ap25':
                ...
                headings = ['Client Alias','MAC Address','RSL','VAP','Total Tx(Mbps)','Total Rx(Mbps)','First AP','First Seen Time','Last AP','Last Seen Time','Connected']
...
# CSV reporting start here.
    def sp_csv_report(self,device_type,user_id,ip_address,cal_list,tab_list,field_list,table_name_list,graph_name_list,start_date,end_date,select_option,limitFlag,graph_list,start_list,limit_list):
        ...
        try:
            ...
            wifi=['wifi11g','wifi11gnHT20','wifi11gnHT40plus','wifi11gnHT40minus']
            radio=['disabled','enabled']
            device_detail=['Radio Status','Radio Channel','No Of VAPs','Software Version','Hardware Version','BootLoader Version','WiFi Mode',
            ...
            if device_type.strip()=='odu16' or device_type.strip()=='odu100':
                device_field=['Frequency','Time Slot','Active Version','Hardware Version','Last Reboot Reason','Channel','Operation State',
            ...
            if device_type.strip()=='idu4':
                ...
                second=0
                device_detail=''
                device_field=['H/W Serial Number','System MAC','TDMOIP Interface MAC','Active Version','Passive Version','BootLoader Version',
...
'odu100_peerNodeStatusTable':{'1':'(Link1)','2':'(Link2)','3':'(Link3)','4':'(Link4)','5':'(Link5)','6':'(Link6)','7':'(Link7)','8':'(Link8)','9':'(Link9)','16':'(Link16)'}}
            ...
            last_reboot_resion={0:'Power cycle',1:'Watchdog reset',2:'Normal',3:'Kernel crash reset',4:'Radio count mismatch reset',
            ...
            im = Image("/omd/sites/%s/share/check_mk/web/htdocs/images/%s/logo.png"%(nms_instance,theme), width=1.5*inch, height=.5*inch )
            im.hAlign = 'LEFT'
            ...
            if device_type.strip()=='idu4':
                ...
                second=0
                device_detail=''
                device_field=['H/W Serial Number','System MAC','TDMOIP Interface MAC','Active Version','Passive Version',
        ...
        if len(status_result)>0:
            if status_result[0][0]==None or status_result[0][0]=='':
                ...
                if int(status_result[0][0])==0 or int(status_result[0][0])==2:
                    ...
                    status=1
        output_dict={'success':0,'status':status,'ip_address':ip_address,'device_type':device_type[0][0]}
        return output_dict 
    except Exception,e:
