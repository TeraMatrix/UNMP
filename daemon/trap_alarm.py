...
global file_name, trap_executed, mask_file, maskfile_stat, mask_alarm_dict, mask_severity_dict,clear_alarm_dict
...
def current_clear_daemon():
    ...
    try:
        ...
        for row_alarm in history_alarms:
            ...
            map_value = mask_alarm_dict.get(row_alarm[7])
            #logging.info(map_value)
    #        print map_value,row_alarm[8]            
            if map_value:
                ...
                if len(current_info) > 0:
                    if real_alarm_list.count(row_alarm[7]): #if real alarm 
                        if (row_alarm[6] < 2) and  (time.mktime(row_alarm[14].timetuple())  > time.mktime(current_info[0][14].timetuple())) and ((row_alarm[7]) != (current_info[0][7])) and ((current_info[0][8]) == (row_alarm[8])):   #future changeable  
                            cursor.execute("INSERT INTO trap_alarm_clear (event_id, trap_id, agent_id, trap_date, trap_receive_date, serevity, trap_event_id, trap_event_type, manage_obj_id, manage_obj_name, component_id, trap_ip, description) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"%(row_alarm[1],row_alarm[2],row_alarm[3],row_alarm[4],row_alarm[5],mask_severity_dict.get(map_value,row_alarm[6]),row_alarm[7],clear_alarm_dict(map_value,row_alarm[8]),row_alarm[9],row_alarm[10],row_alarm[11],row_alarm[12],row_alarm[13]))
                            db.commit()
                            ...
                            if (row_alarm[6] == current_info[0][6])  and  (time.mktime(time.strptime(row_alarm[5])))  > (time.mktime(time.strptime(current_info[0][5]))) and  ((row_alarm[7]) == (current_info[0][7])) and ((current_info[0][8]) == (row_alarm[8])):
                                ...
                                #cursor.execute(sql)
                                #db.commit()
                                sql="UPDATE trap_alarm_current set trap_id = '%s', agent_id = '%s', trap_date = '%s', trap_receive_date = '%s', serevity = '%s', trap_event_id = '%s', trap_event_type = '%s', manage_obj_id = '%s', manage_obj_name = '%s', component_id = '%s', trap_ip = '%s', description = '%s' WHERE event_id = '%s' "%(row_alarm[2],row_alarm[3],row_alarm[4],row_alarm[5],mask_severity_dict.get(row_alarm[7],row_alarm[6]),row_alarm[7],row_alarm[8],row_alarm[9],row_alarm[10],row_alarm[11],row_alarm[12],row_alarm[13],current_info[0][0])
                                cursor.execute(sql)
                                db.commit()
                    else:
                        sql="UPDATE trap_alarm_current set trap_id = '%s',agent_id = '%s',trap_date = '%s',trap_receive_date = '%s',serevity = '%s',trap_event_id = '%s',trap_event_type = '%s',manage_obj_id = '%s',manage_obj_name = '%s',component_id = '%s',trap_ip = '%s',description = '%s' WHERE event_id = '%s' "%(row_alarm[2],row_alarm[3],row_alarm[4],row_alarm[5],mask_severity_dict.get(row_alarm[7],row_alarm[6]),row_alarm[7],row_alarm[8],row_alarm[9],row_alarm[10],row_alarm[11],row_alarm[12],row_alarm[13],current_info[0][0])
                    ...
                    sql="INSERT INTO trap_alarm_current (event_id,trap_id,agent_id,trap_date,trap_receive_date,serevity,trap_event_id,trap_event_type,manage_obj_id,manage_obj_name,component_id,trap_ip,description) values ('%s','%s','%s','%s','%s',%s,'%s','%s','%s','%s','%s','%s','%s')"%(row_alarm[1],row_alarm[2],row_alarm[3],row_alarm[4],row_alarm[5],mask_severity_dict.get(row_alarm[7],row_alarm[6]),row_alarm[7],row_alarm[8],row_alarm[9],row_alarm[10],row_alarm[11],row_alarm[12],row_alarm[13])
                    cursor.execute(sql)
                    db.commit()
            else:
                key_li = [key for key, value in mask_alarm_dict.iteritems() if value == str(row_alarm[7])]
                if len(key_li) > 0:
                    sql="SELECT * FROM  trap_alarm_current WHERE trap_event_id='%s' and manage_obj_id='%s'and manage_obj_name='%s' and agent_id='%s'"% (key_li[0],row_alarm[9],row_alarm[10],row_alarm[3])
                    cursor.execute(sql)
                    current_info=cursor.fetchall()
                    if len(current_info) > 0:
                        cursor.execute("INSERT INTO trap_alarm_clear (event_id,trap_id,agent_id,trap_date,trap_receive_date,serevity,trap_event_id,trap_event_type,manage_obj_id,manage_obj_name,component_id,trap_ip,description) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"%(row_alarm[1],row_alarm[2],row_alarm[3],row_alarm[4],row_alarm[5],mask_severity_dict.get(row_alarm[7],row_alarm[6]),row_alarm[7],row_alarm[8],row_alarm[9],row_alarm[10],row_alarm[11],row_alarm[12],row_alarm[13]))
