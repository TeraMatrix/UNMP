...
global odu16_mask_alarm_dict, odu16_mask_severity_dict, odu16_clear_alarm_dict, odu16_real_alarm_list,rou_trap_type,mou_trap_type

rou_trap_type,mou_trap_type,odu16_mask_alarm_dict, odu16_mask_severity_dict, odu16_clear_alarm_dict, odu16_real_alarm_list = {}, {}, {}, {}, {}, []

file_name = "/omd/daemon/tmp/trap.time"

mask_file = "/omd/daemon/alarm_mask.rg"     # alarm mask file

event_type_file = "/omd/daemon/event_type_file.rg"
        ...
        if len(snmptt_trap_data) > 0:
            ...
            for row in snmptt_trap_data:
                try:
                    ...
                    device_date = 'null'
                    if row[1] == 'ccuTrap' or row[1] == 'mouTrap' or row[1] == 'rouTrap':
                        for_str = row[6].split('|')
                        try:
                            if len(for_str[8]) > 20:
                    ...
                    if row[1] == 'mouTrap':
                        for_str[2] = mou_trap_type.get(for_str[2],for_str[2])
                    if row[1] == 'rouTrap':
                        for_str[2] = rou_trap_type.get(for_str[2],for_str[2])
...
def current_clear_daemon():
    ...
    try:
        ...
        cursor=db.cursor()
        #trap_alarms_tp = ('alarm_id', 'event_id', 'trap_id', 'agent_id', 'trap_date', 'trap_received_date', 'severity', 'trap_event_id', 'trap_event_type', 'manage_obj_id', 'manage_obj_name', 'component_id', 'trap_ip', 'description', 'device_sent_date', 'timestamp')
        prev_event = ''
            ...
            try:
                ...
                if map_value:
                    ...
                    if len(current_info) > 0:
                        if temp_alarm_list.count(row_alarm[7]): #if real alarm 
