...
if [ "${Target}" = "Linux-x86_64" ]; then
	...
	cp -r ${CWD}/../../ThirdParty/64-bit ${WRK_DIR}/ThirdParty
	echo "${toolname}:msg:Copying 64-Bit Third Party RPMs"
	for file_name in  ${ThirdParty_64_bit_List}
	do
		cp -f ${WRK_DIR}/ThirdParty/${file_name} ${WRK_DIR}/ThirdParty_DB || report_error 1 ${file_name} ${WRK_DIR}/ThirdParty_DB
	done
else
	cp -r ${CWD}/../../ThirdParty/32-bit ${WRK_DIR}/ThirdParty
...
rm -rf ${CWD}/../config/skel/etc/apache/apache.conf || report_error 4 ${CWD}/../config/skel/etc/apache/apache.conf
...
zip -r  -q ${WRK_DIR}/ThirdParty_DB.zip ThirdParty_DB || report_error 3  ${WRK_DIR}/ThirdParty_DB ${WRK_DIR}/


echo "${toolname}:msg:Removing ThirdParty_DB folder "
rm -fr ${WRK_DIR}/ThirdParty_DB  || report_error 4 ${WRK_DIR}/ThirdParty_DB

echo "${toolname}:msg:Removing ThirdParty folder "
rm -fr ${WRK_DIR}/ThirdParty  || report_error 4 ${WRK_DIR}/ThirdParty
