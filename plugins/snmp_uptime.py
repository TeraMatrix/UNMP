...
def get(ip_address, port, community='public', version='2c'):
    try:

        # print ip_address, community, port, '--------'
        if isinstance(ip_address, str) and isinstance(community, str) and isinstance(port, int):
            if version == '1':
                args = [
                    cmdgen.CommunityData('v1snmp-agent', community, 0),
                    cmdgen.UdpTransportTarget((ip_address, port)),
                ]
            else:
                args = [
                    cmdgen.CommunityData('v2snmp-agent', community),
                    cmdgen.UdpTransportTarget((ip_address, port)),
                ]
            ...
            response_dict = {}
            try:
                errorIndication, errorStatus, errorIndex, varBinds = klass(
                    *args)
                ...
                if errorIndication:
                    ...
                    if errorStatus > 0 and errorIndex != None:
                        response_dict[name.prettyPrint()] = int(
                            errorStatus) if errorStatus.isdigit() else str(errorStatus)
                        return
                        ...
                        for name, val in varBinds:
                            ...
                            if val != '' and val != None:   # if needed put that and val != 'No Such Instance currently exists at this OID'
                                response_dict[name.prettyPrint(
                                )] = timetick_convert(str(val))
                                return
                            else:
    ...
    try:
        ...
        if arg.count('-i') and arg.count('-p'):
            ...
            if db == 1:
                ...
                cursor = db.cursor()
            sql = "SELECT host_id,snmp_read_community,snmp_version_id from hosts where ip_address = '%s' and is_deleted = 0" % ip_address
            if cursor.execute(sql) == 0 or cursor.execute(sql) == None:
                ...
                if len(host_data[0]) > 1:
                    ...
                    snmp_version = str(host_data[0][2])
                else:
                    plugin_message("host_id dosn't exists in hosts table")
                    exit_status = 1
                    return
            cursor.close()
            db.close()
            snmp_result_dict = get(
                ip_address, int(port_no), snmp_read, snmp_version)
            if snmp_result_dict['success'] == 0:
                ...
                # print " iii ", snmp_result_dict['result']
            ...
            if db == 1:
                ...
                # trap_event_id = 50001
                sel_query = "SELECT * FROM `trap_alarm_current` WHERE agent_id = '%s' and trap_event_id = 50001" % (
                    ip_address)
                cursor.execute(sel_query)
                current_info = cursor.fetchall()
                now_dt = datetime.now()

                # this is provide the last event_id of ip_address from system
                # alarm
                ...
                if snmp_flag:
                    ...
                    if (int(status) != 50002 or int(status) == 0) and host_id != 0:
                        ins_query = """INSERT INTO `system_alarm_table` (`system_alarm_id`, `host_id`,`event_id`, `trap_id`, `agent_id`, `trap_date`, `trap_receive_date`, `serevity`, `trap_event_id`, `trap_event_type`, `manage_obj_id`, `manage_obj_name`, `component_id`, `trap_ip`, `description`) VALUES (NULL,%s,'unmpSystemTrap', '.1.3.6.1.2.1.1.1.0','%s', '%s', '%s', 1, 50002, 'DEVICE_REACHABLE', 601, 'device', 701, '%s:%s', 'NOW response is OK')""" % (
                            host_id, ip_address, now_dt, datetime.strftime(now_dt, '%a %b %d %H:%M:%S %Y'), ip_address, port_no)
                        cursor.execute(ins_query)
                        db.commit()
