...
def main():
    try:
        ...
        if arg.count('-i') and  arg.count('-p') :
            ...
            cursor.close()
            db.close()            
            snmp_result_dict = get(ip_address,int(port_no),snmp_read,snmp_version)
            ...
            if db == 1:
                ...
                if snmp_flag:
                    ...
                    if len(current_info) > 0:
                        ...
                        db.commit()
                        ins_query = """INSERT INTO `trap_alarm_clear` (`trap_alarm_clear_id`, `event_id`, `trap_id`, `agent_id`, `trap_date`, `trap_receive_date`, `serevity`, `trap_event_id`, `trap_event_type`, `manage_obj_id`, `manage_obj_name`, `component_id`, `trap_ip`, `description`, device_sent_date) VALUES (NULL, 'unmpSystemTrap', '.1.3.6.1.2.1.1.1.0','%s', '%s', '%s', 0, 50002, 'DEVICE_REACHABLE', 601, 'device', 701, '%s:%s', 'NOW response is OK', '%s')"""%(ip_address,now_dt,datetime.strftime(now_dt,'%a %b %d %H:%M:%S %Y'),ip_address,port_no, str(now_dt))
                        cursor.execute(ins_query)
                        db.commit()
                        print " UPTIME  Trap Cleared Successfully "
                    
                    if (int(status)!=50002 or int(status)==0) and host_id!=0:
                        ins_query = """INSERT INTO `system_alarm_table` (`system_alarm_id`, `host_id`,`event_id`, `trap_id`, `agent_id`, `trap_date`, `trap_receive_date`, `serevity`, `trap_event_id`, `trap_event_type`, `manage_obj_id`, `manage_obj_name`, `component_id`, `trap_ip`, `description`) VALUES (NULL,%s,'unmpSystemTrap', '.1.3.6.1.2.1.1.1.0','%s', '%s', '%s', 1, 50002, 'DEVICE_REACHABLE', 601, 'device', 701, '%s:%s', 'NOW response is OK')"""%(host_id,ip_address,now_dt,datetime.strftime(now_dt,'%a %b %d %H:%M:%S %Y'),ip_address,port_no)
                        cursor.execute(ins_query)
                        db.commit()
