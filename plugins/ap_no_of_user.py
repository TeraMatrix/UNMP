...
def get_connected_user(ip, vap, auth_string):
    mac_list = []
    try:
        url = "http://%s/cgi-bin/ServerFuncs?Method=DisplayConnectedClients&VapId=%s" % (
            ip, vap)  # url for calling Access Point CGI command
        req = urllib2.Request(url)
        req.add_header("Authorization", "Basic %s" % auth_string)
        f = urllib2.urlopen(req)
        response = f.read()
        if response.find("No Connected Clients") == -1:
            # response ="<TABLE cellpadding='3' cellspacing='0'
            # border='1'><TR><TD align='center' class='header'>SlNo</TD><TD
            # align='center' class='header'>ADDR</TD><TD align='center'
            # class='header'>AID</TD><TD align='center'
            # class='header'>CHAN</TD><TD align='center'
            # class='header'>TxRATE</TD><TD align='center'
            # class='header'>RxRATE</TD><TD align='center'
            # class='header'>RSSI</TD><TD align='center'
            # class='header'>IDLE</TD><TD align='center'
            # class='header'>TXSEQ</TD><TD align='center'
            # class='header'>RXSEQ</TD><TD align='center'
            # class='header'>CAPS</TD></TR><TR><TD class='header'
            # align='center'>1</TD><TD align='center'
            # class='header'>20:7c:8f:2e:80:42</TD><TD align='center'
            # class='header'>1</TD><TD align='center' class='header'>6</TD><TD
            # align='center' class='header'>11M</TD><TD align='center'
            # class='header'>11M</TD><TD align='center'
            # class='header'>68</TD><TD align='center'
            # class='header'>120</TD><TD align='center'
            # class='header'>63016</TD><TD align='center'
            # class='header'>48480</TD><TD align='center'
            # class='header'>ESs</TD></TR><TR><TD class='header'
            # align='center'>1</TD><TD align='center'
            # class='header'>10:7c:8f:2e:80:42</TD><TD align='center'
            # class='header'>1</TD><TD align='center' class='header'>6</TD><TD
            # align='center' class='header'>11M</TD><TD align='center'
            # class='header'>11M</TD><TD align='center'
            # class='header'>68</TD><TD align='center'
            ...
            # class='header'>ESs</TD></TR><TR><TD class='header'
            # align='center'>1</TD><TD align='center'
            # class='header'>00:7c:8f:2e:80:42</TD><TD align='center'
            # class='header'>1</TD><TD align='center' class='header'>6</TD><TD
            # align='center' class='header'>11M</TD><TD align='center'
            # class='header'>11M</TD><TD align='center'
            # class='header'>68</TD><TD align='center'
            ...
            if ("-i" in arg) and ("-s" in arg) and (len(arg) == 5):
                ...
                if result == None:
                    ...
                    auth_string = base64.encodestring(
                        "%s:%s" % (device_user_name, device_password))

                    connected_user = 0
                    for v in range(VAP):
                        mac = get_connected_user(
                            ip_address, (v + 1), auth_string)
                        connected_user += len(mac)
                        for m in mac:
                            sql = "SELECT count(*) FROM nms_devices_connected_user WHERE deviceid = %s AND mac = '%s' AND vap = %s" % (
                                device_id, m, (v + 1))
                            cursor.execute(sql)
                            count = cursor.fetchone()[0]
                            if count == 0:
                                sql = "INSERT INTO nms_devices_connected_user(create_timestamp,deviceid,mac,vap) VALUES(now(),%s,'%s',%s);" % (
                                    device_id, m, (v + 1))
                            else:
                                sql = "UPDATE nms_devices_connected_user SET timestamp = now(), create_timestamp = create_timestamp WHERE deviceid = %s AND mac = '%s' AND vap = %s" % (
                                    device_id, m, (v + 1))
                            cursor.execute(sql)
                    db.commit()
