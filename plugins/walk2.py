...
try:
	...
	print str(e[-1])
	raise SelfCreatedException("pakcege Import Error ")
	sys.exit(2)
...
def defult_data_insert(table_name,ip_address):
	try:
		...
		execfile('/omd/sites/%s/share/check_mk/web/htdocs/mysql_exception.py' % site)
		...
		if table_name.strip()=='get_odu16_synch_statistics_table':
			ins_query="INSERT INTO `get_odu16_synch_statistics_table` (`host_id`, `index`, `sysc_lost_counter`, `timestamp`) VALUES('%s','1','0','%s')"%(result,datetime.now())	
...
# SNMP Walk Function
def snmp_walk(ip,port,oid,option,mib_path,table_name,ip_address):
	# arg=["snmpwalk","-Le","-Ln","-v",version,"-c",community,(ip+":"+port),oid,option]
	# SS=subprocess.Popen(arg, stdout=subprocess.PIPE).communicate()
	myCommand = "snmpwalk -Le -Ln -v 2c -c public " +ip+":"+port+" "+ oid + " " + option+" -m "+mib_path
	walk_result = commands.getstatusoutput(myCommand)
	if walk_result[0]!=0:
		if walk_result[-1] == None or walk_result[-1] == "":
			defult_data_insert(table_name,ip_address)
			raise SelfCreatedException("Host Unrechable / Device not connected to network. ")
			sys.exit(1)
		else:		 
			defult_data_insert(table_name,ip_address)
...
""" % (arg[0])
		...
		mib_path = arg[arg.index("-m") + 1]      # mib path
                site = __file__.split("/")[3]		# take site instance
                execfile('/omd/sites/%s/share/check_mk/web/htdocs/nms_config.py' % site)
                values,count_interface,total_index,split_result=snmp_walk(ip_address,port_no,oid_value,"-sQO",mib_path,table_name,ip_address)
		# select the host)id form hosts table
			...
			for index_val in range(total_index-1):
			    sub_part_results=((re.split("[.=]",split_result[nw]))[1]).strip()
			    sql+=",'%s'"% sub_part_results
                        for value  in  range((len(values))/count_interface):
                            sql+=",'%s'"% (values[nw+(value*count_interface)].replace('"','').split(':')[-1]).strip()
                        sql+=",'%s')"%(datetime.now())
