...
""" % (arg[0],arg[0]) 
            ...
            if ("-i" in arg) and ("-a" in arg) and ("-o" in arg) and (len(arg) == 7):
                ...
                alarm_event_option = arg[arg.index("-a") + 1]        # receive alarm type
                option=arg[arg.index("-o")+1]  # recive option for alarm and event ex all or pipe sepreated
                #site_name= __file__.split("/")[3]
                db = MySQLdb.connect("172.22.0.95","root","root","nms_sample" )
                cursor = db.cursor()         
                if alarm_event_option=="alarm" :
                    if option == "all":
                    #sql="Select trap_receive_date,trap_event_type,trap_event_id,manage_obj_name from trap_alarm_current where agent_id='%s' and alarm_event_type='%s'" %(ip_address,alarm)
...
# This is for event Notification 
                ...
                ip_address = arg[arg.index("-i") + 1]        # receive the ip address
                alarm_event_option = arg[arg.index("-e") + 1]        # receive alarm type
                option=arg[arg.index("-o")+1]  # recive option for alarm and event ex all or pipe sepreated
                #site_name= __file__.split("/")[3]
#		print "option"+str(option)
                ...
                if alarm_event_option=="event" :
                    if option == "all":
                    #sql="Select trap_receive_date,trap_event_type,trap_event_id,manage_obj_name from trap_alarm_current where agent_id='%s' and alarm_event_type='%s'" %(ip_address,alarm)
                        sql="Select trap_receive_date,trap_event_type,trap_event_id,manage_obj_name from trap_alarms where agent_id='%s' and timestamp BETWEEN  (now() - interval 1 minute) and now()  " %(ip_address)
                        cursor.execute(sql)
                        result=cursor.fetchall()
                        if result == None or result == "" or result == ():
