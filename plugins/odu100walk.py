...
def sshmain(ip_address):
    ...
    try:
        ...
        #myssh.put('myssh.py')
        myssh.run("a=$( ps | grep snmpagent | grep -v grep | awk '{print $1}');c=1;for i in $a;do if [ $c -gt 1 ];then kill $i;fi;c=`expr ${c} + 1`;done;")
...
def insert_db(result_dict,table_name):
    global db,host_id
    success = 1
                ...
                for table_name in table_dict:
                    ...
                    if snmp_result_dict['success'] == 0:    
                        ...
                        if len(result_dict) > 0:
                             insert_result = insert_db(result_dict,table_name)
                             if insert_result == 0:
                                 is_exception = 0
                        table_list.remove(table_name)
                        if is_exception:
                        ...
                        sshmain(ip_address)  # agent start
                        
                        snmp_flag = 1
                        return_value = 0
                        if not run_loop:
                            host_status(None,0,ip_address,9)
                            cgi_result = cgi_opener(ip_address,"admin","public",table_list)
                            if cgi_result['success'] == 0:
                                print " Additional Response OK "
                                for table_cgi in cgi_result['result']:
                                    cgi_flag = 1
                                    if len(cgi_result['result'][table_cgi]) > 0:
                                        cgi_flag = insert_db(cgi_result['result'][table_cgi],table_cgi)
                                         
                                    if cgi_flag:
                                        table_list.remove(table_cgi)
                                exit_status = 0
                            else:
                                print "  Addtional result : ",cgi_result['result']
                            if len(table_list) > 0:
                                print table_list[0]," remain: ",len(table_list)," cause: ",cgi_result['result']
                            ...
                            print cgi_result 
                            if cgi_result['success'] == 0 and cgi_result['result'].has_key(table_name):
                                if len(cgi_result['result'][table_name]) > 0:
                                    insert_db(cgi_result['result'][table_name],table_name)                                
                            else:
                                print name_table_dict[table_name],cgi_result['result']
