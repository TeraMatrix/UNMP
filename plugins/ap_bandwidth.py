...
""" % (arg[0])
			...
			if ("-i" in arg) and ("-s" in arg) and (len(arg) == 5):
				...
				# call the perl file which gives you interface, tx/rx values and uptime using snmp get
				pipe = subprocess.Popen(command, stdout=subprocess.PIPE).communicate()[0]
				command_result = pipe.split("\n")
				interface_name = []				# create a list variable to store interface name
				interface_tx = []				# create a list variable to store tx values
				interface_rx = []				# create a list variable to store rx values
				...
				if result == None:
					...
					if len(command_result) == 5:			# 
						...
						for i in range(len(interface_name)):
							sql = "INSERT INTO nms_devices_bandwidth(deviceid,interface,tx,rx) VALUES(%s,'%s',%s,%s)" %(device_id,interface_name[i],interface_tx[i],interface_rx[i])
							cursor.execute(sql)
					# insert uptime values in database
					sql = "SELECT COUNT(*) FROM nms_devices_uptime WHERE deviceid = %s" % (device_id)
					cursor.execute(sql)
					countDevices = cursor.fetchone()[0]
					if countDevices == 0:
						# Insert Uptime details when last details not exist
						sql = "INSERT INTO nms_devices_uptime(deviceid,uptime) VALUES(%s,'%s')" % (device_id,sys_uptime)
					else:
						# Update Uptime details when last details exist
						sql = "UPDATE nms_devices_uptime SET uptime = '%s' WHERE deviceid = %s" % (sys_uptime,device_id)
					cursor.execute(sql)
					db.commit()
					...
					if sys_uptime == STATE:
						plugin_message("Device Not Connected or SNMP not Enable")
						exit(1)
					else:
						exit(0)
				close_connection(db)
