...
import MySQLdb
# client_details=((1, 0, '20:7c:8f:2e:80:44', 1, 6, '11M', '11M', 9, 8,
# 500, 1500, '22877'),)
...
def add_new_client(client, ap_id, now):
    ...
    try:
        ...
        db.commit()
        sql = "UPDATE ap_client_details set client_name='client_%s' where client_id = '%s '" % (
            str(lastrow), str(lastrow))
        cursor.execute(sql)
        db.commit()
        cursor.close()
#		db.close()
...
#		db=db_connect()
        cursor = db.cursor()
        sql = "UPDATE ap_connected_client SET state='0' where client_id='%s' and client_mac='%s' " % (
...
def add_ap_client_ap_data(client_id, client_info, ap_id, now):
    ...
    try:
        ...
        # db=db_connect()
        cursor = db.cursor()
        sql = "INSERT INTO `ap_client_ap_data` (`ap_client_ap_data_id`, `host_id`, `client_id`, `client_mac`, `vap_id`, `slNum`, `aid`, `chan`, `txRate`, `rxRate`, `rssi`, `idle`, `total_tx`, `total_rx`, `caps`, `timestamp`) VALUES (NULL, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % (
            ap_id, client_id, client_info[mac_index], client_info[0], client_info[1], client_info[3], client_info[4], client_info[5], client_info[6], client_info[7], client_info[8], client_info[9], client_info[10], client_info[11], now)
        cursor.execute(sql)
        db.commit()
        # db.close()
...
def clear_clients(ap_id):
    ...
    try:
        ...
        # db=db_connect()
        cursor = db.cursor()
        sql = "UPDATE ap_connected_client SET state='0' where host_id='%s' " % (
            ap_id)
        cursor.execute(sql)
        db.commit()
        cursor.close()
...
def update_ap_connected_client(client_id, client_mac, ap_id, now):
    ...
    try:
        ...
        cursor.execute(sql)
        db.commit()
        sql = "UPDATE `ap_connected_client` set  state ='1' where host_id='%s' and client_id='%s' and client_mac='%s' " % (
            ap_id, client_id, client_mac)
        cursor.execute(sql)
        db.commit()
        cursor.close()
        # db.close()
...
def update_client(client_info, ap_id, now, client_id):
    ...
    try:
        ...
        # db=db_connect()
        cursor = db.cursor()
        sql = "SELECT * FROM ap_client_details where client_id = '%s' " % (
            client_id)
        cursor.execute(sql)
        ...
        if len(result) > 0:
            ...
            sql = "UPDATE ap_client_details set  total_tx=%s, total_rx=%s, last_seen_time='%s', last_seen_ap_id=%s where client_id = '%s'" % (
                new_tx, new_rx, now, ap_id, client_id)
            cursor.execute(sql)
            db.commit()
            cursor.close()
            # db.close()
...
def update_ap_client_ap_data(client_info, ap_id, now, client_id):
    ...
    try:
        ...
        # db=db_connect()
        cursor = db.cursor()
        sql = "SELECT * FROM ap_client_ap_data where client_id = '%s' and host_id='%s' " % (
            client_id, ap_id)
        cursor.execute(sql)
        result = cursor.fetchall()
        if len(result) > 0:
            ...
            sql = "UPDATE ap_client_ap_data set  vap_id='%s' , slNum='%s' , aid='%s' , chan='%s' , txRate='%s' , rxRate='%s' , rssi='%s' , idle='%s' , total_tx='%s' , total_rx='%s' , caps='%s' , timestamp ='%s' where client_id = '%s' and host_id='%s' " % (
                client_info[0], client_info[1], client_info[3], client_info[4], client_info[5], client_info[6], client_info[7], client_info[8], new_tx, new_rx, client_info[11], now, client_id, ap_id)
            cursor.execute(sql)
            db.commit()
            cursor.close()
            # db.close()
            ...
            cursor.close()
            # db.close()
            result_dict = add_ap_client_ap_data(
                client_id, client_info, ap_id, now)
            return result_dict
    except Exception, e:
        # print str(e)
        result_dict = {"success": 1, "result": str(e), "message": ""}
...
def is_new_client(client_mac):
    ...
    try:
        ...
        # db=db_connect()
        cursor = db.cursor()
        sql = "SELECT client_id FROM ap_client_details where mac = '%s' " % (
            client_mac)
        cursor.execute(sql)
        result = cursor.fetchall()
        cursor.close()
...
def is_new_ap_client(client_mac, ap_id):
    ...
    try:
        ...
        # db=db_connect()
        cursor = db.cursor()
        sql = "SELECT client_id FROM ap_connected_client where client_mac = '%s' and host_id='%s' " % (
            client_mac, ap_id)
        cursor.execute(sql)
        result = cursor.fetchall()
        cursor.close()
    ...
    if result_dict["success"] == 0:
        ...
        if client_id == -1:        # it doesn't exists ie new client
            ...
            if client_dict["success"] == 0:
                ...
                # print client_dict
                ap_connected_client_dict = add_ap_connected_client(
                    client_id, client_mac, ap_id, now)
                # print ap_connected_client_dict
                ap_client_ap_data = add_ap_client_ap_data(
                    client_id, client_info, ap_id, now)
            ...
            if new_ap_client_dict["success"] == 0:
                ...
                if new_ap_client != -1:  # it doesn't exists in this table ie new client for this table
                    update_ap_connected_client(
                        client_id, client_mac, ap_id, now)
                    update_ap_client_ap_data(
                        client_info, ap_id, now, client_id)
                else:
                    ap_connected_client_dict = add_ap_connected_client(
                        client_id, client_mac, ap_id, now)
                    # print ap_connected_client_dict
...
def defult_data_insert(table_name, ip_address, is_disable=1):
    try:
        ...
        # print " de ",table_name
        # if is_disable:
        # nwStatsIndex, rx_packets, tx_packets, rx_bytes, tx_bytes, rx_err,
        # tx_err, rx_drop, tx_drop, rx_multicast, colisions, rx_crc_err,
        # rx_phy_err, sig_strength, sync_lost = 1, 1111111, 1111111, 1111111,
        ...
        if table_name.strip() == 'ap25_vapClientStatisticsTable':
            ins_query = "INSERT INTO `ap25_vapClientStatisticsTable` (`ap25_vapClientStatisticsTable_id`, `host_id`, `vap_id`, `slNum`, `addressMAC`, `aid`, `chan`, `txRate`, `rxRate`, `rssi`, `idle`, `txSEQ`, `rxSEQ`, `caps`, `timestamp`) VALUES ( NULL, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s');" % (
                result, 1, 0, '', '1111111', '1111111', '1111111', '1111111', '1111111', '1111111', '1111111', '1111111', '1111111', timestamp)
            cursor.execute(ins_query)
            db.commit()
            clear_clients(result)
        elif table_name.strip() == 'ap25_statisticsTable':
            ins_query = "INSERT INTO `ap25_statisticsTable` (`ap25_systemInfo_id`, `host_id`, `index`, `statisticsInterface`, `statisticsRxPackets`, `statisticsTxPackets`, `statisticsRxError`, `statisticsTxError`, `timestamp`) VALUES ( NULL, '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s');" % (
                result, 0, 'eth0', '1111111', '1111111', '1111111', '1111111', timestamp)
            cursor.execute(ins_query)
            db.commit()
        exit_status = 1
        # print ins_query
    ...
    try:
        ...
        try:
            for table_name in temp_list:
                return_value = 0
                if table_name.strip() == 'ap25_vapClientStatisticsTable':
                    sel_query = "SELECT `rxSEQ`,`txSEQ` FROM ap25_vapClientStatisticsTable WHERE ap25_vapClientStatisticsTable_id = (SELECT max(ap25_vapClientStatisticsTable_id) FROM ap25_vapClientStatisticsTable WHERE host_id='%s')" % (
                        result)
                elif table_name.strip() == 'ap25_statisticsTable':
                    sel_query = "SELECT `statisticsRxPackets`,`statisticsTxPackets` FROM ap25_statisticsTable WHERE ap25_systemInfo_id = (SELECT max(ap25_systemInfo_id) FROM ap25_statisticsTable WHERE host_id='%s')" % (
                        result)
                if sel_query:
                    # print sel_query
...
def main():
    try:
        ...
        if arg.count('-i') and arg.count('-p'):
            ...
            # time.sleep(60)
            # device_type = arg[arg.index("-d") + 1]       # receive device
            # type
            snmp_flag = 0
            run_loop = 1
            is_exception = 1
            ...
            if obj_result['success'] == 0:
                ...
                if host_state == 0:
                    if is_db_connect:
                        ...
                        if db == 1:
                            ...
                            if cursor.execute(sql) == 0 or cursor.execute(sql) == None:
                                plugin_message(
                                    "host_id dosn't exists in hosts table")
                                exit_status = 1
                                return
                            else:
                    ...
                    for table_name in table_dict:
                        ...
                        snmp_result_dict = bulk_obj.bulkget(
                        ...
                        if snmp_result_dict['success'] == 0:
                            ...
                            if len(result_dict) > 0:
                                ...
                                ins_query = "INSERT INTO %s values " % (
                                    table_name)
                                flag = 1
                                ins_str = ""
                                ...
                                try:
                                    cursor.execute(
                                        ins_query)  # execute the query
                                    db.commit(
                                        )               # save the value in data base
                                    if table_name == "ap25_vapClientStatisticsTable":
                                        ap_walk_for_clients(
                                            host_id, result_dict, now)
                                except Exception, e:
                                    is_exception = 0
                            ...
                            if not run_loop:
                                ...
                                if return_value < 2:
                                    for table_name in table_list:
                                        exit_status = defult_data_insert(
                    ...
                    hstatus_dict = {0: 'No operation', 1: 'Firmware download', 2: 'Firmware upgrade', 3: 'Restore default config', 4: 'Flash commit', 5: 'Reboot', 6: 'Site survey', 7: 'Calculate BW', 8: 'Uptime service', 9: 'Statistics gathering', 10:
                        'Reconciliation', 11: 'Table reconciliation', 12: 'Set operation', 13: 'Live monitoring', 14: 'Status capturing', 15: 'Refreshing Site Survey'}
                    schedule_round = 3
