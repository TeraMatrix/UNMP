...
def _get_generator():
    # get rnd_iter.next(), or start ove
...
class BaseSession(dict):
    ...
    def __init__(self, req, sid=None, secret=None, lock=1,
        ...
        if self._sid:
            if not _check_sid(self._sid):
                if sid:
                    #log.info("session self sid raising")
        ...
        self.init_lock()
        #log.info(" session base init ...   ")
        if self._sid:
            # attempt to load ourselves
            #log.info(" session base before _sid lock ")
            #if not self._locked: #rg
            #log.info(" locking in self_sid "+str(self._locked)+" "+str())
            self.lock()
            #log.info(" session base after _sid lock")
            if self.load():
                self._new = 0
            #self.unlock()  # by rg
                
        #log.info(" session base after - befor is new")
        if self._new:
            #log.info(" session base _new and _sid now unlock ")
            # make a new session
        ...
        if random.randint(1, CLEANUP_CHANCE) == 1:
            self.cleanup()
        #log.info(" session base init END ")
    ...
    def lock(self):
        #log.info(" session base ININ init_lock ")
        
        if self._lock:
            if self._locked:
                #log.info(" session base ININ init_lock <<  LOCKED ")
                self.unlock()
            #log.info(" session base ININ init_lock 1 > ")
            try:
                pass #_apache._global_lock(self._req.server, self._sid)
            except Exception,e:
                pass #log.error(" session base lock exception "+str(e))
            finally:
                pass #log.error(" session base lock finally ")
            #log.info(" session base ININ init_lock 2 > ")
            self._locked = 1
            self._req.register_cleanup(unlock_session_cleanup, self)
            #log.info(" session base ININ init_lock 3 > ")

    def unlock(self):
        #log.info(" session base UNLOACKING <<< ")
...
class FileSession(BaseSession):
    ...
    def __init__(self, req, sid=0, secret=None, timeout=0, lock=1,
        ...
        #log.info(" sesssion init "+str(sid))
        BaseSession.__init__(self, req, sid=sid, secret=secret,
            timeout=timeout, lock=lock)
        #log.info(" sesssion in file ")
    def do_cleanup(self):
        data = {'req':self._req,
        ...
        #log.info(" sesssion in cleanup ")
        self._req.log_error("FileSession: registered filesession cleanup.",
                            apache.APLOG_NOTICE)

    def do_load(self):
        #log.info(" session IN do load")
        self.lock_file()
        try:
            try:
                path = os.path.join(self._sessdir, self._sid[0:2])
                filename = os.path.join(path, self._sid)
                #log.info(" session do load"+str(self._sid))
                fp = file(filename,'rb')
                #log.info(" session do load 212 : "+str(self._sid))
                try:
                    try:
                        data = cPickle.load(fp)
                    except Exception,e:
                        pass
                        #log.info(" session do load data except : "+str(e))                    
                    #log.info(" session do load data : "+str(data))
    ...
    def do_save(self, dict):
        ...
        try:
            try:
                path = os.path.join(self._sessdir, self._sid[0:2])
                #log.info(" session do save"+str(self._sid))
    ...
    def lock_file(self):
        ...
        if not self._locked:
            #log.critical(" session locking file "+str(self._req.server)+str(self._sid))
            #_apache._global_lock(self._req.server, self._sid)
            #log.critical(" session locking file ")
            self._locked = 1

    def unlock_file(self):
        if self._locked and not self._lock:
            #log.critical(" session UNlocking file ")
            #_apache._global_unlock(self._req.server, self._sid)
            self._locked = 0
